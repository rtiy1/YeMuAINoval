# Porting From legacy-agent-monorepo: A Practical Merge Guide

This guide is a repeatable checklist for porting changes from legacy-agent-monorepo into this repo.
...
Update this section after each sync; do not reuse the previous range. This commit is an upstream legacy-agent-monorepo marker and may not exist in this repo's local object database.

When starting a new sync, generate patches from this commit forward in a legacy-agent-monorepo checkout or remote that contains the commit:
...
- Examples (adjust to match the actual packages you are porting):
  - `@yemu/agent-runtime` → `@yemu/agent-runtime`
  - `@yemu/agent-core` → `@yemu/agent-core`
  - `@yemu/tui` → `@yemu/tui`
  - `@yemu/model-runtime` → `@yemu/model-runtime`
  - `@yemu/utils` → `@yemu/utils`
- Some upstream packages publish under the `@earendil-works/*` scope instead of `@mariozechner/*`. Map it the same way (`@yemu/agent-runtime` → `@yemu/agent-runtime`, and so on).
- The bare `typebox` package is not an `@yemu/*` scope; do not rewrite it as one. See the Extensions divergence in section 15 for how tool-parameter schemas map.
...
- In `packages/coding-agent`, use `logger` from `@yemu/utils` for internal/runtime logging; CLI command files may use `console.*` for intentional user-facing output.
...
- Prefer existing helpers and utilities over new ad-hoc code.
  ...
  - TypeScript packages generally use source files directly; `@yemu/native` exports generated native bindings from `packages/natives/native`.
...
- Replace legacy-agent-monorepo repo links where appropriate.
...
4. **Context/capabilities shrink** - Old APIs may have exposed `{ logger, typebox, yemu }` that new APIs forgot to include.
...
```
fix(coding-agent): backported legacy-agent-monorepo changes (<from>..<to>)
...
```
fix(coding-agent): backported legacy-agent-monorepo changes (9f3eef65f..52532c7c0)
...
| `ctx.ui.addAutocompleteProvider()`          | Wired in interactive mode; no-op stubs in ACP/RPC/headless contexts | Factory wrapping matches upstream; yemu's editor has no custom `triggerCharacters`, so wrapped providers surface at the built-in trigger points |
...
| Upstream                                           | Our Fork                                                  | Reason                                        |
| -------------------------------------------------- | --------------------------------------------------------- | --------------------------------------------- |
| `clipboard.ts` + `clipboard-image.ts` (tool files) | `src/utils/clipboard.ts` backed by `@yemu/native` | Native implementation with a small TS wrapper |
...
| `pkg.yemu` manifest field                                          | `pkg.yemu` preferred; fallback to `pkg.yemu` remains                                                  |
| `StringEnum` from `model-runtime`                                        | `Type.Enum` from the `yemu.typebox` shim (or author the schema with `yemu.zod`); `model-runtime` no longer exports `StringEnum` |
| `formatSize` from `agent-runtime`                              | `formatBytes` from `@yemu/utils`                                                            |
...
- `clipboard-image.ts` — image clipboard support is exposed through `src/utils/clipboard.ts` backed by `@yemu/native`
