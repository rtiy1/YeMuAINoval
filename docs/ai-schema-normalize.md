...
`@yemu/model-runtime` exposes one unified schema normalizer that providers consume
...
All exports live under `@yemu/model-runtime/utils/schema`:
...
- `adaptSchemaForStrict(schema, strict)` from `./adapt` — thin composer that
  ...
  flag (env `YEMU_NO_STRICT`) honored by every provider that emits `strict: true`.
