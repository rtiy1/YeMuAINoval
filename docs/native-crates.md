...
`@yemu/native` and the embedded shell/PTY runtime. They are intentionally
internal: end users see `@yemu/native` exports, not these crate APIs.
...
| Crate | Path | Role |
| --- | --- | --- |
| `yemu-native` | [`crates/yemu-native`](../crates/yemu-native) | Top-level N-API `cdylib`; aggregates the other crates and exposes the JS-visible API. |
| `yemu-shell` | [`crates/yemu-shell`](../crates/yemu-shell) | Embedded shell / PTY / process management split out of `yemu-native` (wraps `brush-*`). |
| `yemu-ast` | [`crates/yemu-ast`](../crates/yemu-ast) | tree-sitter-based code summarizer and AST utilities; 50+ language grammars. |
| `yemu-iso` | [`crates/yemu-iso`](../crates/yemu-iso) | Task isolation backend resolver: APFS clones, btrfs/zfs reflinks, overlayfs, projfs, rcopy. |
| `yemu-walker` | [`crates/yemu-walker`](../crates/yemu-walker) | Parallel filesystem walker (ignore + globset) shared by grep, glob, and fs-scan cache. |
| `yemu_uu_grep` | [`crates/yemu-uu-grep`](../crates/yemu-uu-grep) | `grep` re-implemented on `grep-regex` / `grep-searcher`; runs in-process as a shell builtin. Entry: `yemu_uu_grep::run`. |
| `yemu-uutils-ctx` | [`crates/yemu-uutils-ctx`](../crates/yemu-uutils-ctx) | Thread-local stdio + cwd context shim for embedding vendored uutils as in-process shell builtins. |
...
- Native API surface and loader (`@yemu/native`):
...
- Porting cross-references:
  [`porting-from-legacy-agent-monorepo.md`](./porting-from-legacy-agent-monorepo.md),
  [`porting-to-natives.md`](./porting-to-natives.md).
- Filesystem scan cache contract that consumes `yemu-walker`:
...
package (`@yemu/native`) and the architecture pages above. Promote a
