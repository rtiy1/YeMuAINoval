...
Each Python kernel is a single subprocess: `<resolved-python> -u <runner.py>`. The runner is bundled with the host binary (Bun text import), written to an `yemu-python-runner` cache under the OS temp directory once per script hash, and reused by subsequent spawns.
...
Status events the prelude emits (e.g. `_emit_status("find", count=…)`) ship inside display bundles under `application/x-yemu-status` so the existing TUI status renderer keeps working.
...
2. Managed venv at `~/.yemu/python-env`
...
`eval.py` / `eval.js` (both default `true`) plus optional boolean env flags `YEMU_PY` / `YEMU_JS` control eval backend exposure:

- Python backend only (`eval.py=true`, `eval.js=false`, or `YEMU_PY=1 YEMU_JS=0`)
- JavaScript backend only (`eval.py=false`, `eval.js=true`, or `YEMU_PY=0 YEMU_JS=1`)
- both backends (`eval.py=true`, `eval.js=true`, or `YEMU_PY=1 YEMU_JS=1`)

`YEMU_PY` and `YEMU_JS` use normal boolean flag parsing. Each flag, when set, overrides only its own setting; an unset flag falls back to its setting (`eval.py` / `eval.js`, both default `true`).
...
- `application/x-yemu-status` MIME inside `display` → structured status events
...
- `application/x-yemu-status` → status events
...
- **Python backend not available** — Check `eval.py`, `YEMU_PY`, and that `python`/`python3` is on PATH. If preflight fails and `eval.js` is enabled, use a `js` cell.
- **No Python on PATH** — Install a system Python 3.8+ or place a venv at `~/.yemu/python-env`. `yemu setup python --check` reports the resolved interpreter.
...
- `YEMU_PY` / `YEMU_JS` — eval backend exposure overrides
- `YEMU_PYTHON_SKIP_CHECK=1` — bypass Python preflight/warm checks
- `YEMU_PYTHON_INTEGRATION=1` — enable gated integration tests that spawn a real Python
- `YEMU_PYTHON_IPC_TRACE=1` — log NDJSON frames exchanged with the runner subprocess
