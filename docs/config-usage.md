...
┌───────────────────────────────────────┐
│ 1) ~/.yemu/agent, ~/.claude, ...       │
│ 2) <cwd>/.yemu, <cwd>/.claude, ...     │
└───────────────────────────────────────┘
                    │
                    ▼
        capability providers enumerate items
 (native provider scans project .yemu before user .yemu;
...
`CONFIG_DIR_NAME` is `.yemu` (`packages/utils/src/dirs.ts`).

## Profiles

A named profile (`yemu --profile <name>`, the `--alias` shortcut, or `YEMU_PROFILE` / `YEMU_LEGACY_PROFILE`) relocates the YeMu user base. When a profile is active, every YeMu-native user-level path written here as `~/.yemu/agent/...` resolves to `~/.yemu/profiles/<name>/agent/...` instead.

The relocation is uniform across the native provider (`builtin.ts`) and the generic `config.ts` helpers, so it covers slash commands, rules, prompts, instructions, hooks, tools, extensions, settings, skills, and MCP, plus the top-level `SYSTEM.md` / `RULES.md` / `AGENTS.md` files and runtime state (sessions, blobs, `agent.db`). A profile sees only its own YeMu config, never the default profile's `~/.yemu/agent`.

Keybindings are the one exception: a named profile merges the default profile's `~/.yemu/agent/keybindings.*` under its own `~/.yemu/profiles/<name>/agent/keybindings.*`, with the profile file overriding per binding ([#4867](upstream-change-4867)). Keybindings describe the terminal/keyboard in front of the user, which doesn't change with the active profile, so user-level remaps keep working in every profile unless the profile explicitly overrides them. The inherited file is read-only for the profile process — legacy-format migration of the default profile's file only happens when the default profile itself runs.

The other source bases are not profile-scoped and load identically under every profile: the external-tool bases (`~/.claude`, `~/.codex`, `~/.gemini`) belong to those tools, and the project-level bases (`<cwd>/.yemu`, `<cwd>/.claude`, ...) are keyed to the working directory. Throughout this document, read `~/.yemu/agent` as shorthand for the active profile's agent directory.

## Important constraint

The generic helpers in `src/config.ts` do **not** include `.yemu` in source discovery order.
...
Walks parent directories upward and returns the **nearest existing directory per source base** (`.yemu`, `.claude`, `.codex`, `.gemini`), then sorts results by source priority.
...
1. Global settings: `~/.yemu/agent/config.yml`
2. Project settings: discovered via settings capability (`settings.json` and `config.yml` from providers)
3. CLI config overlays: `yemu --config <path>` / repeated `--config` files, loaded as `config.yml`-style YAML for this process only
...
1. Migrate from `~/.yemu/agent/settings.json` (renamed to `.bak` on success)
...
## 6) Native `.yemu` provider behavior (`packages/coding-agent/src/discovery/builtin.ts`)
...
- Skills scan `<ancestor>/.yemu/skills` for each ancestor from the current working directory up to the repo root/home boundary, plus `~/.yemu/agent/skills`, without requiring the root `.yemu` directory itself to be non-empty.
- `SYSTEM.md` and `AGENTS.md` read user-level files directly and use nearest-ancestor project `.yemu` lookup for project files, but the project `.yemu` directory must be non-empty. See [`docs/system-prompt-customization.md`](./system-prompt-customization.md) for the full `SYSTEM.md` / `APPEND_SYSTEM.md` contract (replace vs. append, templating).

### Scope-specific loading

- Skills: `<ancestor>/.yemu/skills/*/SKILL.md` and `~/.yemu/agent/skills/*/SKILL.md`
...
## For `SYSTEM.md` and `AGENTS.md`, native provider uses nearest-ancestor project `.yemu` directory search (walk-up) and still requires the project `.yemu` dir to be non-empty.
...
```text
# ~/.yemu/agent/TITLE_SYSTEM.md
Generate a session name using lowercase `<type>:<primary-objective>`.
```

- Missing `TITLE_SYSTEM.md` keeps the bundled title prompts.
- Discovery uses the same project-then-user config directory pattern as `SYSTEM.md`: project `.yemu/TITLE_SYSTEM.md` first, then user `~/.yemu/agent/TITLE_SYSTEM.md` and the other supported config bases.
...
- Legacy-named toggles still exist (`skills.enableYemuUser`, `skills.enableYemuProject`) but they gate the native provider (`provider === "native"`).
...
- Legacy setting names `skills.enableYemuUser` / `skills.enableYemuProject` are still active gates for native skill source.
