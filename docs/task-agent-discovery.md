...
`discoverAgents(cwd, home)` (`src/task/discovery.ts`) merges agents from YeMu-native roots and Claude plugin roots before appending bundled definitions. Cross-harness roots such as `.claude/agents`, `.codex/agents`, and `.gemini/agents` are intentionally skipped — their frontmatter schema is not the YeMu task-agent contract (`TASK_AGENT_CONFIG_SOURCE = ".yemu"` filters both dir lists).

### Discovery inputs

1. Nearest project `.yemu` agents dir from `findAllNearestProjectConfigDirs("agents", cwd)` (filtered to `.yemu`; first hit only)
2. User `.yemu` agents dir from `getConfigDirs("agents", { project: false })` (filtered to `.yemu`; first hit only)
...
1. project `.yemu/agents`
2. user `~/.yemu/agent/agents`
...
- Project `.yemu` overrides user `.yemu`.
...
`YEMU_BLOCKED_AGENT` is read at tool construction. If request matches, execution is rejected with recursion-prevention message.
