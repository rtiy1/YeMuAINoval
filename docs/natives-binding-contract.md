...
- Rust modules under `crates/yemu-native/src/*.rs`
...
3. **Rust N-API exports** (`crates/yemu-native/src`)
...
Consumers in `packages/coding-agent` and `packages/tui` import directly from `@yemu/native`.
...
1. Rust `#[napi]` implementation in the owning `crates/yemu-native/src/<module>.rs`.
2. `crates/yemu-native/src/lib.rs` if a new module is added.
