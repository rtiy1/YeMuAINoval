# LSP configuration in YeMu

This guide explains how to configure language servers for the YeMu coding agent.
...
When no LSP config file is present, YeMu auto-detects servers by intersecting two conditions:
...
YeMu merges LSP config from multiple files, lowest to highest priority:

| Priority    | Location                                                                                                                    |
| ----------- | --------------------------------------------------------------------------------------------------------------------------- |
| 5 (lowest)  | `~/lsp.json`, `~/.lsp.json`, `~/lsp.yaml`, `~/.lsp.yaml`, `~/lsp.yml`, `~/.lsp.yml`                                         |
| 4           | Plugin LSP configs (marketplace / `--plugin-dir` roots)                                                                     |
| 3           | User config dirs: `~/.yemu/agent/lsp.*`, `~/.claude/lsp.*`, `~/.codex/lsp.*`, `~/.gemini/lsp.*`                              |
| 2           | Project config dirs: `<project>/.yemu/lsp.*`, `<project>/.claude/lsp.*`, `<project>/.codex/lsp.*`, `<project>/.gemini/lsp.*` |
...
- User-wide preferences → `~/.yemu/agent/lsp.json`
- Project-specific overrides → `<project>/.yemu/lsp.json`

> **Note:** Auto-detection is skipped only when at least one config file contributes server overrides. A config file that only sets `idleTimeoutMs` still lets YeMu auto-detect built-in servers. When server overrides exist, YeMu merges them with defaults and then loads servers that have matching `rootMarkers`, an available binary, and are not explicitly `disabled`.
...
The `capabilities` object enables optional server-specific features that YeMu supports on a per-server basis:
...
Place the override in `<project>/.yemu/lsp.json`:
...
The user-level config in `~/.yemu/agent/lsp.json` is unaffected; pylsp is only suppressed in this project.
