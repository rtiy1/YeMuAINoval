...
description: Use when creating a new yemu hook. Covers HookAPI, event catalog, blocking/overriding tool calls, and context modification.
---

# Authoring Hooks

Hooks are event-driven interceptors that run alongside the agent loop. They are best used for cross-cutting concerns: safety policy, secret redaction, context pruning, audit logging. A hook module registers handlers via `yemu.on(event, handler)` and can block tool execution, override tool output, or rewrite the message context before each LLM call.
...
export default function myHook(yemu: HookAPI): void {
...
export default function myExtension(yemu: ExtensionAPI): void {
  yemu.on("tool_call", async (event, ctx) => { /* ... */ });
}
```
...
```ts
yemu.on("tool_result", async (event, ctx) => {
...
```ts
yemu.on("context", async (event, ctx) => {
...
export default function rmRfBlocker(yemu: HookAPI): void {
  yemu.on("tool_call", async (event, ctx) => {
...
export default function apiKeyRedactor(yemu: HookAPI): void {
...
export default function contextFilter(yemu: HookAPI): void {
