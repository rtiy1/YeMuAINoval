...
An `yemu-agent` extension that demonstrates `tool_call` blocking. It intercepts `bash` tool calls and returns `{ block: true, reason: "..." }` when the command contains `rm -rf /` with normal whitespace, preventing the tool from executing.

## What it demonstrates

- `yemu.on("tool_call", ...)` — pre-execution interception
...
```
cp -r . ~/.yemu/agent/extensions/safety-hook
```

Restart `yemu`. The hook is active for all sessions.

Or load once:

```
yemu --extension ./safety-hook
```
