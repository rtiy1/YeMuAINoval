# Native Rust task execution and cancellation (`yemu-native`)

This document describes how `crates/yemu-native` schedules native work and how cancellation flows from JS options (`timeoutMs`, `AbortSignal`) into Rust execution.

## Implementation files

- `crates/yemu-native/src/task.rs`
- `crates/yemu-native/src/grep.rs`
- `crates/yemu-native/src/glob.rs`
- `crates/yemu-native/src/fd.rs`
- `crates/yemu-native/src/ast.rs`
- `crates/yemu-native/src/workspace.rs`
- `crates/yemu-native/src/shell.rs`
- `crates/yemu-native/src/pty.rs`
- `crates/yemu-native/src/html.rs`
- `crates/yemu-native/src/sixel.rs`
- `crates/yemu-native/src/clipboard.rs`
- `crates/yemu-native/src/text.rs`
...
| `Shell#run(options, onChunk?)`          | `Shell::run`                | `task::future(env, "shell.run", ...)`                          | JS `CancelToken` is converted into `yemu_shell::cancel::CancelToken`; shell races it against command completion and descendant cleanup |
