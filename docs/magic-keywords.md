...
Magic keywords are standalone words in a user prompt that add a hidden instruction for that turn. They are enabled by default and glow in the editor when `yemu` recognizes them.
...
yemu config set magicKeywords.enabled false

# Disable one keyword while leaving the others enabled
yemu config set magicKeywords.ultrathink false
yemu config set magicKeywords.orchestrate false
yemu config set magicKeywords.workflow false
```

All four settings default to `true`. Run `yemu config list` to inspect every available setting and its current value. See [Settings](./settings.md) for configuration scopes, precedence, and project-local overrides.
