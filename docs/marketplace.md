...
A **marketplace** is a Git repository (or local directory) containing a catalog file at `.yemu-plugin/marketplace.json` (preferred) or `.claude-plugin/marketplace.json` (Claude Code-compatible fallback). The catalog lists available plugins with their sources, descriptions, and metadata.

A **plugin** is a directory containing Claude/YeMu plugin content such as skills, commands, agents, hooks, tools, MCP servers, or LSP servers. Marketplace installs also load extension modules declared by `package.json` `yemu.extensions`: installation symlinks the cached plugin into the scope's `node_modules` tree and records it in `yemu-plugins.lock.json`, the same runtime surfaces used by npm-installed and `yemu plugin link`ed plugins. Plugins are identified by `name@marketplace` (e.g. `code-review@claude-plugins-official`).

**Scopes**: marketplace plugins can be installed at two scopes:

- **user** (default) -- available in all projects, stored in `~/.yemu/plugins/installed_plugins.json`
- **project** -- available only in the active project, stored in the nearest project `.yemu/plugins/installed_plugins.json`
...
```
yemu plugin marketplace add <source>
yemu plugin marketplace remove <name>
yemu plugin marketplace update [name]
yemu plugin marketplace list
yemu plugin discover [marketplace]
yemu plugin install [--force] [--scope user|project] name@marketplace
yemu plugin uninstall [--scope user|project] name@marketplace
yemu plugin upgrade [--scope user|project] [name@marketplace]
yemu plugin enable [--scope user|project] name@marketplace
yemu plugin disable [--scope user|project] name@marketplace
```
...
Git and local sources must contain a catalog at `.yemu-plugin/marketplace.json` (preferred) or `.claude-plugin/marketplace.json` (Claude Code-compatible fallback). Direct catalog URLs cache only the JSON catalog; plugins in URL-sourced catalogs cannot use relative string sources like `"./plugins/foo"`.

## Catalog format (marketplace.json)

A marketplace catalog lives at `.yemu-plugin/marketplace.json` in the repository root. When yemu is the only intended consumer, prefer this path. To remain Claude Code-compatible (yemu loads the same shape from either path), publish at `.claude-plugin/marketplace.json` instead — yemu uses it as a fallback when `.yemu-plugin/marketplace.json` is absent. A repository may ship both: yemu reads the `.yemu-plugin/` copy, Claude Code reads the `.claude-plugin/` copy. Same catalog format either way:
...
~/.yemu/
  ...
  plugins/
    ...
    yemu-plugins.lock.json         # Runtime enable/feature state
...
<project>/.yemu/
  plugins/
    ...
    yemu-plugins.lock.json         # Project runtime enable/feature state
    node_modules/<package>        # Symlink to the cached plugin
```
