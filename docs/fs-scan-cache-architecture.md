...
This document defines the current contract for the shared filesystem scan cache implemented in Rust (`crates/yemu-native/src/fs_cache.rs`) and consumed by native discovery/search APIs exposed to `packages/coding-agent`.
...
- Cache implementation and policy: `crates/yemu-native/src/fs_cache.rs`
- Native consumers:
  - `crates/yemu-native/src/glob.rs`
  - `crates/yemu-native/src/fd.rs` (`fuzzyFind`)
  - `crates/yemu-native/src/grep.rs` (cached directory mode only)
  - `crates/yemu-native/src/ast.rs` (`astGrep`/`astEdit` file discovery; always cached)
- JS binding/export:
