...
- JS/TS hook factories discovered through `hookCapability` (for example `.yemu/hooks/pre/*.ts`) are loaded as extension modules so their `yemu.on(...)` handlers bind to the runtime event bus
...
```ts
import type { HookAPI } from "@yemu/agent-runtime/extensibility/hooks";

export default function hook(yemu: HookAPI): void {
...
- register event handlers with `yemu.on(...)`
- send persistent custom messages with `yemu.sendMessage(...)`
- persist non-LLM state with `yemu.appendEntry(...)`
- register slash commands via `yemu.registerCommand(...)`
- register custom message renderers via `yemu.registerMessageRenderer(...)`
- run shell commands via `yemu.exec(...)`
- author schemas/helpers with injected `yemu.zod`, `yemu.typebox`, and agent exports via `yemu.runtime`
...
export default function (yemu: HookAPI): void {
  yemu.on("tool_call", async (event, ctx) => {
...
`src/extensibility/hooks/index.ts` and the package subpath `@yemu/agent-runtime/extensibility/hooks` export:
...
The package root (`@yemu/agent-runtime`) does not re-export `HookAPI`; import legacy hook types from the hooks subpath.
