...
The SDK is the in-process integration surface for `@yemu/agent-runtime`.
...
```bash
bun add @yemu/agent-runtime
```

## Entry points

`@yemu/agent-runtime` exports the SDK APIs from the package root (and also via `@yemu/agent-runtime/sdk`).
...
```ts
import { createAgentSession } from "@yemu/agent-runtime";
...
- `cwd`: `getProjectDir()`
- `agentDir`: `~/.yemu/agent` (via `getAgentDir()`)
...
```ts
import { SessionManager } from "@yemu/agent-runtime";
