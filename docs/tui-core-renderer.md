...
- `shouldEnableSynchronizedOutputByDefault(env, id)` → DEC 2026 default.
  Precedence: user opt-out (`YEMU_NO_SYNC_OUTPUT`/`YEMU_TUI_SYNC_OUTPUT=0`) → user
  force-on (`YEMU_FORCE_SYNC_OUTPUT=1`/`YEMU_TUI_SYNC_OUTPUT=1`) → `TERM_FEATURES`
...
(`@yemu/native`, Rust `unicode-width`); `visibleWidth` measures with
...
default-on only for kitty/ghostty (`YEMU_NO_KITTY_PLACEHOLDERS` /
`YEMU_KITTY_PLACEHOLDERS`).
...
| Var | Effect |
|---|---|
| `YEMU_NO_SYNC_OUTPUT=1` | Disable DEC 2026 BSU/ESU wrappers (autowrap discipline stays on). |
| `YEMU_TUI_SYNC_OUTPUT=0\|1` / `YEMU_FORCE_SYNC_OUTPUT=1` | Force sync output off / on. |
| `YEMU_NO_DECCARA` | Disable Kitty DECCARA rectangular-fill optimization. |
| `YEMU_FORCE_IMAGE_PROTOCOL=kitty\|iterm2\|sixel\|off` | Override image protocol detection. |
| `YEMU_NO_KITTY_PLACEHOLDERS=1` / `YEMU_KITTY_PLACEHOLDERS=1` | Force Kitty Unicode placeholders off / on. |
| `YEMU_HARDWARE_CURSOR=1` | Show the real hardware cursor instead of a rendered one. |
| `YEMU_NOTIFICATIONS=off\|0\|false` | Suppress terminal notifications. |
| `YEMU_DEBUG_REDRAW=1` | Log the chosen render intent + ledger state per frame to the debug log. |
| `YEMU_TUI_RESIZE_IN_PLACE=1\|0` | Force resize to repaint in place (no alt-screen borrow, no ED3 rewrap) on / off. Default-on for terminals that re-report size on alt-screen toggles (Warp). |

Removed with the old engine: `YEMU_TUI_ED3_SAFE` (no ED3-risk lever exists),
`YEMU_CLEAR_ON_SHRINK` (shrinks always clear exactly), `YEMU_TUI_DEBUG` (per-render
dump superseded by `YEMU_DEBUG_REDRAW` ledger logging and the stress harness
