...
```text
Config sources (.yemu/.claude/.cursor/.vscode/mcp.json, mcp.json, etc.)
...
In practice MCP servers also come from higher-priority providers (for example native `.yemu/...` and tool-specific config dirs). Authoring guidance:

- Prefer `.yemu/mcp.json` (project) or `~/.yemu/agent/mcp.json` (user) for explicit control.
...
YeMu-native MCP config (`.yemu/mcp.json`, `~/.yemu/agent/mcp.json`, plus their `.mcp.json` variants) expands `${VAR}` and `${VAR:-default}` placeholders recursively before converting to runtime config. It also accepts boolean/string forms for `enabled` (`true`, `false`, `1`, `0`) and numeric strings for `timeout`.
