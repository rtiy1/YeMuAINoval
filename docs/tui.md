...
If your extension/tool can run in non-interactive mode, guard with `ctx.hasUI` / `yemu.hasUI`.

## Core component contract (`@yemu/tui`)
...
Custom tools typically use the same UI entrypoint via the factory-scoped `yemu.ui` object, then return the selected value in normal tool content:
...
async execute(toolCallId, params, onUpdate, ctx, signal) {
  ...
  const picked = await yemu.ui.custom<string | undefined>((tui, theme, done) => {
...
export default function extension(yemu: ExtensionAPI): void {
  yemu.registerCommand("pick-model", {
