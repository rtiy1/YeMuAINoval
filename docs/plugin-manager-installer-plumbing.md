...
This document describes how `yemu plugin` npm/git/link and marketplace operations mutate plugin state on disk and become runtime capabilities. Marketplace installs keep their own registries and cache, then register the cached plugin through the same `node_modules` and `yemu-plugins.lock.json` runtime surfaces used by npm/git/link installs; see `docs/marketplace.md`.
...
```text
yemu plugin <npm/link action> ...
  -> src/commands/plugin.ts
  -> runPluginCommand(...) in src/cli/plugin-cli.ts
  -> PluginManager method (install/list/uninstall/link/...)
  -> mutate ~/.yemu/plugins/{package.json,node_modules,yemu-plugins.lock.json}
...
yemu plugin install name@marketplace / yemu install name@marketplace
  -> MarketplaceManager
  -> mutate marketplace registries and cache
  -> symlink the cached package into the scope's node_modules and update yemu-plugins.lock.json
...
Global plugin state lives under `~/.yemu/plugins`:
...
- `yemu-plugins.lock.json` — runtime state for npm/link/marketplace plugins:
...
- `<cwd>/.yemu/plugin-overrides.json`
...
- `~/.yemu/marketplaces.json` — configured marketplace catalogs
- `~/.yemu/plugins/installed_plugins.json` — user-scoped marketplace installs
- `<cwd>/.yemu/plugins/installed_plugins.json` — project-scoped marketplace installs when available
- `~/.yemu/plugins/cache/{marketplaces,plugins}/` — cached catalogs and plugin directories
- `<scope>/plugins/node_modules/<package>` — symlink to the cached plugin, allowing its `package.json` `yemu.extensions` and tools to load
- `<scope>/plugins/yemu-plugins.lock.json` — enablement and feature state shared with the runtime plugin loader
...
1. `package.json.yemu`
2. fallback `package.json.yemu`
3. fallback `{ version: package.version }`

Implications:

- There is no strict schema validation in manager/loader.
- A package missing `yemu`/`yemu` is still installable and listable.
- Runtime plugin loading (`getEnabledPlugins`) skips packages without `yemu`/`yemu` manifest.
...
3. Ensure plugin `package.json` exists (`yemu-plugins`, private dependencies map).
4. Run `bun install <packageSpec>` in `~/.yemu/plugins`.
...
- `yemu plugin install pkg@newVersion` updates dependency and lockfile version.
...
1. Read plugin dependency map from `~/.yemu/plugins/package.json`.
2. Load lockfile runtime config (missing file -> empty defaults).
3. Load project overrides (`<cwd>/.yemu/plugin-overrides.json`, parse/read errors -> empty object with warning).
...
`link` supports local plugin development by symlinking a local package into `~/.yemu/plugins/node_modules/<pkg.name>`.
...
- skip if manifest (`yemu`/`yemu`) absent
