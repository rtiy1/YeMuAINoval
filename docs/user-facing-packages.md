...
- **Include** root docs coverage for package-local CLIs, extension features, dashboards, and benchmark runners that users can run directly or through `yemu`.
...
- Package: `@yemu/swarm-extension`; bin: `yemu-swarm`.
- Feature: multi-agent DAG orchestration from YAML swarms, supporting `pipeline`, `parallel`, and `sequential` modes.
- Standalone CLI: `yemu-swarm path/to/swarm.yaml` runs until completion or process termination.
...
Sources: [`packages/terminal-bench/README.md`](../packages/terminal-bench/README.md), [`packages/terminal-bench/package.json`](../packages/terminal-bench/package.json), [`packages/terminal-bench/src/runner.ts`](../packages/terminal-bench/src/runner.ts), [`packages/terminal-bench/agent/yemu_local.py`](../packages/terminal-bench/agent/yemu_local.py).

- Package: private `@yemu/terminal-bench`; bin: `tb2`.
- Feature: runs `harbor-framework/terminal-bench-2` against a local or published `yemu` build with a live progress, spend, token, ETA, and pass/fail dashboard.
- CLI: `bun src/runner.ts [options] [-- <extra harbor args>]`; package bin exposes `tb2`.
- Modes: default `yemu` agent, `oracle`/`nop`/any Harbor agent via `--agent`; local source packing by default, published npm install via `--install published`; `cleanup` command removes leftover Harbor Docker resources.
...
- Package: `@yemu/stats`; bin: `yemu-stats`; main user path: `yemu stats`.
- Feature: local observability dashboard for AI usage statistics from session JSONL logs.
- CLI modes: `yemu stats` starts the dashboard server, opens `http://localhost:3847`, and keeps running; `yemu stats --port <port>` changes the port; `yemu stats --summary` prints a console summary; `yemu stats --json` prints JSON and exits.
- Programmatic API: exports helpers such as `syncAllSessions()` and `getDashboardStats()` for embedding.
- Inputs/storage: reads `~/.yemu/agent/sessions/`; stores aggregates in `~/.yemu/stats.db`.
...
- Package: private `@yemu/typescript-edit-benchmark`; bin: `typescript-edit-benchmark`.
