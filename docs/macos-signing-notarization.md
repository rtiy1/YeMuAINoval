...
The compiled macOS `yemu` binaries shipped on GitHub Releases are signed with a
...
submission (see [#776](upstream-change-776)).
...
- `curl https://github.com/rtiy1/YeMuAINoval/install | sh` — `curl` sets no quarantine bit, so
...
Drop these into a working directory (default `~/yemu-signing`):
...
```sh
scripts/ci-macos-upload-secrets.sh ~/yemu-signing --dry-run   # validate first
scripts/ci-macos-upload-secrets.sh ~/yemu-signing             # upload all five
gh secret list --repo rtiy1/YeMuAINoval                       # confirm
```
...
APPLE_API_KEY_ID=… APPLE_API_ISSUER_ID=… APPLE_API_KEY=… \
  bash scripts/ci-macos-sign.sh packages/coding-agent/binaries/yemu-darwin-arm64
```
