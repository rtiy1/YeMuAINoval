...
session-title generation (`providers.tinyModel`), YeMuMemory memory extraction/consolidation
...
- **Device policy**: local tiny models default to CPU-only inference and retry once on CPU if an
  ...
  - Pick a provider persistently with the `providers.tinyModelDevice` setting (`default` keeps CPU),
    or per-run with the `YEMU_TINY_DEVICE` env var (which overrides the setting).
  - Accepted values are `cpu`, `gpu`, `metal`/`webgpu`, `auto`, `cuda`, `dml`, `coreml`, `wasm`,
    `webnn`, `webnn-gpu`, `webnn-cpu`, and `webnn-npu`.
  - Direct `coreml` remains opt-in via `YEMU_TINY_DEVICE=coreml`; it is not part of the default because
  ...
  - Use `providers.tinyModelDevice` or `YEMU_TINY_DEVICE` only when explicitly opting out of the CPU
    default.
- **Quantization: q4 is the sweet spot** — smaller on disk, faster to load, and fast at inference.
  ...
  for higher fidelity), or per-run with `YEMU_TINY_DTYPE` (which overrides the setting). Accepts `auto`,
...
## Task 2: YeMuMemory memory (`providers.memoryModel`)

YeMuMemory runs two small-LLM tasks:
...
2. Capable models emit **JSON objects inside arrays**, which YeMuMemory's `String(item)` coerces into
   the literal string `[object Object]`.

The robust fix is a **one-item-per-line output format** (consumed by YeMuMemory's parser line-fallback)
...
### Known YeMuMemory parser bugs (surfaced by these experiments)
...
- The memory local path applies the refined recipes (line-format + small-talk-guarded extraction
  prompt, hardened consolidation prompt) via YeMuMemory prompt overrides; the **online path is
  unchanged**.
