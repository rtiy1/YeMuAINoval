...
2. For each cell, `execute()` maps `cell.language` to an `EvalLanguage` (`"py"` → `"python"`, `"js"` → `"js"`) and calls `resolveBackend(session, language)`:
   - `python` is gated on `resolveEvalBackends(session).python` (the `eval.py` setting, overridden by the `YEMU_PY` env flag) and `pythonBackend.isAvailable(session)`.
   - `js` is gated on `resolveEvalBackends(session).js` (the `eval.js` setting, overridden by the `YEMU_JS` env flag).
...
- Kernel `display` / `result` frames map to:
  - `application/x-yemu-status` → status event
...
- Disabled/unavailable backends throw `ToolError` from `resolveBackend()`:
  - `eval.py = false` (or `YEMU_PY=0`) and a `py` cell is requested
  - `eval.js = false` (or `YEMU_JS=0`) and a `js` cell is requested
...
- Python helper `output(...)` depends on `YEMU_ARTIFACTS_DIR` or `YEMU_SESSION_FILE`; it fails outside a session-backed run.
