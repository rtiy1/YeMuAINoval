...
- YeMuMemory collaborators:
  - `packages/coding-agent/src/yemu-memory/state.ts` — scoped local recall and context formatting.
...
| `context` | `string` | No | Extra guidance. Hindsight sends it as `context`; YeMuMemory appends trimmed context to the recall query under `Additional context:`. |
...
## Flow
1. `MemoryReflectTool.createIf(...)` exposes the tool when `memory.backend` is either `"hindsight"` or `"yemu-memory"`.
...
- YeMuMemory tool path: one local scoped recall followed by context formatting.
...
- Network
  ...
  - YeMuMemory: none unless configured embedding or LLM providers are used by the local runtime during recall.
- Session state
...
- YeMuMemory result count is capped by `yemu-memory.recallLimit`, default `8`.
...
- YeMuMemory recall target failures inside `collectScopedRecallResults(...)` are caught per bank and logged only when `yemu-memory.debug` is enabled; if all targets fail, the tool can return the no-information text.
...
- YeMuMemory `reflect` is local recall plus formatting, so its output shape differs from Hindsight's remote synthesized answer.
