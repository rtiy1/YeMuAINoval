...
- YeMuMemory collaborators:
  - `packages/coding-agent/src/yemu-memory/backend.ts` — local backend bootstrap, prompt injection, subagent aliasing, enqueue/clear.
  - `packages/coding-agent/src/yemu-memory/state.ts` — scoped recall/retain state and local writes.
  - `packages/coding-agent/src/yemu-memory/config.ts` — local SQLite path, bank, scoping, provider settings.
  - `packages/yemu-memory/src/core/memory.ts` — local memory runtime used by `remember(...)`.
...
## Flow
1. `MemoryRetainTool.createIf(...)` exposes the tool when `memory.backend` is either `"hindsight"` or `"yemu-memory"`.
2. `execute(...)` re-reads `memory.backend` and dispatches to the matching session state.
3. If the backend is `yemu-memory`:
   - it fetches `session.getYeMuMemorySessionState()` and throws if the backend was not started;
   ...
   - writes go to the scoped retain bank selected by `packages/coding-agent/src/yemu-memory/config.ts`.
...
- YeMuMemory tool path: direct local `remember(...)` into the scoped retain bank.
...
- YeMuMemory bank scoping from `computeYeMuMemoryBankScope(...)`:
...
- Session scope:
  ...
  - persisted YeMuMemory memories are local SQLite data;
...
- Filesystem
  ...
  - YeMuMemory: writes to local SQLite under `yemu-memory.dbPath`, defaulting beneath the agent memories directory (`yemu-memory/yemu-memory.db`) with one database file per scoped bank when needed.
- Network
  ...
  - YeMuMemory: none unless configured embedding or LLM providers make calls during extraction.
- Session state
  ...
  - YeMuMemory: writes through the session's scoped `YeMuMemory` instance, includes `session_id`, `cwd`, and optional `context`, and shares scoped resources with subagents.
- User-visible prompts / interactive UI
  - Hindsight async flush failures emit `session.emitNotice("warning", ...)`; the model is not told.
  - YeMuMemory write failures are logged by `rememberInScope(...)`; the tool response does not expose per-item failures.
- Background work / cancellation
  - Hindsight flush runs later on timer, queue-size threshold, `agent_end`, backend `enqueue(...)`, or backend `clear(...)`.
  - YeMuMemory fact/entity extraction may continue in the YeMuMemory runtime; backend `enqueue(...)` calls `flushExtractions()` before sleeping sessions.

## Limits & Caps
- Input schema requires `items.length >= 1`.
- Tool availability requires `memory.backend` to be `"hindsight"` or `"yemu-memory"`; default `memory.backend` is `"off"`.
...
- Hindsight auto-retain settings:
  ...
  - `hindsight.retainContext` default `"yemu"`
  - `hindsight.retainMode` default `"full-session"`
- YeMuMemory retain settings:
  - `yemu-memory.retainEveryNTurns` default `4`
  - `yemu-memory.autoRetain` controls automatic retention of completed conversation turns
  - `yemu-memory.scoping` selects `global`, `per-project`, or `per-project-tagged`

## Errors
- Throws `YeMuMemory backend is not initialised for this session.` when `memory.backend == "yemu-memory"` but no state exists.
...
- YeMuMemory `remember(...)` failures are caught in `YeMuMemorySessionState.rememberInScope(...)`, logged, and not rethrown to the tool caller.
...
- YeMuMemory storage is local SQLite. `yemu-memoryBackend.clear(...)` removes the scoped database files for the active configuration.
...
- YeMuMemory auto-retain stores prepared transcripts with `source: "coding-agent-transcript"`, `importance: 0.65`, `veracity: "unknown"`, and `memoryType: "episode"`.
