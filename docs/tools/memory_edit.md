...
> Update, forget, or invalidate YeMuMemory long-term memories by id.
...
- Backend collaborator: `packages/coding-agent/src/yemu-memory/state.ts` (`editScopedMemory(...)`)
...
## Flow
1. `MemoryEditTool.createIf(...)` exposes the tool only when `memory.backend == "yemu-memory"`.
2. `execute(...)` fetches `session.getYeMuMemorySessionState()` and fails if the backend is not initialized.
...
## Modes / Variants
- `update` replaces memory text and/or importance in the scoped YeMuMemory store.
- `forget` permanently deletes the addressed memory.
- `invalidate` softly supersedes a memory and may point at `replacement_id`.

## Side Effects
- Filesystem: mutates the local YeMuMemory SQLite database for the active scoped bank.
- Network: none from the tool itself.
- Session state: reads the active session's YeMuMemory state.

## Limits & Caps
- Availability requires `memory.backend = "yemu-memory"`; Hindsight and local memory backends do not expose this tool.
...
## Errors
- `YeMuMemory backend is not initialised for this session.` when the tool is exposed but session state is missing.
