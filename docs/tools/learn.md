...
## Flow
1. `LearnTool.createIf(...)` exposes the tool only when `autolearn.enabled` is true and `memory.backend` is `"hindsight"`, `"yemu-memory"`, or `"local"`.
2. `execute(...)` stores the lesson first:
   - YeMuMemory: calls `rememberScoped(...)` with `source: "coding-agent-learn"`, `importance: 0.8`, `scope: "bank"`, extraction enabled, `veracity: "tool"`, and `memoryType: "fact"`.
...
- Backend-specific memory persistence: queued Hindsight, scoped YeMuMemory SQLite, or local file backend.

## Side Effects
- Filesystem: local memory backend writes under the agent directory; managed skills write to `~/.yemu/agent/managed-skills/<name>/SKILL.md`.
- Network: Hindsight retention queues server-side work; YeMuMemory/local paths do not make a network call from this tool directly.
...
## Errors
- `YeMuMemory backend is not initialised for this session.` when YeMuMemory state is missing.
- `YeMuMemory did not store the lesson (no memory id returned).` when YeMuMemory silently fails to write.
