...
- Key collaborators:
  ...
  - `crates/yemu-native/src/grep.rs` — native regex/file search implementation.
...
4. Internal URLs are resolved before filesystem scope resolution:
   ...
   - `yemu://` expands to every embedded documentation file via URL completion;
...
9. It calls native `grep()` from `@yemu/native` with:
...
10. Native execution happens in `crates/yemu-native/src/grep.rs`:
...
5. **Internal URL paths**
   ...
   - `yemu://` expands to all embedded documentation files so it can be used as a docs search root.
