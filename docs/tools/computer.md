...
- Native implementation: `crates/yemu-native/src/desktop.rs`
...
YeMu native execution never creates a provider Files upload. The provider contract also accepts `{ type: "computer_screenshot", file_id }`; gateway/history parsing preserves that reference in metadata, and capable-model replay emits it unchanged.
