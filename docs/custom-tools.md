...
1. Capability providers (`toolCapability`), including:
   - Native YeMu config (`~/.yemu/agent/tools`, `.yemu/tools`)
   - Claude config (`~/.claude/tools`, `.claude/tools`)
   - Codex config (`~/.codex/tools`, `.codex/tools`)
   - Claude marketplace plugin cache provider
2. Installed plugin manifests (`~/.yemu/plugins/node_modules/*` via plugin loader)
...
const factory: CustomToolFactory = (yemu) => ({
  ...
  parameters: yemu.zod.object({
    glob: yemu.zod.string().optional().default("**/*.ts"),
  }),
...
Schemas are authored with Zod (`yemu.zod`) and flow through the shared validation/wire pipeline.
...
- `yemu`: injected `@yemu/agent-runtime` exports
...
- Forward `signal` to subprocess work (`yemu.exec(..., { signal })`) for cooperative cancellation.
...
- Guard UI usage with `yemu.hasUI`.
