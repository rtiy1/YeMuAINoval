...
This document covers execution/process/terminal primitives in `@yemu/native`: `shell`, `pty`, `ps`, and `keys`, using the architecture terms from `docs/natives-architecture.md`.

## Implementation files

- `crates/yemu-native/src/shell.rs`
- `crates/yemu-shell/src/shell.rs`
- `crates/yemu-shell/src/fixup.rs`
- `crates/yemu-shell/src/windows.rs` (Windows-only PATH enrichment)
- `crates/yemu-shell/src/process.rs`
- `crates/yemu-native/src/pty.rs`
- `crates/yemu-native/src/ps.rs`
- `crates/yemu-native/src/keys.rs`
- `crates/yemu-native/src/task.rs`
- `packages/natives/native/index.d.ts`

## Layer ownership

- **Package entrypoint** (`packages/natives/native/index.js`): loads the `.node` addon and exports generated N-API bindings.
- **Rust N-API module layer** (`crates/yemu-native/src/*`): JS-facing shell/PTY/process/key exports and callback bridging.
- **Runtime core** (`crates/yemu-shell/src/*`): brush shell execution, cancellation cleanup, minimizer integration, command fixups, and cross-platform process references.
...
- `applyBashFixups(command)` strips safe trailing `| head`/`| tail` pipeline caps and redundant trailing `2>&1` according to `yemu_shell::fixup` rules. It returns `{ command, stripped }` and does not execute anything.
...
- Windows-only path enrichment (`yemu-shell/src/windows.rs`) appends discovered Git-for-Windows paths when present and not already included.
...
- `CancelToken` is constructed from `timeoutMs` and optional `AbortSignal`, then converted into the shared `yemu_shell::cancel::CancelToken`.
...
The platform-specific implementation lives in `yemu_shell::process`; `crates/yemu-native/src/ps.rs` is a N-API shim plus re-exports used by PTY termination.
