...
```text
~/.yemu/agent/sessions/<dir-encoded>/<timestamp>_<sessionId>.jsonl
```
...
```text
~/.yemu/agent/blobs/<sha256>
```

Terminal breadcrumb files are written under:

```text
~/.yemu/agent/terminal-sessions/<terminal-id>
```
...
- `listAllSessions(storage)` (a.k.a. `SessionManager.listAll()`) -> sessions across all project scopes under `~/.yemu/agent/sessions`
