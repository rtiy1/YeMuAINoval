...
- `description` is required for:
  - native `.yemu` provider skill discovery (`requireDescription: true`)
  - `yemu-plugins` extension-package skills and the `github` provider (`.github/skills/`), which also pass `requireDescription: true`
...
3. **Managed (auto-learn) skills** (`yemu-managed` provider) resolved dead-last with first-wins, so any same-named authored skill from any provider or custom directory takes precedence
...
1. `native` (priority 100) — `.yemu` user/project skills via `src/discovery/builtin.ts`
2. `yemu-plugins` (priority 90) — `skills/` bundled next to extension packages loaded through `extensions:`, `--extension`/`-e`, or installed plugins under `~/.yemu/plugins/node_modules`
...
7. `yemu-managed` (priority 5) — auto-learn skills under `~/.yemu/agent/managed-skills`, registered in `src/discovery/builtin.ts` and discovered unconditionally (only writing/nudging is gated by `autolearn.enabled`); always defers to a same-named authored skill
...
- source toggles: `enableCodexUser`, `enableClaudeUser`, `enableClaudeProject`, `enableYemuUser`, `enableYemuProject`, `enableAgentsUser`, `enableAgentsProject`
...
The `agents` provider (`.agent[s]/skills`) is the canonical YeMu-native location and has its own `enableAgentsUser`/`enableAgentsProject` toggles — disabling Claude/Codex/YeMu does **not** turn it off. For providers without a dedicated toggle (`claude-plugins`, `opencode`, `gemini`, `github`, …), enablement falls back to: enabled if **any** named source toggle is enabled.
