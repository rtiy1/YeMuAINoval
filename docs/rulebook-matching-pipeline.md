...
- [`packages/coding-agent/src/discovery/yemu-plugins.ts`](../packages/coding-agent/src/discovery/yemu-plugins.ts)
...
- `native` (priority `100`)
- `yemu-plugins` (priority `90`) — `rules/*.{md,mdc}` inside configured extension package roots, normalized via the shared `buildRuleFromMarkdown` path
- `agents` (priority `70`)
- `cursor` (priority `50`)
...
Loads `.yemu` rules from:

- project: `<cwd>/.yemu/rules/*.{md,mdc}` when the cwd `.yemu` directory exists
- user: `~/.yemu/agent/rules/*.{md,mdc}`
- sticky user rule: `~/.yemu/agent/RULES.md`
- sticky project rule: nearest ancestor `.yemu/RULES.md` while walking from cwd toward the repository root
...
- `native` appends project `.yemu/rules`, user `~/.yemu/agent/rules`, user `RULES.md`, then nearest project `RULES.md`.
- `yemu-plugins` appends `rules/` results per configured extension package root.
...
1. The rule providers currently loaded for `rules` are `native`, `yemu-plugins`, `agents`, `cursor`, `windsurf`, `cline`, and embedded `builtin-defaults`; provider files for other tools may parse other config formats but do not register rule loaders.
