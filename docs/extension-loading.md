...
- Project directory: `<cwd>/.yemu/extensions`
- User directory: `~/.yemu/agent/extensions`
- Native legacy/settings JSON entries: `<cwd>/.yemu/settings.json#extensions` and `~/.yemu/agent/settings.json#extensions`

The project root is the native provider's `.yemu` directory (`SOURCE_PATHS.native.projectDir`), cwd-only; it does not walk ancestors. The user root is the active profile's agent directory via `getAgentDir()`, so under `yemu --profile <name>` it becomes `~/.yemu/profiles/<name>/agent/extensions` (and it honors `YEMU_CODING_AGENT_DIR`). See [Profiles](./config-usage.md#profiles).

Notes:

- Native auto-discovery is currently `.yemu` based.
- Legacy `.yemu` is still accepted in package manifests (`yemu.extensions`) and project override lookup, but `.yemu/extensions` is not a native root here.
...
Plugin extension entries come from package `yemu.extensions` / `yemu.extensions` manifests, including enabled feature entries.
...
- User: `~/.yemu/agent/config.yml` (or custom agent dir via `YEMU_CODING_AGENT_DIR`)
- Project/native settings capability: `<cwd>/.yemu/config.yml` and `<cwd>/.yemu/settings.json`

Native extension-module discovery also reads legacy JSON extension lists from:

- `~/.yemu/agent/settings.json`
- `<cwd>/.yemu/settings.json`
...
```json
{
  "extensions": ["./.yemu/extensions/my-extra"]
}
```
...
1. `package.json` in that directory with `yemu.extensions` (or legacy `yemu.extensions`) -> use declared entries
2. `index.ts`
3. `index.js`
4. Otherwise scan one level for extension entries:
   - direct `*.ts` / `*.js`
   - subdir `index.ts` / `index.js`
   - subdir `package.json` with `yemu.extensions` / `yemu.extensions`
...
Each candidate path is loaded via `loadLegacyAgentModule()` (`src/extensibility/plugins/legacy-agent-compat.ts`):

- the entry's realpath is resolved, then dynamically imported with an `?mtime` cache-buster so edited source reloads
- a scoped Bun `onLoad` hook rewrites legacy yemu-package specifiers (`@mariozechner/*`, `@earendil-works/*`) and bare `@sinclair/typebox` onto the host-bundled copies before evaluation
