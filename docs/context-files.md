...
Context files are Markdown instruction files that `yemu` discovers automatically before a session starts and injects into the agent's project context. Use them for repository conventions, architecture notes, test and review expectations, and instructions that should travel with a user account or a project.
...
The native provider is the recommended format for new projects. It reads from your user agent directory and from `.yemu/` directories inside a project, and it has the highest discovery priority, so its files win over every other convention at the same scope.

| File | Scope | Behavior |
|---|---|---|
| `~/.yemu/agent/AGENTS.md` | User | User-level context for every session unless the `native` provider is disabled. |
| `<ancestor>/.yemu/AGENTS.md` | Project | Project context. `yemu` walks upward from the current directory to the repository root and uses the **nearest** non-empty `.yemu/AGENTS.md`. Farther native project files are not also included. |
| `~/.yemu/agent/RULES.md` | User | User-level sticky rule content. Loaded as an always-apply rule, not as a context file. |
| `<ancestor>/.yemu/RULES.md` | Project | Project sticky rule content. Same nearest-ancestor walk-up as above. Loaded as an always-apply rule. |

Two details matter:

- **Walk-up to the repository root.** Discovery starts in the current working directory and climbs through each ancestor up to the repository root, stopping at the first ancestor that has a usable `.yemu/` directory. The *nearest* match wins; ancestors above it are not loaded as native context.
- **The `.yemu/` directory must be non-empty.** An empty `.yemu/` directory is skipped during the walk-up, so the search continues to the next ancestor. An empty `AGENTS.md` or `RULES.md` file contributes nothing.

`~/.yemu/agent` is the user base. If `YEMU_CODING_AGENT_DIR` is set, it relocates that base, so the user files become `$YEMU_CODING_AGENT_DIR/AGENTS.md` and `$YEMU_CODING_AGENT_DIR/RULES.md`.
...
- The native context file is `repo/packages/api/.yemu/AGENTS.md` (the nearest one). `repo/.yemu/AGENTS.md` is **not** also included.
- The project sticky rule is `repo/packages/api/.yemu/RULES.md` if present; otherwise the walk-up continues and `repo/.yemu/RULES.md` is used.
...
`yemu` also discovers the context and rule files of other agent tools so existing projects keep working without migration.

| Provider id | Convention path | Scope | Notes |
|---|---|---|---|
| `native` | `.yemu/AGENTS.md` | User + project | Recommended `yemu` format. User file at `~/.yemu/agent/AGENTS.md`; project file is the nearest non-empty `.yemu/AGENTS.md` walking up to the repo root. |
...
Providers marked "(no ancestor walk-up)" only look in the current working directory's config directory. If you need ancestor walk-up behavior, prefer the native `.yemu/AGENTS.md` format or a standalone `AGENTS.md` (the `agents-md` provider), or launch `yemu` from the directory that holds the config directory.
...
- **One user context file** is kept across all providers. Because `native` has the highest priority, `~/.yemu/agent/AGENTS.md` shadows every other user-level context file.
...
- If you add `repo/packages/api/.yemu/AGENTS.md`, `native` (priority 100) wins depth 0 outright, shadowing both lower-priority files.
...
- It is read **only** at the native locations — `~/.yemu/agent/RULES.md` and the nearest `<ancestor>/.yemu/RULES.md` from the cwd up to the repo root. A `RULES.md` anywhere else is not a context-file convention and is ignored.
...
Turn a provider off with the `disabledProviders` setting in `~/.yemu/agent/config.yml`, a project's `.yemu/config.yml`, or a `--config` overlay:
...
- Native project context must live at `.yemu/AGENTS.md`, and the `.yemu/` directory must be non-empty; an empty `.yemu/` is skipped and the walk-up continues to the next ancestor.
...
At one user scope or project depth, the higher-priority provider shadows the others (native > claude > agents/codex > gemini > opencode > github > agents-md). To force deterministic behavior, move your guidance into `.yemu/AGENTS.md` (native always wins) or disable the competing discovery provider.

### User context disappeared

Only one user-level context file survives, and `~/.yemu/agent/AGENTS.md` has the highest priority. If it exists, it shadows user-level `~/.claude/CLAUDE.md`, `~/.codex/AGENTS.md`, `~/.gemini/GEMINI.md`, `~/.config/opencode/AGENTS.md`, `~/.copilot/copilot-instructions.md`, and `~/.agent`/`~/.agents` files. Consolidate user guidance into the native file or remove the native one if you prefer another tool's file.

### A `RULES.md` file is ignored

Only the native `RULES.md` locations are sticky: `~/.yemu/agent/RULES.md` and the nearest `<ancestor>/.yemu/RULES.md` from cwd to the repo root. A `RULES.md` in any other directory is not a recognized convention and will not be loaded.
