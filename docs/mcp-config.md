# MCP configuration in YeMu

This guide explains how to add, edit, and validate MCP servers for the YeMu coding agent.
...
YeMu can discover MCP servers from multiple tools (`.claude/`, `.cursor/`, `.vscode/`, `opencode.json`, and more), but for YeMu-native configuration you should usually use one of these primary files:

- Project: `.yemu/mcp.json`
- User: `~/.yemu/agent/mcp.json` (or `~/.yemu/profiles/<name>/agent/mcp.json` when a named profile is active — see [Profiles](#profiles))

The native provider also reads `.yemu/.mcp.json` and `~/.yemu/agent/.mcp.json` for compatibility, but YeMu writes to the primary `mcp.json` paths above.

YeMu also accepts fallback standalone files in the project root:

- `mcp.json`
- `.mcp.json`

Use `.yemu/mcp.json` or `~/.yemu/agent/mcp.json` when you want YeMu to own the configuration. Use root `mcp.json` / `.mcp.json` only when you want a portable fallback file that other MCP clients may also read.

### Profiles

Named profiles (`yemu --profile <name>`, the `--alias` shortcut, or `YEMU_PROFILE`/`YEMU_LEGACY_PROFILE`) isolate user-level MCP config. When a profile is active, the **user** scope resolves to the profile's agent directory instead of the default one:

- Default profile: `~/.yemu/agent/mcp.json`
- Profile `<name>`: `~/.yemu/profiles/<name>/agent/mcp.json`

Discovery, the `/mcp` commands, and the config writer all follow the active profile, so a profile sees **only** its own user-level servers — never the default profile's `~/.yemu/agent/mcp.json`. Add a server to a profile by launching under it (`yemu --profile <name>`) and running `/mcp add` → User level, or by editing `~/.yemu/profiles/<name>/agent/mcp.json` directly.

Project-scoped MCP config (`.yemu/mcp.json`) is keyed to the working directory, not the profile, so it applies under every profile. External-tool configs (`.claude/`, `.cursor/`, etc.) are also profile-independent because they belong to those tools rather than to an YeMu profile.

MCP follows the same profile rules as the rest of YeMu-native config; see [Configuration Discovery → Profiles](./config-usage.md#profiles).
...
```json
{
  "$schema": "https://raw.githubusercontent.com/rtiy1/YeMuAINoval/main/packages/coding-agent/src/config/mcp-schema.json",
  "mcpServers": {}
}
```

YeMu now writes this automatically when `/mcp add`, `/mcp enable`, `/mcp disable`, `/mcp reauth`, or other config-writing flows create or update an YeMu-managed MCP file.

## File shape

YeMu supports this top-level structure:
...
- `disabledServers` — user-level denylist used to turn off discovered servers by name; runtime loading reads this list from the active profile's user MCP file (`~/.yemu/agent/mcp.json`, or `~/.yemu/profiles/<name>/agent/mcp.json` under a named profile)
...
- `auth?: { ... }` — auth metadata used by YeMu for OAuth/API-key flows
- `oauth?: { ... }` — explicit OAuth client settings used during auth/reauth

Set `YEMU_MCP_TIMEOUT_MS=0` to disable the client-side timeout for every MCP server in the current process. Set it to a positive millisecond value, such as `YEMU_MCP_TIMEOUT_MS=120000`, to apply one global timeout without editing each server entry.
...
YeMu understands two auth-related objects.
...
Use this when YeMu should remember how to rehydrate credentials for a server.

You normally do not need to write this block: when YeMu completes an OAuth flow
...
still honored when it resolves; if it points at another profile's row, YeMu falls
...
`prompt` controls the OAuth `prompt` parameter sent with the authorization request. It defaults to `"consent"` so the provider always shows its consent/account screen — without it, a provider with an active browser session silently re-approves the same account, making it impossible to switch accounts or workspaces when reauthorizing (e.g. to use a different Linear workspace per YeMu profile). Set it to `""` to omit the parameter for providers that reject it, or to another value the provider understands (e.g. `"select_account"`).
...
YeMu expands `${VAR}` and `${VAR:-default}` placeholders while discovering MCP configs from YeMu-native files and standalone fallback files. Expansion applies recursively to string values in `command`, `args`, `env`, `cwd`, `url`, `headers`, `auth`, and `oauth`; unresolved placeholders remain literal strings.
...
Before YeMu launches a stdio server or makes an HTTP/SSE request, it resolves stdio `env` values and HTTP/SSE `headers` values like this:

1. If a value starts with `!`, YeMu runs the rest as a shell command with a 10s timeout and uses trimmed stdout.
2. If the command fails, times out, or prints only whitespace, that `env`/`headers` entry is omitted.
3. Otherwise YeMu checks whether the value names an environment variable.
4. If that environment variable is set to a non-empty value, YeMu uses the environment value; otherwise it uses the string literally.
...
`disabledServers` is read from the user config file (`~/.yemu/agent/mcp.json`) when a server is discovered from any source and you want YeMu to ignore it without editing that other tool's config.
...
## Validation rules YeMu enforces
...
- If you paste a remote server config and forget `"type": "http"`, YeMu will treat it as `stdio` and complain that `command` is missing
- `sse` remains valid for compatibility, but new hosted servers should usually be configured as `http`

## Discovery and precedence

YeMu does not merge duplicate server definitions across files. Discovery providers are prioritized, and the higher-priority definition wins. Separately, `disabledServers` from `~/.yemu/agent/mcp.json` can suppress a discovered server by name.

In practice:

- prefer `.yemu/mcp.json` or `~/.yemu/agent/mcp.json` when you want an YeMu-specific override
...
Pick one transport. YeMu treats `command` as stdio and `url` as http/sse.
...
### The server exists in another tool's config but not in YeMu

Run `/mcp list`. YeMu discovers many third-party MCP files, but project-level loading can also be disabled via the `mcp.enableProjectConfig` setting, and a user-level `disabledServers` entry can suppress a server by name.
