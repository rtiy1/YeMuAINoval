...
This document covers the media/system/conversion exports currently present in `@yemu/native`: terminal SIXEL image encoding, HTML conversion, clipboard access, token counting, macOS appearance/power helpers, and work profiling.
...
- `crates/yemu-native/src/tokens.rs`
- `crates/yemu-native/src/appearance.rs`
- `crates/yemu-native/src/power.rs`
- `crates/yemu-native/src/prof.rs`
- `crates/yemu-native/src/task.rs`
- `packages/natives/native/index.d.ts`

There is no native `PhotonImage` class, `image.rs`, or ProjFS overlay helper module in the current `yemu-native` addon. General-purpose image decode/resize/encode is expected to live outside this native surface; the native image export here is only terminal SIXEL encoding.
