...
- Tool calls render through the `<yemu-tool-view>` web component — the React per-tool renderers shared with collab-web (`packages/collab-web/src/tool-render/`), prebuilt into `src/export/html/tool-views.generated.js` by `bun run gen:tool-views`.
...
`loadCustomShare()` checks `~/.yemu/agent` for first existing candidate:
...
4. Upload target is chosen by `share.store`:
   - **Share server** (default, `store: "blob"`) — `POST <share.serverUrl>`
     (default `https://my.yemu.sh/s`) with the raw blob, capped at 1 MB.
...
- Mints a fresh provider session id and re-keys hindsight and yemu-memory memory to
...
Use `--prompt-cache-key <key>` to pin the provider prompt-cache identity explicitly and independently from both the YeMu session id and `--provider-session-id`. `--provider-session-id` continues to control provider session/routing headers and sticky credential selection; `--prompt-cache-key` controls the OpenAI Responses `prompt_cache_key` payload where supported.
