/**
 * Set the OS-visible process name (`/proc/self/comm`) so `yemu` shows up as
 * `yemu` — not `bun` — in `ps`, `pgrep`, `killall`, `top`, `htop`, and systemd.
 *
 ...
 * `bun` and process-name-based tooling can't target yemu (and `pkill bun` becomes
