...
import { Process, ProcessStatus } from "@yemu/native";
...
function buildSpawnEnv(shell: string): Record<string, string> {
	const noCI = $env.YEMU_BASH_NO_CI || $env.CLAUDE_BASH_NO_CI;
	return {
 ...
 * YEMU_BASH_NO_LOGIN / CLAUDE_BASH_NO_LOGIN is set.
 */
function getShellArgs(shell: string): string[] {
	if (isCmdShell(shell)) return ["/c"];
	const noLogin = $env.YEMU_BASH_NO_LOGIN || $env.CLAUDE_BASH_NO_LOGIN;
...
function getShellPrefix(): string | undefined {
	return $env.YEMU_SHELL_PREFIX || $env.CLAUDE_CODE_SHELL_PREFIX;
}
