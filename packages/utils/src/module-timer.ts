 ...
 * when `YEMU_TIMING` is set) that record an inclusive module window plus resolved
 * static child edges:
 *
 ...
 * cheap to preload before yemu-utils (and winston) exist. The buffer is shared with
 ...
 * - **Dev runs only.** In the compiled `yemu` binary every module is pre-bundled
...
const MODULE_COMPLETE_KEY: symbol = Symbol.for("yemu.moduleLoadComplete");
const MODULE_BODY_START_KEY: symbol = Symbol.for("yemu.moduleBodyStart");
...
function bodyStartMarker(path: string): string {
	return `;globalThis[Symbol.for("yemu.moduleBodyStart")]?.(${JSON.stringify(path)});\n`;
}

function completionMarker(path: string): string {
	return `\n;globalThis[Symbol.for("yemu.moduleLoadComplete")]?.(${JSON.stringify(path)});\n`;
}
