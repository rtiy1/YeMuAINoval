 ...
 * crash restore). Unlike codex we redirect to the yemu log file — not
...
export interface SuppressTerminalStderrOptions {
	/** Redirect target path; defaults to today's yemu log file, then /dev/null. */
	redirectPath?: string;
...
export function suppressTerminalStderr(options?: SuppressTerminalStderrOptions): boolean {
	...
	try {
		...
		// on a fresh profile ~/.yemu/logs may not exist yet. Create it here so
