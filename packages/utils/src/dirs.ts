/**
 * Centralized path helpers for YeMu core config directories.
 *
 * Uses YEMU_CONFIG_DIR (default ".yemu") for the config root and
 * YEMU_CODING_AGENT_DIR to override the agent directory.
 *
 * On Linux, if XDG_DATA_HOME / XDG_STATE_HOME / XDG_CACHE_HOME environment
 * variables are set, paths are redirected to XDG-compliant locations under
 * $XDG_*_HOME/yemu/. This requires running `yemu-core config migrate` first to
...
/** Internal core CLI name. */
export const APP_NAME: string = "yemu-core";

/** Config directory name. */
export const CONFIG_DIR_NAME: string = ".yemu";
...
const PROFILE_ENV_KEYS = ["YEMU_PROFILE", "YEMU_LEGACY_PROFILE"] as const;
 ...
 *
 * Exported so consumers of `@yemu/utils/dirs` (CLI bootstrap, tests,
...
export function normalizeProfileName(profile: string | undefined): string | undefined {
	...
	if (
		...
		throw new Error(
			`Invalid YeMu profile "${profile}". Profile names must match ${PROFILE_NAME_RE.source}, ` +
...
/**
 * Resolve the active profile from the canonical and compatibility profile
 * variables. `YEMU_PROFILE` takes precedence; `YEMU_LEGACY_PROFILE` is only
 * consulted when it is undefined. An explicitly empty canonical value selects
 * the default profile.
 */
export function resolveProfileEnv(
	...
	return normalizeProfileName(canonicalProfile !== undefined ? canonicalProfile : legacyProfile);
}

function getProfileFromEnv(): string | undefined {
	return resolveProfileEnv(process.env.YEMU_PROFILE, process.env.YEMU_LEGACY_PROFILE);
}

/**
 * Module-load profile resolution. Unlike {@link getProfileFromEnv}, an invalid
 * YEMU_PROFILE/YEMU_LEGACY_PROFILE value does NOT throw here — a bad env var must not
 ...
 * user still gets a clean "Invalid YeMu profile" message.
 */
...
function readYemuProfileFromEnvSafe(): string | undefined {
	try {
		return normalizeProfileName(process.env.YEMU_PROFILE);
	} catch {
		return undefined;
	}
}
...
/** Get the config directory name relative to home (e.g. ".yemu" or YEMU_CONFIG_DIR override). */
export function getConfigDirName(): string {
	return process.env.YEMU_CONFIG_DIR || CONFIG_DIR_NAME;
}

/** Get the config agent directory name relative to home (e.g. ".yemu/agent" or YEMU_CONFIG_DIR + "/agent"). */
...
/**
 * Resolves and caches all yemu directory paths. On Linux, when XDG environment
 * variables are set, paths are redirected under $XDG_*_HOME/yemu/. A new
...
class DirResolver {
	...
	// With XDG on Linux, they point to $XDG_*_HOME/yemu/.
	...
	constructor(options: { agentDirOverride?: string; profile?: string } = {}) {
		...
		// resolves under $XDG_*_HOME/yemu once `yemu config init-xdg` has migrated
		...
		// profile could resolve to `~/.yemu/profiles/<name>` on first activation
		// (when no $XDG_*_HOME/yemu exists yet) and then silently move to
		// `$XDG_*_HOME/yemu/profiles/<name>` the moment the base appeared, orphaning
		...
		// XDG flattens the agent/ prefix: ~/.yemu/agent/sessions → $XDG_DATA_HOME/yemu/sessions
		this.#agentDirs = {
...
/**
 * Decide which `YEMU_CODING_AGENT_DIR` value to capture as the pre-profile
 ...
 * `YEMU_PROFILE` that was bypassed because `YEMU_PROFILE` explicitly selected the
 ...
 * mode honors a non-profile `YEMU_CODING_AGENT_DIR` (see
...
function resolveActiveAgentDirOverride(): string | undefined {
	return activeProfile
		? undefined
		: resolvePreProfileAgentDir(undefined, process.env.YEMU_CODING_AGENT_DIR, readYemuProfileFromEnvSafe());
}
...
/**
 * Snapshot of `YEMU_CODING_AGENT_DIR` from before the first named-profile
 ...
 * with `YEMU_CODING_AGENT_DIR=/custom` then `setProfile("work")` then
 * `setProfile(undefined)` would silently lose `/custom` and fall back to
 * `~/.yemu/agent`. Captured at module load — ignoring a profile-derived value
...
let preProfileAgentDirEnv: string | undefined = resolvePreProfileAgentDir(
	activeProfile,
	process.env.YEMU_CODING_AGENT_DIR,
	activeProfile ?? readYemuProfileFromEnvSafe(),
);
 ...
 * mode, `YEMU_CODING_AGENT_DIR`) loaded from a profile/agent `.env` only reach
...
/** Get the config root directory (~/.yemu). */
...
export function setAgentDir(dir: string): void {
	...
	process.env.YEMU_CODING_AGENT_DIR = dir;
...
/**
 * Test-only: reset the pre-profile `YEMU_CODING_AGENT_DIR` snapshot to whatever
...
export function setProfile(profile: string | undefined): void {
	...
	if (next && !activeProfile) {
		...
		// current YEMU_CODING_AGENT_DIR so a later reset can restore the user's
		...
		preProfileAgentDirEnv = resolvePreProfileAgentDir(
			undefined,
			process.env.YEMU_CODING_AGENT_DIR,
			readYemuProfileFromEnvSafe(),
		);
	}
	activeProfile = next;
	if (activeProfile) {
		dirs = new DirResolver({ profile: activeProfile });
		process.env.YEMU_PROFILE = activeProfile;
		process.env.YEMU_PROFILE = activeProfile;
		process.env.YEMU_CODING_AGENT_DIR = dirs.agentDir;
	} else {
		for (const key of PROFILE_ENV_KEYS) {
			delete process.env[key];
		}
		if (preProfileAgentDirEnv === undefined) {
			...
			process.env.YEMU_CODING_AGENT_DIR = preProfileAgentDirEnv;
		}
...
/** Get the agent config directory (~/.yemu/agent). */
export function getAgentDir(): string {
	return dirs.agentDir;
}

/** Get the project-local config directory (.yemu). */
export function getProjectAgentDir(cwd: string = getProjectDir()): string {
	return path.join(cwd, CONFIG_DIR_NAME);
}

// =============================================================================
// Config-root subdirectories (~/.yemu/*)
// =============================================================================

/** Get the reports directory (~/.yemu/reports). */
export function getReportsDir(): string {
	return dirs.rootSubdir("reports", "state");
}

/** Get the logs directory (~/.yemu/logs). */
export function getLogsDir(): string {
	return dirs.rootSubdir("logs", "state");
}

/** Get this process's dated log path (~/.yemu/logs/yemu.YYYY-MM-DD.PID.log). */
...
/**
 * Get the plugins directory (~/.yemu/plugins or its XDG equivalent).
 *
 ...
 * (PR checkout, task isolation) and cleanup (`yemu worktree`). A relative value
 * would resolve against whatever cwd happened to launch `yemu`, so checkout and
 ...
 * isolation, and `yemu worktree` cleanup all read the same base). Driven by the
 * `worktree.base` setting in coding-agent; pass `undefined`/empty to clear and
 * fall back to `YEMU_WORKTREE_DIR` or the `~/.yemu/wt` default.
 *
 ...
 * `YEMU_WORKTREE_DIR` env var, then the {@link setWorktreesDir} override (the
 * `worktree.base` setting), then the `~/.yemu/wt` default. The env var and the
 * override are both `~`-expanded and must be absolute; a relative value is
 * ignored and resolution falls through.
 */
export function getWorktreesDir(): string {
	return resolveWorktreeBase(process.env.YEMU_WORKTREE_DIR) ?? worktreesDirOverride ?? dirs.rootSubdir("wt", "data");
}

/** Get the SSH control socket directory (~/.yemu/ssh-control). */
export function getSshControlDir(): string {
	return dirs.rootSubdir("ssh-control", "state");
}

/** Get the remote host info directory (~/.yemu/remote-host). */
export function getRemoteHostDir(): string {
	return dirs.rootSubdir("remote-host", "data");
}

/** Get the managed Python venv directory (~/.yemu/python-env). */
export function getPythonEnvDir(): string {
	return dirs.rootSubdir("python-env", "data");
}

/** Get the shared Python gateway state directory (~/.yemu/agent/python-gateway; XDG default: $XDG_STATE_HOME/yemu/python-gateway). */
export function getPythonGatewayDir(): string {
	return dirs.agentSubdir(undefined, "python-gateway", "state");
}

/** Get the puppeteer sandbox directory (~/.yemu/puppeteer). */
...
/** Get the auto-QA grievances SQLite database path (~/.yemu/autoqa.db; XDG: $XDG_DATA_HOME/yemu/autoqa.db). */
 ...
 * (e.g. PR-checkout and task-isolation worktree dirs under `~/.yemu/wt/`).
...
/** Get the path to a single worktree directory (~/.yemu/wt/<segment>). */
export function getWorktreeDir(segment: string): string {
	return path.join(getWorktreesDir(), segment);
}

/** Get the GPU cache path (~/.yemu/gpu_cache.json). */
export function getGpuCachePath(): string {
	return dirs.rootSubdir("gpu_cache.json", "cache");
}

/**
 * Get the GitHub view cache database path (~/.yemu/cache/github-cache.db).
 * Honors the `YEMU_GITHUB_CACHE_DB` env var when set so tests can isolate the
 * cache file without touching the rest of the config root.
 */
export function getGithubCacheDbPath(): string {
	const override = process.env.YEMU_GITHUB_CACHE_DB;
	if (override) return override;
	return dirs.rootSubdir(path.join("cache", "github-cache.db"), "cache");
}

/**
 * Get the encrypted auth-broker snapshot cache path (~/.yemu/cache/auth-broker-snapshot.enc).
 * Honors the `YEMU_AUTH_BROKER_SNAPSHOT_CACHE` env var when set so tests and
 * operators can isolate or relocate the cache file.
 */
export function getAuthBrokerSnapshotCachePath(): string {
	const override = process.env.YEMU_AUTH_BROKER_SNAPSHOT_CACHE;
	if (override) return override;
	return dirs.rootSubdir(path.join("cache", "auth-broker-snapshot.enc"), "cache");
}

/** Get the local FastEmbed model cache directory (~/.yemu/cache/fastembed). */
export function getFastembedCacheDir(): string {
	return dirs.rootSubdir(path.join("cache", "fastembed"), "cache");
}

/** Get the on-demand fastembed runtime install root (~/.yemu/cache/fastembed-runtime). */
export function getFastembedRuntimeDir(): string {
	return dirs.rootSubdir(path.join("cache", "fastembed-runtime"), "cache");
}

/** Get the natives directory (~/.yemu/natives). */
export function getNativesDir(): string {
	return dirs.rootSubdir("natives", "cache");
}

/** Get the stats database path (~/.yemu/stats.db). */
export function getStatsDbPath(): string {
	return dirs.rootSubdir("stats.db", "data");
}

/** Get the autoresearch state directory (~/.yemu/autoresearch). */
export function getAutoresearchDir(): string {
	return dirs.rootSubdir("autoresearch", "state");
}

/** Get the per-project autoresearch state directory (~/.yemu/autoresearch/<encoded-project>). */
export function getAutoresearchProjectDir(encodedProject: string): string {
	return path.join(getAutoresearchDir(), encodedProject);
}

/** Get the per-project autoresearch SQLite database path (~/.yemu/autoresearch/<encoded-project>.db). */
export function getAutoresearchDbPath(encodedProject: string): string {
	return path.join(getAutoresearchDir(), `${encodedProject}.db`);
}

/** Get the per-run artifact directory (~/.yemu/autoresearch/<encoded-project>/runs/<runId>). */
...
// =============================================================================
// Agent subdirectories (~/.yemu/agent/*)
// =============================================================================
...
/** Get the last-seen-changelog-version marker file (~/.yemu/agent/last-changelog-version). */
...
/** Get the tiny title model cache directory (~/.yemu/agent/cache/tiny-models). */
...
/** Get the document conversion cache directory (~/.yemu/agent/cache/document-conversions; XDG default: $XDG_CACHE_HOME/yemu/cache/document-conversions). */
...
/**
 * Persistent per-install UUID stored at `~/.yemu/install-id`.
 *
 ...
 * `~/.yemu/agent/` does not regenerate it. Server-side dedup for grievance
 * pushes (and similar telemetry) keys on this id.
 *
 * Anchored to the base config root (`~/.yemu/install-id`) regardless of the
