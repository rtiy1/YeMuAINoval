...
/**
 * Centralized logger for yemu.
 *
 * Default: rotating `~/.yemu/logs/yemu.<DATE>.<PID>.log`, no console output (writing
 ...
 *
 * Each entry includes `process.pid` so concurrent yemu instances stay
 * traceable.
 */
...
const PROCESS_LOG_PATTERN = /^yemu\.\d{4}-\d{2}-\d{2}\.(\d+)\.log(?:\.\d+)?$/;
const PROCESS_AUDIT_PATTERN = /^\.yemu\.(\d+)-audit\.json$/;
...
function makeFileTransport(dir?: string): FileTransport {
	...
	return new DailyRotateFileImplementation({
		dirname: logsDir,
		filename: `yemu.%DATE%.${process.pid}.log`,
		datePattern: "YYYY-MM-DD",
		maxSize: "10m",
		maxFiles: 5,
		zippedArchive: false,
		auditFile: path.join(logsDir, `.yemu.${process.pid}-audit.json`),
	}) as FileTransport;
}
...
/**
 * Streaming startup markers, enabled by `YEMU_DEBUG_STARTUP`. Unlike the
 * YEMU_TIMING tree (printed only after startup completes), these write one
...
export function timingModeIncludes(option: "full" | "x"): boolean {
	const value = process.env.YEMU_TIMING;
...
function printModuleLoadSummary(loads: Span[], depth: number, lines: string[]): void {
	...
	if (!showAll && byBody.length > MODULE_LOAD_VERBOSE_TOP) {
		lines.push(`${grandIndent}  … ${byBody.length - MODULE_LOAD_VERBOSE_TOP} more (YEMU_TIMING=full to show all)`);
	}
	...
	if (!showAll && treeRoots.length > MODULE_TREE_ROOT_TOP) {
		lines.push(
			`${grandIndent}  … ${treeRoots.length - MODULE_TREE_ROOT_TOP} more roots (YEMU_TIMING=full to show all)`,
		);
	}
}
...
function renderModuleTimingNode(
	...
	if (!showAll && ancestors.size >= MODULE_TREE_MAX_DEPTH) {
		if (node.children.length > 0) {
			lines.push(`${indent}  … ${node.children.length} imports deeper (YEMU_TIMING=full to show all)`);
		}
	...
	if (!showAll && node.children.length > MODULE_TREE_CHILD_TOP) {
		lines.push(
			`${indent}  … ${node.children.length - MODULE_TREE_CHILD_TOP} more imports (YEMU_TIMING=full to show all)`,
		);
	}
	ancestors.delete(path);
}
...
export function time<T, A extends unknown[]>(op: string, fn?: (...args: A) => T, ...args: A): T | undefined {
	...
	if (!recording && !process.env.YEMU_DEBUG_STARTUP) {
		return fn(...args);
	}
