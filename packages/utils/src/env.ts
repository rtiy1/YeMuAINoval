...
const projectEnvNamesLoadedByYemu = new Set<string>();
...
export function filterChildShellEnv(
	...
	for (const key in launchEnv) {
		...
		if (launchEnvValues || projectEnvNamesLoadedByYemu.has(key)) {
			// Strong provenance: the launch environment is known and this name is
			// absent from it, or YeMu itself injected the value — either way it came
...
const yemuEnv = parseEnvFile(path.join(getConfigRootDir(), ".env"));
...
for (const file of [projectEnv, agentEnv, yemuEnv, homeEnv]) {
	for (const key in file) {
		if (!isMacosMallocStackLoggingEnvName(key) && !Bun.env[key]) {
			Bun.env[key] = file[key];
			if (file === projectEnv) projectEnvNamesLoadedByYemu.add(key);
		}
	}
}

// Directory-affecting keys (XDG_*_HOME, and in default mode YEMU_CODING_AGENT_DIR)
 ...
 *
 * All users should import this env module (import { $env } from "@yemu/utils")
 ...
 * `YEMU_COMPILED` env var (set by the build script's `--define`) is checked
 * first for cheap fast-path detection.
 */
export function isCompiledBinary(): boolean {
	if (process.env.YEMU_COMPILED || Bun.env.YEMU_COMPILED) return true;
