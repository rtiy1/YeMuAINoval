...
import { getAgentDir, getConfigDirName, getPythonGatewayDir, setAgentDir } from "@yemu/utils/dirs";
...
describe("python gateway directory", () => {
	...
	beforeEach(async () => {
		...
		tempRoot = path.join(os.tmpdir(), "yemu-utils-python-gateway", Snowflake.next());
	...
	it("uses XDG state for the default agent profile", async () => {
		...
		process.env.YEMU_CONFIG_DIR = `.yemu-test-${Snowflake.next()}`;
		...
		expect(getPythonGatewayDir()).toBe(path.join(process.env.XDG_STATE_HOME, "yemu", "python-gateway"));
	});
