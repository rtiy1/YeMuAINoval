...
async function runPostmortemProbe(
	...
	const root = await mkdtemp(join(tmpdir(), "yemu-postmortem-probe-"));
	...
	try {
		...
		const proc = Bun.spawn([process.execPath, probePath], {
			...
			stdout: "pipe",
			stderr: "pipe",
			env: { ...process.env, YEMU_AGENT_DIR: join(root, "agent") },
		});
		...
		// yemu-utils module graph can take multiple seconds on a loaded parallel CI runner (observed >2s).
...
describe("postmortem expected cleanup errors", () => {
	it("marks errors with the well-known cleanup symbol and recognizes them", () => {
		...
		expect(Reflect.get(reason, Symbol.for("yemu.expectedCleanupError"))).toBe(true);
