...
describe("run() per-command help", () => {
	// Contract: `yemu <cmd> --help` must load only the requested command module.
	...
	it("loads only the requested command", async () => {
		...
		try {
			await run({ bin: "yemu", version: "0.0.0", argv: ["good", "--help"], commands });
		} finally {
...
describe("run() usage errors", () => {
	...
	it("prints a concise usage error instead of throwing on a missing required arg", async () => {
		...
		try {
			await expect(run({ bin: "yemu", version: "0.0.0", argv: ["bench"], commands })).resolves.toBeUndefined();
		} finally {
		...
		expect(out).toContain("$ yemu bench MODELS... [FLAGS]");
	...
	it("renders a required variadic arg without optional brackets", async () => {
		...
		try {
			await run({ bin: "yemu", version: "0.0.0", argv: ["bench", "--help"], commands });
		} finally {
			stdoutSpy.mockRestore();
		}
	...
	it("prints a concise usage error for an unknown flag", async () => {
		...
		try {
			await expect(
				run({ bin: "yemu", version: "0.0.0", argv: ["bench", "--unknown"], commands }),
