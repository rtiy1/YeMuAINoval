...
import { peekFile, peekFileEnds, peekFileSync, peekFileTail } from "@yemu/utils/peek-file";
...
describe("peekFile", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-peek-file-"));
	});
...
describe("peekFileTail", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-peek-tail-"));
	});
...
describe("peekFileEnds", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-peek-ends-"));
	});
