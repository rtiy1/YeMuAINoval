...
describe("setProcessName", () => {
	it.skipIf(os.platform() !== "linux")("sets /proc/self/comm on Linux", async () => {
		...
		// TASK_COMM_LEN caps comm at 15 chars; "yemu-probe" fits whole.
		expect(report.comm).toBe("yemu-probe");
		expect(report.title).toBe("yemu-probe");
	});
});
