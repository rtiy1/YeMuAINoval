...
import { filterProcessEnv, parseEnvFile } from "@yemu/utils/env";
...
function writeTempEnv(content: string): string {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-utils-env-"));
	tempDirs.push(dir);
...
describe("parseEnvFile", () => {
	...
	it("parses valid YEMU_ variables", () => {
		const filePath = writeTempEnv("YEMU_FEATURE=enabled\nYEMU_BAD=before\0after\n");

		expect(parseEnvFile(filePath)).toEqual({
			YEMU_FEATURE: "enabled",
		});
	});
