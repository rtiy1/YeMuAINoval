...
import { isOfficialAnthropicApiUrl } from "@yemu/model-catalog/compat/anthropic";
import { buildOpenAICompat, buildOpenAIResponsesCompat } from "@yemu/model-catalog/compat/openai";
import { readModelCache, writeModelCache } from "@yemu/model-catalog/model-cache";
import { resolveProviderModels } from "@yemu/model-catalog/model-manager";
import { getBundledModel } from "@yemu/model-catalog/models";
import { openrouterModelManagerOptions } from "@yemu/model-catalog/provider-models/openai-compat";
import type { Model, ModelSpec } from "@yemu/model-catalog/types";
...
describe("OpenRouter model discovery", () => {
	it("keeps refreshed OpenRouter models on the OpenRouter pseudo API", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-openrouter-refresh-"));
	...
	it("ignores legacy OpenRouter chat-completions cache rows", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-openrouter-legacy-cache-"));
		const dbPath = path.join(tempDir, "models.db");
...
describe("model cache spec round trip", () => {
	it("persists sparse specs and rebuilds resolved models on cache reads", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-model-cache-"));
		const dbPath = path.join(tempDir, "models.db");
		const sparse = { supportsDeveloperRole: true } as const;
	...
	it("invalidates schema-v10 rows that predate computer-use capability provenance", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-legacy-computer-cache-"));
		const dbPath = path.join(tempDir, "models.db");
		const model = buildModel({
	...
	it("preserves computer-use provenance across cache restarts and endpoint reroutes", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-computer-use-cache-"));
		const dbPath = path.join(tempDir, "models.db");
		const common = {
			name: "GPT-5.4",
	...
	it("uses current static limits for same-id cache rows when the static fingerprint changed", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-static-fingerprint-"));
		const dbPath = path.join(tempDir, "models.db");
	...
	it("retries an empty discovery result after the short interval and caches recovery", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-empty-discovery-"));
		const dbPath = path.join(tempDir, "models.db");
	...
	it("reports an authoritative catalog emptying as non-stale so removed models prune", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-empty-transition-"));
		const dbPath = path.join(tempDir, "models.db");
	...
	it("restores static model headers on fresh cache reads", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-static-headers-"));
		const dbPath = path.join(tempDir, "models.db");
	...
	it("refetches dynamic-only models whose headers cannot be restored", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-dynamic-headers-"));
		const dbPath = path.join(tempDir, "models.db");
	...
	it("keeps a synthesized request-model variant across an offline restart", async () => {
		...
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-request-model-variant-"));
		const dbPath = path.join(tempDir, "models.db");
	...
	it("refetches a current request-model alias whose headers differ from its static base", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-custom-alias-"));
		const dbPath = path.join(tempDir, "models.db");
	...
	it("recovers a legacy stale-marked request-model variant via requestModelId", async () => {
		...
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-legacy-variant-"));
		const dbPath = path.join(tempDir, "models.db");
		const headers = { "X-GitHub-Api-Version": "2026-06-01" };
		const base = completionsSpec({ id: "sol", provider: "variant-cache-test", headers });
