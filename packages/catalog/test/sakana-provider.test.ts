...
import { getBundledModels } from "@yemu/model-catalog/models";
import { DEFAULT_MODEL_PER_PROVIDER, PROVIDER_DESCRIPTORS } from "@yemu/model-catalog/provider-models/descriptors";
import { sakanaModelManagerOptions } from "@yemu/model-catalog/provider-models/openai-compat";
import type { FetchImpl, ModelSpec, ResolvedOpenAIResponsesCompat } from "@yemu/model-catalog/types";
import { getOAuthProviders } from "@yemu/model-runtime/registry/oauth";
import { getEnvApiKey } from "@yemu/model-runtime/stream";
...
describe("Sakana AI provider support", () => {
	...
	test("drops stale cached Fugu rows when bundled context metadata changes", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-sakana-stale-cache-"));
