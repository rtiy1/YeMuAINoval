...
import { createModelManager } from "@yemu/model-catalog/model-manager";
import { getBundledModel } from "@yemu/model-catalog/models";
import { githubCopilotModelManagerOptions } from "@yemu/model-catalog/provider-models/openai-compat";
...
describe("github copilot model limits mapping", () => {
	...
	it("keeps discovered context window through full model resolution for bundled models", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "model-runtime-copilot-models-"));
		try {
			const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
				const url = typeof input === "string" ? input : input.toString();
	...
	it("invalidates a cached MAI-Code completion route after the endpoint migration", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "model-runtime-copilot-mai-cache-"));
...
describe("github copilot vision endpoint policy", () => {
	...
	it("keeps explicit upstream vision false text-only through the personal endpoint manager merge", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "model-runtime-copilot-vision-"));
		try {
			const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
				const url = typeof input === "string" ? input : input.toString();
				expect(url).toBe("https://api.githubcopilot.com/models");
