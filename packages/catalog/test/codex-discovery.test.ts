...
import { fetchCodexModels } from "@yemu/model-catalog/discovery/codex";
...
import { openaiCodexModelManagerOptions } from "@yemu/model-catalog/provider-models/special";
import type { ModelSpec } from "@yemu/model-catalog/types";
...
describe("Codex model discovery", () => {
	...
	it("keeps account-listed API-unsupported models while pruning hidden and absent models", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-codex-authoritative-"));
	...
	it("unions models across every configured Codex OAuth account (#6265)", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-codex-union-"));
	...
	it("keeps bundled Codex models when any account catalog fetch fails (#6265)", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-codex-union-fail-"));
	...
	it("ignores pre-V2 Codex discovery cache rows", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-codex-v7-cache-"));
	...
	it("does not silently promote legacy v2 Codex cache rows to the current schema", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-catalog-codex-v2-cache-"));
		const dbPath = path.join(tempDir, "models.db");
		try {
