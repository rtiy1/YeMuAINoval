...
import { discoverAuthStorage } from "@yemu/model-runtime/auth-broker/discover";
import type { OAuthAccess } from "@yemu/model-runtime/auth-storage";
import type { OAuthProvider } from "@yemu/model-runtime/oauth/types";
import { getGitLabDuoModels } from "@yemu/model-runtime/providers/gitlab-duo";
...
async function fetchAntigravityModels(): Promise<ModelSpec<"google-gemini-cli">[]> {
	...
	if (!access) {
		...
		console.log("Tip: If you are logged in under a specific profile, run with YEMU_PROFILE=<name>.");
		return [];
	}
	try {
