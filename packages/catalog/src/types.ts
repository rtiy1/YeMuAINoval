...
// Re-exported from @yemu/utils so the whole workspace shares one
// `fetch`-compatible signature (tls-fetch's wrappers produce/accept it).
export type { FetchImpl } from "@yemu/utils";
...
export interface ThinkingConfig {
	 ...
	 * (`variant-collapse.ts`). Keyed by yemu effort; `"off"` applies when
...
export interface OpenAICompat {
	...
	/** Optional mapping from model-runtime reasoning levels to provider/model-specific `reasoning_effort` values. */
	reasoningEffortMap?: Partial<Record<Effort, string>>;
	/** Whether the provider supports `stream_options: { include_usage: true }` for token usage in streaming responses. Default: true. */
	supportsUsageInStreaming?: boolean;
	/**
	 * Enable the Gemini thinking-loop guard (model-runtime stream layer) for this model.
 ...
 * (default) or Chat Completions (`YEMU_OPENROUTER_RESPONSES=0`) with the same
...
export interface Model<TApi extends Api = Api> {
	 ...
	 * than YeMu's general image pipeline. `stb` local backends reject WebP.
	 */
	 ...
	 * output limit YeMu cannot discover — sending the wrong value triggers 400s
	 * from the upstream provider.
	 */
	omitMaxOutputTokens?: boolean;
	headers?: Record<string, string>;
	/**
	 * Streaming transport override. When `"yemu-native"`, `streamSimple` routes
	 * the request to the model's `baseUrl` via the auth-gateway's
	 * `POST /v1/yemu/stream` endpoint instead of dispatching the per-API
	 * provider client. The `baseUrl` must point at an `yemu auth-gateway`
	 * (or compatible) host; `headers.Authorization` (or `apiKey` resolved by
	 * the registry) carries the gateway bearer.
	 *
	 * Used by containerized yemu installs (e.g. yemu slots) to route every
