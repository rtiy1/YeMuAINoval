 ...
 *
 * Auth: provider API keys resolve through yemu's auth storage
 * (~/.yemu/agent/agent.db: stored key, OAuth, or env var fallback).
 */

import { parseArgs } from "node:util";
import { type GeneratedProvider, getBundledModel } from "@yemu/model-catalog/models";
import { type Api, AuthStorage, completeSimple, type Model, SqliteAuthCredentialStore } from "@yemu/model-runtime";
import { getAgentDbPath } from "@yemu/utils";
...
async function openModel(modelSpec: string, storage: AuthStorage): Promise<OpenedModel> {
	...
	if (!apiKey) {
		throw new Error(`no credentials for provider "${provider}" (run \`yemu login\` or set the provider env var)`);
	}
