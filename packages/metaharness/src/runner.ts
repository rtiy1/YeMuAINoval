...
/**
 * Harbor benchmark runner for the local `yemu` build.
 *
 * Orchestrates Harbor (`harbor run`) against any Harbor dataset (default
 * terminal-bench-2) using a custom agent (`agent/yemu_local.py`) that installs
 * the working tree at /work/yemu and routes all model auth through the host pm2
...
const AGENT_IMPORT_PATH = "yemu_local:YemuLocal";

/** Container-side mount points for `--install source` (must match yemu_local.py defaults). */
const SOURCE_SRC_MOUNT = "/opt/yemu/src";
const SOURCE_BIN_MOUNT = "/opt/yemu/bin";
...
/**
 * Resolver injected into Apple Container runs (YEMU_BENCH_CONTAINER_DNS overrides).
 ...
 */
const CONTAINER_DNS = process.env.YEMU_BENCH_CONTAINER_DNS || "1.1.1.1";
...
export interface Config {
	...
	/** Extra args forwarded verbatim to the in-container yemu CLI invocation (repeatable). */
	agentArgs: string[];
...
const HELP = `metaharness runner (local yemu)
...
Model / agent:
  -m, --model <provider/model>   Model (repeatable). Default anthropic/claude-sonnet-4-6
      --agent <name>             yemu (default) | oracle | nop | any harbor agent
      --install <source|local|published> yemu install mode (default: source).
                                 source = mount /work/yemu read-only + prebuilt linux deps tree; TS changes
                                 apply per-trial with no rebuild. local = pack a tarball. published = npm.
      --version <v>              yemu version for published install (default: latest)
      --thinking <level>         off|minimal|low|medium|high|xhigh|max

      --tarball <path>           Reuse a prebuilt yemu tarball (implies --install local, --no-build)
      --no-build                 Skip packing; reuse newest tarball in bench dir (--install local)
      --agent-arg <arg>          Extra arg forwarded verbatim to the in-container yemu CLI (repeatable)
      --env <KEY[=VALUE]>        Forward env into yemu container (repeatable).
                                 KEY alone forwards host value; host YEMU_* auto-forwarded.
...
Gateway (auth, no keys in container):
      ...
      --web-search               Enable yemu web_search (off by default; can't auth via gateway)
 ...
 * `agent/yemu.txt` JSONL. Only bytes appended since the previous call are read
 ...
 */
function probeTrialCost(yemuLogPath: string): CostProbe | null {
	let size: number;
	try {
		size = fs.statSync(yemuLogPath).size;
	} catch {
		return costProbes.get(yemuLogPath) ?? null;
	}
	let probe = costProbes.get(yemuLogPath);
	if (!probe || size < probe.offset) {
		...
		costProbes.set(yemuLogPath, probe);
	}
	if (size === probe.offset) return probe;
	let fd: number;
	try {
		fd = fs.openSync(yemuLogPath, "r");
	} catch {
		return probe;
	}
	try {
...
function parseTrial(dir: string, name: string): Trial | null {
	...
	if (!fs.existsSync(resultPath)) {
		...
		// Realtime cost from the live agent yemu.txt log, parsed incrementally.
		const probe = probeTrialCost(path.join(dir, "agent", "yemu.txt"));
	...
	costProbes.delete(path.join(dir, "agent", "yemu.txt"));
...
function buildTarball(benchDir: string): string {
	process.stdout.write(dim("packing local yemu (bun pm pack)…\n"));
...
/** Linux deps tree + mount plan for running yemu straight from the mounted repo. */
...
function writeComposeOverlay(benchDir: string, cfg: Config, source: SourceMount | null): string | null {
	...
	const file = path.join(benchDir, "yemu-compose-overlay.yaml");
...
function writeModelsYaml(benchDir: string, cfg: Config): string {
	...
	for (const p of providers) {
		...
		lines.push("    transport: yemu-native");
...
function buildHarborArgs(
	...
	if (cfg.agent === "yemu") {
		// Config + secrets travel via env (YEMU_BENCH_*); the agent reads os.environ.
...
/**
 * Env vars injected into the in-container yemu run: every host `YEMU_*` knob (minus
...
export function collectForwardEnv(cfg: Config): Record<string, string> {
	...
	for (const [k, v] of Object.entries(process.env)) {
		if (v === undefined || !k.startsWith("YEMU_") || FORWARD_ENV_DENYLIST.has(k)) continue;
		out[k] = v;
	}
...
export function buildHarborEnv(
	...
	// Drop any stale YEMU_BENCH_FORWARD_ENV inherited from the caller's shell before
	// the agent-type early return, so it never leaks (incl. into the dry-run dump).
	delete env.YEMU_BENCH_FORWARD_ENV;
	if (cfg.agent !== "yemu") return env;
	const prepend = (k: string, v: string): void => {
		env[k] = env[k] ? `${v}:${env[k]}` : v;
	};
	prepend("PYTHONPATH", AGENT_DIR);
	env.YEMU_BENCH_INSTALL = cfg.install;
	env.YEMU_BENCH_VERSION = cfg.version ?? version;
	if (tarball) env.YEMU_BENCH_TARBALL = tarball;
	if (source) {
		env.YEMU_BENCH_SOURCE_DIR = SOURCE_SRC_MOUNT;
		env.YEMU_BENCH_SOURCE_BUN = `${SOURCE_BIN_MOUNT}/bun`;
		env.YEMU_BENCH_SOURCE_ARCH = source.arch;
	}
	if (cfg.binaryArm64) env.YEMU_BENCH_BINARY_ARM64 = cfg.binaryArm64;
	if (cfg.binaryX64) env.YEMU_BENCH_BINARY_X64 = cfg.binaryX64;
	if (cfg.thinking) env.YEMU_BENCH_THINKING = cfg.thinking;
	if (cfg.agentArgs.length > 0) env.YEMU_BENCH_AGENT_ARGS = JSON.stringify(cfg.agentArgs);
	if (cfg.webSearch) env.YEMU_BENCH_WEB_SEARCH = "1";
	env.YEMU_BENCH_GATEWAY = cfg.gateway ? "1" : "0";
	if (cfg.gateway) {
		env.YEMU_BENCH_MODELS_YAML = modelsYaml;
		env.YEMU_BENCH_GATEWAY_URL = cfg.gatewayUrl;
		env.YEMU_BENCH_GATEWAY_TOKEN = cfg.gatewayToken;
		env.YEMU_BENCH_GATEWAY_PROVIDERS = deriveProviders(cfg).join(",");
	}
	if (cfg.envType === "apple-container") env.YEMU_BENCH_CONTAINER_DNS = CONTAINER_DNS;
	const forward = collectForwardEnv(cfg);
	if (Object.keys(forward).length > 0) env.YEMU_BENCH_FORWARD_ENV = JSON.stringify(forward);
	return env;
}
...
async function runBenchmark(cfg: Config): Promise<BenchmarkRun> {
	...
	if (cfg.agent === "yemu" && cfg.envType === "docker" && !which("docker")) {
	...
	if (cfg.agent === "yemu" && cfg.install === "local" && !cfg.binaryArm64 && !cfg.binaryX64) {
		if (tarball) {
	...
	if (cfg.agent === "yemu" && cfg.install === "source" && !cfg.binaryArm64 && !cfg.binaryX64) {
		source = prepareSourceDeps(cfg);
	}

	// models.yml (gateway)
	let modelsYaml = "";
	if (cfg.agent === "yemu" && cfg.gateway) {
		modelsYaml = writeModelsYaml(benchDir, cfg);
		if (!gatewayHealthOk(cfg.gatewayUrl)) {
			process.stderr.write(
				yellow(
					`warning: gateway ${cfg.gatewayUrl} health check failed (continuing). Is the pm2 'yemu-auth-gateway' running?\n`,
				),
			);
		}
	}
	...
	if (cfg.dryRun) {
		...
		process.stdout.write(bold("yemu env:\n"));
		for (const key in harborEnv) {
			if (key === "YEMU_BENCH_FORWARD_ENV") continue;
			if (key.startsWith("YEMU_BENCH_") || key === "PYTHONPATH")
				process.stdout.write(`  ${key}=${harborEnv[key]}\n`);
		}
		if (harborEnv.YEMU_BENCH_FORWARD_ENV) {
			const parsedForwardEnv: unknown = JSON.parse(harborEnv.YEMU_BENCH_FORWARD_ENV);
			if (parsedForwardEnv !== null && typeof parsedForwardEnv === "object" && !Array.isArray(parsedForwardEnv)) {
				...
				process.stdout.write(`  YEMU_BENCH_FORWARD_ENV=${keys.join(",")} (values hidden)\n`);
			}
		}
		process.stdout.write(`\njob dir: ${jobDir}\nbench dir: ${benchDir}\n`);
