...
import type { AgentMessage, ResolvedThinkingLevel, ThinkingLevel } from "@yemu/agent-core";
import type { Model, ToolExample } from "@yemu/model-runtime";
import { formatSessionDumpText, RpcClient } from "@yemu/agent-runtime";
import { prompt } from "@yemu/utils";
import { diffLines } from "diff";
import { formatDirectory } from "@yemu/typescript-edit-benchmark/formatter";
import {
	discoverSharedInfra,
	InProcessClient,
	type SharedInfra,
} from "@yemu/typescript-edit-benchmark/in-process-client";
...
import type { EditTask } from "@yemu/typescript-edit-benchmark/tasks";
import {
	verifyExpectedFileSubset,
	verifyExpectedFiles,
} from "@yemu/typescript-edit-benchmark/verify";

const REPO_ROOT = path.resolve(import.meta.dir, "..", "..", "..", "..");
const RUNS_DIR = path.join(REPO_ROOT, "runs");
const TMP = path.join(RUNS_DIR, `rb-${Math.random().toString(36).slice(2, 10)}`);
const CLI_PATH = Bun.fileURLToPath(import.meta.resolve("@yemu/agent-runtime/cli"));
...
async function runSingleTask(
	...
	const previousEnv = {
		YEMU_EDIT_VARIANT: process.env.YEMU_EDIT_VARIANT,
		YEMU_EDIT_FUZZY: process.env.YEMU_EDIT_FUZZY,
		YEMU_EDIT_FUZZY_THRESHOLD: process.env.YEMU_EDIT_FUZZY_THRESHOLD,
		YEMU_STRICT_EDIT_MODE: process.env.YEMU_STRICT_EDIT_MODE,
		YEMU_NO_TITLE: process.env.YEMU_NO_TITLE,
	};
	try {
		...
		if (config.editVariant !== undefined) process.env.YEMU_EDIT_VARIANT = config.editVariant;
		if (config.editFuzzy !== undefined)
			process.env.YEMU_EDIT_FUZZY = config.editFuzzy === "auto" ? "auto" : config.editFuzzy ? "1" : "0";
		if (config.editFuzzyThreshold !== undefined)
			process.env.YEMU_EDIT_FUZZY_THRESHOLD =
				config.editFuzzyThreshold === "auto" ? "auto" : String(config.editFuzzyThreshold);
		process.env.YEMU_STRICT_EDIT_MODE = "1";
		process.env.YEMU_NO_TITLE = "1";
		...
		restoreEnvKey("YEMU_EDIT_VARIANT");
		restoreEnvKey("YEMU_EDIT_FUZZY");
		restoreEnvKey("YEMU_EDIT_FUZZY_THRESHOLD");
		restoreEnvKey("YEMU_STRICT_EDIT_MODE");
		restoreEnvKey("YEMU_NO_TITLE");
	}
