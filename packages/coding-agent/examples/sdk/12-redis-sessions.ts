 ...
 * `BlobStore` keep writing to `~/.yemu/agent/...`. Reach for an object store
 * (S3, R2, GCS) if you need those off-host too.
 */

import { createAgentSession, RedisSessionStorage, SessionManager } from "@yemu/agent-runtime";
...
const storage = await RedisSessionStorage.create({
	client: redis,
	prefix: "yemu:sessions:", // optional, this is the default
});
