...
export default function planModeExtension(yemu: ExtensionAPI) {
	...
	async function togglePlanMode(ctx: ExtensionContext) {
		...
		if (planModeEnabled) {
			await yemu.setActiveTools(PLAN_MODE_TOOLS);
			ctx.ui.notify(`Plan mode enabled. Tools: ${PLAN_MODE_TOOLS.join(", ")}`);
		} else {
			await yemu.setActiveTools(NORMAL_MODE_TOOLS);
	...
	yemu.registerShortcut(Key.shift("p"), {
	...
	yemu.on("tool_result", async (_event, ctx) => {
...
Execute each step in order.`,
	...
	yemu.on("agent_end", async (event, ctx) => {
