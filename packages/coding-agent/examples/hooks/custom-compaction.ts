 ...
 *
 * Usage:
 *   yemu --hook examples/hooks/custom-compaction.ts
 */

import type { HookAPI } from "@yemu/agent-runtime";
import { convertToLlm, serializeConversation } from "@yemu/agent-runtime";
import { complete, getModel } from "@yemu/model-runtime";

export default function (yemu: HookAPI) {
	yemu.on("session_before_compact", async (event, ctx) => {
