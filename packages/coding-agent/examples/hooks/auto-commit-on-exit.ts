...
export default function (yemu: HookAPI) {
	yemu.on("session_shutdown", async (_event, ctx) => {
		// Check for uncommitted changes
		const { stdout: status, code } = await yemu.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[yemu] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await yemu.exec("git", ["add", "-A"]);
		const { code: commitCode } = await yemu.exec("git", ["commit", "-m", commitMessage]);
