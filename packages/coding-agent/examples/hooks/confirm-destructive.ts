 ...
 */
import type { HookAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "@yemu/agent-runtime";

export default function (yemu: HookAPI) {
	yemu.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
