# Developing `@yemu/agent-runtime`

This package is the `yemu` CLI. This file is a **developer's map**: where things live
...
| Build the `dist/yemu` binary | `bun run build` |
...
and dispatches the hidden `__yemu_worker_*` argv selectors before loading the command
...
| `memories/`, `memory-backend/`, `yemu-memory/`, `hindsight/` | Memory subsystems and backends | [memory.md](../../docs/memory.md), [mnemosyne-memory-backend.md](../../docs/mnemosyne-memory-backend.md) |
...
- [toolconv/](../../docs/toolconv/) — per-family tool-call conversion (anthropic, harmony, qwen3, deepseek, glm-4.5, kimi-k2, gemini, gemma, yemu-native); see also [ERRATA-GPT5-HARMONY.md](../../docs/ERRATA-GPT5-HARMONY.md)
...
### Natives (`crates/yemu-native`, `packages/natives`)
...
- [porting-from-legacy-agent-monorepo.md](../../docs/porting-from-legacy-agent-monorepo.md)
