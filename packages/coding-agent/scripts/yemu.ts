/**
 * Bun `--preload` shim for the yemu dev launcher (`scripts/yemu`).
 *
 * The launcher starts Bun from an empty, bunfig-free directory so a foreign
 * project's `bunfig.toml` `preload` cannot run inside the yemu CLI: Bun reads
 ...
 * import-time snapshots (e.g. `getProjectDir()` in `@yemu/utils/dirs`).
 */
const launchCwd = process.env.YEMU_LAUNCH_CWD;
if (launchCwd) {
	delete process.env.YEMU_LAUNCH_CWD;
	try {
