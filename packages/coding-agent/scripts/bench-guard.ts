 ...
 *
 * Re-runs the `YEMU_TIMING=x` cold-boot benchmark under hyperfine and fails when
 * the median regresses past `baseline * THRESHOLD`. `YEMU_TIMING=x` runs the full
...
const BENCH_COMMAND = "YEMU_TIMING=x YEMU_STRICT_EDIT_MODE=1 bun src/cli.ts";
