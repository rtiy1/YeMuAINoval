...
import { isEnoent } from "@yemu/utils/fs-error";

/** Build-time specifier resolved to bundled legacy YeMu module namespaces. */
export const LEGACY_AGENT_MODULES_SPECIFIER = "yemu-legacy-agent-modules";

const VIRTUAL_NAMESPACE = "yemu-legacy-agent-modules-build";
...
const BUNDLED_PACKAGES: readonly BundledPackage[] = [
	{ dir: "agent", identifier: "YemuAgentCore", rootShim: null },
	{ dir: "ai", identifier: "YemuAi", rootShim: "legacy-model-runtime-shim.ts" },
	{ dir: "coding-agent", identifier: "YemuCodingAgent", rootShim: "legacy-agent-runtime-shim.ts" },
	{ dir: "natives", identifier: "YemuNatives", rootShim: null },
	{ dir: "tui", identifier: "YemuTui", rootShim: "legacy-agent-tui-shim.ts" },
	{ dir: "utils", identifier: "YemuUtils", rootShim: null },
];
...
export interface BundledYemuEntry {
...
/**
 * Derive the bundled legacy YeMu module surface from current package exports.
 ...
 */
export async function collectBundledYemuEntries(): Promise<BundledYemuEntry[]> {
	const entries: BundledYemuEntry[] = [];
	const seenKeys = new Set<string>();
	const seenBindings = new Set<string>();
	function addEntry(key: string, binding: string, importSpecifier: string): void {
		if (seenKeys.has(key)) return;
		if (seenBindings.has(binding)) {
			throw new Error(`Duplicate bundled YeMu binding ${binding} for ${key}`);
		}
		seenKeys.add(key);
	...
	for (const pkg of BUNDLED_PACKAGES) {
		...
		if (!isRecord(manifest) || typeof manifest.name !== "string") {
			throw new Error(`Bundled YeMu package manifest has no name: ${manifestPath}`);
		}
...
export function __renderLegacyAgentVirtualModule(entries: readonly BundledYemuEntry[]): string {
	...
	return [...loaders, "", "export const BUNDLED_YEMU_MODULE_LOADERS = {", ...modules, "};", ""].join("\n");
}

/**
 * Build plugin that materializes lazy legacy YeMu module loaders entirely in
 ...
 */
export async function createLegacyAgentVirtualModulePlugin(): Promise<Bun.BunPlugin> {
	const source = __renderLegacyAgentVirtualModule(await collectBundledYemuEntries());
	return {
		name: "yemu:legacy-agent-modules",
		setup(build) {
			build.onResolve({ filter: /^yemu-legacy-agent-modules$/ }, () => ({
				path: LEGACY_AGENT_MODULES_SPECIFIER,
