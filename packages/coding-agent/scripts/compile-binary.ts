...
import { createLegacyAgentVirtualModulePlugin } from "./legacy-agent-virtual-module";
...
/**
 * Compile the coding-agent executable with its legacy YeMu compatibility module
...
export async function compileCodingAgent(options: CodingAgentCompileOptions): Promise<void> {
	...
	try {
		const output = await Bun.build({
			...
			define: {
				"process.env.YEMU_COMPILED": JSON.stringify("true"),
				"process.env.YEMU_TINY_TRANSFORMERS_VERSION": JSON.stringify(options.transformersVersion),
				"process.env.YEMU_DOCS_EMBED": JSON.stringify((await buildDocsIndexPayload()).payload),
			},
			minify: {
				identifiers: options.minifyIdentifiers ?? false,
				keepNames: true,
			},
			plugins: [await createLegacyAgentVirtualModulePlugin()],
			compile: {
