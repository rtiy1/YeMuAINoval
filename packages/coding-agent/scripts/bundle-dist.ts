...
// `yemu-legacy-agent-modules` exists only in compiled binaries via the build plugin;
...
async function cleanBundleOutputs(): Promise<void> {
	// dist/ is shared with the dev binary (dist/yemu); only remove assets
...
async function main(): Promise<void> {
	...
	try {
		...
		const output = await Bun.build({
			...
			define: {
				"process.env.YEMU_BUNDLED": JSON.stringify("true"),
