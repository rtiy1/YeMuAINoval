...
import { usesCodexTaskPrompt } from "@yemu/agent-runtime/task/prompt-policy";
...
it("renders the prompt date in the startup timezone", async () => {
	setSystemTime(new Date(process.env.YEMU_TEST_NOW!));
	try {
		...
		expect(rendered).toContain(\`Today is \${process.env.YEMU_EXPECTED_DATE}\`);
		expect(rendered).not.toContain(\`Today is \${process.env.YEMU_REJECTED_DATE}\`);
	} finally {
		setSystemTime();
	}
});
`,
	...
	const child = Bun.spawn([process.execPath, "test", scenarioPath], {
		...
		env: {
			...
			TZ: options.timeZone,
			YEMU_TEST_NOW: options.now,
			YEMU_EXPECTED_DATE: options.expectedDate,
			YEMU_REJECTED_DATE: options.rejectedDate,
		},
		stdout: "pipe",
		stderr: "pipe",
	});
...
describe("system prompt model identifier", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-prompt-model-"));
		tempHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-prompt-model-home-"));
...
describe("AgentSession model-change prompt refresh", () => {
	...
	beforeEach(async () => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-prompt-model-session-"));
