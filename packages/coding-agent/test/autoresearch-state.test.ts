...
import { createAutoresearchExtension } from "@yemu/agent-runtime/autoresearch";
import {
...
} from "@yemu/agent-runtime/autoresearch/state";
import { AutoresearchStorage, closeAllAutoresearchStorages } from "@yemu/agent-runtime/autoresearch/storage";
import type { ExperimentResult } from "@yemu/agent-runtime/autoresearch/types";
import type {
	ExtensionAPI,
...
function makeTempDir(): TempDir {
	return TempDir.createSync("@yemu-autoresearch-test-");
}
...
describe("autoresearch slash command", () => {
	...
	beforeEach(() => {
		dbOverride = TempDir.createSync("@yemu-autoresearch-cmd-");
...
describe("autoresearch tool-call hook", () => {
	...
	beforeEach(() => {
		dbOverride = TempDir.createSync("@yemu-autoresearch-hook-");
		process.env.YEMU_AUTORESEARCH_DB_DIR = dbOverride.path();
		cleanups.push(dbOverride);
	});
