...
import type { AsyncJobSnapshot } from "@yemu/agent-runtime/session/agent-session";
...
describe("AsyncJobManager singleton across concurrent top-level sessions", () => {
	...
	beforeAll(async () => {
		sharedTempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-sdk-async-singleton-shared-"));
	...
	async function spawnTopLevelSession(extraSettings?: Record<string, unknown>, extensions: ExtensionFactory[] = []) {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `yemu-sdk-async-singleton-${Snowflake.next()}-`));
	...
	it("exposes the owning session's jobs through a production extension context", async () => {
		...
		const snapshotExtension: ExtensionFactory = yemu => {
			yemu.registerTool({
	...
	it("clears a manager installed before a top-level session startup failure takes ownership", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `yemu-sdk-async-startup-failure-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
		const cwd = path.join(tempDir, `project-${Snowflake.next()}`);
		const agentDir = path.join(tempDir, "agent");
		fs.mkdirSync(cwd, { recursive: true });
