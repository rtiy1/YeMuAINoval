...
import * as pythonExecutor from "@yemu/agent-runtime/eval/py/executor";
import type { PythonKernel as PythonKernelInstance } from "@yemu/agent-runtime/eval/py/kernel";
import * as pythonKernel from "@yemu/agent-runtime/eval/py/kernel";
import { AgentRegistry } from "@yemu/agent-runtime/registry/agent-registry";
import { createAgentSession, type ExtensionFactory, type WorkspaceTree } from "@yemu/agent-runtime/sdk";
...
import { Snowflake, TempDir } from "@yemu/utils";
...
const createTempProject = () => {
	const tempDir = TempDir.createSync(`@yemu-agent-session-python-cleanup-${Snowflake.next()}-`);
...
const createAgentDir = (): string => {
	const dir = TempDir.createSync("@yemu-python-cleanup-agentdir-");
