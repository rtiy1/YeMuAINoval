...
import {
	...
	type TtsrScanArgs,
	type TtsrTestArgs,
} from "@yemu/agent-runtime/cli/ttsr-cli";
import { resetSettingsForTest } from "@yemu/agent-runtime/config/settings";
import { getProjectAgentDir, getProjectDir, removeSyncWithRetries, setProjectDir } from "@yemu/utils";

let testTmpDir: string;

beforeAll(() => {
	testTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-ttsr-tests-"));
});
...
describe("yemu ttsr", () => {
	...
	describe("test — context inference and matching", () => {
		it("infers tool/edit context when a positional resolves to a .ts file and --source is omitted", async () => {
			...
			// Simulate `yemu ttsr test --rule <rule> src/foo.ts`: the command layer
