...
import { clearWorktrees } from "@yemu/agent-runtime/cli/worktree-cli";
import { ISOLATION_OWNER_FILE, writeIsolationOwner } from "@yemu/agent-runtime/task/isolation-ownership";
import { setWorktreesDir } from "@yemu/utils";

/**
 * Regression for #6761: `yemu worktree clear` (no `--all`) must delete only
 * task-isolation sandboxes whose owner process is gone. A sandbox owned by a
 * live yemu process holds a running subagent's uncaptured work and must survive.
 */
describe("worktree clear task-isolation ownership", () => {
	...
	beforeEach(async () => {
		base = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-wt-clear-"));
		savedEnv = process.env.YEMU_WORKTREE_DIR;
	...
	afterEach(async () => {
		...
		if (savedEnv === undefined) delete process.env.YEMU_WORKTREE_DIR;
		else process.env.YEMU_WORKTREE_DIR = savedEnv;
		vi.restoreAllMocks();
