...
import { buildSpec, type CompletionSpec, generateCompletion } from "@yemu/agent-runtime/cli/completion-gen";
import type { CliConfig, CommandCtor } from "@yemu/utils/cli";
...
describe("generateCompletion — bash", () => {
	...
	it("registers the dispatcher and resolves alias arms to the canonical handler", () => {
		expect(out).toContain("complete -F _yemu yemu");
		expect(out).toContain("_yemu_cmd_commit");
	...
	it("completes enum, dynamic, and comma-list flag values by previous flag", () => {
		...
		expect(out).toContain('--model)\n\t\t\tCOMPREPLY=( $(compgen -W "$(command yemu __complete models -- "$cur"');
		expect(out).toContain("--resume|-r)");
		expect(out).toContain("command yemu __complete sessions");
		// static comma list routes through the comma-aware helper
		expect(out).toContain('--tools)\n\t\t\t_yemu_comma "read bash"');
	...
	it("completes a subcommand's positional enum and its own flags", () => {
		expect(out).toContain("_yemu_cmd_worktree()");
...
describe("generateCompletion — zsh", () => {
	...
	it("emits the compdef header and dual-mode (autoload + eval) tail", () => {
		expect(out.startsWith("#compdef yemu")).toBe(true);
		expect(out).toContain('if [ "$funcstack[1]" = "_yemu" ]; then');
		expect(out).toContain("compdef _yemu yemu");
	});

	it("maps value sources to the right _arguments actions", () => {
		expect(out).toContain("'--model[Model to use]:model:_yemu_call models'");
		expect(out).toContain("'--models[Model list]:models:_yemu_models_list'");
		expect(out).toContain("'--thinking[Effort]:value:(low high)'");
		expect(out).toContain("'--tools[Tools]:value:_yemu_tools'");
		expect(out).toContain("'(-r --resume)'{-r,--resume}'[Resume]:session:_yemu_call sessions'");
		...
		expect(out).toContain("_yemu_tools() { _values -s , 'tools' read bash }");
	});

	it("dispatches aliased subcommands and completes positional enums", () => {
		expect(out).toContain("worktree|wt) _yemu_cmd_worktree ;;");
...
describe("generateCompletion — fish", () => {
	...
	it("declares the no-subcommand predicate over every command token", () => {
		expect(out).toContain("function __fish_yemu_no_subcommand");
	...
	it("maps value sources to fish completion args", () => {
		expect(out).toContain("-l model -d 'Model to use' -x -a '(command yemu __complete models -- (commandline -ct))'");
		expect(out).toContain("-l thinking -d 'Effort' -x -a 'low high'");
		expect(out).toContain("-l tools -d 'Tools' -x -a 'read bash'");
		expect(out).toContain("-s r -l resume -d 'Resume' -x -a '(command yemu __complete sessions");
...
describe("yemu completions (integration / drift)", () => {
	it("emits a zsh script reflecting the live command + flag surface", async () => {
		const proc = Bun.spawn([process.execPath, cliEntry, "completions", "zsh"], {
			...
			stdout: "pipe",
			stderr: "pipe",
			env: { ...process.env, NO_COLOR: "1", YEMU_NO_TITLE: "1" },
		});
		...
		expect(stdout).toContain("_yemu_cmd_commit");
		expect(stdout).toContain("'completions:");
		// zsh routes single-value dynamic flags through the _yemu_call action, which
		// itself shells out to `yemu __complete $kind`.
		expect(stdout).toContain("_yemu_call models");
		expect(stdout).toContain("_yemu_call sessions");
		expect(stdout).toContain("command yemu __complete $kind");
		// Hidden/default commands must NOT surface as completable subcommands.
		expect(stdout).not.toContain("_yemu_cmd_launch");
		expect(stdout).not.toContain("_yemu_cmd___complete");
	});
});
