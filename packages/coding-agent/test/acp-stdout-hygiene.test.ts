/**
 * ACP stdout-hygiene smoke: launching `yemu acp` must not leak any banner,
 ...
 * unwind, but force-kill quickly if it hasn't reaped — `yemu acp` blocks on
...
describe("ACP stdout hygiene", () => {
	it("emits a JSON-RPC initialize response as the first bytes on stdout", async () => {
		const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "yemu-acp-stdout-"));
		...
		// run (~12s cold vs ~0.4s warm). XDG_* and YEMU_CODING_AGENT_DIR
		// already isolate YeMu's on-disk state for this smoke test.
