...
describe("profile alias installer", () => {
	it("writes a bash-compatible function that forwards subcommands through yemu", async () => {
		...
		expect(result.command).toBe("yemu --profile=work");
		expect(files.get("/home/me/.bashrc")).toContain("yemu-work() {");
		expect(files.get("/home/me/.bashrc")).toContain('command yemu --profile=work "$@"');
	});
	...
	it("can target the current source invocation instead of the installed yemu binary", async () => {
		const files = new Map<string, string>();
		...
		expect(files.get("/Users/me/.zshrc")).toContain("yemu-work() {");
	...
	it("installs the zsh alias under ZDOTDIR when set", async () => {
		...
		expect(files.get(result.configPath)).toContain("yemu-work() {");
	});
	...
	it("writes a fish function that forwards argv", async () => {
		...
		const content = files.get("/Users/me/.config/fish/conf.d/yemu-profiles.fish") ?? "";
		expect(content).toContain("function yemu-work --wraps yemu");
		expect(content).toContain("command yemu --profile=work $argv");
	});
	...
	it("installs the fish alias under XDG_CONFIG_HOME when set", async () => {
		...
		expect(result.configPath).toBe("/home/me/.dotfiles/config/fish/conf.d/yemu-profiles.fish");
		expect(files.get(result.configPath)).toContain("function yemu-work --wraps yemu");
	});
	...
	it("writes a PowerShell function because aliases cannot carry arguments", async () => {
		...
		expect(content).toContain("function yemu-work");
		expect(content).toContain("& yemu --profile=work @args");
	});
	...
	it("detects pwsh from PSModulePath when SHELL is unset on Windows", async () => {
		...
		expect(files.get(result.configPath)).toContain("& yemu --profile=work @args");
	});
	...
	it("replaces a previous block for the same alias", async () => {
		const files = new Map<string, string>([
			[
				...
				[
					"before",
					"# >>> yemu profile alias: yemu-work >>>",
					"alias yemu-work='command yemu --profile=old'",
					"# <<< yemu profile alias: yemu-work <<<",
					"after",
				].join("\n"),
			],
		...
		expect(content).toContain('command yemu --profile=work "$@"');
	...
	it("refuses to rewrite a malformed managed block missing its end marker", async () => {
		...
		const original = ["# >>> yemu profile alias: yemu-work >>>", "yemu-work() {", "export SECRET=keepme"].join("\n");
	...
	it("refuses to shadow the base yemu command case-insensitively", async () => {
		for (const aliasName of ["yemu", "YeMu"]) {
			await expect(
				installProfileAlias({
					profile: "work",
					aliasName,
	...
	it("validates profile names before rendering shell code", async () => {
		...
		).rejects.toThrow("Invalid YeMu profile");
	...
	it("normalizes backslashes in XDG_CONFIG_HOME for fish config paths on Windows", async () => {
		...
		expect(result.configPath).toBe("D:/xdg/fish/conf.d/yemu-profiles.fish");
		expect(result.reloadedWith).toBe("source 'D:/xdg/fish/conf.d/yemu-profiles.fish'");
	});
