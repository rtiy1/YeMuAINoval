...
import { runConfigCommand } from "@yemu/agent-runtime/cli/config-cli";
...
import { getConfigRootDir, setAgentDir, TempDir } from "@yemu/utils";
...
describe("credential settings", () => {
	...
	it("classifies UI-visible credentials through the same marker", () => {
		...
		for (const path of ["yemu-memory.embeddingApiKey", "yemu-memory.llmApiKey"] as const) {
			expect(isCredential(path)).toBe(true);
		}
	});
...
describe("credential masking reaches every surface", () => {
	it("masks a UI-visible credential in the settings panel", () => {
		...
		// other.
		for (const path of ["hindsight.apiToken", "yemu-memory.embeddingApiKey", "yemu-memory.llmApiKey"] as const) {
...
describe("config list output", () => {
	...
	beforeEach(() => {
		...
		agentDir = TempDir.createSync("@yemu-config-credentials-");
