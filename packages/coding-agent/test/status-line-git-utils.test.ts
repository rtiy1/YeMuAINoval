...
import {
	...
	parseDefaultBranch,
	parseGitHubRepo,
} from "@yemu/agent-runtime/modes/components/status-line/git-utils";

describe("parseGitHubRepo", () => {
	test("parses HTTPS URL", () => {
		expect(parseGitHubRepo("https://github.com/rtiy1/YeMuAINoval.git")).toBe("rtiy1/YeMuAINoval");
	});

	test("parses HTTPS URL without .git suffix", () => {
		expect(parseGitHubRepo("https://github.com/rtiy1/YeMuAINoval")).toBe("rtiy1/YeMuAINoval");
	});

	test("parses SSH scp-style URL", () => {
		expect(parseGitHubRepo("git@github.com:loftiskg/yemu-agent.git")).toBe("loftiskg/yemu-agent");
	});

	test("parses SSH scp-style URL without .git suffix", () => {
		expect(parseGitHubRepo("git@github.com:loftiskg/yemu-agent")).toBe("loftiskg/yemu-agent");
	});
