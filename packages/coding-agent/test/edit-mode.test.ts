...
import { type EditMode, type EditModeSessionLike, resolveEditMode } from "@yemu/agent-runtime/utils/edit-mode";

const originalEditVariant = Bun.env.YEMU_EDIT_VARIANT;
const originalStrictEditMode = Bun.env.YEMU_STRICT_EDIT_MODE;
...
function restoreEnv(): void {
	...
	if (originalStrictEditMode === undefined) {
		delete Bun.env.YEMU_STRICT_EDIT_MODE;
	} else {
		Bun.env.YEMU_STRICT_EDIT_MODE = originalStrictEditMode;
	}
}
...
describe("resolveEditMode", () => {
	...
	test("keeps YEMU_EDIT_VARIANT ahead of the Kimi fallback", () => {
		Bun.env.YEMU_EDIT_VARIANT = "hashline";
	...
	test("keeps strict edit mode ahead of the Kimi fallback", () => {
		delete Bun.env.YEMU_EDIT_VARIANT;
		Bun.env.YEMU_STRICT_EDIT_MODE = "1";
