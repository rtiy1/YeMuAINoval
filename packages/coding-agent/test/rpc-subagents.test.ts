...
import { RpcClient } from "@yemu/agent-runtime/modes/rpc/rpc-client";
import {
	handleRpcSessionChange,
	type RpcSessionChangeCommand,
	type RpcSessionChangeResult,
	type RpcSessionChangeSession,
} from "@yemu/agent-runtime/modes/rpc/rpc-mode";
import { RpcSubagentRegistry, readRpcSubagentTranscript } from "@yemu/agent-runtime/modes/rpc/rpc-subagents";
import type { RpcSubagentFrame } from "@yemu/agent-runtime/modes/rpc/rpc-types";
import {
	type AgentProgress,
...
describe("readRpcSubagentTranscript", () => {
	test("returns complete JSONL entries and byte cursor", async () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-rpc-subagent-transcript-"));
		tempPaths.push(dir);
	...
	test("returns empty cursor result for missing transcript files", async () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-rpc-subagent-transcript-missing-"));
		tempPaths.push(dir);
...
describe("RpcClient subagent frames", () => {
	test("dispatches subagent frames and session-specific events", async () => {
		const scriptPath = path.join(os.tmpdir(), `yemu-rpc-subagent-client-${Date.now()}.js`);
