...
import { HookSelectorComponent } from "@yemu/agent-runtime/modes/components/hook-selector";
import { getThemeByName, setThemeInstance, theme } from "@yemu/agent-runtime/modes/theme/theme";
...
describe("HookSelectorComponent", () => {
	...
	it("wraps outlined option text without omitting the tail", () => {
		const options = [
			"Option A: Move to YeMu-native only by migrating reusable shared AI instructions into .yemu/AGENTS.md, .yemu/rules, .yemu/skills, and .yemu/agents while deliberately not creating a root .github directory.",
			"Option B: Keep dual support by migrating canonical instructions into .yemu while also maintaining a root .github/copilot-instructions.md compatibility bridge for editors that do not understand YeMu resources yet.",
		];
		const component = new HookSelectorComponent(
		...
		expect(normalizedPlain).toContain("do not understand YeMu resources yet");
		for (const line of lines) {
			expect(visibleWidth(Bun.stripANSI(line))).toBeLessThanOrEqual(width);
		}
	});
		...
		const options = [
			{
				...
				description:
					"Authenticate via the provider keys and OAuth state already configured under ~/.yemu without opening a new browser-based setup flow.",
			},
			{
				label: "Set up YeMu Agent in terminal",
				description:
