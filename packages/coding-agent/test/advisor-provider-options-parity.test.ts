 ...
 *
 * Regression for rtiy1/YeMuAINoval#3639: before the fix, the advisor was built
...
import type { FetchImpl, Model, SimpleStreamOptions } from "@yemu/model-runtime";
...
describe("AgentSession advisor provider-options parity", () => {
	...
	beforeAll(async () => {
		sharedDir = TempDir.createSync("@yemu-advisor-parity-shared-");
	...
	beforeEach(async () => {
		tempDir = TempDir.createSync("@yemu-advisor-parity-");
