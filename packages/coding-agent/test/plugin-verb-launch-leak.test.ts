 ...
 * `yemu list` / `yemu remove` / `yemu marketplace <sub>` / `yemu uninstall …` etc.
 * as top-level commands, but only `yemu install` is registered. Before the fix,
 * `resolveCliArgv(["list"])` rewrote the bare verb to `["launch", "list"]`, so
 * `yemu list` silently started an interactive agent session with "list" as the
 * initial LLM prompt instead of managing plugins (the real command is
 * `yemu plugin list`). #4845 extended the same footgun to the multi-word
 * documented grammar: `yemu marketplace add xyz` leaked the whole argv to the
 * model as a prompt.
 *
 ...
 * `yemu plugin <action>` command rather than leaking to the model — while
 * genuine prose prompts that merely begin with one of these words still fall
 * through to `launch`.
 *
 * Imported via a relative path (not the `@yemu/agent-runtime` alias) so the
...
describe("documented-but-unregistered plugin verbs do not leak to launch (#2935)", () => {
	test("bare `yemu list` hints at `yemu plugin list` instead of launching with 'list' as the prompt", () => {
		...
		expect("error" in result && result.error).toContain("yemu plugin list");
	});

	test("bare `yemu remove` hints at `yemu plugin uninstall` instead of launching with 'remove' as the prompt", () => {
		const result = resolveCliArgv(["remove"]);
		expect(result).not.toEqual({ argv: ["launch", "remove"] });
		expect(result).not.toHaveProperty("argv");
		expect(result).toHaveProperty("error");
		expect("error" in result && result.error).toContain("yemu plugin uninstall");
	});
	...
	test("multi-word `yemu marketplace add xyz` hints at `yemu plugin marketplace` instead of leaking to the prompt (#4845)", () => {
		const result = resolveCliArgv(["marketplace", "add", "xyz"]);
		expect(result).not.toEqual({ argv: ["launch", "marketplace", "add", "xyz"] });
		expect(result).not.toHaveProperty("argv");
		expect(result).toHaveProperty("error");
		expect("error" in result && result.error).toContain("yemu plugin marketplace");
	});

	test("bare marketplace-family verbs hint at their `yemu plugin` command (#4845)", () => {
		for (const [verb, hint] of [
			["marketplace", "yemu plugin marketplace"],
			["discover", "yemu plugin discover"],
			["upgrade", "yemu plugin upgrade"],
			["uninstall", "yemu plugin uninstall"],
			["enable", "yemu plugin enable"],
			["disable", "yemu plugin disable"],
		] as const) {
