...
import { resolveCmuxKind } from "@yemu/agent-runtime/tools/browser";
...
describe("resolveCmuxKind", () => {
	...
	it("YEMU_BROWSER_CMUX=0 disables cmux even when the socket environment is present", () => {
		expect(resolveCmuxKind(null, { CMUX_SOCKET_PATH: "/tmp/cmux.sock", YEMU_BROWSER_CMUX: "0" })).toBeNull();
	});

	it("YEMU_BROWSER_CMUX=1 enables cmux over a disabled setting", () => {
		expect(
			resolveCmuxKind({ settingEnabled: false }, { CMUX_SOCKET_PATH: "/tmp/cmux.sock", YEMU_BROWSER_CMUX: "1" }),
		).toEqual({
			kind: "cmux",
			socketPath: "/tmp/cmux.sock",
			password: undefined,
