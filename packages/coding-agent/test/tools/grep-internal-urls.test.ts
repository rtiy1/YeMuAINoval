...
} from "@yemu/agent-runtime/internal-urls";
import { AgentRegistry } from "@yemu/agent-runtime/registry/agent-registry";
import * as sshFileTransfer from "@yemu/agent-runtime/ssh/file-transfer";
...
describe("GrepTool internal URL resolution", () => {
	...
	it("expands yemu:// root to grep embedded documentation files", async () => {
		const session = createSession();
		const tool = new GrepTool(session);
		...
		const text = getResultText(result);
		expect(text).toContain("# yemu://tools/grep.md");
		expect(text).toContain("Grep file contents with a regex across files");
	});

	it("expands yemu://docs to grep embedded documentation files", async () => {
		const session = createSession();
		const tool = new GrepTool(session);
		...
		const text = getResultText(result);
		expect(text).toContain("# yemu://tools/read.md");
