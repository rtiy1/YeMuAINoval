...
import { handleGitHub, parseGitHubUrl, stripActionsLogTimestamps } from "@yemu/agent-runtime/web/scrapers/github";
import { handleGitHubGist } from "@yemu/agent-runtime/web/scrapers/github-gist";
...
describe("parseGitHubUrl — Actions", () => {
	it("classifies a workflow run URL", () => {
		const gh = parseGitHubUrl("https://github.com/rtiy1/YeMuAINoval/actions/runs/27070071296");
		expect(gh).toEqual({ type: "actions-run", owner: "rtiy1", repo: "YeMuAINoval", runId: 27070071296 });
	});

	it("classifies a job URL using the web-form singular `job` segment", () => {
		const gh = parseGitHubUrl("https://github.com/rtiy1/YeMuAINoval/actions/runs/27070071296/job/79897931171");
		expect(gh).toEqual({
			type: "actions-job",
			owner: "rtiy1",
			repo: "YeMuAINoval",
	...
	it("classifies a job URL using the API-form plural `jobs` segment", () => {
		const gh = parseGitHubUrl("https://github.com/rtiy1/YeMuAINoval/actions/runs/27070071296/jobs/79897931171");
	...
	it("does not treat non-run Actions URLs (e.g. workflow files) as runs/jobs", () => {
		expect(parseGitHubUrl("https://github.com/rtiy1/YeMuAINoval/actions/workflows/ci.yml")?.type).toBe("other");
		expect(parseGitHubUrl("https://github.com/rtiy1/YeMuAINoval/actions")?.type).toBe("other");
	});

	it("does not misparse a run URL with a non-numeric id", () => {
		expect(parseGitHubUrl("https://github.com/rtiy1/YeMuAINoval/actions/runs/latest")?.type).toBe("other");
	});
...
describe("parseGitHubUrl — commit", () => {
	it("classifies a commit URL with a full SHA", () => {
		const gh = parseGitHubUrl("https://github.com/rtiy1/YeMuAINoval/commit/c1a1cb6149e73b345919dd4cf629b0d9ac74fb57");
		expect(gh).toEqual({
			type: "commit",
			owner: "rtiy1",
			repo: "YeMuAINoval",
	...
	it("accepts an abbreviated SHA", () => {
		expect(parseGitHubUrl("https://github.com/rtiy1/YeMuAINoval/commit/c1a1cb6")).toEqual({
			type: "commit",
			owner: "rtiy1",
			repo: "YeMuAINoval",
			ref: "c1a1cb6",
	...
	it("falls back to `other` for a bare /commit segment with no SHA", () => {
		expect(parseGitHubUrl("https://github.com/rtiy1/YeMuAINoval/commit")?.type).toBe("other");
	});
});
