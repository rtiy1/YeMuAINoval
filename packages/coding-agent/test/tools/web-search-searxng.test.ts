...
import { searchSearXNG } from "@yemu/agent-runtime/web/search/providers/searxng";
import { SearchProviderError } from "@yemu/agent-runtime/web/search/types";
...
describe("SearXNG web search provider", () => {
	...
	it("demotes engine-hostile operators while keeping bare-domain site: filters", async () => {
		...
		await searchSearXNG({
			query: "site:github.com/rtiy1/YeMuAINoval inurl:releases site:github.com 17.1.1 release",
			fetch: fetchMock,
		});

		expect(captured.q).toBe("17.1.1 release github.com/rtiy1/YeMuAINoval releases site:github.com");
	});
