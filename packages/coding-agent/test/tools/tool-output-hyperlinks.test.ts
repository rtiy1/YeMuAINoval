...
import { resetSettingsForTest, Settings, settings } from "@yemu/agent-runtime/config/settings";
import { editToolRenderer } from "@yemu/agent-runtime/edit/renderer";
import { getThemeByName, initTheme } from "@yemu/agent-runtime/modes/theme/theme";
import type { ToolSession } from "@yemu/agent-runtime/tools";
import { astGrepToolRenderer } from "@yemu/agent-runtime/tools/ast-grep";
import { ReadTool, readToolRenderer } from "@yemu/agent-runtime/tools/read";
import { WriteTool, writeToolRenderer } from "@yemu/agent-runtime/tools/write";
...
describe("tool output OSC 8 file:// hyperlinks", () => {
	it("links plain text and image read titles to the resolved filesystem path", async () => {
		...
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-link-read-"));
		try {
	...
	it("links the write header to the absolute path it wrote", async () => {
		settings.override("tui.hyperlinks", "always");
		const theme = (await getThemeByName("dark"))!;
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-link-write-"));
		try {
	...
	it("resolves scoped search links against cwd, not the (sub)scope path", async () => {
		...
		const projectRoot = path.resolve("/tmp/yemu-project");
	...
	it("links the edit header to the absolute details.path even when the arg path is relative", async () => {
		settings.override("tui.hyperlinks", "always");
		const theme = (await getThemeByName("dark"))!;
		const editPath = path.resolve("/tmp/yemu-project/src/a.ts");
