...
import type { CmuxKind } from "@yemu/agent-runtime/tools/browser/cmux/rpc";
import { CmuxSocketClient } from "@yemu/agent-runtime/tools/browser/cmux/socket-client";
import { acquireBrowser, getBrowsersMapForTest } from "@yemu/agent-runtime/tools/browser/registry";
import {
	acquireTab,
	getTabsMapForTest,
	releaseTab,
	releaseTabsForOwner,
} from "@yemu/agent-runtime/tools/browser/tab-supervisor";
import { ToolAbortError } from "@yemu/agent-runtime/tools/tool-errors";

function makeKind(socketSuffix: string): CmuxKind {
	return { kind: "cmux", socketPath: `/tmp/yemu-test-${socketSuffix}.sock`, surface: `surface-${socketSuffix}` };
}
...
describe("browser lifecycle — close deadlines", () => {
	...
	it("rejects a stuck close with the backend, tab, and pending resource", async () => {
		...
		const kind: CmuxKind = { kind: "cmux", socketPath: "/tmp/yemu-close-deadline.sock" };
		const browser = await acquireBrowser(kind, { cwd: "/tmp" });
