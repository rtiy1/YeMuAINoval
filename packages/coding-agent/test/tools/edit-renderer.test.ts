...
import { renderGalleryState, resolveFixture } from "@yemu/agent-runtime/cli/gallery-cli";
...
import { renderDiff } from "@yemu/agent-runtime/modes/components/diff";
import { ToolExecutionComponent } from "@yemu/agent-runtime/modes/components/tool-execution";
import * as themeModule from "@yemu/agent-runtime/modes/theme/theme";
import { InMemorySnapshotStore } from "@yemu/hashline";
import { Text, type TUI, visibleWidth } from "@yemu/tui";
...
describe("editToolRenderer", () => {
	...
	it("shows hashline envelope target path while preview diff is not computable yet", async () => {
		...
		const rendered = Bun.stripANSI(component.render(160).join("\n"));
		expect(rendered).toContain("crates/yemu-native/src/shell.rs");
