...
import type { CustomToolContext } from "@yemu/agent-runtime/extensibility/custom-tools";
import type { ReadonlySessionManager } from "@yemu/agent-runtime/session/session-manager";
import {
	getImageGenTools,
	getImageGenToolsWithRegistry,
	imageGenTool,
	setImageProviderOrder,
} from "@yemu/agent-runtime/tools/image-gen";
...
describe("imageGenTool", () => {
	...
	it("routes xAI image generation with xAI-only aspect ratios", async () => {
		...
		expect(requestUrl).toBe("https://api.x.ai/v1/images/generations");
		expect(captured.authorization).toBe("Bearer test-xai-token");
		expect(captured.userAgent).toBe("yemu-agent/xai");
		expect(requestBody).toMatchObject({
