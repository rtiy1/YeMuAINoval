...
import { BrowserTool } from "@yemu/agent-runtime/tools/browser";
import { CmuxSocketClient } from "@yemu/agent-runtime/tools/browser/cmux/socket-client";
import { getBrowsersMapForTest } from "@yemu/agent-runtime/tools/browser/registry";
import { getTabsMapForTest, releaseTab } from "@yemu/agent-runtime/tools/browser/tab-supervisor";
import type { ToolSession } from "@yemu/agent-runtime/tools/index";
import { ToolAbortError, ToolError } from "@yemu/agent-runtime/tools/tool-errors";
...
beforeEach(() => {
	...
	process.env.CMUX_SOCKET_PATH = `/tmp/yemu-open-lease-${process.pid}-${Math.random().toString(36).slice(2)}.sock`;
});
