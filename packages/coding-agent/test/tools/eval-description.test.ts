...
import { EvalTool, getEvalToolDescription } from "@yemu/agent-runtime/tools/eval";
import type { Tool as AiTool } from "@yemu/model-runtime";
...
describe("eval tool dynamic schema", () => {
	// resolveEvalBackends lets YEMU_* env flags override settings; neutralize them per-test
	// so the schema is driven purely by the isolated settings (and restore to avoid leaks).
	const EVAL_ENV_FLAGS = ["YEMU_PY", "YEMU_JS", "YEMU_RB", "YEMU_JL"] as const;
