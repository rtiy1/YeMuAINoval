...
import { resolveExplicitSearchPaths } from "@yemu/agent-runtime/tools/path-utils";
...
describe.skipIf(isWindows)("search with omitted paths", () => {
	...
	beforeEach(async () => {
		cwd = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-search-default-cwd-"));
...
describe.skipIf(isWindows)("search across unrelated filesystem trees", () => {
	...
	beforeEach(async () => {
		...
		dirA = await fs.mkdtemp(path.join("/tmp", "yemu-search-multi-A-"));
		dirB = await fs.mkdtemp(path.join("/var/tmp", "yemu-search-multi-B-"));
		cwd = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-search-multi-cwd-"));
...
describe.skipIf(isWindows)("resolveExplicitSearchPaths shared non-root ancestor", () => {
	...
	beforeEach(async () => {
		parent = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-search-ancestor-"));
...
describe.skipIf(isWindows)("search with explicit walker-pruned file targets", () => {
	...
	beforeEach(async () => {
		repo = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-search-pruned-"));
