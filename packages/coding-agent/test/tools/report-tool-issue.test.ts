...
import * as reportIssue from "@yemu/agent-runtime/tools/report-tool-issue";
import {
...
} from "@yemu/agent-runtime/tools/report-tool-issue";
...
function pushSettings(overrides: Record<string, unknown> = {}): Settings {
	return Settings.isolated({
		...
		// gates on (or `YEMU_AUTO_QA_PUSH=1` for headless overrides).
...
let originalYemuAutoQa: string | undefined;

function restoreAutoQaEnv(): void {
	if (originalYemuAutoQa === undefined) {
		delete Bun.env.YEMU_AUTO_QA;
		return;
	}
	Bun.env.YEMU_AUTO_QA = originalYemuAutoQa;
}
...
describe("flushGrievances", () => {
	...
	beforeEach(() => {
		__resetAutoQaFlushStateForTests();
		originalYemuAutoQa = Bun.env.YEMU_AUTO_QA;
		delete Bun.env.YEMU_AUTO_QA;
		db = openTempDb();
	});
	...
	it("lets YEMU_AUTO_QA=false disable auto QA when the setting is enabled", () => {
		Bun.env.YEMU_AUTO_QA = "0";

		expect(isAutoQaEnabled(Settings.isolated({ "dev.autoqa": true }))).toBe(false);
	});

	it("lets YEMU_AUTO_QA=true enable auto QA when the setting is disabled", () => {
		Bun.env.YEMU_AUTO_QA = "1";
	...
	it("posts pending rows with bearer header and marks them pushed=1 on 200", async () => {
		vi.spyOn(yemuUtils, "getInstallId").mockReturnValue("11111111-2222-3333-4444-555555555555");
		insertGrievance(db, "glob", "weird ordering");
		...
		expect(body.agent?.name).toBe("yemu");
		...
		// stays so users can browse what they've shipped via `yemu grievances`.
...
describe("dispatchReportIssueDevice", () => {
	...
	it("writes nothing while consent is unresolved", async () => {
		Bun.env.YEMU_AUTO_QA = "1";
		const originalPush = Bun.env.YEMU_AUTO_QA_PUSH;
		delete Bun.env.YEMU_AUTO_QA_PUSH;
		const db = openTempDb();
		const openSpy = vi.spyOn(reportIssue, "openAutoQaDb").mockReturnValue(db);
		try {
			...
			if (originalPush === undefined) delete Bun.env.YEMU_AUTO_QA_PUSH;
			else Bun.env.YEMU_AUTO_QA_PUSH = originalPush;
			openSpy.mockRestore();
			db.close();
		}
	});
