...
import type { ToolSession } from "@yemu/agent-runtime/sdk";
import { BrowserTool } from "@yemu/agent-runtime/tools/browser";
import { ensureChromiumExecutable } from "@yemu/agent-runtime/tools/browser/launch";
import { getTabsMapForTest } from "@yemu/agent-runtime/tools/browser/tab-supervisor";
	...
	it("runs tab.evaluate in the page's main JavaScript world", async () => {
		...
		try {
			await tool.execute("open", {
				action: "open",
				name,
				url: "data:text/html,<script>globalThis.__yemuMainWorld = 42</script>",
			});
			const result = await tool.execute("run", {
				action: "run",
				name,
				code: "return await tab.evaluate(() => globalThis.__yemuMainWorld);",
			});
