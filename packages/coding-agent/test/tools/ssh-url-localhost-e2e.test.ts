...
import { parseInternalUrl } from "@yemu/agent-runtime/internal-urls/parse";
import { InternalUrlRouter } from "@yemu/agent-runtime/internal-urls/router";
import { SshProtocolHandler } from "@yemu/agent-runtime/internal-urls/ssh-protocol";
import type { ToolSession } from "@yemu/agent-runtime/tools";
import { GrepTool } from "@yemu/agent-runtime/tools/grep";
...
describe.skipIf(!SSH_OK)("ssh:// handler against a real localhost ssh", () => {
	...
	const TMP = `/tmp/yemu-ssh-e2e-${process.pid}`;
	...
	it("writes byte-exact, leaves no temp, and the read path round-trips", async () => {
		...
		const leftovers = await Bun.$`ssh -o BatchMode=yes localhost ls ${TMP} | grep -c yemu-tmp || true`.text();
	...
	it("refuses to write to a directory and cleans up its temp", async () => {
		...
		const leftovers =
			await Bun.$`ssh -o BatchMode=yes localhost ls -A ${TMP} | grep -c "wdir.yemu-tmp" || true`.text();
		expect(leftovers.trim()).toBe("0");
	});
...
describe.skipIf(!SSH_OK)("ssh:// through the real read/grep/write tools (localhost)", () => {
	const TMP = `/tmp/yemu-ssh-tools-e2e-${process.pid}`;
	...
	it("GrepTool reports matches under the ssh:// URL with no scratch-temp leak", async () => {
		...
		expect(out).not.toContain("yemu-search-probe");
		expect(detailsJson).not.toContain("yemu-search-probe");
	});
