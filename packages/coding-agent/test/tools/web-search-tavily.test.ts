...
import { searchTavily } from "@yemu/agent-runtime/web/search/providers/tavily";
import type { SearchProviderError } from "@yemu/agent-runtime/web/search/types";
...
describe("Tavily web search provider", () => {
	...
	it("retries recency-filtered empty responses without time_range", async () => {
		...
		const response = await searchTavily({
			...makeParams("YeMu Agent yemu latest release notes advisor"),
			numSearchResults: 5,
			recency: "month",
			fetch: fetchMock,
		});
		...
		expect(requestBodies[0]).toMatchObject({
			query: "YeMu Agent yemu latest release notes advisor",
			max_results: 5,
			time_range: "month",
		});
