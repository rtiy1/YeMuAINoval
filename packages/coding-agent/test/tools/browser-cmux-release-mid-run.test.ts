...
import { acquireBrowser } from "@yemu/agent-runtime/tools/browser/registry";
import { acquireTab, getTabsMapForTest, releaseTab, runInTab } from "@yemu/agent-runtime/tools/browser/tab-supervisor";
import type { ToolSession } from "@yemu/agent-runtime/tools/index";

function makeKind(socketSuffix: string): CmuxKind {
	return {
		kind: "cmux",
		socketPath: `/tmp/yemu-test-${socketSuffix}.sock`,
...
describe("browser tab-supervisor — cmux tab close mid-run (#4499)", () => {
	...
	it("saves screenshots under the configured screenshot directory", async () => {
		...
		const screenshotDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-cmux-screenshot-"));
		try {
