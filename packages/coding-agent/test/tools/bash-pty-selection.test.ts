...
import { canUseInteractiveBashPty } from "@yemu/agent-runtime/tools/bash-pty-selection";

const originalPlatform = process.platform;
const originalNoPty = Bun.env.YEMU_NO_PTY;
...
function setNoPty(value: string | undefined): void {
	if (value === undefined) {
		delete Bun.env.YEMU_NO_PTY;
		return;
	}
	Bun.env.YEMU_NO_PTY = value;
}
