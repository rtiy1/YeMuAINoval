...
import { createTools, HIDDEN_TOOLS, type ToolSession } from "@yemu/agent-runtime/tools";
...
describe("createTools", () => {
	...
	it("still exposes eval when python kernel is unavailable (dispatches to js)", async () => {
		const session = createTestSession();
		vi.spyOn(await import("@yemu/agent-runtime/eval/py/kernel"), "checkPythonKernelAvailability").mockResolvedValue({
			ok: false,
