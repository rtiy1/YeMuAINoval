...
} from "@yemu/agent-runtime/tools/path-utils";
import { ReadTool } from "@yemu/agent-runtime/tools/read";
import { GrepOutputMode } from "@yemu/native";
...
describe("leading-colon path recovery (issue #5508)", () => {
	...
	it("strips a leading colon before an absolute path in resolveToCwd", () => {
		expect(resolveToCwd(":/tmp/yemu-colon-test.txt", tmpDir)).toBe("/tmp/yemu-colon-test.txt");
	});
...
// Regression: the `yemu grep` CLI subcommand resolved its path argument with a
