...
import type { SearchParams } from "@yemu/agent-runtime/web/search/providers/base";
import { hasCodexSearch, searchCodex } from "@yemu/agent-runtime/web/search/providers/codex";
import type { AuthStorage, FetchImpl } from "@yemu/model-runtime";
...
const originalCodexSearchModel = process.env.YEMU_CODEX_WEB_SEARCH_MODEL;
...
describe("searchCodex model selection", () => {
	...
	afterEach(() => {
		...
		if (originalCodexSearchModel === undefined) {
			delete process.env.YEMU_CODEX_WEB_SEARCH_MODEL;
		} else {
			process.env.YEMU_CODEX_WEB_SEARCH_MODEL = originalCodexSearchModel;
		}
	});
	...
	it("uses configured Codex endpoint, API key, and headers without OAuth", async () => {
		process.env.YEMU_CODEX_WEB_SEARCH_MODEL = "gpt-5.4";
	...
	it("falls back to the default model when YEMU_CODEX_WEB_SEARCH_MODEL is blank", async () => {
		process.env.YEMU_CODEX_WEB_SEARCH_MODEL = "   ";
	...
	it("encodes explicit gpt-5.6-sol as a Responses-Lite request", async () => {
		process.env.YEMU_CODEX_WEB_SEARCH_MODEL = "gpt-5.6-sol";
	...
	it("does not retry default candidates when YEMU_CODEX_WEB_SEARCH_MODEL is explicitly unsupported", async () => {
		process.env.YEMU_CODEX_WEB_SEARCH_MODEL = "gpt-5.5";
		let calls = 0;
		capturedRequest = null;
		const fetchMock: FetchImpl = (url, init) => {
			calls += 1;
	...
	it("fails a configured Responses-Lite model that answers without running web search (#6988)", async () => {
		process.env.YEMU_CODEX_WEB_SEARCH_MODEL = "gpt-5.6-terra";
		const sse = [
			`data: ${JSON.stringify({
				type: "response.output_item.done",
				item: {
