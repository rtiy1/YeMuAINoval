...
async function startPtyDaemonWithShell(shell: string, initialMarker: string, expectedMarker: string): Promise<void> {
	const projectDir = await tempDir("yemu-daemon-shell-project-");
	const runtimeDir = await tempDir("yemu-daemon-shell-runtime-");
	const runner = `
		...
		try {
			const started = await client.request({
				...
				spec: {
					...
					args: [
						"-e",
						"process.stdout.write(process.env.YEMU_TEST_SHELL_MARKER); process.stdout.write(String.fromCharCode(10)); process.stdin.resume();",
					],
					env: {},
					cwd: projectDir,
					pty: true,
	...
	const child = Bun.spawn([process.execPath, "--eval", runner], {
		...
		env: {
			...process.env,
			SHELL: shell,
			YEMU_TEST_SHELL_MARKER: initialMarker,
		},
		stdout: "pipe",
		stderr: "pipe",
	});
...
describe("daemon broker", () => {
	it("shares PTY output and input across project clients", async () => {
		const projectDir = await tempDir("yemu-daemon-project-");
		const runtimeDir = await tempDir("yemu-daemon-runtime-");
...
`,
	...
	it("omits terminal rows for non-PTY logs", async () => {
		const projectDir = await tempDir("yemu-daemon-plain-project-");
		const runtimeDir = await tempDir("yemu-daemon-plain-runtime-");
	...
	it("uses a basic shell when the login shell cannot run POSIX commands", async () => {
		...
		const shellPath = path.join(await tempDir("yemu-daemon-nonposix-shell-"), "csh");
	...
	it("preserves compatible login shells for PTY daemons", async () => {
		if (process.platform === "win32") return;
		const shellPath = path.join(await tempDir("yemu-daemon-posix-shell-"), "zsh");
		await Bun.write(shellPath, '#!/bin/sh\nexport YEMU_TEST_SHELL_MARKER="compatible-shell"\nexec /bin/sh "$@"\n');
		await fs.chmod(shellPath, 0o755);
	...
	it("returns promptly when a finite PTY child does not write the broker PID file", async () => {
		if (process.platform === "win32") return;
		const shellPath = path.join(await tempDir("yemu-daemon-no-pid-shell-"), "zsh");
		await Bun.write(
			shellPath,
			`#!/bin/sh
case "$2" in
...
`,
		...
		await fs.chmod(shellPath, 0o755);
		const projectDir = await tempDir("yemu-daemon-finite-project-");
		const runtimeDir = await tempDir("yemu-daemon-finite-runtime-");
		const runner = `
			import { createDaemonBrokerClient } from "./src/launch/client";
	...
	it("stops non-persistent daemons after the last project yemu exits", async () => {
		const projectDir = await tempDir("yemu-daemon-exit-project-");
		const runtimeDir = await tempDir("yemu-daemon-exit-runtime-");
		const scriptPath = path.join(projectDir, "service.ts");
	...
	it("keeps detached daemons alive through broker replacement", async () => {
		const projectDir = await tempDir("yemu-daemon-detached-project-");
		const runtimeDir = await tempDir("yemu-daemon-detached-runtime-");
		const scriptPath = path.join(projectDir, "service.ts");
		await Bun.write(scriptPath, `process.stdout.write("READY\\n"); setInterval(() => {}, 1000);\n`);
		const first = await createDaemonBrokerClient(projectDir, { runtimeDir, idleGraceMs: 5_000 });
	...
	it("names the unmet readiness condition when start times out", async () => {
		const projectDir = await tempDir("yemu-daemon-ready-project-");
		const runtimeDir = await tempDir("yemu-daemon-ready-runtime-");
		const scriptPath = path.join(projectDir, "service.ts");
	...
	it("returns promptly when the process becomes ready then exits within a poll", async () => {
		const projectDir = await tempDir("yemu-daemon-fast-project-");
		const runtimeDir = await tempDir("yemu-daemon-fast-runtime-");
	...
	it('wakes start and for:"ready" waits when the process exits before readiness', async () => {
		const projectDir = await tempDir("yemu-daemon-preexit-project-");
		const runtimeDir = await tempDir("yemu-daemon-preexit-runtime-");
	...
	it("clears stale readiness while a daemon is restarting", async () => {
		const projectDir = await tempDir("yemu-daemon-restart-project-");
		const runtimeDir = await tempDir("yemu-daemon-restart-runtime-");
