...
import { InspectImageTool } from "@yemu/agent-runtime/tools/inspect-image";
import { inspectImageToolRenderer } from "@yemu/agent-runtime/tools/inspect-image-renderer";
import { toolRenderers } from "@yemu/agent-runtime/tools/renderers";
import { buildModel } from "@yemu/model-catalog/build";
import { AuthStorage, type completeSimple, type ImageContent, type Model } from "@yemu/model-runtime";
import { removeSyncWithRetries, sanitizeText } from "@yemu/utils";
...
describe("InspectImageTool", () => {
	...
	beforeEach(() => {
		testDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-inspect-image-"));
	});
