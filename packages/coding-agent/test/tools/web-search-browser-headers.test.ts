...
import { buildBrowserNavigationHeaders } from "@yemu/agent-runtime/web/search/providers/browser-headers";
...
describe("browser navigation headers", () => {
	...
	it("imports cleanly and falls back when header-generator data files are absent", async () => {
		const script = [
			...
			'const { buildBrowserNavigationHeaders } = await import("@yemu/agent-runtime/web/search/providers/browser-headers");',
