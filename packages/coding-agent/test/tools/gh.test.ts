...
} from "@yemu/agent-runtime/tools/gh";
import * as git from "@yemu/agent-runtime/utils/git";
import type { ToolCall } from "@yemu/model-runtime";
import { toolWireSchema } from "@yemu/model-runtime/utils/schema";
import { validateToolArguments } from "@yemu/model-runtime/utils/validation";
import * as yemuUtils from "@yemu/utils";
import { $which, getAgentDir, hashPath, removeWithRetries, setAgentDir, WhichCachePolicy } from "@yemu/utils";
...
/**
 * Stub `os.homedir()` AND rebuild the cached `dirs` resolver in yemu-utils so
 * `getWorktreesDir()` resolves under an isolated temp home instead of the
 * user's real `~/.yemu/wt`. Returns the temp home and a cleanup hook.
 */
interface TempHome {
	home: string;
...
async function setupTempHome(): Promise<{ home: string; cleanup: () => Promise<void> }> {
	...
	// through the spied homedir instead of `$XDG_DATA_HOME/yemu/wt` (CI sets these).
...
async function expectedWorktreePath(home: string, primaryRoot: string, localBranch: string): Promise<string> {
	...
	return fs.realpath(path.join(home, ".yemu", "wt", segment));
}
...
describe("github tool", () => {
	...
	it("reads repository files through the GitHub contents API", async () => {
		...
		expect(textSpy).toHaveBeenCalledWith(
			...
			[
				"api",
				"/repos/rtiy1/YeMuAINoval/contents/packages/coding-agent/package.json",
				"--method",
				"GET",
				"-H",
	...
	describe("git.remote.add idempotency", () => {
		...
		it("does not depend on localized git remote-add stderr for existing remotes", async () => {
			...
			const fakeBin = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-fake-git-"));
...
`,
		...
		it("pins Git messages while preserving UTF-8 character locale", async () => {
			...
			const fakeBin = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-fake-git-locale-"));
...
`,
	...
	it("pins gh messages while preserving UTF-8 character locale", async () => {
		...
		const fakeBin = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-fake-gh-locale-"));
