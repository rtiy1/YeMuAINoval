...
import { createLspWritethrough, type FileDiagnosticsResult, FileFormatResult } from "@yemu/agent-runtime/lsp";
import * as lspClient from "@yemu/agent-runtime/lsp/client";
import * as lspConfig from "@yemu/agent-runtime/lsp/config";
import type { Diagnostic, LinterClient, LspClient, ServerConfig } from "@yemu/agent-runtime/lsp/types";
import { fileToUri } from "@yemu/agent-runtime/lsp/utils";
import type { DeferredDiagnosticsEntry, ToolSession } from "@yemu/agent-runtime/tools";
import { WriteTool } from "@yemu/agent-runtime/tools/write";
import { TempDir } from "@yemu/utils";
import type * as ptree from "@yemu/utils/ptree";
...
describe("LSP diagnostics freshness", () => {
	...
	beforeEach(() => {
		tempDir = TempDir.createSync("@yemu-lsp-freshness-");
	});
	...
	it("suppresses TypeScript project diagnostics for orphan files but keeps syntax errors", async () => {
		...
		const orphanDir = TempDir.createSync("@yemu-lsp-orphan-");
		try {
