 ...
 *
 * Each test isolates `YEMU_GITHUB_CACHE_DB` to a temp file and clears
...
import { getOrFetchIssue, getOrFetchPr } from "@yemu/agent-runtime/tools/gh";
import {
	clearAll,
	getCached,
	getOrFetchView,
...
} from "@yemu/agent-runtime/tools/github-cache";
import { ToolAbortError, throwIfAborted } from "@yemu/agent-runtime/tools/tool-errors";
...
beforeEach(async () => {
	...
	originalEnv = process.env.YEMU_GITHUB_CACHE_DB;
	process.env.YEMU_GITHUB_CACHE_DB = path.join(tempDir, "github-cache.db");
	resetCacheForTests();
});
...
afterEach(async () => {
	...
	if (originalEnv === undefined) {
		delete process.env.YEMU_GITHUB_CACHE_DB;
	} else {
		process.env.YEMU_GITHUB_CACHE_DB = originalEnv;
	}
	vi.restoreAllMocks();
...
describe("github-cache db layer", () => {
	...
	it("does not chmod an existing cache parent directory", async () => {
		...
		process.env.YEMU_GITHUB_CACHE_DB = path.join(parent, "github-cache.db");
		resetCacheForTests();
