...
import { MCPOAuthFlow, refreshMCPOAuthToken } from "@yemu/agent-runtime/mcp/oauth-flow";
...
describe("mcp oauth flow", () => {
	it("uses yemu-agent client name for dynamic client registration", async () => {
		...
		expect((registrationPayload as { client_name?: string } | null)?.client_name).toBe("yemu-agent");
			...
			if (url === "https://cropwise.example/oauth/register") {
				return new Response(
					JSON.stringify({
						...
						error_description: "client_name 'yemu-agent' is not on the approved list.",
					}),
