...
import { TodoTool, type ToolSession, USER_TODO_EDIT_CUSTOM_TYPE } from "@yemu/agent-runtime/tools";
import { getBundledModel } from "@yemu/model-catalog/models";
import type { Model, TextContent } from "@yemu/model-runtime";
import * as codexResponses from "@yemu/model-runtime/providers/openai-codex-responses";
...
describe("AgentSession eager prelude re-injection after compaction", () => {
	...
	beforeEach(() => {
		tempDir = TempDir.createSync("@yemu-agent-session-eager-compaction-");
		cleanups.length = 0;
	});
