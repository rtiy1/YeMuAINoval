...
import { executePython } from "@yemu/agent-runtime/eval/py/executor";
import type { KernelExecuteOptions, KernelExecuteResult } from "@yemu/agent-runtime/eval/py/kernel";
...
describe("executePython (per-call)", () => {
	...
	it("returns a cancelled timeout result when kernel startup exceeds the deadline", async () => {
		...
		using tempDir = TempDir.createSync("@yemu-python-executor-per-call-");
