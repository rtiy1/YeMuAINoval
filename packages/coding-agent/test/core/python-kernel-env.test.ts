...
} from "@yemu/agent-runtime/eval/py/runtime";
...
describe("Python gateway environment filtering", () => {
	it("filters sensitive and unknown variables from shell env", () => {
		...
		expect(filtered.YEMU_CUSTOM).toBe("1");
...
describe("enumeratePythonRuntimes", () => {
	...
	const managedDir = path.join(path.sep, "fake", ".yemu", "python-env");
	...
	it("enumerates the managed env AND the system interpreter so a broken managed env can fall through", () => {
		vi.spyOn(yemuUtils, "getPythonEnvDir").mockReturnValue(managedDir);
		vi.spyOn(yemuUtils, "$which").mockImplementation(bin => (bin === "python" ? systemPy : null));
