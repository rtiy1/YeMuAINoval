 ...
 *
 * Gated by `YEMU_RUBY_INTEGRATION=1` so CI without a real Ruby interpreter
 * (or sandboxes where subprocess spawning is restricted) does not fail.
 */
import { afterEach, describe, expect, it } from "bun:test";
import { disposeAllRubyKernelSessions, executeRubyWithKernel } from "@yemu/agent-runtime/eval/rb/executor";
import { RubyKernel } from "@yemu/agent-runtime/eval/rb/kernel";
import { TempDir } from "@yemu/utils";

const SHOULD_RUN = Bun.env.YEMU_RUBY_INTEGRATION === "1";
