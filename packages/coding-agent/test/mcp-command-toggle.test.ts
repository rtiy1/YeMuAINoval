...
import { MCPCommandController } from "@yemu/agent-runtime/modes/controllers/mcp-command-controller";
...
describe("/mcp enable and disable", () => {
	...
	beforeEach(async () => {
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-mcp-toggle-project-"));
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-mcp-toggle-agent-"));
