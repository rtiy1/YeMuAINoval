...
} from "@yemu/agent-runtime/session/indexed-session-storage";
import { FileSessionStorage } from "@yemu/agent-runtime/session/session-storage";
import { type SessionTitleUpdate, serializeTitleSlot } from "@yemu/agent-runtime/session/session-title-slot";
...
describe("FileSessionStorage.deleteSessionWithArtifacts", () => {
	...
	beforeEach(async () => {
		tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), "yemu-session-storage-"));
