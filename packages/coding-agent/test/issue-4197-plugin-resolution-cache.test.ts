...
import { clearClaudePluginRootsCache } from "@yemu/agent-runtime/discovery/helpers";
import {
...
test("getEnabledPlugins caches repeated discovery for the same cwd and home until plugin caches clear", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-plugin-cache-"));
	tempRoots.push(root);
	const home = path.join(root, "home");
	const cwd = path.join(root, "project");
	const pluginsDir = path.join(home, ".yemu", "plugins");
	const pluginPackageJson = path.join(pluginsDir, "node_modules", "yemu-cache-repro", "package.json");
	await fs.mkdir(path.dirname(pluginPackageJson), { recursive: true });
	await fs.mkdir(cwd, { recursive: true });
	await writeJson(path.join(pluginsDir, "package.json"), { dependencies: { "yemu-cache-repro": "1.0.0" } });
	await writeJson(path.join(pluginsDir, "yemu-plugins.lock.json"), {
		plugins: { "yemu-cache-repro": { version: "1.0.0", enabled: true, enabledFeatures: null } },
		settings: {},
	});
	await writeJson(pluginPackageJson, {
		name: "yemu-cache-repro",
		version: "1.0.0",
		yemu: { tools: "tools" },
	});
...
test("legacy bare dependency rewrites cache fallback package resolution until plugin caches clear", async () => {
	const root = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-legacy-cache-"));
	tempRoots.push(root);
