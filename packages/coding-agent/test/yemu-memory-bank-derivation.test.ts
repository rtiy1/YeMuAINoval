...
import { computeYeMuMemoryBankScope, extendRecallWithLegacyBanks } from "@yemu/agent-runtime/yemu-memory/config";
import { removeWithRetries, TempDir } from "@yemu/utils";

// Set up a fixture filesystem we can reuse across the two regression
// suites — same shape as `~/.yemu/memories/yemu-memory/` on a real install.
let rootDir: TempDir;
let dbDir: string;
let banksDir: string;
...
beforeAll(async () => {
	rootDir = await TempDir.create("@yemu-memory-bank-derivation-");
	dbDir = rootDir.join("yemu-memory");
	banksDir = path.join(dbDir, "banks");
	await fs.mkdir(banksDir, { recursive: true });
	mainDbPath = path.join(dbDir, "yemu-memory.db");
});
...
// Schema mirrors the subset of `packages/yemu-memory/src/core/beam/schema.ts`
...
function createBankFixture(bank: string, metadataRows: readonly Record<string, unknown>[]): void {
	...
	const dbPath = path.join(bankDir, "yemu-memory.db");
...
describe("computeYeMuMemoryBankScope (#2412)", () => {
	...
	it("returns the same per-project bank for one cwd regardless of git state", async () => {
		const baseDir = await TempDir.create("@yemu-memory-stable-bank-");
		try {
			const project = baseDir.join("projects", "yemu-workstation");
			await fs.mkdir(project, { recursive: true });
			const withoutGit = computeYeMuMemoryBankScope(undefined, project, "per-project").bank;
			...
			const withAncestorGit = computeYeMuMemoryBankScope(undefined, project, "per-project").bank;
			expect(withAncestorGit).toBe(withoutGit);

			await removeWithRetries(baseDir.join("projects", ".git"));
			const afterGitRemoved = computeYeMuMemoryBankScope(undefined, project, "per-project").bank;
	...
	it("derives different banks for different cwds (sanity)", () => {
		const a = computeYeMuMemoryBankScope(undefined, "/projects/repo-a", "per-project").bank;
		const b = computeYeMuMemoryBankScope(undefined, "/projects/repo-b", "per-project").bank;
		expect(a).not.toBe(b);
	});

	it("per-project-tagged opens both the project bank and the shared default", () => {
		const scope = computeYeMuMemoryBankScope(undefined, "/projects/repo", "per-project-tagged");
	...
	it("global ignores the cwd entirely", () => {
		const here = computeYeMuMemoryBankScope(undefined, "/projects/here", "global");
		const there = computeYeMuMemoryBankScope(undefined, "/elsewhere", "global");
...
describe("extendRecallWithLegacyBanks edge cases", () => {
	...
	it("returns the input unchanged when banks/ does not exist", () => {
		const missingRoot = rootDir.join("no-such-yemu-memory", "yemu-memory.db");
	...
	it("tolerates a corrupt bank database without throwing", async () => {
		...
		await fs.writeFile(path.join(corruptDir, "yemu-memory.db"), "not a sqlite file");
