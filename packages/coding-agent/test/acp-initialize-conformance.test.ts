...
import { AcpAgent } from "@yemu/agent-runtime/modes/acp/acp-agent";
import { ACP_TERMINAL_AUTH_FLAG, prepareAcpTerminalAuthArgs } from "@yemu/agent-runtime/modes/acp/terminal-auth";
...
import { getConfigRootDir, setAgentDir, VERSION } from "@yemu/utils";
...
async function createAgent(): Promise<AcpAgent> {
	const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "yemu-acp-init-"));
