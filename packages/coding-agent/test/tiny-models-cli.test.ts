...
import { resolveModels, runTinyModelsCommand } from "@yemu/agent-runtime/cli/tiny-models-cli";
import { TINY_LOCAL_MODELS } from "@yemu/agent-runtime/tiny/models";
import { tinyTitleClient } from "@yemu/agent-runtime/tiny/title-client";
...
describe("tiny-models download model resolution", () => {
	...
	it("prints actionable CUDA provider diagnostics from worker errors", async () => {
		...
		const diagnostic = [
			...
			"  YEMU_TINY_DEVICE=cuda requested CUDAExecutionProvider",
			"  side runtime: /home/user/.yemu/cache/tiny-title-runtime/transformers-test/node_modules",
		...
		expect(text).toContain("YEMU_TINY_DEVICE=cuda");
