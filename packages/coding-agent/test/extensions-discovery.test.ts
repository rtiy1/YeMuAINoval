...
import { type ExtensionModule, extensionModuleCapability } from "@yemu/agent-runtime/capability/extension-module";
import { resetSettingsForTest, Settings } from "@yemu/agent-runtime/config/settings";
import { getCapability, initializeWithSettings } from "@yemu/agent-runtime/discovery";
import {
	discoverAndLoadExtensions,
	discoverExtensionPaths,
	loadExtensions,
} from "@yemu/agent-runtime/extensibility/extensions/loader";
...
describe("extensions discovery", () => {
	...
	beforeEach(() => {
		tempDir = TempDir.createSync("@yemu-ext-test-");
	...
	const extensionCode = `
		export default function(yemu) {
			yemu.registerCommand("test", { handler: async () => {} });
		}
	`;
	...
	it("discovers subdirectory with package.json yemu field", async () => {
	...
	it("package.json with yemu field takes precedence over index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
	...
	it("ignores package.json without yemu field, falls back to index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
		fs.writeFileSync(path.join(subdir, "index.ts"), extensionCode);
		fs.writeFileSync(
	...
	it("handles mixed direct files and subdirectories", async () => {
		...
		fs.writeFileSync(path.join(subdir2, "package.json"), JSON.stringify({ yemu: { extensions: ["./entry.ts"] } }));
	...
	it("discovers a symlinked extension directory with a package.json manifest", async () => {
		...
		fs.writeFileSync(
			...
			JSON.stringify({ name: "ctk", yemu: { extensions: ["./index.ts"] } }),
		);
	...
	it("resolves dependencies from extension's own node_modules", async () => {
		...
		fs.writeFileSync(
			...
			JSON.stringify({
				name: "yemu-extension-with-deps",
				version: "1.0.0",
				type: "module",
				yemu: { extensions: ["./index.ts"] },
			}),
		);
	...
	it("registers message renderers", async () => {
		const extCode = `
			export default function(yemu) {
				yemu.registerMessageRenderer("my-custom-type", (message, options, theme) => {
	...
	it("reports error when extension has no default export", async () => {
		const extCode = `
			export function notDefault(yemu) {
				yemu.registerCommand("test", { handler: async () => {} });
			}
		`;
	...
	it("loads extension with event handlers", async () => {
		const extCode = `
			export default function(yemu) {
				...
				yemu.on("tool_call", async (event) => undefined);
				yemu.on("agent_end", async () => {});
			}
		`;
	...
	it("loads hookCapability JS factories as extension handlers", async () => {
		...
		fs.writeFileSync(
			...
			`
				export default function(yemu) {
					yemu.on("tool_call", async () => ({ block: true, reason: "blocked by hook" }));
				}
			`,
		);
