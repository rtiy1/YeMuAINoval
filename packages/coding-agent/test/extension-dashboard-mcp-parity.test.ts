...
import { initializeWithSettings, reset as resetDiscoveryCache } from "@yemu/agent-runtime/discovery";
import { readMCPConfigFile, setMcpServerEnabled, setServerDisabled } from "@yemu/agent-runtime/mcp/config-writer";
import { loadAllExtensions } from "@yemu/agent-runtime/modes/components/extensions/state-manager";
import { __resetDirsFromEnvForTests, getMCPConfigPath, removeWithRetries, setAgentDir } from "@yemu/utils";
...
describe("loadAllExtensions MCP parity with /mcp list (issue #3827)", () => {
	...
	beforeEach(async () => {
		...
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-3827-project-"));
		userAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-3827-user-"));
		...
		await fs.writeFile(
			path.join(projectDir, ".yemu", "mcp.json"),
			JSON.stringify({
				mcpServers: {
	...
	test("dashboard re-enable flips enabled:false in mcp.json (PR #3829 review)", async () => {
		...
		const projectMcpPath = path.join(projectDir, ".yemu", "mcp.json");
	...
	test("dashboard disable on a config-resident server writes enabled:false (not denylist)", async () => {
		...
		const projectConfig = await readMCPConfigFile(path.join(projectDir, ".yemu", "mcp.json"));
	...
	test("dashboard re-enable updates the row's non-primary source mcp.json before denylisting", async () => {
		const alternatePath = path.join(projectDir, ".yemu", ".mcp.json");
		await Bun.write(
			alternatePath,
			JSON.stringify({
				mcpServers: {
