...
import {
	...
	type SettingPath,
	Settings,
} from "@yemu/agent-runtime/config/settings";
import { AgentStorage } from "@yemu/agent-runtime/session/agent-storage";
import { AUTO_IMAGE_PROVIDER_ORDER } from "@yemu/agent-runtime/tools/image-providers";
import { SEARCH_PROVIDER_ORDER } from "@yemu/agent-runtime/web/search/types";
import { Effort } from "@yemu/model-runtime";
import { clearCustomApis } from "@yemu/model-runtime/api-registry";
import { createMockModel, registerMockApi } from "@yemu/model-runtime/providers/mock";
import { __providerInFlightForTesting, streamSimple } from "@yemu/model-runtime/stream";
import type { Context } from "@yemu/model-runtime/types";
import { getProjectAgentDir, TempDir } from "@yemu/utils";
...
describe("Settings", () => {
	...
	beforeEach(() => {
		...
		tempDir = TempDir.createSync("@yemu-settings-test-");
	...
	describe("config file failure safety", () => {
		...
		it("backs up a corrupted project config and retains the pending project role for retry", async () => {
			await writeSettings({});
			const projectConfigPath = path.join(projectDir, ".yemu", "config.yml");
			await Bun.write(
				projectConfigPath,
		...
		it("observes both failures when global and project configs are malformed", async () => {
			...
			await Promise.all([
				...
				Bun.write(path.join(projectDir, ".yemu", "config.yml"), malformed),
			]);
			...
			try {
				...
				expect(
					fs.readdirSync(path.join(projectDir, ".yemu")).some(name => name.startsWith("config.yml.broken-")),
				).toBe(true);
			} finally {
	...
	describe("model role overrides", () => {
		...
		it("preserves concurrent external per-role edits when saving one global role", async () => {
			...
			// External edit (another yemu instance / manual edit): changes advisor,
	...
	describe("migrations", () => {
		...
		it("migrates the legacy mnemosyne memory backend to yemu-memory", async () => {
			await writeSettings({
			...
			expect(settings.get("memory.backend")).toBe("yemu-memory");
			expect(settings.get("yemu-memory.dbPath")).toBe("/tmp/old.db");
			expect(settings.get("yemu-memory.scoping")).toBe("global");
		});

		it("does not clobber an explicit yemu-memory block when the legacy mnemosyne block is also present", async () => {
			await writeSettings({
				mnemosyne: { dbPath: "/tmp/old.db" },
				"yemu-memory": { dbPath: "/tmp/new.db" },
			});

			const settings = await Settings.init({ cwd: projectDir, agentDir });

			expect(settings.get("yemu-memory.dbPath")).toBe("/tmp/new.db");
		});
