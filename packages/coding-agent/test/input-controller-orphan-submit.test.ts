...
describe("InputController orphaned submit", () => {
	...
	it("starts a real idle session even when steer drain would be non-resumable", async () => {
		const tempDir = TempDir.createSync("@yemu-orphan-submit-");
	...
	it("skips automatic titles only for locally consumed extension commands", async () => {
		const previousNoTitle = Bun.env.YEMU_NO_TITLE;
		delete Bun.env.YEMU_NO_TITLE;
		const tempDir = TempDir.createSync("@yemu-extension-title-");
		let session: AgentSession | undefined;
		let authStorage: AuthStorage | undefined;
		try {
			const model = getBundledModel("anthropic", "claude-sonnet-4-5");
			...
			const extension = await loadExtensionFromFactory(
				yemu => {
					yemu.registerCommand("widget-status", {
			...
			if (previousNoTitle === undefined) {
				...
				Bun.env.YEMU_NO_TITLE = previousNoTitle;
			}
		}
	});
});
