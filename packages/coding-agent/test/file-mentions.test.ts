...
import { extractFileMentions, generateFileMentionMessages } from "@yemu/agent-runtime/utils/file-mentions";
...
async function createTempDir(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-file-mentions-"));
	tempDirs.push(dir);
	return dir;
}
