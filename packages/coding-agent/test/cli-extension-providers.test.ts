 ...
 *
 * One-shot CLIs (`yemu bench`, dry-balance) build a bare `ModelRegistry` that
 ...
 * (`yemu.registerProvider(...)`, e.g. a custom OpenAI-compatible gateway under
 * `~/.yemu/agent/extensions/`) was invisible to model resolution and
 * `yemu bench <provider>/<model>` failed with "Model not found".
 *
 ...
 * passed explicitly so the test never touches the developer's real `~/.yemu`.
 */

import { afterAll, beforeAll, expect, test } from "bun:test";
import * as fs from "node:fs/promises";
import { ModelRegistry } from "@yemu/agent-runtime/config/model-registry";
import { getModelMatchPreferences, resolveCliModel } from "@yemu/agent-runtime/config/model-resolver";
import { resetSettingsForTest, Settings } from "@yemu/agent-runtime/config/settings";
import { loadCliExtensionProviders } from "@yemu/agent-runtime/sdk";
...
beforeAll(async () => {
	...
	yemu.registerProvider("bench-gw", {
