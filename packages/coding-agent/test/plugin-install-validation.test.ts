...
describe("PluginManager.install load validation", () => {
	...
	beforeEach(async () => {
		tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-plugin-validation-"));
		...
		vi.spyOn(yemuUtils, "getPluginsDir").mockReturnValue(pluginsDir);
		vi.spyOn(yemuUtils, "getPluginsNodeModules").mockReturnValue(pluginsNodeModules);
		vi.spyOn(yemuUtils, "getPluginsPackageJson").mockReturnValue(pluginsPkgJson);
		vi.spyOn(yemuUtils, "getPluginsLockfile").mockReturnValue(path.join(tmpRoot, "yemu-plugins.lock.json"));
		vi.spyOn(yemuUtils, "getProjectDir").mockReturnValue(tmpRoot);
		vi.spyOn(yemuUtils, "getProjectPluginOverridesPath").mockReturnValue(path.join(tmpRoot, "plugin-overrides.json"));
	});
	...
	test("installs npm protocol specs with the resolved package name", async () => {
		vi.spyOn(Bun, "spawn").mockImplementation(((cmd: string[]) => {
			expect(cmd).toEqual(["bun", "install", "npm:yemu-figma-remote-auth"]);
					...
					JSON.stringify(
						{
							name: "yemu-plugins",
							private: true,
							dependencies: { "yemu-figma-remote-auth": "npm:yemu-figma-remote-auth" },
						},
						null,
						2,
					),
				);
				await writePluginPackage(pluginsNodeModules, "yemu-figma-remote-auth", {
					version: "1.2.3",
					source:
						'export default function(yemu) { yemu.registerCommand("figma-auth", { handler: async () => {} }); }\n',
				});
			})();
		...
		const result = await new PluginManager(tmpRoot).install("npm:yemu-figma-remote-auth");

		expect(result.name).toBe("yemu-figma-remote-auth");
		expect(result.version).toBe("1.2.3");
		expect(result.path).toBe(path.join(pluginsNodeModules, "yemu-figma-remote-auth"));
	});
	...
	test("rejects an install whose extension entry cannot resolve its dependencies", async () => {
		vi.spyOn(Bun, "spawn").mockImplementation(((cmd: string[]) => {
			...
			const prepare = (async () => {
				await Bun.write(
					pluginsPkgJson,
					JSON.stringify(
						{ name: "yemu-plugins", private: true, dependencies: { "broken-plugin": "1.0.0" } },
						null,
						2,
					),
				);
				await writePluginPackage(pluginsNodeModules, "broken-plugin", {
					...
					source:
						'import { missing } from "missing-peer";\nexport default function(yemu) { yemu.registerCommand(String(missing), { handler: async () => {} }); }\n',
				});
			})();
		...
		expect(await Bun.file(path.join(tmpRoot, "yemu-plugins.lock.json")).exists()).toBe(false);
	});

	test("restores the previous package tree when reinstall validation fails", async () => {
		await Bun.write(
			pluginsPkgJson,
			JSON.stringify({ name: "yemu-plugins", private: true, dependencies: { "broken-plugin": "1.0.0" } }, null, 2),
		);
		await Bun.write(
			path.join(tmpRoot, "yemu-plugins.lock.json"),
			JSON.stringify(
		...
		await writePluginPackage(pluginsNodeModules, "broken-plugin", {
			version: "1.0.0",
			source: 'export default function(yemu) { yemu.registerCommand("old-ok", { handler: async () => {} }); }\n',
		});
		...
		vi.spyOn(Bun, "spawn").mockImplementation(((cmd: string[]) => {
			...
			const prepare = (async () => {
				await Bun.write(
					pluginsPkgJson,
					JSON.stringify(
						{ name: "yemu-plugins", private: true, dependencies: { "broken-plugin": "2.0.0" } },
						null,
						2,
					),
				);
		...
		const lock = await Bun.file(path.join(tmpRoot, "yemu-plugins.lock.json")).json();
	...
	test("restores the previous git plugin tree when reinstalling a different ref fails validation", async () => {
		await Bun.write(
			pluginsPkgJson,
			JSON.stringify(
				{ name: "yemu-plugins", private: true, dependencies: { "git-plugin": "github:org/plugin#v1" } },
				null,
				2,
			),
		);
		...
		await writePluginPackage(pluginsNodeModules, "git-plugin", {
			version: "1.0.0",
			source: 'export default function(yemu) { yemu.registerCommand("git-old-ok", { handler: async () => {} }); }\n',
		});
			...
			if (cmd[1] === "install") {
				...
				const prepare = (async () => {
					await Bun.write(
						pluginsPkgJson,
						JSON.stringify(
							{ name: "yemu-plugins", private: true, dependencies: { "git-plugin": "github:org/plugin#v2" } },
							null,
							2,
						),
					);
		...
		vi.spyOn(Bun, "spawn").mockImplementation(((cmd: string[]) => {
			...
			const prepare = (async () => {
				await Bun.write(
					pluginsPkgJson,
					JSON.stringify(
						{ name: "yemu-plugins", private: true, dependencies: { "partial-plugin": "1.0.0" } },
						null,
						2,
					),
				);
					...
					JSON.stringify(
						{
							name: "partial-plugin",
							version: "1.0.0",
							yemu: { extensions: ["./dist/valid.ts", "./dist/missing.ts"] },
						},
						null,
						2,
					),
				...
				await Bun.write(
					...
					'export default function(yemu) { yemu.registerCommand("valid-ext", { handler: async () => {} }); }\n',
				);
			})();
			...
			if (cmd[1] === "install") {
				const prepare = (async () => {
					...
					await Bun.write(
						pluginsPkgJson,
						JSON.stringify(
							{ name: "yemu-plugins", private: true, dependencies: { "git-plugin": "github:org/plugin" } },
							null,
							2,
						),
					);
