...
import { type OAuthCredential, type UsageProvider, withAuth } from "@yemu/model-runtime";
import * as oauth from "@yemu/model-runtime/oauth";
import type { OAuthCredentials, OAuthProviderId } from "@yemu/model-runtime/oauth/types";
...
describe("AuthStorage account rotation", () => {
	...
	beforeEach(async () => {
		tempDir = path.join(os.tmpdir(), `yemu-test-auth-rotation-${Snowflake.next()}`);
