...
import { getAgentDir, getBlobsDir, getHistoryDbPath, getSessionsDir, setAgentDir, setProjectDir } from "@yemu/utils";
...
beforeEach(async () => {
	...
	root = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-gc-"));
	writes = [];
	stderrWrites = [];
	process.exitCode = 0;
