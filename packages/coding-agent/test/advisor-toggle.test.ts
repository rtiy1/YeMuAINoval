...
import { Agent, type AgentMessage } from "@yemu/agent-core";
import * as compactionModule from "@yemu/agent-core/compaction";
import { loadAdvisorTranscriptCosts } from "@yemu/agent-runtime/advisor/transcript-recorder";
...
import type { AssistantMessage, Model } from "@yemu/model-runtime";
...
describe("AgentSession advisor toggle", () => {
	...
	beforeAll(async () => {
		sharedDir = TempDir.createSync("@yemu-advisor-toggle-shared-");
	...
	beforeEach(async () => {
		tempDir = TempDir.createSync("@yemu-advisor-toggle-");
	...
	it("clears advisor cost when a branch skips conversation restore", async () => {
		...
		const branchDir = TempDir.createSync("@yemu-advisor-branch-");
	...
	it("clears advisor cost when a branch hook throws after the session changed", async () => {
		...
		const branchDir = TempDir.createSync("@yemu-advisor-branch-fail-");
		const branchManager = SessionManager.create(branchDir.path(), branchDir.path());
		const branchSession = new AgentSession({
			agent: new Agent({
				initialState: { model, systemPrompt: ["Test"], tools: [], messages: [] },
