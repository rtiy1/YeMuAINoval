...
import type { AgentEvent, AgentTool, AgentToolContext } from "@yemu/agent-core";
import { Settings } from "@yemu/agent-runtime/config/settings";
import { CursorExecHandlers } from "@yemu/agent-runtime/cursor";
import { bridgeToolMap, createBridgeEditTool, createBridgeGrepFactory } from "@yemu/agent-runtime/cursor-bridge-tools";
import { EditTool } from "@yemu/agent-runtime/edit";
import type { ExtensionRunner } from "@yemu/agent-runtime/extensibility/extensions";
import { ExtensionToolWrapper } from "@yemu/agent-runtime/extensibility/extensions";
import { BUILTIN_TOOLS, GrepTool, ReadTool, type Tool, type ToolSession } from "@yemu/agent-runtime/tools";
import { BashTool } from "@yemu/agent-runtime/tools/bash";
import type { TruncationMeta } from "@yemu/agent-runtime/tools/output-meta";
import {
...
import { type BlockState, handleServerMessage, type ToolCallState } from "@yemu/model-runtime/providers/cursor";
import { buildYemuLsResult, yemuTruncation } from "@yemu/model-runtime/providers/cursor/exec-modern";
import type { AssistantMessage } from "@yemu/model-runtime/types";
...
describe("CursorExecHandlers.grep bridge", () => {
	...
	it("honors yemu_grep's requested match limit against real files", async () => {
	...
	it("honors yemu_grep's requested context width against real files", async () => {
		...
		const noContext = await scopedHandlers.yemuGrep({
			toolCallId: "c1",
		...
		const withContext = await scopedHandlers.yemuGrep({
			toolCallId: "c2",
	...
	it("satisfies a yemu_grep limit that spans more files than one page", async () => {
		// The local tool windows results to the first 20 files and tells the
		// caller to paginate with `skip`. `YemuGrepExecArgs` has no `skip` field,
		...
		const wide = await scopedHandlers.yemuGrep({
			toolCallId: "c1",
		...
		const exact = await scopedHandlers.yemuGrep({
			toolCallId: "c3",
...
describe("yemu_bash truncation reaches the wire from a real BashTool result", () => {
	let cwd: string;
	...
	it("translates the metadata BashTool actually emits, not a hand-built shape", async () => {
		// Producer/consumer contract. `yemuTruncation` lives in `model-runtime`, which
	...
	it("reports the entry cap ReadTool actually records for a large listing", async () => {
		...
		// but `read`, which serves `yemu_ls`, records it only through `OutputMeta`.
...
describe("bridge tool resolution beyond the model-facing registry", () => {
	...
	it("edits a real file from a yemu_edit frame when `edit` is withheld from the model", async () => {
		// For Cursor the session drops `edit` from the tool registry so the model
		// is steered to full-file `write`. The native `yemu_edit` frame arrives
		...
		const result = await withheld.yemuEdit({
			toolCallId: "e1",
	...
	it("reports the failure instead of editing when no edit tool is reachable", async () => {
		...
		const result = await unreachable.yemuEdit({
			toolCallId: "e2",
			args: { path: target, edits: [{ oldText: "beta", newText: "gamma" }] },
		} as never);
	...
	it("substitutes a replace-mode edit into a granted advisor tool map", async () => {
		...
		// `input` string. A `yemu_edit` frame's `old_text`/`new_text` pairs fail
		...
		const result = await handlers.yemuEdit({
			toolCallId: "e3",
			args: { path: target, edits: [{ oldText: "beta", newText: "gamma" }] },
		} as never);
	...
	it("still refuses a yemu_edit frame when the session granted no edit tool", async () => {
	...
	it("refuses a scoped yemu_grep when no grep tool was granted", async () => {
		...
		const result = await denied.yemuGrep({
			toolCallId: "g0",
	...
	it("denies a native yemu_edit frame the user's policy blocks", async () => {
	...
	it("denies a scoped yemu_grep frame the user's policy blocks", async () => {
		...
		const result = await handlers.yemuGrep({
			toolCallId: "g2",
	...
	it("denies a yemu_write frame the user's policy blocks when the tool came from the caller's map", async () => {
		// The advisor hands the bridge its own tool map. Those instances are run
		// directly by `yemuWrite`/`yemuBash`, so an unwrapped one executes whatever
		...
		const result = await handlers.yemuWrite({
			toolCallId: "w1",
...
describe("yemu_bash timeout presence", () => {
	let cwd: string;
	let handlers: CursorExecHandlers;
	...
	it("disables the deadline for an explicit zero instead of applying the default", async () => {
		...
		// be killed.
		const disabled = await handlers.yemuBash({
			toolCallId: "b1",
		...
		const defaulted = await handlers.yemuBash({
			toolCallId: "b2",
	...
	it("passes a positive timeout through", async () => {
		const result = await handlers.yemuBash({
			toolCallId: "b3",
	...
	it("falls back to the default for a negative timeout", async () => {
		...
		const negative = await handlers.yemuBash({
			toolCallId: "b4",
			args: { command: "echo hi", timeout: -5 },
		} as never);
		const omitted = await handlers.yemuBash({
			toolCallId: "b5",
			args: { command: "echo hi" },
		} as never);
...
// The YeMu frames (`ExecServerMessage` 45-51) are a separate wire family from the
...
describe("CursorExecHandlers YeMu frame translation", () => {
	let cwd: string;

	beforeEach(async () => {
		cwd = await fs.mkdtemp(path.join(os.tmpdir(), "cursor-yemu-test-"));
	});
	...
	it("inverts yemu_grep's ignore_case into the local tool's case-sensitivity flag", async () => {
		...
		await handlers.yemuGrep({ toolCallId: "c1", args: { pattern: "x", ignoreCase: true } } as never);
		await handlers.yemuGrep({ toolCallId: "c2", args: { pattern: "x", ignoreCase: false } } as never);
	...
	it("folds yemu_grep's separate glob onto the local tool's single path spec", async () => {
		const { handlers, calls } = recordingHandlers("grep");

		await handlers.yemuGrep({ toolCallId: "c1", args: { pattern: "x", path: "src", glob: "**/*.ts" } } as never);
		await handlers.yemuGrep({ toolCallId: "c2", args: { pattern: "x", glob: "**/*.ts" } } as never);
		await handlers.yemuGrep({ toolCallId: "c3", args: { pattern: "x", path: ".", glob: "**/*.ts" } } as never);
		await handlers.yemuGrep({ toolCallId: "c4", args: { pattern: "x", path: "src", glob: "/abs/**/*.ts" } } as never);
	...
	it("composes yemu_read's offset/limit onto the path as the read tool's range selector", async () => {
		...
		await handlers.yemuRead({ toolCallId: "c1", args: { path: "a.ts", offset: 5, limit: 20 } } as never);
		await handlers.yemuRead({ toolCallId: "c2", args: { path: "a.ts", offset: 5 } } as never);
		await handlers.yemuRead({ toolCallId: "c3", args: { path: "a.ts", limit: 20 } } as never);
		await handlers.yemuRead({ toolCallId: "c4", args: { path: "a.ts" } } as never);
		...
		await handlers.yemuRead({ toolCallId: "c5", args: { path: "a.ts", offset: 0, limit: 20 } } as never);
	...
	it("answers a present yemu_read limit of zero with empty output instead of the whole file", async () => {
		...
		const result = await handlers.yemuRead({ toolCallId: "c1", args: { path: "a.ts", limit: 0 } } as never);
	...
	it("composes the legacy read frame's offset/limit the same way yemu_read does", async () => {
	...
	it("returns exactly the lines a yemu_read range asked for", async () => {
		...
		try {
			...
			const result = await handlers.yemuRead({
				toolCallId: "c1",
	...
	it("escapes yemu_grep's pattern when the frame asks for a literal search", async () => {
		...
		await handlers.yemuGrep({ toolCallId: "c1", args: { pattern: "a.b(c)", literal: true } } as never);
		await handlers.yemuGrep({ toolCallId: "c2", args: { pattern: "a.b(c)" } } as never);

		expect((calls[0] as { pattern: string }).pattern).toBe("a\\.b\\(c\\)");
		expect((calls[1] as { pattern: string }).pattern).toBe("a.b(c)");
	});

	it("routes yemu_find to glob, not grep, joining its pattern onto the path", async () => {
		// `yemu_find` searches filenames. Routing it to `grep` would search file
		// contents for the glob text and return nothing.
		const { handlers, calls } = recordingHandlers("glob");

		await handlers.yemuFind({ toolCallId: "c1", args: { pattern: "*.ts", path: "src", limit: 10 } } as never);
		await handlers.yemuFind({ toolCallId: "c2", args: { pattern: "*.ts", limit: 0 } } as never);
		await handlers.yemuFind({ toolCallId: "c3", args: { pattern: "*.ts" } } as never);
	...
	it("renames yemu_edit's camelCase replacements to the local tool's snake_case pairs", async () => {
		const { handlers, calls } = recordingHandlers("edit");

		await handlers.yemuEdit({
			toolCallId: "c1",
	...
	it("lists directories for yemu_ls through read, defaulting an empty path to cwd", async () => {
		const { handlers, calls } = recordingHandlers("read");

		await handlers.yemuLs({ toolCallId: "c1", args: { path: "" } } as never);
