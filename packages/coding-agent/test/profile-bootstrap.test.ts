...
describe("extractProfileFlags", () => {
	...
	it("does not eat the value of known string-valued flags", () => {
		// `yemu --system-prompt --profile foo` must pass the literal `--profile`
	...
	it("does not eat the value of --approval-mode", () => {
		...
		// `yemu --approval-mode --profile foo` silently activates profile `foo`
	...
	it("keeps the built-in --plan from swallowing the profile boundary when its extension is absent", () => {
		...
		// become "--yemu-profile-boundary" and the user's message would be dropped.
		const extracted = extractProfileFlags(["--plan", "--profile", "work", "follow up"]);
	...
	it("stops extracting global flags at a subcommand boundary", () => {
		// `yemu grep --profile <path>` must reach the grep subcommand intact; the
	...
	it("treats explicit launch as the default command and keeps extracting globals", () => {
		expect(extractProfileFlags(["launch", "--profile", "work", "--alias", "yemu-work"])).toEqual({
			argv: ["launch"],
			profile: "work",
			aliasName: "yemu-work",
		});
	...
	it("exempts known value-less launch flags so a trailing profile still activates", () => {
		...
		// the token after them is a fresh argument: `yemu --print --profile work`
	...
	it("does not hide a global --profile/--alias behind an unknown flag with a flag-looking successor", () => {
		...
		// consume value-like (non-`-`) successors. So `yemu --some-ext-flag --profile
		...
		expect(extractProfileFlags(["--some-ext-flag", "--alias", "yemu-work"])).toEqual({
			argv: ["--some-ext-flag"],
			profile: undefined,
			aliasName: "yemu-work",
		});
