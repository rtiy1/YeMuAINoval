...
describe("createAgentSession MCP server instructions (deferred UI)", () => {
	...
	beforeAll(async () => {
		registryDir = path.join(os.tmpdir(), `yemu-sdk-mcp-instr-registry-${Snowflake.next()}`);
		fs.mkdirSync(registryDir, { recursive: true });
		isolatedHome = path.join(os.tmpdir(), `yemu-sdk-mcp-instr-home-${Snowflake.next()}`);
		fs.mkdirSync(isolatedHome, { recursive: true });
		isolatedAgentDir = path.join(isolatedHome, ".yemu", "agent");
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `yemu-sdk-mcp-instr-${Snowflake.next()}`);
