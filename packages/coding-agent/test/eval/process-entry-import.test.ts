...
it("imports the CLI entry graph without loading dotenv before profile bootstrap", async () => {
	using tempDir = TempDir.createSync("@yemu-js-process-import-");
	await Bun.write(path.join(tempDir.path(), ".env"), "YEMU_PROCESS_ENTRY_ENV_PROBE=loaded-too-early\n");
	const env = Object.fromEntries(
		Object.entries(process.env).filter((entry): entry is [string, string] => typeof entry[1] === "string"),
	);
	delete env.YEMU_PROCESS_ENTRY_ENV_PROBE;
...
async function pingComputerWorker(
	entry: string,
	id: string,
	argv: string[] = ["__yemu_worker_computer"],
): Promise<unknown> {
...
it("dispatches the computer worker from a single npm-style host bundle", async () => {
	...
	try {
		const output = await Bun.build({
			...
			naming: "cli.js",
			target: "bun",
			external: ["@yemu/native"],
			define: { "process.env.YEMU_BUNDLED": JSON.stringify("true") },
			throw: false,
		});
...
it("keeps non-computer selectors isolated in a compiled single-entry worker host", async () => {
	using tempDir = TempDir.createSync("@yemu-compiled-worker-selector-");
	const packageDir = path.resolve(import.meta.dir, "../..");
