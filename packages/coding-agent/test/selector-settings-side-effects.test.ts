...
import { ToolExecutionComponent } from "@yemu/agent-runtime/modes/components/tool-execution";
import { SelectorController } from "@yemu/agent-runtime/modes/controllers/selector-controller";
...
import type { ResolvedRoleModel } from "@yemu/agent-runtime/session/agent-session";
import { AUTO_THINKING } from "@yemu/agent-runtime/thinking";
import { buildModel } from "@yemu/model-catalog/build";
import { getSupportedEfforts } from "@yemu/model-catalog/model-thinking";
...
describe("selector setting side effects", () => {
	...
	it("applies memory backend changes to the live session", () => {
		...
		controller.handleSettingChange("memory.backend", "yemu-memory");
	...
	it("switches a global edit when a byte-identical startup runtime override shadows the project default", async () => {
		...
		fs.mkdirSync(path.join(projectDir, ".yemu"), { recursive: true });
		fs.writeFileSync(path.join(projectDir, ".yemu", "config.yml"), `modelRoles:\n  default: ${projectSelector}\n`);
	...
	it("persists project and global defaults shadowed by a config overlay without switching", async () => {
		...
		try {
			...
			try {
				...
				expect(settings.getModelRole("default")).toBe(overlaySelector);
				expect(settings.getModelRoleProvenance("default")).toBe("overlay");
				expect(await Bun.file(path.join(projectDir, ".yemu", "config.yml")).text()).toContain(
					`default: ${projectSelector}`,
				);
				expect(setModel).not.toHaveBeenCalled();
