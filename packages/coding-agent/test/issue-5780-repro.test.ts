...
import { clearCustomApis, type FetchImpl } from "@yemu/model-runtime";
import { unregisterOAuthProviders } from "@yemu/model-runtime/oauth";
...
describe("issue #5780 post-auth runtime provider refresh", () => {
	...
	beforeEach(async () => {
		tempDir = path.join(os.tmpdir(), `yemu-test-issue-5780-${Snowflake.next()}`);
