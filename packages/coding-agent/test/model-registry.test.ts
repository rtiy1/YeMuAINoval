...
import { Effort, type FetchImpl, type Model, type OpenAICompat, type ThinkingConfig } from "@yemu/model-runtime";
...
describe("ModelRegistry", () => {
	...
	beforeAll(async () => {
		...
		sharedDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-test-mr-shared-"));
	...
	describe("baseUrl override (no custom models)", () => {
		...
		test("refresh keeps transport override on built-in provider (#2555 openrouter gateway)", async () => {
			// Reporter ran `yemu` with the auth-gateway broker proxying OpenRouter.
			...
			// `mergeDiscoveredModel` dropped `transport: yemu-native` (raw catalog
			...
			expect(preRefresh.every(m => m.transport === "yemu-native")).toBe(true);
			...
			// discovered — must still route through the yemu-native transport.
			...
			for (const model of postRefresh) {
				expect(model.transport).toBe("yemu-native");
