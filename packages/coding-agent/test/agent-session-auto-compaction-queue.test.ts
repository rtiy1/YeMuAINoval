...
import * as unexpectedStopClassifier from "@yemu/agent-runtime/session/unexpected-stop-classifier";
import { getBundledModel } from "@yemu/model-catalog/models";
import { getProjectAgentDir, TempDir, withTimeout } from "@yemu/utils";
import * as logger from "@yemu/utils/logger";

const runtimeSignalStoreKey = "__yemuRuntimeSignals";
...
describe("AgentSession auto-compaction queue resume", () => {
	...
	beforeEach(async () => {
		tempDir = TempDir.createSync("@yemu-auto-compaction-queue-");
		vi.useFakeTimers();
		...
		fs.writeFileSync(
			...
			[
				...
				"\t\tconst gate = globalThis.__yemuManualCompactGate;",
				"\t\tif (gate) await gate;",
				"\t\treturn {",
				"\t\t\tcompaction: {",
				'\t\t\t\tsummary: "compacted",',
	...
	afterEach(async () => {
		try {
			...
			try {
				...
				getRuntimeSignals().length = 0;
				(globalThis as typeof globalThis & { __yemuManualCompactGate?: Promise<void> }).__yemuManualCompactGate =
					undefined;
				vi.restoreAllMocks();
			}
		}
