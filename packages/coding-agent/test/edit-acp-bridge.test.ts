...
} from "@yemu/agent-runtime/edit";
import { HashlineFilesystem } from "@yemu/agent-runtime/edit/hashline/filesystem";
import { resolveLocalUrlToPath } from "@yemu/agent-runtime/internal-urls";
import type { WritethroughCallback } from "@yemu/agent-runtime/lsp";
import type { PlanModeState } from "@yemu/agent-runtime/plan-mode/state";
import type { ClientBridge } from "@yemu/agent-runtime/session/client-bridge";
import type { ToolSession } from "@yemu/agent-runtime/tools";
import { computeFileHash } from "@yemu/hashline";
...
describe("HashlineFilesystem ACP fs routing", () => {
	...
	beforeEach(async () => {
		...
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-acp-hashline-"));
	...
	it("keeps a local sandbox artifact addressed by absolute path off the ACP bridge", async () => {
		...
		// the YeMu-owned artifact must be written to disk, never pushed to the editor.
		const { bridge, spy: bridgeSpy } = makeBridge();
		const session = createSession(tmpDir, {
			bridge,
...
describe("executeHashlineSingle model-visible payload under write-time drift", () => {
	...
	beforeEach(async () => {
		resetSettingsForTest();
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-acp-hashline-e2e-"));
		await Settings.init({ inMemory: true, cwd: tmpDir });
	});
...
describe("executeReplaceSingle ACP fs routing", () => {
	...
	beforeEach(async () => {
		resetSettingsForTest();
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-acp-replace-"));
		await Settings.init({ inMemory: true, cwd: tmpDir });
	});
...
describe("executePatchSingle ACP fs routing", () => {
	...
	beforeEach(async () => {
		resetSettingsForTest();
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-acp-patch-"));
		await Settings.init({ inMemory: true, cwd: tmpDir });
	});
