...
import { type AssistantMessageEventStream, clearCustomApis, getCustomApi } from "@yemu/model-runtime";
...
describe("ModelRegistry runtime source cleanup", () => {
	...
	beforeEach(async () => {
		tempDir = path.join(os.tmpdir(), `yemu-test-model-registry-runtime-cleanup-${Snowflake.next()}`);
