...
import { loadLegacyAgentModule } from "@yemu/agent-runtime/extensibility/plugins/legacy-agent-compat";
...
describe("Extension Loader Graph Read Dedup", () => {
	...
	beforeEach(() => {
		tempDir = TempDir.createSync("@yemu-ext-dedup-");
...
`;
		...
		const ns = (await loadLegacyAgentModule(entryPath)) as { readLazy(): Promise<string> };
