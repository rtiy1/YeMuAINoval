...
import { MCPManager } from "@yemu/agent-runtime/mcp/manager";
import type { AgentRef } from "@yemu/agent-runtime/registry/agent-registry";
...
import type { AgentSession, AgentSessionEvent } from "@yemu/agent-runtime/session/agent-session";
import { SessionManager } from "@yemu/agent-runtime/session/session-manager";
import { createPersistedSubagentReviverFactory } from "@yemu/agent-runtime/task/persisted-revive";
...
describe("persisted subagent revival", () => {
	it("cold-revives a restricted contract without loading hostile same-name capabilities", async () => {
		const cwd = makeTempDir("@yemu-restricted-revive-");
	...
	it("preserves normal revival capability wiring for contracts without the marker", async () => {
		const cwd = makeTempDir("@yemu-normal-revive-");
