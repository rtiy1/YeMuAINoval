...
import type { ModelRegistry } from "@yemu/agent-runtime/config/model-registry";
import { Settings } from "@yemu/agent-runtime/config/settings";
import type { LoadExtensionsResult } from "@yemu/agent-runtime/extensibility/extensions/types";
import type { PlanModeState } from "@yemu/agent-runtime/plan-mode/state";
import type { CreateAgentSessionOptions, CreateAgentSessionResult } from "@yemu/agent-runtime/sdk";
import * as sdkModule from "@yemu/agent-runtime/sdk";
import type { AgentSession, AgentSessionEvent, PromptOptions } from "@yemu/agent-runtime/session/agent-session";
import { TaskTool } from "@yemu/agent-runtime/task";
import * as discoveryModule from "@yemu/agent-runtime/task/discovery";
import type { AgentDefinition, TaskParams } from "@yemu/agent-runtime/task/types";
import type { IsolationHandle, WorktreeBaseline } from "@yemu/agent-runtime/task/worktree";
import * as worktreeModule from "@yemu/agent-runtime/task/worktree";
...
import "@yemu/agent-runtime/tools/yield";
...
describe("subagent LSP availability", () => {
	...
	it("opens isolated persisted subagent sessions with the worktree cwd", async () => {
		...
		mockIsolation();
		const { getOptions } = mockCreateAgentSession();
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-isolated-session-cwd-"));
		try {
