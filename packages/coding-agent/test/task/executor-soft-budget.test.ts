...
import { IrcBus } from "@yemu/agent-runtime/irc/bus";
...
import { resolveSoftRequestBudget, runSubprocess } from "@yemu/agent-runtime/task/executor";
...
describe("runSubprocess soft request budget", () => {
	...
	beforeEach(() => {
		...
		tempDir = TempDir.createSync("@yemu-soft-budget-");
	});
	afterEach(() => {
		vi.restoreAllMocks();
		AgentLifecycleManager.resetGlobalForTests();
