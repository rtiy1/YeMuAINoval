...
} from "@yemu/agent-runtime/task/isolation-runner";
import type { SingleResult } from "@yemu/agent-runtime/task/types";
import * as worktreeModule from "@yemu/agent-runtime/task/worktree";
import * as gitModule from "@yemu/agent-runtime/utils/git";
...
async function seedFooRepo(finalContent: string): Promise<{ repoRoot: string; patchPath: string }> {
	const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-isolation-merge-"));
...
describe("runIsolatedSubprocess", () => {
	...
	it("preserves branch-mode output as a patch when branch transfer fails", async () => {
		const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-isolation-run-"));
		tempRoots.push(repoRoot);
		...
		expect(deleteSpy).toHaveBeenCalledWith(repoRoot, "yemu/task/PreserveBranchFailure");
