...
import {
	...
	mergeTaskBranches,
	parseIsolationMode,
} from "@yemu/agent-runtime/task/worktree";
import * as git from "@yemu/agent-runtime/utils/git";
import * as jj from "@yemu/agent-runtime/utils/jj";
import * as natives from "@yemu/native";
import { removeWithRetries, setWorktreesDir } from "@yemu/utils";
...
async function createGitRepo(): Promise<{ baseBranch: string; repo: string }> {
	const repo = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-worktree-"));
	tempDirs.push(repo);
...
describe("worktree isolation helpers", () => {
	...
	describe("git-backed worktree helpers", () => {
		...
		beforeAll(async () => {
			repo = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-worktree-"));
		...
		it("uses compact isolation paths that do not embed long task ids", async () => {
			const originalWorktreeDir = process.env.YEMU_WORKTREE_DIR;
			const worktreeBase = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-worktree-base-"));
			tempDirs.push(worktreeBase);
			delete process.env.YEMU_WORKTREE_DIR;
			...
			try {
				...
				if (originalWorktreeDir === undefined) {
					...
					process.env.YEMU_WORKTREE_DIR = originalWorktreeDir;
				}
		...
		describe("after rewinding the shared fixture", () => {
			...
			it("keeps the index clean when stash pop would conflict with a cherry-picked change", async () => {
				...
				expect(stashList).toContain("yemu-task-merge");
			...
			it("commits isolated edits when parent dirt only changes nearby context", async () => {
				...
				const isoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-worktree-iso-"));
				...
				let branchName = `yemu/task/${taskId}`;
				try {
...
describe("getRepoRoot", () => {
	...
	it("rejects pure jj workspaces with an actionable Jujutsu message", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-purejj-"));
		tempDirs.push(dir);
	...
	it("preserves the generic git-not-found error for directories without any repo", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-norepo-"));
		tempDirs.push(dir);
	...
	it("returns the nested git root when a git checkout lives under an outer jj workspace", async () => {
		...
		const outer = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-outerjj-"));
		tempDirs.push(outer);
...
describe("detachGitDir", () => {
	...
	async function makeLinkedWorktree(): Promise<{ main: string; wt: string; commonDir: string; baseSha: string }> {
		const main = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-detach-main-"));
		tempDirs.push(main);
	...
	async function copyTree(source: string): Promise<string> {
		const iso = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-detach-iso-"));
		tempDirs.push(iso);
	...
	it("severs a copied linked-worktree from the parent so task git ops stay isolated", async () => {
		...
		await runGit(wt, ["fetch", iso, "feature/a:refs/heads/yemu-fetched"]);
		expect(await runGit(wt, ["rev-parse", "yemu-fetched"])).toBe(taskCommit);
	});

	it("leaves an already-independent full-copy checkout untouched", async () => {
		const src = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-detach-src-"));
		tempDirs.push(src);
	...
	it("carries filemode, split-index, and shallow state into the detached repo", async () => {
		...
		const origin = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-detach-origin-"));
	...
	it("detaches when sourceCommonDir is reached through a symlinked path", async () => {
		...
		const aliasBase = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-detach-alias-"));
	...
	it("keeps ensureIsolation from mutating a linked-worktree parent (rcopy backend)", async () => {
		...
		const worktreeBase = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-detach-wtbase-"));
		tempDirs.push(worktreeBase);
		const originalWorktreeDir = process.env.YEMU_WORKTREE_DIR;
		delete process.env.YEMU_WORKTREE_DIR;
		setWorktreesDir(worktreeBase);
		try {
			...
			setWorktreesDir(undefined);
			if (originalWorktreeDir === undefined) delete process.env.YEMU_WORKTREE_DIR;
			else process.env.YEMU_WORKTREE_DIR = originalWorktreeDir;
		}
	});
});
...
describe("applyNestedPatches", () => {
	...
	beforeEach(async () => {
		parentRepo = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-nested-apply-"));
	...
	it("returns a stash-restore warning when pop conflicts with the agent commit", async () => {
		...
		expect(committedFiles.trim()).toBe("file.txt");
		expect(stashList).toContain("yemu-isolation-");
	});
});
...
describe("commitToBranch preserves agent commits", () => {
	...
	beforeEach(async () => {
		parent = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-commit-parent-"));
		isolation = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-commit-iso-"));
	...
	it("preserves the agent's commit message after merge", async () => {
		...
		expect(result?.branchName).toBe(`yemu/task/${taskId}`);
	...
	it("preserves every message when the agent makes multiple commits", async () => {
		...
		expect(result?.branchName).toBe("yemu/task/multi");

		const merge = await mergeTaskBranches(parent, [
			{ branchName: result!.branchName!, taskId: "multi", baseSha: result!.baseSha! },
		]);
		expect(merge).toEqual({ failed: [], merged: ["yemu/task/multi"] });
	...
	it("appends one trailing commit when the agent leaves uncommitted work after committing", async () => {
		...
		expect(result?.branchName).toBe("yemu/task/leftover");
	...
	it("filters baseline WIP when the agent commits with git add -A", async () => {
		...
		expect(result?.branchName).toBe("yemu/task/dirty-baseline");
		expect(aiMessage).not.toHaveBeenCalled();
		...
		expect(merge).toEqual({ failed: [], merged: ["yemu/task/dirty-baseline"] });
	...
	it("falls back to the AI-generated message when the agent never committed", async () => {
		...
		expect(result?.branchName).toBe("yemu/task/nocommit");
		expect(aiMessage).toHaveBeenCalledTimes(1);
	...
	describe("with baseline WIP overlapping the agent's changes (#4136)", () => {
		...
		it("commits an agent-only delta via --3way when WIP and agent modify unrelated hunks of the same tracked file", async () => {
			...
			expect(result?.branchName).toBe("yemu/task/wip-tracked-file");
		...
		it("commits an untracked WIP file that the agent modifies inside isolation", async () => {
			...
			expect(result?.branchName).toBe("yemu/task/wip-untracked");
		...
		it("commits a staged-new WIP file that the agent modifies inside isolation", async () => {
			...
			expect(result?.branchName).toBe("yemu/task/wip-staged-new");
		...
		it("does not leak WIP-only files into the branch commit when the agent leaves them untouched", async () => {
			...
			expect(result?.branchName).toBe("yemu/task/wip-filter");
		...
		it("still seeds WIP when the agent commits only baseline WIP and leaves the real edit uncommitted", async () => {
			...
			expect(result?.branchName).toBe("yemu/task/wip-only-commit");
