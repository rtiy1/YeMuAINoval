...
} from "@yemu/agent-runtime/internal-urls/registry-helpers";
import * as planHandoff from "@yemu/agent-runtime/plan-mode/plan-handoff";
...
import * as isolationRunner from "@yemu/agent-runtime/task/isolation-runner";
import {
...
} from "@yemu/agent-runtime/task/structured-subagent";
...
describe("structured subagent primitive", () => {
	...
	it("gives task and eval invocations identical blocked-agent preflight errors", async () => {
		const previous = Bun.env.YEMU_BLOCKED_AGENT;
		Bun.env.YEMU_BLOCKED_AGENT = "worker";
		try {
			...
			if (previous === undefined) delete Bun.env.YEMU_BLOCKED_AGENT;
			else Bun.env.YEMU_BLOCKED_AGENT = previous;
		}
	});
	...
	it("leases temporary artifacts for a retained invocation and registers them for agent URLs", async () => {
		...
		expect(path.basename(settled.artifactsDir)).toStartWith("yemu-task-");
	...
	it("persists nested patch text with the compatible recovery path and wording", async () => {
		const artifactsDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-structured-subagent-"));
		const completed = result();
		completed.patchPath = "/recovery/Worker.patch";
		completed.branchName = "yemu/task/Worker";
		...
		expect(hint).toContain("Captured branch preserved as yemu/task/Worker.");
