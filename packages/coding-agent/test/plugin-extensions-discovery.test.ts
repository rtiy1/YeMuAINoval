...
import { getAgentDir, getPluginsDir, removeSyncWithRetries, setAgentDir, TempDir } from "@yemu/utils";
...
describe("plugin extension discovery", () => {
	...
	beforeEach(() => {
		projectDir = TempDir.createSync("@yemu-plugin-ext-");
		// Redirect the whole config root to an isolated temp home so plugin discovery
		// resolves into `<tempHome>/.yemu/plugins` on every platform. Two things are needed:
		//  - mock os.homedir() so configRoot = `<tempHome>/.yemu` (the previous
		//    XDG_DATA_HOME redirect was a no-op on Windows, where these tests then wrote
		//    into and rm'd the developer's real `~/.yemu/plugins`);
		//  - clear the XDG_* vars, because on Linux/macOS the resolver prefers
		//    `$XDG_DATA_HOME/yemu` over the home config root when that dir exists, so an
		//    XDG-migrated environment would otherwise still resolve the real plugins dir.
		tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-plugin-home-"));
		for (const key of xdgVars) {
			originalXdg.set(key, process.env[key]);
			delete process.env[key];
		}
		spyOn(os, "homedir").mockReturnValue(tempHome);
		setAgentDir(path.join(tempHome, ".yemu", "agent"));
		...
		// ~/.yemu must fail loudly here instead of clobbering the developer's plugins.
		...
		fs.writeFileSync(
			...
			`
				export default function(yemu) {
					yemu.registerCommand("plugin-ext", { handler: async () => {} });
				}
			`,
		);
	});
	...
	it("loads installed legacy YeMu plugin extensions from Windows drive-letter paths", async () => {
		const pluginsDir = getPluginsDir();
		const pluginDir = path.join(pluginsDir, "node_modules", "legacy-agent-plugin");
		...
		fs.writeFileSync(
			...
			[
				...
				"\tpi.registerTool({",
				'\t\tname: "legacy-agent-ext",',
				'\t\tdescription: "Legacy YeMu extension smoke test",',
		...
		expect(result.errors).toHaveLength(0);
		expect(extension).toBeDefined();
		expect(extension?.tools.has("legacy-agent-ext")).toBe(true);
	});

	it("loads installed legacy YeMu plugin extensions that use package imports", async () => {
		const pluginsDir = getPluginsDir();
	...
	it("rewrites side-effect imports of package-import aliases and legacy YeMu scopes", async () => {
		const pluginsDir = getPluginsDir();
	...
	it("loads installed plugin extensions whose manifest entry points at a directory with index.ts", async () => {
		...
		const extensionDir = path.join(pluginDir, ".yemu", "extensions", "dir-entry");
		...
		fs.writeFileSync(
			...
			JSON.stringify({
				...
				yemu: {
					...
					extensions: [".yemu/extensions/dir-entry"],
				},
			}),
		);
		fs.writeFileSync(
		...
		const result = await discoverAndLoadExtensions([], projectDir.path());
		const extension = result.extensions.find(ext => ext.path === extensionPath);
		const pluginError = result.errors.find(err => err.path.includes(path.join("dir-entry-plugin", ".yemu")));
		...
		fs.writeFileSync(
			...
			JSON.stringify({
				...
				yemu: {
					...
					// level and pick up `extensions/<name>/index.ts` (yemu package layout).
	...
	it("resolves a sub-extension directory via its own package.json manifest over a decoy index", async () => {
		...
		fs.writeFileSync(
			...
			JSON.stringify({ name: "feature-ext", version: "1.0.0", yemu: { extensions: ["./dist/real-ext.ts"] } }),
		);
		fs.writeFileSync(
			realEntry,
			[
	...
	it("skips .d.ts declaration files when scanning a flat extensions directory", async () => {
		...
		fs.writeFileSync(
			moduleEntry,
			["export default function(yemu) {", '\tpi.registerCommand("dts-ext", { handler: async () => {} });', "}"].join(
				"\n",
			),
		);
		// A sibling declaration file must be ignored — importing it would fail.
		fs.writeFileSync(path.join(extensionsDir, "ext.d.ts"), "export default function (yemu: unknown): void;\n");
