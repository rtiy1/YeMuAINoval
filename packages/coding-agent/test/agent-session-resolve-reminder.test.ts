...
import { Agent, isSoftToolRequirement } from "@yemu/agent-core";
...
import { dispatchResolutionDevice, queueResolveHandler } from "@yemu/agent-runtime/tools/resolve";
import { buildNamedToolChoice } from "@yemu/agent-runtime/utils/tool-choice";
import { getBundledModel } from "@yemu/model-catalog/models";
import { createMockModel, type MockModel } from "@yemu/model-runtime/providers/mock";
...
describe("AgentSession resolve reminder", () => {
	...
	beforeEach(async () => {
		tempDir = path.join(os.tmpdir(), `yemu-resolve-reminder-test-${Snowflake.next()}`);
