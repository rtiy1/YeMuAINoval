...
import { applyResolvedSystemPromptInputs, submitInteractiveInput } from "@yemu/agent-runtime/main";
import type { SubmittedUserInput } from "@yemu/agent-runtime/modes/types";
import type { CreateAgentSessionOptions } from "@yemu/agent-runtime/sdk";
import { discoverTitleSystemPromptFile } from "@yemu/agent-runtime/system-prompt";
...
describe("discoverTitleSystemPromptFile", () => {
	it("discovers TITLE_SYSTEM.md from the project yemu config directory", async () => {
		const projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-title-system-"));
		cleanupDirs.push(projectDir);
		const configDir = path.join(projectDir, ".yemu");
