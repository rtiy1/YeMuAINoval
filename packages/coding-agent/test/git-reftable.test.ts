...
describe.skipIf(!supportsReftable)("git reftable support", () => {
	...
	beforeAll(async () => {
		...
		sharedRepoDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-reftable-"));
		...
		worktreeDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-reftable-wt-"));
		...
		configRepoDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-reftable-cfg-"));
