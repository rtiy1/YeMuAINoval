...
import { listSessions } from "@yemu/agent-runtime/session/session-listing";
...
import { MemorySessionStorage } from "@yemu/agent-runtime/session/session-storage";
...
describe("large session memory guards", () => {
	...
	it("streams large session files without discarding historical compactions", async () => {
		const tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), "yemu-large-session-"));
	...
	it("preserves loaded compactions on every branch", async () => {
		const tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), "yemu-branch-load-"));
		tempDirs.push(tempDir);
