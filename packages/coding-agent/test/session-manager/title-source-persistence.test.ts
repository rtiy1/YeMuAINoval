...
} from "@yemu/agent-runtime/session/session-entries";
...
import { FileSessionStorage, type WriteTextAtomicOptions } from "@yemu/agent-runtime/session/session-storage";
import type { SessionTitleUpdate } from "@yemu/agent-runtime/session/session-title-slot";
...
describe("session title source persistence", () => {
	...
	beforeEach(() => {
		testAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-title-source-"));
