...
import { readTerminalBreadcrumbEntry } from "@yemu/agent-runtime/session/session-paths";
...
describe("SessionManager subagent breadcrumb isolation", () => {
	...
	beforeEach(async () => {
		...
		testAgentDir = await fsp.mkdtemp(path.join(os.tmpdir(), "yemu-subagent-crumb-"));
