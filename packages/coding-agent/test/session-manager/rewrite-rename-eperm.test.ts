...
import { recoverOrphanedBackups } from "@yemu/agent-runtime/session/session-listing";
import { SessionManager } from "@yemu/agent-runtime/session/session-manager";
import { FileSessionStorage, MemorySessionStorage } from "@yemu/agent-runtime/session/session-storage";
...
describe("SessionManager rewrite EPERM replacement fallback", () => {
	...
	beforeEach(async () => {
		sessionDir = await fsp.mkdtemp(path.join(os.tmpdir(), "yemu-eperm-"));
	});
...
describe("FileSessionStorage.writeTextAtomic commitGuard cleanup", () => {
	...
	beforeEach(async () => {
		sessionDir = await fsp.mkdtemp(path.join(os.tmpdir(), "yemu-guard-cleanup-"));
	});
