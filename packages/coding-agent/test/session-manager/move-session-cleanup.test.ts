...
import { cleanupEmptyMoveSession, SessionManager } from "@yemu/agent-runtime/session/session-manager";
...
describe("move-session cleanup tracking", () => {
	...
	beforeEach(async () => {
		testAgentDir = await fsp.mkdtemp(path.join(os.tmpdir(), "yemu-move-cleanup-"));
