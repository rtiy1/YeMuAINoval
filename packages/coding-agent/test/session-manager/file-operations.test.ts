...
import type { FileEntry, SessionHeader } from "@yemu/agent-runtime/session/session-entries";
import { findMostRecentSession, resolveResumableSession } from "@yemu/agent-runtime/session/session-listing";
...
import { getConfigRootDir, getSessionsDir, removeSyncWithRetries, Snowflake, setAgentDir } from "@yemu/utils";
...
describe("SessionManager temp cwd session dirs", () => {
	...
	beforeEach(() => {
		testAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-session-dir-test-"));
...
describe("SessionManager legacy session migration persistence", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-session-manager-legacy-"));
	});
