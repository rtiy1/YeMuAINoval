...
import { EventController } from "@yemu/agent-runtime/modes/controllers/event-controller";
import { InputController } from "@yemu/agent-runtime/modes/controllers/input-controller";
import { getThemeByName, setThemeInstance } from "@yemu/agent-runtime/modes/theme/theme";
import type { CompactionQueuedMessage, InteractiveModeContext } from "@yemu/agent-runtime/modes/types";
...
import { SKILL_PROMPT_MESSAGE_TYPE, type SkillPromptDetails } from "@yemu/agent-runtime/session/messages";
...
import { Container } from "@yemu/tui";
...
describe("InputController skill queue chip metadata", () => {
	...
	beforeEach(async () => {
		tempDir = TempDir.createSync("@yemu-skill-queue-stub-");
...
describe("compaction skill re-invocation", () => {
	...
	beforeEach(async () => {
		tempDir = TempDir.createSync("@yemu-skill-compaction-stub-");
		const skill = await writeSkillFile(tempDir.path(), "test-skill", "Do the thing.");
		skillCommands = new Map<string, Skill>([["skill:test-skill", skill]]);
	});
...
async function createRealSession(): Promise<SessionFixture> {
	const tempDir = TempDir.createSync("@yemu-skill-queue-real-");
