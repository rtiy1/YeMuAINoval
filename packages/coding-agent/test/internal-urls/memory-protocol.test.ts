...
import { getMemoryRoot } from "@yemu/agent-runtime/memories";
...
import { GlobTool } from "@yemu/agent-runtime/tools/glob";
import {
	loadYeMuMemory,
	loadYeMuMemoryCore,
...
import { getAgentDir, removeWithRetries, setAgentDir, TempDir } from "@yemu/utils";

// YeMuMemory state is loaded lazily; preload so `new YeMuMemorySessionState(...)` can
...
describe("MemoryProtocolHandler", () => {
	...
	it("throws for unknown memory namespace when no yemu-memory backend is active", async () => {
...
interface YeMuMemoryFixture {
	state: YeMuMemorySessionState;
	dbDir: TempDir;
}

async function withYeMuMemorySession(
	fn: (fixture: YeMuMemoryFixture) => Promise<void>,
	options: { bank?: string } = {},
): Promise<void> {
	const dbDir = TempDir.createSync(`memory-protocol-yemu-memory-${Date.now()}-`);
	const bank = options.bank ?? "test-bank";
	const config = {
		dbPath: dbDir.join("yemu-memory.db"),
		bank,
		autoRecall: false,
		autoRetain: false,
	...
	} as unknown as ConstructorParameters<typeof YeMuMemorySessionState>[0]["config"];
	const session = {
		sessionId: "test-yemu-memory",
		sessionManager: {
			getEntries: () => [],
			getCwd: () => dbDir.path(),
			getArtifactsDir: () => null,
			getSessionId: () => "test-yemu-memory",
		},
		emitNotice: () => {},
		getHindsightSessionState: () => undefined,
	} as unknown as AgentSession;
	const state = new YeMuMemorySessionState({ sessionId: "test-yemu-memory", config, session });
	setYeMuMemorySessionState(session, state);
	AgentRegistry.global().register({
		id: "test-yemu-memory",
		displayName: "test-yemu-memory",
		kind: "main",
		session,
		sessionFile: null,
	});
...
describe("MemoryProtocolHandler — yemu-memory bridge (issue #4443)", () => {
	beforeEach(() => {
		AgentRegistry.resetGlobalForTests();
		InternalUrlRouter.resetForTests();
	});
	...
	it("resolves memory://<id> to the full yemu-memory memory row", async () => {
		await withYeMuMemorySession(async ({ state }) => {
	...
	it("throws a clear error when the yemu-memory id is not stored in any scoped bank", async () => {
		await withYeMuMemorySession(async () => {
			const router = InternalUrlRouter.instance();
			await expect(router.resolve("memory://deadbeefdeadbeef")).rejects.toThrow(
				/YeMuMemory memory deadbeefdeadbeef not found/,
			);
		});
	});
	...
	it("routes memory://root to the file-backed summary even when yemu-memory is active", async () => {
		await withYeMuMemorySession(async () => {
			const router = InternalUrlRouter.instance();
