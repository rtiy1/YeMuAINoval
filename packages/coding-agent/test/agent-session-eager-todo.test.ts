...
import { AgentSession, type AgentSessionConfig } from "@yemu/agent-runtime/session/agent-session";
...
import { TodoTool } from "@yemu/agent-runtime/tools";
import { getBundledModel } from "@yemu/model-catalog/models";
import type { AssistantMessage, TextContent, ToolCall } from "@yemu/model-runtime";
import * as ai from "@yemu/model-runtime";
import { AssistantMessageEventStream } from "@yemu/model-runtime/utils/event-stream";
import { setInteractiveHost, TempDir } from "@yemu/utils";
...
describe("AgentSession eager todo enforcement", () => {
	...
	beforeEach(async () => {
		tempDir = TempDir.createSync("@yemu-agent-session-eager-todo-");
		streamCallCount = 0;
		scriptedResponses = [];
		observedCalls.length = 0;
		await createSession();
