...
import * as sessionColor from "@yemu/agent-runtime/utils/session-color";
import type { Container, NativeScrollbackLiveRegion } from "@yemu/tui";
...
async function createHarness(sessionName: string): Promise<Harness> {
	const tempDir = TempDir.createSync("@yemu-working-accent-");
