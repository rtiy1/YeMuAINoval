...
import CommitCommand from "@yemu/agent-runtime/commands/commit";
import * as commitModule from "@yemu/agent-runtime/commit";
import * as themeModule from "@yemu/agent-runtime/modes/theme/theme";
import { postmortem } from "@yemu/utils";

describe("yemu commit command lifecycle (issue #1041)", () => {
	afterEach(() => {
		vi.restoreAllMocks();
	});
