...
import type { SegmentContext } from "@yemu/agent-runtime/modes/components/status-line/segments";
import { renderSegment } from "@yemu/agent-runtime/modes/components/status-line/segments";
import { initTheme, theme } from "@yemu/agent-runtime/modes/theme/theme";
import { getProjectDir, removeSyncWithRetries, setProjectDir } from "@yemu/utils";
...
function createFakeHome(): { home: string; projectsRoot: string } {
	...
	const home = fs.mkdtempSync(path.join(homeRoot, "yemu-status-line-home-"));
...
describe("status line path segment", () => {
	it("strips the Projects root for symlink-equivalent aliases", () => {
		...
		const realProjectDir = fs.mkdtempSync(path.join(projectsRoot, "yemu-status-line-"));
		const nestedDir = path.join(realProjectDir, "nested");
		const aliasRoot = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-status-line-alias-"));
	...
	it("strips the scratch root and shows only the trailing folder inside the OS tmp dir", () => {
		const scratchDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-status-line-scratch-"));
		try {
	...
	it("keeps nested subpaths visible under a scratch root", () => {
		const scratchDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-status-line-scratch-nest-"));
	...
	it("keeps the folder icon for scratch paths when stripWorkPrefix is disabled", () => {
		const scratchDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-status-line-scratch-noprefix-"));
		try {
			setProjectDir(scratchDir);
	...
	it("keeps the folder icon for paths outside any scratch root", () => {
		const { home, projectsRoot } = createFakeHome();
		const realProjectDir = fs.mkdtempSync(path.join(projectsRoot, "yemu-status-line-real-"));
		try {
	...
	it("renders the active nested repo suffix after the parent cwd", () => {
		const parentDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-status-line-parent-"));
...
describe("status line path segment in a linked worktree", () => {
	...
	it("collapses to the project name and drops the worktree dir when it equals the branch", () => {
		const rendered = renderSegment("path", worktreeContext({ projectName: "yemu", worktreeName: "xx" }, "xx"));
		const content = Bun.stripANSI(rendered.content);
		expect(rendered.visible).toBe(true);
		expect(content).toBe(`${theme.icon.worktree} yemu`);
	...
	it("keeps the worktree dir when it diverges from the branch", () => {
		const rendered = renderSegment("path", worktreeContext({ projectName: "yemu", worktreeName: "wt-icon" }, "icon"));
		expect(Bun.stripANSI(rendered.content)).toBe(`${theme.icon.worktree} yemu/wt-icon`);
	});

	it("keeps the worktree dir when no branch is shown", () => {
		const rendered = renderSegment("path", worktreeContext({ projectName: "yemu", worktreeName: "xx" }, null));
		expect(Bun.stripANSI(rendered.content)).toBe(`${theme.icon.worktree} yemu/xx`);
	});

	it("falls back to the on-disk path when stripWorkPrefix is disabled", () => {
		const scratchDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-status-line-wt-noprefix-"));
		try {
			setProjectDir(scratchDir);
			const ctx = worktreeContext({ projectName: "yemu", worktreeName: "xx" }, "xx");
			ctx.options.path = { ...ctx.options.path, stripWorkPrefix: false };
