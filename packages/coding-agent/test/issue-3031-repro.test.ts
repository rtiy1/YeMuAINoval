/**
 * Regression for upstream-change-3031
 *
 * YeMuMemory's local embedding provider used to `import("onnxruntime-node")` and
 * `import("fastembed")` directly inside `fastembed-runtime.ts`. With
 * `memory.backend: yemu-memory` enabled on Windows that crashed Bun in two ways:
 ...
 * agent's main process hands `yemu-memory.setLocalModelInitializer` a wrapper that
 * round-trips through `__yemu_worker_yemu-memory_embed`, and `SIGKILL`s the child
...
import {
	createYeMuMemoryEmbedSubprocess,
	YeMuMemoryEmbedClient,
	type YeMuMemoryEmbedWorkerHandle,
} from "@yemu/agent-runtime/yemu-memory/embed-client";
import type {
	YeMuMemoryEmbedWorkerInbound,
	YeMuMemoryEmbedWorkerOutbound,
} from "@yemu/agent-runtime/yemu-memory/embed-protocol";

describe("issue #3031 — yemu-memory embeddings live in an isolated subprocess", () => {
	it("ping/pongs through the spawned worker subprocess and tears it down cleanly", async () => {
		// `smokeTestYeMuMemoryEmbedWorker` is the runtime probe wired into
		// `yemu --smoke-test`. Run it in a child Bun process instead of this
		...
		const script =
			'const { smokeTestYeMuMemoryEmbedWorker } = await import("@yemu/agent-runtime/yemu-memory/embed-client"); await smokeTestYeMuMemoryEmbedWorker({ timeoutMs: 15000 });';
	...
	it("surfaces unexpected signal exits so in-flight callers don't await forever", async () => {
		...
		const sub = createYeMuMemoryEmbedSubprocess();
		try {
	...
	it("carries (model, cacheDir) on every embed so a respawned worker can self-init", async () => {
		// Without this contract: after `shutdownYeMuMemoryEmbedClient()` runs on
		// session dispose, yemu-memory still holds the cached `LocalEmbeddingModel`
		...
		const sentMessages: YeMuMemoryEmbedWorkerInbound[] = [];
		let messageHandler: ((message: YeMuMemoryEmbedWorkerOutbound) => void) | undefined;
		const spawnCount = { value: 0 };
		const spawn = (): YeMuMemoryEmbedWorkerHandle => {
			spawnCount.value += 1;
			return {
				send(message) {
		...
		const client = new YeMuMemoryEmbedClient(spawn);
		...
		const embeds = sentMessages.filter(
			(m): m is Extract<YeMuMemoryEmbedWorkerInbound, { type: "embed" }> => m.type === "embed",
		);
