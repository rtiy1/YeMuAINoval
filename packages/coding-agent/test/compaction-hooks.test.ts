...
import {
	...
	type SessionEvent,
} from "@yemu/agent-runtime/extensibility/hooks";
import { theme } from "@yemu/agent-runtime/modes/theme/theme";
...
describe.skipIf(!e2eApiKey("ANTHROPIC_API_KEY"))("Compaction hooks", () => {
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `yemu-compaction-hooks-test-${Snowflake.next()}`);
