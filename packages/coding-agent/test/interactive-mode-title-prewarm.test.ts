...
describe("InteractiveMode tiny-title prewarm", () => {
	...
	let tempDir: TempDir;
	// Titling (and thus the prewarm gate) is disabled when YEMU_NO_TITLE is set,
	...
	beforeEach(async () => {
		...
		previousNoTitle = Bun.env.YEMU_NO_TITLE;
		delete Bun.env.YEMU_NO_TITLE;

		resetSettingsForTest();
		tempDir = TempDir.createSync("@yemu-interactive-mode-title-prewarm-");
	...
	afterEach(async () => {
		...
		tempDir?.removeSync();
		resetSettingsForTest();
		if (previousNoTitle === undefined) delete Bun.env.YEMU_NO_TITLE;
		else Bun.env.YEMU_NO_TITLE = previousNoTitle;
	});
