...
import { closeAllAutoresearchStorages } from "@yemu/agent-runtime/autoresearch/storage";
import type {
...
describe("autoresearch before_agent_start handler", () => {
	...
	beforeEach(() => {
		dbDir = TempDir.createSync("@yemu-autoresearch-bas-test-");
		process.env.YEMU_AUTORESEARCH_DB_DIR = dbDir.path();
		cwdDir = TempDir.createSync("@yemu-autoresearch-bas-cwd-");
