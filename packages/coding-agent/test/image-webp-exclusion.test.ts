...
} from "@yemu/agent-runtime/utils/image-loading";
...
describe("modelLacksWebpSupport", () => {
	...
	test("webpExclusionForModel yields true|undefined so the YEMU_NO_WEBP fallback survives", () => {
...
describe("normalizeModelContextImages model-aware WebP exclusion", () => {
	const prior = Bun.env.YEMU_NO_WEBP;

	beforeEach(() => {
		delete (Bun.env as Record<string, string | undefined>).YEMU_NO_WEBP;
	});

	afterEach(() => {
		if (prior === undefined) delete (Bun.env as Record<string, string | undefined>).YEMU_NO_WEBP;
		else Bun.env.YEMU_NO_WEBP = prior;
	});
	...
	test("keeps WebP for a WebP-capable model when YEMU_NO_WEBP is unset", async () => {
