...
import { Agent, type AgentMessage, type AgentTool, AppendOnlyContextManager, type StreamFn } from "@yemu/agent-core";
...
import * as memoryBackend from "@yemu/agent-runtime/memory-backend";
import type { MemoryBackend } from "@yemu/agent-runtime/memory-backend/types";
import { createAgentSession, type ExtensionFactory } from "@yemu/agent-runtime/sdk";
import { obfuscateProviderContext, SecretObfuscator } from "@yemu/agent-runtime/secrets";
import { AgentSession, type AgentSessionEvent } from "@yemu/agent-runtime/session/agent-session";
import { AuthStorage } from "@yemu/agent-runtime/session/auth-storage";
import { convertToLlm, wrapSteeringForModel } from "@yemu/agent-runtime/session/messages";
import { SessionManager } from "@yemu/agent-runtime/session/session-manager";
import { setYeMuMemorySessionState, type YeMuMemorySessionState } from "@yemu/agent-runtime/yemu-memory/state";
...
async function withNativeDialectEnv<T>(fn: () => Promise<T>): Promise<T> {
	const previous = Bun.env.YEMU_DIALECT;
	delete Bun.env.YEMU_DIALECT;
	try {
		return await fn();
	} finally {
...
describe("AgentSession message pipeline", () => {
	...
	it("preserves append-only prefixes in subagent sessions when context handlers rewrite prior turns", async () => {
		using tempDir = TempDir.createSync("@yemu-subagent-append-only-");
		...
		const rewritePriorAssistant: ExtensionFactory = yemu => {
			yemu.on("context", async event => {
	...
	it("applies a tool_call input revision at arg-prep time across events, execution, and history", async () => {
		...
		using tempDir = TempDir.createSync("@yemu-tool-call-revision-");
		...
		let handlerCalls = 0;
		const reviseBash: ExtensionFactory = yemu => {
	...
	it("clears promoted memory from the base prompt when switching sessions", async () => {
		using tempDir = TempDir.createSync("@yemu-injected-memory-switch-");
		...
		sessions.push(session);
		setYeMuMemorySessionState(session, {
			aliasOf: undefined,
			...
			async dispose() {},
		} as unknown as YeMuMemorySessionState);
	...
	it("does not duplicate promoted memory in the base prompt when forking", async () => {
		using tempDir = TempDir.createSync("@yemu-injected-memory-fork-");
		const sessionManager = SessionManager.create(tempDir.path(), tempDir.join("sessions"));
