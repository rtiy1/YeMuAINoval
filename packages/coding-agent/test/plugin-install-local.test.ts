/**
 * Routing tests for `yemu plugin install <local-path>` (#1945).
 *
...
import { runPluginCommand } from "@yemu/agent-runtime/cli/plugin-cli";
import { PluginManager } from "@yemu/agent-runtime/extensibility/plugins/manager";
import { MarketplaceManager } from "@yemu/agent-runtime/extensibility/plugins/marketplace";
import type { InstalledPlugin } from "@yemu/agent-runtime/extensibility/plugins/types";
...
async function createLocalPlugin(root: string): Promise<string> {
	...
	await Bun.write(
		...
		JSON.stringify({
			name: "kimi-datasource",
			version: "1.0.0",
			yemu: { extensions: ["./src/extension.ts"] },
		}),
	);
	return localPlugin;
}
...
describe("runPluginCommand({ action: 'install', args: [<local>] })", () => {
	...
	beforeEach(async () => {
		tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-plugin-install-local-"));
		...
		spyOn(yemuUtils, "getPluginsNodeModules").mockReturnValue(path.join(pluginsDir, "node_modules"));
		spyOn(yemuUtils, "getPluginsPackageJson").mockReturnValue(path.join(pluginsDir, "package.json"));
		spyOn(yemuUtils, "getPluginsLockfile").mockReturnValue(path.join(tmpRoot, "yemu-plugins.lock.json"));
	...
	afterEach(async () => {
		...
		// linkSpy/installSpy/console spies. Without this, the yemuUtils.*
	...
	test("real local plugin directory: install symlinks it like link would", async () => {
		...
		// symlink rather than copy-install, matching `yemu plugin link`.
	...
	test("doctor --fix preserves linked local plugin state without package dependencies", async () => {
		await Bun.write(
			...
			JSON.stringify({ name: "yemu-plugins", private: true, dependencies: {} }),
		);
		const localPlugin = await createLocalPlugin(tmpRoot);
