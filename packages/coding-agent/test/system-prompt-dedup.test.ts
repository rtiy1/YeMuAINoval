...
} from "@yemu/agent-runtime/system-prompt";
...
describe("SYSTEM.md prompt assembly", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-system-prompt-"));
		tempHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-system-home-"));
	...
	it("renders SYSTEM.md exactly once when it is used as the custom base prompt", async () => {
		...
		const systemDir = path.join(projectDir, ".yemu");
	...
	it("suppresses discovered SYSTEM.md while preserving the project footer", async () => {
		...
		fs.writeFileSync(path.join(projectDir, ".yemu", "SYSTEM.md"), "Discovered project SYSTEM prompt");
	...
	it("prefers project SYSTEM.md over user SYSTEM.md", async () => {
		const projectDir = path.join(tempDir, "project");
		fs.mkdirSync(path.join(projectDir, ".yemu"), { recursive: true });
		fs.mkdirSync(path.join(tempHomeDir, ".yemu", "agent"), { recursive: true });
		fs.writeFileSync(path.join(tempHomeDir, ".yemu", "agent", "SYSTEM.md"), "User SYSTEM prompt");
		fs.writeFileSync(path.join(projectDir, ".yemu", "SYSTEM.md"), "Project SYSTEM prompt");
