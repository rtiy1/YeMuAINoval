...
import { KeybindingsManager as AppKeybindingsManager } from "@yemu/agent-runtime/config/keybindings";
import { getGithubRefContext, getGithubRefSuggestions } from "@yemu/agent-runtime/modes/github-ref-autocomplete";
import { createPromptActionAutocompleteProvider } from "@yemu/agent-runtime/modes/prompt-action-autocomplete";
import type { SlashCommand } from "@yemu/tui";
...
describe("github-ref autocomplete — token detection", () => {
	...
	it("requires a token boundary before # so embedded hashes don't match", () => {
		...
		expect(getGithubRefContext("https://github.com/rtiy1/YeMuAINoval#3164")).toBeNull();
