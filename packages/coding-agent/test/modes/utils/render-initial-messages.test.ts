 ...
 *
 * Also guards the cold-launch terminal cleanup: `yemu` / `yemu -c` leave the
...
import type { SessionContext } from "@yemu/agent-runtime/session/session-context";
import { SessionManager } from "@yemu/agent-runtime/session/session-manager";
import type { AssistantMessage, ImageContent, Usage } from "@yemu/model-runtime";
import { kStreamingPartialJson } from "@yemu/model-runtime/utils/block-symbols";
...
describe("UiHelpers.renderInitialMessages — image replay", () => {
	...
	it("replays reopened session image blocks through the cold-start rebuild path", async () => {
		await Settings.init({ inMemory: true, overrides: { "terminal.showImages": true } });
		setTerminalImageProtocol(ImageProtocol.Sixel);
		using tempDir = TempDir.createSync("@yemu-render-initial-image-replay-");
