...
import * as capability from "@yemu/agent-runtime/capability";
import type { Rule } from "@yemu/agent-runtime/capability/rule";
import { resetActiveRulesForTests, setActiveRules } from "@yemu/agent-runtime/capability/rule";
import type { SSHHost } from "@yemu/agent-runtime/capability/ssh";
import type { CapabilityResult } from "@yemu/agent-runtime/capability/types";
import type { Skill } from "@yemu/agent-runtime/extensibility/skills";
import { resetActiveSkillsForTests, setActiveSkills } from "@yemu/agent-runtime/extensibility/skills";
import { InternalUrlRouter } from "@yemu/agent-runtime/internal-urls/router";
import {
	applyInternalUrlCompletion,
	extractInternalUrlContext,
	getInternalUrlSuggestions,
	isInternalUrlPrefix,
} from "@yemu/agent-runtime/modes/internal-url-autocomplete";
import { PromptActionAutocompleteProvider } from "@yemu/agent-runtime/modes/prompt-action-autocomplete";
...
describe("internal-url-autocomplete", () => {
	beforeEach(() => {
		...
		setActiveRules([rule("python", "yemu rules"), rule("style")]);
	});
	...
	describe("getInternalUrlSuggestions", () => {
		...
		it("carries the candidate description through", async () => {
			...
			expect(result!.items[0]).toMatchObject({ value: "rule://python", description: "yemu rules" });
		});
	...
	describe("router.complete dispatch", () => {
		...
		it("exposes the completion-capable schemes", () => {
			...
			expect(schemes).toEqual(["agent", "artifact", "history", "local", "memory", "yemu", "rule", "skill", "ssh"]);
		});
	});
