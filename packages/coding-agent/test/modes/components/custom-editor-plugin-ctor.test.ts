...
import { ProcessTerminal, TUI } from "@yemu/tui";
import { CustomEditor } from "../../../src/modes/components/custom-editor";
import { getEditorTheme, initTheme } from "../../../src/modes/theme/theme";

/**
 * Regression for issue #4766: plugins written against upstream yemu subclass
 * `CustomEditor`/`Editor` and forward `super(tui, theme, keybindings)`. yemu's
 ...
 */
describe("CustomEditor upstream-yemu constructor compatibility (#4766)", () => {
	...
	it("still accepts yemu's own (theme) constructor", async () => {
		await initTheme();
