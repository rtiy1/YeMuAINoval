...
import { PluginManager } from "@yemu/agent-runtime/extensibility/plugins";
import { type InstalledPluginSummary, MarketplaceManager } from "@yemu/agent-runtime/extensibility/plugins/marketplace";
...
} from "@yemu/agent-runtime/modes/components/plugin-settings";
...
describe("PluginListComponent", () => {
	...
	it("empty-state mentions both npm and marketplace install commands", () => {
		...
		expect(text).toContain("yemu plugin install <package>");
		expect(text).toContain("yemu plugin install <name>@<marketplace>");
	});
...
describe("MarketplacePluginDetailComponent", () => {
	...
	it("shortens home-relative install paths to ~ before rendering", () => {
		...
		const installPath = `${home}/.yemu/cache/plugins/sample@mkt`;
		...
		expect(text).toContain("~/.yemu/cache/plugins/sample@mkt");
