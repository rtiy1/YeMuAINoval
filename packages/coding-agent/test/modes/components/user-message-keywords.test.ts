...
import { CustomEditor } from "@yemu/agent-runtime/modes/components/custom-editor";
import { UserMessageComponent } from "@yemu/agent-runtime/modes/components/user-message";
import { getEditorTheme, initTheme } from "@yemu/agent-runtime/modes/theme/theme";
...
describe("UserMessageComponent magic-keyword highlighting", () => {
	...
	it("wraps image references in file hyperlinks when a blob path is available", () => {
		const imagePath = path.resolve("/tmp/yemu-image.png");
