...
import { LoginDialogComponent } from "@yemu/agent-runtime/modes/components/login-dialog";
...
describe("SelectorController login", () => {
	...
	it("routes enhanced paste into a direct API-key prompt", async () => {
		...
		dialog.pasteText("YEMU_PASTE_TEST_123");
		dialog.handleInput("\n");

		await expect(prompt).resolves.toBe("YEMU_PASTE_TEST_123");
	});
});
