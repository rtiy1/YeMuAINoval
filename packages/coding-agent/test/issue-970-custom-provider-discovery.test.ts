...
import type { ModelRegistry, ProviderDiscoveryState } from "@yemu/agent-runtime/config/model-registry";
import { ModelRegistry as ModelRegistryImpl } from "@yemu/agent-runtime/config/model-registry";
import { Settings } from "@yemu/agent-runtime/config/settings";
import { ModelHubComponent } from "@yemu/agent-runtime/modes/components/model-hub";
...
describe("issue #970 custom provider discovery", () => {
	...
	beforeEach(async () => {
		tempDir = path.join(os.tmpdir(), `yemu-test-issue-970-${Snowflake.next()}`);
