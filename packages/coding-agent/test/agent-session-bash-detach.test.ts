 ...
 * verified at the unit level by `yemu-native::shell::tests::child_session_action`
 * (truth-table) and `embedded_external_command_runs_in_its_own_session` (real
 * brush spawn). This test pulls the fix end-to-end through the YeMu coding
 * agent stack:
 *
 *   AgentSession.prompt
 *     → Agent.prompt (real)
 ...
 *         → BashTool.execute
 *           → executeBash
 *             → yemu-native `Shell.run` (real native binding)
 ...
 * SIGTTIN/SIGTTOU the YeMu host process.
 */
...
import { BashTool, type ToolSession } from "@yemu/agent-runtime/tools";
import { getBundledModel } from "@yemu/model-catalog/models";
import { createMockModel, type MockResponse } from "@yemu/model-runtime/providers/mock";
...
describe("BashTool through AgentSession runs children in their own session (e2e)", () => {
	...
	beforeEach(async () => {
		...
		tempDir = path.join(os.tmpdir(), `yemu-bash-detach-test-${Snowflake.next()}`);
