 ...
 * default, unbounded when `YEMU_MCP_TIMEOUT_MS=0`). `dispose()` wraps the
...
import { removeSyncWithRetries, withTimeout } from "@yemu/utils";
...
describe("owned-manager dispose disconnect is bounded (PR #2839)", () => {
	...
	beforeEach(() => {
		workDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-mcp-dispose-"));
	});
