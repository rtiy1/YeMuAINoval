...
import { AsyncJobManager } from "@yemu/agent-runtime/async/job-manager";
...
import { SessionManager, SessionPersistenceIndeterminateError } from "@yemu/agent-runtime/session/session-manager";
import {
	FileSessionStorage,
	type SessionStorage,
	type SessionStorageWriter,
	type WriteTextAtomicOptions,
} from "@yemu/agent-runtime/session/session-storage";
import * as executorModule from "@yemu/agent-runtime/task/executor";
import type { AgentProgress, SingleResult } from "@yemu/agent-runtime/task/types";
...
describe("vibe session registry", () => {
	...
	async function createPersistedParent(storage?: SessionStorage): Promise<SessionManager> {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-vibe-resume-"));
		tempRoots.push(root);
