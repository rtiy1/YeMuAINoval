...
import { getExistingSecretPlaceholderKey, getSecretPlaceholderKey, loadSecrets } from "@yemu/agent-runtime/secrets";
import {
...
} from "@yemu/agent-runtime/secrets/obfuscator";
import { compileSecretRegex } from "@yemu/agent-runtime/secrets/regex";
import type { AssistantMessage, Context, Message, TextContent } from "@yemu/model-runtime";
import { getActiveProfile, getAgentDir, setProfile } from "@yemu/utils/dirs";
...
describe("getSecretPlaceholderKey", () => {
	...
	async function withTempAgentHome(run: () => Promise<void>): Promise<void> {
		...
		const tempHomeDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-secret-key-"));
...
describe("SecretObfuscator friendlyName placeholders", () => {
	...
	it("omits invalid friendlyName metadata without dropping the secret", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-secret-friendly-"));
		try {
			const project = path.join(root, "project");
			const agentDir = path.join(root, "agent");
			await fs.mkdir(path.join(project, ".yemu"), { recursive: true });
			await fs.mkdir(agentDir, { recursive: true });
			await fs.writeFile(
				path.join(project, ".yemu", "secrets.yml"),
