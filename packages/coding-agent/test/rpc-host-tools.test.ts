...
import type { AgentEvent } from "@yemu/agent-core";
import { defineRpcClientTool, RpcClient } from "@yemu/agent-runtime/modes";
import { RpcHostToolBridge } from "@yemu/agent-runtime/modes/rpc/host-tools";
import type {
	RpcHostToolCallRequest,
	RpcHostToolCancelRequest,
	RpcHostToolUpdate,
} from "@yemu/agent-runtime/modes/rpc/rpc-types";
...
describe("RpcClient custom tools", () => {
	it("registers host custom tools and serves tool calls over the RPC transport", async () => {
		const scriptPath = path.join(os.tmpdir(), `yemu-rpc-host-tools-${Date.now()}.js`);
