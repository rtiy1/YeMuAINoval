...
import { type BenchModelRegistry, runBenchCommand } from "@yemu/agent-runtime/cli/bench-cli";
import type {
	Api,
	ApiKeyResolver,
	AssistantMessage,
...
describe("bench cache mode", () => {
	it("splits the cache breakpoint prefix from each variable suffix with native YeMu messages", async () => {
	...
	it("keeps yemu-native credential affinity while marking gateway-hidden payload diagnostics unavailable", async () => {
		const calls: SimpleStreamOptions[] = [];
		const summary = await runBenchCommand(
			{ models: ["openai/gpt-cache-test"], flags: { cache: true, json: true } },
			{
				createRuntime: async () => ({
					modelRegistry: { ...registry, getAll: () => [yemuNativeModel] },
					close: () => {},
				}),
				randomSessionId: (() => {
					let id = 0;
	...
	it("truncates the default prefix-file reader at a UTF-8 boundary before decoding", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-bench-cache-prefix-"));
	...
	it("preserves significant whitespace and replacement patterns from the default prefix-file reader", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-bench-cache-prefix-whitespace-"));
		const prefixPath = path.join(tempDir, "prefix.txt");
