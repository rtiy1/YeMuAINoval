...
import { detectIndentFromContent, resolveFormatOptions } from "@yemu/agent-runtime/lsp/format-options";
import { getProjectDir, removeWithRetries, Snowflake, setProjectDir } from "@yemu/utils";
...
describe("resolveFormatOptions", () => {
	...
	beforeEach(async () => {
		...
		tempDir = path.join(os.tmpdir(), "agent-runtime-format-options", Snowflake.next());
