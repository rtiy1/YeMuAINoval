...
import type { AgentEvent, AgentMessage } from "@yemu/agent-core";
import {
...
import type { BashExecutionMessage } from "@yemu/agent-runtime/session/messages";
import { type AssistantMessage, Effort, type TextContent } from "@yemu/model-runtime";
...
describe.skipIf(!e2eApiKey("ANTHROPIC_API_KEY"))("RPC mode", () => {
	...
	beforeEach(() => {
		sessionDir = path.join(os.tmpdir(), `yemu-rpc-test-${Snowflake.next()}`);
		client = new RpcClient({
			cliPath: path.join(import.meta.dir, "..", "dist", "cli.js"),
			cwd: path.join(import.meta.dir, ".."),
			env: { YEMU_CODING_AGENT_DIR: sessionDir },
			provider: "anthropic",
...
describe("RPC fast mode with unsupported Fireworks model and priority tier", () => {
	...
	beforeEach(async () => {
		sessionDir = path.join(os.tmpdir(), `yemu-rpc-fast-mode-test-${Snowflake.next()}`);
		...
		client = new RpcClient({
			...
			cwd: path.join(import.meta.dir, ".."),
			env: {
				YEMU_CODING_AGENT_DIR: sessionDir,
