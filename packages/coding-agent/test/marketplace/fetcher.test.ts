...
describe("fetchMarketplace", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-fetcher-test-"));
	});
	...
	it("loads catalog from .yemu-plugin/marketplace.json when present", async () => {
		const root = path.join(tmpDir, "yemu-only");
		fs.mkdirSync(path.join(root, ".yemu-plugin"), { recursive: true });
		const catalog = {
			name: "yemu-only-marketplace",
			owner: { name: "Test" },
			plugins: [{ name: "yemu-plugin", source: "./plugins/yemu-plugin", description: "x" }],
		};
		fs.writeFileSync(path.join(root, ".yemu-plugin", "marketplace.json"), JSON.stringify(catalog));

		const result = await fetchMarketplace(root, tmpDir);
		expect(result.catalog.name).toBe("yemu-only-marketplace");
		expect(result.catalog.plugins[0].name).toBe("yemu-plugin");
	});

	it("prefers .yemu-plugin/marketplace.json over .claude-plugin/marketplace.json when both exist", async () => {
		const root = path.join(tmpDir, "both-catalogs");
		fs.mkdirSync(path.join(root, ".yemu-plugin"), { recursive: true });
		fs.mkdirSync(path.join(root, ".claude-plugin"), { recursive: true });
		const yemuCatalog = {
			name: "from-yemu-plugin",
			owner: { name: "Test" },
		...
		fs.writeFileSync(path.join(root, ".yemu-plugin", "marketplace.json"), JSON.stringify(yemuCatalog));
		fs.writeFileSync(path.join(root, ".claude-plugin", "marketplace.json"), JSON.stringify(claudeCatalog));

		const result = await fetchMarketplace(root, tmpDir);
		expect(result.catalog.name).toBe("from-yemu-plugin");
	});

	it("falls back to .claude-plugin/marketplace.json when .yemu-plugin is absent", async () => {
	...
	it("error message names both candidate paths when neither exists", async () => {
		...
		await expect(fetchMarketplace(empty, tmpDir)).rejects.toThrow(
			/\.yemu-plugin[\\/]marketplace\.json.*\.claude-plugin[\\/]marketplace\.json/,
		);
	});
