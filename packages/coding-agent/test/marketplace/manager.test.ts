...
import { listYemuExtensionRoots } from "@yemu/agent-runtime/discovery/yemu-extension-roots";
import { getEnabledPlugins } from "@yemu/agent-runtime/extensibility/plugins/loader";
...
function buildMinimalFixture(): string {
	const root = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-mgr-fixture-"));
...
function createTestContext(): TestContext {
	const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-mgr-test-"));
...
function mockPluginManagerPaths(root: string) {
	return [
		spyOn(yemuUtils, "getPluginsDir").mockReturnValue(root),
		spyOn(yemuUtils, "getPluginsNodeModules").mockReturnValue(path.join(root, "node_modules")),
		spyOn(yemuUtils, "getPluginsPackageJson").mockReturnValue(path.join(root, "package.json")),
		spyOn(yemuUtils, "getPluginsLockfile").mockReturnValue(path.join(root, "yemu-plugins.lock.json")),
		spyOn(yemuUtils, "getProjectPluginOverridesPath").mockReturnValue(path.join(root, "plugin-overrides.json")),
	];
}
...
describe("MarketplaceManager", () => {
	...
	it("installPlugin → plugin in cache + in registry", async () => {
		...
		const runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "yemu-plugins.lock.json")).json();
	...
	it("installPlugin exposes marketplace package to the runtime loader", async () => {
		const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-mgr-home-"));
		try {
			const pluginsDir = path.join(tmpHome, ".yemu", "plugins");
			const manager = new MarketplaceManager({
				marketplacesRegistryPath: path.join(tmpHome, ".yemu", "marketplaces.json"),
	...
	it("installPlugin keeps marketplace packages out of YeMu extension roots", async () => {
		const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-mgr-home-"));
		try {
			const pluginsDir = path.join(tmpHome, ".yemu", "plugins");
			const manager = new MarketplaceManager({
			...
			const roots = await listYemuExtensionRoots({ cwd: tmpHome, home: tmpHome, repoRoot: null });
	...
	it("installPlugin with scope:project exposes the marketplace package to the runtime loader", async () => {
		const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-mgr-home-"));
		const projectAnchor = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-mgr-project-"));
		try {
			const userPluginsDir = path.join(tmpHome, ".yemu", "plugins");
			const projectPluginsDir = path.join(projectAnchor, ".yemu", "plugins");
			...
			const projectLock = await Bun.file(path.join(projectPluginsDir, "yemu-plugins.lock.json")).json();
			...
			expect(fs.existsSync(path.join(userPluginsDir, "yemu-plugins.lock.json"))).toBe(false);
		} finally {
			fs.rmSync(tmpHome, { recursive: true, force: true });
	...
	it("setPluginEnabled → persisted in registry", async () => {
		...
		let runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "yemu-plugins.lock.json")).json();
		expect(runtimeConfig.plugins["hello-plugin"].enabled).toBe(false);
		...
		runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "yemu-plugins.lock.json")).json();
	...
	it("installPlugin scope:project when no projectInstalledRegistryPath → throws", async () => {
		const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-mgr-noproj-"));
		try {
