/**
 * Discovery integration tests for YeMu plugin registry reading.
 *
 * NOTE: listClaudePluginRoots() lives in discovery/helpers.ts which imports
 * @yemu/native (native Rust addon via glob). We cannot call it here.
 *
 * Instead these tests validate the structural contract that listClaudePluginRoots
 * depends on:
 *   1. YeMu registry lives at path.join(home, ".yemu", "plugins", "installed_plugins.json")
 *      (matches getConfigDirName() == ".yemu")
...
import type { InstalledPluginEntry } from "@yemu/agent-runtime/extensibility/plugins/marketplace";
import {
	addInstalledPlugin,
	buildPluginId,
	readInstalledPluginsRegistry,
	writeInstalledPluginsRegistry,
} from "@yemu/agent-runtime/extensibility/plugins/marketplace";
import { removeSyncWithRetries } from "@yemu/utils";

// ── Inline validator ───────────────────────────────────────────────────────────
//
// Mirrors parseClaudePluginsRegistry() in discovery/helpers.ts exactly.
// Kept here to avoid importing helpers.ts (which pulls in @yemu/native).
...
// Matches getConfigDirName() — single source of truth is in @yemu/utils,
// but we know the value is ".yemu" and hardcoding it here keeps tests free of
// native-addon transitive imports.
const YEMU_CONFIG_DIR = ".yemu";
...
let tmpHome: string;
/** ~/.yemu/plugins/installed_plugins.json inside tmpHome */
let yemuRegistryPath: string;

beforeEach(() => {
	tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-discovery-test-"));
	yemuRegistryPath = path.join(tmpHome, YEMU_CONFIG_DIR, "plugins", "installed_plugins.json");
	fs.mkdirSync(path.dirname(yemuRegistryPath), { recursive: true });
});
...
describe("YeMu registry path contract", () => {
	it("YeMu registry lives at home/.yemu/plugins/installed_plugins.json", () => {
		// This is the path that listClaudePluginRoots reads.
		// Any change to this path must be reflected in helpers.ts.
		const expected = path.join(tmpHome, ".yemu", "plugins", "installed_plugins.json");
		expect(yemuRegistryPath).toBe(expected);
	});
});

// ── Format compatibility ───────────────────────────────────────────────────────

describe("YeMu registry format compatibility with Claude parser", () => {
	it("empty registry written by writeInstalledPluginsRegistry passes validator", async () => {
		await writeInstalledPluginsRegistry(yemuRegistryPath, { version: 2, plugins: {} });

		const content = fs.readFileSync(yemuRegistryPath, "utf8");
	...
	it("registry with installed plugin passes validator", async () => {
		...
		let reg = await readInstalledPluginsRegistry(yemuRegistryPath);
		reg = addInstalledPlugin(reg, pluginId, entry);
		await writeInstalledPluginsRegistry(yemuRegistryPath, reg);
...
describe("YeMu registry round-trip", () => {
	it("reads back what was written — single plugin", async () => {
		...
		const readBack = await readInstalledPluginsRegistry(yemuRegistryPath);
...
//
// listClaudePluginRoots must replace Claude entries with YeMu entries when the same
...
describe("YeMu precedence contract (registry structure)", () => {
	it("same plugin ID in both registries — YeMu entry has required fields for deduplication", () => {
		// The replacement logic: roots.filter(r => r.id !== pluginId) keyed by id.
		// YeMu entries must have installPath so they can be added to roots[].
		const id = buildPluginId("shared-plugin", "common-mkt");
		const yemuEntry = makeEntry("/yemu/cached/path");

		// YeMu registry entry has installPath (required by listClaudePluginRoots)
		expect(yemuEntry.installPath).toBeTruthy();
		expect(typeof yemuEntry.installPath).toBe("string");
