 ...
 *
 * Note: helpers.ts imports @yemu/native (Rust addon via glob).
...
describe("resolveActiveProjectRegistryPath", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-proj-scope-"));
	});
	...
	it("walk-up finds nearest .yemu/ directory", async () => {
		// Layout: tmpDir/.yemu/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must climb from cwd → sub → tmpDir and find .yemu/ there.
		fs.mkdirSync(path.join(tmpDir, ".yemu"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, ".yemu", "plugins", "installed_plugins.json"));
	});

	it("walk-up stops at the nearest .yemu/ — does not skip to a more distant one", async () => {
		// Layout: tmpDir/.yemu/   +   tmpDir/sub/.yemu/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must stop at tmpDir/sub/.yemu/, not climb further to tmpDir/.yemu/.
		fs.mkdirSync(path.join(tmpDir, ".yemu"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".yemu"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".yemu", "plugins", "installed_plugins.json"));
	});

	it("falls back to .git root when no .yemu/ exists", async () => {
		// Layout: tmpDir/.git/   +   tmpDir/sub/  (cwd)
		// No .yemu/ anywhere → second pass finds .git/ at tmpDir.
	...
	it("returns null when neither .yemu/ nor .git/ found anywhere in the tree", async () => {
		// Start at the filesystem root — guaranteed to have no .yemu/ or .git/ ancestors.
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		// resolveActiveProjectRegistryPath must NOT return ~/.yemu/.../installed_plugins.json.
		const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-proj-scope-home-"));
		...
		try {
			const result = await resolveActiveProjectRegistryPath(cwd);
			const homeYemuPath = path.join(homeDir, ".yemu", "plugins", "installed_plugins.json");
			expect(result).not.toBe(homeYemuPath);
			expect(result).toBeNull();
		} finally {
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	/** Path where listClaudePluginRoots reads the user YeMu registry. */
	...
	beforeEach(() => {
		tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-shadow-home-"));
		tmpProject = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-shadow-proj-"));

		// Create .yemu/ in project so resolveActiveProjectRegistryPath finds it.
		fs.mkdirSync(path.join(tmpProject, ".yemu", "plugins"), { recursive: true });

		userRegPath = path.join(tmpHome, ".yemu", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".yemu", "plugins", "installed_plugins.json");
	});
