...
import { substitutePluginRoot } from "@yemu/agent-runtime/discovery/substitute-plugin-root";
...
const YEMU_VAR = "$" + "{YEMU_PLUGIN_ROOT}";
...
describe("substitutePluginRoot", () => {
	...
	it("replaces YEMU_PLUGIN_ROOT in strings", () => {
		expect(substitutePluginRoot(`${YEMU_VAR}/bin/server`, ROOT)).toBe("/plugins/my-plugin/bin/server");
	});

	it("replaces both variables in same string", () => {
		expect(substitutePluginRoot(`${CLAUDE_VAR}:${YEMU_VAR}`, ROOT)).toBe("/plugins/my-plugin:/plugins/my-plugin");
	});
