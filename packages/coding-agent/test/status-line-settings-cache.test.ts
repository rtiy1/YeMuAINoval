...
import { StatusLineComponent, type StatusLineSettings } from "@yemu/agent-runtime/modes/components/status-line";
import { STATUS_LINE_PRESETS } from "@yemu/agent-runtime/modes/components/status-line/presets";
import { initTheme } from "@yemu/agent-runtime/modes/theme/theme";
import * as git from "@yemu/agent-runtime/utils/git";
import { removeSyncWithRetries, setProjectDir } from "@yemu/utils";
...
beforeEach(async () => {
	...
	projectDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-status-line-settings-cache-"));
