...
import { generateThemeVars } from "@yemu/agent-runtime/export/html";
import { defaultThemes } from "@yemu/agent-runtime/modes/theme/defaults";
import { getResolvedThemeColors, getThemeByName, isLightTheme } from "@yemu/agent-runtime/modes/theme/theme";
import { getAgentDir, getCustomThemesDir, removeWithRetries, setAgentDir } from "@yemu/utils";
...
describe("getResolvedThemeColors HTML export defaults", () => {
	...
	it("uses light text when a light-status custom theme derives dark export surfaces from userMessageBg", async () => {
		...
		tempAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-theme-export-"));
