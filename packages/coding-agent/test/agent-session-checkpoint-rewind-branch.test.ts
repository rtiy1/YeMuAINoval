...
import { Agent, type AgentMessage, type AgentTool } from "@yemu/agent-core";
...
import { RewindTool, type ToolSession } from "@yemu/agent-runtime/tools";
import { EventBus } from "@yemu/agent-runtime/utils/event-bus";
import type { AssistantMessage, Message, ThinkingContent } from "@yemu/model-runtime";
...
import {
	...
	type MockModel,
	type MockResponse,
} from "@yemu/model-runtime/providers/mock";
...
async function createHarness(
	...
	const tempDir = TempDir.createSync("@yemu-checkpoint-rewind-branch-");
	...
	if (options?.onAgentEnd) {
		...
		const extension = await loadExtensionFromFactory(
			yemu => {
				yemu.on("agent_end", event => options.onAgentEnd?.(event.willContinue));
			},
			tempDir.path(),
			new EventBus(),
			runtime,
