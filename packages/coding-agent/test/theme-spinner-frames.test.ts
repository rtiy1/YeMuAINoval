...
import { SPINNER_GLYPH_ADVANCE_MS, sharedSpinnerFrame } from "@yemu/agent-runtime/modes/components/tool-execution";
import { getThemeByName } from "@yemu/agent-runtime/modes/theme/theme";
import { getConfigRootDir, getCustomThemesDir, removeWithRetries, setAgentDir } from "@yemu/utils";
...
describe("theme symbols.spinnerFrames", () => {
	beforeEach(async () => {
		tmpAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-spinner-frames-"));
