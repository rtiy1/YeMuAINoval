...
import { resolvePluginExtensionPaths, resolvePluginToolPaths } from "@yemu/agent-runtime/extensibility/plugins/loader";
import type { InstalledPlugin, PluginManifest } from "@yemu/agent-runtime/extensibility/plugins/types";
...
describe("plugin manifest path resolution", () => {
	it("resolves a directory tools entry to its index, not the yemu.extensions modules", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-manifest-paths-"));
		try {
			// The package declares both extensions and a directory-based tool entry.
			// `yemu.extensions` and the sub-extension scan are extensions-specific and
