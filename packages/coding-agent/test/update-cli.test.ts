...
import * as pluginCli from "@yemu/agent-runtime/cli/plugin-cli";
import * as updateCli from "@yemu/agent-runtime/cli/update-cli";
import {
	buildBunInstallArgs,
	...
	sweepStaleBackups,
	updateViaBinaryAt,
} from "@yemu/agent-runtime/cli/update-cli";
import Update from "@yemu/agent-runtime/commands/update";
import { removeWithRetries } from "@yemu/utils";
import type { CliConfig } from "@yemu/utils/cli";

const tempDirs: string[] = [];

async function makeTempDir(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-update-test-"));
	tempDirs.push(dir);
	return dir;
}
...
describe("update-cli install target detection", () => {
	it("uses bun update when prioritized yemu is inside bun global bin", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.bun/bin/yemu", "/Users/test/.bun/bin");

		expect(method).toBe("bun");
	});

	it("uses npm update when prioritized yemu is inside an npm global bin", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.npm-global/bin/yemu", undefined, {
	...
	it("uses binary update when a plain file in the npm global bin dir is the standalone binary, not an npm symlink", () => {
		...
		const method = resolveUpdateMethodForTest("/home/u/.local/bin/yemu", undefined, {
			npmBinDir: "/home/u/.local/bin",
			yemuIsRegularFile: true,
		});
	...
	it("uses binary update when a plain file in the bun global bin dir is the standalone binary", () => {
		const method = resolveUpdateMethodForTest("/home/u/.local/bin/yemu", "/home/u/.local/bin", {
			yemuIsRegularFile: true,
		});
	...
	it("keeps bun update for regular-file entries in the bun global bin dir on Windows, where bun writes .exe shims", () => {
		...
		try {
			const method = resolveUpdateMethodForTest("C:/Users/test/.bun/bin/yemu.exe", "C:/Users/test/.bun/bin", {
				yemuIsRegularFile: true,
			});
	...
	it("still uses npm update when the npm global bin entry is a package-manager symlink, not a plain file", () => {
		const method = resolveUpdateMethodForTest("/home/u/.local/bin/yemu", undefined, {
			npmBinDir: "/home/u/.local/bin",
			yemuIsRegularFile: false,
		});

		expect(method).toBe("npm");
	});

	it("uses binary update when prioritized yemu is outside bun global bin", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.local/bin/yemu", "/Users/test/.bun/bin");

		expect(method).toBe("binary");
	});

	it("uses binary update when bun global bin cannot be resolved", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.local/bin/yemu", undefined);

		expect(method).toBe("binary");
	});

	it("uses Homebrew update when prioritized yemu resolves into the Homebrew formula", async () => {
		const dir = await makeTempDir();
		const prefix = path.join(dir, "opt", "yemu");
		const linkedBin = path.join(dir, "bin");
		await fs.mkdir(path.join(prefix, "bin"), { recursive: true });
		await fs.mkdir(linkedBin, { recursive: true });
		await Bun.write(path.join(prefix, "bin", "yemu"), "binary");
		await fs.symlink(path.join(prefix, "bin", "yemu"), path.join(linkedBin, "yemu"));

		const method = resolveUpdateMethodForTest(path.join(linkedBin, "yemu"), "/Users/test/.bun/bin", {
	...
	it("uses mise update when prioritized yemu is in an active mise bin path", () => {
		const method = resolveUpdateMethodForTest(
			"/Users/test/.local/share/mise/installs/github-rtiy1-YeMuAINoval/latest/bin/yemu",
			undefined,
			{
				miseBinDirs: ["/Users/test/.local/share/mise/installs/github-rtiy1-YeMuAINoval/latest/bin"],
			},
		);
	...
	it("uses mise update when prioritized yemu is a mise shim", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.local/share/mise/shims/yemu", undefined, {
...
describe("update-cli package manager commands", () => {
	it("targets the YeMu Homebrew formula and switches to reinstall for forced updates", () => {
		expect(buildHomebrewUpdateArgs(false)).toEqual(["upgrade", "yemu"]);
		expect(buildHomebrewUpdateArgs(true)).toEqual(["reinstall", "yemu"]);
	});

	it("targets the mise GitHub backend tool and force-reinstalls the checked version when requested", () => {
		expect(buildMiseUpgradeArgs()).toEqual(["upgrade", "github:rtiy1/YeMuAINoval", "--bump"]);
		expect(buildMiseForceInstallArgs("15.10.5")).toEqual(["install", "--force", "github:rtiy1/YeMuAINoval@15.10.5"]);
	});
	...
	it("pins npm package installs to the official registry and the checked native package versions", () => {
		...
		expect(args).toContain("@yemu/agent-runtime@16.3.15");
		expect(args).toContain("@yemu/native@16.3.15");
		expect(args).toContain("@yemu/native-win32-x64@16.3.15");
	});
});
...
describe("update-cli bun install command", () => {
	it("pins the official npm registry and bypasses the manifest cache so a stale mirror or snapshot cannot mask a freshly published version", () => {
		// Regression: yemu queries https://registry.npmjs.org/<pkg>/latest directly.
	...
	it("pins the native addon core and the platform-specific leaf to the same version so the loader sentinel cannot drift on supported tags", () => {
		...
		// package, leaving @yemu/native and @yemu/native-<tag>
	...
	it("omits the leaf on unsupported platform tags so an EBADPLATFORM swap does not mask the underlying `no matching version` error", () => {
		...
		expect(args).toContain("@yemu/native@15.9.0");
		expect(args.some(arg => arg.startsWith("@yemu/native-"))).toBe(false);
	});
...
describe("update-cli bun cache pruning", () => {
	it("keeps only the newest cached version for filtered global install packages", async () => {
		...
		await Bun.write(path.join(dir, "@yemu", "yemu-utils", "15.7.6@@@1"), "");
		await Bun.write(path.join(dir, "@yemu", "yemu-utils", "15.8.0@@@1"), "");
		await Bun.write(
			path.join(dir, "@yemu", "yemu-utils@15.7.6@@@1", "package.json"),
			JSON.stringify({ name: "@yemu/utils", version: "15.7.6" }),
		);
		await Bun.write(
			path.join(dir, "@yemu", "yemu-utils@15.8.0@@@1", "package.json"),
			JSON.stringify({ name: "@yemu/utils", version: "15.8.0" }),
		);
		...
		const result = await pruneBunInstallCache(dir, new Set(["react", "@yemu/utils"]));
		...
		expect(await Bun.file(path.join(dir, "@yemu", "yemu-utils", "15.7.6@@@1")).exists()).toBe(false);
		expect(await Bun.file(path.join(dir, "@yemu", "yemu-utils@15.7.6@@@1", "package.json")).exists()).toBe(false);
		expect(await Bun.file(path.join(dir, "@yemu", "yemu-utils", "15.8.0@@@1")).exists()).toBe(true);
		expect(await Bun.file(path.join(dir, "@yemu", "yemu-utils@15.8.0@@@1", "package.json")).exists()).toBe(true);
...
describe("update-cli release binary integrity", () => {
	const tag = "v17.1.2";
	const binaryName = "yemu-linux-x64";
	const url = `https://github.com/rtiy1/YeMuAINoval/releases/download/${tag}/${binaryName}`;
	...
	it("rejects release metadata that does not identify one exact stable asset", () => {
		...
		expect(() =>
			resolveReleaseBinaryAsset(
				releaseAsset({ browser_download_url: "https://example.com/yemu-linux-x64" }),
				tag,
				binaryName,
			),
...
describe("update-cli binary replacement", () => {
	it("restores the previous binary when the replacement fails verification", async () => {
		const dir = await makeTempDir();
		const targetPath = path.join(dir, "yemu");
		...
		).rejects.toThrow("restored previous yemu binary");
...
describe("update-cli binary replacement on locked backups", () => {
	it("treats an EPERM on backup cleanup as a successful, completed update", async () => {
		...
		const dir = await makeTempDir();
		const targetPath = path.join(dir, "yemu.exe");
		const tempPath = `${targetPath}.new`;
