...
import { applyLlamaCppQwenThinking } from "@yemu/agent-runtime/config/model-discovery";
import { kNoAuth, ModelRegistry } from "@yemu/agent-runtime/config/model-registry";
...
import { writeModelCache } from "@yemu/model-catalog/model-cache";
import { getBundledModel } from "@yemu/model-catalog/models";
import type { ModelSpec, OpenAICompat } from "@yemu/model-catalog/types";
import { Effort, type FetchImpl, type Model } from "@yemu/model-runtime";
import type { OAuthCredentials } from "@yemu/model-runtime/oauth/types";
...
describe("ModelRegistry runtime discovery", () => {
	...
	beforeEach(async () => {
		...
		tempDir = path.join(os.tmpdir(), `yemu-test-model-registry-${Snowflake.next()}`);
	...
	test("keeps OLLAMA_BASE_URL precedence over OLLAMA_HOST", async () => {
		using _baseUrl = withEnv("OLLAMA_BASE_URL", "http://yemu-ollama.example:2222");
		using _host = withEnv("OLLAMA_HOST", "ollama-host.example:3333");
		const fetchMock = mockOllamaDiscovery(["phi4-mini"], "http://yemu-ollama.example:2222");
		const registry = new ModelRegistry(authStorage, modelsJsonPath, { fetch: fetchMock });
		await registry.refresh();

		const model = registry.find("ollama", "phi4-mini");
		expect(model?.baseUrl).toBe("http://yemu-ollama.example:2222/v1");
	});
	...
	test("applyLlamaCppQwenThinking keeps a yemu-native gateway base URL without doubling /v1", () => {
		...
		// streamYemuNative appends `/v1/yemu/stream`, so the gateway URL must stay bare
		// rather than gaining a `/v1` that would double to `.../v1/v1/yemu/stream`.
		expect(upgraded.baseUrl).toBe("http://gw:4000");
		expect(upgraded.transport).toBe("yemu-native");
