...
import { type Args, parseArgs } from "@yemu/agent-runtime/cli/args";
import * as modelResolverModule from "@yemu/agent-runtime/config/model-resolver";
import { Settings } from "@yemu/agent-runtime/config/settings";
import * as pluginHelpers from "@yemu/agent-runtime/discovery/helpers";
import { createSessionManager, runRootCommand } from "@yemu/agent-runtime/main";
import { AuthStorage } from "@yemu/agent-runtime/session/auth-storage";
import type { SessionHeader } from "@yemu/agent-runtime/session/session-entries";
import type { SessionInfo } from "@yemu/agent-runtime/session/session-listing";
import * as sessionListingModule from "@yemu/agent-runtime/session/session-listing";
import { loadEntriesFromFile } from "@yemu/agent-runtime/session/session-loader";
import { SessionManager } from "@yemu/agent-runtime/session/session-manager";
import { getProjectDir, normalizePathForComparison, setProjectDir } from "@yemu/utils";
...
describe("createSessionManager — cross-project --resume", () => {
	...
	beforeEach(async () => {
		existingProject = await fsp.mkdtemp(path.join(os.tmpdir(), "yemu-xproj-"));
...
describe("runRootCommand — cross-project --resume", () => {
	...
	beforeEach(async () => {
		...
		root = await fsp.mkdtemp(path.join(os.tmpdir(), "yemu-xproj-root-"));
...
describe("createSessionManager — cross-project --resume relocation (moved worktree)", () => {
	...
	beforeEach(async () => {
		missingRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "yemu-moved-xproj-"));
