...
import type { BuildSessionContextOptions, SessionContext } from "@yemu/agent-runtime/session/session-context";
...
describe("AgentSession.switchSession previous-context build", () => {
	...
	beforeAll(async () => {
		sharedDir = TempDir.createSync("@yemu-switch-prev-ctx-shared-");
	...
	it("skips building the previous display context when switching to a different session", async () => {
		const tempDir = TempDir.createSync("@yemu-switch-prev-ctx-different-");
	...
	it("builds the previous display context for same-session reloads", async () => {
		const tempDir = TempDir.createSync("@yemu-switch-prev-ctx-reload-");
		tempDirs.push(tempDir);
