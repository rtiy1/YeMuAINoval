...
describe("SessionManager cwd adoption on resume", () => {
	it("adopts the resumed session's own cwd and session directory", async () => {
		const projectA = makeTempDir("@yemu-cwd-a-");
		const projectB = makeTempDir("@yemu-cwd-b-");
	...
	it("keeps the current cwd when the resumed session's project directory is gone", async () => {
		const launch = makeTempDir("@yemu-cwd-launch-");
		const store = makeTempDir("@yemu-cwd-store-");
		const goneProject = makeTempDir("@yemu-cwd-gone-");
		// The session file survives in `store` (like ~/.yemu), but its header cwd
