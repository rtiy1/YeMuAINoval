...
import type { AssistantMessage, Context, Model, SimpleStreamOptions, StopReason } from "@yemu/model-runtime";
...
describe("session_stop block continuation — idle recap resilience (#4323)", () => {
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `yemu-4323-recap-${Snowflake.next()}`);
