...
import { runAuthBrokerCommand } from "@yemu/agent-runtime/cli/auth-broker-cli";
import { AuthStorage, SqliteAuthCredentialStore } from "@yemu/model-runtime";
import { type AuthBrokerServerHandle, startAuthBroker } from "@yemu/model-runtime/auth-broker";
import { getAgentDbPath, removeWithRetries, setAgentDir } from "@yemu/utils";
...
describe("auth-broker import (CLIProxyAPI)", () => {
	...
	beforeEach(async () => {
		originalAgentDir = process.env.YEMU_AGENT_DIR;
		savedEnv.YEMU_AUTH_BROKER_URL = process.env.YEMU_AUTH_BROKER_URL;
		savedEnv.YEMU_AUTH_BROKER_TOKEN = process.env.YEMU_AUTH_BROKER_TOKEN;
		delete process.env.YEMU_AUTH_BROKER_URL;
		delete process.env.YEMU_AUTH_BROKER_TOKEN;
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-import-agent-"));
		cliproxyDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-import-cliproxy-"));
		setAgentDir(agentDir);
	});

	afterEach(async () => {
		process.stdout.write = ORIGINAL_STDOUT_WRITE;
		if (originalAgentDir === undefined) delete process.env.YEMU_AGENT_DIR;
		else process.env.YEMU_AGENT_DIR = originalAgentDir;
		await removeWithRetries(agentDir);
		await removeWithRetries(cliproxyDir);
		for (const key of ["YEMU_AUTH_BROKER_URL", "YEMU_AUTH_BROKER_TOKEN"] as const) {
	...
	test("imports a directory of CLIProxyAPI JSONs and maps types to yemu providers", async () => {
...
describe("auth-broker import (broker-routed)", () => {
	...
	beforeEach(async () => {
		savedEnv.YEMU_AUTH_BROKER_URL = process.env.YEMU_AUTH_BROKER_URL;
		savedEnv.YEMU_AUTH_BROKER_TOKEN = process.env.YEMU_AUTH_BROKER_TOKEN;
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-import-client-"));
		brokerAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-import-broker-"));
		cliproxyDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-import-cliproxy-broker-"));
		setAgentDir(agentDir);
		...
		process.env.YEMU_AUTH_BROKER_URL = handle.url;
		process.env.YEMU_AUTH_BROKER_TOKEN = token;
	});
