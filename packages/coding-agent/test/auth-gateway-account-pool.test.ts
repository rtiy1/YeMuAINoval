...
import { runAuthGatewayCommand } from "@yemu/agent-runtime/cli/auth-gateway-cli";
...
const ENV_KEYS = ["YEMU_AUTH_BROKER_URL", "YEMU_AUTH_BROKER_TOKEN", "YEMU_AUTH_BROKER_ACCOUNT_POOL_FILE"] as const;
...
describe("auth-gateway account pool", () => {
	...
	beforeEach(async () => {
		...
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-auth-gateway-pool-"));
		...
		process.env.YEMU_AUTH_BROKER_TOKEN = BROKER_TOKEN;
		process.env.YEMU_AUTH_BROKER_ACCOUNT_POOL_FILE = poolPath;
	});
