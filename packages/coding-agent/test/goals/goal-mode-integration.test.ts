...
import { GoalTool } from "@yemu/agent-runtime/goals/tools/goal-tool";
...
import { normalizeCustomMessagePayload } from "@yemu/agent-runtime/session/messages";
...
import type { TodoPhase } from "@yemu/agent-runtime/tools/todo";
...
async function createSharedFixture(): Promise<SharedFixture> {
	const baseDir = TempDir.createSync("@yemu-goal-mode-shared-");
...
async function createGoalHarness(shared: SharedFixture): Promise<GoalHarness> {
	...
	const tempDir = TempDir.createSync("@yemu-goal-mode-");
