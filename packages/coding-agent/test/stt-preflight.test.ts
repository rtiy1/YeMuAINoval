...
import { Settings, settings } from "@yemu/agent-runtime/config/settings";
import * as asrClient from "@yemu/agent-runtime/stt/asr-client";
import * as downloader from "@yemu/agent-runtime/stt/downloader";
import { STTController } from "@yemu/agent-runtime/stt/stt-controller";
import { getTinyModelsCacheDir, removeWithRetries, setAgentDir } from "@yemu/utils";
...
describe("isSttModelCached completeness", () => {
	...
	beforeEach(async () => {
		...
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-stt-cache-"));
		setAgentDir(tmp);
