...
import { AgentDashboard } from "@yemu/agent-runtime/modes/components/agent-dashboard";
import { initTheme } from "@yemu/agent-runtime/modes/theme/theme";
import * as discovery from "@yemu/agent-runtime/task/discovery";
...
async function makeTempCwd(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-agent-dashboard-"));
	tempDirs.push(dir);
	return dir;
}
