...
import { BlobStore } from "@yemu/agent-runtime/session/blob-store";
import type { SessionMessageEntry } from "@yemu/agent-runtime/session/session-entries";
import { prepareEntryForPersistence } from "@yemu/agent-runtime/session/session-persistence";
import type { AssistantMessage, ProviderPayload, Usage } from "@yemu/model-runtime";
...
describe("session reasoning-signature dedup", () => {
	it("keeps a thinkingSignature the payload does not cover", () => {
		using tempDir = TempDir.createSync("@yemu-session-reasoning-keep-");
	...
	it("leaves thinkingSignatures untouched when there is no provider payload", () => {
		using tempDir = TempDir.createSync("@yemu-session-reasoning-nopayload-");
		const blobStore = new BlobStore(tempDir.path());
...
describe("session atomic reasoning persistence", () => {
	...
	it("preserves an oversized signed thinking block and its signature verbatim", () => {
		using tempDir = TempDir.createSync("@yemu-session-atomic-thinking-");
		const blobStore = new BlobStore(tempDir.path());
	...
	it("preserves an oversized redactedThinking blob verbatim", () => {
		using tempDir = TempDir.createSync("@yemu-session-atomic-redacted-");
		const blobStore = new BlobStore(tempDir.path());
	...
	it("still truncates oversized UNSIGNED thinking and text blocks", () => {
		using tempDir = TempDir.createSync("@yemu-session-atomic-unsigned-");
		const blobStore = new BlobStore(tempDir.path());
	...
	it("survives a full JSONL string round-trip for signed thinking", () => {
		using tempDir = TempDir.createSync("@yemu-session-atomic-roundtrip-");
		const blobStore = new BlobStore(tempDir.path());
