...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-html-template-"));
...
const bundledDependencyStubs: Record<string, string> = {
	"@yemu/utils": 'export const APP_NAME = "yemu"; export const isEnoent = () => false;',
	"../../modes/theme/theme":
...
describe("HTML export template", () => {
	...
	test("composes the exact source template once and returns the cached bytes", async () => {
		...
		expect(first).toContain("const THEME_STORAGE_KEY = 'yemu-export-theme';");
