...
import { type CreateAgentSessionResult, createAgentSession } from "@yemu/agent-runtime/sdk";
...
import { getRestorableSessionModels } from "@yemu/agent-runtime/session/session-context";
import { EPHEMERAL_MODEL_CHANGE_ROLE } from "@yemu/agent-runtime/session/session-entries";
...
import { type Api, type AssistantMessage, Effort, type Model } from "@yemu/model-runtime";
...
describe("AgentSession model persistence", () => {
	...
	beforeAll(async () => {
		sharedDir = TempDir.createSync("@yemu-model-persistence-shared-");
	...
	beforeEach(() => {
		tempDir = TempDir.createSync("@yemu-model-persistence-");
	});
	...
	it("marks an incomplete process-exit transcript aborted during SDK resume without dropping history", async () => {
		...
		expect(messages.at(-1)).toMatchObject({
			...
			content: [],
			stopReason: "aborted",
			errorMessage: "Previous YeMu process exited before completing the turn.",
		});
		expect(
			messages.some(
				message =>
