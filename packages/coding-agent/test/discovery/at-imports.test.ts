...
import { expandAtImports, MAX_AT_IMPORT_DEPTH } from "@yemu/agent-runtime/discovery/at-imports";
...
describe("expandAtImports", () => {
	...
	beforeEach(async () => {
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-at-import-"));
	});
	...
	test("resolves ~/path against the home override", async () => {
		const fakeHome = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-at-home-"));
		try {
