 ...
 *
 * The native `yemu` discovery provider only walks `.yemu/` and `~/.yemu/agent/`.
 * Extension packages registered via `extensions:` in settings or
 * `--extension` on the CLI ship their own `skills/`, `hooks/`, `tools/`,
 * `commands/`, `rules/`, `prompts/`, and `.mcp.json`. The `yemu-plugins`
 * provider (`src/discovery/yemu-plugins.ts`) is what wires those sub-trees
...
import { getCapability } from "@yemu/agent-runtime/capability";
import { clearCache } from "@yemu/agent-runtime/capability/fs";
import { hookCapability } from "@yemu/agent-runtime/capability/hook";
import { mcpCapability } from "@yemu/agent-runtime/capability/mcp";
import { promptCapability } from "@yemu/agent-runtime/capability/prompt";
import { ruleCapability } from "@yemu/agent-runtime/capability/rule";
import { skillCapability } from "@yemu/agent-runtime/capability/skill";
import { slashCommandCapability } from "@yemu/agent-runtime/capability/slash-command";
import { toolCapability } from "@yemu/agent-runtime/capability/tool";
import type { LoadContext, Provider } from "@yemu/agent-runtime/capability/types";
// Register all discovery providers as a side effect.
import "@yemu/agent-runtime/discovery";
import {
	clearYemuExtensionCliRoots,
	injectYemuExtensionCliRoots,
} from "@yemu/agent-runtime/discovery/yemu-extension-roots";
import { getConfigRootDir, removeSyncWithRetries, setAgentDir } from "@yemu/utils";

const PROVIDER_ID = "yemu-plugins";
...
const originalAgentDirEnv = process.env.YEMU_CODING_AGENT_DIR;
...
function buildExtensionPackage(packageDir: string): void {
	writeFile(
		...
		JSON.stringify({ name: path.basename(packageDir), yemu: { extensions: ["./src/main.ts"] } }),
	);
	writeFile(path.join(packageDir, "src", "main.ts"), "export default function (_yemu) {}\n");
	writeFile(
...
beforeEach(() => {
	clearCache();
	clearYemuExtensionCliRoots();
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-plugins-"));
	...
	setAgentDir(path.join(home, ".yemu", "agent"));
});
...
test("project settings.json#extensions surfaces every sub-directory", async () => {
	writeFile(path.join(project, ".yemu", "settings.json"), JSON.stringify({ extensions: [ext] }));
...
test("user settings.json#extensions also feeds sub-discovery", async () => {
	writeFile(path.join(home, ".yemu", "agent", "settings.json"), JSON.stringify({ extensions: [ext] }));
...
test("`--extension` CLI injection is wired through the same provider", async () => {
	...
	injectYemuExtensionCliRoots([ext], home, project);
...
test("file-extension entrypoints contribute zero sub-surface (the file has no siblings to scan)", async () => {
	...
	fs.writeFileSync(standaloneFile, "export default function (_yemu) {}\n");
	writeFile(path.join(project, ".yemu", "settings.json"), JSON.stringify({ extensions: [standaloneFile] }));
...
test("relative paths in settings resolve against the project cwd", async () => {
	...
	writeFile(path.join(project, ".yemu", "settings.json"), JSON.stringify({ extensions: [`./${relative}`] }));
...
test("installed plugins under `<plugins>/node_modules/` are surfaced (e.g. via `yemu plugin link`/`install`)", async () => {
	...
	const pluginsDir = path.join(home, ".yemu", "plugins");
	...
	writeFile(
		...
		JSON.stringify({ name: "yemu-plugins", dependencies: { "my-installed-ext": "1.0.0" } }),
	);
	// Plugin's own package.json must carry an `yemu`/`yemu` manifest for the
	// loader to recognise it; the buildExtensionPackage fixture already wrote
	// one with `yemu.extensions`, which is sufficient.
...
test("project-scoped installed plugins surface project-level sub-discovery", async () => {
	const pluginsDir = path.join(project, ".yemu", "plugins");
	const installed = path.join(pluginsDir, "node_modules", "my-project-ext");
	fs.mkdirSync(installed, { recursive: true });
	fs.cpSync(ext, installed, { recursive: true });
	writeFile(
		path.join(pluginsDir, "yemu-plugins.lock.json"),
		JSON.stringify({
...
test("disabled installed plugins do not contribute sub-discovery", async () => {
	...
	writeFile(
		path.join(pluginsDir, "package.json"),
		JSON.stringify({ name: "yemu-plugins", dependencies: { "my-disabled-ext": "1.0.0" } }),
	);
	writeFile(
		path.join(pluginsDir, "yemu-plugins.lock.json"),
...
test("linked plugins (only in lockfile, not in package.json#dependencies) are surfaced", async () => {
	// `yemu plugin link ./local-ext` creates a symlink under
	...
	// still find the package — otherwise the documented `yemu install
