 ...
 * in any stranger script (Codex hook scripts, scratch files, …) killed YeMu
...
import { type Hook, hookCapability } from "@yemu/agent-runtime/capability/hook";
...
describe("codex hook discovery", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-codex-hooks-home-"));
		tempCwd = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-codex-hooks-cwd-"));
	...
	test("skips untyped Codex hook scripts so they never reach the extension loader (#3680)", async () => {
		...
		// YeMu-shaped pre-*/post-* files but do not match the prefix.
