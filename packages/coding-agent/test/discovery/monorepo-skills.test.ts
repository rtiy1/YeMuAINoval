...
import type { LoadContext, LoadResult } from "@yemu/agent-runtime/capability/types";
import { scanSkillsFromDir } from "@yemu/agent-runtime/discovery/helpers";
...
describe("monorepo skill discovery", () => {
	...
	beforeEach(() => {
		clearCache();
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-monorepo-skills-"));
	...
	test("finds skills in ancestor .yemu/skills/ directories", async () => {
		// Root has a skill
		writeSkill(path.join(repoRoot, ".yemu", "skills"), "root-skill", "From repo root");
		// Sub-project has a skill
		writeSkill(path.join(subProject, ".yemu", "skills"), "local-skill", "From sub-project");
		...
		while (true) {
			const result = await scanSkillsFromDir(ctx, {
				dir: path.join(current, ".yemu", "skills"),
				providerId: "native",
				level: "project",
			});
			results.push(result);
	...
	test("closest skill wins when same name exists at multiple levels", async () => {
		...
		writeSkill(path.join(repoRoot, ".yemu", "skills"), "shared-skill", "Root version");
		writeSkill(path.join(subProject, ".yemu", "skills"), "shared-skill", "Local version");
	...
	test("finds skills across multiple ancestor levels", async () => {
		...
		writeSkill(path.join(repoRoot, ".yemu", "skills"), "root-skill", "Root");
		writeSkill(path.join(packagesDir, ".yemu", "skills"), "packages-skill", "Packages");
		writeSkill(path.join(subProject, ".yemu", "skills"), "app-skill", "App");
	...
	test("walk-up stops at repo root and does not find skills above it", async () => {
		...
		writeSkill(path.join(tempDir, ".yemu", "skills"), "above-repo-skill", "Above repo");
		// Skill AT the repo root (should be found)
		writeSkill(path.join(repoRoot, ".yemu", "skills"), "root-skill", "At repo root");
