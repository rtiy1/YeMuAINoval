 ...
 * from both `~/.yemu/agent/RULES.md` (user) and the nearest `.yemu/RULES.md`
...
import { type Rule, ruleCapability } from "@yemu/agent-runtime/capability/rule";
...
beforeEach(() => {
	clearCache();
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-rules-md-"));
...
test("user ~/.yemu/agent/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(
		path.join(home, ".yemu", "agent", "RULES.md"),
...
test("project .yemu/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(path.join(project, ".yemu", "RULES.md"), "# Project rule\nAlways say hi.\n");
...
test("project RULES.md is found walking up from a sub-package cwd", async () => {
	...
	writeFile(path.join(project, ".yemu", "RULES.md"), "# Repo-wide sticky rule\n");
	...
	expect(projectRule).toBeDefined();
	expect(projectRule?.alwaysApply).toBe(true);
	expect(projectRule?.path).toBe(path.join(project, ".yemu", "RULES.md"));
});

test("user and project sticky RULES.md both survive public capability dedup", async () => {
	const userRulesPath = path.join(home, ".yemu", "agent", "RULES.md");
	const projectRulesPath = path.join(project, ".yemu", "RULES.md");
...
test("alwaysApply is forced even when frontmatter says false", async () => {
	writeFile(path.join(home, ".yemu", "agent", "RULES.md"), "---\nalwaysApply: false\n---\nStick around anyway.\n");
...
test("absent RULES.md does not produce a rule", async () => {
	// No RULES.md anywhere — only a sibling .yemu/rules/ to make sure the directory exists.
	writeFile(path.join(home, ".yemu", "agent", "rules", "other.md"), "# Unrelated rule\n");
