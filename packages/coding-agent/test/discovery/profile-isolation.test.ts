/**
 * Regression: YeMu-native user-level config discovery must follow the active
 * profile. A profile relocates the agent directory to ~/.yemu/profiles/<name>/agent;
 * the native provider used to read user config (commands, skills, rules, etc.)
 * from the literal home (~/.yemu/agent) via `ctx.home`, leaking the default
 ...
 * (ctx.home + ".yemu/agent") points at the tempdir decoys below; without the fix
...
import { type Skill, skillCapability } from "@yemu/agent-runtime/capability/skill";
import { type SlashCommand, slashCommandCapability } from "@yemu/agent-runtime/capability/slash-command";
import { loadCapability } from "@yemu/agent-runtime/discovery";
import { getConfigRootDir, removeWithRetries, setAgentDir } from "@yemu/utils";
...
describe("native user-level config discovery follows the active profile", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-profile-iso-home-"));
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-profile-iso-project-"));
		profileAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-profile-iso-agent-"));
		...
		const defaultAgentDir = path.join(tempHome, ".yemu", "agent");
