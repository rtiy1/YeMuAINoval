...
import { clearCache, readFile } from "@yemu/agent-runtime/capability/fs";
import type { Rule } from "@yemu/agent-runtime/capability/rule";
import type { LoadContext } from "@yemu/agent-runtime/capability/types";
import { getProjectPathCandidates } from "@yemu/agent-runtime/discovery/agents";
import {
...
describe("agents provider project-level discovery", () => {
	...
	beforeEach(() => {
		clearCache();
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-agents-monorepo-"));
	...
	describe("skills", () => {
		...
		test("project walk-up skips home directory (no repo root)", async () => {
			// Regression for upstream-change-1116:
