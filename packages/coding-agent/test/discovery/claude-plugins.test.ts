...
import { loadCapability } from "@yemu/agent-runtime/capability";
import { clearCache as clearFsCache } from "@yemu/agent-runtime/capability/fs";
import {
	clearClaudePluginRootsCache,
	listClaudePluginRoots,
	parseClaudePluginsRegistry,
} from "@yemu/agent-runtime/discovery/helpers";
import { loadSlashCommands } from "@yemu/agent-runtime/extensibility/slash-commands";
import { discoverAgents } from "@yemu/agent-runtime/task/discovery";
import { removeWithRetries } from "@yemu/utils";
import "@yemu/agent-runtime/discovery/claude-plugins";
import { type MCPServer, mcpCapability } from "@yemu/agent-runtime/capability/mcp";
import type { Skill } from "@yemu/agent-runtime/capability/skill";
import type { SlashCommand } from "@yemu/agent-runtime/capability/slash-command";
...
describe("listClaudePluginRoots", () => {
	...
	test("expands env placeholders in marketplace plugin MCP url and headers", async () => {
		...
		const originalApiKey = process.env.YEMU_PLUGIN_MCP_API_KEY;
		const originalUrl = process.env.YEMU_PLUGIN_MCP_URL;
		const envPlaceholder = (name: string): string => ["$", "{", name, ":-}"].join("");
		process.env.YEMU_PLUGIN_MCP_API_KEY = "ctx7sk-test-key";
		process.env.YEMU_PLUGIN_MCP_URL = "https://mcp.context7.example";
		...
		try {
			...
			await fs.writeFile(
				...
				JSON.stringify({
					context7: {
						type: "http",
						url: `${envPlaceholder("YEMU_PLUGIN_MCP_URL")}/mcp`,
						headers: {
							CONTEXT7_API_KEY: envPlaceholder("YEMU_PLUGIN_MCP_API_KEY"),
						},
					},
				}),
			);
			...
			if (originalApiKey === undefined) delete process.env.YEMU_PLUGIN_MCP_API_KEY;
			else process.env.YEMU_PLUGIN_MCP_API_KEY = originalApiKey;
			if (originalUrl === undefined) delete process.env.YEMU_PLUGIN_MCP_URL;
			else process.env.YEMU_PLUGIN_MCP_URL = originalUrl;
		}
	});

	test("uses YeMu then Claude manifest mcpServers paths before .mcp.json", async () => {
		const pluginsDir = path.join(tempDir, ".claude", "plugins");
		const yemuPluginPath = path.join(tempDir, "plugins", "yemu-pointer");
		const claudePluginPath = path.join(tempDir, "plugins", "claude-pointer");
		await fs.mkdir(pluginsDir, { recursive: true });
		await Promise.all([
			fs.mkdir(path.join(yemuPluginPath, ".yemu-plugin"), { recursive: true }),
			fs.mkdir(path.join(yemuPluginPath, ".claude-plugin"), { recursive: true }),
		...
		await Promise.all([
			fs.writeFile(
				path.join(yemuPluginPath, ".yemu-plugin", "plugin.json"),
				JSON.stringify({ mcpServers: "./mcp-yemu.json" }),
			),
			fs.writeFile(
				path.join(yemuPluginPath, ".claude-plugin", "plugin.json"),
				JSON.stringify({ mcpServers: "./mcp-claude.json" }),
			),
			fs.writeFile(path.join(yemuPluginPath, "mcp-yemu.json"), JSON.stringify({ "from-yemu": { command: "yemu" } })),
			fs.writeFile(
				path.join(yemuPluginPath, "mcp-claude.json"),
				JSON.stringify({ "from-claude": { command: "claude" } }),
			),
			fs.writeFile(path.join(yemuPluginPath, ".mcp.json"), JSON.stringify({ "from-root": { command: "root" } })),
			fs.writeFile(
	...
	test("loads inline manifest mcpServers object and roots relative stdio at plugin root", async () => {
		...
		await fs.mkdir(pluginsDir, { recursive: true });
		await fs.mkdir(path.join(pluginPath, ".yemu-plugin"), { recursive: true });
		await fs.writeFile(
			path.join(pluginsDir, "installed_plugins.json"),
			JSON.stringify({
				version: 2,
		...
		await fs.writeFile(
			path.join(pluginPath, ".yemu-plugin", "plugin.json"),
	...
	test("warns when a manifest mcpServers pointer names a missing file", async () => {
		...
		expect(result.warnings).toEqual([
			`[Claude Code Marketplace] [claude-plugins] Missing mcpServers file declared by broken-pointer@market: ${path.join(pluginPath, "mcp-yemu.json")}`,
		]);
	});
	...
	test("deduplicates a plugin alias of a directly configured MCP connection", async () => {
		const pluginsDir = path.join(tempDir, ".claude", "plugins");
		const pluginPath = path.join(tempDir, "plugins", "context7");
		const directConfigPath = path.join(tempDir, ".yemu", "mcp.json");
		const connection = {
			type: "http",
