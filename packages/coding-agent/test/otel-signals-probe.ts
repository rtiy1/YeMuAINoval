 ...
 * `@yemu/utils` logger and metric instruments through the agent
 * telemetry hooks, flushes, and exits 0 only if the receiver got a non-empty
 * protobuf POST at both /v1/logs and /v1/metrics.
 */

import type { AgentRunCoverage, AgentRunSummary, ChatUsageEvent } from "@yemu/agent-core";
import { emptyAgentRunCoverage, emptyAgentRunSummary } from "@yemu/agent-core";
import {
	createTelemetryExportConfig,
	flushTelemetryExport,
	initTelemetryExport,
	isTelemetryExportEnabled,
} from "@yemu/agent-runtime/telemetry-export";
...
process.env.OTEL_SERVICE_NAME = "yemu-agent-signals-probe";
...
await Bun.sleep(700);
await flushTelemetryExport();
assertSingleMetricPoint("yemu.agent.agent.chat.calls");
assertSingleMetricPoint("yemu.agent.agent.tool.calls");
assertSingleMetricPoint("yemu.agent.agent.tool.duration");
await server.stop(true);
