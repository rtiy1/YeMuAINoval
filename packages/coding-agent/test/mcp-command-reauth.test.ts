...
import * as mcpClient from "@yemu/agent-runtime/mcp/client";
import * as oauthFlow from "@yemu/agent-runtime/mcp/oauth-flow";
...
describe("/mcp auth commands", () => {
	...
	beforeEach(async () => {
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-mcp-reauth-project-"));
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-mcp-reauth-agent-"));
