...
import type { AgentMessage, AgentTool } from "@yemu/agent-core";
import { ModelRegistry } from "@yemu/agent-runtime/config/model-registry";
import { discoverAndLoadExtensions, ExtensionRuntime } from "@yemu/agent-runtime/extensibility/extensions/loader";
import {
	EXTENSION_HANDLER_TIMEOUT_MS,
	ExtensionRunner,
	testSetExtensionHandlerTimeoutMs,
} from "@yemu/agent-runtime/extensibility/extensions/runner";
import type {
	ExtensionError,
	ExtensionServiceTier,
	ExtensionUIContext,
} from "@yemu/agent-runtime/extensibility/extensions/types";
import { ExtensionToolWrapper } from "@yemu/agent-runtime/extensibility/extensions/wrapper";
import { Type } from "@yemu/agent-runtime/extensibility/typebox";
...
import type { ImageContent, TextContent } from "@yemu/model-runtime";
import { getProjectAgentDir, logger, TempDir } from "@yemu/utils";
...
describe("ExtensionRunner", () => {
	...
	beforeAll(async () => {
		sharedTempDir = TempDir.createSync("@yemu-runner-shared-");
	...
	beforeEach(() => {
		tempDir = TempDir.createSync("@yemu-runner-test-");
	...
	describe("shortcut conflicts", () => {
		...
		it("warns when two extensions register same shortcut", async () => {
			...
			const extCode1 = `
				export default function(yemu) {
					yemu.registerShortcut("ctrl+shift+x", {
	...
	describe("message renderers", () => {
		it("gets message renderer by type", async () => {
			const extCode = `
				export default function(yemu) {
					yemu.registerMessageRenderer("my-type", (message, options, theme) => null);
				}
			`;
		...
		it("collects assistant thinking renderers", async () => {
			const extCode = `
				export default function(yemu) {
					yemu.registerAssistantThinkingRenderer((context, theme) => null);
				}
			`;
	...
	describe("before_provider_request chaining", () => {
		it("exposes the request model instead of the primary session model", async () => {
			...
			const extCode = `
				export default function(yemu) {
					yemu.on("before_provider_request", async (_event, ctx) => {
		...
		it("chains payload replacements across handlers in load order", async () => {
			const extCode1 = `
				export default function(yemu) {
					yemu.on("before_provider_request", async (event) => {
		...
		it("keeps chaining after handler errors", async () => {
			const extCode1 = `
				export default function(yemu) {
					yemu.on("before_provider_request", async () => {
	...
	describe("after_provider_response", () => {
		it("calls handlers with response metadata and reports handler errors without throwing", async () => {
			...
			export default function(yemu) {
				yemu.on("after_provider_response", async (event) => {
					fs.appendFileSync(
				...
				yemu.on("after_provider_response", async () => {
	...
	describe("session_stop", () => {
		...
		it("observes a session_stop signal aborted synchronously by the handler", async () => {
			...
			await Bun.write(
				...
				`
				export default function(yemu) {
					yemu.on("session_stop", async (_event, ctx) => {
						ctx.abort();
						await Promise.withResolvers().promise;
					});
				}
		...
		it("continues to later handlers after empty continuation feedback", async () => {
			await Bun.write(
				...
				`
				export default function(yemu) {
					yemu.on("session_stop", async () => ({ continue: true }));
					yemu.on("session_stop", async () => ({ decision: "block", reason: "Continue from second handler." }));
				}
			`,
			);
	...
	describe("tool_result chaining", () => {
		it("chains content modifications across handlers", async () => {
			const extCode1 = `
				export default function(yemu) {
					yemu.on("tool_result", async (event) => {
						return {
		...
		it("aborts a tool_call handler's confirmation before returning its timeout block", async () => {
			...
			fs.writeFileSync(
				...
				`
					...
					export default function(yemu) {
						yemu.on("tool_call", async (_event, ctx) => {
		...
		it("exposes the lazy memory runtime after initialization", async () => {
			const extCode = `
				export default function(yemu) {
					yemu.on("session_start", async (_event, ctx) => {
						globalThis.__yemuMemoryStatus = await ctx.memory.status();
					});
				}
			`;
			const explicitExtensionPath = path.join(tempDir.path(), "memory-context.ts");
			fs.writeFileSync(explicitExtensionPath, extCode);
			const globalState = globalThis as typeof globalThis & { __yemuMemoryStatus?: unknown };
			delete globalState.__yemuMemoryStatus;
			...
			const runner = new ExtensionRunner(
				...
				() => ({
					...
					search: async query => ({ backend: "yemu-memory", query, count: 0, items: [] }),
					save: async () => ({ backend: "yemu-memory", stored: 1 }),
				}),
			);
			runner.initialize(
				{
			...
			expect(globalState.__yemuMemoryStatus).toMatchObject({
				backend: "yemu-memory",
				active: true,
				searchable: true,
			});
	...
	describe("service tier API", () => {
		...
		it("returns a detached snapshot, forwards valid changes, and rejects invalid family tiers", async () => {
			const extCode = `
				export default function(yemu) {
					yemu.on("session_start", () => {
						const tiers = yemu.getServiceTiers();
						tiers.openai = "scale";
						yemu.appendEntry("service-tier-snapshot", tiers);
						yemu.setServiceTier("google", "flex");
						yemu.setServiceTier("openai", undefined);
					});
					yemu.on("session_start", () => {
						yemu.setServiceTier("anthropic", "scale");
					});
					yemu.on("session_start", () => {
						yemu.setServiceTier("bogus", "priority");
					});
				}
			`;
	...
	describe("session name API", () => {
		it("lets extensions read and set the session name after initialization", async () => {
			const extCode = `
				export default function(yemu) {
					yemu.on("session_start", async () => {
						if (yemu.getSessionName() !== undefined) {
							throw new Error("expected unnamed session");
						}
						await yemu.setSessionName("Named by extension");
					});
				}
			`;
	...
	describe("tool approval lifecycle", () => {
		...
		it("emits requested before waiting and resolved after approval", async () => {
			...
			const extCode = `
				export default function(yemu) {
					yemu.on("tool_approval_requested", async (event) => {
						globalThis.__approvalEvents.push({ type: event.type });
					});
					yemu.on("tool_approval_resolved", async (event) => {
		...
		it("emits tool_call before the approval prompt so approval sees the final input", async () => {
			...
			const extCode = `
				export default function(yemu) {
					...
					yemu.on("tool_approval_requested", async () => {
	...
	describe("hasHandlers", () => {
		it("returns true when handlers exist for event type", async () => {
			const extCode = `
				export default function(yemu) {
					yemu.on("tool_call", async () => undefined);
				}
			`;
	...
	describe("credential_disabled", () => {
		it("delivers credential_disabled events to subscribed extensions with the typed payload", async () => {
			...
			const extCode = `
				...
				export default function(yemu) {
					yemu.on("credential_disabled", async (event) => {
						fs.appendFileSync(
							${JSON.stringify(eventsPath)},
							JSON.stringify({
								type: event.type,
		...
		it("isolates subscriber failures so other handlers still receive the event", async () => {
			...
			const ext1Code = `
				export default function(yemu) {
					yemu.on("credential_disabled", async () => {
	...
	describe("mcp_notification", () => {
		it("delivers mcp_notification events to subscribed extensions with the typed payload", async () => {
			...
			const extCode = `
				...
				export default function(yemu) {
					yemu.on("mcp_notification", async (event) => {
						fs.appendFileSync(
							${JSON.stringify(eventsPath)},
							JSON.stringify({
								type: event.type,
