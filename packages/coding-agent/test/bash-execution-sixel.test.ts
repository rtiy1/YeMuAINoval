...
import { BashExecutionComponent } from "@yemu/agent-runtime/modes/components/bash-execution";
import { getThemeByName, setThemeInstance } from "@yemu/agent-runtime/modes/theme/theme";
import { sanitizeWithOptionalSixelPassthrough } from "@yemu/agent-runtime/utils/sixel";
import type { TUI } from "@yemu/tui";
import { sanitizeText } from "@yemu/utils";

const SIXEL = "\x1bPqabc\x1b\\";

describe("BashExecutionComponent SIXEL sanitization", () => {
	const originalForceProtocol = Bun.env.YEMU_FORCE_IMAGE_PROTOCOL;
	const originalAllowPassthrough = Bun.env.YEMU_ALLOW_SIXEL_PASSTHROUGH;
	...
	afterEach(() => {
		if (originalForceProtocol === undefined) delete Bun.env.YEMU_FORCE_IMAGE_PROTOCOL;
		else Bun.env.YEMU_FORCE_IMAGE_PROTOCOL = originalForceProtocol;
		if (originalAllowPassthrough === undefined) delete Bun.env.YEMU_ALLOW_SIXEL_PASSTHROUGH;
		else Bun.env.YEMU_ALLOW_SIXEL_PASSTHROUGH = originalAllowPassthrough;
	});

	it("preserves SIXEL output when passthrough gates are enabled", () => {
		Bun.env.YEMU_FORCE_IMAGE_PROTOCOL = "sixel";
		Bun.env.YEMU_ALLOW_SIXEL_PASSTHROUGH = "1";
	...
	it("strips SIXEL control escapes when passthrough gates are disabled", () => {
		delete Bun.env.YEMU_FORCE_IMAGE_PROTOCOL;
		delete Bun.env.YEMU_ALLOW_SIXEL_PASSTHROUGH;
