...
import type { StreamFn } from "@yemu/agent-core";
...
import type { CursorExecHandlers } from "@yemu/agent-runtime/cursor";
import type { MCPManager } from "@yemu/agent-runtime/mcp/manager";
import {
...
import { VIBE_TOOL_NAMES } from "@yemu/agent-runtime/tools/vibe";
import { getBundledModel } from "@yemu/model-catalog/models";
import type { Model, ToolResultMessage } from "@yemu/model-runtime";
import { createMockModel } from "@yemu/model-runtime/providers/mock";
import { logger, removeSyncWithRetries, Snowflake } from "@yemu/utils";
import { type } from "arktype";

const toolActivationExtension: ExtensionFactory = yemu => {
	yemu.registerTool({
...
describe("createAgentSession defaultInactive tool activation", () => {
	...
	const makeTempDir = (): string => {
		const tempDir = path.join(os.tmpdir(), `yemu-sdk-tool-activation-${Snowflake.next()}`);
	...
	beforeAll(async () => {
		registryAuthDir = path.join(os.tmpdir(), `yemu-sdk-tool-activation-auth-${Snowflake.next()}`);
	...
	// session must still reach a replace-mode instance for `yemu_edit` (whose
	...
	const captureCursorExecHandlers = async (session: AgentSession, cursorModel: Model): Promise<CursorExecHandlers> => {
		...
		const streamFn: StreamFn = (_model, _context, options) => {
			...
			// typed as the wire-level interface, whose `yemuEdit` answers a proto
	...
	it("answers a native yemu_edit after a session switches onto Cursor", async () => {
		const tempDir = makeTempDir();
		const cursorModel = getBundledModel("cursor", "composer-1.5");
		if (!cursorModel) throw new Error("expected bundled Cursor model");
	...
	it("refuses a native yemu_edit after a read-only session switches onto Cursor", async () => {
	...
	it("does not execute an unadvertised edit call through the fallback resolver", async () => {
		...
		// `yemu_edit` gets its instance from `getEditReplaceTool` instead.
		const tempDir = makeTempDir();
		const target = path.join(tempDir, "sample.txt");
		fs.writeFileSync(target, "alpha\nbeta\n");
	...
	it("runs advisor tools through the approval gate", async () => {
		...
		// Cursor exec bridge (`yemuWrite`/`yemuBash`) run those instances directly,
