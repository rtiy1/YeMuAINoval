...
import {
	...
	resolveModelScope,
} from "@yemu/agent-runtime/config/model-resolver";
import { DEFAULT_MODEL_ROLE_ALIAS, LEGACY_MODEL_ROLE_ALIAS_PREFIX } from "@yemu/agent-runtime/config/model-roles";
...
describe("extractExplicitThinkingSelector", () => {
	...
	test("treats max on yemu role aliases as an explicit selector before expansion", () => {
		const settings = Settings.isolated();
