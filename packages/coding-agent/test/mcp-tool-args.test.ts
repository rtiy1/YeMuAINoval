...
import { DeferredMCPTool, MCPTool, type MCPToolDefinition } from "@yemu/agent-runtime/mcp";
import type { MCPServerConnection } from "@yemu/agent-runtime/mcp/types";
...
describe("MCP tool arguments", () => {
	...
	it("resolves local image arguments before forwarding tools/call", async () => {
		using tempDir = TempDir.createSync("@yemu-mcp-local-image-");
		const calls: CapturedRequest[] = [];
