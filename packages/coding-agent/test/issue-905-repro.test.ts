 ...
 * (via `yemu.registerProvider(...)`), regardless of whether the extension was
 ...
 *
 * Contract under test: the `yemu models` listing entry point loads extensions
...
import { runModelsListing } from "@yemu/agent-runtime/cli/models-cli";
...
`,
	...
	await fs.writeFile(
		shutdownExtPath,
		`export default function (yemu) {
	yemu.on("session_shutdown", async () => {
...
test("yemu models surfaces extension-registered providers (issue #905)", async () => {
...
test("yemu models emits extension shutdown after listing (issue #6297)", async () => {
...
test("yemu models prints invalid models.yml schema errors before listing output", async () => {
