...
import { applyStartupCwd } from "@yemu/agent-runtime/cli/startup-cwd";
...
describe("parseArgs — --cwd flag", () => {
	...
	it("applies --cwd before session lookup callers read the project directory", async () => {
		const launchDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-cwd-launch-"));
		const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-cwd-target-"));
	...
	it("normalizes a relative --cwd target to the resolved absolute path", async () => {
		const launchDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-cwd-rel-"));
