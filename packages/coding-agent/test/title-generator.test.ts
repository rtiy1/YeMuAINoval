...
} from "@yemu/agent-runtime/utils/title-generator";
...
import { logger, setTerminalHeadless } from "@yemu/utils";
...
// separator (spinner frame, static Windows `:`, `>`, or `!` between the `Y`
...
describe("terminal title runtime", () => {
	...
	it("keeps the working title static with ':' on Windows", () => {
		...
		try {
			...
			setTerminalTitleState("working");
			expect(emittedTitles()).toEqual(["Y : windows-project"]);
