...
async function runProbeScenario(options: {
	...
	const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-gpu-probe-"));
	try {
		const binDir = path.join(tempRoot, "bin");
		const cacheRoot = path.join(tempRoot, "cache");
		const probeCountPath = path.join(tempRoot, "probe-count");
		await fs.mkdir(binDir, { recursive: true });
		await fs.mkdir(path.join(cacheRoot, "yemu"), { recursive: true });
		const lspciPath = path.join(binDir, "lspci");
		await Bun.write(
			lspciPath,
			'#!/usr/bin/env sh\nprintf x >> "$YEMU_GPU_PROBE_COUNT"\nif [ -n "$YEMU_GPU_PROBE_VALID_OUTPUT" ]; then printf "%s\\n" "$YEMU_GPU_PROBE_VALID_OUTPUT"; fi\nif [ "$YEMU_GPU_PROBE_DESCENDANT_HOLDS_STDOUT" = "true" ]; then sleep "$YEMU_GPU_PROBE_SLEEP" & exit 0; fi\nif [ "$YEMU_GPU_PROBE_HOLD_STDOUT_OPEN" = "true" ]; then sleep "$YEMU_GPU_PROBE_SLEEP" & wait "$!"; fi\nif [ -n "$YEMU_GPU_PROBE_SLEEP" ]; then exec sleep "$YEMU_GPU_PROBE_SLEEP"; fi\nexit 0\n',
		);
...
for (let index = 0; index < Number(process.env.YEMU_GPU_PROBE_RUNS ?? "1"); index += 1) {
...
const countFile = Bun.file(process.env.YEMU_GPU_PROBE_COUNT ?? "");
...
`,
		...
		const env: Record<string, string | undefined> = {
			...
			YEMU_GPU_PROBE_COUNT: probeCountPath,
			YEMU_GPU_PROBE_RUNS: String(options.runs),
		};
		...
		for (const key of ["YEMU_CODING_AGENT_DIR", "YEMU_PROFILE", "YEMU_LEGACY_PROFILE", "YEMU_CONFIG_DIR"]) {
			delete env[key];
		}
		if (options.sleepSeconds === undefined) {
			delete env.YEMU_GPU_PROBE_SLEEP;
		} else {
			env.YEMU_GPU_PROBE_SLEEP = String(options.sleepSeconds);
		}
		if (options.holdStdoutOpen) {
			env.YEMU_GPU_PROBE_HOLD_STDOUT_OPEN = "true";
		} else {
			delete env.YEMU_GPU_PROBE_HOLD_STDOUT_OPEN;
		}
		if (options.descendantHoldsStdout) {
			env.YEMU_GPU_PROBE_DESCENDANT_HOLDS_STDOUT = "true";
		} else {
			delete env.YEMU_GPU_PROBE_DESCENDANT_HOLDS_STDOUT;
		}
		if (options.validOutput !== undefined) {
			env.YEMU_GPU_PROBE_VALID_OUTPUT = options.validOutput;
		} else {
			delete env.YEMU_GPU_PROBE_VALID_OUTPUT;
		}
