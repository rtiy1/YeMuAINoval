...
beforeEach(async () => {
	tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-foreign-sessions-"));
});
...
describe("foreign session persistence", () => {
	it("writes a fresh YeMu identity with source provenance", async () => {
		const { info, store } = await createClaudeFixture();
		const sessionDir = path.join(tempRoot, "yemu-sessions");
