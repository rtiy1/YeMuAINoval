...
} from "@yemu/agent-runtime/extensibility/legacy-agent-runtime-shim";
...
// Issue #4567: every published version of yemu-schedule-prompt (and every yemu
// extension spawning subagents) imports `DefaultResourceLoader` at module
// scope from `@yemu/agent-runtime` and hands it to
...
describe("DefaultPackageManager.resolve() (issue #5658)", () => {
	it("enumerates configured extension paths through YeMu discovery", async () => {
		const tmp = await mkTempCwd("yemu-legacy-default-package-manager-");
...
describe("DefaultResourceLoader.reload() (issue #4567)", () => {
	it("populates the discovery snapshot honoring no* flags and applying every override", async () => {
		const tmp = await mkTempCwd("yemu-legacy-default-resource-loader-reload-");
		...
		// which is the contract yemu extensions rely on to inject synthetic
	...
	it("loads relative additional skills and prompt templates before override callbacks when discovery is disabled", async () => {
		const tmp = await mkTempCwd("yemu-legacy-default-resource-loader-additional-paths-");
...
describe("createAgentSession({ resourceLoader }) (issue #4567)", () => {
	it("translates a DefaultResourceLoader into the SDK's option surface without silently discarding it", async () => {
		const tmp = await mkTempCwd("yemu-legacy-default-resource-loader-session-");
