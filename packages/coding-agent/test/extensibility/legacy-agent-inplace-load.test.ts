...
// Issue #1674: legacy YeMu extensions load browser-UI assets (HTML/CSS) at module
...
// public `loadLegacyAgentModule` entry point.
...
async function writePackage(files: Record<string, string>): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-legacy-inplace-"));
	tempRoots.push(dir);
...
describe("legacy-agent in-place module loading (issue #1674)", () => {
	it("reads __dirname-relative HTML assets from the real extension directory", async () => {
		const dir = await writePackage({
			...
			"index.ts": [
				...
				"export default function (yemu) { void yemu; }",
			].join("\n"),
		});

		const mod = (await loadLegacyAgentModule(path.join(dir, "index.ts"))) as { dirName: string; html: string };
	...
	it("loads CommonJS helpers required by an ES module extension", async () => {
		...
		const mod = (await loadLegacyAgentModule(path.join(dir, "index.js"))) as { value: string };
	...
	it("loads a relative CommonJS helper imported by a TypeScript extension", async () => {
		...
		const mod = (await loadLegacyAgentModule(path.join(dir, "index.ts"))) as { value: number };

		expect(mod.value).toBe(42);
	});

	it("remaps legacy YeMu requires in graph-owned CommonJS packages to the host shim", async () => {
		const dir = await writePackage({
			"package.json": JSON.stringify({ name: "cjs-legacy-agent-require-ext", version: "1.0.0", type: "module" }),
			"node_modules/direct/package.json": JSON.stringify({
				name: "direct",
				version: "1.0.0",
				main: "index.js",
			}),
			"node_modules/direct/index.js": 'module.exports = require("@yemu/model-runtime").Type;\n',
			"index.ts": [
		...
		const mod = (await loadLegacyAgentModule(path.join(dir, "index.ts"))) as { sharesHostType: boolean };
	...
	it("loads a default import from linkedom's CommonJS canvas fallback", async () => {
		...
		const mod = await loadLegacyAgentModule(path.join(dir, "index.js"));
	...
	it("reads a lazy CommonJS helper at import time", async () => {
		...
		const mod = (await loadLegacyAgentModule(path.join(dir, "index.js"))) as { loadValue(): Promise<string> };
	...
	it("reloads an edited CommonJS helper imported from ESM", async () => {
		...
		const first = await loadLegacyAgentModule(entry);
		...
		const second = await loadLegacyAgentModule(entry);
	...
	it("reloads an edited entry module without polluting fileURLToPath-derived paths", async () => {
		...
		const first = (await loadLegacyAgentModule(entry)) as { version: string; entryPath: string; asset: string };
		...
		const second = (await loadLegacyAgentModule(entry)) as { version: string; entryPath: string; asset: string };
	...
	it("reloads an edited relative helper module on same-process re-import", async () => {
		...
		const first = (await loadLegacyAgentModule(entry)) as { entryVersion: string; H: string; entryPath: string };
		...
		const second = (await loadLegacyAgentModule(entry)) as { entryVersion: string; H: string; entryPath: string };
	...
	it("reloads edited children of an extension-local bare dependency", async () => {
		...
		const first = (await loadLegacyAgentModule(entry)) as { entryVersion: string; depValue: string };
		expect(first.entryVersion).toBe("v1");
		...
		const second = (await loadLegacyAgentModule(entry)) as { entryVersion: string; depValue: string };
		expect(second.entryVersion).toBe("v2");
	...
	it("keeps transitive bare dependency importer paths query-free", async () => {
		...
		const mod = (await loadLegacyAgentModule(path.join(dir, "index.ts"))) as { nestedUrl: string };
	...
	it("preserves named re-exports from relative ESM children in a no-type dual package", async () => {
		...
		const mod = (await loadLegacyAgentModule(path.join(dir, "index.ts"))) as { result: string };
	...
	it("keeps a syntax-ambiguous conditional import target on its ESM branch", async () => {
		...
		const mod = (await loadLegacyAgentModule(path.join(dir, "index.ts"))) as { loaded: boolean };
	...
	it("reloads a hoisted CommonJS dependency required by a relative CommonJS child", async () => {
		...
		const first = (await loadLegacyAgentModule(entry)) as { entryVersion: string; dependencyValue: string };
		expect(first.entryVersion).toBe("v1");
		...
		const second = (await loadLegacyAgentModule(entry)) as { entryVersion: string; dependencyValue: string };
		expect(second.entryVersion).toBe("v2");
	...
	it("preserves named imports through CommonJS package re-exports", async () => {
		...
		const mod = (await loadLegacyAgentModule(path.join(dir, "index.ts"))) as { value: string };
	...
	it("preserves named imports from CommonJS defineProperty and exportStar patterns", async () => {
		...
		const mod = (await loadLegacyAgentModule(path.join(dir, "index.ts"))) as {
			local: string;
			value: string;
		};
	...
	it("reloads a lazily imported CommonJS package re-export", async () => {
		...
		const first = (await loadLegacyAgentModule(entry)) as {
			entryVersion: string;
		...
		const second = (await loadLegacyAgentModule(entry)) as {
			entryVersion: string;
			loadValue(): Promise<string>;
		};
		expect(second.entryVersion).toBe("v2");
	...
	it("reloads modules added to the relative import graph after the first load", async () => {
		...
		const first = (await loadLegacyAgentModule(entry)) as { entryVersion: string; entryPath: string };
		expect(first.entryVersion).toBe("v1");
		expect(first.entryPath).toBe(expectedEntryPath);
		...
		const third = (await loadLegacyAgentModule(entry)) as {
			entryVersion: string;
			entryPath: string;
			leafValue: string;
		};
	...
	it("resolves a .css sibling of a relatively-imported submodule", async () => {
		const dir = await writePackage({
			...
			"index.ts": ['export { css } from "./sub/widget.ts";', "export default function (yemu) { void yemu; }"].join(
				"\n",
			),
		});

		const mod = (await loadLegacyAgentModule(path.join(dir, "index.ts"))) as { css: string };
	...
	it("leaves JSON import-attribute targets on Bun's native loader", async () => {
		...
		const mod = (await loadLegacyAgentModule(path.join(dir, "index.ts"))) as { inputPrice: number };

		expect(mod.inputPrice).toBe(0.15);
	});

	it("loads the extension's own node_modules deps natively while remapping legacy yemu imports", async () => {
		const dir = await writePackage({
		...
		const mod = (await loadLegacyAgentModule(path.join(dir, "index.ts"))) as { depValue: string; hasZod: boolean };

		// CJS dep under node_modules keeps Bun's native resolution (it is excluded
		// from the rewrite onLoad), and the legacy yemu import is remapped to the
		// bundled Zod-backed shim.
		expect(mod.depValue).toBe("cjs-native");
		expect(mod.hasZod).toBe(true);
	});

	it("exposes legacy root tool factories used by yemu-lean-ctx", async () => {
		const dir = await writePackage({
			"package.json": JSON.stringify({ name: "legacy-tool-factory-ext", version: "1.0.0" }),
			"index.ts": [
				'import { Text } from "@yemu/tui";',
				"import {",
				...
				"};",
				"export default function (yemu) { for (const definition of definitions) yemu.registerTool(definition); }",
			].join("\n"),
		});
	...
	it("exposes a fresh legacy extension runtime through the package root", async () => {
		const dir = await writePackage({
			...
			"index.ts": [
				'import { createExtensionRuntime } from "@yemu/agent-runtime";',
	...
	it("honors legacy bash operations overrides", async () => {
		const dir = await writePackage({
			...
			"index.ts": [
				'import { createBashToolDefinition } from "@yemu/agent-runtime";',
				"const updates = [];",
				"let captured;",
				...
				"  updates,",
				"};",
				"export default function (yemu) { yemu.registerTool(tool); }",
			].join("\n"),
		});
	...
	it("preserves relative paths from legacy find operations", async () => {
		const dir = await writePackage({
			...
			"index.ts": [
				'import { createFindToolDefinition } from "@yemu/agent-runtime";',
		...
		const mod = (await loadLegacyAgentModule(path.join(dir, "index.ts"))) as { lines: string[] };
	...
	it("remaps legacy model-runtime utils/oauth subpaths to registry OAuth exports", async () => {
		const dir = await writePackage({
			"package.json": JSON.stringify({ name: "legacy-oauth-ext", version: "1.0.0" }),
			"index.ts": [
				'import { registerOAuthProvider } from "@yemu/model-runtime/utils/oauth";',
				'import { refreshAnthropicToken } from "@yemu/model-runtime/utils/oauth/anthropic";',
	...
	it("rewrites legacy imports in ../src modules reached through relative imports", async () => {
		const dir = await writePackage({
			...
			"src/helper.ts": [
				'import { isCompiledBinary } from "@yemu/utils";',
		...
		const mod = (await loadLegacyAgentModule(path.join(dir, "dist", "extension.ts"))) as { ok: boolean };
	...
	it("does not rewrite sibling files outside the loaded extension's import graph", async () => {
		const dir = await writePackage({
			...
			"index.ts": ['export { local } from "./local.ts";', "export default function (yemu) { void yemu; }"].join(
				"\n",
			),
		...
		const entryMod = (await loadLegacyAgentModule(path.join(dir, "index.ts"))) as { local: string };
		...
		await expect(import(siblingUrl)).rejects.toThrow(/@earendil-works\/model-runtime/);
	});
});
