...
import type { Rule } from "@yemu/agent-runtime/capability/rule";
...
import { LocalProtocolHandler } from "@yemu/agent-runtime/internal-urls/local-protocol";
import { AgentLifecycleManager } from "@yemu/agent-runtime/registry/agent-lifecycle";
import { AgentRegistry } from "@yemu/agent-runtime/registry/agent-registry";
import { createAgentSession } from "@yemu/agent-runtime/sdk";
import * as secrets from "@yemu/agent-runtime/secrets";
...
import { VibeSessionRegistry } from "@yemu/agent-runtime/vibe/runtime";
import { getBundledModel } from "@yemu/model-catalog/models";
import type { AssistantMessage } from "@yemu/model-runtime";
import { getSessionsDir, removeSyncWithRetries, Snowflake } from "@yemu/utils";
import { getActiveProfile, getConfigRootDir, setProfile } from "@yemu/utils/dirs";
...
async function withTempConfigRoot<T>(run: () => Promise<T>): Promise<T> {
	...
	const originalConfigDir = process.env.YEMU_CONFIG_DIR;
	const originalAgentDir = process.env.YEMU_CODING_AGENT_DIR;
	const configDirName = `.yemu-sdk-session-${Snowflake.next()}`;
	const configRoot = path.join(os.homedir(), configDirName);
	try {
		process.env.YEMU_CONFIG_DIR = configDirName;
		...
		if (originalAgentDir === undefined) {
			...
			process.env.YEMU_CODING_AGENT_DIR = originalAgentDir;
		}
...
describe("createAgentSession session storage isolation", () => {
	...
	it("uses the provided agentDir for the default persistent session root", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `yemu-sdk-session-isolation-${Snowflake.next()}-`));
	...
	it("keeps subagent local:// mappings from replacing the process-global override", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `yemu-sdk-local-override-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
	...
	it("does not replace a newer registry generation when creation expected the id to be absent", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `yemu-sdk-generation-cas-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
		const cwd = path.join(tempDir, "project");
		fs.mkdirSync(cwd, { recursive: true });
	...
	it("reuses the exact parked ref authorized for revival", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `yemu-sdk-generation-revive-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
		const cwd = path.join(tempDir, "project");
		fs.mkdirSync(cwd, { recursive: true });
	...
	it("suspends the exact Vibe owner scope before global lifecycle teardown", async () => {
		...
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `yemu-sdk-vibe-dispose-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
		const cwd = path.join(tempDir, "project");
		fs.mkdirSync(cwd, { recursive: true });
		const { session } = await createAgentSession({
	...
	it("wires the discovered TTSR manager into the created session", async () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `yemu-sdk-ttsr-${Snowflake.next()}-`));
		tempDirs.push(tempDir);
		const cwd = path.join(tempDir, `project-${Snowflake.next()}`);
		const agentDir = path.join(tempDir, "agent");
	...
	it("loads obfuscator only when secrets exist", async () => {
		await withClearedSecretEnv(async () => {
			const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `yemu-sdk-secrets-${Snowflake.next()}-`));
			tempDirs.push(tempDir);
			const cwd = path.join(tempDir, "project");
			const agentDir = path.join(tempDir, "agent");
			fs.mkdirSync(cwd, { recursive: true });
			...
			fs.mkdirSync(path.join(cwd, ".yemu"), { recursive: true });
			fs.writeFileSync(
				path.join(cwd, ".yemu", "secrets.yml"),
	...
	it("restores keyed assistant placeholders across reloads", async () => {
		await withClearedSecretEnv(async () => {
			await withTempConfigRoot(async () => {
				const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `yemu-sdk-session-secrets-${Snowflake.next()}-`));
				tempDirs.push(tempDir);
				const cwd = path.join(tempDir, "project");
				const agentDir = path.join(tempDir, "agent");
				fs.mkdirSync(path.join(cwd, ".yemu"), { recursive: true });
	...
	it("creates the placeholder key only when an obfuscate-mode secret is configured", async () => {
		await withClearedSecretEnv(async () => {
			const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `yemu-sdk-secrets-key-${Snowflake.next()}-`));
			tempDirs.push(tempDir);
			const cwd = path.join(tempDir, "project");
			const agentDir = path.join(tempDir, "agent");
			fs.mkdirSync(path.join(cwd, ".yemu"), { recursive: true });
	...
	it("redacts a pre-existing placeholder key when only ignored short secrets remain", async () => {
		await withClearedSecretEnv(async () => {
			const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `yemu-sdk-secrets-stale-key-${Snowflake.next()}-`));
			tempDirs.push(tempDir);
			const cwd = path.join(tempDir, "project");
			const agentDir = path.join(tempDir, "agent");
			fs.mkdirSync(path.join(cwd, ".yemu"), { recursive: true });
			...
			fs.writeFileSync(path.join(cwd, ".yemu", "secrets.yml"), "- type: plain\n  content: abc\n");
	...
	it("stores placeholder keys under the configured agentDir", async () => {
		await withClearedSecretEnv(async () => {
			await withTempConfigRoot(async () => {
				const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `yemu-sdk-secrets-agent-key-${Snowflake.next()}-`));
				tempDirs.push(tempDir);
				const cwd = path.join(tempDir, "project");
				const agentDir = path.join(tempDir, "agent");
				fs.mkdirSync(path.join(cwd, ".yemu"), { recursive: true });
