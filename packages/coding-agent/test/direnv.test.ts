...
import { applyDirenvPreflight, executeBash } from "@yemu/agent-runtime/exec/bash-executor";
import { findEnvrc, loadDirenvEnv, parseDirenvExport } from "@yemu/agent-runtime/exec/direnv";
import { $which, TempDir } from "@yemu/utils";
...
function tmp(): string {
	const dir = TempDir.createSync("@yemu-direnv-");
	tmpDirs.push(dir);
	return dir.path();
}
...
describe.skipIf(!hasDirenv)("loadDirenvEnv (real direnv, allow-list honored)", () => {
	...
	it("reports variables a .envrc unsets", async () => {
		const root = tmp();
		await Bun.write(path.join(root, ".envrc"), "unset YEMU_DIRENV_UNSET_TEST\n");
		await allowEnvrc(root);
		...
		Bun.env.YEMU_DIRENV_UNSET_TEST = "present";
		try {
			const diff = await loadDirenvEnv(root);
			expect(diff?.set.YEMU_DIRENV_UNSET_TEST).toBeUndefined();
			expect(diff?.unset).toContain("YEMU_DIRENV_UNSET_TEST");
		} finally {
			delete Bun.env.YEMU_DIRENV_UNSET_TEST;
		}
	});
...
describe.skipIf(!hasDirenv)("bash executor direnv wiring (end-to-end)", () => {
	...
	it("removes variables the .envrc unsets from the command environment", async () => {
		const root = tmp();
		await Bun.write(path.join(root, ".envrc"), "unset YEMU_DIRENV_UNSET_E2E\n");
		await allowEnvrc(root);
		// Inherited from the process env (as an YeMu-provided var would be); the
		...
		// (Avoid a `DIRENV_`-prefixed name — the loader strips those before spawn.)
		Bun.env.YEMU_DIRENV_UNSET_E2E = "leaked";
		try {
			const result = await executeBash('printenv YEMU_DIRENV_UNSET_E2E; printf "rc=%s" "$?"', {
				cwd: root,
			...
			delete Bun.env.YEMU_DIRENV_UNSET_E2E;
		}
	});
});
...
describe.skipIf(!hasDirenv)("applyDirenvPreflight (shared all-backends preflight)", () => {
	...
	it("prepends a regex-gated `unset -v` for vars the .envrc removes, skipping caller-resupplied names", async () => {
		const root = tmp();
		await Bun.write(path.join(root, ".envrc"), "unset YEMU_PF_UNSET_A\nunset YEMU_PF_UNSET_B\n");
		await allowEnvrc(root);
		Bun.env.YEMU_PF_UNSET_A = "present";
		Bun.env.YEMU_PF_UNSET_B = "present";
		try {
			const { command } = await applyDirenvPreflight("run-it", root, {
				// Caller re-supplies B, so its unset must be skipped (caller wins).
				callerEnv: { YEMU_PF_UNSET_B: "kept" },
				direnvSetting: "auto",
			});
			expect(command).toContain("unset -v YEMU_PF_UNSET_A");
			expect(command).not.toContain("YEMU_PF_UNSET_B");
			expect(command.endsWith("run-it")).toBe(true);
		} finally {
			delete Bun.env.YEMU_PF_UNSET_A;
			delete Bun.env.YEMU_PF_UNSET_B;
		}
	});

	it("applies the shell commandPrefix after the unset prefix", async () => {
		const root = tmp();
		await Bun.write(path.join(root, ".envrc"), "unset YEMU_PF_ORDER\n");
		await allowEnvrc(root);
		Bun.env.YEMU_PF_ORDER = "present";
		try {
			...
			expect(command).toBe("unset -v YEMU_PF_ORDER; strace -f payload");
		} finally {
			delete Bun.env.YEMU_PF_ORDER;
		}
	});
