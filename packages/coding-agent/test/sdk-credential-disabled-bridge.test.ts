...
import type { Extension, ExtensionError, ExtensionFactory } from "@yemu/agent-runtime/extensibility/extensions";
import { ExtensionRunner } from "@yemu/agent-runtime/extensibility/extensions";
import { ExtensionRuntime } from "@yemu/agent-runtime/extensibility/extensions/loader";
...
import { AuthStorage, type CredentialDisabledEvent } from "@yemu/model-runtime";
import * as oauthUtils from "@yemu/model-runtime/oauth";
...
describe("createAgentSession credential_disabled subscription", () => {
	...
	const makeDirs = (label: string): SessionDirs => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `yemu-credential-disabled-${label}-${Snowflake.next()}-`));
	...
	const baseOptions = (dirs: SessionDirs, authStorage: AuthStorage, extensions: ExtensionFactory[] = []) => ({
		...
		// loads the developer's real ~/.yemu models config on every construction (~100ms each,
	...
	const makeRecordingExtension = () => {
		...
		const factory: ExtensionFactory = yemu => {
			yemu.on("credential_disabled", event => {
