...
import { discoverAuthStorage } from "@yemu/agent-runtime/sdk";
...
const ENV_KEYS = [
	"YEMU_AUTH_BROKER_URL",
	"YEMU_AUTH_BROKER_TOKEN",
	"YEMU_AUTH_BROKER_SNAPSHOT_CACHE",
	"YEMU_AUTH_BROKER_SNAPSHOT_TTL_MS",
] as const;
...
describe("discoverAuthStorage auth-broker snapshot cache", () => {
	...
	test("boots from a fresh encrypted cache when the broker is down", async () => {
		...
		process.env.YEMU_AUTH_BROKER_URL = downUrl;
		process.env.YEMU_AUTH_BROKER_TOKEN = TOKEN;
		process.env.YEMU_AUTH_BROKER_SNAPSHOT_CACHE = cachePath;
		process.env.YEMU_AUTH_BROKER_SNAPSHOT_TTL_MS = "3600000";
	...
	test("boots from a fresh cache when revalidation returns a server error", async () => {
		...
		let storage: AuthStorage | undefined;
		try {
			process.env.YEMU_AUTH_BROKER_URL = url;
			process.env.YEMU_AUTH_BROKER_TOKEN = TOKEN;
			process.env.YEMU_AUTH_BROKER_SNAPSHOT_CACHE = cachePath;
			process.env.YEMU_AUTH_BROKER_SNAPSHOT_TTL_MS = "3600000";
			await writeAuthBrokerSnapshotCache({
