...
import { RawSseDebugBuffer } from "@yemu/agent-runtime/debug/raw-sse-buffer";
import { createReportBundle } from "@yemu/agent-runtime/debug/report-bundle";
...
describe("raw SSE report bundle", () => {
	it("includes captured raw SSE text and dropped-record disclosure", async () => {
		cleanupRoot = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-raw-sse-report-"));
		const xdgStateHome = path.join(cleanupRoot, "state");
		await fs.mkdir(path.join(xdgStateHome, "yemu"), { recursive: true });
		...
		expect(rawSseText).toContain(": yemu-debug-dropped records=");
