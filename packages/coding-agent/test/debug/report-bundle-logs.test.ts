...
import { getConfigRootDir, getLogsDir, removeWithRetries, setAgentDir } from "@yemu/utils";
...
describe("report bundle logs", () => {
	it("collects every same-day PID log, not only the current process", async () => {
		cleanupRoot = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-report-logs-"));
		...
		const crashedName = `yemu.${today}.4242.log`;
		const rotatedName = `${crashedName}.1`;
		const currentName = `yemu.${today}.${process.pid}.log`;
