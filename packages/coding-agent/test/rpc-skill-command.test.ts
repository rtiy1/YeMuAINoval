...
import { tryRunRpcSkillCommand } from "@yemu/agent-runtime/modes/rpc/rpc-mode";
import { type CustomMessage, SKILL_PROMPT_MESSAGE_TYPE } from "@yemu/agent-runtime/session/messages";
import { removeWithRetries, Snowflake } from "@yemu/utils";

describe("tryRunRpcSkillCommand", () => {
	test("dispatches registered /skill commands as skill prompt messages", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), `yemu-rpc-skill-${Snowflake.next()}-`));
