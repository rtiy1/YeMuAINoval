...
import { getOAuthProviders, unregisterOAuthProviders } from "@yemu/model-runtime/oauth";
...
describe("ModelRegistry runtime provider registration", () => {
	...
	beforeEach(async () => {
		tempDir = path.join(os.tmpdir(), `yemu-test-model-registry-runtime-${Snowflake.next()}`);
