...
import * as connectionManager from "@yemu/agent-runtime/ssh/connection-manager";
import { removeWithRetries } from "@yemu/utils";

async function withLooseKey<T>(run: (keyPath: string) => Promise<T>): Promise<T> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-ssh-key-"));
...
describe("buildRemoteCommand", () => {
	...
	it("still rejects directory identity paths on Windows args", async () => {
		const keyPath = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-ssh-key-"));
		try {
			await expect(
				connectionManager.buildRemoteCommand(
					{
