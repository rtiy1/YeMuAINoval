...
import { createSessionRuntime } from "@yemu/agent-runtime/autoresearch/state";
import {
	type AutoresearchStorage,
	closeAllAutoresearchStorages,
	openAutoresearchStorage,
	type SessionRow,
} from "@yemu/agent-runtime/autoresearch/storage";
import { createInitExperimentTool } from "@yemu/agent-runtime/autoresearch/tools/init-experiment";
import { createLogExperimentTool } from "@yemu/agent-runtime/autoresearch/tools/log-experiment";
import { createRunExperimentTool } from "@yemu/agent-runtime/autoresearch/tools/run-experiment";
import { createUpdateNotesTool } from "@yemu/agent-runtime/autoresearch/tools/update-notes";
import type { ASIData, LogDetails, NumericMetricMap, RunDetails } from "@yemu/agent-runtime/autoresearch/types";
import type { ExtensionAPI, ExtensionContext } from "@yemu/agent-runtime/extensibility/extensions";
...
function makeTempDir(prefix = "@yemu-autoresearch-tools-"): TempDir {
...
function createYemuHarness(initialTools: string[] = []): YemuHarness {
...
beforeAll(async () => {
	templateRepo = makeTempDir("@yemu-autoresearch-template-");
	...
	templateBranchRepo = makeTempDir("@yemu-autoresearch-template-branch-");
...
describe("init_experiment", () => {
	...
	beforeEach(() => {
		dbOverride = makeTempDir("@yemu-autoresearch-init-db-");
		process.env.YEMU_AUTORESEARCH_DB_DIR = dbOverride.path();
	});

	afterEach(async () => {
		delete process.env.YEMU_AUTORESEARCH_DB_DIR;
		closeAllAutoresearchStorages();
		await Bun.sleep(0);
...
describe("run_experiment", () => {
	...
	beforeEach(() => {
		dbOverride = makeTempDir("@yemu-autoresearch-run-db-");
		process.env.YEMU_AUTORESEARCH_DB_DIR = dbOverride.path();
	});
...
describe("log_experiment", () => {
	...
	beforeEach(() => {
		dbOverride = makeTempDir("@yemu-autoresearch-log-db-");
		process.env.YEMU_AUTORESEARCH_DB_DIR = dbOverride.path();
	});
	...
	async function setupRun(dir: string, runtime = createSessionRuntime()) {
		...
		const harness = createYemuHarness();
		const init = createInitExperimentTool({
			dashboard: dashboardStub(),
			getRuntime: () => runtime,
			yemu: harness.api,
...
describe("update_notes", () => {
	...
	beforeEach(() => {
		dbOverride = makeTempDir("@yemu-autoresearch-notes-db-");
		process.env.YEMU_AUTORESEARCH_DB_DIR = dbOverride.path();
	});
