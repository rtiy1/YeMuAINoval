...
import type { ObservableSession } from "@yemu/agent-runtime/modes/session-observer-registry";
...
function makeSubagentRegistry(sessions: ObservableSession[]) {
	...
	} as unknown as import("@yemu/agent-runtime/modes/session-observer-registry").SessionObserverRegistry;
}
...
describe("Agent hub silent-abort regression", () => {
	...
	beforeEach(async () => {
		...
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-overlay-test-"));
	});
