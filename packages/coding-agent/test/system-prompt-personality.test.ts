...
import type { Personality } from "@yemu/agent-runtime/config/settings-schema";
...
describe("system prompt personality block", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-prompt-personality-"));
		tempHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-prompt-personality-home-"));
