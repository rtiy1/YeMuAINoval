...
import type { AgentToolContext } from "@yemu/agent-core";
import { AsyncJobManager } from "@yemu/agent-runtime/async";
import { DEFAULT_BASH_INTERCEPTOR_RULES, Settings } from "@yemu/agent-runtime/config/settings";
...
import { wrapToolWithMetaNotice } from "@yemu/agent-runtime/tools/output-meta";
import { ReadTool } from "@yemu/agent-runtime/tools/read";
import * as toolTimeouts from "@yemu/agent-runtime/tools/tool-timeouts";
import { WriteTool } from "@yemu/agent-runtime/tools/write";
import { unzip } from "@yemu/agent-runtime/utils/zip";
import { $which, removeSyncWithRetries, Snowflake } from "@yemu/utils";
...
describe("Coding Agent Tools", () => {
	...
	beforeEach(() => {
		...
		originalEditVariant = Bun.env.YEMU_EDIT_VARIANT;
		Bun.env.YEMU_EDIT_VARIANT = "replace";
	...
	afterEach(() => {
		...
		if (originalEditVariant === undefined) {
			delete Bun.env.YEMU_EDIT_VARIANT;
		} else {
			Bun.env.YEMU_EDIT_VARIANT = originalEditVariant;
		}
...
`,
	...
	describe("bash tool", () => {
		...
		it("should persist environment variables between commands", async () => {
			if (process.platform === "win32" || Bun.env.YEMU_SHELL_PERSIST !== "1") {
				return;
			}

			await bashTool.execute("test-call-8-env-set", { command: "export YEMU_TEST_VAR=hello" });
			const result = await bashTool.execute("test-call-8-env-get", { command: "echo $YEMU_TEST_VAR" });
	...
	describe("search tool", () => {
		it("should include filename when searching a single file", async () => {
			...
			// YEMU_EDIT_VARIANT=replace in beforeEach disables hashlines; expect line-number mode
