...
import { runRootCommand } from "@yemu/agent-runtime/main";
...
describe("parseArgs — --max-time flag", () => {
	...
	it("reports invalid --max-time values as CLI usage errors", async () => {
		...
		expect(stderr).toContain("Run `yemu --help` for available flags.");
	...
	it("converts maxTime to an absolute session deadline", async () => {
		using tempDir = TempDir.createSync("@yemu-max-time-");
