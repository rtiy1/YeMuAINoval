...
import type { ScopedModel } from "@yemu/agent-runtime/config/model-resolver";
import { Settings } from "@yemu/agent-runtime/config/settings";
import { buildSessionOptions } from "@yemu/agent-runtime/main";
import { type CreateAgentSessionOptions, createAgentSession } from "@yemu/agent-runtime/sdk";
...
import { CURRENT_SESSION_VERSION, type SessionHeader } from "@yemu/agent-runtime/session/session-entries";
...
describe("provider prompt-cache key session affinity", () => {
	...
	it("creates an agent whose prompt-cache key can differ from provider request lineage", async () => {
		using tempDir = TempDir.createSync("@yemu-prompt-cache-sdk-");
	...
	it("initializes a full fork with child request lineage and parent prompt-cache affinity", async () => {
		using tempDir = TempDir.createSync("@yemu-prompt-cache-fork-");
	...
	it("does not auto-inherit parent prompt-cache affinity when fork startup changes request-shaping inputs", async () => {
		...
		for (const entry of cases) {
			using tempDir = TempDir.createSync(`@yemu-prompt-cache-fork-${entry.name}-`);
	...
	it("does not pre-pin parent prompt-cache affinity when a scoped model selects the startup route", async () => {
		using tempDir = TempDir.createSync("@yemu-prompt-cache-scoped-model-");
