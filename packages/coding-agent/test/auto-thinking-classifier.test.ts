...
} from "@yemu/agent-runtime/auto-thinking/classifier";
...
} from "@yemu/agent-runtime/thinking";
import type { TinyMemoryLocalModelKey } from "@yemu/agent-runtime/tiny/models";
import { tinyModelClient } from "@yemu/agent-runtime/tiny/title-client";
...
describe("auto thinking classifier helpers", () => {
	...
	async function createLocalClassifierFixture(
		...
		const tempDir = TempDir.createSync("@yemu-auto-thinking-classifier-");
	...
	it("returns undefined for reasoning models without controllable efforts (devin-agent shape)", () => {
		// Repro for upstream-change-3356 — Devin
