...
import { type Skill as CapabilitySkill, skillCapability } from "@yemu/agent-runtime/capability/skill";
import { getCapability } from "@yemu/agent-runtime/discovery";
import {
	loadSkills,
	loadSkillsFromDir,
	parseSkillInvocation,
	type Skill,
} from "@yemu/agent-runtime/extensibility/skills";
...
const DISABLE_ALL_BUILTIN_SKILLS = {
	...
	enableYemuUser: false,
	enableYemuProject: false,
...
describe("skills", () => {
	...
	describe("loadSkills with options", () => {
		...
		it("should keep user Claude skills when project .claude/skills is missing", async () => {
			const tempHomeDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-claude-home-"));
			const tempProjectDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-claude-project-"));
		...
		// canonical YeMu-native `~/.agent[s]/skills` (the `agents` provider).
		...
		it("should still load ~/.agents/skills when codex/claude/native toggles are off (#2401)", async () => {
			const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-agents-home-"));
			const tempCwd = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-agents-cwd-"));
		...
		it("respects an explicit enableAgentsUser: false (#2401)", async () => {
			const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-agents-home-off-"));
			const tempCwd = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-agents-cwd-off-"));
		...
		// MUST NOT consider the YeMu-native `enableAgentsUser`/`...Project`
		// toggles. Otherwise a user who disables Codex/Claude/YeMu to silence
		...
		it("does not re-enable third-party providers via the agents toggles (PR #2405)", async () => {
			const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-opencode-home-"));
			const tempCwd = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-opencode-cwd-"));
		...
		it("should skip skills disabled via frontmatter", async () => {
			const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-disabled-skill-"));
...
`,
		...
		it("should hide skills with disable-model-invocation frontmatter (Agent Skills spec)", async () => {
			const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-dmi-skill-"));
	...
	it("should expand ~ in customDirectories", async () => {
		const fakeHome = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-skills-home-"));
		const homedirSpy = spyOn(os, "homedir").mockReturnValue(fakeHome);
		const tempHomeSkillsDir = await fs.mkdtemp(path.join(fakeHome, ".yemu-skills-test-"));
