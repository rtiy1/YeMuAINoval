 ...
 * `process.exit()` at the top level must not terminate the host YeMu process.
 *
 * The harness intercepts the load via `withHostGuard`; this test pins that the
 * intercepted error surfaces as a per-module load failure (so YeMu keeps going)
...
import { ExtensionExitError, withHostGuard } from "@yemu/agent-runtime/extensibility/utils";
...
describe("extension/hook loader process.exit guard (#3680)", () => {
	...
	beforeEach(() => {
		project = TempDir.createSync("@yemu-exit-guard-");
	});
...
import { withHostGuard } from "@yemu/agent-runtime/extensibility/utils";
...
`);
	...
	it("converts hard exits from extension and hook factories into load errors", async () => {
		const extension = writeModule(
			"factory-exit-extension.ts",
			"export default function(yemu) { process.exit(31); }\n",
		);
		const hook = writeModule("factory-exit-hook.ts", "export default function(yemu) { process.exit(32); }\n");
		const reallyExitExtension = writeModule(
			"factory-really-exit-extension.ts",
			"export default function(yemu) { process.reallyExit(33); }\n",
		);
		const cwd = project!.path();
		const originalExit = process.exit;
	...
	it("loads sibling modules even when one of them tries to exit", async () => {
		...
		const good = writeModule(
			"good-extension.ts",
			"export default function(yemu) { yemu.registerCommand('ok', { handler: async () => {} }); }\n",
		);
		const cwd = project!.path();
