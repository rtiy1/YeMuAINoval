...
import { Agent, AgentBusyError, type AgentMessage, type AgentTool } from "@yemu/agent-core";
...
import { TtsrManager } from "@yemu/agent-runtime/export/ttsr";
import { ExtensionRuntime, loadExtensionFromFactory } from "@yemu/agent-runtime/extensibility/extensions/loader";
import { ExtensionRunner } from "@yemu/agent-runtime/extensibility/extensions/runner";
import { GoalRuntime } from "@yemu/agent-runtime/goals/runtime";
...
import { convertToLlm } from "@yemu/agent-runtime/session/messages";
...
import type { AssistantMessage, Message, ToolCall } from "@yemu/model-runtime";
...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(() => {
		...
		tempDir = path.join(os.tmpdir(), `yemu-concurrent-test-${Snowflake.next()}`);
	...
	// already processing` from yemu-rpc clients (yemu triage reminder path).
...
describe("AgentSession TTSR resume gate", () => {
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `yemu-ttsr-gate-test-${Snowflake.next()}`);
		fs.mkdirSync(tempDir, { recursive: true });
	});
	...
	it("relativizes the rule file path in the TTSR interrupt injection (no absolute leak)", async () => {
		...
		const ruleAbsPath = path.join(cwd, ".yemu", "rules", "no-unwrap.md");
