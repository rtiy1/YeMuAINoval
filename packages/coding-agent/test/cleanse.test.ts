...
import * as cleanseAgent from "@yemu/agent-runtime/cleanse/agent";
import { balanceDiagnostics } from "@yemu/agent-runtime/cleanse/balance";
import * as cleanseCheckers from "@yemu/agent-runtime/cleanse/checkers";
import { runCleanseCommand } from "@yemu/agent-runtime/cleanse/index";
import { runCleanseLoop } from "@yemu/agent-runtime/cleanse/loop";
import { parseCleanseDiagnostics } from "@yemu/agent-runtime/cleanse/parsers";
import { createCleanseProgressReporter } from "@yemu/agent-runtime/cleanse/progress";
import type {
	CleanseAgentOutcome,
	CleanseDiagnostic,
	CleanseDiagnosticReport,
} from "@yemu/agent-runtime/cleanse/types";
import { resolveCliArgv } from "@yemu/agent-runtime/cli-commands";
...
describe("cleanse diagnostics", () => {
	...
	test("discovers package test scripts only when test mode is enabled", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-cleanse-tests-"));
		try {
			await Bun.write(
	...
	test("excludes generated Bazel trees from checker discovery", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-cleanse-ignore-"));
		try {
			await Bun.write(path.join(root, "src", "index.ts"), "export const value = 1;\n");
			await Bun.write(
