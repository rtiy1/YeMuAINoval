...
import type { SourceMeta } from "@yemu/agent-runtime/capability/types";
import type { MCPServerConfig } from "@yemu/agent-runtime/mcp/types";
import { collectMcpServerNames } from "@yemu/agent-runtime/modes/controllers/mcp-command-controller";
import type { InteractiveModeContext } from "@yemu/agent-runtime/modes/types";
import { buildTuiBuiltinSlashCommands } from "@yemu/agent-runtime/slash-commands/builtin-registry";
import type { TuiSlashCommandRuntime } from "@yemu/agent-runtime/slash-commands/types";
import {
	getConfigRootDir,
	getMCPConfigPath,
	getProjectDir,
...
function restoreAgentDir(): void {
	if (originalAgentDir) {
		...
		Bun.env.YEMU_CODING_AGENT_DIR = originalAgentDir;
		return;
	}
	setAgentDir(fallbackAgentDir);
	delete process.env.YEMU_CODING_AGENT_DIR;
	delete Bun.env.YEMU_CODING_AGENT_DIR;
}
...
describe("MCP server-name autocomplete", () => {
	...
	beforeEach(async () => {
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-mcp-autocomplete-project-"));
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-mcp-autocomplete-agent-"));
