...
import { TASK_SUBAGENT_LIFECYCLE_CHANNEL } from "@yemu/agent-runtime/task";
import type { TodoPhase } from "@yemu/agent-runtime/tools/todo";
import { EventBus } from "@yemu/agent-runtime/utils/event-bus";
import type { NativeScrollbackLiveRegion } from "@yemu/tui";
...
describe("InteractiveMode todo HUD persistence", () => {
	...
	beforeEach(async () => {
		...
		tempDir = TempDir.createSync("@yemu-todo-clear-");
	});
...
describe("InteractiveMode todo HUD anchor", () => {
	...
	beforeEach(async () => {
		resetSettingsForTest();
		tempDir = TempDir.createSync("@yemu-todo-hud-");
