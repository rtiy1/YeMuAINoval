...
import { parseArgs } from "@yemu/agent-runtime/cli/args";
import { ModelRegistry, type ProviderConfigInput } from "@yemu/agent-runtime/config/model-registry";
import { getModelMatchPreferences, resolveModelScope } from "@yemu/agent-runtime/config/model-resolver";
import { Settings } from "@yemu/agent-runtime/config/settings";
import { buildSessionOptions as buildCliSessionOptions } from "@yemu/agent-runtime/main";
...
import { Effort, type FetchImpl } from "@yemu/model-runtime";
...
describe("createAgentSession deferred model pattern resolution", () => {
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `yemu-sdk-model-selection-${Snowflake.next()}`);
	...
	const providerExtension: ExtensionFactory = yemu => {
		yemu.registerProvider("runtime-provider", {
	...
	const dynamicOnlyProviderExtension: ExtensionFactory = yemu => {
		yemu.registerProvider("runtime-provider", dynamicOnlyProviderConfig);
	};
