...
import { resetSettingsForTest, Settings, type ShellMinimizerSettings } from "@yemu/agent-runtime/config/settings";
import {
	applyDirenvPreflight,
	buildMinimizerOptions,
	executeBash,
	isPersistentShellCdCommand,
} from "@yemu/agent-runtime/exec/bash-executor";
import * as direnvModule from "@yemu/agent-runtime/exec/direnv";
import { DEFAULT_MAX_BYTES } from "@yemu/agent-runtime/session/streaming-output";
import * as shellSnapshot from "@yemu/agent-runtime/utils/shell-snapshot";
import type { Shell, ShellRunResult } from "@yemu/native";
import * as yemuNatives from "@yemu/native";
...
function makeTempDir(): string {
	return fs.mkdtempSync(path.join(os.tmpdir(), "yemu-bash-exec-"));
}
...
describe("executeBash", () => {
	...
	it("passes env vars", async () => {
		const result = await executeBash("echo $YEMU_TEST_ENV", {
			cwd: tempDir,
			timeout: 5000,
			env: { YEMU_TEST_ENV: "hello" },
		});
		expect(result.output.trim()).toBe("hello");
	});

	it("applies non-interactive environment defaults", async () => {
		const result = await executeBash('echo "$GIT_TERMINAL_PROMPT:$YEMU_TEST_ENV"', {
			cwd: tempDir,
			timeout: 5000,
			env: { YEMU_TEST_ENV: "hello" },
		});
	...
	it("runs non-bash shellPath commands through the configured shell", async () => {
		...
		const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-shellpath-"));
...
`,
	...
	it("persists cd, bare cd, and cd - when shortcut commands use a non-bash user shell", async () => {
		...
		const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-cd-shellpath-"));
		const marker = path.join(shellDir, "fake-shell-ran");
		const fakeShell = path.join(shellDir, "fake-shell");
...
`,
	...
	it("uses executable SHELL for user-shell shortcut commands", async () => {
		...
		const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-env-shell-"));
...
`,
	...
	it("loads zshrc aliases for user-shell shortcut commands", async () => {
		...
		const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-zsh-shellpath-"));
		fs.writeFileSync(path.join(shellDir, ".zshrc"), "alias yemu_shell_alias='printf zsh-alias-ok\\\\n'\n");
		...
		try {
			const result = await executeBash("yemu_shell_alias", {
				cwd: tempDir,
				timeout: 5000,
	...
	it("runs fish user-shell commands without login-shell side effects", async () => {
		...
		const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-fish-shellpath-"));
		const configDir = path.join(shellDir, ".config", "fish");
		fs.mkdirSync(path.join(configDir, "conf.d"), { recursive: true });
		fs.writeFileSync(path.join(configDir, "config.fish"), "function yemu_fish_fn; echo fish-fn-ok; end\n");
		// Login-gated snippet: fires only when the spawned fish is a login shell.
		fs.writeFileSync(
			path.join(configDir, "conf.d", "yemu-login.fish"),
		...
		try {
			const result = await executeBash("yemu_fish_fn", {
				cwd: tempDir,
				timeout: 5000,
	...
	it("returns promptly and quarantines the session key when native abort cleanup stalls", async () => {
		...
		const originalRun = yemuNatives.Shell.prototype.run;
		let runCalls = 0;
		const dispatched = Promise.withResolvers<void>();
		vi.spyOn(yemuNatives.Shell.prototype, "run").mockImplementation(function (this: Shell, options, onChunk) {
			runCalls++;
			if (runCalls === 1) {
		...
		const abortSpy = vi.spyOn(yemuNatives.Shell.prototype, "abort").mockResolvedValue();
	...
	it("restores persistent sessions after native abort cleanup settles", async () => {
		...
		const dispatched = Promise.withResolvers<void>();
		vi.spyOn(yemuNatives.Shell.prototype, "run").mockImplementation((_options, onChunk) => {
			onChunk?.(null, "started\n");
			dispatched.resolve();
			return nativeResult.promise;
		});
		vi.spyOn(yemuNatives.Shell.prototype, "abort").mockResolvedValue();
		...
		await executeBash("export YEMU_AFTER_ABORT=still_persistent", {
			cwd: tempDir,
			timeout: 5000,
			sessionKey: "settled-native-abort",
		});
	...
	it("aborts the shell without aborting its native signal when the JavaScript timeout fallback wins", async () => {
		...
		vi.spyOn(yemuNatives.Shell.prototype, "run").mockImplementation((options, onChunk) => {
	...
	it("explicitly aborts an overlapping one-shot shell when timeout cleanup stalls", async () => {
		...
		vi.spyOn(yemuNatives.Shell.prototype, "run").mockImplementation(options => {
	...
	it("resets persistent session state after abort", async () => {
		...
		await executeBash("export YEMU_RESET_VAR=alive", { cwd: tempDir, timeout: 5000, sessionKey });
		const beforeAbort = await executeBash("echo $YEMU_RESET_VAR", { cwd: tempDir, timeout: 5000, sessionKey });
		...
		const afterAbort = await executeBash("echo ${YEMU_RESET_VAR:-unset}", {
			cwd: tempDir,
			timeout: 5000,
			sessionKey,
		});
	...
	it("sources snapshot env vars across session commands", async () => {
		...
		fs.writeFileSync(snapshotPath, "export YEMU_SNAPSHOT_TEST=from_snapshot\n");
		vi.spyOn(Settings.prototype, "getShellConfig").mockReturnValue({
			shell: bashPath,
			args: ["-l", "-c"],
			env: {
		...
		const result = await executeBash("echo $YEMU_SNAPSHOT_TEST", { cwd: tempDir, timeout: 5000, sessionKey });
	...
	it("sources large bash functions without base64 eval wrappers", async () => {
		...
		fs.writeFileSync(path.join(tempDir, ".bashrc"), `yemu_snapshot_large_function ()\n{\n${largeBody}\n}\n`);
		...
		expect(snapshot).toContain("yemu_snapshot_large_function");
	...
	it("waits for native timeout teardown to flush piped output", async () => {
		...
		vi.spyOn(yemuNatives.Shell.prototype, "run").mockImplementation((options, onChunk) => {
			...
			const nativeResult = Promise.withResolvers<yemuNatives.ShellRunResult>();
			realSetTimeout(() => {
