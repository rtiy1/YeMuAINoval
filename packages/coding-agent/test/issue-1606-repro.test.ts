/**
 * Regression for upstream-change-1606
 *
 ...
 * `process.execPath … __yemu_tiny_inference`, `cli.ts` dispatches that flag into
...
import { createTinyTitleSubprocess } from "@yemu/agent-runtime/tiny/title-client";
...
describe("issue #1606 — tiny model lives in an isolated subprocess", () => {
	it("ping/pongs through the spawned worker subprocess and tears it down cleanly", async () => {
		...
		const script =
			'const { smokeTestTinyTitleWorker } = await import("@yemu/agent-runtime/tiny/title-client"); await smokeTestTinyTitleWorker({ timeoutMs: 15000 });';
