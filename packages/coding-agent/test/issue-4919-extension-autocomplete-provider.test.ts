/**
 * Issue #4919: a yemu extension calling `ctx.ui.addAutocompleteProvider(...)` in its
 * `session_start` handler crashed at load under yemu — the method was absent from
 * `ExtensionUIContext`, so the call threw `TypeError: ... is not a function` and
 * (for extensions that wrap init in try/catch, e.g. @ff-labs/yemu-fff) aborted the
 * extension's entire initialization.
 *
 * These tests pin the yemu-compatible contract:
 ...
 * even when `node_modules/@yemu/agent-runtime` resolves elsewhere.
 */
...
import { type Api, Effort, type Model } from "@yemu/model-runtime";
import type { AutocompleteProvider } from "@yemu/tui";
...
/**
 * Wrap `current` the way a well-behaved yemu extension does: contribute items for
...
describe("extension autocomplete provider API (#4919)", () => {
	...
	beforeAll(async () => {
		...
		tempDir = TempDir.createSync("@yemu-ext-autocomplete-");
	...
	it("does not abort a session_start handler that registers a provider without UI", async () => {
		// Mimics @ff-labs/yemu-fff: registerAutocompleteProvider(ctx) runs first and
