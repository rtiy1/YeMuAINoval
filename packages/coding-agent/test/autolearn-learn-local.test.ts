...
import {
	...
	saveLearnedLesson,
} from "@yemu/agent-runtime/memories";
import { localBackend } from "@yemu/agent-runtime/memory-backend/local-backend";
...
Bun.env.YEMU_PYTHON_SKIP_CHECK = "1";
...
describe("learned-lesson storage (local backend)", () => {
	...
	beforeEach(async () => {
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-learned-"));
...
describe("learned-lesson read-back", () => {
	...
	beforeEach(async () => {
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-learned-read-"));
		agentDir = path.join(tmp, "agent");
	});
	afterEach(async () => {
		await removeWithRetries(tmp);
...
describe("learn tool (local backend)", () => {
	...
	beforeEach(async () => {
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-learn-local-"));
		agentDir = path.join(tmp, "agent");
		projCwd = path.join(tmp, "proj");
		learnedFile = path.join(getMemoryRoot(agentDir, projCwd), "learned.md");
	});
