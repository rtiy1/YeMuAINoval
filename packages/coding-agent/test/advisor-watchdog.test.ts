...
describe("advisor watchdog prompt discovery", () => {
	...
	it("discovers and appends WATCHDOG.md to the advisor prompt", async () => {
		const tempDir = TempDir.createSync("@yemu-advisor-watchdog-");
	...
	it("discovers user-level and native project-level watchdog files", async () => {
		...
		tempDirs.push(tempDir);
		const cwd = tempDir.join("project-root");
		const yemuDir = path.join(cwd, ".yemu");
		const userAgentDir = tempDir.join("user-agent");
		fs.mkdirSync(cwd, { recursive: true });
		fs.mkdirSync(yemuDir, { recursive: true });
		...
		fs.writeFileSync(path.join(yemuDir, "WATCHDOG.md"), nativeWatchdogContent, "utf8");
		...
		try {
			...
			// Check ordering: user-level should appear first, then native project level (.yemu/WATCHDOG.md has depth 0),
