...
import { buildSkillPromptMessage, type Skill } from "@yemu/agent-runtime/extensibility/skills";
import { removeWithRetries, Snowflake } from "@yemu/utils";

async function createSkill(body: string): Promise<{ dir: string; skill: Skill }> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), `yemu-skill-prompt-${Snowflake.next()}-`));
