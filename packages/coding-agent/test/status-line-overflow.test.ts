...
import type { StatusLineSegmentId } from "@yemu/agent-runtime/config/settings-schema";
import { StatusLineComponent } from "@yemu/agent-runtime/modes/components/status-line";
...
import { getSessionAccentAnsi, getSessionAccentHex } from "@yemu/agent-runtime/utils/session-color";
import { visibleWidth } from "@yemu/tui";
import { getProjectDir, setProjectDir } from "@yemu/utils";
...
describe("path segment truncation at varying maxLength", () => {
	...
	beforeAll(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-overflow-very-long-directory-name-for-testing-"));
...
describe("overflow: path shrinks before git is dropped", () => {
	...
	beforeAll(() => {
		...
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-overflow-a-very-long-worktree-directory-name-here-"));
		setProjectDir(tmpDir);
	});
	...
	it("shrinks a short path when maxLength exceeds actual path length", () => {
		...
		const shortDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-short-"));
	...
	it("preserves git when overflow is only 1-2 columns", () => {
		const shortDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-narrow-ovf-"));
		setProjectDir(shortDir);
		try {
...
describe("overflow: path survives before model", () => {
	it("drops the model segment before the cwd path when both cannot fit", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-statusline-overflow-"));
		...
		const yemu = renderSegment("yemu", ctx).content;
		...
		const groupWidth = (parts: string[]) =>
			...
			2;
		const width = groupWidth([yemu, model]) + 1;

		expect(groupWidth([yemu, model, minPath])).toBeGreaterThan(width);
		expect(groupWidth([yemu, minPath])).toBeLessThanOrEqual(width);
