...
} from "@yemu/agent-runtime/modes/acp/acp-agent";
import type { PlanModeState } from "@yemu/agent-runtime/plan-mode/state";
import type {
	AgentSession,
	AgentSessionEvent,
	UsageFallbackConfirmation,
} from "@yemu/agent-runtime/session/agent-session";
import { SILENT_ABORT_MARKER } from "@yemu/agent-runtime/session/messages";
import { SessionManager } from "@yemu/agent-runtime/session/session-manager";
import { DEFAULT_STT_MODEL_KEY, STT_MODEL_OPTIONS } from "@yemu/agent-runtime/stt/models";
import {
	DEFAULT_TTS_LOCAL_MODEL_KEY,
	DEFAULT_TTS_VOICE,
	TTS_LOCAL_MODELS,
	TTS_LOCAL_VOICE_OPTIONS,
} from "@yemu/agent-runtime/tts/models";
import { buildModel } from "@yemu/model-catalog/build";
import type { Model } from "@yemu/model-runtime";
import { getConfigRootDir, setAgentDir } from "@yemu/utils";
...
async function createHarness(
	...
	const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "yemu-acp-test-"));
...
describe("ACP agent", () => {
	...
	it("accepts YeMu extension methods and rejects unknown unprefixed methods", async () => {
		const harness = await createHarness();

		const result = await harness.agent.extMethod("_yemu/sessions/listAll", { limit: 2 });

		expect(Array.isArray(result.sessions)).toBe(true);
		expect(typeof result.total).toBe("number");
		await expect(harness.agent.extMethod("yemu/sessions/listAll", { limit: 2 })).rejects.toThrow(
