...
import { Agent, type AgentMessage, type StreamFn } from "@yemu/agent-core";
import * as compactionModule from "@yemu/agent-core/compaction";
...
import {
	...
	loadExtensions,
} from "@yemu/agent-runtime/extensibility/extensions";
import { SecretObfuscator } from "@yemu/agent-runtime/secrets";
...
import type { AssistantMessage, Model, ToolCall } from "@yemu/model-runtime";
...
describe("AgentSession handoff", () => {
	...
	beforeAll(async () => {
		sharedDir = TempDir.createSync("@yemu-handoff-shared-");
	...
	beforeEach(async () => {
		tempDir = TempDir.createSync("@yemu-handoff-");
	...
	it("strips hook-supplied snapcompact data when persisting context-full compaction", async () => {
		const localTempDir = TempDir.createSync("@yemu-context-full-preserve-data-");
	...
	it("strips hook-supplied snapcompact data when persisting auto context-full compaction", async () => {
		const localTempDir = TempDir.createSync("@yemu-auto-context-full-preserve-data-");
		const localSessionManager = SessionManager.inMemory(localTempDir.path());
		const firstKeptEntryId = localSessionManager.appendMessage({
			role: "user",
			content: [{ type: "text", text: "kept" }],
