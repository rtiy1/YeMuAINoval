...
import { matchesAppFollowUp } from "@yemu/agent-runtime/modes/utils/keybinding-matchers";
import { type KeybindingsConfig, setKeybindings } from "@yemu/tui";
import { __resetDirsFromEnvForTests, getAgentDir, getProfileRootDir, removeWithRetries, setProfile } from "@yemu/utils";
...
describe("KeybindingsManager.create", () => {
	...
	it("migrates legacy keybinding JSON to YAML during create", async () => {
		const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-keybindings-"));
...
`,
	...
	it("inherits default user keybindings for a named profile without a profile keybindings file (#4867)", async () => {
		const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-keybindings-profile-"));
	...
	it("merges default user keybindings when create uses the active profile with no arguments (#4867)", async () => {
		...
		const configRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-keybindings-active-profile-"));

		try {
			process.env.YEMU_CONFIG_DIR = path.relative(os.homedir(), configRootDir);
			restoreEnvValue("YEMU_CODING_AGENT_DIR", originalAgentDirEnv);
			restoreEnvValue("YEMU_PROFILE", originalYemuProfile);
			restoreEnvValue("YEMU_LEGACY_PROFILE", originalYemuLegacyProfile);
			...
			restoreEnvValue("YEMU_CONFIG_DIR", originalConfigDir);
			restoreEnvValue("YEMU_CODING_AGENT_DIR", originalAgentDirEnv);
			restoreEnvValue("YEMU_PROFILE", originalYemuProfile);
			restoreEnvValue("YEMU_LEGACY_PROFILE", originalYemuLegacyProfile);
			__resetDirsFromEnvForTests();
