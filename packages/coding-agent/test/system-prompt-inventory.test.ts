...
import { buildSystemPrompt as buildSdkSystemPrompt } from "@yemu/agent-runtime/sdk";
import {
	buildSystemPrompt,
...
describe("system prompt tool inventory", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-prompt-inv-"));
		tempHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-prompt-inv-home-"));
