...
describe("normalizeShareServerUrl", () => {
	test("strips trailing slashes and falls back to the default", () => {
		expect(normalizeShareServerUrl("https://my.yemu.sh/s/")).toBe("https://my.yemu.sh/s");
		expect(normalizeShareServerUrl("https://example.com/s///")).toBe("https://example.com/s");
		expect(normalizeShareServerUrl(undefined)).toBe("https://my.yemu.sh/s");
		expect(normalizeShareServerUrl("   ")).toBe("https://my.yemu.sh/s");
	});
});
