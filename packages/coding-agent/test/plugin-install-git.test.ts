 ...
 *
 * Strategy: spy on the six `@yemu/utils` plugin-path getters so the
 ...
 * `yemuUtils.getInstallId`), so we know namespace spying on `yemu-utils` exports
...
describe("PluginManager.install with git sources", () => {
	...
	beforeEach(async () => {
		tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-plugin-git-"));
	...
	test("installs from github: shorthand and resolves real package name from deps diff", async () => {
		...
		await Bun.write(
			pluginsPkgJson,
			JSON.stringify({ name: "yemu-plugins", private: true, dependencies: {} }, null, 2),
		);
	...
	test("refreshes Bun's cached git clone before updating an existing plugin (#5401)", async () => {
		...
		await Bun.write(
			...
			JSON.stringify({ name: "@test/yemu-package", version: "1.0.0" }, null, 2),
		);
		...
		try {
			...
			await Bun.write(
				path.join(sourceDir, "package.json"),
				JSON.stringify({ name: "@test/yemu-package", version: "2.0.0" }, null, 2),
			);
			await runCommand(["git", "add", "package.json"], sourceDir);
