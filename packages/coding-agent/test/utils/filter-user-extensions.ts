...
import { getAgentDir, getConfigRootDir, getPluginsDir } from "@yemu/utils";
...
//    or `../`-relative entry pointing somewhere else under `~/.yemu/`), plus
//    the legacy non-XDG `~/.yemu/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.yemu/agent` or `$YEMU_CODING_AGENT_DIR`)
//    Handles `YEMU_CODING_AGENT_DIR` overrides that relocate the agent dir
//    (and therefore `agent/extensions/`) out from under the config root.
// 3. `getPluginsDir()` (XDG-aware: `$XDG_DATA_HOME/yemu/plugins` or legacy)
//    Handles installed plugin extensions that live outside `~/.yemu` when
//    XDG_DATA_HOME resolves the plugins dir somewhere else.
//
// We deliberately do NOT use `pathIsWithin` from yemu-utils here: that helper
// resolves symlinks via `fs.realpathSync` on both sides, so a contributor who
// symlinks `~/.yemu/agent/extensions/foo.ts` to a dotfiles repo would see the
