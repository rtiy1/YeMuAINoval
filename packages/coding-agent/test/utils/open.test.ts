...
import { openPath } from "@yemu/agent-runtime/utils/open";
...
describe("openPath", () => {
	it("opens existing WSL mount files through wslview with a Windows path", () => {
		...
		vi.spyOn(yemuUtils, "$which").mockImplementation(command => (command === "wslview" ? "/usr/bin/wslview" : null));
	...
	it("keeps WSL URL opening on xdg-open without path conversion", () => {
		...
		vi.spyOn(yemuUtils, "$which").mockReturnValue("/usr/bin/wslview");
	...
	it("falls back to xdg-open when wslview is unavailable", () => {
		setPlatform("linux");
		process.env.WSL_DISTRO_NAME = "Ubuntu";
		vi.spyOn(yemuUtils, "$which").mockReturnValue(null);
		const spawnSyncSpy = vi.spyOn(Bun, "spawnSync");
		const spawnCalls: SpawnCall[] = [];
		spySpawn(spawnCalls);
	...
	it("logs when the opener exits non-zero so Start-Process failures are diagnosable", async () => {
		setPlatform("win32");
		vi.spyOn(fs, "existsSync").mockReturnValue(true);
		const warnSpy = vi.spyOn(yemuUtils.logger, "warn").mockImplementation((() => {}) as never);
		const failing = {
			pid: 1,
