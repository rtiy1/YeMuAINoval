...
describe("git reference directory fallback", () => {
	...
	beforeAll(async () => {
		repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-ref-fallback-"));
		...
		// There is a branch called "yemu-flash" which is packed (in packed-refs).
		// But there is also a subdirectory created inside refs/heads under the same name:
		// refs/heads/yemu-flash/... because a branch called "yemu-flash/something" exists as a loose ref.
		// Thus:
		// 1. refs/heads/yemu-flash is a directory on disk.
		// 2. packed-refs contains: <commitSha> refs/heads/yemu-flash
		// In this case, trying to read refs/heads/yemu-flash as a file throws EISDIR.
		// We want to verify that we gracefully handle this and fallback to resolving from packed-refs.

		// Let's pack the current main branch as refs/heads/yemu-flash so we have it in packed-refs
		await $`git update-ref refs/heads/yemu-flash ${commitSha}`.cwd(repoDir).quiet();
		// Also point HEAD to refs/heads/yemu-flash so we exercise HEAD -> ref -> readRef / readRefSync
		await fs.writeFile(path.join(repoDir, ".git", "HEAD"), "ref: refs/heads/yemu-flash\n");
		...
		expect(packedRefs).toContain("refs/heads/yemu-flash");

		// Delete the loose ref file for refs/heads/yemu-flash if git pack-refs didn't already delete it (it usually does).
		await removeWithRetries(path.join(repoDir, ".git", "refs", "heads", "yemu-flash"));

		// Now, create refs/heads/yemu-flash as a directory to simulate another branch like "yemu-flash/feature" existing.
		// We can just create the directory and a file inside it, or just the directory.
		await fs.mkdir(path.join(repoDir, ".git", "refs", "heads", "yemu-flash"), { recursive: true });
		await fs.writeFile(path.join(repoDir, ".git", "refs", "heads", "yemu-flash", "feature"), commitSha);
	});
	...
	test("resolves branch that has directory conflict via resolve on ref", async () => {
		...
		const resolved = await git.ref.resolve(repoDir, "refs/heads/yemu-flash");
