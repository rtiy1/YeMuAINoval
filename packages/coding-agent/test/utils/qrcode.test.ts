...
import { QrCode, type QrEcLevel, renderQrHalfBlocks } from "@yemu/agent-runtime/utils/qrcode";
...
describe("QR encoder", () => {
	...
	const vectors: ReadonlyArray<{
		...
		{
			text: "https://my.yemu.sh/#mgAYTZwEnpRQtca0CTgn-Q.gdJU",
			ecl: "M",
			mask: 4,
			version: 4,
			size: 33,
	...
	it("deterministically selects a penalty-minimizing mask when none is forced", () => {
		...
		const qr = QrCode.encodeText("https://my.yemu.sh/#demo", "M");
...
describe("renderQrHalfBlocks", () => {
	it("frames the symbol in a light quiet zone wide enough for the margin", () => {
		const qr = QrCode.encodeText("https://github.com/rtiy1/YeMuAINoval/#demo", "M");
		const margin = 3;
