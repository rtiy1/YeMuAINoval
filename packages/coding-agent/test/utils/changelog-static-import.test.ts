...
import { VERSION } from "@yemu/utils";
...
/**
 * Swap `@yemu/utils` and the changelog module's `../config` import for a
 * dependency-free stub. Both pull the native addon loader into the bundle graph, and
 * that loader resolves `yemu_natives.<platform>.node` relative to the emitted artifact,
...
function changelogUtilsStubPlugin(): BunPlugin {
	return {
		...
		setup(build) {
			build.onResolve({ filter: /^@yemu\/utils$/ }, () => ({ path: utilsStubPath }));
...
describe("bundled changelog asset path resolution", () => {
	const moduleUrl = new URL("file:///opt/yemu/dist/cli.js");

	test.each([
		["Windows drive-letter", String.raw`C:\yemu\dist\CHANGELOG.md`],
		["Windows UNC", String.raw`\\server\share\yemu\CHANGELOG.md`],
		["POSIX", "/opt/yemu/dist/CHANGELOG.md"],
...
describe("changelog static import resources", () => {
	...
	test("reads the emitted changelog asset when run outside the bundle directory", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-changelog-bundle-"));
		try {
			...
			const buildOutput = await Bun.build({
				...
				outdir: bundleDir,
				target: "bun",
				external: ["yemu-legacy-agent-modules"],
	...
	test("reads the emitted changelog asset from a compiled binary", async () => {
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-changelog-compiled-"));
		try {
