...
import { tinyWorkerEnvOverlay } from "@yemu/agent-runtime/tiny/title-client";
...
describe("tinyWorkerEnvOverlay", () => {
	it("maps non-default settings onto the worker env vars when neither is already set", () => {
		expect(tinyWorkerEnvOverlay({}, "cuda", "fp16")).toEqual({
			YEMU_TINY_DEVICE: "cuda",
			YEMU_TINY_DTYPE: "fp16",
		});
	});

	it("lets a present env var win over the persisted setting", () => {
		expect(tinyWorkerEnvOverlay({ YEMU_TINY_DEVICE: "cpu" }, "cuda", "fp16")).toEqual({ YEMU_TINY_DTYPE: "fp16" });
		expect(tinyWorkerEnvOverlay({ YEMU_TINY_DTYPE: "q8" }, "cuda", "fp16")).toEqual({ YEMU_TINY_DEVICE: "cuda" });
	});
