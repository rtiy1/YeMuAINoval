...
import { buildNonInteractiveEnv } from "@yemu/agent-runtime/exec/non-interactive-env";
...
it("filters expanded dotenv values while preserving matching launcher values", async () => {
	const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-env-"));
	try {
		await Bun.write(
			path.join(tmp, ".env"),
			[
		...
		const script = [
			...
			"	inherited: env.YEMU_TEST_INHERITED_MARKER ?? null,",
		...
		for (const bunArgs of bunArgSets) {
			const proc = Bun.spawn([process.execPath, ...bunArgs, "--no-install", "--eval", script], {
				...
				env: {
					...
					YEMU_TEST_INHERITED_MARKER: "keep-me",
					NODE_ENV: "development",
...
it("keeps an empty launcher value instead of the project dotenv value", async () => {
	const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-env-empty-"));
	try {
