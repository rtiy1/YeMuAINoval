...
} from "@yemu/agent-runtime/mcp/startup-events";
...
describe("mcp/startup-events — connection-status cross-module contract", () => {
	...
	it("sanitizes failure errors before rendering them in status text", () => {
		const homePath = `${os.homedir()}/.yemu/mcp.log`;
		...
		expect(message).not.toContain("\t");
		expect(message).toContain("broken: failed at   ~/.yemu/mcp.log");
	});

	it("sanitizes server names before rendering them in status text", () => {
		const homePath = `${os.homedir()}/.yemu`;
		const message = formatMCPConnectionStatusMessage({
		...
		expect(message).not.toContain("\n");
		expect(message).not.toContain("\t");
		expect(message).toContain("Connected: ~/.yemu/connected   server.");
		expect(message).toContain("Failed: ~/.yemu/broken server: missing command.");
		expect(message).toContain("Still connecting: ~/.yemu/pending");
	});
