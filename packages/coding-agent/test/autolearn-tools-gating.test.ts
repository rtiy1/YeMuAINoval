...
import { getManagedSkillsDir } from "@yemu/agent-runtime/autolearn/managed-skills";
import { type SettingPath, Settings } from "@yemu/agent-runtime/config/settings";
import { resetActiveSkillsForTests, type Skill, setActiveSkills } from "@yemu/agent-runtime/extensibility/skills";
import type { HindsightSessionState } from "@yemu/agent-runtime/hindsight/state";
import { createTools, type ToolSession } from "@yemu/agent-runtime/tools";
import { LearnTool } from "@yemu/agent-runtime/tools/learn";
import { ManageSkillTool } from "@yemu/agent-runtime/tools/manage-skill";
import type { YeMuMemorySessionState } from "@yemu/agent-runtime/yemu-memory/state";
...
describe("autolearn tool gating", () => {
	...
	it("offers both tools, marked essential, when enabled with a live backend", async () => {
		const tools = await createTools(makeSession({ "autolearn.enabled": true, "memory.backend": "yemu-memory" }));
	...
	it("force-includes the tools into an explicit restricted toolNames list", async () => {
		...
		const withBackend = (
			await createTools(makeSession({ "autolearn.enabled": true, "memory.backend": "yemu-memory" }), ["read"])
		).map(t => t.name);
	...
	it("excludes the tools from a subagent when not in the explicit list", async () => {
		...
		const sub = (
			await createTools(
				makeSession({ "autolearn.enabled": true, "memory.backend": "yemu-memory" }, { taskDepth: 1 }),
				["read"],
			)
		).map(t => t.name);
...
describe("manage_skill execute", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-manage-skill-"));
...
describe("learn execute", () => {
	...
	function learnSession(): ToolSession {
		...
		return makeSession(
			{ "autolearn.enabled": true, "memory.backend": "yemu-memory" },
			{ getYeMuMemorySessionState: () => fakeState as unknown as YeMuMemorySessionState },
		);
	}

	beforeEach(async () => {
		originalAgentDir = getAgentDir();
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-learn-"));
		spyOn(os, "homedir").mockReturnValue(tempHome);
		setAgentDir(path.join(tempHome, ".yemu", "agent"));
		remembered = [];
	});
	...
	it("fails the lesson and skips the skill when yemu-memory returns no id", async () => {
		const failingState = {
			sessionId: "sess-2",
			session: { sessionManager: { getCwd: () => "/tmp/work" } },
		...
		const session = makeSession(
			{ "autolearn.enabled": true, "memory.backend": "yemu-memory" },
			{ getYeMuMemorySessionState: () => failingState as unknown as YeMuMemorySessionState },
		);
		await expect(
