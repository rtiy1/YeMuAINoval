...
import { shouldSkipHistory } from "@yemu/agent-runtime/modes/controllers/input-controller";
...
describe("shouldSkipHistory — security filter for slash command history", () => {
	...
	it("skips /join with a link argument (carries 32-byte room key and write token)", () => {
		expect(shouldSkipHistory("/join yemu://share/abc123def456...")).toBe(true);
		expect(shouldSkipHistory("/join yemu:abc123def456...")).toBe(true);
	});
