...
import { BUILTIN_SLASH_COMMANDS } from "@yemu/agent-runtime/slash-commands/builtin-registry";
...
describe("/move directory completion", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-move-completion-"));
		vi.spyOn(yemuUtils, "getProjectDir").mockReturnValue(tempDir);
	});
	...
	it("completes parent directory paths", async () => {
		...
		const siblingName = `yemu-move-sibling-${path.basename(tempDir)}`;
