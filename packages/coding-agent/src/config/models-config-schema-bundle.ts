...
export const getModelsConfigSchemaBundle = once(() => {
	...
	const ProviderConfigSchema = type({
		...
		/**
		 * Streaming transport override. When set to `"yemu-native"`, yemu dispatches
		 * every model under this provider via the auth-gateway's
		 * `POST /v1/yemu/stream` endpoint instead of the per-provider SDK. The
		 * provider's `baseUrl` must point at a compatible `yemu auth-gateway`
