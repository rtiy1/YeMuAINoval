...
import { ThinkingLevel } from "@yemu/agent-core";
import { buildModel } from "@yemu/model-catalog/build";
import { modelMatchesHost } from "@yemu/model-catalog/hosts";
import { buildModelProviderPriorityRank } from "@yemu/model-catalog/identity";
import { stripThinkingVariantToken } from "@yemu/model-catalog/identity/family";
...
import { DEFAULT_MODEL_PER_PROVIDER } from "@yemu/model-catalog/provider-models";
import { resolveBareVariantAlias, resolveVariantAlias } from "@yemu/model-catalog/variant-collapse";
import type { Api, Effort, KnownProvider, Model, ModelSpec } from "@yemu/model-runtime";
import { fuzzyMatch } from "@yemu/tui";
 ...
 * offset of the role name for prefixed aliases (`@role`, `yemu/role`); the bare
...
export async function resolveModelScope(
	...
	for (const pattern of patterns) {
		...
		// Role aliases (`@smol`, `yemu/slow`) resolve to the role's single concrete
...
export function resolveCliModel(options: {
	...
	if (cliProvider && !provider) {
		return {
			...
			selector: undefined,
			warning: undefined,
			error: `Unknown provider "${cliProvider}". Run "yemu models" to see available providers/models.`,
		};
	}
		...
		if (roleSelector) {
			...
			if (configuredPatterns && configuredPatterns.length > 0) {
				return {
					...
					thinkingLevel: undefined,
					warning: resolved.warning,
					error: `Model "${trimmedModel}" not found. Run "yemu models" to see available models.`,
				};
			}
		}
	}
	...
	if (!model) {
		...
		return {
			...
			thinkingLevel: undefined,
			warning,
			error: `Model "${display}" not found. Run "yemu models" to see available models.`,
		};
	}
