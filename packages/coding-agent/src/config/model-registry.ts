...
import { isVertexExpressOpenAIUrl } from "@yemu/model-catalog/hosts";
import { readModelCache } from "@yemu/model-catalog/model-cache";
import {
	createModelManager,
	type ModelManagerOptions,
	type ModelRefreshStrategy,
} from "@yemu/model-catalog/model-manager";
import { getBundledModels, getBundledProviders } from "@yemu/model-catalog/models";
import {
...
} from "@yemu/model-catalog/provider-models";
import {
	collapseBuiltModelVariants,
	getVariantAliasSources,
	resolveVariantAlias,
} from "@yemu/model-catalog/variant-collapse";
import { registerCustomApi, unregisterCustomApis } from "@yemu/model-runtime/api-registry";
import type {
	Api,
	Context,
	Model,
...
import type { AssistantMessageEventStream } from "@yemu/model-runtime/utils/event-stream";
...
} from "@yemu/model-catalog/identity";
import type { ApiKeyResolver, FetchImpl } from "@yemu/model-runtime";
import { registerOAuthProvider, unregisterOAuthProviders } from "@yemu/model-runtime/oauth";
import type { OAuthCredentials, OAuthLoginCallbacks } from "@yemu/model-runtime/oauth/types";
import { setCodexAttestationProvider } from "@yemu/model-runtime/providers/openai-codex-responses";
import { isBunTestRuntime, isRecord, logger, wrapFetchForExtraCa } from "@yemu/utils";
...
// requests; the model-runtime provider resolves it just-in-time per request.
 ...
 *   1. `providerOverride.transport` (e.g. `yemu-native` for auth-gateway users)
 ...
 * marked `transport: yemu-native` in models.yml silently reverts to the
 ...
 * chat-completions URL instead of the gateway's `/v1/yemu/stream` (#2555).
...
export class ModelRegistry {
	...
	#loadCustomModels(): CustomModelsResult {
		...
		// or route a yemu-native gateway back into itself.
	 ...
	 *
	 * Mirrors the upstream `@yemu/agent-runtime` API surface so that
	...
	/** Resolve request authentication through the historical YeMu extension facade. */
