 ...
 * discovery lives in yemu-catalog's provider-models.
 */
...
} from "@yemu/model-catalog/provider-models/openai-compat";
import type { ModelSpec, OpenAICompat } from "@yemu/model-catalog/types";
import { type ApiKey, type FetchImpl, withAuth } from "@yemu/model-runtime";
import type { Api, Model, RemoteCompactionConfig } from "@yemu/model-runtime/types";
 ...
 * `qwen-template-false` dialect; yemu emits `preserve_thinking` inside
 ...
 * `yemu-native`, whose client appends `/v1/yemu/stream`) keeps its base URL so the
