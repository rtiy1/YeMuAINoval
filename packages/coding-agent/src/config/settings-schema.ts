import { SHAPE_VARIANT_NAMES } from "@yemu/compact";
import { THINKING_EFFORTS } from "@yemu/model-runtime";
import { DEFAULT_SHARE_URL } from "@yemu/wire";
...
export const TAB_GROUPS: Record<SettingTab, readonly string[]> = {
	...
	memory: ["General", "Auto-Learn", "YeMuMemory", "Hindsight"],
...
export const DEFAULT_BASH_INTERCEPTOR_RULES: BashInterceptorRule[] = [
	...
	},
	{
		...
		tool: "hub",
		message:
			'Use the `hub` tool (`op:"start"`) for services, watchers, and debuggers so other yemu instances can observe and control them.',
	},
	{
		pattern:
...
export const SETTINGS_SCHEMA = {
	...
	// Auth broker — credentials proxied through a remote `yemu auth-broker serve`
	// host. Hidden from the UI; populate via env vars or hand-edited config.yml.
	// Env (`YEMU_AUTH_BROKER_URL` / `YEMU_AUTH_BROKER_TOKEN`) takes precedence so
	...
	"providers.maxInFlightRequests": {
		...
		ui: {
			...
			description:
				'Maximum concurrent LLM requests per provider id (for example "openai" or "anthropic"), shared across local YeMu processes with this config root. Omitted providers are unlimited.',
		},
	},
		...
		ui: {
			...
			options: [
				...
				},
				{
					value: "project",
					label: "Per-project",
					description: "Save project role models in .yemu/config.yml; missing project roles use global defaults",
				},
			],
		},
	},
	...
	"startup.checkUpdate": {
		...
		ui: {
			...
			description: "Check for yemu updates on startup",
		},
	},
	...
	// YeMuMemory local SQLite, Hindsight remote memory, or off. The legacy
	// `memories.enabled` flag is migration input only; see config/settings.ts.
	"memory.backend": {
		type: "enum",
		values: ["off", "local", "hindsight", "yemu-memory"] as const,
		default: "off",
		ui: {
			tab: "memory",
			group: "General",
			label: "Memory Backend",
			description: "Off, local summary pipeline, YeMuMemory SQLite, or Hindsight remote memory",
			options: [
	...
	// and mint/enhance isolated managed skills under ~/.yemu/agent/managed-skills.
	...
	// YeMuMemory local SQLite memory backend.
	"yemu-memory.dbPath": {
		type: "string",
		default: undefined,
		ui: {
			...
			options: [
				{
					value: "global",
					label: "Global",
					description: "One shared YeMuMemory bank for every project",
				},
				{
					value: "per-project",
					label: "Per project",
					description: "Project-local YeMuMemory bank per cwd basename",
				},
				{
	...
	"yemu-memory.autoRecall": {
		type: "boolean",
		default: true,
		ui: {
			tab: "memory",
			group: "YeMuMemory",
			label: "YeMuMemory Auto Recall",
			description: "Recall local memories into the first turn of each session",
			condition: "yemuMemoryActive",
		},
	},
	"yemu-memory.autoRetain": {
		type: "boolean",
		default: true,
		ui: {
			tab: "memory",
			group: "YeMuMemory",
			label: "YeMuMemory Auto Retain",
			description: "Retain completed conversation turns into local YeMuMemory memory",
			condition: "yemuMemoryActive",
		},
	},
	"yemu-memory.polyphonicRecall": {
		type: "boolean",
		default: false,
		ui: {
			tab: "memory",
			group: "YeMuMemory",
			label: "YeMuMemory Polyphonic Recall",
			description: "Enable 4-voice recall (vector, graph, fact, temporal) fused with reciprocal rank fusion",
			condition: "yemuMemoryActive",
		},
	},
	"yemu-memory.enhancedRecall": {
		type: "boolean",
		default: false,
		ui: {
			tab: "memory",
			group: "YeMuMemory",
			label: "YeMuMemory Enhanced Recall",
			description: "Enable the tiered query result cache for repeated and similar recall queries",
			condition: "yemuMemoryActive",
		},
	},
	"yemu-memory.proactiveLinking": {
		type: "boolean",
		default: false,
		ui: {
			tab: "memory",
			group: "YeMuMemory",
			label: "YeMuMemory Proactive Linking",
			description:
				"Ingest new memories into the episodic graph as they are stored, linking them to related entities and memories",
			condition: "yemuMemoryActive",
		},
	},
	"yemu-memory.noEmbeddings": {
		type: "boolean",
		default: false,
		ui: {
			tab: "memory",
			group: "YeMuMemory",
			label: "YeMuMemory Disable Embeddings",
			description: "Force deterministic FTS-only recall instead of vector embeddings",
			condition: "yemuMemoryActive",
		},
	},
	"yemu-memory.embeddingModel": {
		type: "string",
		default: undefined,
		ui: {
			tab: "memory",
			group: "YeMuMemory",
			label: "YeMuMemory Embedding Model",
			description:
				"Advanced: explicit embedding model id that overrides the variant. Leave empty to use yemu-memory.embeddingVariant.",
			condition: "yemuMemoryActive",
		},
	},
	"yemu-memory.embeddingApiUrl": {
		type: "string",
		default: undefined,
		ui: {
			tab: "memory",
			group: "YeMuMemory",
			label: "YeMuMemory Embedding API URL",
			description: "Optional OpenAI-compatible embedding endpoint passed to YeMuMemory",
			condition: "yemuMemoryActive",
		},
	},
	"yemu-memory.embeddingApiKey": {
		type: "string",
		credential: true,
		default: undefined,
		ui: {
			tab: "memory",
			group: "YeMuMemory",
			label: "YeMuMemory Embedding API Key",
			description: "Optional embedding API key passed to YeMuMemory",
			condition: "yemuMemoryActive",
		},
	},
	"yemu-memory.llmMode": {
		...
		ui: {
			tab: "memory",
			group: "YeMuMemory",
			label: "YeMuMemory LLM Mode",
			description:
				"Use no LLM, the online tiny model (the TINY role from /models, else @smol), or a remote OpenAI-compatible endpoint",
			condition: "yemuMemoryActive",
			options: [
				{ value: "none", label: "None", description: "Disable YeMuMemory LLM-backed extraction" },
				{
					value: "smol",
					label: "Online (tiny)",
					description: "Use the online tiny model (the TINY role from /models, else @smol)",
				},
				{ value: "remote", label: "Remote", description: "Use the YeMuMemory remote LLM settings below" },
			],
		},
	},
	"yemu-memory.llmBaseUrl": {
		type: "string",
		default: undefined,
		ui: {
			tab: "memory",
			group: "YeMuMemory",
			label: "YeMuMemory LLM Base URL",
			description: "Optional OpenAI-compatible LLM endpoint for YeMuMemory remote mode",
			condition: "yemuMemoryActive",
		},
	},
	"yemu-memory.llmApiKey": {
		type: "string",
		credential: true,
		default: undefined,
		ui: {
			tab: "memory",
			group: "YeMuMemory",
			label: "YeMuMemory LLM API Key",
			description: "Optional LLM API key for YeMuMemory remote mode",
			condition: "yemuMemoryActive",
		},
	},
	"yemu-memory.llmModel": {
		type: "string",
		default: undefined,
		ui: {
			tab: "memory",
			group: "YeMuMemory",
			label: "YeMuMemory LLM Model",
			description: "Optional LLM model name for YeMuMemory remote mode",
			condition: "yemuMemoryActive",
		},
	},
	"yemu-memory.retainEveryNTurns": { type: "number", default: 4 },
	"yemu-memory.recallLimit": { type: "number", default: 8 },
	"yemu-memory.recallContextTurns": { type: "number", default: 3 },
	"yemu-memory.recallMaxQueryChars": { type: "number", default: 4000 },
	"yemu-memory.injectionTokenLimit": { type: "number", default: 5000 },
	"yemu-memory.debug": { type: "boolean", default: false },
	...
	"hindsight.retainContext": { type: "string", default: "yemu" },
	...
	"github.cache.enabled": {
		...
		ui: {
			...
			description: "Cache rendered issue/PR view output in ~/.yemu/cache/github-cache.db so repeated reads are free",
		},
	},
	...
	"browser.cmux": {
		...
		ui: {
			...
			label: "cmux Browser",
			description:
				"Use cmux WKWebView surfaces for browser automation when a cmux socket is available. Set YEMU_BROWSER_CMUX=0 or YEMU_BROWSER_CMUX=1 to override.",
		},
	},
	...
	"worktree.base": {
		...
		ui: {
			...
			description:
				"Base directory for agent-managed worktrees — task-isolation copies, `github` PR checkouts, and `yemu worktree` cleanup all live here. Unset uses ~/.yemu/wt. Must be an absolute or ~-relative path; relative paths are ignored. The YEMU_WORKTREE_DIR env var overrides this.",
		},
	},
	...
	"skills.enableYemuUser": { type: "boolean", default: true },

	"skills.enableYemuProject": { type: "boolean", default: true },
	...
	"providers.tinyModelDevice": {
		...
		ui: {
			...
			description:
				"ONNX execution provider for local tiny models (titles + memory). Default uses CPU-only inference. The YEMU_TINY_DEVICE env var overrides this.",
	...
	"providers.tinyModelDtype": {
		...
		ui: {
			...
			description:
				"ONNX quantization/precision for local tiny models. Default uses each model's shipped dtype (q4); lower precision is faster, higher is more faithful. The YEMU_TINY_DTYPE env var overrides this.",
	...
	"providers.memoryModel": {
		...
		ui: {
			...
			label: "Memory Model",
			description:
				"YeMuMemory LLM for fact extraction + consolidation: online (the TINY role from /models, else smol/remote) by default, or a local on-device model",
			condition: "yemuMemoryActive",
	...
	"providers.streamFirstEventTimeoutSeconds": {
		...
		ui: {
			...
			options: [
				{ value: "-1", label: "Auto", description: "Use provider defaults and YEMU_* timeout env vars" },
	...
	"dev.autoqaPush.endpoint": {
		type: "string",
		default: "https://qa.yemu.sh/v1/grievances" as const,
		ui: {
			tab: "tools",
			group: "Developer",
			label: "Auto QA Push Endpoint",
			description: "Full URL receiving Auto QA JSON reports (default https://qa.yemu.sh/v1/grievances)",
		},
	},
...
export interface SkillsSettings {
	...
	enableYemuUser?: boolean;
	enableYemuProject?: boolean;
