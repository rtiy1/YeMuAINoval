...
import { configureProviderMaxInFlightRequests } from "@yemu/model-runtime/stream";
import {
	getAgentDbPath,
	getAgentDir,
...
import * as procmgr from "@yemu/utils/procmgr";
...
export class Settings {
	...
	#project: RawSettings = {};
	/** Last successfully loaded native .yemu/config.yml contents. */
	...
	private constructor(options: SettingsOptions = {}) {
		...
		const configFiles = process.env.YEMU_CONFIG_FILES?.split(path.delimiter).filter(Boolean) ?? [];
	...
	async #loadProjectSettings(): Promise<RawSettings> {
		...
		const projectConfigPath = path.join(this.#cwd, ".yemu", "config.yml");
	...
	#migrateRawSettings(raw: RawSettings): RawSettings {
		...
		// task.isolation.mode: legacy values from before the yemu-iso PAL refactor.
		...
		// Rename the legacy local `mnemosyne` memory backend to `yemu-memory`.
		// - `memory.backend: "mnemosyne"` now selects the renamed backend.
		// - the top-level `mnemosyne` settings object becomes `yemu-memory`.
		// Idempotent: skips the object move once `yemu-memory` is materialised.
		if (memoryBackendObj && memoryBackendObj.backend === "mnemosyne") {
			memoryBackendObj.backend = "yemu-memory";
		}
		if ("mnemosyne" in raw && !("yemu-memory" in raw)) {
			raw["yemu-memory"] = raw.mnemosyne;
