...
import { getAutoresearchDbPath, getAutoresearchProjectDir, logger } from "@yemu/utils";
 ...
 *
 * Used to key per-project autoresearch state under `~/.yemu/autoresearch/`.
...
async function resolveAutoresearchPaths(cwd: string): Promise<{ dbPath: string; projectDir: string }> {
	const override = process.env.YEMU_AUTORESEARCH_DB_DIR;
