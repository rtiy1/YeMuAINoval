...
import type { AutocompleteItem } from "@yemu/tui";
import { logger, prompt } from "@yemu/utils";
...
export const createAutoresearchExtension: ExtensionFactory = api => {
	...
	api.registerTool(createInitExperimentTool({ dashboard, getRuntime, yemu: api }));
	api.registerTool(createRunExperimentTool({ dashboard, getRuntime, yemu: api }));
	api.registerTool(createLogExperimentTool({ dashboard, getRuntime, yemu: api }));
	api.registerTool(createUpdateNotesTool({ dashboard, getRuntime, yemu: api }));
