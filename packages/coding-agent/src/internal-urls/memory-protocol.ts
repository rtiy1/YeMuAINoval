...
import { getAgentDir, isEnoent } from "@yemu/utils";
import { getMemoryRoot } from "../memories";
import { AgentRegistry } from "../registry/agent-registry";
import { isMarkdownPath } from "../utils/lang-from-path";
import {
	getYeMuMemorySessionState,
	type YeMuMemoryScopedMemoryHit,
	type YeMuMemorySessionState,
} from "../yemu-memory/state";
...
/**
 * Snapshot of live yemu-memory session states, deduplicated. A yemu-memory backend
 ...
 */
function yemuMemorySessionStatesFromRegistry(): YeMuMemorySessionState[] {
	const seen = new Set<unknown>();
	const states: YeMuMemorySessionState[] = [];
	for (const ref of AgentRegistry.global().list()) {
...
/**
 * Look up a yemu-memory memory row by id across every live session's scoped banks.
 * First hit wins; returns `null` when the id is not stored anywhere in scope.
 */
function tryResolveYeMuMemoryMemory(id: string): YeMuMemoryScopedMemoryHit | null {
	for (const state of yemuMemorySessionStatesFromRegistry()) {
...
/**
 * Render a yemu-memory memory row as text/markdown with a small YAML-front-matter
 ...
 */
function renderYeMuMemoryMemory(url: InternalUrl, hit: YeMuMemoryScopedMemoryHit): InternalResource {
...
export class MemoryProtocolHandler implements ProtocolHandler {
	...
	async resolve(url: InternalUrl, context?: ResolveContext): Promise<InternalResource> {
		...
		// YeMuMemory rows live in SQLite banks per session, keyed by memory id.
		// Any host other than the file-backed `root` namespace is treated as a
		// yemu-memory memory id lookup. This is the read counterpart to
		// `memory_edit update` and lets agents inspect the full content of a
		// clipped recall preview before overwriting it (issue #4443).
		if (namespace !== MEMORY_NAMESPACE) {
			const yemuMemoryStates = yemuMemorySessionStatesFromRegistry();
			if (yemuMemoryStates.length === 0) {
				throw new Error(
					`Unknown memory namespace: ${namespace}. Supported: ${MEMORY_NAMESPACE} (file-backed memory summary), or a yemu-memory memory id when memory.backend=yemu-memory is active.`,
				);
			}
			const hit = tryResolveYeMuMemoryMemory(namespace);
			if (hit) return renderYeMuMemoryMemory(url, hit);
			throw new Error(
				`YeMuMemory memory ${namespace} not found in any scoped bank. Use \`recall\` to list available ids.`,
			);
		}
	...
	async complete(_query?: string, context?: ResolveContext): Promise<UrlCompletion[]> {
		...
		if (yemuMemorySessionStatesFromRegistry().length > 0) {
			completions.push({
				value: "<memory-id>",
				description: "Full yemu-memory memory by id (from recall)",
			});
		}
		return completions;
	}
