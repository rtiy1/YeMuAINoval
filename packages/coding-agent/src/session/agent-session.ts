...
import {
	...
	type ShakeConfig,
} from "@yemu/agent-core/compaction";
import type { Clipboard, InMemorySnapshotStore } from "@yemu/hashline";
import { modelsAreEqual } from "@yemu/model-catalog/models";
import type {
	AssistantMessage,
...
import { type Effort, streamSimple } from "@yemu/model-runtime";
import * as AIError from "@yemu/model-runtime/error";
import { resetOpenAICodexHistoryAfterCompaction } from "@yemu/model-runtime/providers/openai-codex-responses";
import { toolWireSchema } from "@yemu/model-runtime/utils/schema";
import { MacOSPowerAssertion } from "@yemu/native";
import {
	escapeXmlText,
	formatDuration,
	getAgentDbPath,
...
import { shutdownYeMuMemoryEmbedClient } from "../yemu-memory/embed-client";
import {
...
export class AgentSession {
	...
	#acquirePowerAssertion(): void {
		...
		try {
			this.#powerAssertion = MacOSPowerAssertion.start({
				reason: "YeMu Agent agent session",
				idle: true,
	...
	constructor(config: AgentSessionConfig) {
		...
		const memoryHost: SessionMemoryHost = {
			...
			getYeMuMemorySessionState: () => this.getYeMuMemorySessionState(),
			takeYeMuMemorySessionState: () => setYeMuMemorySessionState(this, undefined),
	...
	getYeMuMemorySessionState(): YeMuMemorySessionState | undefined {
		return getYeMuMemorySessionState(this);
	}
	 ...
	 * `device_id` is derived from both the persistent yemu install id and that
	...
	async #disposeYeMuMemory(
		...
		try {
			...
			await shutdownYeMuMemoryEmbedClient();
		}
	}
	...
	async #doDispose(options: AgentSessionDisposeOptions = {}): Promise<void> {
		...
		const yemuMemoryState = setYeMuMemorySessionState(this, undefined);
		...
		const results = await Promise.allSettled([
			...
			this.#disposeYeMuMemory(yemuMemoryState, options.yemuMemoryConsolidateTimeoutMs),
		]);
	 ...
	 * - Handles extension commands (registered via yemu.registerCommand) immediately, even during streaming
	...
	#createCommandContext(): ExtensionCommandContext {
		...
		return {
			...
			shutdown: () => {
				...
				// YeMu-owned Chromium alive (#5643).
	...
	async dumpLlmRequestToTmpDir(): Promise<string | undefined> {
		...
		const filePath = path.join(os.tmpdir(), `yemu-llm-request-${Snowflake.next()}.json`);
