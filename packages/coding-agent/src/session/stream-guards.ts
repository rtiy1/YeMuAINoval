...
import type { Agent, AgentEvent, AgentMessage, AgentTurnEndContext } from "@yemu/agent-core";
import type { AssistantMessage, AssistantMessageEvent, Model, ToolCall } from "@yemu/model-runtime";
import { GeminiHeaderRunDetector, isGeminiThinkingModel } from "@yemu/model-runtime/utils/thinking-loop";
import { type RepeatedToolCallDetection, ToolCallLoopGuard } from "@yemu/model-runtime/utils/tool-call-loop-guard";
import { isEnoent, logger, prompt } from "@yemu/utils";
...
export class LoopGuards {
	...
	#geminiHeaderGuardActive(): boolean {
		...
		return (
			process.env.YEMU_NO_THINKING_LOOP_GUARD !== "1" &&
