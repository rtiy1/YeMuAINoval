...
import { AsyncDrain, getAgentDbPath, getStatsDbPath, isRecord, logger } from "@yemu/utils";
...
/** Row shape read from an `yemu stats` messages table during backfill. */
 ...
 * Delegates auth credential operations to AuthCredentialStore from @yemu/model-runtime.
...
`);
	 ...
	 * `yemu stats` database (`~/.yemu/stats.db`) into model_perf. Fire-and-forget:
	 ...
	 * A missing stats.db leaves the marker unset so a later `yemu stats` run
	...
	/**
	 * Imports recent measurable request rows from an `yemu stats` database
...
ON CONFLICT(model_key) DO UPDATE SET
	 ...
	 * the lower-level model-runtime abstraction (e.g. `findAnthropicAuth(store)`) can
