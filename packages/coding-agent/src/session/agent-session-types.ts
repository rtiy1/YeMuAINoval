import type { Agent, AgentMessage, AgentTool, AgentToolContext, StreamFn, ThinkingLevel } from "@yemu/agent-core";
import type {
	Context,
	Effort,
	ImageContent,
...
import type { postmortem } from "@yemu/utils";
...
/** Maximum time the interactive shutdown path waits for YeMuMemory consolidation. */
...
export interface AgentSessionDisposeOptions {
	yemuMemoryConsolidateTimeoutMs?: number;
	/**
...
export interface AgentSessionConfig {
	...
	/**
	 * Build the `replace`-mode `edit` a Cursor `yemu_edit` frame needs, against the
