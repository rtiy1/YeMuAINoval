/**
 * Resolve auth-broker connection configuration for the local yemu client.
 *
 * This is a thin coding-agent wrapper around the shared resolver in
 * `@yemu/model-runtime/auth-broker/discover` that preserves the process-lifetime
 ...
 *   2. `auth.broker.url` / `auth.broker.token` in `~/.yemu/agent/config.yml`
 *      (hidden from the settings UI; `!command` resolution supported).
 *   3. Token file `~/.yemu/auth-broker.token` (paired with URL from env or config).
 *
...
} from "@yemu/model-runtime/auth-broker/discover";
 ...
 * that flip `YEMU_AUTH_BROKER_*` between cases still observe the change, while
...
export function resolveAuthBrokerConfig(): Promise<AuthBrokerClientConfig | null> {
	const key = `${process.env.YEMU_AUTH_BROKER_URL ?? ""}\u0000${process.env.YEMU_AUTH_BROKER_TOKEN ?? ""}\u0000${getAgentDir()}`;
 ...
 * model-runtime so the CLI, subagents, and the catalog generator all see the same
 * credentials.
 *
