...
import { isUsageLimitOutcome, resolveModelServiceTier, streamSimple } from "@yemu/model-runtime";
...
export interface SessionAdvisorsOptions {
	...
	tools?: AgentTool[];
	/**
	 * Build a `grep` honoring a Cursor `yemu_grep` frame's own context width and
	...
	/**
	 * Build the `replace`-mode `edit` a Cursor `yemu_edit` frame needs. The
...
export class SessionAdvisors {
	...
	#buildAdvisorRuntime(seedToCurrent = false): boolean {
		...
		for (const descriptor of descriptors) {
			...
			// `yemu_edit` speaks `replace`'s `old_text`/`new_text` schema, which the
