import { logger, toError } from "@yemu/utils";
import {
...
export interface RedisSessionStorageOptions {
	...
	/**
	 * Key prefix applied to every Redis key this storage owns. Default `yemu:sessions:`.
...
const DEFAULT_PREFIX = "yemu:sessions:";
const DEFAULT_SCAN_COUNT = 500;

const WRITE_FULL_SCRIPT = `-- YEMU_WRITE_FULL
...
const APPEND_SCRIPT = `-- YEMU_APPEND
local size = redis.call("APPEND", KEYS[1], ARGV[1])
redis.call("HSET", KEYS[2], ARGV[2], ARGV[3])
return size`;

const UPDATE_TITLE_SCRIPT = `-- YEMU_UPDATE_TITLE
