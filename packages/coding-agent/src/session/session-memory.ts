...
import type { Agent, AgentTool } from "@yemu/agent-core";
...
export interface SessionMemoryHost {
	...
	takeYeMuMemorySessionState(): YeMuMemorySessionState | undefined;
...
export class SessionMemory {
	...
	rekeyForCurrentSessionId(): void {
		...
		this.#rekeyYeMuMemoryMemoryForCurrentSessionId();
	}
	...
	#rekeyYeMuMemoryMemoryForCurrentSessionId(): void {
		if (this.#host.settings.get("memory.backend") !== "yemu-memory") return;
		const sid = this.#host.agent.sessionId;
		if (!sid) return;
		this.#host.getYeMuMemorySessionState()?.setSessionId(sid);
	}
	...
	#resetYeMuMemoryConversationTrackingIfYeMuMemory(): boolean {
		if (this.#host.settings.get("memory.backend") !== "yemu-memory") return false;
		const state = this.#host.getYeMuMemorySessionState();
		if (!state || state.aliasOf) return false;
		state.resetConversationTracking();
		return true;
	}
	...
	async resetContextForNewTranscript(): Promise<void> {
		...
		const resetYeMuMemory = this.#resetYeMuMemoryConversationTrackingIfYeMuMemory();
		if (hadPromotedMemoryPrompt) {
			this.#host.setBaseSystemPrompt(this.#baseSystemPromptBeforeMemoryPromotion!);
			this.#baseSystemPromptBeforeMemoryPromotion = undefined;
		}
		if (resetHindsight || resetYeMuMemory || hadPromotedMemoryPrompt) {
	...
	async #disposeMemoryBackendState(consolidateYeMuMemory = true): Promise<void> {
		this.cancelLocalMemoryStartup();
		...
		const yemuMemory = this.#host.takeYeMuMemorySessionState();
		if (yemuMemory) {
			try {
				await yemuMemory.dispose({ consolidate: consolidateYeMuMemory });
			} catch (error) {
				logger.warn("Memory lifecycle: YeMuMemory dispose failed", { error: String(error) });
			}
		}
	}
