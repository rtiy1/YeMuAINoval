...
import type { OAuthCallbackFlowOptions } from "@yemu/model-runtime/oauth/callback-server";
import { OAuthCallbackFlow } from "@yemu/model-runtime/oauth/callback-server";
import type { OAuthController, OAuthCredentials } from "@yemu/model-runtime/oauth/types";
import type { FetchImpl } from "@yemu/model-runtime/types";
import { getActiveProfile } from "@yemu/utils/dirs";
import type { OAuthCredential } from "../session/auth-storage";

/** Credential-id prefix for YeMu-managed MCP OAuth credentials keyed by profile and server URL. */
...
/**
 * Deterministic credential id for an MCP server URL scoped to an YeMu profile.
 *
...
/** Whether a credential id was minted by YeMu's MCP OAuth flows (either era). */
 ...
 *
 * Plane is stricter for YeMu-synthesized fallback resources (e.g. using the
...
export class MCPOAuthFlow extends OAuthCallbackFlow {
	...
	async generateAuthUrl(state: string, redirectUri: string): Promise<{ url: string; instructions?: string }> {
		...
		if (existingResource) {
			...
			// provider-authored, not YeMu's server-URL fallback. Preserve same-host
