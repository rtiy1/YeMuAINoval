...
import { isDefinitiveOAuthFailure, type TSchema } from "@yemu/model-runtime";
...
export class MCPManager {
	 ...
	 * injecting a steer via `yemu.sendMessage`).
	 *
	...
	async connectServers(
		...
		if (connectionTasks.length > 0) {
			...
			// full 30 s `YEMU_MCP_TIMEOUT_MS`). Leave them in flight — the background
