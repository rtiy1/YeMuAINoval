...
const SIXEL_PLACEHOLDER_PREFIX = "__YEMU_SIXEL_SEQUENCE_";

/**
 * Returns whether SIXEL passthrough is explicitly enabled.
 *
 * Both gates must be enabled to preserve SIXEL control sequences:
 * - YEMU_FORCE_IMAGE_PROTOCOL=sixel
 * - YEMU_ALLOW_SIXEL_PASSTHROUGH=1
 */
export function isSixelPassthroughEnabled(): boolean {
	const forcedProtocol = $env.YEMU_FORCE_IMAGE_PROTOCOL?.trim().toLowerCase();
	return forcedProtocol === "sixel" && $flag("YEMU_ALLOW_SIXEL_PASSTHROUGH");
}
...
export function sanitizeWithOptionalSixelPassthrough(text: string, sanitize: (text: string) => string): string {
	...
	return sanitized.replace(/__YEMU_SIXEL_SEQUENCE_(\d+)__/gu, (_, indexText: string) => {
