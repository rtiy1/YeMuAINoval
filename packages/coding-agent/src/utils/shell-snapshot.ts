...
echo "# Shell snapshot - generated by yemu agent" >| "$SNAPSHOT_FILE"
...
${fnEnvHelper}
printf '%s\\n' "$__yemu_funcs" | __yemu_emit_referenced_exports >> "$SNAPSHOT_FILE"
unset -f __yemu_sq_quote __yemu_emit_export_for __yemu_emit_referenced_exports 2>/dev/null
unset __yemu_funcs __yemu_qbuf __yemu_qout __yemu_sq __yemu_xv __yemu_name 2>/dev/null
...
export async function getOrCreateSnapshot(
	...
	const snapshotDir = path.join(os.tmpdir(), "yemu-shell-snapshots");
