...
import { getLastChangelogVersionPath, isEnoent, logger } from "@yemu/utils";
...
export function formatStartupChangelogSummary(selection: StartupChangelogSelection): string {
	...
	if (!latestVersion || selection.selectedEntries === 0) {
		return "Updated yemu. Use /changelog for recent changes.";
	}
...
/**
 * Parse changelog entries from yemu's package asset when available, falling back
...
/**
 * Parse an yemu changelog marker version into comparable parts.
 */
...
/**
 * Last yemu version whose changelog the user has seen. Stored as a plain-text
 * marker file (`~/.yemu/agent/last-changelog-version`) rather than in
