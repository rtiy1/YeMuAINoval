...
import type { ImageContent, Model } from "@yemu/model-runtime";
import { formatBytes, readImageMetadata, SUPPORTED_IMAGE_MIME_TYPES } from "@yemu/utils";
 ...
 * to PNG/JPEG instead — the automatic equivalent of `YEMU_NO_WEBP=1`.
 */
 ...
 * `YEMU_NO_WEBP` env fallback in {@link resizeImage} still applies. Feed straight
...
export interface LoadImageInputOptions {
	...
	/** Force non-WebP output (e.g. for Ollama). Leave unset to honor `YEMU_NO_WEBP`. */
	excludeWebP?: boolean;
}
