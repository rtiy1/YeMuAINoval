...
#
# `__yemu_emit_referenced_exports` reads captured function bodies from stdin,
...
__yemu_sq_quote() {
	...
	__yemu_qout=
	__yemu_sq=\'
	while case "$__yemu_qbuf" in *$__yemu_sq*) true ;; *) false ;; esac; do
		__yemu_qout=$__yemu_qout${__yemu_qbuf%%"$__yemu_sq"*}"'\\''"
		__yemu_qbuf=${__yemu_qbuf#*"$__yemu_sq"}
	done
	__yemu_qout=$__yemu_qout$__yemu_qbuf
	printf "'%s'" "$__yemu_qout"
}
...
__yemu_emit_export_for() {
	...
	__yemu_xv=
	eval "__yemu_xv=\"\${$1}\"" 2>/dev/null || return
	printf 'export %s=%s\n' "$1" "$(__yemu_sq_quote "$__yemu_xv")"
}

# Read function bodies on stdin, emit dedup'd export lines on stdout.
__yemu_emit_referenced_exports() {
	grep -oE '\$\{?[A-Za-z_][A-Za-z0-9_]*' \
		| sed -E 's/^\$\{?//' \
		| sort -u \
		| while IFS= read -r __yemu_name; do
			__yemu_emit_export_for "$__yemu_name"
		done
}
