...
import {
	...
	yemuReadPath,
	yemuTimeout,
} from "@yemu/model-runtime/providers/cursor/exec-modern";
import { sanitizeText } from "@yemu/utils";
...
interface CursorExecBridgeOptions {
	...
	/**
	 * The `replace`-mode `edit` instance `yemu_edit` must run, when the session
	 * granted `edit` at all.
	 *
	 * `YemuEditExecArgs` is that mode's schema verbatim, and the session's own
	 ...
	 *
	 * The modern `yemu_grep` frame carries both, and the shared `grep` instance
...
export class CursorExecHandlers implements ICursorExecHandlers {
	 ...
	 * `offset`/`limit`, exactly as `yemu_read` does. Dropping them returns the
	...
	async read(args: Parameters<NonNullable<ICursorExecHandlers["read"]>>[0]) {
		...
		const composed = yemuReadPath(args.path, args.offset, args.limit);
	...
	/**
	 * Modern Cursor CLI YeMu tool frames (`ExecServerMessage` 45-51).
	 *
	 ...
	 * against the reference `LocalYemuReadExecutor`), which is exactly the local
	 ...
	 */
	async yemuRead(call: Parameters<NonNullable<ICursorExecHandlers["yemuRead"]>>[0]) {
		const { path: readPath, offset, limit } = call.args;
		const composed = yemuReadPath(readPath, offset, limit);
	...
	async yemuBash(call: Parameters<NonNullable<ICursorExecHandlers["yemuBash"]>>[0]) {
		return await executeTool(this.options, "bash", call.toolCallId, {
			command: call.args.command,
			timeout: yemuTimeout(call.args.timeout),
		});
	}

	/**
	 * `YemuEditExecArgs` is the local `edit` tool's replace mode verbatim: a path
	 ...
	 */
	async yemuEdit(call: Parameters<NonNullable<ICursorExecHandlers["yemuEdit"]>>[0]) {
	...
	async yemuWrite(call: Parameters<NonNullable<ICursorExecHandlers["yemuWrite"]>>[0]) {
	 ...
	 * so the pattern is escaped on the way in (same translation the legacy yemu
	 * shim does).
	 *
	 ...
	 */
	async yemuGrep(call: Parameters<NonNullable<ICursorExecHandlers["yemuGrep"]>>[0]) {
		const { pattern, path, glob, ignoreCase, literal, context, limit } = call.args;
		const scoped =
			context !== undefined || limit !== undefined
				? this.options.createGrepTool?.({ context, totalMatchLimit: yemuLimit(limit) })
				: undefined;
		...
		return await executeTool(
			...
			call.toolCallId,
			{
				pattern: literal === true ? yemuEscapeRegexLiteral(pattern) : pattern,
				path: glob ? yemuJoinPath(path, glob) : path || ".",
	...
	/**
	 * `yemu_find` is a filename search, which is the local `glob` tool — not
	 ...
	 */
	async yemuFind(call: Parameters<NonNullable<ICursorExecHandlers["yemuFind"]>>[0]) {
		const { pattern, path, limit } = call.args;
		return await executeTool(this.options, "glob", call.toolCallId, {
			path: yemuJoinPath(path, pattern),
			limit: yemuLimit(limit),
		});
	}

	/**
	 * Redirected to `read`, which lists directories — same as the legacy `ls`.
	 * The frame's entry `limit` is not mapped; see {@link yemuLsPath}.
	 */
	async yemuLs(call: Parameters<NonNullable<ICursorExecHandlers["yemuLs"]>>[0]) {
		return await executeTool(this.options, "read", call.toolCallId, { path: yemuLsPath(call.args.path) });
	}
