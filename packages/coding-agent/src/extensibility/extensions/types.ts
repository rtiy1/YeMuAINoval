...
import type { CompactionResult } from "@yemu/agent-core/compaction";
import type {
	Api,
...
import type { AutocompleteItem, AutocompleteProvider, Component, EditorTheme, KeyId, TUI } from "@yemu/tui";
import type { logger as YemuLogger } from "@yemu/utils";
...
import type * as YemuCodingAgent from "../../index";
...
/** Wrap the current autocomplete provider with additional behavior (yemu-compatible). */
...
export interface ExtensionUIContext {
	 ...
	 * (yemu-compatible). Interactive mode rebuilds the editor's provider through
 ...
 * via `yemu.sendMessage(..., { deliverAs })` or `yemu.sendUserMessage(...)`.
 */
...
export interface ExtensionAPI {
	...
	/** Injected agent-runtime exports for accessing SDK utilities */
	runtime: typeof YemuCodingAgent;
	 ...
	 * yemu.registerProvider("google-vertex-claude", {
	 ...
	 * yemu.registerProvider("anthropic", {
...
/** Configuration for registering a provider via yemu.registerProvider(). */
...
export type ExtensionFactory = (yemu: ExtensionAPI) => void | Promise<void>;
