...
import type { CredentialDisabledEvent, ImageContent, Model, ProviderResponseMetadata } from "@yemu/model-runtime";
 ...
 * `yemu.setModel()` and then reading `ctx.model` would see a stale model.
...
export class ExtensionRunner {
	...
	initialize(
		...
		// spread adds the `type` discriminator — `event` is the model-runtime shape (no `type`).
