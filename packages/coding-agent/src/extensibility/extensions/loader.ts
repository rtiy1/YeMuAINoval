...
import type { ThinkingLevel } from "@yemu/agent-core";
import type {
	ImageContent,
	Model,
	ServiceTier,
...
import type { KeyId } from "@yemu/tui";
import { hasFsCode, isEacces, isEnoent, logger } from "@yemu/utils";
...
import * as YemuCodingAgent from "../../index";
import type { CustomMessagePayload } from "../../session/messages";
import { EventBus } from "../../utils/event-bus";
import { installLegacyAgentSpecifierShim, loadLegacyAgentModule } from "../plugins/legacy-agent-compat";
...
class ConcreteExtensionAPI implements ExtensionAPI, IExtensionRuntime {
	...
	constructor(
		public readonly runtime: typeof YemuCodingAgent,
		private readonly extension: Extension,
		private readonly hostRuntime: IExtensionRuntime,
	...
	registerFlag(
		...
		if (options.default !== undefined) {
			this.hostRuntime.flagValues.set(name, options.default);
		}
	}
	...
	getFlag(name: string): boolean | string | undefined {
		...
		return this.hostRuntime.flagValues.get(name);
	}
	...
	sendMessage<T = unknown>(
		...
		this.hostRuntime.sendMessage(message, options);
	}
	...
	sendUserMessage(
		...
		this.hostRuntime.sendUserMessage(content, options);
	}

	appendEntry(customType: string, data?: unknown): void {
		this.hostRuntime.appendEntry(customType, data);
	}
	...
	getActiveTools(): string[] {
		return this.hostRuntime.getActiveTools();
	}

	getAllTools(): string[] {
		return this.hostRuntime.getAllTools();
	}

	setActiveTools(toolNames: string[]): Promise<void> {
		return this.hostRuntime.setActiveTools(toolNames);
	}

	getCommands() {
		return this.hostRuntime.getCommands();
	}

	setModel(model: Model): Promise<boolean> {
		return this.hostRuntime.setModel(model);
	}

	getThinkingLevel(): ThinkingLevel | undefined {
		return this.hostRuntime.getThinkingLevel();
	}

	setThinkingLevel(level: ThinkingLevel, persist?: boolean): void {
		this.hostRuntime.setThinkingLevel(level, persist);
	}

	getServiceTiers(): Readonly<ServiceTierByFamily> {
		return { ...this.hostRuntime.getServiceTiers() };
	}
	...
	setServiceTier(family: ServiceTierFamily, tier: ServiceTier | undefined): void {
		...
		this.hostRuntime.setServiceTier(family, tier);
	}

	getSessionName(): string | undefined {
		return this.hostRuntime.getSessionName();
	}

	setSessionName(name: string): Promise<void> {
		return this.hostRuntime.setSessionName(name);
	}

	registerProvider(name: string, config: ProviderConfig): void {
		this.hostRuntime.pendingProviderRegistrations.push({ name, config, sourceId: this.extension.path });
	}
}
...
async function loadExtension(
	...
	try {
		const module = (await withHostGuard(() => loadLegacyAgentModule(resolvedPath))) as LoadedExtensionModule;
		...
		const api = new ConcreteExtensionAPI(YemuCodingAgent, extension, runtime, cwd, eventBus);
...
async function readExtensionManifest(packageJsonPath: string): Promise<ExtensionManifest | null> {
	try {
		const pkg = (await Bun.file(packageJsonPath).json()) as { yemu?: ExtensionManifest };
 ...
 * `.yemu` extension capabilities, JS/TS hook factories, the
...
export async function discoverExtensionPaths(
	...
	// 1. Discover extension modules via capability API (native .yemu only).
