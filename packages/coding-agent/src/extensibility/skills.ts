...
import { getProjectDir, prompt } from "@yemu/utils";
import {
...
export async function loadSkills(options: LoadSkillsOptions = {}): Promise<LoadSkillsResult> {
	...
	// third-party toggles count here: the YeMu-native providers (`agents`,
	...
	// Codex/Claude/YeMu (issue #2401 / PR #2405 review).
	const anyThirdPartySkillToggleEnabled =
		enableCodexUser || enableClaudeUser || enableClaudeProject || enableYemuUser || enableYemuProject;

	function isSourceEnabled(source: SourceMeta): boolean {
		const { provider, level } = source;
		// Managed skills (auto-learn) are YeMu-native and discovered unconditionally
		...
		if (provider === "native" && level === "user") return enableYemuUser;
		if (provider === "native" && level === "project") return enableYemuProject;
