 ...
 *
 * Dependencies (the zod-backed typebox shim and agent-runtime) are injected via the
...
export async function loadCustomTools(
	...
	const loader = new CustomToolLoader(YemuCodingAgent, cwd, builtInToolNames, pushPendingAction);
 ...
 * `.yemu/tools/`, `.claude/tools/`, the plugin tree, and any configured paths.
 *
...
export async function discoverCustomToolPaths(configuredPaths: string[], cwd: string): Promise<ToolPathWithSource[]> {
	...
	// 2. Plugin tools: ~/.yemu/plugins/node_modules/*/
 ...
 * 2. Installed plugins (~/.yemu/plugins/node_modules/*)
