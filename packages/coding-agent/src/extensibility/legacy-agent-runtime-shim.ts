 ...
 * `@yemu/agent-runtime` (or one of its aliased scopes like
 * `@yemu/agent-runtime` or `@yemu/agent-runtime`).
 *
 ...
 * the same module identity as a direct `@yemu/agent-runtime` import.
 */

import { Database } from "bun:sqlite";
import * as fs from "node:fs";
import * as path from "node:path";
import type { AgentToolResult, AgentToolUpdateCallback } from "@yemu/agent-core";
import { type AuthCredential, SqliteAuthCredentialStore, type TSchema } from "@yemu/model-runtime";
import { yemuEscapeRegexLiteral, yemuJoinPath } from "@yemu/model-runtime/providers/cursor-yemu-args";
import { getKeybindings, type Keybinding, Text } from "@yemu/tui";
import {
	getAgentDbPath,
	getAgentDir,
	getProjectDir,
	isCompiledBinary,
	parseFrontmatter as parseYemuFrontmatter,
} from "@yemu/utils";
import { getPackageDir as getYemuPackageDir } from "../config";
...
import {
	...
	discoverSkills,
	createAgentSession as yemuCreateAgentSession,
} from "../sdk";
import {
	DEFAULT_MAX_BYTES,
	DEFAULT_MAX_LINES,
...
const LEGACY_BUILTIN_TOOL_MARKER = "__yemuLegacyBuiltinTool";
...
/** Parse frontmatter using the historical YeMu package-root helper. */
...
export function parseFrontmatter<T extends Record<string, unknown> = Record<string, unknown>>(
	...
	const { frontmatter, body } = parseYemuFrontmatter(content, { level: "fatal" });
...
/** Mark an extension-authored tool as a YeMu-compatible tool definition. */
...
export function createGrepToolDefinition(cwd: string, options?: GrepToolOptions): ToolDefinition {
	...
	return markToolDefinition({
		...
		execute: (toolCallId, params, signal, onUpdate) => {
			...
			const pattern = booleanField(params, "literal") ? yemuEscapeRegexLiteral(rawPattern) : rawPattern;
			const searchPath = stringField(params, "path") ?? ".";
			...
			return grepTool.execute(
				...
				{
					pattern,
					path: glob ? yemuJoinPath(searchPath, glob) : searchPath,
...
export function createFindToolDefinition(cwd: string, options?: FindToolOptions): ToolDefinition {
	...
	return markToolDefinition({
		...
		execute: async (toolCallId, params, signal, onUpdate) => {
			...
			return tool.execute(
				toolCallId,
				{ path: yemuJoinPath(searchPath, pattern), hidden: true, gitignore: true, limit },
				signal,
				onUpdate,
			);
		},
...
/**
 * Enumerates the extensions YeMu would load through the historical package
...
export class DefaultPackageManager {
	...
	/** Resolve enabled extension paths with their YeMu plugin provenance. */
...
/**
 * Resource-loader compatibility layer for legacy yemu extensions.
 *
 * Upstream `@yemu/agent-runtime` centralizes extension / skill /
 ...
 * yemu-schedule-prompt (≥0.2.0) and other yemu extensions that spawn subagents
 * import the class at module scope; a missing export takes the whole
 * extension down at parse time (issue #4567).
 *
 * YeMu does the same discovery inline inside `createAgentSession()`, so this
 * shim intentionally does NOT re-implement yemu's ResourceLoader plumbing.
 ...
 * override below translates them into YeMu's native session options
 ...
 *
 * The yemu surface it emulates is the intersection actually used by real
 * extensions in the wild — themes are silently dropped (YeMu has no
...
/** Marker interface preserved for yemu extensions that type against upstream. */
...
export interface ResourceLoader {
	...
	readonly __yemuLegacyAgentLoader?: true;
}
...
export class DefaultResourceLoader implements ResourceLoader {
	readonly __yemuLegacyAgentLoader = true as const;
	...
	/**
	 * Discovery snapshot used to seed the session. Emulates upstream yemu's
...
/**
 * Legacy yemu extensions call `createAgentSession({ resourceLoader })`. YeMu's
 ...
 * `noExtensions`/`noSkills` opt-outs), re-run YeMu's own discovery, and
 ...
 *
 * Translate the loader's captured state into YeMu's option fields, then
 * delegate to the underlying SDK. Explicit fields on `options` override the
 * loader (matches upstream yemu semantics — a caller can partially override a
 * shared loader).
 *
 ...
 */
export type LegacyAgentCreateAgentSessionOptions = CreateAgentSessionOptions & {
	resourceLoader?: ResourceLoader;
};

export async function createAgentSession(
	options: LegacyAgentCreateAgentSessionOptions = {},
): Promise<CreateAgentSessionResult> {
	const loader = options.resourceLoader;
	if (!loader) {
		return yemuCreateAgentSession(options);
	}
	...
	return yemuCreateAgentSession(forwarded);
}

/**
 * Synchronous auth storage surface retained for legacy extensions.
 *
 * Modern YeMu auth storage is asynchronous, while older provider extensions
...
// YeMu SDK path helpers. `export * from "../index"` above only forwards
// `getAgentDir`; `getProjectDir` (a `@yemu/utils` helper) and
...
export { getProjectDir } from "@yemu/utils";

/**
 * Coding-agent package install directory, matching yemu's string-valued
 * `getPackageDir()` contract (extensions do `path.join(getPackageDir(), ...)`
 * to auto-allow bundled docs/resources).
 *
 * yemu's canonical `getPackageDir()` (`../config`) returns `undefined` inside a
 ...
 * `YEMU_PACKAGE_DIR` override and dev/source/npm-dist walk-up still win via the
 * canonical helper.
 */
export function getPackageDir(): string {
	return getYemuPackageDir() ?? (isCompiledBinary() ? path.dirname(process.execPath) : process.cwd());
}

// Legacy yemu's `@yemu/agent-runtime` re-exported `estimateTokens`
// from its package root (via `./core/compaction/index.ts`). In yemu it lives in
// `@yemu/agent-core/compaction`, and the coding-agent barrel below does
// not forward it, so legacy extensions importing it fail Bun's static export
// check during validation (issue #6583).
export { estimateTokens } from "@yemu/agent-core/compaction";

// Same barrel gap for two more legacy package-root exports: yemu re-exported the
// `CONFIG_DIR_NAME` constant and the CLI parser `parseArgs`. In yemu
// `CONFIG_DIR_NAME` lives in `@yemu/utils` and `parseArgs` in
...
export { CONFIG_DIR_NAME } from "@yemu/utils";
