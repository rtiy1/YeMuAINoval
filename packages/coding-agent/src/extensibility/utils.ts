	...
	constructor(
		...
		super(
			`Module called ${alias}(${code === undefined ? "" : String(code)}) during guarded extension/hook loading; ` +
				`YeMu extension/hook modules must not terminate the host process.`,
		);
 ...
 * from user directories (including Claude Code's `~/.claude/tools`, which YeMu
 ...
 *   YeMu during startup with no error surface, since `try/catch` cannot
