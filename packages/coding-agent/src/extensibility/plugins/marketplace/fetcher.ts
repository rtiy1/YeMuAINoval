...
import { isEnoent, logger } from "@yemu/utils";
...
/**
 * Catalog paths tried in priority order: yemu-namespaced override first, then
 * the Claude Code-compatible fallback so existing marketplaces keep loading.
 */
const CATALOG_RELATIVE_PATHS: readonly string[] = [".yemu-plugin/marketplace.json", ".claude-plugin/marketplace.json"];
