...
import { getPluginsDir, getPluginsLockfile, isEnoent } from "@yemu/utils";
import { getConfigDirPaths } from "../../config";
import { registerPluginCacheInvalidator, resolveActiveProjectRegistryPath } from "../../discovery/helpers";
import { installLegacyAgentSpecifierShim } from "./legacy-agent-compat";
 ...
 *
 * `home` controls which `<plugins>/yemu-plugins.lock.json` is read — pass it
...
/**
 * Load project-local plugin overrides from the .yemu directory.
 */
 ...
 * `<root>/package.json#dependencies`, and `<root>/yemu-plugins.lock.json#plugins`.
...
async function collectPluginsAtRoot(
	...
	const lockPath = path.join(root, "yemu-plugins.lock.json");
	...
	for (const name of names) {
		...
		let pluginPkg: { version: string; yemu?: PluginManifest };
		try {
		...
		const manifest: PluginManifest | undefined = pluginPkg.yemu;
		if (!manifest) {
			// Not an yemu plugin, skip
			continue;
		}
 ...
 * (`getPluginsDir(home)`) and, when a project anchor (`.yemu/` or `.git/`)
 * exists at or above `cwd`, the project root
 * (`<projectAnchor>/.yemu/plugins`). Each root contributes the union of its
 * `package.json#dependencies` and `yemu-plugins.lock.json#plugins`. Project
...
interface DeclaredManifestEntries {
	/** True when the directory's package.json declares a non-empty `yemu`/`yemu` `extensions` array. */
	declared: boolean;
...
/**
 * Read the extension entries declared by `dir`'s own package.json `yemu`/`yemu`
...
function readDeclaredManifestEntries(dir: string): DeclaredManifestEntries {
	...
	let pkg: { yemu?: { extensions?: unknown } };
	try {
		pkg = JSON.parse(raw) as { yemu?: { extensions?: unknown } };
	} catch {
		return { declared: false, files: [] };
	}
	const declared = pkg.yemu?.extensions;
 ...
 *   1. the directory's own package.json `yemu`/`yemu` `extensions` entries —
 ...
 *     {@link resolveDirectoryEntries} — its own package.json `yemu`/`yemu`
 *     `extensions`, then a direct index, then a one-level scan of
 *     sub-extensions — matching the yemu `extensions/<name>/index.ts` convention
 *     and YeMu's configured-directory (`-e`) extension loader
 *   - otherwise (tools/hooks/commands) only a direct index.{ts,js,mjs,cjs}.
 *     The sub-extension scan and the `yemu`/`yemu` `extensions` manifest are
