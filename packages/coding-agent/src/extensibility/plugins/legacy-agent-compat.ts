/// <reference path="./legacy-agent-virtual-modules.d.ts" />
...
import { isCompiledBinary, stripWindowsExtendedLengthPathPrefix } from "@yemu/utils";
...
// surfaces through `yemu-legacy-agent-bundled:<key>` synthetic modules.
// `scripts/legacy-agent-virtual-module.ts` derives literal dynamic-import edges
...
const BUNDLED_VIRTUAL_SCHEME = "yemu-legacy-agent-bundled:";
const BUNDLED_VIRTUAL_NAMESPACE = "yemu-legacy-agent-bundled";
const BUNDLED_MODULES_GLOBAL = "__yemuLegacyAgentBundledModules";
...
interface LegacyAgentResolveResult {
	path: string;
	namespace?: string;
}
...
function ensureBundledModuleLoadersLoaded(): Promise<BundledModuleLoaders> {
	if (!IS_COMPILED_BINARY) {
		return Promise.reject(new Error("yemu:legacy-agent-shim: bundled modules are only available in compiled mode"));
	}
	if (!bundledModuleLoadersPromise) {
		bundledModuleLoadersPromise = import("yemu-legacy-agent-modules").then(module => {
			Reflect.set(globalThis, BUNDLED_MODULES_GLOBAL, loadedBundledModules);
			return module.BUNDLED_YEMU_MODULE_LOADERS;
		});
	}
...
async function loadBundledModule(moduleKey: string): Promise<void> {
	...
	if (!loader) {
		throw new Error(`yemu:legacy-agent-shim: no bundled module registered for ${moduleKey}`);
	}
...
function toLegacyAgentResolveResult(resolvedPath: string): LegacyAgentResolveResult {
...
export function resolveBundledVirtualSpecifier(specifier: string): BundledVirtualResolveResult {
	const registryKey = isBundledVirtualSpecifier(specifier)
		...
		: specifier;
	if (!registryKey) {
		throw new Error("yemu:legacy-agent-shim: bundled virtual specifier has no registry key");
	}
	return { path: registryKey, namespace: BUNDLED_VIRTUAL_NAMESPACE };
}
...
function synthesizeBundledModuleSourceFromModules(moduleKey: string, modules: BundledModules): string {
	...
	const lines: string[] = [
		`const __yemu_bundled = globalThis[${JSON.stringify(BUNDLED_MODULES_GLOBAL)}][${JSON.stringify(moduleKey)}];`,
	];
	...
	for (const exportName in mod) {
		...
		lines.push(`export const ${exportName} = __yemu_bundled[${JSON.stringify(exportName)}];`);
	}
	if (hasDefault) {
		lines.push("export default __yemu_bundled.default;");
	}
	lines.push("");
	return lines.join("\n");
}
 ...
 * `yemu-legacy-agent-bundled:<key>` import.
 */
...
export function __synthesizeLegacyAgentBundledSourceWithModules(
	moduleKey: string,
...
export function __getLegacyAgentBundledModulesGlobal(): string {
	return BUNDLED_MODULES_GLOBAL;
}

// Canonical scope for in-process YeMu packages. Extensions resolve against the
// bundled copy that ships inside the YeMu binary so the host keeps one module
// registry and one tool registry.
const CANONICAL_YEMU_SCOPE = "@yemu";

const YEMU_SCOPE_ALIASES = ["yemu"] as const;

// Internal package basenames bundled inside the YeMu binary.
const YEMU_PACKAGE_NAMES = ["agent-core", "model-runtime", "agent-runtime", "native", "tui", "utils"] as const;

const YEMU_SCOPE_ALTERNATION = YEMU_SCOPE_ALIASES.join("|");
const YEMU_PACKAGE_ALTERNATION = YEMU_PACKAGE_NAMES.join("|");
...
const YEMU_SUBPATH_REMAPS: ReadonlyMap<string, string> = new Map<string, string>([
	["model-runtime/utils/oauth", "model-runtime/oauth"],
	["model-runtime/utils/oauth/", "model-runtime/oauth/"],
	["model-runtime/compat", "model-runtime"],
]);

function remapLegacyAgentSubpath(rest: string): string {
	const exact = YEMU_SUBPATH_REMAPS.get(rest);
	if (exact) {
		return exact;
	}

	for (const [from, to] of YEMU_SUBPATH_REMAPS) {
...
const LEGACY_AGENT_SPECIFIER_FILTER = new RegExp(
	`^@(?:${YEMU_SCOPE_ALTERNATION})/(?:${YEMU_PACKAGE_ALTERNATION})(?:/.*)?$`,
);
...
function clearLegacyAgentResolutionCaches(): void {
...
registerPluginCacheInvalidator(clearLegacyAgentResolutionCaches);
...
// is served through the `yemu-legacy-agent-bundled:` virtual namespace (see the
...
// bundle (`YEMU_BUNDLED=true`) or directly from the monorepo source tree.

/**
 * Compute the package root for the npm prebuilt `dist/cli.js` bundle.
 *
 * `bundle-dist.ts` defines `process.env.YEMU_BUNDLED="true"`; after bundling,
 * `import.meta.dir` points at `<package>/dist`. Do not resolve the package via
 * bare `@yemu/agent-runtime` here: from a global install Bun can pick an
...
function resolveBundledSelfPackageRoot(): string | undefined {
	if (!process.env.YEMU_BUNDLED) return undefined;
 ...
 * `yemu-legacy-agent-bundled:` virtual namespace (issue #3423) — bunfs paths are
 ...
 * `__validateLegacyAgentPackageRootOverrides` (#2168): if the computed candidate
...
// the package root of `@(scope)/model-runtime`. model-runtime 15.1.0 removed the runtime `Type`
...
// model-runtime package root onto a sibling shim that re-exports the canonical surface
// plus the borrowed `Type` runtime from the Zod-backed TypeBox shim. Subpath
// imports such as `@yemu/model-runtime/oauth` continue to resolve directly
// against the bundled model-runtime package.
const LEGACY_AGENT_AI_SHIM_PATH = IS_COMPILED_BINARY
	? bundledModuleVirtualSpecifier(`${CANONICAL_YEMU_SCOPE}/model-runtime`)
	: sourceShimPath("legacy-model-runtime-shim.ts");
...
// `@(scope)/agent-runtime` root therefore resolves through the bundled
...
const LEGACY_AGENT_CODING_AGENT_SHIM_PATH = IS_COMPILED_BINARY
	? bundledModuleVirtualSpecifier(`${CANONICAL_YEMU_SCOPE}/agent-runtime`)
	: sourceShimPath("legacy-agent-runtime-shim.ts");

// The compatibility TUI surface exports `decodeKittyPrintable` from its package root. The
...
const LEGACY_AGENT_TUI_SHIM_PATH = IS_COMPILED_BINARY
	? bundledModuleVirtualSpecifier(`${CANONICAL_YEMU_SCOPE}/tui`)
	: sourceShimPath("legacy-agent-tui-shim.ts");

// Package-root overrides. Shim entries (`model-runtime`, `agent-runtime`, `tui`)
// always replace the canonical surface so legacy helpers stay reachable. The
// other bundled host packages (`agent-core`, `native`, `utils`) are
...
// `node_modules/@yemu-*`.
//
// Compiled-binary entries are `yemu-legacy-agent-bundled:<key>` specifiers handed
// to the synthetic onLoad in `installLegacyAgentSpecifierShim()` — bunfs paths
 ...
 * through to the canonical-resolution path. Virtual `yemu-legacy-agent-bundled:`
 ...
 */
export function __validateLegacyAgentPackageRootOverrides(
 ...
 * directly: the model-runtime / agent-runtime roots (compat shims that re-attach
 * legacy helpers) plus, in compiled mode, every build-supplied module key.
 * Subpath coverage stops `@(scope)/model-runtime/oauth` and friends from falling
 * through to the extension's absent peer install when bunfs walks fail.
 */
export function __buildLegacyAgentPackageRootOverrides(
	isCompiled: boolean,
	bundledModuleKeys: Iterable<string> = [],
): Record<string, string> {
	const candidates: Record<string, string> = {
		[`${CANONICAL_YEMU_SCOPE}/model-runtime`]: LEGACY_AGENT_AI_SHIM_PATH,
		[`${CANONICAL_YEMU_SCOPE}/agent-runtime`]: LEGACY_AGENT_CODING_AGENT_SHIM_PATH,
		[`${CANONICAL_YEMU_SCOPE}/tui`]: LEGACY_AGENT_TUI_SHIM_PATH,
	};
	if (isCompiled) {
	...
	return __validateLegacyAgentPackageRootOverrides(candidates);
}
...
let legacyAgentPackageRootOverrides = __buildLegacyAgentPackageRootOverrides(IS_COMPILED_BINARY);
let legacyAgentOverridesReadyPromise: Promise<void> | null = null;

/** Complete compiled-mode overrides from the lazy host-module registry. */
function ensureLegacyAgentOverridesReady(): Promise<void> {
	if (!IS_COMPILED_BINARY) {
		return Promise.resolve();
	}
	if (!legacyAgentOverridesReadyPromise) {
		legacyAgentOverridesReadyPromise = ensureBundledModuleLoadersLoaded().then(loaders => {
			legacyAgentPackageRootOverrides = __buildLegacyAgentPackageRootOverrides(true, Object.keys(loaders));
		});
	}
	return legacyAgentOverridesReadyPromise;
}

let isLegacyAgentSpecifierShimInstalled = false;

function remapLegacyAgentSpecifier(specifier: string): string | null {
	if (!LEGACY_AGENT_SPECIFIER_FILTER.test(specifier)) {
		return null;
	}
	...
	const remappedSubpath = remapLegacyAgentSubpath(rest);
	return `${CANONICAL_YEMU_SCOPE}/${remappedSubpath}`;
}
...
/**
 * Resolve a canonical `@yemu/*` specifier to a filesystem path, preferring
 ...
 * specifiers.
 */
function resolveCanonicalYemuSpecifier(remappedSpecifier: string): string {
	const override = legacyAgentPackageRootOverrides[remappedSpecifier];
	if (override) {
		return override;
	}
...
function toImportSpecifier(resolvedPath: string): string {
	// Virtual `yemu-legacy-agent-bundled:` specifiers are served by the synthetic
	// onLoad in `installLegacyAgentSpecifierShim()`; wrapping them as `file://`
...
/**
 * Rewrite the extension-owned specifiers YeMu must host-resolve — legacy
 * `@(scope)/yemu-*`, bare TypeBox packages, package `imports` aliases like
 ...
 * `@(scope)/yemu-*`, TypeBox shim) always emit `file://` URLs because they resolve
...
async function rewriteLegacyExtensionSource(
	...
	await ensureLegacyAgentOverridesReady();
	...
	for (const reference of references) {
		...
		const remappedSpecifier = remapLegacyAgentSpecifier(specifier);
		if (remappedSpecifier) {
			try {
				replacement = toImportSpecifier(resolveCanonicalYemuSpecifier(remappedSpecifier));
			} catch {
...
async function resolveExtensionCommonJsRequire(specifier: string, importerPath: string): Promise<string | null> {
	...
	if (remappedSpecifier) {
		try {
			const resolved = resolveCanonicalYemuSpecifier(remappedSpecifier);
...
const COMMONJS_REQUIRE_GLOBAL = "__yemuLegacyAgentRequireGraphModule";
 ...
 *
 * On source-link installs the `@(scope)/agent-runtime` root shim is served
...
let legacyAgentLoadTag = 0;

function nextLegacyAgentLoadTag(): string {
	legacyAgentLoadTag = Math.max(legacyAgentLoadTag + 1, Date.now());
	return String(legacyAgentLoadTag);
}
 ...
 * realpath of every reachable source module YeMu must rewrite at load time.
		...
		for (const reference of references) {
			...
			try {
				...
				if (specifier.startsWith(".")) {
					...
					!remapLegacyAgentSpecifier(specifier) &&
					specifier !== "typebox" &&
...
function synthesizeCommonJsDefaultModule(modulePath: string, source: string, targetPath = modulePath): string {
	...
	commonJsModuleCache.delete(modulePath);
	const exportsBinding = "__yemuLegacyAgentCommonJsExports";
	const namedExports = collectCommonJsNamedExports(executableSource, targetPath)
		.map(
			(name, index) =>
				`const __yemuLegacyAgentCommonJsExport${index} = ${exportsBinding}[${JSON.stringify(name)}]; export { __yemuLegacyAgentCommonJsExport${index} as ${name} };`,
		)
		.join("\n");
...
/**
 * Linkedom's canvas bridge uses its bundled fallback because YeMu does not ship
 * native canvas.
 */
...
async function installExtensionGraphHook(
	...
	if (asyncModules.size > 0) {
		...
		Bun.plugin({
			name: `yemu:legacy-agent-ext:${hookId}`,
			setup(build) {
...
/**
 * Load a legacy YeMu extension module from its real on-disk location.
 *
 ...
 * the entry) resolve exactly as they do under the original YeMu runtime — no
 ...
 */
export async function loadLegacyAgentModule(resolvedPath: string): Promise<unknown> {
	...
	try {
		...
		const entrySpecifier =
			process.platform === "win32" || isBundledVirtualSpecifier(entryRealPath)
				...
				: entryRealPath;
		return await import(`${entrySpecifier}?mtime=${nextLegacyAgentLoadTag()}`);
	} finally {
...
function resolveLegacyAgentSpecifier(args: { path: string; importer: string }): LegacyAgentResolveResult | undefined {
	const remappedSpecifier = remapLegacyAgentSpecifier(args.path);
	if (!remappedSpecifier) {
		return undefined;
	}

	// Primary: resolve the canonical @yemu/* specifier from the host binary
	// location. Works in dev mode and in source-link installs.
	try {
		return toLegacyAgentResolveResult(resolveCanonicalYemuSpecifier(remappedSpecifier));
	} catch {
		...
		// plugin installed @yemu peer deps, then try the original legacy
		// specifier for plugins that still vendor only @mariozechner or
		// @earendil-works peer deps.
		const importerDir = path.dirname(args.importer);
		try {
			return toLegacyAgentResolveResult(Bun.resolveSync(remappedSpecifier, importerDir));
		} catch {
			try {
				return toLegacyAgentResolveResult(Bun.resolveSync(args.path, importerDir));
			} catch {
				return undefined;
			}
		}
...
function resolveTypeBoxSpecifier(): LegacyAgentResolveResult | undefined {
	return TYPEBOX_SHIM_PATH ? toLegacyAgentResolveResult(TYPEBOX_SHIM_PATH) : undefined;
}

export function installLegacyAgentSpecifierShim(): void {
	if (isLegacyAgentSpecifierShimInstalled) {
		return;
	}
	isLegacyAgentSpecifierShimInstalled = true;

	Bun.plugin({
		name: "yemu:legacy-agent-shim",
		setup(build) {
			build.onResolve({ filter: LEGACY_AGENT_SPECIFIER_FILTER, namespace: "file" }, resolveLegacyAgentSpecifier);
			...
			// Compiled mode serves `yemu-legacy-agent-bundled:<key>` imports from
...
export function __resetLegacyAgentResolutionCache(): void {
	clearLegacyAgentResolutionCaches();
}
