import type { ImageContent, Message, Model, TextContent } from "@yemu/model-runtime";
import type { Component, TUI } from "@yemu/tui";
...
export interface HookContext {
	...
	cwd: string;
	/** Session manager (read-only) - use yemu.sendMessage()/yemu.appendEntry() for writes */
 ...
 * Hooks use yemu.on() to subscribe to events and yemu.sendMessage() to inject messages.
 */
export interface HookAPI {
	 ...
	 * yemu.appendEntry("permissions", { level: "full", grantedAt: Date.now() });
	 *
	 * // On reload, reconstruct state from entries
	 * yemu.on("session", async (event, ctx) => {
	...
	zod: typeof zod;
	/** Injected agent-runtime exports */
	yemu: typeof YemuCodingAgent;
}
 ...
 */
export type HookFactory = (yemu: HookAPI) => void;
