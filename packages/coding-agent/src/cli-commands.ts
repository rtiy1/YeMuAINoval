 ...
 */
import type { CommandEntry } from "@yemu/utils/cli";
...
// commands. Without a guard `resolveCliArgv` rewrites e.g. `yemu marketplace add
// xyz` to `yemu launch marketplace add xyz`, silently forwarding the argv to the
...
// The real commands live under `yemu plugin <action>`; each entry maps a verb to
...
const RESERVED_TOP_LEVEL_WORDS: Record<string, string> = {
	extensions:
		'`yemu extensions` is not a management command. Use `yemu plugin list` / `yemu plugin install`, or run `yemu launch extensions` if you meant to send "extensions" as a prompt.',
	list: '`yemu list` is not a top-level command. Use `yemu plugin list` to list installed plugins, or run `yemu launch list` if you meant to send "list" as a prompt.',
	remove:
		'`yemu remove` is not a top-level command. Use `yemu plugin uninstall <name>` to remove a plugin, or run `yemu launch remove` if you meant to send "remove" as a prompt.',
	uninstall:
		'`yemu uninstall` is not a top-level command. Use `yemu plugin uninstall <name@marketplace>` to remove a plugin, or run `yemu launch uninstall` if you meant to send "uninstall" as a prompt.',
	marketplace:
		'`yemu marketplace` is not a top-level command. Use `yemu plugin marketplace <add|remove|update|list>` to manage marketplaces, or run `yemu launch marketplace` if you meant to send "marketplace" as a prompt.',
	discover:
		'`yemu discover` is not a top-level command. Use `yemu plugin discover [marketplace]` to browse available plugins, or run `yemu launch discover` if you meant to send "discover" as a prompt.',
	upgrade:
		'`yemu upgrade` is not a top-level command. Use `yemu plugin upgrade [name@marketplace]` to upgrade plugins, or run `yemu launch upgrade` if you meant to send "upgrade" as a prompt.',
	enable:
		'`yemu enable` is not a top-level command. Use `yemu plugin enable <name@marketplace>` to enable a plugin, or run `yemu launch enable` if you meant to send "enable" as a prompt.',
	disable:
		'`yemu disable` is not a top-level command. Use `yemu plugin disable <name@marketplace>` to disable a plugin, or run `yemu launch disable` if you meant to send "disable" as a prompt.',
};

// Sub-actions that make `yemu marketplace <sub>` unambiguously a management
// command even when multi-word (the reporter's `yemu marketplace add xyz`,
 ...
 *
 * A bare verb (`yemu marketplace`) always hints. A multi-word invocation only
 * hints when the arguments follow the documented plugin grammar — a marketplace
 * sub-action (`yemu marketplace add …`) or a `name@marketplace` plugin id
 * (`yemu uninstall foo@bar`) — so genuine prompts that merely begin with one of
 * these words (`yemu list all my files`, `yemu upgrade the deps`) still launch.
 *
...
export function resolveCliArgv(argv: string[]): ResolvedCliArgv {
	...
	// (`yemu --approval-mode=yolo acp`). `run` dispatches strictly on argv[0], so
