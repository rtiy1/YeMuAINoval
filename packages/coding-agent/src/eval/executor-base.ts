...
export function buildManagedKernelEnvPatch(options: ManagedKernelEnvOptions): Record<string, string | null> {
	...
	return {
		YEMU_SESSION_FILE: options.sessionFile ?? null,
		YEMU_ARTIFACTS_DIR: options.artifactsDir ?? null,
		YEMU_TOOL_BRIDGE_URL: options.bridge?.url ?? null,
		YEMU_TOOL_BRIDGE_TOKEN: options.bridge?.token ?? null,
		YEMU_TOOL_BRIDGE_SESSION: options.bridge && options.bridgeSessionId ? options.bridgeSessionId : null,
		YEMU_EVAL_LOCAL_ROOTS: localRoots && Object.keys(localRoots).length > 0 ? JSON.stringify(localRoots) : null,
	};
}
