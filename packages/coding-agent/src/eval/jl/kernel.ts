...
import { $flag, Snowflake } from "@yemu/utils";
import { $ } from "bun";
...
const TRACE_IPC = $flag("YEMU_JULIA_IPC_TRACE");
...
const RUNNER_CACHE_DIR = path.join(os.tmpdir(), "yemu-julia-runner");
...
function buildInitScript(cwd: string, env?: Record<string, string | undefined>): string {
	...
	const lines = [
		`__yemu_init_cwd = String(Base64.base64decode("${Buffer.from(cwd).toString("base64")}"))`,
		"try cd(__yemu_init_cwd) catch; end",
	];
	...
	lines.push("if !(__yemu_init_cwd in LOAD_PATH); pushfirst!(LOAD_PATH, __yemu_init_cwd); end");
	return lines.join("\n");
}
