...
import { getProjectDir, logger } from "@yemu/utils";
...
function buildKernelEnvPatch(options: {
	...
	if (options.sessionFile) patch.YEMU_SESSION_FILE = options.sessionFile;
	if (options.artifactsDir) patch.YEMU_ARTIFACTS_DIR = options.artifactsDir;
	if (options.bridge) {
		patch.YEMU_TOOL_BRIDGE_URL = options.bridge.url;
		patch.YEMU_TOOL_BRIDGE_TOKEN = options.bridge.token;
		patch.YEMU_TOOL_BRIDGE_SESSION = options.bridgeSessionId ?? "";
	}
	if (options.localRoots) {
		patch.YEMU_EVAL_LOCAL_ROOTS = JSON.stringify(options.localRoots);
	}
	return patch;
}
