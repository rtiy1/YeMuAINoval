# YeMu Julia runner — subprocess wrapper used by the coding-agent host.
...
function next_drain_marker()
    ...
    return Vector{UInt8}(codeunits("\0__YEMU_DRAIN__:" * string(drain_marker_counter[]) * ":" * string(time_ns()) * "\0"))
end
...
struct YemuDisplay <: AbstractDisplay end

function Base.display(d::YemuDisplay, value)
    rid = current_rid
    if rid !== nothing
