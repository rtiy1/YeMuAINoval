import { logger, postmortem, Snowflake, workerHostEntry } from "@yemu/utils";
import {
	createWorkerHandle,
 ...
 * `yemu --smoke-test` so binary / source / tarball installs all exercise it.
 */
...
function spawnBunWorker(): WorkerHandle {
	try {
		...
		const worker = hostEntry
			? new Worker(hostEntry, { type: "module", argv: ["__yemu_worker_js_eval"] })
