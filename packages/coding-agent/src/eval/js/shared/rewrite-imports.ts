...
// `__yemu_import__` helper, which resolves the specifier against the active session cwd. A real
...
// worker-injected `__yemu_import__` global does not exist. The swap therefore guards on the
...
const DYNAMIC_IMPORT_CALLEE = '(typeof __yemu_import__ === "function" ? __yemu_import__ : (s, o) => import(s, o))';

function buildYemuImportCall(sourceLiteral: string, optionsLiteral: string | undefined): string {
	// Route every static import through the worker-injected `__yemu_import__` helper so the
	// specifier resolves against the session cwd (and `with`-attribute imports keep working).
	return optionsLiteral ? `__yemu_import__(${sourceLiteral}, ${optionsLiteral})` : `__yemu_import__(${sourceLiteral})`;
}
...
function buildOptionsLiteral(node: BabelImportDeclaration): string | undefined {
	...
	// Native dynamic import takes options as `{ with: { ... } }`. `__yemu_import__` forwards the
...
function rewriteImportNode(node: BabelImportDeclaration): string {
	...
	const importCall = buildYemuImportCall(sourceLiteral, optionsLiteral);
...
export async function rewriteImports(code: string): Promise<string> {
	...
	// Top-level static `import` declarations become `await __yemu_import__(...)` calls.
	...
	// Dynamic `import(...)` expressions (anywhere) get their callee swapped for `__yemu_import__`
...
export async function rewriteDynamicImports(code: string, callee = "__yemu_import__"): Promise<string> {
	if (!code.includes("import")) return code;
	const ast = await parseProgram(code);
	if (!ast) return code;
...
async function returnFinalExpression(code: string): Promise<{ source: string; returned: boolean }> {
	...
	if (last?.type === "ExpressionStatement") {
		...
		return { source: `${prefix}__yemu_set_final_expr__((${trimmedStatement}));${suffix}`, returned: true };
	}
	if (last?.type === "ReturnStatement") {
		// Top-level `return value;` is otherwise swallowed: it forces the cell into an async IIFE
		// wrapper that discards the returned value. Rewrite into `__yemu_set_final_expr__((expr))`
		...
		return { source: `${prefix}__yemu_set_final_expr__((${expr}));${suffix}`, returned: true };
	}
