...
# YeMu Ruby prelude helpers (loaded once into the runner's TOPLEVEL_BINDING).
#
...
# uses (YEMU_TOOL_BRIDGE_URL/TOKEN/SESSION). Path helpers honor YEMU_EVAL_LOCAL_ROOTS
# so `write("local://x")` lands where `read local://x` resolves.
#
# `__yemu_*` primitives (emit/present/status/scrub/run_id) are provided by
# runner.rb; this file only consumes them.

unless defined?($__yemu_prelude_loaded) && $__yemu_prelude_loaded
  $__yemu_prelude_loaded = true
  ...
  # an injected on-disk root (YEMU_EVAL_LOCAL_ROOTS, e.g. `local://`) is rewritten
  # under that root; plain paths pass through; any other `scheme://` is rejected.
  def __yemu_resolve_path(path)
    ...
    relative = __yemu_url_decode(m[2].tr("\\", "/"))
  ...
  def display_image(base64, mime_type: "image/png")
    __yemu_emit_display({ mime_type.to_s => base64.to_s })
    nil
  end
  ...
  def env(key = nil, value = nil)
    if key.nil?
      ...
      __yemu_emit_status("env", "count" => items.size, "keys" => items.keys.first(20))
      return items
    end
    unless value.nil?
      ENV[key.to_s] = value.to_s
      __yemu_emit_status("env", "key" => key.to_s, "value" => value.to_s, "action" => "set")
      return value
    end
    val = ENV[key.to_s]
    __yemu_emit_status("env", "key" => key.to_s, "value" => val, "action" => "get")
    val
  end
  ...
  def read(path, offset = 1, limit = nil)
    resolved = __yemu_resolve_path(path)
    ...
    __yemu_emit_status("read", "path" => resolved.to_s, "chars" => data.length, "preview" => __yemu_scrub(data[0, 500].to_s))
    data
  end
  ...
  def write(path, content)
    ...
    __yemu_emit_status("write", "path" => resolved.to_s, "chars" => content.to_s.length)
    resolved.to_s
  end
  ...
  def __yemu_apply_query(data, query)
  ...
  def output(*ids, format: "raw", query: nil, offset: nil, limit: nil)
    artifacts_dir = ENV["YEMU_ARTIFACTS_DIR"]
    if artifacts_dir.nil? || artifacts_dir.empty?
      session_file = ENV["YEMU_SESSION_FILE"]
      if session_file.nil? || session_file.empty?
        __yemu_emit_status("output", "error" => "No session file available")
        raise "No session - output artifacts unavailable"
      end
      artifacts_dir = session_file.sub(/\.[^.]*\z/, "")
    end
    unless File.directory?(artifacts_dir)
      __yemu_emit_status("output", "error" => "Artifacts directory not found", "path" => artifacts_dir)
    ...
    if query && (!offset.nil? || !limit.nil?)
      __yemu_emit_status("output", "error" => "query cannot be combined with offset/limit")
      ...
      if query
        json_value =
          begin
            ...
            __yemu_emit_status("output", "id" => output_id, "error" => "Not valid JSON: #{e.message}")
        ...
        if start_line > total_lines
          __yemu_emit_status("output", "id" => output_id, "error" => "Offset #{start_line} beyond end (#{total_lines} lines)")
    ...
    unless not_found.empty?
      ...
      __yemu_emit_status("output", "not_found" => not_found, "available_count" => available.length)
      raise msg
    end

    if ids.length == 1
      if format == "json"
        __yemu_emit_status("output", "id" => ids[0], "chars" => results[0]["char_count"])
        return results[0]
      end
      __yemu_emit_status("output", "id" => ids[0], "chars" => results[0]["content"].length)
      return results[0]["content"]
    end

    if format == "json"
      __yemu_emit_status("output", "count" => results.length, "total_chars" => results.sum { |r| r["char_count"] })
      return results
    end
    combined = results.map { |r| { "id" => r["id"], "content" => r["content"] } }
    __yemu_emit_status("output", "count" => combined.length, "total_chars" => combined.sum { |r| r["content"].length })
    combined
  end
  ...
  module YemuBridge
    ...
    def proxy_env
      base = ENV["YEMU_TOOL_BRIDGE_URL"]
      token = ENV["YEMU_TOOL_BRIDGE_TOKEN"]
      session = ENV["YEMU_TOOL_BRIDGE_SESSION"]
    ...
    def call(name, args)
      ...
      payload = JSON.generate("session" => session, "run" => $__yemu_current_rid, "name" => name, "args" => args)
  ...
  class YemuToolCallable
    def initialize(name)
      @name = name
    end

    def call(args = nil, **kwargs)
      YemuBridge.tool_call(@name, args, kwargs)
    end
  ...
  class YemuToolProxy < BasicObject
    def method_missing(name, args = nil, **kwargs)
      ::YemuBridge.tool_call(name.to_s, args, kwargs)
    end

    def [](name)
      ::YemuToolCallable.new(name.to_s)
    end
  ...
  def tool
    $__yemu_tool_proxy ||= YemuToolProxy.new
  end
  ...
  def completion(prompt, model: "default", system: nil, schema: nil)
    ...
    res = YemuBridge.call("__completion__", args)
  ...
  def agent(prompt, agent: "task", model: nil, label: nil, schema: nil, schema_mode: nil, isolated: nil, apply: nil, merge: nil, handle: false)
    ...
    res = YemuBridge.call("__agent__", args)
    text = res.is_a?(Hash) ? res["text"] : res
  ...
  def __yemu_concurrency_limit
    snap = (YemuBridge.call("__concurrency__", {}) rescue nil) || {}
    n = (snap["limit"] || 0).to_i
    n > 0 ? n : 0
  rescue StandardError
    0
  ...
  def __yemu_pool_map(items)
    arr = items.to_a
    return [] if arr.empty?
    limit = __yemu_concurrency_limit
  ...
  def pipeline(items, *stages)
    ...
    stages.each do |stage|
      ...
      current = __yemu_pool_map(current) { |item| stage.call(item) }
    end
    current
  end
  ...
  def log(message)
    __yemu_emit_status("log", "message" => message.to_s)
    nil
  end

  def phase(title)
    $__yemu_current_phase = title.to_s
    __yemu_emit_status("phase", "title" => title.to_s)
    nil
  end
  ...
  class YemuBudget
    def total
      snap = (YemuBridge.call("__budget__", {}) || {})
      snap["total"]
    end
    ...
    def inspect
      snap = ((YemuBridge.call("__budget__", {}) rescue nil) || {})
