...
import { $flag, isBunTestRuntime, logger, Snowflake } from "@yemu/utils";
import { $ } from "bun";
...
const TRACE_IPC = $flag("YEMU_RUBY_IPC_TRACE");
...
const RUNNER_CACHE_DIR = path.join(os.tmpdir(), "yemu-ruby-runner");
...
export async function checkRubyKernelAvailability(cwd: string, interpreter?: string): Promise<RubyKernelAvailability> {
	if (isBunTestRuntime() || $flag("YEMU_RUBY_SKIP_CHECK")) {
		return { ok: true };
	}
...
export class RubyKernel extends BaseKernel<KernelExecuteOptions> {
	...
	static async start(options: KernelStartOptions): Promise<RubyKernel> {
		...
		// YEMU_RUBY_SKIP_CHECK), where no candidate was probed.
...
function buildInitScript(cwd: string, env?: Record<string, string | undefined>): string {
	...
	const lines = [`__yemu_init_cwd = ${JSON.stringify(cwd)}`, "Dir.chdir(__yemu_init_cwd) rescue nil"];
	for (const key in envPayload) {
		lines.push(`ENV[${JSON.stringify(key)}] = ${JSON.stringify(envPayload[key])}`);
	}
	lines.push("$LOAD_PATH.delete(__yemu_init_cwd)", "$LOAD_PATH.unshift(__yemu_init_cwd)");
	return lines.join("\n");
}
