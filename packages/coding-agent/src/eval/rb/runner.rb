...
# YeMu Ruby runner — subprocess wrapper used by the coding-agent host.
#
...
$__yemu_out_mutex = Mutex.new
$__yemu_raw_stderr = (STDERR.dup rescue STDERR)
$__yemu_current_rid = nil
$__yemu_capture_rid = nil
$__yemu_exec_count = 0
$__yemu_active_exec = 0
$__yemu_silent = false

begin
  $__yemu_frame_io = STDOUT.dup
  $__yemu_frame_io.sync = true
  __yemu_stdout_cap_r, __yemu_stdout_cap_w = IO.pipe
  STDOUT.reopen(__yemu_stdout_cap_w)
  STDOUT.sync = true
  __yemu_stdout_cap_w.close
  $__yemu_stdout_capture_read = __yemu_stdout_cap_r
rescue StandardError
  $__yemu_frame_io = STDOUT
  ($__yemu_frame_io.sync = true) rescue nil
  $__yemu_stdout_capture_read = nil
end

begin
  __yemu_stderr_cap_r, __yemu_stderr_cap_w = IO.pipe
  STDERR.reopen(__yemu_stderr_cap_w)
  STDERR.sync = true
  __yemu_stderr_cap_w.close
  $__yemu_stderr_capture_read = __yemu_stderr_cap_r
rescue StandardError
  $__yemu_stderr_capture_read = nil
end
...
begin
  $__yemu_proto_stdin = STDIN.dup
  STDIN.reopen(File.open(File::NULL, "r"))
rescue StandardError
  $__yemu_proto_stdin = STDIN
end
...
def __yemu_emit(frame)
  ...
  $__yemu_out_mutex.synchronize do
    $__yemu_frame_io.write(line)
    $__yemu_frame_io.write("\n")
    $__yemu_frame_io.flush
  end
rescue StandardError
  nil
end
...
def __yemu_emit_stream(kind, text)
  rid = $__yemu_current_rid
  if rid.nil?
    ($__yemu_raw_stderr.write(text) rescue nil)
    return
  end
  __yemu_emit("type" => kind, "id" => rid, "data" => __yemu_scrub(text))
end

def __yemu_emit_display(bundle, kind = "display")
  rid = $__yemu_current_rid
  return if rid.nil?
  __yemu_emit("type" => kind, "id" => rid, "bundle" => bundle)
end

def __yemu_emit_status(op, data = {})
  status = { "op" => op.to_s }
  data.each { |k, v| status[k.to_s] = v }
  __yemu_emit_display({ "application/x-yemu-status" => status }, "display")
end

YEMU_IMAGE_MIMES = %w[image/png image/jpeg].freeze
...
def __yemu_image_payload(content)
  require "base64"
  s = content.to_s
  return s.gsub(/\s+/, "") if __yemu_base64?(s)
...
def __yemu_sniff_image_mime(bytes)
  b = bytes.to_s.b
...
# bundle is always JSON-safe before it reaches __yemu_emit.
def __yemu_normalize_bundle(hash)
  bundle = {}
  hash.each do |key, val|
    k = key.to_s
    bundle[k] =
      if YEMU_IMAGE_MIMES.include?(k)
        __yemu_image_payload(val)
...
def __yemu_finalize_bundle(bundle, value)
  bundle["text/plain"] ||= __yemu_scrub((value.inspect rescue value.class.name))
  bundle
end

# Rich-display resolution for non-collection objects. Honors the repo
# `to_yemu_mime` convention first, then the IRuby protocol
...
def __yemu_rich_mime_bundle(value)
  if value.respond_to?(:to_yemu_mime)
    mime = (value.to_yemu_mime rescue nil)
    return __yemu_finalize_bundle(__yemu_normalize_bundle(mime), value) if mime.is_a?(Hash) && !mime.empty?
  end
  if value.respond_to?(:to_iruby_mimebundle)
    ...
    return __yemu_finalize_bundle(__yemu_normalize_bundle(data), value) if data.is_a?(Hash) && !data.empty?
  end
  if value.respond_to?(:to_iruby)
    pair = (value.to_iruby rescue nil)
    if pair.is_a?(Array) && pair.size == 2 && !pair[0].nil?
      return __yemu_finalize_bundle(__yemu_normalize_bundle({ pair[0].to_s => pair[1] }), value)
    end
  end
  ...
  if value.respond_to?(:to_png)
    ...
    return __yemu_finalize_bundle({ "image/png" => __yemu_image_payload(png) }, value) if png
  end
  jpeg_method = %i[to_jpeg to_jpg].find { |m| value.respond_to?(m) }
  if jpeg_method
    jpg = (value.public_send(jpeg_method) rescue nil)
    return __yemu_finalize_bundle({ "image/jpeg" => __yemu_image_payload(jpg) }, value) if jpg
  end
  if value.respond_to?(:to_blob)
    blob = (value.to_blob rescue nil)
    if blob.is_a?(String) && (mime = __yemu_sniff_image_mime(blob))
      return __yemu_finalize_bundle({ mime => __yemu_image_payload(blob) }, value)
    end
  end
...
# Other objects may expose a rich representation via `to_yemu_mime` or the IRuby
# protocol (`to_iruby`/`to_iruby_mimebundle`); otherwise they fall back to inspect.
def __yemu_mime_bundle(value)
  case value
  when String
    { "text/plain" => __yemu_scrub(value) }
  when Hash, Array
    safe = begin
    ...
    if safe.nil?
      { "text/plain" => __yemu_scrub(value.inspect) }
    else
      { "application/json" => safe, "text/plain" => __yemu_scrub(value.inspect) }
    end
  when nil
    { "text/plain" => "nil" }
  else
    __yemu_rich_mime_bundle(value) || { "text/plain" => __yemu_scrub(value.inspect) }
  end
end

def __yemu_present(value, kind = "display")
  __yemu_emit_display(__yemu_mime_bundle(value), kind)
end
...
def __yemu_start_capture_drain(io, kind)
  return if io.nil?
  Thread.new do
    loop do
      chunk =
      ...
      rid = $__yemu_capture_rid
      if rid.nil?
        ($__yemu_raw_stderr.write(chunk) rescue nil)
      else
        __yemu_emit("type" => kind, "id" => rid, "data" => __yemu_scrub(chunk))
      end
    end
  end
end

def __yemu_start_parent_watchdog
...
def __yemu_install_idle_sigint
  Signal.trap("INT", "IGNORE") rescue nil
end

def __yemu_install_exec_sigint
  Signal.trap("INT", "DEFAULT") rescue nil
end

def __yemu_begin_exec
  $__yemu_active_exec += 1
  __yemu_install_exec_sigint
end

def __yemu_end_exec
  $__yemu_active_exec -= 1 if $__yemu_active_exec > 0
  __yemu_install_idle_sigint if $__yemu_active_exec.zero?
end
...
YEMU_MANAGED_ENV_KEYS = %w[
  YEMU_SESSION_FILE
  YEMU_ARTIFACTS_DIR
  YEMU_TOOL_BRIDGE_URL
  YEMU_TOOL_BRIDGE_TOKEN
  YEMU_TOOL_BRIDGE_SESSION
  YEMU_EVAL_LOCAL_ROOTS
].freeze

def __yemu_apply_request_runtime(req)
  cwd = req["cwd"]
  ...
  env = req["env"]
  if env.is_a?(Hash)
    YEMU_MANAGED_ENV_KEYS.each do |key|
...
def __yemu_should_display_result?(src)
  ...
  last = __yemu_ast_last(node)
  return true if last.nil?
  !YEMU_NON_DISPLAY_NODES.include?(last.type)
end
...
def __yemu_emit_error(rid, exc, name_override = nil)
  ...
  __yemu_emit(
    ...
    "id" => rid,
    "ename" => ename,
    "evalue" => __yemu_scrub(evalue),
    "traceback" => traceback.map { |l| __yemu_scrub(l) },
  )
end

def __yemu_handle_request(req)
  rid = req["id"].to_s
  $__yemu_current_rid = rid
  $__yemu_capture_rid = rid
  $__yemu_silent = req["silent"] == true
  $__yemu_exec_count += 1
  count = $__yemu_exec_count
  __yemu_emit("type" => "started", "id" => rid)

  status = "ok"
  cancelled = false
  begin
    begin
      __yemu_apply_request_runtime(req)
      src = req["code"].to_s
    rescue Exception => e # rubocop:disable Lint/RescueException
      __yemu_emit_error(rid, e)
      __yemu_emit("type" => "done", "id" => rid, "status" => "error", "executionCount" => count, "cancelled" => false)
      return
    end
    ...
    begin
      ...
      unless $__yemu_silent || value.nil? || !__yemu_should_display_result?(src)
        __yemu_present(value, "result")
      end
    rescue Interrupt => e
      cancelled = true
      status = "error"
      __yemu_emit_error(rid, e, "Interrupt")
    rescue SystemExit => e
      status = "error"
      __yemu_emit_error(rid, e)
    rescue Exception => e # rubocop:disable Lint/RescueException
    ...
    __yemu_emit("type" => "done", "id" => rid, "status" => status, "executionCount" => count, "cancelled" => cancelled)
  ensure
    $__yemu_capture_rid = nil if $__yemu_capture_rid == rid
    $__yemu_current_rid = nil
  end
end
...
def __yemu_main
  $stdout = YemuStreamProxy.new("stdout", STDOUT, 1)
  $stderr = YemuStreamProxy.new("stderr", STDERR, 2)
  __yemu_install_idle_sigint
  __yemu_start_parent_watchdog
  __yemu_start_capture_drain($__yemu_stdout_capture_read, "stdout")
  __yemu_start_capture_drain($__yemu_stderr_capture_read, "stderr")

  $__yemu_proto_stdin.each_line do |raw|
    line = raw.strip
    next if line.empty?
    req =
      begin
    ...
    __yemu_handle_request(req) if req.is_a?(Hash)
  end
end
