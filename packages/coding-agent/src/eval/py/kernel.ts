...
const TRACE_IPC = $flag("YEMU_PYTHON_IPC_TRACE");
...
const RUNNER_CACHE_DIR = path.join(os.tmpdir(), "yemu-python-runner");
...
export async function checkPythonKernelAvailability(
	...
	if (isBunTestRuntime() || $flag("YEMU_PYTHON_SKIP_CHECK")) {
		return { ok: true };
	}
...
function buildInitScript(cwd: string, env?: Record<string, string | undefined>): string {
	...
	return [
		"import os, sys",
		`__yemu_cwd = ${JSON.stringify(cwd)}`,
		"os.chdir(__yemu_cwd)",
		`__yemu_env = ${JSON.stringify(envPayload)}`,
		"for __yemu_key, __yemu_val in __yemu_env.items():\n    os.environ[__yemu_key] = __yemu_val",
		"if __yemu_cwd not in sys.path:\n    sys.path.insert(0, __yemu_cwd)",
	].join("\n");
}
