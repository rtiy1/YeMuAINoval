...
export interface PythonExecutorOptions {
	 ...
	 * dir. When present, exported to the kernel as `YEMU_ARTIFACTS_DIR` and
	 * preferred over `YEMU_SESSION_FILE`-derived paths.
	 */
	artifactsDir?: string;
	 ...
	 * the kernel as `YEMU_EVAL_LOCAL_ROOTS` (JSON) so `write("local://x")` lands
