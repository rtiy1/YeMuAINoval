import { $env, logger } from "@yemu/utils";
 ...
 */
export const TINY_WORKER_ARG = "__yemu_worker_tiny_inference";
...
function readTinyModelSetting(path: "providers.tinyModelDevice" | "providers.tinyModelDtype"): string | undefined {
	try {
		...
		// Settings may be uninitialized (e.g. `yemu --smoke-test`); fall back to env/default.
		return undefined;
	}
}

/**
 * Decide which `YEMU_TINY_DEVICE` / `YEMU_TINY_DTYPE` vars to overlay onto the worker
...
export function tinyWorkerEnvOverlay(
	...
	if (!env.YEMU_TINY_DEVICE) {
		const device = tinyModelDeviceSettingToEnv(deviceSetting);
		if (device) overlay.YEMU_TINY_DEVICE = device;
	}
	if (!env.YEMU_TINY_DTYPE) {
		const dtype = tinyModelDtypeSettingToEnv(dtypeSetting);
		if (dtype) overlay.YEMU_TINY_DTYPE = dtype;
	}
	return overlay;
}
 ...
 * `YEMU_TINY_DEVICE` / `YEMU_TINY_DTYPE` env vars win; otherwise the persisted
