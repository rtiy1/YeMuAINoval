...
import { getAgentDir, isEnoent, logger } from "@yemu/utils";
 ...
 * key file live sessions use, per `~/.yemu/agent/secret-placeholder.key` in
 * docs/secrets.md.
 */
...
export async function loadSecrets(cwd: string, agentDir: string): Promise<SecretEntry[]> {
	const projectPath = path.join(cwd, ".yemu", "secrets.yml");
