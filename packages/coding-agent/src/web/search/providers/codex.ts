...
import { $env, readSseJson } from "@yemu/utils";
...
function getConfiguredModel(): CodexModelCandidate | undefined {
	const configuredModel = $env.YEMU_CODEX_WEB_SEARCH_MODEL?.trim();
 ...
 * - If `YEMU_CODEX_WEB_SEARCH_MODEL` is set, use it exactly once and surface any
...
export async function searchCodex(params: SearchParams): Promise<SearchResponse> {
	...
	if (transport.customEndpoint) {
		...
		if (!seed) {
			throw new Error(
				"No Codex OAuth credentials found. Login with 'yemu /login openai-codex' to enable Codex web search.",
			);
		}
