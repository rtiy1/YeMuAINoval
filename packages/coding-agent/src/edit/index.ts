import type { AgentTool, AgentToolContext, AgentToolResult, AgentToolUpdateCallback } from "@yemu/agent-core";
import { MismatchError as HashlineMismatchError } from "@yemu/hashline";
import hashlineGrammar from "@yemu/hashline/grammar.lark" with { type: "text" };
import hashlineDescription from "@yemu/hashline/prompt.md" with { type: "text" };
import type { ToolExample } from "@yemu/model-runtime";
import { isEnoent, isEnotdir, prompt } from "@yemu/utils";
...
function resolveConfiguredEditMode(rawEditMode: string): EditMode | undefined {
	...
	if (!editMode) {
		throw new Error(`Invalid YEMU_EDIT_VARIANT: ${rawEditMode}`);
	}
...
function resolveAllowFuzzy(session: ToolSession, rawValue: string): boolean {
	switch (rawValue) {
		...
		default:
			throw new Error(`Invalid YEMU_EDIT_FUZZY: ${rawValue}`);
	}
}
...
function resolveFuzzyThreshold(session: ToolSession, rawValue: string): number {
	...
	if (Number.isNaN(threshold) || threshold < 0 || threshold > 1) {
		throw new Error(`Invalid YEMU_EDIT_FUZZY_THRESHOLD: ${rawValue}`);
	}
...
export class EditTool implements AgentTool<TInput> {
	 ...
	 * fixes the shape of an edit. The Cursor `yemu_edit` frame carries
	...
	constructor(
		...
		const {
			YEMU_EDIT_FUZZY: editFuzzy = "auto",
			YEMU_EDIT_FUZZY_THRESHOLD: editFuzzyThreshold = "auto",
			YEMU_EDIT_VARIANT: envEditVariant = "auto",
		} = Bun.env;
