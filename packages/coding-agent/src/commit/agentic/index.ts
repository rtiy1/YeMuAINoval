...
import { $env, getProjectDir, isEnoent, prompt } from "@yemu/utils";
...
export async function runAgenticCommit(args: CommitCommandArgs): Promise<void> {
	...
	const forceFallback = $env.YEMU_COMMIT_TEST_FALLBACK?.toLowerCase() === "true";
	if (forceFallback) {
...
async function completeAgentCommitState(
	...
	if (!commitState.proposal && !commitState.splitProposal) {
		if ($env.YEMU_COMMIT_NO_FALLBACK?.toLowerCase() !== "true") {
