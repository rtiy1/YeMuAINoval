...
import { BracketedPasteHandler } from "@yemu/tui/bracketed-paste";
 ...
 * argument list, tolerating both the yemu `(theme)` and upstream-yemu
...
export class CustomEditor extends Editor {
	 ...
	 * the upstream-yemu `(tui, theme, keybindings)` convention. Undefined for yemu's
	...
	/**
	 * Accept both the yemu constructor convention — `new CustomEditor(theme)` —
	 * and the upstream-yemu `Editor` convention — `new Editor(tui, theme, keybindings)`
	 * — that {@link ExtensionUIContext.setEditorComponent}'s factory contract
	 * advertises `(tui, theme, keybindings)`. Plugins written against upstream yemu
