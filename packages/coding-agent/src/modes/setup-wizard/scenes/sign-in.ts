...
import { type Component, type Focusable, Input, matchesKey, type SgrMouseEvent, wrapTextWithAnsi } from "@yemu/tui";
		...
		try {
			await this.#authStorage.login(providerId as OAuthProvider, {
				...
				onAuth: info => {
					...
					// the YeMu-hosted `launchUrl` would resolve against the user's
