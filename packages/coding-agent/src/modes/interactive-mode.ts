...
import { type Agent, AgentBusyError, type AgentMessage, EventLoopKeepalive, ThinkingLevel } from "@yemu/agent-core";
import type { CompactionOutcome } from "@yemu/agent-core/compaction";
import { modelsAreEqual } from "@yemu/model-catalog/models";
import type { AssistantMessage, ImageContent, Message, Model, Usage, UsageReport } from "@yemu/model-runtime";
import type {
...
import { isInsideTerminalMultiplexer } from "@yemu/tui/terminal-capabilities";
import {
	$env,
	APP_NAME,
	adjustHsv,
...
export class InteractiveMode implements InteractiveModeContext {
	...
	async init(options: InteractiveModeInitOptions = {}): Promise<void> {
		...
		this.#signalTeardown = createSessionTeardown({
			...
			disposeSession: reason =>
				this.session.dispose({ yemuMemoryConsolidateTimeoutMs: SHUTDOWN_CONSOLIDATE_BUDGET_MS, reason }),
		});
		...
		// Start the UI. Cold `yemu` launch opts into clearing on the first paint so
		...
		setImmediate(() => {
			if (!$env.YEMU_NO_TITLE && !this.sessionManager.getSessionName()) {
		...
		// ignored. This way `yemu --continue` (or auto-resume) that finds no recent
	...
	#vibeParentSession(): VibeParentSession {
		return {
			...
			// yemu/task inheritance tracks the reopened session's model.
	...
	async shutdown(): Promise<void> {
		...
		try {
			if (this.#signalTeardown) {
				...
				await this.session.dispose({ yemuMemoryConsolidateTimeoutMs: SHUTDOWN_CONSOLIDATE_BUDGET_MS });
			}
		} finally {
		...
		// hands Bash the cursor immediately after visible YeMu content.
		...
		// terminal back (which would leave the parent shell with a `Y ⠋ …` tab).
		disposeTerminalTitleState();
		popTerminalTitle();
		this.stop();
