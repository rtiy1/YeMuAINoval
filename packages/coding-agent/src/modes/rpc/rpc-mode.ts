...
import { getOAuthProviders } from "@yemu/model-runtime/oauth";
import { isZodSchema, zodToWireSchema } from "@yemu/model-runtime/utils/schema";
import { $env, isRecord, readLines, Snowflake } from "@yemu/utils";
 ...
 * scheduling agent work through yemu.sendUserMessage() or yemu.sendMessage()
 ...
 *
 * `yemu.shutdown()` from an extension only *requests* shutdown; the process must
...
function shouldEmitRpcTitles(): boolean {
	const raw = $env.YEMU_RPC_EMIT_TITLE;
	if (!raw) return false;
...
export async function runRpcMode(
	...
	// may write there.
	process.env.YEMU_NOTIFICATIONS = "off";
	...
	class RpcExtensionUIContext implements ExtensionUIContext {
		...
		setTitle(title: string): void {
			// Title updates are low-value noise for most RPC hosts; opt in via YEMU_RPC_EMIT_TITLE=1.
	...
	// Deferred shutdown (yemu.shutdown() from an extension) must not kill the
	...
	const shutdownCoordinator = new RpcShutdownCoordinator({
		...
		performShutdown: async () => {
			...
			// dispose left YeMu-owned Chromium alive after RPC shutdown (#5643).
	...
	// prior yemu.shutdown() through the coordinator makes this await settle
	// immediately.
	await session.dispose();
	process.exit(0);
}
