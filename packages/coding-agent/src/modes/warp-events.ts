...
import { isInsideTmux, wrapTmuxPassthrough } from "@yemu/tui/terminal-capabilities";
import { VERSION } from "@yemu/utils/dirs";
	...
	return {
		emit(event): void {
			...
			const body = {
				...
				// Warp resolves this via CLIAgent.command_prefix(); YeMu is "yemu".
				agent: "yemu",
	...
	return api => {
		...
		api.on("tool_approval_requested", event => {
			emitter?.emit({
				...
				summary: `yemu wants to run ${event.toolName}`,
			});
		});
