...
import { type AutocompleteProvider, matchesKey, type SlashCommand } from "@yemu/tui";
import { $env, isEnoent, logger, sanitizeText } from "@yemu/utils";
...
const YEMU_STATUS_LINE_RE = /^\s*in:\s+\d+\s+out:\s+\d+(?:\s+cache\s+\S+)?\s+t:\s+\S+\s+tok\/s:\s+\S+/m;
...
export class InputController {
	 ...
	 * already-named sessions, YEMU_NO_TITLE, and low-signal greetings — no model or
	...
	handleCtrlZ(): void {
		...
		try {
			...
			// longer stops yemu: tokio swallows it and the TUI ends up torn down
			...
			//
			// pid=0 (foreground process group, not just our PID): yemu is not
			// always the shell's direct child. Package-manager launchers (`npx`,
			// `pnpm exec`, `bunx`, …) wait on the real CLI from a parent shim
			// that shares yemu's process group, and a `yemu … | tee log` style
	...
	async openExternalEditor(): Promise<void> {
		...
		try {
			...
			const result = await openInEditor(editorCmd, currentText, { extension: ".yemu.md", stdio });
			if (result !== null) {
