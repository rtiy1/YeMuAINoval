...
import { CompactionCancelledError, type CompactionOutcome } from "@yemu/agent-core/compaction";
import {
	getEnvApiKey,
	getProviderDetails,
...
import { Loader, Markdown, padding, Spacer, Text, visibleWidth } from "@yemu/tui";
import { formatDuration, Snowflake, sanitizeText } from "@yemu/utils";
	...
	async handleBashCommand(command: string, excludeFromContext = false): Promise<void> {
		...
		try {
			...
			try {
				...
				this.ctx.showError(
					`Bash command completed, but YeMu failed to update its working directory: ${
...
export function renderUsageReports(
	...
	for (const { provider, providerReports } of providerEntries) {
		...
		// Provider-wide disclaimers (e.g. "YeMu-observed spend only") render once
