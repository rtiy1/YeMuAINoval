 ...
 *
 * Used for:
 * - `yemu -p "prompt"` - text output
 * - `yemu --mode json "prompt"` - JSON event stream
 */
import type { AgentMessage } from "@yemu/agent-core";
import type { ImageContent } from "@yemu/model-runtime";
import { logger, sanitizeText } from "@yemu/utils";
	...
	if (mode === "text") {
		...
		if (assistantMsg) {
			...
			if (
				...
				// YeMu-owned Chromium survives this exit (issue #5643). `dispose()`
				...
				await session.dispose({ yemuMemoryConsolidateTimeoutMs: SHUTDOWN_CONSOLIDATE_BUDGET_MS });
