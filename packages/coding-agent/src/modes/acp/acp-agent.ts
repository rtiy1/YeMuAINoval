...
import { getBlobsDir, isEnoent, logger, type postmortem, VERSION } from "@yemu/utils";
...
export class AcpAgent implements Agent {
	...
	async initialize(params: InitializeRequest): Promise<InitializeResponse> {
		...
		const authMethods: AuthMethod[] = [
			{
				...
				description: "Authenticate via the provider keys/OAuth state already configured under ~/.yemu.",
			},
		];
		if (params.clientCapabilities?.auth?.terminal === true) {
			authMethods.push({
				type: "terminal",
				id: "terminal",
				name: "Set up YeMu Agent in terminal",
				description: "Launch the yemu TUI to add provider keys and select models.",
	...
	async #runPromptOrCommand(record: ManagedSessionRecord, text: string, images: AgentImageContent[]): Promise<void> {
		...
		// An ACP extension command can still call yemu.sendUserMessage(), which starts
