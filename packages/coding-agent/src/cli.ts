 ...
 * lightweight CLI runner from yemu-utils.
 */
import { parentPort } from "node:worker_threads";
import type { CliConfig } from "@yemu/utils/cli";
import { APP_NAME, getActiveProfile, MIN_BUN_VERSION, resolveProfileEnv, setProfile, VERSION } from "@yemu/utils/dirs";
import { interceptUnhandledRejections } from "@yemu/utils/postmortem";
import { setProcessName } from "@yemu/utils/process-name";
import { declareWorkerHostEntry, installWorkerInbox, isWorkerHostSelector } from "@yemu/utils/worker-host";
...
// the process entry, so the define-folded YEMU_COMPILED marker stands in.
const isProcessEntry = import.meta.main || process.env.YEMU_COMPILED === "true";
...
// `@yemu/utils/env` eagerly loads `.env` from the agent directory at
// import time, so it must not be imported before `setProfile` runs.

async function showHelp(config: CliConfig): Promise<void> {
	const { renderRootHelp } = await import("@yemu/utils/cli");
...
async function runSmokeTest(): Promise<void> {
	const { smokeTestSyncWorker, startServer } = await import("@yemu/stats");
	const { smokeTestTinyTitleWorker } = await import("./tiny/title-client");
	const { smokeTestSttWorker } = await import("./stt/asr-client");
	const { smokeTestTtsWorker } = await import("./tts/tts-client");
	const { smokeTestYeMuMemoryEmbedWorker } = await import("./yemu-memory/embed-client");
	...
	await smokeTestYeMuMemoryEmbedWorker();
	await smokeTestDaemonBroker();
	await smokeTestTerminalOutputWorker();
	process.stdout.write("smoke-test: ok\n");
}

const TINY_WORKER_ARG = "__yemu_worker_tiny_inference";
const STATS_SYNC_WORKER_ARG = "__yemu_worker_stats_sync";
const TAB_WORKER_ARG = "__yemu_worker_tab";
const JS_EVAL_WORKER_ARG = "__yemu_worker_js_eval";
const JS_EVAL_PROCESS_ARG = "__yemu_worker_js_eval_process";
const STT_WORKER_ARG = "__yemu_worker_stt";
const TTS_WORKER_ARG = "__yemu_worker_tts";
const YEMU_MEMORY_EMBED_WORKER_ARG = "__yemu_worker_yemu-memory_embed";
...
async function runWorkerEntrypoint(arg: string | undefined): Promise<boolean> {
	...
	if (arg === STATS_SYNC_WORKER_ARG) {
		...
		await import("@yemu/stats/sync-worker");
	...
	if (arg === YEMU_MEMORY_EMBED_WORKER_ARG) {
		const { startYeMuMemoryEmbedWorker } = await import("./yemu-memory/embed-worker");
		await runIpcSubprocessWorker(startYeMuMemoryEmbedWorker);
		return true;
	}
...
export async function runCli(argv: string[]): Promise<void> {
	...
	try {
		...
		if (extracted.profile !== undefined) {
			...
			// No explicit --profile: activate YEMU_PROFILE or YEMU_LEGACY_PROFILE inherited
			...
			// YEMU_PROFILE after importing this module (profile aliases/tests). Surfacing
			// validation here turns `YEMU_PROFILE=.. yemu --version` into a clean error;
			// calling setProfile keeps every later path helper on the env-selected
			// profile instead of the default agent directory.
			setProfile(resolveProfileEnv(process.env.YEMU_PROFILE, process.env.YEMU_LEGACY_PROFILE));
		}
		if (extracted.aliasName !== undefined) {
			const profile = extracted.profile ?? getActiveProfile();
			if (!profile) {
				throw new Error("--alias requires --profile <name> or YEMU_PROFILE");
			}
	...
	// `@yemu/utils/env` here would snapshot the wrong agent `.env`.
	...
	// `__yemu_worker_` dispatch, and importers (`runCli` in profile-CLI tests,
