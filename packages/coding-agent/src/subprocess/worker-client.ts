 ...
 * (yemu-memory embeddings, speech-to-text, tiny-model titles/completions, TTS).
 ...
 * `tts/yemu-memory/…: worker error` lines carry the actual stack instead of a bare
...
function createStderrCapture(exitLabel: string): StderrCapture {
	try {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "yemu-worker-stderr-"));
...
/**
 * Drive the ping/pong readiness probe wired into `yemu --smoke-test`: send one
