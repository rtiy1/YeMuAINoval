...
const COMPILED_TRANSFORMERS_VERSION = process.env.YEMU_TINY_TRANSFORMERS_VERSION;
...
export async function installSharpStubResolver(runtimeDir: string): Promise<string> {
	...
	const sharpStub = path.join(runtimeDir, "yemu-sharp-stub.cjs");
...
export async function ensureOnnxRuntimeCudaProviders(
	runtimeDir: string,
	device = process.env.YEMU_TINY_DEVICE,
): Promise<void> {
...
export interface TransformersRuntimeMetadata {
	__yemuRuntimeNodeModules?: string;
	__yemuTransformersEntry?: string;
	__yemuCudaRepairError?: string;
}
...
function attachTransformersRuntimeMetadata<T extends ConfigurableTransformers>(
	...
	runtime.__yemuRuntimeNodeModules = metadata.__yemuRuntimeNodeModules;
	runtime.__yemuTransformersEntry = metadata.__yemuTransformersEntry;
	runtime.__yemuCudaRepairError = metadata.__yemuCudaRepairError;
	return runtime;
}
...
function cudaFailureCause(
	...
	if (metadata.__yemuCudaRepairError) {
		return `ONNX Runtime CUDA provider install failed: ${metadata.__yemuCudaRepairError}`;
	}
...
function resolveOnnxRuntimePackageDir(metadata: TransformersRuntimeMetadata): string | null {
	const entry = metadata.__yemuTransformersEntry;
	if (entry) {
		try {
	...
	const nodeModules = metadata.__yemuRuntimeNodeModules;
...
export async function formatOnnxRuntimeCudaDiagnostics(
	...
	if (!packageDir) {
		return [
			...
			`  YEMU_TINY_DEVICE=${requestedDevice} requested CUDAExecutionProvider`,
	...
	const sideRuntime = metadata.__yemuRuntimeNodeModules;
	const lines = [
		"ONNX Runtime CUDA diagnostics:",
		`  YEMU_TINY_DEVICE=${requestedDevice} requested CUDAExecutionProvider`,
...
export function loadTransformersRuntime<T extends ConfigurableTransformers, K>(
	...
	return holder.load(async () => {
		...
		return attachTransformersRuntimeMetadata(configureTransformers(require_(entry) as T), {
			__yemuRuntimeNodeModules: path.join(installedDir, "node_modules"),
			__yemuTransformersEntry: entry,
			__yemuCudaRepairError: cudaRepairError,
		});
	});
}
