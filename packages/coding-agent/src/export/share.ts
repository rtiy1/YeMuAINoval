...
import type { AgentMessage, AgentState } from "@yemu/agent-core";
import type { AssistantMessage, ImageContent, TextContent } from "@yemu/model-runtime";
import { $which, logger } from "@yemu/utils";
...
async function tryCreateGist(sealed: Uint8Array): Promise<{ id: string; url: string } | null> {
	...
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-share-"));
	try {
