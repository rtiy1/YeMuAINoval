...
import { EventLoopKeepalive } from "@yemu/agent-core";
import type { ImageContent } from "@yemu/model-runtime";
import {
	$env,
	directoryExists,
	getLogPath,
...
import { injectYemuExtensionCliRoots } from "./discovery/yemu-extension-roots";
...
async function checkForNewVersion(currentVersion: string): Promise<string | undefined> {
	...
	try {
		const response = await fetch("https://registry.npmjs.org/@yemu/agent-runtime/latest", {
...
// re-apply at startup so embedders inherit YeMu's neutral defaults instead of
...
// YEMU_DEBUG_STARTUP markers for the synchronous-hang counterpart).
	...
	startupWatchdogTimer = setInterval(() => {
		...
		process.stderr.write(
			`${chalk.yellow(`Still starting after ${elapsed}s`)}${chalk.dim(` — phase: ${phase}`)}\n` +
				`${chalk.dim(`  logs: ${getLogPath()} · re-run with YEMU_DEBUG_STARTUP=1 for streaming phase markers`)}\n`,
		);
...
async function runInteractiveMode(
	...
	// `yemu join <link>`: dispatch through the same builtin path as a typed
	...
	if (parsed.fork) {
		...
		if (!match) {
			throw new SessionResolutionError(
				...
				"Run `yemu --resume` without an argument to pick from recent sessions, or `yemu` to start a new one.",
			);
		}
...
function discoverSystemPromptFile(): string | undefined {
	// Check project-local first (.yemu/SYSTEM.md)
...
export async function runRootCommand(
	...
	// the `yemu-plugins` discovery provider can surface their `skills/`, `hooks/`,
	...
	if (!parsedArgs.noExtensions) {
		...
		if (cliExtensions.length > 0) {
			injectYemuExtensionCliRoots(cliExtensions, home, getProjectDir());
		}
	}
	...
	const smolModel = parsedArgs.smol ?? $env.YEMU_SMOL_MODEL;
	const slowModel = parsedArgs.slow ?? $env.YEMU_SLOW_MODEL;
	const planModel = parsedArgs.plan ?? $env.YEMU_PLAN_MODEL;
	...
	// fresh persisted YeMu session before constructing the AgentSession.
	...
	if (mode === "acp") {
		...
		// Fail fast on stale/typo flags (e.g. `yemu --list-models`) now that we
		...
		const showStartupSplash = shouldShowStartupSplash({
			...
			timing: Boolean($env.YEMU_TIMING),
