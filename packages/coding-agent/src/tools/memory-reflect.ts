...
import { logger, untilAborted } from "@yemu/utils";
...
export class MemoryReflectTool implements AgentTool<typeof memoryReflectSchema> {
	...
	static createIf(session: ToolSession): MemoryReflectTool | null {
		...
		if (backend !== "hindsight" && backend !== "yemu-memory") return null;
		...
		return untilAborted(signal, async () => {
			...
			if (backend === "yemu-memory") {
				...
				try {
					...
					logger.warn("reflect failed", { backend: "yemu-memory", bank: state.config.bank, error: String(err) });
