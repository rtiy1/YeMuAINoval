...
import { type AstReplaceChange, type AstReplaceFileChange, astEdit } from "@yemu/native";
import type { Component } from "@yemu/tui";
import { replaceTabs, Text } from "@yemu/tui";
import { $envpos, prompt, untilAborted } from "@yemu/utils";
...
export class AstEditTool implements AgentTool<typeof astEditSchema, AstEditToolDetails> {
	...
	async execute(
		...
		return untilAborted(signal, async () => {
			...
			const maxFiles = $envpos("YEMU_MAX_AST_FILES", 1000);
