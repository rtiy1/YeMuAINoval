import type { AgentTelemetryConfig, AgentTool } from "@yemu/agent-core";
import type { Clipboard, InMemorySnapshotStore } from "@yemu/hashline";
import type { FetchImpl, ImageContent, Model, ServiceTierByFamily, ToolChoice } from "@yemu/model-runtime";
...
/** Tool type (AgentTool from model-runtime) */
...
export interface ToolSession {
	...
	/** Get YeMuMemory runtime state for this agent session. */
...
export async function createTools(session: ToolSession, toolNames?: string[]): Promise<Tool[]> {
	...
	if (requestedTools && !restrictToolNames) {
		...
		if (["hindsight", "yemu-memory"].includes(session.settings.get("memory.backend") ?? "")) {
			for (const name of ["recall", "retain", "reflect"]) {
				if (!requestedTools.includes(name)) requestedTools.push(name);
			}
		}
		if (session.settings.get("memory.backend") === "yemu-memory" && !requestedTools.includes("memory_edit")) {
		...
		if (session.settings.get("autolearn.enabled") && (session.taskDepth ?? 0) === 0) {
			...
			if (
				["hindsight", "yemu-memory", "local"].includes(session.settings.get("memory.backend") ?? "") &&
	...
	const isToolAllowed = (name: string) => {
		...
		if (name === "retain" || name === "recall" || name === "reflect") {
			return ["hindsight", "yemu-memory"].includes(session.settings.get("memory.backend") ?? "");
		}
		if (name === "memory_edit") return session.settings.get("memory.backend") === "yemu-memory";
