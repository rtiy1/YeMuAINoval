...
import { logger, postmortem, Snowflake, untilAborted } from "@yemu/utils";
...
export class CmuxTab {
	...
	async screenshot(opts: ScreenshotOptions = {}): Promise<string> {
		...
		const dest = context.session.browserScreenshotDir
			...
			: path.join(os.tmpdir(), `yemu-sshots-${Snowflake.next()}.${ext}`);
	...
	async #responseCursor(): Promise<number> {
		const value = await this.#evalScript<unknown>(
			"(() => Math.max(0, ((globalThis.__yemuCmuxResponses && globalThis.__yemuCmuxResponses.nextId) || 1) - 1))()",
		);
	...
	async #responseRecordsAfter(id: number): Promise<CmuxResponseRecord[]> {
		const value = await this.#evalScript<unknown>(
			`(() => ((globalThis.__yemuCmuxResponses && globalThis.__yemuCmuxResponses.records) || []).filter(record => record.id > ${JSON.stringify(id)}))()`,
		);
