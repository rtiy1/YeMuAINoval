...
export function serializeEvalWithEnvelope(fn: string | ((...args: unknown[]) => unknown), args: unknown[]): string {
	...
	return `(() => {
		try {
			const __v = (${expr});
			if (__v && typeof __v.then === "function") return { __yemuPromise: true };
			return { __yemuOk: __v === undefined ? null : __v };
		} catch (e) {
			return { __yemuErr: (e && (e.stack || e.message)) || String(e) };
		}
	})()`;
}
...
export function unwrapEvalEnvelope<TResult>(value: unknown, label: string): TResult {
	if (value && typeof value === "object") {
		if ("__yemuErr" in value && typeof value.__yemuErr === "string") {
			throw new ToolError(`${label} threw a JavaScript exception:\n${value.__yemuErr}`);
		}
		if ("__yemuPromise" in value && value.__yemuPromise === true) {
			throw new ToolError(
		...
		if ("__yemuOk" in value) {
			return value.__yemuOk as TResult;
		}
	}
...
export function resolveCmuxKind(
	...
	if (!resolveCmuxEnabled(env.YEMU_BROWSER_CMUX, options?.settingEnabled ?? true)) {
		return null;
	}
