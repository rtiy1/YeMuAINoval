 ...
 *
 * Enabled by default (`dev.autoqa` defaults to true); `YEMU_AUTO_QA=0` or an
 ...
 * read the cached decision without prompting. `YEMU_AUTO_QA_PUSH=1` bypasses
 * the dialog for headless environments.
 *
 * When the user grants consent, push is automatically active against the
 * bundled endpoint (`dev.autoqaPush.endpoint`, default `qa.yemu.sh`). Each
 * insert schedules a background flush that POSTs pending rows and deletes them
 * on HTTP 2xx. `YEMU_AUTO_QA_PUSH=1` forces push in non-interactive environments
...
import type { AgentToolResult } from "@yemu/agent-core";
import type { FetchImpl } from "@yemu/model-runtime";
import type { Component } from "@yemu/tui";
import { Text } from "@yemu/tui";
import { $env, $flag, getAutoQaDbPath, getInstallId, logger, VERSION } from "@yemu/utils";
 ...
 *
 * Precedence: `YEMU_AUTO_QA` env flag > explicit `dev.autoqa` setting >
...
export function isAutoQaEnabled(settings?: Settings): boolean {
	...
	return $flag("YEMU_AUTO_QA", fallback);
}
 ...
 * `~/.yemu/autoqa.db` (XDG: `$XDG_DATA_HOME/yemu/autoqa.db`), creating the
...
/**
 * Optional per-flush controls. Used by `yemu grievances push` to surface
...
function resolvePushConfig(settings: Settings | undefined, bypassConsent: boolean): PushConfig | null {
	...
	// covers explicit user-driven pushes (`yemu grievances push`) where the
	// user clearly intends to ship regardless of dialog state. The
	// `YEMU_AUTO_QA_PUSH` env flag stays as a CI/headless override too.
	if (!bypassConsent) {
		const consented = settings?.get("dev.autoqaConsent") === "granted";
		if (!consented && !$flag("YEMU_AUTO_QA_PUSH")) return null;
	}

	const endpoint = envOverrideString("YEMU_AUTO_QA_PUSH_URL") ?? settings?.get("dev.autoqaPush.endpoint");
	if (!endpoint || endpoint.trim().length === 0) return null;

	const token = envOverrideString("YEMU_AUTO_QA_PUSH_TOKEN") ?? settings?.get("dev.autoqaPush.token");
...
async function performFlush(db: Database, config: PushConfig, options: FlushOptions = {}): Promise<FlushResult> {
	...
	for (;;) {
		...
		const body = JSON.stringify({
			agent: { name: "yemu", version: VERSION },
 ...
 * `YEMU_AUTO_QA_PUSH=1` forces headless recording), and the device result
...
function recordToolIssue(session: ToolSession, tool: string, report: string): void {
	...
	lastRecordPipeline = (async () => {
		try {
			if (!$flag("YEMU_AUTO_QA_PUSH") && !(await resolveAutoQaConsent(session.settings))) return;
