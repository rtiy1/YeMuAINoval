...
import { getProjectDir, isEnoent, logger, prompt } from "@yemu/utils";
 ...
 * deployment shape of an editor spawning yemu as a co-hosted subprocess.
 */
...
/**
 * Mirrors yemu-shell's `uutils_env_disabled` gate for `YEMU_DISABLE_UUTILS_BUILTINS`:
...
function shellBuiltinsDisabled(settings: Settings): boolean {
	const raw = settings.getShellConfig().env?.YEMU_DISABLE_UUTILS_BUILTINS ?? Bun.env.YEMU_DISABLE_UUTILS_BUILTINS;
...
export class BashTool implements AgentTool<typeof bashSchemaBase | typeof bashSchemaWithAsync, BashToolDetails> {
	...
	async execute(
		...
		// must still cancel the call or job, but YeMu does not impose a deadline.
