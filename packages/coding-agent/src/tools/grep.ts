...
import type { AgentTool, AgentToolContext, AgentToolResult, AgentToolUpdateCallback, ToolTier } from "@yemu/agent-core";
import { formatHashlineHeader } from "@yemu/hashline";
import { type GrepMatch, GrepOutputMode, type GrepResult, grep } from "@yemu/native";
import type { Component } from "@yemu/tui";
import { Text } from "@yemu/tui";
import { prompt, untilAborted } from "@yemu/utils";
...
/** Mirrors `MAX_FILE_BYTES` in `crates/yemu-native/src/grep.rs`. Native grep
...
async function resolveArchiveSearchPaths(
	...
	for (let idx = 0; idx < pathSpecs.length; idx++) {
		...
		if (!tempDir) {
			tempDir = await mkdtemp(path.join(tmpdir(), "yemu-search-archive-"));
		}
...
const YEMU_ROOT_URL_RE = /^yemu:\/\/(?:\/?|docs\/?)$/i;
...
async function searchVirtualResources(
	...
	const dir = await mkdtemp(path.join(tmpdir(), "yemu-search-virtual-"));
	try {
...
async function expandVirtualInternalResource(
	...
	if (YEMU_ROOT_URL_RE.test(rawPath)) {
		const completions = await internalRouter.complete("yemu", "");
		if (completions && completions.length > 0) {
			...
			for (const completion of completions) {
				...
				const docUrl = `yemu://${completion.value}`;
 ...
 * wire bridges (the Cursor `yemu_grep` frame) whose protocol carries an explicit
	...
	async execute(
		...
		return untilAborted(signal, async () => {
			...
			try {
				...
				// output, and the only consumer that sets one (`yemu_grep`) has no `skip`
