...
import type { FetchImpl, ImageContent, TextContent } from "@yemu/model-runtime";
import { htmlToMarkdown } from "@yemu/native";
import { type Component, Text } from "@yemu/tui";
import { $which, truncate } from "@yemu/utils";
...
async function renderNotebookPayload(bytes: Uint8Array, displayUrl: string): Promise<string> {
	return withTempBinaryFile("yemu-url-notebook-", ".ipynb", bytes, tempPath =>
		readEditableNotebookText(tempPath, displayUrl),
	);
}

async function renderSqlitePayload(bytes: Uint8Array): Promise<string> {
	return withTempBinaryFile("yemu-url-sqlite-", ".sqlite", bytes, async tempPath => {
...
async function renderUrl(
	...
	// Handle internal protocol URLs (e.g., yemu-internal://) - return empty
	if (url.startsWith("yemu-internal://")) {
		return {
			url,
			finalUrl: url,
