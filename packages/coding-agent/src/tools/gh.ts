...
import { getWorktreeDir, hashPath, isEnoent, logger, prompt, untilAborted } from "@yemu/utils";
...
async function resolvePrBranchPushTarget(
	...
	const headRef = await git.config.getBranch(repoRoot, localBranch, "yemuPrHeadRef", signal);
	if (!headRef) {
	...
	const prUrl = await git.config.getBranch(repoRoot, localBranch, "yemuPrUrl", signal);
	const maintainerCanModifyValue = await git.config.getBranch(
		repoRoot,
		localBranch,
		"yemuPrMaintainerCanModify",
		signal,
	);
	const isCrossRepositoryValue = await git.config.getBranch(repoRoot, localBranch, "yemuPrIsCrossRepository", signal);
...
async function checkoutPullRequest(
	...
	return git.withRepoLock(
		...
		async () => {
			...
			await git.config.setBranch(repoRoot, localBranch, "yemuPrHeadRef", headRefName, signal);
			await git.config.setBranch(repoRoot, localBranch, "yemuPrUrl", data.url ?? "", signal);
			await git.config.setBranch(
				repoRoot,
				localBranch,
				"yemuPrIsCrossRepository",
