...
import { formatHashlineHeader, formatNumberedLine, formatNumberedLines } from "@yemu/hashline";
import type { ImageContent, TextContent } from "@yemu/model-runtime";
import { type SummaryResult, summarizeCode } from "@yemu/native";
...
export class ReadTool implements AgentTool<typeof readSchema, ReadToolDetails> {
	...
	#pdfImageCacheDir(absolutePdfPath: string, contentDigest: string): string {
		...
		if (root === undefined) {
			...
			root = sessionFile?.endsWith(".jsonl")
				...
				: path.join(os.tmpdir(), "yemu-read-pdf-images");
		}
		const basename = path
	...
	async #snapshotPdfSource(absolutePdfPath: string, signal?: AbortSignal): Promise<PdfImageSnapshot> {
		const directory = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-read-pdf-"));
		try {
	...
	async execute(
		...
		// Handle native YeMu URLs and custom-scheme resources advertised by MCP servers.
