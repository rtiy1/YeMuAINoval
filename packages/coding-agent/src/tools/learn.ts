 ...
 * memory backend — `hindsight`/`yemu-memory` (remote/SQLite) or `local` (the
...
export class LearnTool implements AgentTool<typeof learnSchema> {
	...
	static createIf(session: ToolSession): LearnTool | null {
		...
		if (backend !== "hindsight" && backend !== "yemu-memory" && backend !== "local") return null;
	...
	async execute(_id: string, params: LearnParams): Promise<AgentToolResult> {
		...
		if (backend === "yemu-memory") {
			const state = this.session.getYeMuMemorySessionState?.();
			if (!state) {
				throw new Error("YeMuMemory backend is not initialised for this session.");
			}
			...
			// disk error); mirror yemuMemoryBackend.save and fail loudly rather than
			// reporting (and minting a skill for) a lesson that was silently dropped.
			if (!id) {
				throw new Error("YeMuMemory did not store the lesson (no memory id returned).");
			}
