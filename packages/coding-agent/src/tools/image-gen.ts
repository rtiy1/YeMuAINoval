...
import { getAntigravityUserAgent } from "@yemu/model-catalog/wire/gemini-headers";
import { type ApiKey, type FetchImpl, getEnvApiKey, type Model, withAuth } from "@yemu/model-runtime";
import { ProviderHttpError } from "@yemu/model-runtime/error";
import { $env, isEnoent, parseImageMetadata, prompt, readSseJson, Snowflake, untilAborted } from "@yemu/utils";
...
import { resolveXAIHttpCredentials, yemuXAIUserAgent } from "../lib/xai-http";
...
async function saveImageToTemp(image: InlineImageData): Promise<string> {
	...
	const filename = `yemu-image-${Snowflake.next()}.${ext}`;
