...
import { CONFIG_DIR_NAME, getConfigAgentDirName, getProjectDir } from "@yemu/utils";
 ...
 *
 * Honors the `YEMU_PACKAGE_DIR` override (useful for Nix/Guix store paths);
 ...
 * project with yemu's own assets and was the source of issue #1423 (the host
 * project's `CHANGELOG.md` rendered as yemu's startup changelog).
 */
export function getPackageDir(): string | undefined {
	const envDir = process.env.YEMU_PACKAGE_DIR;
	if (envDir) {
...
/**
 * Path to yemu's own `CHANGELOG.md`, or `undefined` when the package directory
 ...
 * User-level: ~/.yemu/agent, ~/.claude, ~/.codex, ~/.gemini
 * Project-level: .yemu, .claude, .codex, .gemini
 */
...
export interface ConfigDirEntry {
	path: string;
	source: string; // e.g., ".yemu", ".claude"
	level: "user" | "project";
}

export interface GetConfigDirsOptions {
	/** Include user-level directories (~/.yemu/agent/...). Default: true */
	user?: boolean;
	/** Include project-level directories (.yemu/...). Default: true */
	project?: boolean;
 ...
 * // → [{ path: "~/.yemu/agent/commands", source: ".yemu", level: "user" }, ...]
 *
 * @example
 ...
 * Returns one entry per config base (.yemu, .claude) - the nearest one found.
