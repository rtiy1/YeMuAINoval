...
import type { WorkProfile } from "@yemu/native";
import { APP_NAME, getLogPath, getLogsDir, getReportsDir, isEnoent } from "@yemu/utils";
...
export async function createReportBundle(options: ReportBundleOptions): Promise<ReportBundleResult> {
	...
	const outputPath = path.join(reportsDir, `yemu-report-${timestamp}.tar.gz`);
 ...
 * after a crash still captures the fatal PID's `yemu.<date>.<pid>.log`. Files
