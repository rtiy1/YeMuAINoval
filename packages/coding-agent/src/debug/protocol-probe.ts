...
export interface ProtocolProbeOptions {
	...
	/** Whether the desktop notification was suppressed (e.g. `YEMU_NOTIFICATIONS=off`). */
...
export class ProtocolProbeComponent extends Container {
	constructor(options: ProtocolProbeOptions) {
		...
		this.addChild(
			new Text(
				[
					...
					`  \x1b]8;;https://github.com/rtiy1/YeMuAINoval\x07yemu-agent repo\x1b]8;;\x07`,
				].join("\n"),
				1,
				0,
			),
		...
		const notifyStatus = options.notificationSuppressed
			? theme.fg("warning", "suppressed (YEMU_NOTIFICATIONS)")
