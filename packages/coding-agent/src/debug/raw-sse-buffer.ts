import type { Model, ProviderResponseMetadata, RawSseEvent } from "@yemu/model-runtime";

const MAX_RAW_SSE_EVENTS = 1_000;
const MAX_RAW_SSE_CHARS = 512_000;
const MAX_RAW_SSE_EVENT_CHARS = 64_000;
// Reserve room for the `: yemu-debug-truncated` / `: yemu-debug-elided` marker
...
// (usage/finish_reason) both stay visible. A `: yemu-debug-elided` comment
...
function headTailTrim(lines: string[], budget: number, elidedTotal: number): string[] {
	...
	if (i <= j) {
		...
		out.push(`: yemu-debug-elided chars=${Math.max(0, elided)}`);
		if (tailSlice.length > 0) out.push(`…${tailSlice}`);
	} else if (elided > 0) {
		out.push(`: yemu-debug-elided chars=${elided}`);
	}
	out.push(...tail);
	return out;
}
...
// Any trimmed result ends with the `: yemu-debug-truncated` marker carrying
// the original size.
function trimRawLines(raw: string[]): TrimResult {
	...
	lines.push(`: yemu-debug-truncated originalChars=${originalChars}`);
...
export class RawSseDebugBuffer {
	...
	toRawText(): string {
		...
		const dropped = `: yemu-debug-dropped records=${this.#droppedRecords} chars=${this.#droppedChars}\n\n`;
