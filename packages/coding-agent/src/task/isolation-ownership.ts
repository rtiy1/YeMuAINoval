/**
 * Ownership marker for task-isolation sandboxes under `~/.yemu/wt/`.
 *
 * Each isolation base dir (`ensureIsolation` in {@link ./worktree}) holds a
 * compact `m` mount plus this marker file naming the yemu process that created
 * it. `yemu worktree clear` consults the marker so it can distinguish a live
...
export const ISOLATION_OWNER_FILE = ".yemu-isolation-owner.json";

/** Recorded owner of a task-isolation sandbox. */
export interface IsolationOwner {
	/** PID of the yemu process that created and owns the sandbox. */
	pid: number;
 ...
 * `yemu worktree clear` never sees an owner-less sandbox mid-creation.
 */
...
/**
 * Whether a live yemu process still owns the sandbox at `baseDir`.
 *
