...
import { $env, prompt, Snowflake } from "@yemu/utils";
...
function assertDepthAndSpawnAllowed(request: StructuredSubagentRequest, agentName: string): void {
	...
	const blockedAgent = request.blockedAgent ?? $env.YEMU_BLOCKED_AGENT;
...
async function leaseArtifacts(
	...
	const artifactsDir = path.join(
		os.tmpdir(),
		`${invocationKind === "eval" ? "yemu-eval-agent" : "yemu-task"}-${Snowflake.next()}`,
	);
	await fs.mkdir(artifactsDir, { recursive: true });
...
function buildExecutorOptions(
	...
	return {
		...
		parentYeMuMemorySessionState: session.getYeMuMemorySessionState?.(),
