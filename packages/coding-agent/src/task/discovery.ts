 ...
 *
 * Discovers agent definitions from YeMu-native task-agent roots:
 *   - ~/.yemu/agent/agents/*.md (user-level)
 *   - .yemu/agents/*.md (project-level)
 *   - <ext>/agents/*.md for every YeMu extension package wired through
 *     `listYemuExtensionRoots` (CLI `--extension` roots, `extensions:` in
 ...
 *     `hooks/`, `tools/`, etc. by `discovery/yemu-plugins.ts`.
 *
 ...
 * are intentionally skipped because their frontmatter schema is not the YeMu
 * task-agent contract.
 *
...
import { listYemuExtensionRoots } from "../discovery/yemu-extension-roots";
import { loadBundledAgents, parseAgent } from "./agents";
import type { AgentDefinition, AgentSource } from "./types";

const TASK_AGENT_CONFIG_SOURCE = ".yemu";
 ...
 * Precedence (highest wins): project `.yemu/agents`, user `.yemu/agents`,
 * YeMu extension-package agents in `listYemuExtensionRoots` source order
...
export async function discoverAgents(cwd: string, home: string = os.homedir()): Promise<DiscoveryResult> {
	...
	// YeMu extension-package agents/ dirs. `listYemuExtensionRoots` returns roots in
	...
	// surface in `discovery/yemu-plugins.ts`. Gate on `yemu-plugins` so
	// disabledProviders suppresses the whole extension-package surface.
	const extensionRoots = isProviderEnabled("yemu-plugins")
		? await listYemuExtensionRoots({ cwd: resolvedCwd, home, repoRoot: null })
		: [];
