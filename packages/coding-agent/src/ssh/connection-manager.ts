...
import { $which, getRemoteHostDir, getSshControlDir, isEnoent, logger, postmortem } from "@yemu/utils";
...
export interface SSHHostInfo {
	...
	shell: SSHHostShell;
	/**
	 * Shell name YeMu verified can execute the POSIX transfer snippets
 ...
 */
export const HOST_PROBE_MARKER = "YEMU_HOST_PROBE=";

/** Marker for the transfer-shell capability probe. */
export const TRANSFER_PROBE_MARKER = "YEMU_TRANSFER_OK|";
...
async function probeHostInfo(host: SSHConnectionTarget): Promise<SSHHostInfo> {
	...
	if (os === "windows" && host.compat !== false) {
		const bashProbe = await runSshCaptureSync(await buildRemoteCommand(host, 'bash -lc "echo YEMU_BASH_OK"'));
		if (bashProbe.exitCode === 0 && bashProbe.stdout.includes("YEMU_BASH_OK")) {
			compatShell = "bash";
		} else {
			const shProbe = await runSshCaptureSync(await buildRemoteCommand(host, 'sh -lc "echo YEMU_SH_OK"'));
			if (shProbe.exitCode === 0 && shProbe.stdout.includes("YEMU_SH_OK")) {
				compatShell = "sh";
			}
		}
