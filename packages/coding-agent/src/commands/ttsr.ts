...
/**
 * `yemu ttsr` — inspect and test Time-Traveling Stream Rules.
 *
 * `yemu ttsr test` feeds a snippet (inline, --file, or stdin) through the real
 * TTSR matching pipeline and reports which rules would trigger. `yemu ttsr list`
 * shows every TTSR-registered rule the current project/user config would load.
 */
import { Args, Command, Flags } from "@yemu/utils/cli";
import {
	runTtsrCommand,
	TTSR_ACTIONS,
	TTSR_SOURCES,
...
export default class Ttsr extends Command {
	...
	static examples = [
		"yemu ttsr list",
		"yemu ttsr test 'const x: any = 1'",
		"yemu ttsr test src/foo.ts",
		"yemu ttsr test --file src/foo.ts",
		"yemu ttsr test --file src/foo.ts --source text",
		"yemu ttsr test --rule .yemu/rules/no-any.md --source tool --path src/foo.ts 'const x: any = 1'",
		"echo 'Box::leak(&mut v)' | yemu ttsr test --file - --path src/lib.rs",
		"yemu ttsr test --source tool --tool edit --path src/foo.ts 'const x: any = 1'",
		"yemu ttsr scan",
		"yemu ttsr scan src/",
		"yemu ttsr scan -r .yemu/rules/no-any.md src/",
	];
	...
	async run(): Promise<void> {
		...
		// inline text — so `yemu ttsr test src/foo.ts` works without --file.
