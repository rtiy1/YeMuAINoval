/**
 * List and clean up agent-managed git worktrees under `~/.yemu/wt`.
 */
import { getProjectDir } from "@yemu/utils";
...
export default class Worktree extends Command {
	static description = "List or clear agent-managed git worktrees (~/.yemu/wt)";

	static aliases = ["wt"];

	static args = {
		// `list` (default) inspects the worktree dir; `clear` removes entries.
		// A positional action keeps `yemu worktree` (the no-arg form) useful.
		action: Args.string({
	...
	static examples = [
		"yemu worktree",
		"yemu worktree list --json",
		"yemu worktree clear",
		"yemu worktree clear --dry-run",
		"yemu worktree clear --all",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(Worktree);
		// Load settings so the `worktree.base` override is applied before we scan
		// — otherwise this command would inspect ~/.yemu/wt while the agent created
