/**
 * `yemu install <target>` — top-level convenience over `yemu plugin install` /
 * `yemu plugin link`.
 *
 * The docs (yemu.sh/docs/extension-authoring) advertise
 *
 *   yemu install ./my-extension
 *
...
/**
 * Heuristic used to decide whether `yemu install <target>` should `link` a
...
export default class Install extends Command {
	...
	async run(): Promise<void> {
		...
		if (targets.length === 0) {
			process.stderr.write("Usage: yemu install <path | npm-spec | name@marketplace> [...]\n");
			process.exit(1);
		}
