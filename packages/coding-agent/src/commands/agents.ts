...
export default class Agents extends Command {
	...
	static flags = {
		...
		user: Flags.boolean({ description: "Write to ~/.yemu/agent/agents (default)" }),
		project: Flags.boolean({ description: "Write to ./.yemu/agents" }),
	};

	static examples = [
		"# Export bundled agents into user config (default)\n  yemu agents unpack",
		"# Export bundled agents into project config\n  yemu agents unpack --project",
		"# Overwrite existing local agent files\n  yemu agents unpack --project --force",
		"# Export into a custom directory\n  yemu agents unpack --dir ./tmp/agents --json",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(Agents);
		if (!args.action) {
			renderCommandHelp("yemu", "agents", Agents);
			return;
		}
