/**
 * `yemu auth-broker` — manage the yemu credential vault.
 */
import { Args, Command, Flags, renderCommandHelp } from "@yemu/utils/cli";
import {
	AUTH_BROKER_ACTIONS,
...
export default class AuthBroker extends Command {
	static description = "Manage the yemu auth-broker (credential vault)";
	...
	static examples = [
		"# Boot the broker against the local SQLite store\n  yemu auth-broker serve",
		"# Boot on a non-default port\n  yemu auth-broker serve --bind=127.0.0.1:9000",
		"# Print the bearer token\n  yemu auth-broker token",
		"# Rotate the bearer token\n  yemu auth-broker token --regenerate",
		"# List supported OAuth providers\n  yemu auth-broker list",
		"# Local login (run on the broker host)\n  yemu auth-broker login anthropic",
		"# Interactive provider selection\n  yemu auth-broker login",
		"# Remote login over SSH tunnel\n  yemu auth-broker login anthropic --via=user@broker",
		"# Log out of a provider (interactive without provider arg)\n  yemu auth-broker logout anthropic",
		"# Import a CLIProxyAPI auth dump\n  yemu auth-broker import ~/.cliproxy/auth",
		"# Import a single CLIProxyAPI JSON, overriding the provider mapping\n  yemu auth-broker import ~/.cliproxy/auth/claude-foo.json --provider anthropic",
		"# Preview a migration from local store + env vars to the configured broker\n  yemu auth-broker migrate --from-local --include-env --dry-run",
		"# Apply the migration\n  yemu auth-broker migrate --from-local --include-env",
		"# Health-check the configured remote broker\n  yemu auth-broker status",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(AuthBroker);
		if (!args.action) {
			renderCommandHelp("yemu", "auth-broker", AuthBroker);
			return;
		}
