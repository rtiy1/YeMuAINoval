...
export default class Say extends Command {
	...
	static examples = [
		'yemu say "hello world"',
		"yemu say --file notes.md --voice bm_fable",
		'yemu say "hello world" --out /tmp/hello.wav',
	];
	...
	#synthesisFailed(model: string): void {
		process.stderr.write(
			chalk.red(
				`error: could not synthesize with local TTS model "${model}". Run \`yemu setup speech\` to install it.\n`,
			),
		);
	}
}
