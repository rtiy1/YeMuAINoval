...
export default class Index extends Command {
	...
	static flags = {
		...
		smol: Flags.string({
			description: "Smol/fast model for lightweight tasks (or YEMU_SMOL_MODEL env)",
		}),
		slow: Flags.string({
			description: "Slow/reasoning model for thorough analysis (or YEMU_SLOW_MODEL env)",
		}),
		plan: Flags.string({
			description: "Plan model for architectural planning (or YEMU_PLAN_MODEL env)",
		}),
		...
		"from-claude": Flags.boolean({
			description: "Import a Claude Code session into YeMu",
		}),
		"from-codex": Flags.boolean({
			description: "Import a Codex session into YeMu",
		}),
	...
	static examples = [
		...
		`# Create a shell shortcut for a work profile\n  ${APP_NAME} --profile work --alias yemu-work`,
		`# Use different model (fuzzy matching)\n  ${APP_NAME} --model opus "Help me refactor this code"`,
		`# Limit model cycling to specific models\n  ${APP_NAME} --models claude-sonnet,claude-haiku,gpt-4o`,
		`# Export a session file to HTML\n  ${APP_NAME} --export ~/.yemu/agent/sessions/--path--/session.jsonl`,
	];
