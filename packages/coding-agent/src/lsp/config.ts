...
import { $which, isRecord, logger, pathIsWithin, type WhichOptions } from "@yemu/utils";
...
function getConfigSources(cwd: string): ConfigSource[] {
	...
	// Project config directories (.yemu/, .claude/)
	...
	// User config directories (~/.yemu/agent/, ~/.claude/)
 ...
 * 2. Project config dirs: .yemu/lsp.*, .claude/lsp.* (+ hidden variants)
 * 3. User config dirs: ~/.yemu/agent/lsp.*, ~/.claude/lsp.* (+ hidden variants)
