 ...
 *
 * yemu-agent's agent core (`@yemu/agent-core`) emits OpenTelemetry GenAI
 ...
 * vars are set so `yemu` can be observed by any OTLP collector without vendor
 * coupling.
 *
...
import type {
	...
	ChatUsageEvent,
	ToolStatus,
} from "@yemu/agent-core";
import { logger, postmortem } from "@yemu/utils";

/**
 * Periodic flush interval. A long-lived `yemu` process (the ACP server is
...
async function registerProviders(signalConfig: SignalConfig): Promise<void> {
	...
	if (signalConfig.metric) {
		...
		metricRecorder = new AgentMetricRecorder(metrics.getMeter("@yemu/agent-runtime"));
	}
	...
	if (signalConfig.log) {
		...
		otelLogger = logProvider.getLogger("@yemu/agent-runtime");
...
class AgentMetricRecorder {
	...
	constructor(meter: Meter) {
		...
		this.#chatCostUsd = meter.createCounter("yemu.agent.agent.chat.cost.estimated_usd", {
			description: "Estimated USD cost for completed chat calls.",
			unit: "USD",
		});
		this.#runs = meter.createCounter("yemu.agent.agent.runs", {
			description: "Completed agent runs.",
			unit: "{run}",
		});
		this.#steps = meter.createCounter("yemu.agent.agent.steps", {
			description: "Agent loop steps completed inside a run.",
			unit: "{step}",
		});
		this.#chatCalls = meter.createCounter("yemu.agent.agent.chat.calls", {
			description: "Chat calls completed inside agent runs.",
			unit: "{call}",
		});
		this.#chatDurationMs = meter.createHistogram("yemu.agent.agent.chat.duration", {
			description: "Total chat latency observed in an agent run.",
			unit: "ms",
		});
		this.#toolCalls = meter.createCounter("yemu.agent.agent.tool.calls", {
			description: "Tool calls completed inside agent runs.",
			unit: "{call}",
		});
		this.#toolDurationMs = meter.createHistogram("yemu.agent.agent.tool.duration", {
			description: "Total tool latency observed in an agent run.",
			unit: "ms",
		});
		this.#errors = meter.createCounter("yemu.agent.agent.errors", {
	...
	recordChatUsage(event: ChatUsageEvent): void {
		const baseAttrs = metricAttributes({
			...
			"yemu.gen_ai.agent.id": event.agent?.id,
			"yemu.gen_ai.agent.name": event.agent?.name,
		});
	...
	recordRun(summary: AgentRunSummary, coverage: AgentRunCoverage): void {
		const runAttrs = metricAttributes({
			"yemu.agent.agent.models_used.count": coverage.modelsUsed.length,
			"yemu.agent.agent.providers_used.count": coverage.providersUsed.length,
			"yemu.agent.agent.tools_available.count": coverage.toolsAvailable.length,
			"yemu.agent.agent.tools_invoked.count": coverage.toolsInvoked.length,
			"yemu.agent.agent.tools_unused.count": coverage.toolsUnused.length,
		});
		...
		for (const toolName in summary.tools.byName) {
			...
			for (const status of TOOL_STATUSES) {
				...
				if (count > 0)
					this.#toolCalls.add(count, metricAttributes({ ...toolAttrs, "yemu.agent.tool.status": status }));
			}
		}
...
function emitRunSummaryLog(summary: AgentRunSummary, coverage: AgentRunCoverage): void {
	emitOtelLog(
		...
		"agent run completed",
		{
			"yemu.agent.agent.step_count": summary.stepCount,
			"yemu.agent.agent.chats.total": summary.chats.total,
			"yemu.agent.agent.chats.total_latency_ms": summary.chats.totalLatencyMs,
			"yemu.agent.agent.tools.total": summary.tools.total,
			"yemu.agent.agent.tools.ok": summary.tools.ok,
			"yemu.agent.agent.tools.error": summary.tools.error,
			"yemu.agent.agent.tools.skipped": summary.tools.skipped,
			"yemu.agent.agent.tools.blocked": summary.tools.blocked,
			"yemu.agent.agent.tools.timeout": summary.tools.timeout,
			"yemu.agent.agent.tools.aborted": summary.tools.aborted,
			"yemu.agent.agent.tools.total_latency_ms": summary.tools.totalLatencyMs,
			"yemu.agent.agent.usage.input_tokens": summary.usage.inputTokens,
			"yemu.agent.agent.usage.output_tokens": summary.usage.outputTokens,
			"yemu.agent.agent.usage.cached_input_tokens": summary.usage.cachedInputTokens,
			"yemu.agent.agent.usage.cache_write_tokens": summary.usage.cacheWriteTokens,
			"yemu.agent.agent.usage.reasoning_output_tokens": summary.usage.reasoningOutputTokens,
			"yemu.agent.agent.usage.total_tokens": summary.usage.totalTokens,
			"yemu.agent.agent.cost.estimated_usd": summary.cost.estimatedUsd,
			"yemu.agent.agent.cost.unavailable_reasons": summary.cost.unavailableReasons.join(","),
			"yemu.agent.agent.errors.total": summary.errors.total,
			"yemu.agent.agent.coverage.tools_available": coverage.toolsAvailable.join(","),
			"yemu.agent.agent.coverage.tools_invoked": coverage.toolsInvoked.join(","),
			"yemu.agent.agent.coverage.tools_unused": coverage.toolsUnused.join(","),
			"yemu.agent.agent.coverage.models_used": coverage.modelsUsed.join(","),
			"yemu.agent.agent.coverage.providers_used": coverage.providersUsed.join(","),
		},
		"yemu.agent.agent.run.completed",
	);
}
...
function emitTelemetryWarningLog(warning: AgentTelemetryWarning): void {
	...
	emitOtelLog("warn", warning.message, attrs, "yemu.agent.telemetry.warning");
}
