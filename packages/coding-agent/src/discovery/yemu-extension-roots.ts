 ...
 * `prompts/`, `.mcp.json`) are wired into discovery by `yemu-plugins.ts`.
 *
 * CLI-provided paths are injected via {@link injectYemuExtensionCliRoots}
 * before discovery runs; settings paths are read lazily from
 * `<scope>/settings.json` in {@link listYemuExtensionRoots} to mirror what
...
import { getAgentDir, isEnoent, logger, tryParseJson } from "@yemu/utils";
...
export interface YemuExtensionRoot {
 ...
 * the registered set; {@link clearYemuExtensionCliRoots} resets for tests.
 */
export function injectYemuExtensionCliRoots(paths: readonly string[], home: string, cwd: string): void {
...
export function clearYemuExtensionCliRoots(): void {
	injectedCliRoots = [];
}

/** Inspect currently-injected CLI roots (read-only). Exposed for diagnostics + tests. */
export function getInjectedYemuExtensionCliRoots(): readonly YemuExtensionRoot[] {
...
function scopeDirs(ctx: LoadContext): ScopeDirs {
	return {
		project: path.join(ctx.cwd, ".yemu"),
		user: getAgentDir(),
	};
}
 ...
 * are dropped):
 *
 * 1. CLI roots injected via {@link injectYemuExtensionCliRoots}
 * 2. Project `<cwd>/.yemu/settings.json#extensions`
 * 3. User `~/.yemu/agent/settings.json#extensions`
 * 4. Enabled npm/link plugins installed under `<plugins>/node_modules/` (for
 *    `yemu install <pkg>` / `yemu plugin install` / `yemu plugin link`). Marketplace
 ...
 */
export async function listYemuExtensionRoots(ctx: LoadContext): Promise<YemuExtensionRoot[]> {
 ...
 * "Claude Code Marketplace" and "YeMu Extension Packages".
 */
