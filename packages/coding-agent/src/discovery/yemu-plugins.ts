/**
 * YeMu extension-package sub-discovery provider.
 *
 ...
 * `rules/`, `prompts/`, and `.mcp.json` — are picked up by yemu's standard
 * discovery surfaces. The native `yemu` provider in `builtin.ts` only walks
 * `.yemu/` and `~/.yemu/agent/`, so without this provider those sub-trees are
 * silently ignored.
 *
 * Provider priority is set below the native `yemu` provider (100) so an
 * extension package never shadows the user's own `.yemu/` configuration on
 * dedup.
 *
 * @see ./yemu-extension-roots.ts
 * @see ../../docs/extension-loading.md
 */
import * as path from "node:path";
import { logger, parseFrontmatter, tryParseJson } from "@yemu/utils";
...
import { listYemuExtensionRoots, type YemuExtensionRoot } from "./yemu-extension-roots";

const PROVIDER_ID = "yemu-plugins";
const DISPLAY_NAME = "YeMu Extension Packages";
const DESCRIPTION =
...
async function loadSkills(ctx: LoadContext): Promise<LoadResult<Skill>> {
	const roots = await listYemuExtensionRoots(ctx);
...
async function loadHooks(ctx: LoadContext): Promise<LoadResult<Hook>> {
	const roots = await listYemuExtensionRoots(ctx);
	const tasks: Array<{ root: YemuExtensionRoot; hookType: "pre" | "post" }> = [];
...
async function loadMCPServers(ctx: LoadContext): Promise<LoadResult<MCPServer>> {
	...
	const tasks: Array<{ root: YemuExtensionRoot; mcpPath: string }> = [];
	for (const root of roots) {
	...
	for (let i = 0; i < tasks.length; i++) {
		...
		if (!parsed) {
			warnings.push(`[yemu-plugins] Invalid JSON in ${mcpPath}`);
			logger.warn(`[yemu-plugins] Invalid JSON in ${mcpPath}`);
			continue;
		}
		...
		for (const [serverName, serverCfg] of Object.entries(servers)) {
			...
			if (typeof cfg.command !== "string" && typeof cfg.url !== "string") {
				warnings.push(`[yemu-plugins] Skipping MCP server "${serverName}" in ${mcpPath}: missing command or url`);
				continue;
			}
