...
import { logger, parseFrontmatter } from "@yemu/utils";
...
async function loadHooks(ctx: LoadContext): Promise<LoadResult<Hook>> {
	...
	// YeMu hooks must be named `pre-<tool>.<ts|js>` or `post-<tool>.<ts|js>`.
	// Files without that prefix are not YeMu hooks (e.g. the standalone Codex
