...
import { FileType, glob } from "@yemu/native";
import {
	CONFIG_DIR_NAME,
	getAgentDir,
	getConfigDirName,
...
async function readExtensionModuleManifest(
	...
	const pkg = tryParseJson<{ yemu?: ExtensionModuleManifest }>(content);
	const manifest = pkg?.yemu;
 ...
 * 3. Subdirectory with package.json: `extensions/<ext>/package.json` with "yemu" field → load declared paths
 *
 ...
 *
 * Walk order:
 * 1. Walk up from `cwd` looking for the nearest directory containing `.yemu/`.
 *    The first match returns `<dir>/.yemu/plugins/installed_plugins.json`.
 * 2. If no `.yemu/` is found, rescan from `cwd` upward looking for `.git`.
 *    The git root is used as an anchor: `<gitRoot>/.yemu/plugins/installed_plugins.json`.
...
export async function resolveActiveProjectRegistryPath(cwd: string): Promise<string | null> {
	// Pass 1: walk up looking for an existing .yemu/ directory (nearest wins).
	// Stop before os.homedir() — ~/.yemu/ is the user-level config dir, not a project root.
...
/**
 * Like resolveActiveProjectRegistryPath, but falls back to `<cwd>/.yemu/plugins/installed_plugins.json`
 * when no project anchor (.yemu/ or .git/) is found.
 *
 * Use this when the caller accepts an explicit --scope project so that installing into a freshly
 * bootstrapped directory (no .yemu/ or .git/ yet) works: writeInstalledPluginsRegistry auto-creates
 ...
 * Reads ~/.claude/plugins/installed_plugins.json and ~/.yemu/plugins/installed_plugins.json,
...
export async function listClaudePluginRoots(
	...
	// ── YeMu installed plugins registry ───────────────────────────────────────
	// YeMu registry is authoritative: its entries replace Claude's entries for the same plugin ID.
	...
	const yemuRegistryPath = path.join(getPluginsDir(home), "installed_plugins.json");
	const yemuContent = await readFile(yemuRegistryPath);
	if (yemuContent) {
		const yemuRegistry = parseClaudePluginsRegistry(yemuContent);
		if (yemuRegistry) {
			for (const [pluginId, entries] of Object.entries(yemuRegistry.plugins)) {
				if (!Array.isArray(entries) || entries.length === 0) continue;
				...
				// YeMu is authoritative: drop all Claude-sourced entries for this plugin ID
			...
			warnings.push(`Failed to parse YeMu plugin registry: ${yemuRegistryPath}`);
		}
	}

	// ── Project-scoped YeMu registry ────────────────────────────────────────
	// Loaded from the nearest .yemu/plugins/installed_plugins.json relative to cwd.
 ...
 * These are prepended to the cache with highest precedence (before YeMu/Claude entries).
