 ...
 *
 * A `yemu_grep` frame carries its own context width and total match cap. Neither
...
/**
 * Build the `replace`-mode `edit` the bridge answers `yemu_edit` with.
 *
 * `YemuEditExecArgs` carries `old_text`/`new_text` pairs, which is exactly
 ...
 *
 * `yemu_edit` needs a `replace`-mode instance, but only when `edit` was granted:
