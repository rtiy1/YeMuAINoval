...
import { isEexist, isEisdir, isEnoent, postmortem } from "@yemu/utils";
...
/** Close every project broker connection held by this yemu process. */
...
export async function smokeTestDaemonBroker(): Promise<void> {
	const projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-daemon-smoke-project-"));
	const runtimeDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-daemon-smoke-run-"));
