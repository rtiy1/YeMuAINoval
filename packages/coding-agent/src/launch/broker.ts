...
import { Process, type PtyRunResult, PtySession } from "@yemu/native";
import { isEexist, isEnoent, logger, postmortem, sanitizeText, setProcessName } from "@yemu/utils";
...
export async function startDaemonBrokerFromEnvironment(): Promise<void> {
	...
	if (!lease) return;
	setProcessName("yemu daemon broker");
