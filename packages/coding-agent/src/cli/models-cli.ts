/**
 * `yemu models` — list, search, and refresh available models.
 *
 * Subcommands:
 ...
 *   then list. This is the supported replacement for `rm -rf ~/.yemu/models.db`
...
import type { Api, Effort, Model } from "@yemu/model-runtime";
import { formatNumber, getProjectDir } from "@yemu/utils";
 ...
 * name doubles as an `yemu models <provider>` shortcut.
 */
...
/** `yemu models ls`/`find`: provider-grouped listing (one box table per provider). */
...
/**
 * Entry point for the standalone `yemu models` command: bootstraps auth storage,
...
export async function runModelsCommand(command: ModelsCommandArgs): Promise<void> {
	...
	if (action === "find" && (!pattern || pattern.trim().length === 0)) {
		process.stderr.write("`yemu models find` requires a search substring, e.g. `yemu models find minimax`\n");
		process.exitCode = 1;
		return;
	}
