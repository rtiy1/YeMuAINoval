 ...
 * `@yemu/utils/env`, which eagerly loads `.env` from the agent directory
 ...
 * doesn't look like another flag. Without this, `yemu --system-prompt --profile
 ...
 * `yemu --some-ext-flag --profile work` still selects a profile. Known value-less
 * launch flags ({@link VALUELESS_FLAGS}) are exempt so a trailing profile after
 * them also activates (`yemu --print --profile work`).
 */
 ...
 * positionals are never stolen (`yemu grep --profile <path>` greps for
 * `--profile`; it does not select a profile). `launch` and `acp` are explicit
 * spellings of launch-shaped commands, so `yemu launch --profile work` and
 * `yemu acp --profile work` still select profile `work`.
 *
...
export function extractProfileFlags(argv: readonly string[]): ProfileBootstrapResult {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		// successor only when it is value-like — `yemu --bar val --profile work` keeps
		...
		// a trailing global flag still applies (`yemu --some-ext-bool --profile work`
		...
		// activates (`yemu --print --profile work`).
