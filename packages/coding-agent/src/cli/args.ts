 ...
 */
import { APP_NAME, CONFIG_DIR_NAME, logger } from "@yemu/utils";
 ...
 * setter table itself can stay free of `@yemu/utils` runtime imports
...
export function getExtraHelpText(): string {
  ...
  YEMU_PROFILE                 - Named profile for isolated agent state (same as --profile)
  Use \`yemu --profile <name> --alias <command>\` to create a shell shortcut for a profile
  YEMU_CODING_AGENT_DIR        - Session storage directory (default: ~/${CONFIG_DIR_NAME}/agent)
  YEMU_PACKAGE_DIR             - Override package directory (for Nix/Guix store paths)
  YEMU_SMOL_MODEL              - Override smol/fast model (see --smol)
  YEMU_SLOW_MODEL              - Override slow/reasoning model (see --slow)
  YEMU_PLAN_MODEL              - Override planning model (see --plan)
  YEMU_NO_PTY                  - Disable PTY-based interactive bash execution
...
${chalk.bold("Useful Commands:")}
  yemu agents unpack           - Export bundled subagents to ~/.yemu/agent/agents (default)
  yemu agents unpack --project - Export bundled subagents to ./.yemu/agents`;
}
