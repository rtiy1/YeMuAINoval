/**
 * `yemu auth-gateway` command handlers.
 *
 * Boots a forward-proxy server that lets less-trusted clients (the macOS
 * usage widget, yemu containers, …) make provider API calls without ever
 ...
 * `YEMU_AUTH_BROKER_URL` / `auth.broker.url` precedence used elsewhere).
 *
 * Sub-verbs:
...
import { type GeneratedProvider, getBundledModels } from "@yemu/model-catalog/models";
import {
	type Api,
	AuthStorage,
...
} from "@yemu/model-runtime/auth-broker";
import { DEFAULT_AUTH_GATEWAY_BIND, startAuthGateway } from "@yemu/model-runtime/auth-gateway";
import { getConfigRootDir, isEnoent, logger, VERSION } from "@yemu/utils";
...
async function runServe(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"`yemu auth-gateway serve` requires YEMU_AUTH_BROKER_URL (or `auth.broker.url`/`auth.broker.token` in config.yml). The gateway is itself a broker client.",
		);
	}
	...
	// cached + broker-discovered), so the discovery-only models yemu itself
	...
	// a client-requested `model` field into a model-runtime `Model<Api>` before dispatch;
...
async function runStatus(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		...
		if (flags.json) {
			...
			process.stdout.write(`${chalk.yellow("No broker configured.")} Set YEMU_AUTH_BROKER_URL.\n`);
			process.stdout.write(
	...
	try {
		...
		if (flags.json) {
			...
			if (!tokenPresent) {
				process.stdout.write(
					"Run `yemu auth-gateway token` or `yemu auth-gateway serve` to create a bearer token.\n",
				);
			}
		}
 ...
 * - `cursor-agent` and `yemu-native` (gateway forwarding) have transport quirks
 ...
 * yemu-native transport (would loop through the gateway), and placeholder /
...
function pickProbeCandidates(provider: string): Model<Api>[] {
	...
	const candidates = bundled.filter(model => {
		if (model.transport === "yemu-native") return false;
...
/**
 * `yemu auth-gateway check` — probe each broker-supplied credential and print
...
async function runCheck(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"`yemu auth-gateway check` requires YEMU_AUTH_BROKER_URL (or `auth.broker.url`/`auth.broker.token` in config.yml). It probes the same credentials the gateway would serve.",
		);
	}
