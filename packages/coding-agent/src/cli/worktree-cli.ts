/**
 * CLI handler for `yemu worktree` — list and clean up agent-managed worktrees.
 *
 * Layout under `~/.yemu/wt/`:
 *
 ...
 *     marker naming the live yemu process; a
...
import { getWorktreesDir, isEnoent } from "@yemu/utils";
...
export interface WorktreeEntry {
	/** Absolute path to the worktree dir (or stray container) under `~/.yemu/wt/`. */
	path: string;
	...
	branch?: string;
	/** When set, the entry is unhealthy and `yemu worktree clear` will remove it. */
	orphanReason?: string;
}
...
async function scanWorktrees(): Promise<WorktreeEntry[]> {
	...
	for (const name of topLevel) {
		...
		// Legacy nesting: ~/.yemu/wt/<encoded-project>/<branch-or-id>
		let children: string[];
		try {
