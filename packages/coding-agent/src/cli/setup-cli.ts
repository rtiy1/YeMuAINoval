 ...
 *
 * Handles `yemu setup` for onboarding and `yemu setup <component>` for optional dependencies.
 */
import * as path from "node:path";
import { $which, APP_NAME, getProjectDir, getPythonEnvDir } from "@yemu/utils";
import { $ } from "bun";
...
// package dependencies beyond a working interpreter. `yemu setup python --check`
...
/**
 * Unified `yemu setup speech` flow. Drives every {@link SpeechComponent} through
