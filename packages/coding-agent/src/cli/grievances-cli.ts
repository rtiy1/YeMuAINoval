/**
 * CLI handler for `yemu grievances` — view, clean, and manually push reported tool issues.
 */
...
// ───────────────────────────────────────────────────────────────────────────
// Manual push (`yemu grievances push`)
// ───────────────────────────────────────────────────────────────────────────
 ...
 *
 * Requires endpoint configuration (default `qa.yemu.sh/v1/grievances`).
 */
export async function pushGrievances(options: PushGrievancesOptions): Promise<void> {
	...
	try {
		...
		if (result.skipped) {
			console.log(
				chalk.yellow(
					"Push skipped — no endpoint configured. Set `dev.autoqaPush.endpoint` or `YEMU_AUTO_QA_PUSH_URL`.",
				),
			);
			return;
		}
