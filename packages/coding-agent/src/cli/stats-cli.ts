 ...
 *
 * Handles `yemu stats` subcommand for viewing AI usage statistics.
 */

import { truncateToWidth } from "@yemu/tui/utils";
import { APP_NAME, formatDuration, formatNumber, formatPercent } from "@yemu/utils";
...
async function printStatsSummary(): Promise<void> {
	const { getDashboardStats } = await import("@yemu/stats");
	const stats = await getDashboardStats();
