...
import { normalizeProfileName } from "@yemu/utils/dirs";
...
// Keep local: importing the yemu-utils root here would eagerly load env before
...
function validateAliasName(aliasName: string, shell: ProfileAliasShell): string {
	...
	if (normalized.toLowerCase() === "yemu") {
		throw new Error('Invalid alias "yemu". Refusing to shadow the base yemu command.');
	}
...
function resolveShellConfigPath(
	...
	switch (shell) {
		...
		case "fish": {
			...
			return posixJoinUnc(configHome, "fish", "conf.d", "yemu-profiles.fish");
		}
		case "pwsh":
...
function renderAliasBlock(
	...
	const start = `# >>> yemu profile alias: ${aliasName} >>>`;
	const end = `# <<< yemu profile alias: ${aliasName} <<<`;
	let body: string;
	switch (shell) {
		case "fish":
			body = [
				`function ${aliasName} --wraps yemu --description 'YeMu profile ${profile}'`,
