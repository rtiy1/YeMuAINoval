 ...
 *
 * Handles `yemu usage` — fetches provider usage reports for every
...
import { formatDuration, formatNumber, sanitizeText } from "@yemu/utils";
...
/**
 * Tombstones worth a row in `yemu usage`: OAuth credentials torn down
 ...
 * `yemu usage --provider xai` can still confirm the stored credential has no
 * usage endpoint.
 */
		...
		if (cmd.history) {
			...
			if (entries.length === 0) {
				...
				process.stderr.write(
					chalk.yellow(
						`No usage history recorded${scope} yet. Snapshots accumulate whenever usage is fetched (TUI footer, /usage, yemu usage).\n`,
					),
				);
				process.exitCode = 1;
				return;
		...
		if (filteredReports.length === 0 && accounts.length === 0) {
			...
			const message =
				storedAccounts.length > 0
					...
					: `No credentials found${scope}. Run \`yemu\` and use /login to add accounts.\n`;
