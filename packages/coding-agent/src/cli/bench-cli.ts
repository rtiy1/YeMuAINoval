import type { ResolvedThinkingLevel } from "@yemu/agent-core";
...
import { resolveModelServiceTier, streamSimple } from "@yemu/model-runtime";
import { replaceTabs, truncateToWidth } from "@yemu/tui";
import { formatDuration, getProjectDir, prompt } from "@yemu/utils";
...
const CACHE_PREFIX_PLACEHOLDER = "__YEMU_CACHE_BENCH_RAW_PREFIX__";
...
interface BenchRequestOptions {
	...
	sessionId: string;
	prompt: string;
	/** Native YeMu messages; cache mode splits the stable prefix from the suffix. */
...
async function runBenchRequest(
	...
	try {
		...
		const stream = streamFn(model, context, {
			...
			// model-runtime opts every OpenRouter request into response caching (1h TTL).
...
export async function runBenchCommand(command: BenchCommandArgs, deps: BenchDependencies = {}): Promise<BenchSummary> {
	...
	if (command.models.length === 0) {
		throw new Error("Pass at least one model selector, e.g. `yemu bench opus gpt-5.2`");
	}
		...
		for (const { selector, model, thinking } of targets) {
			...
			if (!preflightKey) {
				const failure: BenchRunFailure = {
					ok: false,
					error: `No credentials for provider "${model.provider}". Run \`yemu\` and use /login, or set the provider API key.`,
				};
