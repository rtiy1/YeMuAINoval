 ...
 *
 * Handles `yemu update` to check for and install updates.
 * Uses the installer that owns the active yemu executable when it can be detected.
 */
...
import { $env, $which, APP_NAME, isEnoent, VERSION } from "@yemu/utils";
import { $ } from "bun";
import chalk from "chalk";
import { theme } from "../modes/theme/theme";
import { isTimeoutError, withTimeoutSignal } from "../utils/fetch-timeout";

const REPO = "rtiy1/YeMuAINoval";
const PACKAGE = "@yemu/agent-runtime";
const HOMEBREW_FORMULA = APP_NAME;
const MISE_TOOL = "github:rtiy1/YeMuAINoval";
/**
 ...
 */
const NATIVES_PACKAGE = "@yemu/native";

/**
 * Platform tags the release pipeline publishes as
 * `@yemu/native-<tag>` leaves. Mirrors `SUPPORTED_PLATFORMS` in
...
async function getHomebrewFormulaPrefix(): Promise<string | undefined> {
	...
	for (const formula of [HOMEBREW_FORMULA]) {
		try {
...
function isPathInDirectory(filePath: string, directoryPath: string): boolean {
	...
	// PATH-resolved yemu path can refer to the same directory through different
	...
	// links like Homebrew's `bin/yemu -> Cellar/.../bin/yemu`; the parent fallback
...
interface UpdateMethodResolutionOptions {
	...
	npmBinDir?: string;
	/**
	 * Whether the resolved yemu path is a plain file (the standalone binary)
...
function resolveUpdateMethod(
	...
	const { homebrewPrefix, miseBinDirs = [], miseDataDir, npmBinDir, yemuIsRegularFile = false } = options;
	const launcherExtension = path.extname(yemuPath).toLowerCase();
	const isWindowsScriptLauncher =
		launcherExtension === ".cmd" || launcherExtension === ".ps1" || launcherExtension === ".bat";
	if (homebrewPrefix && isPathInDirectory(yemuPath, path.join(homebrewPrefix, "bin"))) return "brew";
	if (miseBinDirs.some(dir => isPathInDirectory(yemuPath, dir))) return "mise";
	if (miseDataDir && isPathInDirectory(yemuPath, path.join(miseDataDir, "shims"))) return "mise";
	...
	const isStandaloneRegularFile = yemuIsRegularFile && process.platform !== "win32";
	if (bunBinDir && isPathInDirectory(yemuPath, bunBinDir) && !isStandaloneRegularFile) return "bun";
	if ((npmBinDir && isPathInDirectory(yemuPath, npmBinDir) && !isStandaloneRegularFile) || isWindowsScriptLauncher)
		return "npm";
	return "binary";
}
...
export function resolveUpdateMethodForTest(
	...
	return resolveUpdateMethod(yemuPath, bunBinDir, options);
}
async function resolveUpdateTarget(): Promise<UpdateTarget> {
	...
	const yemuPath = resolveYemuPath();
	...
	if (yemuPath) {
		...
		let yemuIsRegularFile = false;
		try {
			const stat = fs.lstatSync(yemuPath);
			yemuIsRegularFile = stat.isFile() && !stat.isSymbolicLink();
		} catch {}
		const method = resolveUpdateMethod(yemuPath, bunBinDir, {
			homebrewPrefix,
			miseBinDirs,
			miseDataDir,
			npmBinDir,
		...
		if (method === "binary") return { method, path: yemuPath };
		return { method };
	}
 ...
 * (`react@19.2.6@@@1`). Global `yemu` updates can leave one full copy per
...
async function pruneBunCacheAfterGlobalInstall(): Promise<BunInstallCachePruneResult | undefined> {
	...
	const packageNames = globalNodeModulesDir
		...
		: new Set<string>();
	if (packageNames.size === 0 && !path.basename(cacheDir).toLowerCase().includes("yemu")) return undefined;
...
/**
 * Resolve the path that `yemu` maps to in the user's PATH.
 */
function resolveYemuPath(): string | undefined {
	return $which(APP_NAME) ?? undefined;
}

/**
 * Run the resolved yemu binary and check if it reports the expected version.
 */
async function verifyInstalledVersion(expectedVersion: string): Promise<InstalledVersionVerification> {
	const yemuPath = resolveYemuPath();
	if (!yemuPath) return { ok: false };
	try {
		const result = await $`${yemuPath} --version`.quiet().nothrow();
		if (result.exitCode !== 0) return { ok: false, path: yemuPath };
		const output = result.text().trim();
		// Output format: "yemu/X.Y.Z"
		const match = output.match(/\/(\d+\.\d+\.\d+)/);
		const actual = match?.[1];
		return { ok: actual === expectedVersion, actual, path: yemuPath };
	} catch {
		return { ok: false, path: yemuPath };
	}
}
...
async function printVerification(expectedVersion: string): Promise<void> {
	...
	console.log(chalk.yellow(`You may need to reinstall: curl -fsSL https://github.com/rtiy1/YeMuAINoval/install | sh`));
}
...
/**
 * Build the bun argv used to globally install a specific yemu version.
 *
 ...
 *
 * Together these two flags make `yemu update` produce exactly the registry
 * lookup the version check just performed. See #1686.
 *
 * Also pins {@link NATIVES_PACKAGE} and the platform-specific
 * `@yemu/native-<tag>` leaf to `expectedVersion`. `bun install -g`
 ...
 * `@yemu/agent-runtime` and the loader aborts at
...
export async function runUpdateCommand(opts: { force: boolean; check: boolean }): Promise<void> {
	...
	// Choose update method based on the prioritized yemu binary in PATH
	try {
