 ...
 */
import { ExponentialYield } from "@yemu/agent-core/utils/yield";
import { type MinimizerOptions, Shell, type ShellRunResult } from "@yemu/native";
import { isCmdShell, isExecutable, type ShellConfig } from "@yemu/utils/procmgr";
...
export async function executeBash(command: string, options?: BashExecutorOptions): Promise<BashResult> {
	...
	if (deadlineTimeoutMs !== undefined) {
		...
		timeoutTimer = setTimeout(() => {
			// Explicit timeouts are enforced inside yemu-native via `timeoutMs`.
