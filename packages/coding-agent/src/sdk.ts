...
import { FALLBACK_DIALECT, preferredDialect } from "@yemu/model-catalog/identity";
import type {
	Context,
...
import type { Dialect } from "@yemu/model-runtime/dialect";
import {
	getOpenAICodexTransportDetails,
	prewarmOpenAICodexResponses,
} from "@yemu/model-runtime/providers/openai-codex-responses";
import type { Component } from "@yemu/tui";
import { $env, $flag, getAgentDir, getProjectDir, logger, postmortem, prompt, Snowflake } from "@yemu/utils";
import { INTENT_FIELD } from "@yemu/wire";
import {
...
import type { YeMuMemorySessionState } from "./yemu-memory/state";
...
export interface CreateAgentSessionOptions {
	...
	/** Global config directory. Default: ~/.yemu/agent */
	agentDir?: string;
	...
	/**
	 * Pre-discovered custom-tool source paths from `.yemu/tools/`, `.claude/tools/`,
	...
	/** Prompt templates. Default: discovered from cwd/.yemu/prompts/ + agentDir/prompts/ */
	...
	/** Parent YeMuMemory state to alias for subagent memory tools. */
	parentYeMuMemorySessionState?: YeMuMemorySessionState;
	...
	settings?: Settings;
	/**
	 * Legacy alias for `settings`. Older YeMu extensions pass SettingsManager.create(...)
 ...
 *
 * Broker mode: when `YEMU_AUTH_BROKER_URL` is set, credentials are pulled from
 ...
 * (`yemu bench`, dry-balance) build a bare {@link ModelRegistry} that only knows
 ...
 * `~/.yemu/agent/extensions/`) never reach model resolution. Mirrors the
 * session / `yemu models` path: drain the queued provider registrations, then
...
function isLegacyBuiltinToolDefinition(tool: CustomTool | ToolDefinition): boolean {
	return !isCustomTool(tool) && "__yemuLegacyBuiltinTool" in tool && tool.__yemuLegacyBuiltinTool === true;
}
 ...
 *
 * // With explicit model
 * import { getModel } from '@yemu/model-runtime';
 * const { session } = await createAgentSession({
...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	try {
		...
		const toolSession: ToolSession = {
			...
			getYeMuMemorySessionState: () => session?.getYeMuMemorySessionState(),
		...
		if (!restrictToolNames) {
			...
			// Discover custom tools from `.yemu/tools/`, `.claude/tools/`, plugins, etc.
		...
		// cache that `yemu models find` uses before the online refresh continues in
		// the background.
		...
		if (!model && deferredModelPatterns.length === 0) {
			...
			if (!model) {
				...
				// `yemu models` (which awaits discovery) lists the model. Await one
		...
		// bridge is a different consumer: the server sends native `yemu_edit`
		// frames regardless of the advertised catalog, and answering them needs
		// a real tool.
		//
		// It must be a `replace`-mode instance. `YemuEditExecArgs` carries
		...
		const cursorExecHandlers = new CursorExecHandlers({
			...
			// `yemu_edit` needs the `replace`-mode instance specifically, and the
			...
			// `yemu_grep` carries its own context width and match cap, which the
		...
		const intentField = $flag("YEMU_INTENT_TRACING", settings.get("tools.intentTracing")) ? INTENT_FIELD : undefined;
		const includeWorkspaceTree = settings.get("includeWorkspaceTree") ?? false;
		...
		session = new AgentSession({
			...
			// advisor's own tool session so a `yemu_grep` frame's context width and
		...
		const startMemoryBackend = async () => {
			...
			await memoryBackend.start({
				...
				parentYeMuMemorySessionState: options.parentYeMuMemorySessionState,
			});
		};
