#!/usr/bin/env bun
/**
 * Generate edit benchmark cases from TypeScript sources in a legacy-agent-monorepo-style repo.
 *
...
import { prompt, TempDir } from "@yemu/utils";
import { $ } from "bun";
...
using DEFAULT_SOURCE_REPO_DIR = TempDir.createSync("@legacy-agent-monorepo-source");
...
const DEFAULT_SOURCE_REPO_URL = "https://github.com/badlogic/legacy-agent-monorepo.git";
...
async function ensureSourceRepo(typescriptDir: string): Promise<void> {
	...
	console.log(`Cloning legacy-agent-monorepo repository to ${typescriptDir}…`);
	...
	if (result.exitCode !== 0) {
		...
		throw new Error(`Failed to clone legacy-agent-monorepo: ${stderr.trim()}`);
	}
