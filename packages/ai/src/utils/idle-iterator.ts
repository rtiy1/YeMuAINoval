 ...
 *
 * `YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS` is accepted as a backward-compatible alias.
 * Set `YEMU_STREAM_IDLE_TIMEOUT_MS=0` to disable the watchdog.
 *
...
export function getStreamIdleTimeoutMs(fallbackMs: number = DEFAULT_STREAM_IDLE_TIMEOUT_MS): number | undefined {
	return normalizeIdleTimeoutMs(
		$env.YEMU_STREAM_IDLE_TIMEOUT_MS ?? $env.YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS,
		fallbackMs,
	);
}
 ...
 *
 * `YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS` takes precedence over the generic
 * `YEMU_STREAM_IDLE_TIMEOUT_MS` because some deployments tune OpenAI-compatible
 * backends separately from Anthropic/Gemini-style transports.
 *
 * Set `YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS=0` to disable the watchdog.
 */
export function getOpenAIStreamIdleTimeoutMs(fallbackMs: number = DEFAULT_STREAM_IDLE_TIMEOUT_MS): number | undefined {
	return normalizeIdleTimeoutMs(
		$env.YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS ?? $env.YEMU_STREAM_IDLE_TIMEOUT_MS,
		fallbackMs,
	);
}
 ...
 *
 * Set `YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS=0` to disable the watchdog.
 *
...
export function getStreamFirstEventTimeoutMs(
	...
	return normalizeIdleTimeoutMs($env.YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS, fallback);
}

/**
 * Returns the first-event timeout used for OpenAI-family streaming transports.
 *
 * Precedence: explicit `YEMU_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS` (including a
 ...
 * `streamIdleTimeoutMs` or `YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS` resolved
 * upstream) floors the first-event budget so slow OpenAI-compatible servers
 * are not undercut by a shorter `YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS` or the
...
export function getOpenAIStreamFirstEventTimeoutMs(
	...
	const openAIFirstEventRaw = $env.YEMU_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS;
	if (openAIFirstEventRaw !== undefined) {
		return normalizeIdleTimeoutMs(openAIFirstEventRaw, fallbackMs) ?? 0;
	}
	const base = normalizeIdleTimeoutMs($env.YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS, fallbackMs);
