...
/**
 * Normalizes provider id (e.g. github-copilot -> YEMU_PROXY_GITHUB_COPILOT) and looks it up.
 * If not found, falls back to YEMU_PROXY. Results are memoized because env values are static
...
export function getProxyForProvider(provider: string): string | undefined {
	...
	const envKey = `YEMU_PROXY_${normalized}`;
	const value = Bun.env[envKey] || Bun.env.YEMU_PROXY;
