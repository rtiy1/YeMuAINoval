...
function getOAuthClient(): { clientId: string; clientSecret: string } {
	const clientId = $env.YEMU_GOOGLE_ANTIGRAVITY_CLIENT_ID?.trim();
	const clientSecret = $env.YEMU_GOOGLE_ANTIGRAVITY_CLIENT_SECRET?.trim();
	if (!clientId || !clientSecret) {
		throw new AIError.OAuthError(
			"Antigravity OAuth requires YEMU_GOOGLE_ANTIGRAVITY_CLIENT_ID and YEMU_GOOGLE_ANTIGRAVITY_CLIENT_SECRET.",
			{ kind: "configuration", provider: "google-antigravity" },
		);
	}
