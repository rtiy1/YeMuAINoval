...
import { getGeminiCliHeaders } from "@yemu/model-catalog/wire/gemini-headers";
...
function getOAuthClient(): { clientId: string; clientSecret: string } {
	const clientId = $env.YEMU_GOOGLE_GEMINI_CLIENT_ID?.trim();
	const clientSecret = $env.YEMU_GOOGLE_GEMINI_CLIENT_SECRET?.trim();
	if (!clientId || !clientSecret) {
		throw new AIError.OAuthError(
			"Gemini CLI OAuth requires YEMU_GOOGLE_GEMINI_CLIENT_ID and YEMU_GOOGLE_GEMINI_CLIENT_SECRET.",
