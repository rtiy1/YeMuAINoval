...
import { isVertexExpressOpenAIUrl, isVertexRawPredictUrl, resolveVertexEndpointHost } from "@yemu/model-catalog/hosts";
import {
	...
	resolveWireModelId,
} from "@yemu/model-catalog/model-thinking";
import { CATALOG_PROVIDERS, type ProviderCatalogEntry } from "@yemu/model-catalog/provider-models";
import { CODEX_BASE_URL } from "@yemu/model-catalog/wire/codex";
import { $env, $pickenv, getConfigRootDir, isEnoent, logger, withExtraCaFetch } from "@yemu/utils";
...
import { streamYemuNative } from "./providers/yemu-native-client";
 ...
 * Used by `yemu auth-broker migrate --include-env` to discover env-sourced keys
...
function streamDispatch<TApi extends Api>(
	...
	switch (api) {
		...
		case "openrouter": {
			const useResponses = $env.YEMU_OPENROUTER_RESPONSES !== "0";
			if (useResponses) {
...
export function streamSimple<TApi extends Api>(
	...
	// YeMu-native transport short-circuits the per-provider dispatch entirely:
	...
	// yemu-native transport.
	if (model.transport === "yemu-native") {
		return withGeminiThinkingLoopGuard(model, requestOptions, opts =>
			withProviderInFlightLimit(model, opts, () => streamYemuNative(model, context, opts)),
		);
	}
...
const ANTHROPIC_USE_INTERLEAVED_THINKING = Bun.env.YEMU_NO_INTERLEAVED_THINKING !== "1";
...
function isOpenAIResponsesPromptCacheSurface<TApi extends Api>(model: Model<TApi>): boolean {
	return (
		...
		(model.api === "openrouter" && $env.YEMU_OPENROUTER_RESPONSES !== "0")
	);
}
