...
import { supportsAllTurnsReasoningContext, supportsCodexReasoningSummary } from "@yemu/model-catalog/identity";
import { requireSupportedEffort } from "@yemu/model-catalog/model-thinking";
 ...
 * explicit option wins, then the `YEMU_CODEX_RESPONSES_LITE` env override
...
export function resolveCodexResponsesLite(
	...
	const env = $env.YEMU_CODEX_RESPONSES_LITE?.trim().toLowerCase();
