...
import { isAnthropicSigningProxyUrl, isOfficialAnthropicApiUrl } from "@yemu/model-catalog/compat/anthropic";
import { hostMatchesUrl, isVertexRawPredictUrl } from "@yemu/model-catalog/hosts";
import { mapEffortToAnthropicAdaptiveEffort } from "@yemu/model-catalog/model-thinking";
import { calculateCost, getBundledModel } from "@yemu/model-catalog/models";
import { isAnthropicOAuthToken } from "@yemu/model-catalog/utils";
import { parseGitHubCopilotApiKey } from "@yemu/model-catalog/wire/github-copilot";
import {
	$env,
	getInstallId,
	isEnoent,
...
function getCacheControl(
	...
	// breakpoint cold-misses the entire prefix on resume. YEMU_CACHE_RETENTION
...
const CLAUDE_DEVICE_ID_INSTALL_HASH_DOMAIN = "yemu-claude-device-id-v1:";
const CLAUDE_DEVICE_ID_ACCOUNT_HASH_DOMAIN = "yemu-claude-device-id-v2";
...
function resolveEagerToolInputStreamingSupport(
	...
	// override (`yemu.registerProvider("anthropic", { baseUrl })`) or Foundry
...
export function maybeAddReplayUnsignedThinkingHint(model: Model<"anthropic-messages">, message: string): string {
	...
	const hint = `Provider "${model.provider}" looks like an Anthropic-compatible signing proxy: it rejected a replayed unsigned thinking block. Set \`compat.replayUnsignedThinking: false\` under \`providers.${model.provider}\` in your models.yml and retry. See upstream-change-4297.`;
	return `${hint}\n\n${message}`;
}
...
/**
 * Serialize yemu {@link Message}s to Anthropic wire messages.
 *
