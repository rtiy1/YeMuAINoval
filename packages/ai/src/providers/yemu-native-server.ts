/**
 * YeMu-native wire format for the auth-gateway.
 *
 * Where the OpenAI / Anthropic / Responses route modules translate foreign
 * wire shapes through model-runtime's canonical {@link Context}, this module accepts
 * the canonical shape *directly* — for clients that already speak model-runtime
 * (containerized yemu, the swarm extension, yemu's sidecar auth-gateway).
 ...
 * translations impose on first-class model-runtime fields (service tier, cache
 * markers, thinking budgets, tool-choice variants, …).
 *
 * The streaming wire is {@link AssistantMessageEvent} serialized verbatim and
 * SSE-framed. Same type model-runtime already produces internally; the client feeds
...
export interface YemuNativeParsedRequest {
	modelId: string;
	context: Context;
 ...
 * `SimpleStreamOptions` from older / newer yemu builds without per-version
 * conditionals.
 */
...
/**
 * Parse a yemu-native request body. Validation is intentionally minimal — only
 * the shape the gateway itself reads is checked (`modelId`, `context.messages`
 * array, options is an object). Everything downstream is the canonical model-runtime
 ...
 */
export function parseRequest(body: unknown, _headers?: Headers): YemuNativeParsedRequest {
	...
	// `stream` defaults to true — yemu-native clients overwhelmingly stream, and
 ...
 *
 * No per-event re-shaping: the yemu-native client is model-runtime itself, so the
 ...
 * targets (containerized yemu → host gateway, yemu slot → yemu-auth-gateway
...
/**
 * YeMu-native error envelope:
 *   `{ error: { type, message } }`
 *
 * Mirrors OpenAI's outer shape (which clients/SDKs already parse) without the
 * provider-specific status taxonomy — yemu-native callers consume `type`
 * directly.
 */
