/**
 * OpenAI Responses HTTP wire-format ↔ yemu Context bridge for the auth-gateway.
 *
 * Inbound: parses `POST /v1/responses` request bodies into a {@link ParsedRequest}.
 * Outbound: encodes yemu's {@link AssistantMessage} (and event stream) back into
...
import { logger, structuredCloneJSON } from "@yemu/utils";
...
const UNSUPPORTED_EXPLICIT_PROMPT_CACHE_MESSAGE =
	"openai-responses: prompt_cache_options and prompt_cache_breakpoint are unsupported by this auth-gateway route; use /v1/yemu/stream with options.promptCache instead";
...
function mapToolChoice(value: ParsedToolChoice | undefined): ParsedRequest["options"]["toolChoice"] {
	...
	if ("type" in value) {
		...
		// Other hosted tools + allowed_tools are not surfaced to model-runtime.
		return "auto";
	}
	return undefined;
}
...
export function parseRequest(body: unknown, headers?: Headers): ParsedRequest {
	...
	if (typeof data.input === "string") {
		...
		for (const item of data.input) {
			...
			if (effectiveType === "custom_tool_call") {
				...
				// matching model-runtime's openai-shared convention, and tag the call
	...
	// visible summary. model-runtime has no per-level plumbing — log once and let the
	...
	// `store` is a stateful-storage hint that yemu's gateway doesn't honour;
...
/**
 * model-runtime responses providers mint composite `"{call_id}|{item_id}"` tool-call
				...
				for await (const ev of events) {
					...
					switch (ev.type) {
						...
						case "text_delta": {
							...
							// TODO: when model-runtime surfaces output_text annotations
						...
						case "toolcall_end": {
							...
							if (cur.customWireName) {
								...
								// Standard JSON tool: arguments object on the yemu side, the
