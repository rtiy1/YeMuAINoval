...
/**
 * Parsed inbound OpenAI chat-completions request, ready to feed into model-runtime
...
function parseUserLikeContent(
	...
	for (const part of content) {
		...
		// schema for forward-compat but dropped here — model-runtime's canonical user
 ...
 * Tool-result content may carry images alongside text; model-runtime's
...
export function encodeResponse(message: AssistantMessage, requestedModelId: string): Record<string, unknown> {
	...
	const responseMessage: Record<string, unknown> = {
		...
		// model-runtime does not surface real refusals yet; emit `null` so SDKs that
...
function mapFinishReason(reason: StopReason, hasToolCalls: boolean): string {
	...
	// model-runtime's StopReason does not currently carry a content-filter signal;
...
export function encodeStream(
	...
	return new ReadableStream<Uint8Array>({
		async start(controller) {
			// contentIndex (from model-runtime events) -> tool_calls index on the wire.
