...
import type { McpToolDefinition } from "@yemu/model-catalog/discovery/cursor-gen/agent_pb";
import {
...
import { calculateCost } from "@yemu/model-catalog/models";
import { $env, parseJsonWithRepair, parseStreamingJson, parseStreamingJsonThrottled, sanitizeText } from "@yemu/utils";
...
import {
	...
	buildYemuReadResult,
	buildYemuWriteError,
	buildYemuWriteRejected,
	buildYemuWriteResult,
	...
	switch (execCase) {
		case "readArgs": {
			...
			synthesizeCursorExecToolCall(output, stream, state, args.toolCallId, "read", {
				path: yemuReadDisplayPath(args.path, args.offset, args.limit),
			});
			const { execResult } = await resolveExecHandler(
				args,
		...
		case "lsArgs": {
			...
			// `CursorExecHandlers.ls` in `agent-runtime/src/cursor.ts`); mirror
		...
		case "yemuReadArgs": {
			...
			const { execResult } = await resolveExecHandler(
				{ args, toolCallId },
				execHandlers?.yemuRead?.bind(execHandlers),
				onToolResult,
				buildYemuReadResult,
				buildYemuReadError,
				buildYemuReadError,
			...
			sendExecClientMessage(h2Request, execMsg, "yemuReadResult", execResult);
			return;
		}
		case "yemuBashArgs": {
			const args = execMsg.message.value;
			...
			synthesizeCursorExecToolCall(output, stream, state, toolCallId, "bash", {
				command: args.command,
				timeout: yemuTimeout(args.timeout),
			});
			const { execResult } = await resolveExecHandler(
				{ args, toolCallId },
				execHandlers?.yemuBash?.bind(execHandlers),
				onToolResult,
				buildYemuBashResult,
				buildYemuBashError,
				buildYemuBashError,
			...
			sendExecClientMessage(h2Request, execMsg, "yemuBashResult", execResult);
			return;
		}
		case "yemuEditArgs": {
			const args = execMsg.message.value;
			const toolCallId = crypto.randomUUID();
			// `YemuEditReplacement` is the local `edit` tool's replace mode verbatim:
			...
			const { execResult } = await resolveExecHandler(
				{ args, toolCallId },
				execHandlers?.yemuEdit?.bind(execHandlers),
				onToolResult,
				buildYemuEditResult,
				buildYemuEditRejected,
				buildYemuEditError,
			...
			sendExecClientMessage(h2Request, execMsg, "yemuEditResult", execResult);
			return;
		}
		case "yemuWriteArgs": {
			const args = execMsg.message.value;
			...
			const { execResult } = await resolveExecHandler(
				{ args, toolCallId },
				execHandlers?.yemuWrite?.bind(execHandlers),
				onToolResult,
				buildYemuWriteResult,
				buildYemuWriteRejected,
				buildYemuWriteError,
			...
			sendExecClientMessage(h2Request, execMsg, "yemuWriteResult", execResult);
			return;
		}
		case "yemuGrepArgs": {
			const args = execMsg.message.value;
			...
			synthesizeCursorExecToolCall(output, stream, state, toolCallId, "grep", {
				pattern: args.literal === true ? yemuEscapeRegexLiteral(args.pattern) : args.pattern,
				path: args.glob ? yemuJoinPath(args.path, args.glob) : args.path || ".",
				...
				// the same reason `yemu_read` renders its range into the displayed path:
				...
				limit: yemuLimit(args.limit),
			});
			const { execResult } = await resolveExecHandler(
				{ args, toolCallId },
				execHandlers?.yemuGrep?.bind(execHandlers),
				onToolResult,
				buildYemuGrepResult,
				buildYemuGrepError,
				buildYemuGrepError,
			...
			sendExecClientMessage(h2Request, execMsg, "yemuGrepResult", execResult);
			return;
		}
		case "yemuFindArgs": {
			const args = execMsg.message.value;
			...
			synthesizeCursorExecToolCall(output, stream, state, toolCallId, "glob", {
				path: yemuJoinPath(args.path, args.pattern),
				limit: yemuLimit(args.limit),
			});
			const { execResult } = await resolveExecHandler(
				{ args, toolCallId },
				execHandlers?.yemuFind?.bind(execHandlers),
				onToolResult,
				buildYemuFindResult,
				buildYemuFindError,
				buildYemuFindError,
			...
			sendExecClientMessage(h2Request, execMsg, "yemuFindResult", execResult);
			return;
		}
		case "yemuLsArgs": {
			const args = execMsg.message.value;
			...
			synthesizeCursorExecToolCall(output, stream, state, toolCallId, "read", { path: yemuLsPath(args.path) });
			const { execResult } = await resolveExecHandler(
				{ args, toolCallId },
				execHandlers?.yemuLs?.bind(execHandlers),
				onToolResult,
				buildYemuLsResult,
				buildYemuLsError,
				buildYemuLsError,
			...
			sendExecClientMessage(h2Request, execMsg, "yemuLsResult", execResult);
			return;
		}
 ...
 * own `ExecServerMessage` frame — the YeMu family (45-51), plus the two MCP
 ...
 * not the key: MCP, YeMu and connect-SCM blocks are filed under the id carried
...
export function processInteractionUpdate(
	...
	if (updateCase === "textDelta") {
		...
		// create a second one. Modern builds stream a `yemu_*_tool_call` envelope
		...
		// Correlate on the envelope's `call_id`, NOT the block id: MCP, YeMu and SCM
...
export function buildMcpToolDefinitions(tools: Tool[] | undefined): McpToolDefinition[] {
	...
	// re-include the built-in `write` (dropped as native above) whenever yemu-agent
