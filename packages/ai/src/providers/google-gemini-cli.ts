...
import { extractHttpStatusFromError, fetchWithRetry, readSseJson } from "@yemu/utils";
...
function isPlanningLeakObject(parsed: unknown, toolNames: Set<string>): boolean {
	...
	const isYemuTool = typeof record.call === "string" && toolNames.has(record.call);
	const hasToolSignature =
		"_i" in record || "paths" in record || "command" in record || ("path" in record && "content" in record);
	return hasThought || isYemuTool || hasToolSignature;
}
