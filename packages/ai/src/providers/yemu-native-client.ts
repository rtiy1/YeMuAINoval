/**
 * Client half of the yemu-native auth-gateway protocol.
 *
 * Dispatches a {@link streamSimple}-shaped request to an `yemu auth-gateway`
 * via `POST /v1/yemu/stream`, reads the SSE event stream back, and pushes the
 ...
 * way.
 *
 * Activated when a {@link Model} has `transport: "yemu-native"` set; the
 * dispatch hook lives in `streamSimple()` (see `../stream.ts`). Used by
 * containerized yemu deployments (yemu slots, the swarm extension) that
 * route every LLM call through a credential-holding sidecar so the slot
 * itself stays credential-free.
 */
import { readSseJson } from "@yemu/utils";
...
const YEMU_NATIVE_STREAM_IDLE_TIMEOUT_ERROR = "yemu-native stream stalled while waiting for the next event";
const YEMU_NATIVE_STREAM_FIRST_EVENT_TIMEOUT_ERROR = "yemu-native stream timed out while waiting for the first event";

function isYemuNativeProgressEvent(event: unknown): boolean {
...
/**
 * Resolve the `/v1/yemu/stream` endpoint URL from the model's `baseUrl`.
 * Trims a trailing slash so concatenation can't double-slash; throws when
 * the baseUrl is missing (transport=yemu-native without a gateway target is
 * a configuration error, not a runtime recoverable one).
 */
function resolveStreamUrl(model: Model<Api>): string {
	if (!model.baseUrl) {
		throw new AIError.ConfigurationError(
			`yemu-native transport requires \`baseUrl\` on model ${model.id} (set it on the provider config in models.yml)`,
		);
	}
	return `${model.baseUrl.replace(/\/+$/, "")}/v1/yemu/stream`;
}
...
/**
 * Stream a turn through an `yemu auth-gateway` over the yemu-native protocol.
 *
 ...
 * non-streaming path.
 */
export function streamYemuNative<TApi extends Api>(
	model: Model<TApi>,
	context: Context,
	...
	void (async () => {
		...
		try {
			...
			const watchedSource = iterateWithIdleTimeout(source, {
				...
				errorMessage: YEMU_NATIVE_STREAM_IDLE_TIMEOUT_ERROR,
				firstItemErrorMessage: YEMU_NATIVE_STREAM_FIRST_EVENT_TIMEOUT_ERROR,
				onIdle: () =>
					abortTracker.abortLocally(new AIError.StreamTimeoutError(YEMU_NATIVE_STREAM_IDLE_TIMEOUT_ERROR)),
				onFirstItemTimeout: () =>
					abortTracker.abortLocally(new AIError.StreamTimeoutError(YEMU_NATIVE_STREAM_FIRST_EVENT_TIMEOUT_ERROR)),
				isProgressItem: isYemuNativeProgressEvent,
			});
			...
			if (!sawTerminal) {
				...
				// `yemu-native-server.encodeStream`.
