...
import {
	...
	OPENAI_HEADERS,
} from "@yemu/model-catalog/wire/codex";
import {
	$env,
	$flag,
	asRecord,
...
const CODEX_DEBUG = $flag("YEMU_CODEX_DEBUG");
...
const CODEX_WEBSOCKET_PING_INTERVAL_MS = Number($env.YEMU_CODEX_WEBSOCKET_PING_INTERVAL_MS || 10_000);
const CODEX_WEBSOCKET_PONG_TIMEOUT_MS = Number($env.YEMU_CODEX_WEBSOCKET_PONG_TIMEOUT_MS || 60_000);
const CODEX_WEBSOCKET_MESSAGE_QUEUE_CAPACITY = Number($env.YEMU_CODEX_WEBSOCKET_MESSAGE_QUEUE_CAPACITY || 4096);
/**
 ...
 * execution, user typing, etc.). Set `YEMU_CODEX_WEBSOCKET_MAX_IDLE_REUSE_MS=0`
 * to disable.
 */
const CODEX_WEBSOCKET_MAX_IDLE_REUSE_MS = Number($env.YEMU_CODEX_WEBSOCKET_MAX_IDLE_REUSE_MS || 30_000);
/**
 * Steady-state liveness ceiling for the Codex WebSocket transport. Distinct from
 * the YeMu-wide stream watchdog removed in #1392: a WebSocket can stay TCP-open
 ...
 */
const CODEX_WEBSOCKET_IDLE_TIMEOUT_MS = Number($env.YEMU_CODEX_WEBSOCKET_IDLE_TIMEOUT_MS || 300_000);
/**
 ...
 */
const CODEX_WEBSOCKET_FIRST_EVENT_TIMEOUT_MS = Number($env.YEMU_CODEX_WEBSOCKET_FIRST_EVENT_TIMEOUT_MS || 300_000);
const CODEX_WEBSOCKET_RETRY_BUDGET = Number($env.YEMU_CODEX_WEBSOCKET_RETRY_BUDGET || CODEX_MAX_RETRIES);
const CODEX_WEBSOCKET_RETRY_DELAY_MS = Number($env.YEMU_CODEX_WEBSOCKET_RETRY_DELAY_MS || CODEX_RETRY_DELAY_MS);
...
/** Raw provider usage plus the normalized buckets YeMu displays for the latest Codex turn. */
...
const CODEX_RESERVED_METADATA_KEYS: Record<string, true> = {
	...
	// mapping (#35271); YeMu never emits it, but callers must not smuggle it in
	// as an extra either.
...
function getCodexWebSocketEnvValue(): boolean | undefined {
	const envVal = $env.YEMU_CODEX_WEBSOCKET;
	if (envVal !== undefined) {
		return $flag("YEMU_CODEX_WEBSOCKET");
	}
	return undefined;
}
...
function compressCodexRequestBody(bodyJson: string, baseUrl: string): Uint8Array | undefined {
	if (!isOfficialCodexApiUrl(baseUrl) || !$flag("YEMU_CODEX_ZSTD", true)) return undefined;
	try {
...
function createCodexHeaders(
	...
	headers.set("User-Agent", `yemu/${packageJson.version} (${os.platform()} ${os.release()}; ${os.arch()})`);
	if (sessionId) {
