/**
 * Translate a YeMu frame's args into the local tool kwargs that run it.
 *
 ...
 * yemu shim performs the identical translation for the old wire. Separate
...
/**
 * A `yemu_read` range composed onto the path as `read`'s inline `:raw:N+K`
 * selector.
 *
 ...
 */
export function yemuReadPath(readPath: string, offset?: number, limit?: number): string | null {
...
/**
 * The same range as {@link yemuReadPath}, rendered for a transcript block rather
 * than for execution.
 *
 * Differs only at `limit: 0`, where `yemuReadPath` returns `null` because no
 ...
 */
export function yemuReadDisplayPath(readPath: string, offset?: number, limit?: number): string {
 ...
 */
export function yemuGrepSkip(offset?: number): number | undefined {
	return offset !== undefined && offset > 0 ? Math.floor(offset) : undefined;
}

/**
 * Join a YeMu frame's optional `path` with the `glob`/`pattern` it scopes.
 *
 ...
 */
export function yemuJoinPath(basePath: string | undefined, pattern: string): string {
...
/**
 * The path a `yemu_ls` frame lists.
 *
 ...
 */
export function yemuLsPath(basePath: string | undefined): string {
	return basePath || ".";
}

/** Escape a literal string so the regex-only local `grep` tool matches it verbatim. */
export function yemuEscapeRegexLiteral(value: string): string {
	return value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}

/** Clamp a present `optional int32` result cap the way the reference does; `undefined` stays unset. */
export function yemuLimit(limit: number | undefined): number | undefined {
	return limit === undefined ? undefined : Math.max(1, Math.floor(limit));
}

/**
 * A `yemu_bash` frame's timeout as the local `bash` tool's kwarg.
 *
 ...
 */
export function yemuTimeout(timeout: number | undefined): number | undefined {
