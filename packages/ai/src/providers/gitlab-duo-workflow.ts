...
} from "@yemu/model-catalog/discovery/gitlab-duo-workflow";
 ...
 * keepalive contract YeMu can rely on, so a connection silently going half-open
 ...
 * reports its per-workflow step (graph-recursion) limit. Long YeMu tool-call loops
...
const GITLAB_DUO_WORKFLOW_AVAILABLE_MODELS_QUERY = `query yemu_gitlabDuoWorkflowAvailableModels($rootNamespaceId: GroupID!) {
...
const GITLAB_DUO_WORKFLOW_INLINE_AGENT_NAME = "yemu_agent";
const GITLAB_DUO_WORKFLOW_INLINE_PROMPT_ID = "yemu_inline_prompt";
...
// whose system slot carries YeMu's own authoritative system prompt (no GitLab jinja
...
function buildGitLabDuoWorkflowActionToolCall(action: GitLabDuoWorkflowActionDescriptor): ToolCall {
	const args =
		action.args && typeof action.args === "object" && !Array.isArray(action.args)
			...
			: {};
	const mapped = mapGitLabDuoWorkflowActionToYemuTool(action.name, args);
	return {
		type: "toolCall",
		id: action.requestID,
		name: mapped.name,
...
function mapGitLabDuoWorkflowActionToYemuTool(
	actionName: string,
...
function mapGitLabDuoWorkflowMcpToolCall(args: Record<string, unknown>): {
	...
	const toolName = rawName.startsWith("mcp__yemu__") ? rawName.slice("mcp__yemu__".length) : rawName;
...
function createGitLabDuoWorkflowProviderSessionState(): GitLabDuoWorkflowProviderSessionState {
	const state: GitLabDuoWorkflowProviderSessionState = {
		close: () => {
			...
			// PATCH a workflow the server is still running on YeMu would be stranded.
			try {
				state.active?.stop?.();
			} catch {
...
async function runGitLabDuoWorkflow(
	...
	const setupForNamespace = async (
		...
		// The inline `ambient` flow fails server-side without a project, and YeMu has
	...
	try {
		for (let attempt = 0; attempt < 12; attempt++) {
			...
			// A long but healthy YeMu tool-call loop legitimately overruns it; that is
...
// YeMu has no GitLab project of its own, but the inline `ambient` flow fails
		...
		void (async () => {
			...
			while (!settled) {
				...
				if (!handleSocketResult(result, data, pending)) {
					...
					// to YeMu. Clear the pause flag first: the live `onmessage` handler must
...
async function handleGitLabDuoWorkflowSocketMessage(
	...
	if (status === "FAILED" || status === "STOPPED") {
		...
		// RECURSION_LIMIT). A long but healthy YeMu tool-call loop legitimately hits
...
function buildGitLabMcpToolDefinition(tool: Tool): GitLabMcpToolDefinition {
	...
	// Register the tool under its BARE name (no `mcp__yemu__` prefix). The server does
	...
	// the name the model sees, the toolset key it is matched against, and YeMu's own
	// tool docs must all be the same bare name. A prefixed wire name only forced the
	// model to learn `mcp__yemu__read` while YeMu docs say `read`, with no upside.
...
// The YeMu system prompt that rides the inline flow's `prompt_template.system` slot.
// DWS wraps it in its own gateway boilerplate, but the slot content is delivered to
// the model verbatim, so YeMu's authoritative rules go here directly — no redirect
...
function renderGitLabDuoWorkflowChatMlToolCall(toolCall: GitLabDuoWorkflowReplayToolCall): string {
	...
	// call). The call id is YeMu-internal wiring the model never reads (call→result pair
...
// The `i` key is YeMu's per-call intent narration (e.g. "Reading kernel smoke body").
...
// Every workflow definition YeMu ships is the inline ambient flow (Path B /
