export * from "@yemu/model-catalog/effort";
export * from "@yemu/model-catalog/types";
...
import { isOpenAIModelId } from "@yemu/model-catalog/identity/family";
import type { Api, FetchImpl, KnownApi, Model, Provider, ThinkingBudgets, Usage } from "@yemu/model-catalog/types";
...
/**
 * Ceiling on the output-token count yemu requests from any OpenAI-family endpoint
...
export interface StreamOptions {
	 ...
	 * requests across local YeMu processes that share the same config root. Omitted
	 ...
	 * back to `YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS` and then to a 100s default.
	 * OpenAI-family transports additionally honor
	 * `YEMU_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS` as the most-specific override and
	 * floor the first-event budget at the resolved idle (per-call
	 * `streamIdleTimeoutMs` or `YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS`) so slow local
	 ...
	 * `YEMU_STREAM_IDLE_TIMEOUT_MS` (alias: `YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS`)
...
/**
 * A modern YeMu exec frame plus the call id the dispatcher minted for it.
 *
 * Unlike the legacy exec args (`ReadArgs`, `ShellArgs`, ...), the YeMu frames
 ...
 */
export interface CursorYemuCall<TArgs> {
	args: TArgs;
	toolCallId: string;
}
...
export interface CursorExecHandlers {
	...
	/**
	 * Modern Cursor CLI YeMu tool frames (`ExecServerMessage` 45-51). They are a
	 ...
	 */
	yemuRead?: (call: CursorYemuCall<YemuReadExecArgs>) => Promise<CursorExecHandlerResult<YemuReadExecResult>>;
	yemuBash?: (call: CursorYemuCall<YemuBashExecArgs>) => Promise<CursorExecHandlerResult<YemuBashExecResult>>;
	yemuEdit?: (call: CursorYemuCall<YemuEditExecArgs>) => Promise<CursorExecHandlerResult<YemuEditExecResult>>;
	yemuWrite?: (call: CursorYemuCall<YemuWriteExecArgs>) => Promise<CursorExecHandlerResult<YemuWriteExecResult>>;
	yemuGrep?: (call: CursorYemuCall<YemuGrepExecArgs>) => Promise<CursorExecHandlerResult<YemuGrepExecResult>>;
	yemuFind?: (call: CursorYemuCall<YemuFindExecArgs>) => Promise<CursorExecHandlerResult<YemuFindExecResult>>;
	yemuLs?: (call: CursorYemuCall<YemuLsExecArgs>) => Promise<CursorExecHandlerResult<YemuLsExecResult>>;
	/**
