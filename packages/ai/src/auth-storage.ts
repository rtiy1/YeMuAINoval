...
import { parseAlibabaTokenPlanCredential } from "@yemu/model-catalog/wire/alibaba-token-plan";
import { $env, getAgentDbPath, logger } from "@yemu/utils";
 ...
 * accounts (e.g. an expired Anthropic OAuth grant) stay visible in `yemu usage`
...
export type AuthStorageOptions = {
	 ...
	 * - coding-agent injects its resolveConfigValue (supports "!command" syntax via yemu-native)
	 ...
	 *
	 * Examples:
	 * - `"local ~/.yemu/agent/agent.db"`
	 * - `"broker http://yemu.internal:8765"`
	 */
	sourceLabel?: string;
	/**
 ...
 * Does NOT support "!command" syntax (that requires yemu-native).
 */
...
// `@yemu/model-runtime` entrypoint and the in-module call sites below.
...
export class AuthStorage {
	 ...
	 * Convenience factory for standalone use (e.g., model-runtime CLI).
	...
	/**
	 * Disabled credential tombstones for display surfaces (`yemu usage`,
	 ...
	 * per-credential data with stored identities (`yemu usage`) use this so a
 ...
 *
 * Used by the model-runtime CLI and as the default store for `AuthStorage.create()`.
...
export class SqliteAuthCredentialStore implements AuthCredentialStore {
	...
	static async open(dbPath: string = getAgentDbPath()): Promise<SqliteAuthCredentialStore> {
		...
		// Concurrent yemu startups can race against WAL recovery and the schema
	...
	#initializeSchema(): void {
		...
		// recovery). Without this, concurrent yemu startups can crash here with
