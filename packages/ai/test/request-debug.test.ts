...
import { clearCustomApis, registerCustomApi } from "@yemu/model-runtime/api-registry";
import { stream } from "@yemu/model-runtime/stream";
import type { AssistantMessage, FetchImpl, Model, ModelSpec } from "@yemu/model-runtime/types";
import { AssistantMessageEventStream } from "@yemu/model-runtime/utils/event-stream";
import { wrapFetchForRequestDebug } from "@yemu/model-runtime/utils/request-debug";
...
beforeEach(async () => {
	previousDebugFlag = Bun.env.YEMU_REQ_DEBUG;
	previousCwd = process.cwd();
	tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-req-debug-"));
...
afterEach(async () => {
	...
	if (previousDebugFlag === undefined) delete Bun.env.YEMU_REQ_DEBUG;
	else Bun.env.YEMU_REQ_DEBUG = previousDebugFlag;
...
/** Find the latest rr-session-*.json written to the temp dir by YEMU_REQ_DEBUG
...
describe("YEMU_REQ_DEBUG request/response recording", () => {
	it("leaves fetch untouched when the flag is disabled", () => {
		delete Bun.env.YEMU_REQ_DEBUG;
