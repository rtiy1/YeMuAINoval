...
import { streamSimple } from "@yemu/model-runtime";
import {
...
import { __resetProxyCache } from "@yemu/model-runtime/utils/proxy";
import * as yemuUtils from "@yemu/utils";

const { getAgentDir, setAgentDir, TempDir } = yemuUtils;

const originalAgentDir = getAgentDir();
const originalWebSocket = global.WebSocket;
const originalCodexWebSocketV2 = Bun.env.YEMU_CODEX_WEBSOCKET_V2;
const originalProxyEnv: Record<string, string | undefined> = {
	YEMU_PROXY: Bun.env.YEMU_PROXY,
	YEMU_PROXY_CODEX_PROXY_TEST: Bun.env.YEMU_PROXY_CODEX_PROXY_TEST,
...
beforeEach(() => {
	...
	__resetProxyCache();
	vi.spyOn(yemuUtils, "getInstallId").mockReturnValue(TEST_INSTALLATION_ID);
});

afterEach(() => {
	global.WebSocket = originalWebSocket;
	setAgentDir(originalAgentDir);
	restoreEnv("YEMU_CODEX_WEBSOCKET_V2", originalCodexWebSocketV2);
	vi.useRealTimers();
...
describe("openai-codex streaming", () => {
	it("normalizes Codex response endpoint base URLs", async () => {
		const tempDir = TempDir.createSync("@yemu-codex-stream-");
	...
	it("omits chatgpt account headers for opaque custom provider API keys", async () => {
		...
		expect(requestHeaders?.get("originator")).toBe("yemu");
	});
	...
	it("omits chatgpt account headers on opaque custom provider websockets", async () => {
		...
		expect(capturedHeaders?.originator).toBe("yemu");
	});
	...
	it("passes the provider proxy to websocket handshakes", async () => {
		...
		Bun.env.YEMU_PROXY_CODEX_PROXY_TEST = proxy;
		__resetProxyCache();
		...
		try {
			...
			delete Bun.env.YEMU_PROXY_CODEX_PROXY_TEST;
		}
	});
	...
	it("bypasses configured proxies for NO_PROXY websocket targets", async () => {
		Bun.env.YEMU_PROXY_CODEX_PROXY_TEST = "http://127.0.0.1:7890";
	...
	it("sends the Responses Lite marker on the upgrade and in response.create client_metadata", async () => {
		...
		// CODE_MODE_TOOL_NAMES_KEY, #35271): YeMu never emits it, and caller extras
	...
	it("streams SSE responses into AssistantMessageEventStream", async () => {
		...
		const fetchMock = vi.fn(async (input: string | URL, init?: RequestInit) => {
			...
			if (url === "https://chatgpt.com/backend-api/codex/responses") {
				...
				expect(headers?.get("originator")).toBe("yemu");
	...
	it("drops a stale terminal frame from the prior response leaking onto a reused websocket", async () => {
		const tempDir = TempDir.createSync("@yemu-codex-stale-frame-");
		setAgentDir(tempDir.path());
		const token = createCodexTestToken();
		const fetchMock = vi.fn(async () => {
			throw new Error("SSE fallback should not be called");
	...
	it("applies onPayload to the final chained websocket frame", async () => {
		const tempDir = TempDir.createSync("@yemu-codex-ws-payload-hook-");
		setAgentDir(tempDir.path());
		const token = createCodexTestToken();
		const sentRequests: Array<Record<string, unknown>> = [];
		const fetchMock = vi.fn(async () => {
	...
	it("uses websocket v2 beta header when v2 mode is enabled", async () => {
		const tempDir = TempDir.createSync("@yemu-codex-stream-");
		setAgentDir(tempDir.path());
		Bun.env.YEMU_CODEX_WEBSOCKET_V2 = "1";
...
describe("openai-codex SSE statelessness", () => {
	...
	it("never sends previous_response_id over SSE; every turn replays the full transcript", async () => {
		...
		const tempDir = TempDir.createSync("@yemu-codex-sse-stateless-");
		setAgentDir(tempDir.path());
		const sentRequests: Array<Record<string, unknown>> = [];
