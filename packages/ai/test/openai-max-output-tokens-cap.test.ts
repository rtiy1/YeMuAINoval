...
import { type Context, type Model, type ModelSpec, OPENAI_MAX_OUTPUT_TOKENS } from "@yemu/model-runtime/types";
...
async function drainResponses(
	...
	const previousOpenRouterResponses = Bun.env.YEMU_OPENROUTER_RESPONSES;
	if (model.api === "openrouter") Bun.env.YEMU_OPENROUTER_RESPONSES = "1";
	try {
		...
		if (previousOpenRouterResponses === undefined) {
			delete Bun.env.YEMU_OPENROUTER_RESPONSES;
		} else {
			Bun.env.YEMU_OPENROUTER_RESPONSES = previousOpenRouterResponses;
		}
	}
}
