...
import type { Context, FetchImpl, Model } from "@yemu/model-runtime/types";
...
describe("codex SSE request body zstd compression", () => {
	it("compresses the request body with zstd and sets content-encoding by default", async () => {
		await withEnv({ YEMU_CODEX_ZSTD: undefined }, async () => {
	...
	it("sends the plain JSON string without content-encoding when YEMU_CODEX_ZSTD=0", async () => {
		await withEnv({ YEMU_CODEX_ZSTD: "0" }, async () => {
			const { body, headers } = await runAndCaptureRequest();
