...
import { AuthStorage, type FetchImpl, type OAuthCredential, SqliteAuthCredentialStore } from "@yemu/model-runtime";
...
describe("AuthStorage openai-codex email dedupe", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "model-runtime-auth-email-dedupe-"));
...
describe("AuthStorage OAuth login upgrade and multi-account coexistence", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-auth-login-test-"));
	});
...
describe("AuthStorage persistent session stickiness", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-auth-test-"));
