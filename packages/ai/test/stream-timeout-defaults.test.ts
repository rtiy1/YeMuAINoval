...
} from "@yemu/model-runtime/utils/idle-iterator";
...
const ENV_KEYS = [
	"YEMU_STREAM_IDLE_TIMEOUT_MS",
	"YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS",
	"YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS",
	"YEMU_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS",
] as const;
...
describe("getStreamIdleTimeoutMs(fallbackMs)", () => {
	...
	it("lets YEMU_STREAM_IDLE_TIMEOUT_MS override the per-provider fallback", () => {
		Bun.env.YEMU_STREAM_IDLE_TIMEOUT_MS = "42";
		expect(getStreamIdleTimeoutMs(300_000)).toBe(42);
	});

	it("treats YEMU_STREAM_IDLE_TIMEOUT_MS=0 as a watchdog disable", () => {
		Bun.env.YEMU_STREAM_IDLE_TIMEOUT_MS = "0";
...
describe("getOpenAIStreamIdleTimeoutMs(fallbackMs)", () => {
	...
	it("lets YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS override the fallback before the generic env var", () => {
		Bun.env.YEMU_STREAM_IDLE_TIMEOUT_MS = "42";
		Bun.env.YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS = "84";
		expect(getOpenAIStreamIdleTimeoutMs(600_000)).toBe(84);
	});

	it("treats YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS=0 as a watchdog disable", () => {
		Bun.env.YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS = "0";
...
describe("getStreamFirstEventTimeoutMs(idleTimeoutMs, fallbackMs)", () => {
	...
	it("lets YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS override the per-provider fallback", () => {
		Bun.env.YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS = "42";
		expect(getStreamFirstEventTimeoutMs(undefined, 300_000)).toBe(42);
	});

	it("treats YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS=0 as a watchdog disable", () => {
		Bun.env.YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS = "0";
...
describe("getOpenAIStreamFirstEventTimeoutMs(idleTimeoutMs, fallbackMs)", () => {
	...
	it("honors YEMU_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS even when caller pins per-call idle", () => {
		Bun.env.YEMU_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS = "42";
		Bun.env.YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS = "20";
		expect(getOpenAIStreamFirstEventTimeoutMs(5_000, 100_000)).toBe(42);
	});

	it("treats YEMU_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS=0 as an explicit watchdog disable", () => {
		Bun.env.YEMU_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS = "0";
	...
	it("respects YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS=0 disable when no OpenAI override is set", () => {
		Bun.env.YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS = "0";
		expect(getOpenAIStreamFirstEventTimeoutMs(1500, 100_000)).toBe(0);
	});
});
