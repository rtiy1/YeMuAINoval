...
} from "@yemu/model-runtime/providers/openai-codex/request-transformer";
import {
...
import { isOpenAIResponsesProgressEvent } from "@yemu/model-runtime/providers/openai-shared";
...
describe("openai-codex Responses Lite input shaping", () => {
	...
	it("resolves Lite from explicit options, the environment, then the model default", async () => {
		const previous = Bun.env.YEMU_CODEX_RESPONSES_LITE;
		const model = createCodexModel("gpt-5.6-terra", { useResponsesLite: true });
		try {
			delete Bun.env.YEMU_CODEX_RESPONSES_LITE;
			...
			Bun.env.YEMU_CODEX_RESPONSES_LITE = "false";
			...
			Bun.env.YEMU_CODEX_RESPONSES_LITE = "true";
			...
			if (previous === undefined) delete Bun.env.YEMU_CODEX_RESPONSES_LITE;
			else Bun.env.YEMU_CODEX_RESPONSES_LITE = previous;
		}
	});
});
