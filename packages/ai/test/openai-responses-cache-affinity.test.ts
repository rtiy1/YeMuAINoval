...
import { buildOpenAIResponsesCompat } from "@yemu/model-catalog/compat/openai";
import { getBundledModel } from "@yemu/model-catalog/models";
import {
	type AzureOpenAIResponsesOptions,
	streamAzureOpenAIResponses,
} from "@yemu/model-runtime/providers/azure-openai-responses";
import {
	buildParams,
	type OpenAIResponsesOptions,
	streamOpenAIResponses,
} from "@yemu/model-runtime/providers/openai-responses";
import { stream as streamModel, streamSimple } from "@yemu/model-runtime/stream";
import type { Context, FetchImpl, Model, ProviderSessionState, SimpleStreamOptions } from "@yemu/model-runtime/types";
...
describe("OpenAI Responses explicit prompt cache policy", () => {
	...
	it("defers explicit policy validation to the gateway-resolved model for yemu-native transport", async () => {
	...
	it("honors cache retention environment overrides before public explicit policy validation", async () => {
		...
		await withEnv({ YEMU_CACHE_RETENTION: "none" }, async () => {
		...
		for (const retention of ["short", "long"] as const) {
			await withEnv({ YEMU_CACHE_RETENTION: retention }, () => {
				expect(() =>
					streamSimple(
						unsupportedModel,
						{ messages: [{ role: "user", content: [{ type: "text", text: "prompt" }], timestamp: 0 }] },
