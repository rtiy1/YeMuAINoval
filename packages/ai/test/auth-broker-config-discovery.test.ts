...
import { discoverAuthStorage, resolveAuthBrokerConfig } from "@yemu/model-runtime/auth-broker";
import { removeWithRetries } from "../../utils/src/temp";
import { withEnv } from "./helpers";

const SUPPRESS_AUTH_BROKER_ENV = {
	YEMU_AUTH_BROKER_URL: undefined,
	YEMU_AUTH_BROKER_TOKEN: undefined,
	YEMU_AUTH_BROKER_ACCOUNT_POOL_FILE: undefined,
} as const;
...
describe("resolveAuthBrokerConfig config discovery", () => {
	...
	beforeEach(async () => {
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "model-runtime-auth-broker-config-"));
	});
	...
	test("rejects unreadable or malformed account-pool files before connecting", async () => {
		...
		const brokerEnv = {
			...
			YEMU_AUTH_BROKER_URL: "http://127.0.0.1:1",
			YEMU_AUTH_BROKER_TOKEN: "test-token",
			YEMU_AUTH_BROKER_ACCOUNT_POOL_FILE: poolPath,
		} as const;

		await withEnv(brokerEnv, async () => {
			await expect(discoverAuthStorage({ agentDir })).rejects.toThrow(
				"Unable to read YEMU_AUTH_BROKER_ACCOUNT_POOL_FILE",
			);
