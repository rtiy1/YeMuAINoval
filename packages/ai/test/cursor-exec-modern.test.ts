...
import {
	...
	YemuBashExecArgsSchema,
	YemuEditExecArgsSchema,
	YemuEditReplacementSchema,
	YemuFindExecArgsSchema,
	YemuGrepExecArgsSchema,
	YemuLsExecArgsSchema,
	YemuReadExecArgsSchema,
	YemuWriteExecArgsSchema,
...
import {
	...
	type ToolCallState,
} from "@yemu/model-runtime/providers/cursor";
import type { AssistantMessage, CursorExecHandlers, ToolResultMessage } from "@yemu/model-runtime/types";
import { kCursorExecResolved, setStreamingPartialJson } from "@yemu/model-runtime/utils/block-symbols";
					...
					args: create(ConnectScmArgsSchema, {
						...
						target: {
							...
							value: create(ConnectScmGithubSchema, {
								repository: create(ConnectScmGithubRepositorySchema, { owner: "rtiy1", repo: "YeMuAINoval" }),
							}),
						},
					}),
				}),
	...
	it("declines every allowlist precheck, since no allowlist is configured", async () => {
		...
		const mcp = await dispatchExec(
			buildExecMessage({
				...
				value: create(McpAllowlistPrecheckArgsSchema, { providerIdentifier: "yemu-agent", toolName: "task" }),
			}),
		);
...
describe("Cursor modern exec frames: MCP state", () => {
	it("regroups the advertised tool catalog by provider identifier", async () => {
		const { frames } = await dispatchExec(
			...
			{
				requestContextTools: [
					mcpTool("task", "yemu-agent"),
					mcpTool("hub", "yemu-agent"),
		...
		expect(servers.map(server => server.serverIdentifier).sort()).toEqual(["fixture", "yemu-agent"]);
		const yemuAgent = servers.find(server => server.serverIdentifier === "yemu-agent");
		expect(yemuAgent?.tools.map(t => t.name)).toEqual(["task", "hub"]);
	});
	...
	it("restricts the answer to the requested server identifiers", async () => {
		const { frames } = await dispatchExec(
			...
			{ requestContextTools: [mcpTool("task", "yemu-agent"), mcpTool("mcp__fixture_report", "fixture")] },
		);
...
describe("Cursor modern exec frames: YeMu tools", () => {
	it("round-trips yemuRead through its handler into a typed success", async () => {
		const { frames, output, results } = await dispatchExec(
			buildExecMessage({
				case: "yemuReadArgs",
				value: create(YemuReadExecArgsSchema, { path: "/repo/a.ts", offset: 5, limit: 20 }),
			}),
			{
				execHandlers: {
					async yemuRead(call) {
						...
						// The YeMu frames carry no tool_call_id; the dispatcher mints one.
		...
		const answer = soleResult(frames);
		if (answer.case !== "yemuReadResult") throw new Error(`got ${answer.case}`);
		if (answer.value.result.case !== "success") throw new Error("expected success");
	...
	it("maps a failing YeMu handler onto the frame's own error variant", async () => {
		const { frames } = await dispatchExec(
			buildExecMessage({ case: "yemuReadArgs", value: create(YemuReadExecArgsSchema, { path: "/nope" }) }),
			{
				execHandlers: {
					async yemuRead(call) {
	...
	it("rejects a YeMu frame with no handler installed instead of faking a success", async () => {
		const { frames } = await dispatchExec(
			buildExecMessage({ case: "yemuGrepArgs", value: create(YemuGrepExecArgsSchema, { pattern: "foo" }) }),
		);

		const answer = soleResult(frames);
		if (answer.case !== "yemuGrepResult") throw new Error(`got ${answer.case}`);
		expect(answer.value.result.case).toBe("error");
	});
		...
		const edit = soleResult(
			(
				await dispatchExec(
					buildExecMessage({
						case: "yemuEditArgs",
						value: create(YemuEditExecArgsSchema, {
							path: "/a.ts",
							edits: [create(YemuEditReplacementSchema, { oldText: "x", newText: "y" })],
						}),
					}),
				)
			).frames,
		);
		if (edit.case !== "yemuEditResult") throw new Error(`got ${edit.case}`);
		...
		const write = soleResult(
			(
				await dispatchExec(
					buildExecMessage({
						case: "yemuWriteArgs",
						value: create(YemuWriteExecArgsSchema, { path: "/a.ts", content: "x" }),
					}),
				)
			).frames,
		);
		if (write.case !== "yemuWriteResult") throw new Error(`got ${write.case}`);
		expect(write.value.result.case).toBe("rejected");
	});

	it("carries a truncation summary onto the YeMu success payload", async () => {
		const { frames } = await dispatchExec(
			buildExecMessage({ case: "yemuBashArgs", value: create(YemuBashExecArgsSchema, { command: "yes" }) }),
			{
				execHandlers: {
					async yemuBash() {
		...
		const answer = soleResult(frames);
		if (answer.case !== "yemuBashResult") throw new Error(`got ${answer.case}`);
		if (answer.value.result.case !== "success") throw new Error("expected success");
	...
	it("omits truncation entirely when nothing was truncated", async () => {
		// `optional YemuTruncation` — a zeroed message would claim the output was
		// trimmed to nothing.
		const { frames } = await dispatchExec(
			buildExecMessage({ case: "yemuBashArgs", value: create(YemuBashExecArgsSchema, { command: "echo hi" }) }),
			{
				execHandlers: {
					async yemuBash() {
	...
	it("passes edit replacements through to the handler and surfaces diff/patch", async () => {
		const { frames, output } = await dispatchExec(
			buildExecMessage({
				...
				value: create(YemuEditExecArgsSchema, {
					path: "/repo/a.ts",
					edits: [create(YemuEditReplacementSchema, { oldText: "before", newText: "after" })],
				}),
			}),
			{
				execHandlers: {
		...
		const answer = soleResult(frames);
		if (answer.case !== "yemuEditResult") throw new Error(`got ${answer.case}`);
		if (answer.value.result.case !== "success") throw new Error("expected success");
	...
	it("answers each remaining YeMu frame with a populated success payload", async () => {
		...
		const grep = soleResult(
			(
				await dispatchExec(
					buildExecMessage({ case: "yemuGrepArgs", value: create(YemuGrepExecArgsSchema, { pattern: "hit" }) }),
					{ execHandlers: handlers },
				)
			).frames,
		);
		if (grep.case !== "yemuGrepResult") throw new Error(`got ${grep.case}`);
		...
		const find = soleResult(
			(
				await dispatchExec(
					buildExecMessage({ case: "yemuFindArgs", value: create(YemuFindExecArgsSchema, { pattern: "*.ts" }) }),
					{ execHandlers: handlers },
				)
			).frames,
		);
		if (find.case !== "yemuFindResult") throw new Error(`got ${find.case}`);
		...
		const ls = soleResult(
			(
				await dispatchExec(buildExecMessage({ case: "yemuLsArgs", value: create(YemuLsExecArgsSchema, {}) }), {
					execHandlers: handlers,
				})
			).frames,
		);
		if (ls.case !== "yemuLsResult") throw new Error(`got ${ls.case}`);
...
describe("Cursor modern exec frames: server-resolved tool calls leave a paired block", () => {
	...
	it("reads the connectScm repository out of the target oneof, not a flat field", async () => {
		const { output, results } = runConnectScm(
			{ toolCallId: "call-scm-1", repository: { owner: "rtiy1", repo: "YeMuAINoval" } },
			{ case: "success", value: {} },
		);
		...
		expect(block).toMatchObject({
			id: "call-scm-1",
			name: "connect_scm",
			arguments: { owner: "rtiy1", repo: "YeMuAINoval" },
		});
	...
	it("does not double-create a block for a YeMu call the exec channel already synthesized", async () => {
		// Modern builds stream a `yemu_*_tool_call` envelope alongside the exec
		...
		processInteractionUpdate(
			{
				message: {
					case: "toolCallStarted",
					value: {
						callId: "call-yemu-read",
						toolCall: { tool: { case: "yemuReadToolCall", value: { args: { path: "/a.ts" } } } },
					},
				},
			},
			output,
...
describe("Cursor exec answers: what the result claims about the work", () => {
	...
	it("records the scoped grep's context and cap in the synthesized call", async () => {
		...
		const { output } = await dispatchExec(
			buildExecMessage({
				case: "yemuGrepArgs",
				value: create(YemuGrepExecArgsSchema, { pattern: "hit", context: 3, limit: 50 }),
			}),
			{
				execHandlers: {
					async yemuGrep() {
