...
} from "@yemu/model-runtime/providers/anthropic";
import type { MessageCreateParamsStreaming } from "@yemu/model-runtime/providers/anthropic-wire";
import { claudeCodeVersion } from "@yemu/model-runtime/providers/claude-code-fingerprint";
import { getEnvApiKey, streamSimple } from "@yemu/model-runtime/stream";
import type {
	AssistantMessage,
	Context,
	Model,
...
describe("Anthropic request fingerprint alignment", () => {
	...
	it("loads Foundry mTLS and CA material from file paths", async () => {
		const tmpDir = path.join(
			os.tmpdir(),
			`model-runtime-foundry-${Date.now()}-${Math.random().toString(16).slice(2)}`,
		);
