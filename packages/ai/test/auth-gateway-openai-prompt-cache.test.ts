...
describe("auth-gateway explicit OpenAI prompt cache controls", () => {
	it("rejects raw controls clearly and forwards the yemu-native policy", async () => {
		registerMockApi();
		...
		try {
			...
			expect(chatBody.error).toEqual({
				...
				message:
					"openai-chat: prompt_cache_options and prompt_cache_breakpoint are unsupported by this auth-gateway route; use /v1/yemu/stream with options.promptCache instead",
			});
			...
			const yemuResponse = await fetch(`${handle.url}/v1/yemu/stream`, {
				method: "POST",
				headers: { "Content-Type": "application/json", Authorization: "Bearer t" },
				body: JSON.stringify({
			...
			expect(yemuResponse.status).toBe(200);
