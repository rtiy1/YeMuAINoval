...
import type { Context, Model, ModelSpec, VercelGatewayRouting } from "@yemu/model-runtime/types";
...
describe("Vercel AI Gateway automatic cache controls", () => {
	...
	it("uses the Vercel default TTL for default, explicit, and environment short retention", async () => {
		...
		await withEnv({ YEMU_CACHE_RETENTION: "short" }, async () => {
	...
	it("omits Chat and Responses automatic cache controls when YEMU_CACHE_RETENTION is none", async () => {
		const routing: VercelGatewayRouting = {
			only: ["anthropic"],
			order: ["anthropic", "bedrock"],
			caching: "auto",
