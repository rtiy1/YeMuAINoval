...
import { convertAnthropicMessages, streamAnthropic } from "@yemu/model-runtime/providers/anthropic";
import {
	AnthropicMessages,
	type AnthropicMessagesClientLike,
	type AnthropicRequestOptions,
} from "@yemu/model-runtime/providers/anthropic-client";
import type { WebSearchToolResultBlockParam } from "@yemu/model-runtime/providers/anthropic-wire";
import type { AssistantMessageEvent, Context, Model, ModelSpec, ProviderSessionState } from "@yemu/model-runtime/types";
import { structuredCloneJSON } from "@yemu/utils";
...
describe("anthropic stream envelope handling", () => {
	...
	it("defaults API-key requests to 1h cache TTL where long retention is supported", async () => {
		...
		await withEnv({ YEMU_CACHE_RETENTION: "short" }, () => drain(model));
		...
		expect(cacheControls[1]).toEqual({ type: "ephemeral" });
		// YEMU_CACHE_RETENTION=short opts back out of the 1h default.
		expect(cacheControls[2]).toEqual({ type: "ephemeral" });
	});
});
