...
import { streamYemuNative } from "@yemu/model-runtime/providers/yemu-native-client";
import type {
	AssistantMessage,
...
describe("streamYemuNative request shape", () => {
	it("POSTs `{modelId, context, options, stream:true}` to `<baseUrl>/v1/yemu/stream`", async () => {
		...
		const stream = streamYemuNative(fakeModel(), baseContext, {
			apiKey: "gw-bearer",
			fetch: fetchImpl,
			temperature: 0.7,
		});
		...
		expect(captured.url).toBe("http://llm-gateway.internal:4000/v1/yemu/stream");
	...
	it("normalizes trailing slashes on `baseUrl` so the endpoint never double-slashes", async () => {
		...
		await streamYemuNative(fakeModel({ baseUrl: "http://llm-gateway.internal:4000///" }), baseContext, {
			apiKey: "k",
			fetch: fetchImpl,
		}).result();
		expect(captured.url).toBe("http://llm-gateway.internal:4000/v1/yemu/stream");
	...
	it("forwards `model.headers` and lets a caller-supplied Authorization win", async () => {
		...
		await streamYemuNative(
			fakeModel({ headers: { "x-yemu-slot": "yemu-1", Authorization: "Bearer model-wins" } }),
			baseContext,
			{ apiKey: "options-loses", fetch: fetchImpl },
		).result();

		const headers = captured.init?.headers as Record<string, string>;
		expect(headers["x-yemu-slot"]).toBe("yemu-1");
	...
	it("throws synchronously when `baseUrl` is missing", async () => {
		...
		const stream = streamYemuNative(broken, baseContext, { apiKey: "k" });
		await expect(stream.result()).rejects.toThrow(/baseUrl/);
	});
});

describe("streamYemuNative event flow", () => {
	it("pushes parsed events verbatim and resolves `.result()` on terminal `done`", async () => {
		...
		const stream = streamYemuNative(fakeModel(), baseContext, { apiKey: "k", fetch: fetchImpl });
	...
	it("uses YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS for silent yemu-native streams", async () => {
		const previous = Bun.env.YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS;
		...
		try {
			...
			if (previous === undefined) {
				...
				Bun.env.YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS = previous;
			}
		}
	});

	it("rejects when a yemu-native stream stalls after semantic progress", async () => {
		const partial = baseAssistant({ content: [{ type: "text", text: "hi" }] });
		const chunks = [
	...
	it("does not time out a healthy yemu-native stream that keeps making semantic progress", async () => {
