...
import { streamGoogle } from "@yemu/model-runtime/providers/google";
import type { GoogleGeminiCliOptions } from "@yemu/model-runtime/providers/google-gemini-cli";
import { buildGoogleGenerateContentParams } from "@yemu/model-runtime/providers/google-shared";
import { streamGoogleVertex } from "@yemu/model-runtime/providers/google-vertex";
import { parseRequest as parseYemuNativeRequest } from "@yemu/model-runtime/providers/yemu-native-server";
import { streamSimple } from "@yemu/model-runtime/stream";
import type { ApiOptionsMap, AssistantMessageEvent, Context, FetchImpl, Model, Tool } from "@yemu/model-runtime/types";
...
function yemuNativeSseStop(): Response {
	const message = {
		role: "assistant",
		content: [],
		api: "google-generative-ai",
...
function yemuNativeGatewayModel<T extends "google-generative-ai" | "google-vertex">(model: Model<T>): Model<T> {
	return { ...model, baseUrl: "http://yemu-native-gateway.test", transport: "yemu-native" };
}
...
describe("Google caller-owned cachedContent", () => {
	...
	it("round-trips cachedContent through the yemu-native gateway for Gemini and Vertex", async () => {
		const requests: unknown[] = [];
		const fetch: FetchImpl = async (_input, init) => {
			requests.push(JSON.parse(String(init?.body ?? "{}")));
			return yemuNativeSseStop();
		};

		for (const [model, cachedContent] of [
			[yemuNativeGatewayModel(geminiModel), CACHE_NAME],
			[yemuNativeGatewayModel(vertexModel), VERTEX_CACHE_NAME],
		] as const) {
			await drain(
		...
		expect(requests.map(request => parseYemuNativeRequest(request).options.cachedContent)).toEqual([
			CACHE_NAME,
			VERTEX_CACHE_NAME,
		]);
	});
