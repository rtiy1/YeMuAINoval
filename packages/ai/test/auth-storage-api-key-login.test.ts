...
import { serializeAlibabaTokenPlanCredential } from "@yemu/model-catalog/wire/alibaba-token-plan";
import { AuthStorage, SqliteAuthCredentialStore } from "@yemu/model-runtime/auth-storage";
import * as deepseekModule from "@yemu/model-runtime/registry/deepseek";
import * as kagiModule from "@yemu/model-runtime/registry/kagi";
import * as ollamaCloudModule from "@yemu/model-runtime/registry/ollama-cloud";
...
describe("AuthStorage api-key login upsert", () => {
	...
	beforeEach(async () => {
		...
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "model-runtime-auth-api-key-login-"));
