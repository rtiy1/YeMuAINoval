 ...
 *     → anthropic-messages parser → yemu Context (cacheRetention derived)
 *     → model-runtime openai-codex-responses provider
 ...
 * directly — caching only works if model-runtime's codex transport reaches Codex
 ...
 * (override via `YEMU_E2E_GATEWAY_URL`) AND the bearer token file exists at
 * `~/.yemu/auth-gateway.token`.
 *
...
const MODEL = Bun.env.YEMU_E2E_CODEX_MODEL ?? "gpt-5.3-codex";
...
the yemu auth-gateway's cross-protocol prompt-caching pipeline. The request
...
describe.skipIf(!gateway.ok)("auth-gateway: anthropic-messages → openai-codex caching e2e", () => {
	...
	it("caches the system prefix across a cross-protocol translate (messages→codex)", async () => {
		...
		//   - the anthropic-messages encoder forgot to map model-runtime's
