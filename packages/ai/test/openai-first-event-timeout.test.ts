...
import { streamAzureOpenAIResponses } from "@yemu/model-runtime/providers/azure-openai-responses";
import { streamOpenAICompletions } from "@yemu/model-runtime/providers/openai-completions";
import { streamOpenAIResponses } from "@yemu/model-runtime/providers/openai-responses";
import { streamSimple } from "@yemu/model-runtime/stream";
import type { Context, FetchImpl, Model, TextContent } from "@yemu/model-runtime/types";
...
describe("OpenAI-family first-event timeouts", () => {
	...
	it("lets YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS widen OpenAI responses first-event request setup", async () => {
		const previousOpenAIIdleTimeout = Bun.env.YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS;
		const previousGenericFirstEventTimeout = Bun.env.YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS;
		const timeoutHeaders: string[] = [];
		Bun.env.YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS = "1500";
		Bun.env.YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS = "20";
		...
		try {
			...
			if (previousOpenAIIdleTimeout === undefined) {
				delete Bun.env.YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS;
			} else {
				Bun.env.YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS = previousOpenAIIdleTimeout;
			}
			if (previousGenericFirstEventTimeout === undefined) {
				delete Bun.env.YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS;
			} else {
				Bun.env.YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS = previousGenericFirstEventTimeout;
			}
		}
	});

	it("lets YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS widen native Ollama first-event request setup", async () => {
		const previousOpenAIIdleTimeout = Bun.env.YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS;
		const previousGenericFirstEventTimeout = Bun.env.YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS;
		Bun.env.YEMU_OPENAI_STREAM_IDLE_TIMEOUT_MS = "1500";
		Bun.env.YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS = "20";
	...
	it("honors YEMU_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS even when caller pins streamIdleTimeoutMs", async () => {
		const previousOpenAIFirstEventTimeout = Bun.env.YEMU_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS;
		const previousGenericFirstEventTimeout = Bun.env.YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS;
		const timeoutHeaders: string[] = [];
		Bun.env.YEMU_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS = "1500";
		Bun.env.YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS = "20";
		const fetchMock = createDelayedFetch(30, createOpenAIResponsesSuccessResponse, (input, init) => {
			timeoutHeaders.push(getRequestHeader(input, init, "X-Stainless-Timeout") ?? "");
		});
		...
		try {
			...
			if (previousOpenAIFirstEventTimeout === undefined) {
				delete Bun.env.YEMU_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS;
			} else {
				Bun.env.YEMU_OPENAI_STREAM_FIRST_EVENT_TIMEOUT_MS = previousOpenAIFirstEventTimeout;
			}
			if (previousGenericFirstEventTimeout === undefined) {
				delete Bun.env.YEMU_STREAM_FIRST_EVENT_TIMEOUT_MS;
			} else {
