...
import { withAuth } from "@yemu/model-runtime";
...
import { registerOAuthProvider, unregisterOAuthProviders } from "@yemu/model-runtime/registry/oauth";
import type { CredentialRankingStrategy, UsageProvider } from "@yemu/model-runtime/usage";
...
describe("AuthStorage forceRefresh + rotateSessionCredential", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "model-runtime-rotate-"));
