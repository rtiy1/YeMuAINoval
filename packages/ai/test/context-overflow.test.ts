...
import { isContextOverflow as originalIsContextOverflow } from "@yemu/model-runtime/error";
import { complete } from "@yemu/model-runtime/stream";
import type { AssistantMessage, Context, Model, Usage } from "@yemu/model-runtime/types";
...
async function discoverLmStudioModel(): Promise<LmStudioDiscoveredModel | undefined> {
	if (!Bun.env.YEMU_LOCAL_LLM || Bun.env.YEMU_NO_LOCAL_LLM) return undefined;
...
describe("Context overflow error handling", () => {
	...
	// Ollama tests require YEMU_LOCAL_LLM=1 and ollama installed
	const ollamaInstalled = !!Bun.env.YEMU_LOCAL_LLM && !!$which("ollama");
	...
	// =============================================================================
	// LM Studio (local) - requires YEMU_LOCAL_LLM=1 and a visible local model
	// =============================================================================
