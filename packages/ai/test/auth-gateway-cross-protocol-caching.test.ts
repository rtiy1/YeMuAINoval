 ...
 *     → openai-responses parser → yemu Context
 *     → model-runtime anthropic provider (auto cache_control via cacheRetention)
 ...
 * with the gateway live (`yemu auth-gateway serve` or pm2).
 */
...
const MODEL = Bun.env.YEMU_E2E_ANTHROPIC_MODEL ?? "claude-sonnet-4-5";
