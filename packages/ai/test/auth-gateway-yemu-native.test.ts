...
import { startAuthGateway } from "@yemu/model-runtime/auth-gateway";
import { AuthStorage } from "@yemu/model-runtime/auth-storage";
import { createMockModel, registerMockApi } from "@yemu/model-runtime/providers/mock";
import { encodeStream, formatError, parseRequest } from "@yemu/model-runtime/providers/yemu-native-server";
import type {
	AssistantMessage,
...
describe("yemu-native parseRequest", () => {
...
describe("yemu-native gateway cache controls", () => {
	it("delivers statefulResponses false to the provider stream", async () => {
		registerMockApi();
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "gw-yemu-native-cache-"));
		const storage = await AuthStorage.create(path.join(dir, "auth.db"));
		storage.setRuntimeApiKey("openrouter", "test-key");
		const mock = createMockModel({ provider: "openrouter", id: "yemu-native-cache" });
		...
		try {
			...
			const response = await fetch(`${handle.url}/v1/yemu/stream`, {
				method: "POST",
				headers: { Authorization: "Bearer test-token", "Content-Type": "application/json" },
				body: JSON.stringify({
					modelId: "yemu-native-cache",
					context: baseContext,
					options: { promptCacheKey: "bench-cache-pair", statefulResponses: false },
					stream: false,
				}),
...
describe("yemu-native encodeStream", () => {
	it("ships every AssistantMessageEvent verbatim, terminated by [DONE]", async () => {
		// YeMu-native is yemu-talks-to-yemu: the client feeds parsed events directly
...
describe("yemu-native formatError", () => {
