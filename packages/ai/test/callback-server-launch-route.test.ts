...
import { OAuthCallbackFlow } from "@yemu/model-runtime/registry/oauth/callback-server";
import type { OAuthAuthInfo, OAuthCredentials } from "@yemu/model-runtime/registry/oauth/types";
...
describe("OAuthCallbackFlow /launch route", () => {
	...
	it("suppresses launchUrl and routes /launch to the callback handler when callbackPath is /launch", async () => {
		...
		const flow = new LaunchProbeFlow(
			...
			// Caller pins the provider redirect at `/launch` — an YeMu config
