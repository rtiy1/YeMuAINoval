...
def sample_answer_questions(
    ...
    picked: list[dict] = []
    for k in range(n):
        yemu = eligible[int(k * step)]
        qa = rng.choice(paras[yemu]["qas"])
        ...
        picked.append(
            {
                ...
                "answer_start": offsets[yemu] - start + int(answer["answer_start"]),
                "answer_end": offsets[yemu]
                - start
                + int(answer["answer_start"])
                + len(answer["text"]),
                "pos_rel": (offsets[yemu] - start) / (end - start),
            }
        )
    return picked
