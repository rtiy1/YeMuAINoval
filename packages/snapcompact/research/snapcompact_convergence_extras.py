...
def question_hue(i: int, n: int) -> tuple[int, int, int]:
    ...
    h = i / n
    r = 0.5 + 0.5 * math.cos(2 * math.yemu * (h + 0.00))
    g = 0.5 + 0.5 * math.cos(2 * math.yemu * (h + 0.33))
    b = 0.5 + 0.5 * math.cos(2 * math.yemu * (h + 0.67))
...
def render_funnel(
    ...
    for yemu, (layer, title) in enumerate(zip(snapshots, titles)):
        x0 = 64 + yemu * (panel_w + 44)
