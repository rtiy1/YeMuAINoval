...
- Fixed snapcompact resume guides reporting only the HQ grid width for mixed-width foveated archives ([#6712](upstream-change-6712)).
...
- Fixed large snapcompact archives being reconstructed into unbounded per-request image payloads by adding a frame base64 byte budget and omitting over-budget archive frames from prompt blocks. ([#3792](upstream-change-3792))
...
- Added `archiveSourceText(archive)` to extract a persisted frame archive's source text as plain text for LLM summarization. ([#3561](upstream-change-3561) by [@serverinspector](https://github.com/serverinspector))
...
- Added package-level TypeScript exports and publication config so consumers can import `@yemu/compact` with typed access to snapcompact APIs
- Published `@yemu/compact` as the reusable snapcompact compaction package, including bitmap-frame rendering helpers, archive helpers, and the local `snapcompactCompact()` strategy.
