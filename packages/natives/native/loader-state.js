...
/**
 * Native addon loader for `@yemu/native`.
 *
 * Owns every step between "Node imports `native/index.js`" and "the right
 * `yemu_natives.<platform>-<arch>*.node` is required, validated, and returned":
 ...
 *
 * Background (issue #823): `bun build --compile --define YEMU_COMPILED=true`
 * substitutes the bare identifier `YEMU_COMPILED`, NOT `process.env.YEMU_COMPILED`,
...
/**
 * Streaming startup marker, enabled by `YEMU_DEBUG_STARTUP`. Local copy of the
 * yemu-utils helper (this loader cannot depend on yemu-utils). Synchronous on
 * purpose: extraction/dlopen hangs must still leave the `:start` marker.
 * @param {string} text
 */
function startupMarker(text) {
	if (!process.env.YEMU_DEBUG_STARTUP) return;
	try {
...
function getNativesDir() {
	...
	if (xdgDataHome && fs.existsSync(path.join(xdgDataHome, "yemu"))) {
		return path.join(xdgDataHome, "yemu", "natives");
	}
	return path.join(os.homedir(), ".yemu", "natives");
}

function resolveLeafPackageDir(platformTag) {
	try {
		const require_ = createRequire(import.meta.url);
		return path.dirname(require_.resolve(`@yemu/native-${platformTag}/package.json`));
	} catch {
		return null;
	}
}
...
export function detectCompiledBinary({ embeddedAddon, env, importMetaUrl }) {
	...
	if (env && env.YEMU_COMPILED) return true;
...
export function getAddonFilenames({ tag, arch, variant }) {
	const defaultFilename = `yemu_natives.${tag}.node`;
	if (arch !== "x64" || !variant) return [defaultFilename];
	const baselineFilename = `yemu_natives.${tag}-baseline.node`;
	const modernFilename = `yemu_natives.${tag}-modern.node`;
	if (variant === "modern") {
 ...
 * into the per-version cache directory (`~/.yemu/natives/<version>/`) before loading.
 *
 * Windows-only safety net for `bun install -g` updates: when a previous `yemu`
 * process is running, bun cannot overwrite the locked `.node` inside
 * `node_modules/@yemu/native/native/`, leaving an old binary next to a
 * newer `index.js` and producing `<sym> is not a function` crashes on the next
 * launch. Staging into the version-pinned cache:
 *   1. Gives every package version its own filesystem path, so concurrent yemu
 ...
 */
const VARIANT_CACHE_ENV_KEY = "__YEMU_NATIVE_VARIANT_CACHE";
...
function getVariantOverride() {
	const value = process.env.YEMU_NATIVE_VARIANT;
 ...
 *
 *   1. `override` (user-facing `YEMU_NATIVE_VARIANT` env var). Always wins.
 *   2. The private `__YEMU_NATIVE_VARIANT_CACHE` env var, populated by the first
...
export function validateLoadedBindings(ctx, bindings, candidate) {
	...
	const residentSentinel = Object.keys(bindings).find(
		key => key !== ctx.versionSentinelExport && /^__yemuNativesV[A-Za-z0-9_]+$/.test(key),
	);
	...
	if (residentSentinel && diskHasExpectedSentinel) {
		const residentVersion = residentSentinel.slice("__yemuNativesV".length).replace(/_/g, ".");
		throw new Error(
			`Loaded ${candidate}, which exposes the @yemu/native@${residentVersion} version ` +
				`sentinel \`${residentSentinel}\` but not the @${ctx.packageVersion} sentinel ` +
				`\`${ctx.versionSentinelExport}\` this loader expects. yemu was upgraded to ` +
				`${ctx.packageVersion} while this session was running; the ${residentVersion} addon is ` +
				"still resident in this process. Disk is already consistent — restart yemu to pick up " +
				`${ctx.packageVersion} (reinstalling changes nothing).`,
		);
	}
	throw new Error(
		`Loaded ${candidate} but it does not expose the @yemu/native@${ctx.packageVersion} ` +
 ...
 * deadlocks on some hosts — so it exposes `__yemuInstallTokioRuntime` for the
...
function installNativeTokioRuntime(bindings) {
	const install = bindings.__yemuInstallTokioRuntime;
...
function buildHelpMessage(ctx) {
	if (ctx.isCompiledBinary) {
		...
		const downloadHints = ctx.addonFilenames
			.map(filename => {
				const downloadUrl = `https://github.com/rtiy1/YeMuAINoval/releases/latest/download/${filename}`;
				const targetPath = path.join(ctx.versionedDir, filename);
	...
	return (
		"If installed via npm/bun, try reinstalling: bun install @yemu/native\n" +
		"If developing locally, regenerate host bindings with: " +
...
function initLoaderContext() {
	...
	const userDataDir =
		process.platform === "win32"
			? path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local"), "yemu")
	...
	// the package version (`__yemuNativesV{major}_{minor}_{patch}`).
	// `scripts/release.ts` bumps the name in `crates/yemu-native/src/lib.rs` in
	...
	const versionSentinelExport = `__yemuNativesV${packageVersion.replace(/[^A-Za-z0-9]/g, "_")}`;
...
export function loadNative() {
	...
	throw new Error(
		`Failed to load yemu_natives native addon for ${ctx.addonLabel}.\n\nTried:\n${details}\n\n${buildHelpMessage(ctx)}`,
	);
}
