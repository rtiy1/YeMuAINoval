 ...
 * (native/index.d.ts) and the runtime enum exports. Run this only when the
 * Rust API changes its exported typedefs. Host target only, local cargo
 * profile — no cross-compilation.
 */
...
const rustDir = path.join(repoRoot, "crates/yemu-natives");
...
async function resolveBuiltAddonPath(outputDir: string, canonicalFilename: string): Promise<string> {
	...
	const generatedCandidates = entries.filter(
		entry => entry.startsWith(`yemu_natives.${process.platform}-${process.arch}`) && entry.endsWith(".node"),
	);
...
const canonicalAddonFilename = `yemu_natives.${process.platform}-${process.arch}${variantSuffix}.node`;
const canonicalAddonPath = path.join(nativeDir, canonicalAddonFilename);

console.log(`Building YeMu native bindings for ${process.platform}-${process.arch}${variantSuffix} (local)…`);
