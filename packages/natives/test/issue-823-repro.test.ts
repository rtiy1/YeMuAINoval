/**
 * Regression for upstream-change-823.
 *
 * On WSL (and any host where the user moves the standalone binary away from the
 * build-time native artifacts), the compiled `yemu` binary fails to load
 * `yemu_natives.linux-x64-*.node`. Root cause: the old loader's
 * `isCompiledBinary` detection relied on signals that are unreliable in a Bun
 * standalone binary:
 *   - `process.env.YEMU_COMPILED` — never set, because `bun build --compile
 *     --define YEMU_COMPILED=true` substitutes the bare identifier, not
 ...
 * `execDir`. On WSL with `~/.local/bin/yemu` and no sibling `.node` file, this
...
describe("issue 823: standalone-binary native loader path resolution", () => {
	it("detects compiled-binary mode from embedded-addon presence when env and url markers are absent", () => {
		...
		// - `process.env.YEMU_COMPILED` is undefined (the build flag does not substitute property accesses).
			...
			detectCompiledBinary({
				embeddedAddon: {
					...
					files: [
						{
							variant: "modern",
							filename: "yemu_natives.linux-x64-modern.node",
							filePath: "/$bunfs/root/packages/natives/native/yemu_natives.linux-x64-modern.node",
						},
					],
				},
				env: {},
		...
		// Env override (e.g. user-set YEMU_COMPILED=1) still wins.
		expect(
			detectCompiledBinary({
				embeddedAddon: null,
				env: { YEMU_COMPILED: "1" },
	...
	it("places embedded-extracted candidates ahead of build-host candidates for linux-x64 standalone", () => {
		const versionedDir = "/home/u/.yemu/natives/14.5.2";
		...
		const versionedModern = path.join(versionedDir, "yemu_natives.linux-x64-modern.node");
		const versionedBaseline = path.join(versionedDir, "yemu_natives.linux-x64-baseline.node");
		const userDataModern = path.join(userDataDir, "yemu_natives.linux-x64-modern.node");
		const buildHostModern = path.join(nativeDir, "yemu_natives.linux-x64-modern.node");

		// Versioned cache and user-data dir candidates must exist for compiled binaries —
		// these are where the embedded-addon extraction lands (~/.yemu/natives/<v>) and where
		// `yemu update` writes the standalone binary on linux (~/.local/bin).
	...
	it("does not probe user-data candidates when running outside a standalone binary", () => {
		...
		expect(candidates).not.toContain(path.join(versionedDir, "yemu_natives.linux-x64-baseline.node"));
		expect(candidates).not.toContain(path.join(userDataDir, "yemu_natives.linux-x64-baseline.node"));
	});

	it("prefers platform leaf package candidates ahead of core nativeDir candidates on npm installs", () => {
		const leafPackageDir = "/app/node_modules/@yemu/native-linux-x64";
		const nativeDir = "/app/node_modules/@yemu/native/native";
		const candidates = resolveLoaderCandidates({
			addonFilenames: getAddonFilenames({ tag: "linux-x64", arch: "x64", variant: "baseline" }),
			isCompiledBinary: false,
			leafPackageDir,
			...
			versionedDir: "/home/u/.yemu/natives/15.5.15",
			userDataDir: "/home/u/.local/bin",
		});

		const leafBaseline = path.join(leafPackageDir, "yemu_natives.linux-x64-baseline.node");
		const coreBaseline = path.join(nativeDir, "yemu_natives.linux-x64-baseline.node");
	...
	it("keeps Windows staging ahead of leaf package and core nativeDir candidates", () => {
		const versionedDir = "/home/u/.yemu/natives/15.5.15";
		const leafPackageDir = "/app/node_modules/@yemu/native-win32-x64";
		const nativeDir = "/app/node_modules/@yemu/native/native";
		const candidates = resolveLoaderCandidates({
			...
			execDir: "/app/node_modules/.bin",
			versionedDir,
			userDataDir: "/home/u/AppData/Local/yemu",
		});

		const stagedBaseline = path.join(versionedDir, "yemu_natives.win32-x64-baseline.node");
		const leafBaseline = path.join(leafPackageDir, "yemu_natives.win32-x64-baseline.node");
		const coreBaseline = path.join(nativeDir, "yemu_natives.win32-x64-baseline.node");
	...
	it("extracts all bundled native variants from one gzip archive and skips current files", async () => {
		...
		try {
			...
			const modernFilename = "yemu_natives.linux-x64-modern.node";
			const baselineFilename = "yemu_natives.linux-x64-baseline.node";
			await Bun.write(
				archivePath,
