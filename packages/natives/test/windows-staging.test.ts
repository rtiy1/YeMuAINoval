/**
 * Regression for the Windows `bun install -g` update path: when an `yemu`
 * process is running, bun cannot overwrite a locked
 * `node_modules/@yemu/native/native/yemu_natives.win32-x64.node` during
 ...
 *      (per-package-version cache under `~/.yemu/natives/<version>/`) so the
...
const winNodeModulesNativeDir = "C:\\Users\\Admin\\node_modules\\@yemu\\yemu-native\\native";
const winWorkspaceNativeDir = "C:\\Users\\Admin\\dev\\yemu-agent\\packages\\natives\\native";
const posixNodeModulesNativeDir = "/home/u/proj/node_modules/@yemu/native/native";
...
describe("windows native addon staging", () => {
	...
	it("prepends versionedDir candidates ahead of node_modules when staging on Windows", () => {
		const versionedDir = "C:\\Users\\Admin\\.yemu\\natives\\15.0.1";
		const userDataDir = "C:\\Users\\Admin\\AppData\\Local\\yemu";
		...
		const versionedBaseline = path.join(versionedDir, "yemu_natives.win32-x64-baseline.node");
		const versionedDefault = path.join(versionedDir, "yemu_natives.win32-x64.node");
		const nodeModulesBaseline = path.join(winNodeModulesNativeDir, "yemu_natives.win32-x64-baseline.node");
		...
		// addon anyway).
		const userDataBaseline = path.join(userDataDir, "yemu_natives.win32-x64-baseline.node");
	...
	it("falls back to the node_modules-only candidate list when staging is off", () => {
		...
		const versionedDir = "/home/u/.yemu/natives/15.0.1";
		const candidates = resolveLoaderCandidates({
		...
		const nodeModulesBaseline = path.join(posixNodeModulesNativeDir, "yemu_natives.linux-x64-baseline.node");
	...
	it("removes only older version directories after the current native version loads", async () => {
		const nativesDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-natives-cache-"));
...
describe("yemu-native version sentinel", () => {
	it("Rust `js_name` matches the package version", async () => {
		...
		const libRs = await Bun.file(path.join(import.meta.dir, "../../../crates/yemu-native/src/lib.rs")).text();
		const sentinelMatch = libRs.match(/js_name = "(__yemuNativesV[A-Za-z0-9_]+)"/);
		expect(sentinelMatch, 'Rust sentinel `js_name = "__yemuNativesV…"` not found in lib.rs').not.toBeNull();
		const expected = `__yemuNativesV${packageJson.version.replace(/[^A-Za-z0-9]/g, "_")}`;
