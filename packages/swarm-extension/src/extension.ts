 ...
 * then use /swarm in any yemu-agent session.
 */

import * as fs from "node:fs/promises";
import * as path from "node:path";
import type { ExtensionAPI, ExtensionCommandContext } from "@yemu/agent-runtime";
import { formatDuration } from "@yemu/utils";
...
export default function swarmExtension(yemu: ExtensionAPI): void {
	yemu.setLabel("Swarm Orchestrator");

	yemu.registerCommand("swarm", {
		...
		handler: async (args: string, ctx: ExtensionCommandContext) => {
			...
			switch (subcommand) {
				case "run": {
					...
					await handleRun(yamlPath, ctx, yemu);
					return;
				}
				case "status": {
...
async function handleRun(yamlPath: string, ctx: ExtensionCommandContext, yemu: ExtensionAPI): Promise<void> {
	...
	yemu.logger.debug("Swarm starting", {
		name: def.name,
		mode: def.mode,
		agents: agentList,
		waves: waveDesc,
	...
	const result = await controller.run({
		...
		settings: yemu.runtime.settings,
	});
	...
	// Log errors
	if (result.errors.length > 0) {
		yemu.logger.warn("Swarm completed with errors", { errors: result.errors });
	}
