...
import { workerHostEntry } from "@yemu/utils";
import {
...
// JavaScript entry. Standalone source `yemu-stats` keeps using this package's
...
function defaultWorkerCount(): number {
	...
	// the compiled `yemu` binary. Keep macOS on the documented serial path.
 ...
 * self-dispatching CLI entry (yemu in source, npm-bundle, or compiled form),
 * re-enter that entry with a worker argv selector; otherwise (standalone
 * yemu-stats, bun test, SDK embedding) load the worker module directly, so this
 * package keeps zero runtime dependency on `@yemu/agent-runtime`.
 */
function createSyncWorker(): Worker {
	const hostEntry = workerHostEntry();
	if (hostEntry) {
		return new Worker(hostEntry, { type: "module", argv: ["__yemu_worker_stats_sync"] });
	}
 ...
 * then terminates. Used by `yemu --smoke-test` so the install-method CI jobs
