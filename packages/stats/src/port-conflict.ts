...
import { $which } from "@yemu/utils";
import { $ } from "bun";

const STATS_PROBE_TIMEOUT_MS = 500;
const PROCESS_EXIT_POLL_MS = 50;
const PROCESS_EXIT_POLLS = 10;
const STATS_RUNTIME_IMAGES: Record<string, true> = { bun: true, node: true, yemu: true, "yemu-stats": true };
...
export const STATS_DASHBOARD_HEADER = "x-yemu-stats-dashboard";
...
async function probeStatsDashboard(port: number): Promise<boolean> {
	try {
		...
		// A live yemu-stats dashboard stamps this header on every response.
...
/** Reuse a live stats dashboard or reclaim the port from a stale yemu runtime. */
export async function recoverStatsPort(port: number): Promise<"retry" | "reuse"> {
	...
	const hasStatsIdentity =
		normalizedImage === "yemu-stats" ||
		/(?:^|[/"'\s])yemu-stats(?:\.exe)?(?:["'\s]|$)/.test(normalizedCommand) ||
		/\/packages\/stats\/src\/index\.ts(?:["'\s]|$)/.test(normalizedCommand) ||
		(normalizedImage === "yemu" && /(?:^|\s)stats(?:\s|$)/.test(normalizedCommand)) ||
		/(?:^|\/)yemu(?:\.exe)?["'\s]+stats(?:["'\s]|$)/.test(normalizedCommand);
	if (!STATS_RUNTIME_IMAGES[normalizedImage] || !hasStatsIdentity) {
		throw new Error(
			`Port ${port} is in use by ${holder.image} (PID ${holder.pid}), which is not identifiable as an yemu stats dashboard; refusing to stop it.`,
		);
	}
