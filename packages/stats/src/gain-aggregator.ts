...
import { getStatsDbPath, isEnoent, logger } from "@yemu/utils";
...
const TEMP_PATH_RE = /(?:^|\/)(?:T|tmp|yemu-bash-exec|yemu-bash-exec|yemu-bash-detach)(?:\/|$)|^\/var\/folders(?:\/|$)/;
 ...
 *
 * Rules are generic: yemu internal wt paths are dropped; conventional worktree
...
export function normalizeProjectPath(p: string): string | null {
	...
	if (/\/\.yemu\/wt\//u.test(clean)) return null;
