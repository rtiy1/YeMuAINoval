...
import { isEnoent } from "@yemu/utils";
import { $ } from "bun";
import {
...
const IS_BUN_COMPILED =
	Boolean(process.env.YEMU_COMPILED || Bun.env.YEMU_COMPILED) ||
	import.meta.url.includes("$bunfs") ||
	import.meta.url.includes("~BUN") ||
	import.meta.url.includes("%7EBUN");
// The prepacked npm bundle (coding-agent dist/cli.js) constant-folds
// process.env.YEMU_BUNDLED at build time. Like compiled binaries, it ships no
// dashboard sources or prebuilt dist/client next to the bundle, so the
// embedded archive is the only viable asset source.
const IS_PREBUILT = IS_BUN_COMPILED || Boolean(process.env.YEMU_BUNDLED || Bun.env.YEMU_BUNDLED);
const USE_EMBEDDED_CLIENT = EMBEDDED_CLIENT_ARCHIVE !== null || IS_PREBUILT;

const EMBEDDED_CLIENT_DIR_ROOT = path.join(os.tmpdir(), "yemu-stats-client");
...
async function getEmbeddedClientDir(): Promise<string> {
	...
	if (!EMBEDDED_CLIENT_ARCHIVE) {
		throw new Error(
			"Embedded stats client bundle missing. Rebuild the yemu binary or npm bundle with embedded stats assets.",
		);
	}
...
function createDashboardServer(port: number) {
	const server = Bun.serve({
		...
		async fetch(req) {
			...
			// yemu session's reuse probe positively recognize this dashboard.
...
/**
 * Start the HTTP server, reusing a live dashboard or reclaiming a stale yemu listener.
 */
