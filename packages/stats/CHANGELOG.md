...
- Added a Tools tab to the `yemu stats` dashboard (`/#/tools`): per-tool call counts, error rates, result/argument payload sizes, per-model breakdown, and a stacked calls-over-time chart. Token and cost columns attribute each invoking turn's real provider usage evenly across that turn's tool calls. Existing databases re-parse sessions once on the next sync to backfill historical tool calls.
...
- Fixed application crashes and Bun aborts on macOS and when parsing large stats session files, including during `yemu --smoke-test` runs, by utilizing a more resilient serial parser and lenient line scanner.
...
- Redesigned the local stats dashboard with an YeMu-themed product shell, dedicated per-section views, accessible loading/empty/error states, and flicker-free navigation between screens and time ranges.
...
- Renamed `__yemu_stats_sync_worker` to `__yemu_worker_stats_sync`.
...
- Fixed the stats dashboard's SQLite init never setting `PRAGMA busy_timeout`, so a concurrent `yemu` startup hitting WAL recovery could crash `initDb()` with `SQLITE_BUSY` instead of waiting through it. The busy handler is now installed before `PRAGMA journal_mode=WAL` ([#2421](upstream-change-2421)).

## [15.11.0] - 2026-06-10

### Added

- Added support for prebuilt npm bundle mode via `YEMU_BUNDLED`, allowing the stats server to use an embedded dashboard bundle in packaged CLI distributions
...
- Bundled-model lookups (`getBundledModel`, `GeneratedProvider`) now import from the new `@yemu/model-catalog` package instead of the `@yemu/model-runtime` barrel, which no longer re-exports catalog values
- The session-sync worker re-enters the host CLI entry (`workerHostEntry()` + `__yemu_stats_sync_worker` argv selector) when running inside yemu — source, npm bundle, or compiled binary — and keeps loading its own `sync-worker.ts` module directly for standalone `yemu-stats`, bun test, and SDK hosts

## [15.1.6] - 2026-05-19

### Fixed

- Fixed `yemu stats` crashing on first session sync in published `yemu-{linux,darwin,windows}-*` binaries with `BuildMessage: ModuleNotFound resolving "./packages/stats/src/sync-worker.ts"`; the release build script now lists the stats sync, browser tab, and JS eval workers as explicit `--compile` entrypoints so Bun emits them into bunfs, matching the dev build script and the AGENTS.md worker spawn contract. ([#1150](upstream-change-1150))
...
- Fixed `yemu stats` in compiled binaries by using the serial sync path instead of spawning a raw file-asset worker that cannot import bundled parser code.
...
- Fixed `yemu stats` failing to build the client on globally-installed installs by promoting `tailwindcss` from `devDependencies` to `dependencies` (the client build runs at runtime)
...
- Include subtask session files in usage stats ([#250](upstream-change-250))
