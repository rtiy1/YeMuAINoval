...
import { dedupeProjects, getGainDashboardStats, normalizeProjectPath } from "@yemu/stats/gain-aggregator";
import type { MessageStats } from "@yemu/stats/types";
import { getStatsDbPath } from "@yemu/utils";
import { installStatsTestIsolation } from "./helpers/temp-agent";

installStatsTestIsolation("@yemu-stats-gain-");
...
describe("gain project normalization", () => {
	it("normalizes conventional worktree paths without author-specific path rules", () => {
		...
		expect(normalizeProjectPath("/tmp/yemu-bash-exec/session")).toBeNull();
	});
