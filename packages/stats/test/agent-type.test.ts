...
import { getOverviewStats } from "@yemu/stats/aggregator";
import { getStatsByAgentType, initDb, insertMessageStats } from "@yemu/stats/db";
import { classifyAgentType } from "@yemu/stats/parser";
import type { AgentType, MessageStats } from "@yemu/stats/types";
import { getConfigRootDir, getSessionsDir, getStatsDbPath } from "@yemu/utils";
import { installStatsTestIsolation } from "./helpers/temp-agent";

installStatsTestIsolation("@yemu-stats-agent-type-");
...
describe("classifyAgentType", () => {
	it("classifies transcripts by their location inside the session directory", () => {
		const project = path.join(getSessionsDir(), "--work--yemu");
