 ...
 *
 * The default profile's stats.db is redirected to `$XDG_DATA_HOME/yemu/stats.db`
 * by {@link DirResolver} whenever `agentDirOverride === defaultAgent`. Tests
 * that only set `YEMU_CONFIG_DIR` + `setAgentDir(<home>/<config>/agent)` resolve
 ...
 * `YEMU_CONFIG_DIR` for the test, points the agent directory at a fresh
...
import { closeDb } from "@yemu/stats/db";
import { getAgentDir, setAgentDir, TempDir } from "@yemu/utils";
