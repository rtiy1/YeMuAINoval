/**
 * `<yemu-tool-view>` — web-component wrapper around ToolView for non-React
 * hosts (the exported-session HTML page).
 *
 * Payload sources, in priority order:
 * 1. `el.data = {...}` property assignment.
 * 2. `data-key` attribute → lookup in `globalThis.__YEMU_TOOL_VIEW_DATA`
...
export class YemuToolViewElement extends HTMLElement {
	#root: Root | null = null;
	...
	#resolveProps(): ToolViewProps | null {
		...
		if (key) {
			const store = (globalThis as { __YEMU_TOOL_VIEW_DATA?: PayloadStore }).__YEMU_TOOL_VIEW_DATA;
...
export function defineToolViewElement(tag = "yemu-tool-view"): void {
	if (!customElements.get(tag)) customElements.define(tag, YemuToolViewElement);
}
