process.env.YEMU_TUI_SCROLLBACK_REBUILD = "true";

import { afterEach, beforeEach, describe, expect, it, vi } from "bun:test";
import { type Component, CURSOR_MARKER, type Focusable, setTerminalScreenToScrollback, TERMINAL, TUI } from "@yemu/tui";
...
describe("TUI terminal-state regressions", () => {
	...
	beforeEach(() => {
		...
		for (const key of ["TERM_PROGRAM", "YEMU_TUI_RESIZE_IN_PLACE"]) {
	...
	describe("screen clearing", () => {
		...
		it("clears stale screen content on a supported non-destructive paint when the terminal ignores CSI 22 J", async () => {
			...
			await term.flush();
			const tui = new TUI(term);
			tui.addChild(new MutableLinesComponent(["yemu line 1", "yemu line 2"]));
			...
			try {
				...
				expect(viewport).toContain("yemu line 1");
			} finally {
				tui.stop();
				setTerminalScreenToScrollback(saved);
			}
		...
		it("keeps destructive paints synchronized when DECRQM is unavailable", async () => {
			await withEnvPatch(
				{
					TERM_FEATURES: "Sy",
					YEMU_NO_SYNC_OUTPUT: undefined,
					YEMU_FORCE_SYNC_OUTPUT: undefined,
					YEMU_TUI_SYNC_OUTPUT: undefined,
				},
				async () => {
	...
	describe("scrollback integrity", () => {
		...
		it("rebuilds an offscreen expansion identically when the viewport probe is unavailable", async () => {
			...
			// the parked-reader anchor (upstream yemu semantics).
		...
		it("rebuilds an offscreen expansion and snaps a parked reader to the tail", async () => {
			...
			// bottom — the price of exactly-once history (upstream yemu semantics).
			const term = new VirtualTerminal(32, 5);
			const tui = new TUI(term);
			const component = new MutableLinesComponent(rows("line-", 12));
			tui.addChild(component);
		...
		it("rebuilds a huge collapse and snaps a parked reader to the tail", async () => {
			...
			// stale history is no longer preserved for them (upstream yemu
		...
		it("leaves the parent shell prompt directly after short content on stop", async () => {
			...
			const tui = new TUI(term);
			let stopped = false;
			tui.addChild(new MutableLinesComponent(["yemu0", "yemu1", "yemu2"]));
			...
			try {
				...
				expect(visible(term)).toEqual(["yemu0", "yemu1", "yemu2", "bash$", ""]);
			} finally {
				if (!stopped) tui.stop();
			}
		});
