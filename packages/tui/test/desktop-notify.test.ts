...
describe("shouldDeliverDesktopNotification", () => {
	...
	it("respects the YEMU_NO_DESKTOP_NOTIFY=1 opt-out", () => {
		expect(
			shouldDeliverDesktopNotification("trueColor", true, "linux", {
				...LINUX_ENV,
				YEMU_NO_DESKTOP_NOTIFY: "1",
			}),
		).toBe(false);
	});
...
describe("buildDesktopNotifyCommand", () => {
	...
	it("produces a freedesktop Notify call for gdbus including the urgency hint byte", () => {
		expect(buildDesktopNotifyCommand(gdbus, { title: "YeMu Agent", body: "ping", urgency: "low" })).toEqual([
			"/usr/bin/gdbus",
			"call",
			"--session",
			"--dest",
