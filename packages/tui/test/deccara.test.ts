...
describe("detectRectangularSgrSupport", () => {
	...
	it("honors the YEMU_NO_DECCARA kill switch (truthy values only)", () => {
		expect(detectRectangularSgrSupport("kitty", { YEMU_NO_DECCARA: "1" })).toBe(false);
		expect(detectRectangularSgrSupport("kitty", { YEMU_NO_DECCARA: "true" })).toBe(false);
		// A falsey assignment is not a kill: support stays on.
		expect(detectRectangularSgrSupport("kitty", { YEMU_NO_DECCARA: "0" })).toBe(true);
		expect(detectRectangularSgrSupport("kitty", { YEMU_NO_DECCARA: "false" })).toBe(true);
	});
...
describe("TUI DECCARA integration", () => {
	...
	const savedForceSyncOutput = Bun.env.YEMU_FORCE_SYNC_OUTPUT;
	const savedNoSyncOutput = Bun.env.YEMU_NO_SYNC_OUTPUT;
	const savedTuiSyncOutput = Bun.env.YEMU_TUI_SYNC_OUTPUT;

	beforeEach(() => {
		Bun.env.YEMU_FORCE_SYNC_OUTPUT = "1";
		process.env.YEMU_FORCE_SYNC_OUTPUT = "1";
		delete Bun.env.YEMU_NO_SYNC_OUTPUT;
		delete process.env.YEMU_NO_SYNC_OUTPUT;
		delete Bun.env.YEMU_TUI_SYNC_OUTPUT;
		delete process.env.YEMU_TUI_SYNC_OUTPUT;
		let monotonic = 0;
	...
	afterEach(() => {
		if (savedForceSyncOutput === undefined) {
			delete Bun.env.YEMU_FORCE_SYNC_OUTPUT;
			delete process.env.YEMU_FORCE_SYNC_OUTPUT;
		} else {
			Bun.env.YEMU_FORCE_SYNC_OUTPUT = savedForceSyncOutput;
			process.env.YEMU_FORCE_SYNC_OUTPUT = savedForceSyncOutput;
		}
		if (savedNoSyncOutput === undefined) {
			delete Bun.env.YEMU_NO_SYNC_OUTPUT;
			delete process.env.YEMU_NO_SYNC_OUTPUT;
		} else {
			Bun.env.YEMU_NO_SYNC_OUTPUT = savedNoSyncOutput;
			process.env.YEMU_NO_SYNC_OUTPUT = savedNoSyncOutput;
		}
		if (savedTuiSyncOutput === undefined) {
			delete Bun.env.YEMU_TUI_SYNC_OUTPUT;
			delete process.env.YEMU_TUI_SYNC_OUTPUT;
		} else {
			Bun.env.YEMU_TUI_SYNC_OUTPUT = savedTuiSyncOutput;
			process.env.YEMU_TUI_SYNC_OUTPUT = savedTuiSyncOutput;
		}
	...
	it("keeps padded fallback bytes when synchronized output is disabled", async () => {
		await withEnvPatch(
			{ YEMU_NO_SYNC_OUTPUT: "1", YEMU_FORCE_SYNC_OUTPUT: undefined, YEMU_TUI_SYNC_OUTPUT: undefined },
			async () => {
				setTerminalDeccara(true);
				const term = new VirtualTerminal(40, 8);
				const tui = new TUI(term);
