...
import { visibleWidth } from "@yemu/native";
import {
...
describe("detectKittyUnicodePlaceholdersSupport", () => {
	...
	it("uses scroll-aware placeholders when Kitty is explicitly forced through tmux", () => {
		const forced = env({ TMUX: "/tmp/tmux-1000/default,1,0", YEMU_FORCE_IMAGE_PROTOCOL: "kitty" });
	...
	it("honors YEMU_NO_KITTY_PLACEHOLDERS=1 as a hard off override on supporting terminals", () => {
		expect(detectKittyUnicodePlaceholdersSupport("kitty", env({ YEMU_NO_KITTY_PLACEHOLDERS: "1" }))).toBe(false);
		expect(detectKittyUnicodePlaceholdersSupport("ghostty", env({ YEMU_NO_KITTY_PLACEHOLDERS: "true" }))).toBe(false);
	});

	it("honors YEMU_KITTY_PLACEHOLDERS=1 as opt-in on otherwise-unsupported terminals", () => {
		expect(detectKittyUnicodePlaceholdersSupport("wezterm", env({ YEMU_KITTY_PLACEHOLDERS: "1" }))).toBe(true);
	});

	it("YEMU_NO_KITTY_PLACEHOLDERS beats YEMU_KITTY_PLACEHOLDERS when both are set", () => {
		const both = env({ YEMU_NO_KITTY_PLACEHOLDERS: "1", YEMU_KITTY_PLACEHOLDERS: "1" });
		expect(detectKittyUnicodePlaceholdersSupport("kitty", both)).toBe(false);
	});

	it("YEMU_KITTY_PLACEHOLDERS=0 forces off on a default-on terminal", () => {
		expect(detectKittyUnicodePlaceholdersSupport("kitty", env({ YEMU_KITTY_PLACEHOLDERS: "0" }))).toBe(false);
		expect(detectKittyUnicodePlaceholdersSupport("ghostty", env({ YEMU_KITTY_PLACEHOLDERS: "off" }))).toBe(false);
	});

	it("placeholder opt-out beats forced Kitty under tmux", () => {
		const forcedOff = env({
			TMUX: "/tmp/tmux-1000/default,1,0",
			YEMU_FORCE_IMAGE_PROTOCOL: "kitty",
			YEMU_KITTY_PLACEHOLDERS: "0",
		});
