...
import * as desktopNotify from "@yemu/tui/desktop-notify";
import { ProcessTerminal } from "@yemu/tui/terminal";
import {
	getTerminalInfo,
	isInsideTmux,
	isInsideZellij,
...
import { setTerminalHeadless } from "@yemu/utils";

const stdinIsTtyDescriptor = Object.getOwnPropertyDescriptor(process.stdin, "isTTY");
const stdoutIsTtyDescriptor = Object.getOwnPropertyDescriptor(process.stdout, "isTTY");
const stdinSetRawModeDescriptor = Object.getOwnPropertyDescriptor(process.stdin, "setRawMode");
const originalOsc99Probe = Bun.env.YEMU_TUI_OSC99_PROBE;
const originalTmux = Bun.env.TMUX;
const originalZellij = Bun.env.ZELLIJ;
const originalYemuNotifications = Bun.env.YEMU_NOTIFICATIONS;
...
describe("terminal notifications", () => {
	beforeEach(() => {
		...
		// `YEMU_NOTIFICATIONS=off` is set in this workspace's CI env, which would
		...
		delete Bun.env.YEMU_NOTIFICATIONS;
	});
	...
	afterEach(() => {
		...
		restoreEnv("YEMU_TUI_OSC99_PROBE", originalOsc99Probe);
		restoreEnv("TMUX", originalTmux);
		restoreEnv("ZELLIJ", originalZellij);
		restoreEnv("YEMU_NOTIFICATIONS", originalYemuNotifications);
	...
	it("queries and confirms OSC 99 support before rich notifications", () => {
		Bun.env.YEMU_TUI_OSC99_PROBE = "1";
		mutableTerminal.notifyProtocol = NotifyProtocol.Osc99;
		const { terminal, writes, received } = setupProcessTerminal();
		try {
			const query = writes.find(w => w.startsWith("\x1b]99;i=yemu-probe-") && w.endsWith("\x1b\\\x1b[c"));
	...
	it("under tmux, the OSC 99 capability probe is suppressed (reply cannot route back to the pane)", () => {
		...
		try {
			...
			const probe = writes.find(w => w.includes("]99;i=yemu-probe-") && w.includes(":p=?"));
