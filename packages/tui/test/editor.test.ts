...
import { CURSOR_MARKER, TUI } from "@yemu/tui";
import { CombinedAutocompleteProvider } from "@yemu/tui/autocomplete";
import { Editor } from "@yemu/tui/components/editor";
import { KeybindingsManager, setKeybindings, TUI_KEYBINDINGS } from "@yemu/tui/keybindings";
import { setKittyProtocolActive } from "@yemu/tui/keys";
import { visibleWidth } from "@yemu/tui/utils";
...
describe("Editor component", () => {
	...
	describe("Grapheme-aware text wrapping", () => {
		...
		it("keeps terminal-local IME preedit from displacing the editor border (#5563)", async () => {
			...
			try {
				...
				// committed bytes reach YeMu. The open cursor row must not carry right
	...
	describe("Korean NFC paste normalization", () => {
		...
		// landed on the legacy `Input` component; YeMu's interactive prompt
