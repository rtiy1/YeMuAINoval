...
import { $flag, getDebugLogPath } from "@yemu/utils";
 ...
 */
export const CURSOR_MARKER = "\x1b_yemu:c\x07";
 ...
 *
 * `YEMU_TUI_RESIZE_IN_PLACE=1|0` forces this on/off for any terminal.
 */
function reportsSizeOnAltScreenToggle(): boolean {
	const override = Bun.env.YEMU_TUI_RESIZE_IN_PLACE;
...
// must process. On by default; `YEMU_NO_SGR_COALESCE=1` disables it.
const SGR_COALESCE_ENABLED = !$flag("YEMU_NO_SGR_COALESCE");
const CC_ESC = 0x1b;
...
export class TUI extends Container {
	...
	#showHardwareCursor = $flag("YEMU_HARDWARE_CURSOR");
	...
	#scrollbackRebuildEnabled =
		Bun.env.YEMU_TUI_SCROLLBACK_REBUILD === "1" || Bun.env.YEMU_TUI_SCROLLBACK_REBUILD === "true";
	 ...
	 * An explicit `YEMU_TUI_RESIZE_IN_PLACE=0|false` suppresses the runtime latch;
	...
	/** Optional intent log under YEMU_DEBUG_REDRAW. */
	#logRedraw(intent: RenderIntent, newLength: number, height: number): void {
		if (!$flag("YEMU_DEBUG_REDRAW")) return;
		const detail =
			intent.kind === "update"
