...
export function emergencyTerminalRestore(): void {
	try {
		...
		if (terminal) {
			...
			// frame before leaving it, then let stop() pop yemu's main-screen frame.
			if (altScreenActive) {
				const keyboardExit =
...
export interface Terminal {
	...
	// Terminals built against older yemu-tui versions keep working.
	...
	// so custom Terminals built against older yemu-tui versions keep working.
	 ...
	 * Optional so custom Terminals built against older yemu-tui versions keep working.
	 */
	 ...
	 * without a restart. Optional so custom Terminals built against older yemu-tui
...
export class ProcessTerminal implements Terminal {
	...
	#writeLogPath = $env.YEMU_TUI_WRITE_LOG || "";
	...
	start(onInput: (data: string) => void, onResize: () => void, onDisconnect?: () => void): void {
		...
		// stderr-guard in yemu-utils (mirrors openai/codex#24459).
		suppressTerminalStderr();
		...
		// `rmkx` = "\x1b[?1l\x1b>"). yemu decodes both CSI ("\x1b[A") and SS3
		...
		// are disabled while YeMu owns the TTY so typing in the editor does not
	...
	#shouldQueryOsc99Support(): boolean {
		...
		return !isBunTestRuntime() || $env.YEMU_TUI_OSC99_PROBE === "1";
	}
	...
	#queryOsc99Support(): void {
		...
		const id = `yemu-probe-${nextOsc99ProbeId++}`;
	...
	stop(): void {
		...
		// navigation stays broken after yemu exits (#6374).
		this.#safeWrite("\x1b[?1l\x1b>");
