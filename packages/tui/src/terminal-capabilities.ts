import { encodeSixel } from "@yemu/native";
import { $env, isBunTestRuntime, isTerminalHeadless } from "@yemu/utils";
...
const CMUX_NOTIFICATION_TITLE = "YeMu Agent";
...
export function isNotificationSuppressed(): boolean {
	const value = $env.YEMU_NOTIFICATIONS;
	if (!value) return false;
	return value === "off" || value === "0" || value === "false";
}

function getForcedImageProtocol(): ImageProtocol | null | undefined {
	const raw = $env.YEMU_FORCE_IMAGE_PROTOCOL?.trim().toLowerCase();
...
export function synchronizedOutputUserOverride(env: NodeJS.ProcessEnv = Bun.env): boolean | null {
	if (env.YEMU_NO_SYNC_OUTPUT || env.YEMU_TUI_SYNC_OUTPUT === "0") return false;
	if (env.YEMU_FORCE_SYNC_OUTPUT === "1" || env.YEMU_TUI_SYNC_OUTPUT === "1") return true;
	return null;
}
 ...
 *   1. Explicit user override (`YEMU_NO_SYNC_OUTPUT`/`YEMU_TUI_SYNC_OUTPUT=0` off,
 *      `YEMU_FORCE_SYNC_OUTPUT=1`/`YEMU_TUI_SYNC_OUTPUT=1` on).
 ...
 * `YEMU_NO_DECCARA` kill switch. Pure helper for tests and `TERMINAL` construction.
 */
export function detectRectangularSgrSupport(terminalId: TerminalId, env: NodeJS.ProcessEnv = Bun.env): boolean {
	...
	const kill = env.YEMU_NO_DECCARA;
...
export function hyperlinksUserOverride(env: NodeJS.ProcessEnv = Bun.env): boolean | null {
	if (env.YEMU_NO_HYPERLINKS === "1") return false;
	if (env.YEMU_FORCE_HYPERLINKS === "1") return true;
	return null;
}
 ...
 *
 * Policy (highest precedence first):
 *   1. Explicit user override (`YEMU_NO_HYPERLINKS=1` off, `YEMU_FORCE_HYPERLINKS=1`
...
export const TERMINAL: RuntimeTerminal = (() => {
	...
	// YEMU_FORCE_HYPERLINKS / YEMU_NO_HYPERLINKS overrides plus a tmux>=3.4 gate so
	...
	// the YEMU_NO_DECCARA kill switch via detectRectangularSgrSupport — and force it
...
const OSC99_APP_NAME = "YeMu Agent";
...
function osc99Id(id: string | undefined): string {
	return sanitizeOsc99Id(id) || `yemu-${nextOsc99NotificationId++}`;
}
