...
import type { KeyEventType } from "@yemu/native";
import {
 ...
 * limited to direct Windows Terminal sessions. `YEMU_TUI_RAW_BACKSPACE_IS_CTRL=1`
...
export function matchesRawBackspace(data: string, expectedModifier: number): boolean {
	...
	const rawBackspaceIsCtrl =
		process.env.YEMU_TUI_RAW_BACKSPACE_IS_CTRL === "1" ||
 ...
 * `@yemu/tui` `Key` export verbatim so plugins built against any
 * scope alias (`@mariozechner`, `@earendil-works`, `@yemu`) keep working
