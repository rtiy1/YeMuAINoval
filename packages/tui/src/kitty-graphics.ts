 ...
 * know pane scroll/reflow state, so an explicit `YEMU_FORCE_IMAGE_PROTOCOL=kitty`
 ...
 *
 * `YEMU_NO_KITTY_PLACEHOLDERS=1` and `YEMU_KITTY_PLACEHOLDERS=0` remain hard
 * opt-outs; `YEMU_KITTY_PLACEHOLDERS=1` explicitly opts in anywhere else.
 */
export function detectKittyUnicodePlaceholdersSupport(terminalId: string, env: NodeJS.ProcessEnv = Bun.env): boolean {
	const offRaw = env.YEMU_NO_KITTY_PLACEHOLDERS?.trim().toLowerCase();
	if (offRaw === "1" || offRaw === "true" || offRaw === "on" || offRaw === "yes" || offRaw === "y") return false;
	const force = env.YEMU_KITTY_PLACEHOLDERS?.trim().toLowerCase();
	...
	if (env.TMUX && env.YEMU_FORCE_IMAGE_PROTOCOL?.trim().toLowerCase() === "kitty") return true;
