...
function resolveBank(args: ToolArguments): string {
	return stringArg(args, "bank") || process.env.YEMU_MEMORY_MCP_BANK || "default";
}
...
function createBeam(args: ToolArguments, bank = resolveBank(args)): BeamMemory {
	const sessionId = process.env.YEMU_MEMORY_SESSION_ID || `mcp_${bank}`;
	return new BeamMemory({
		sessionId,
		dbPath: bankDbPath(bank),
		authorId: optionalStringArg(args, "author_id") ?? process.env.YEMU_MEMORY_AUTHOR_ID ?? null,
		authorType: optionalStringArg(args, "author_type") ?? process.env.YEMU_MEMORY_AUTHOR_TYPE ?? null,
		channelId: optionalStringArg(args, "channel_id") ?? process.env.YEMU_MEMORY_CHANNEL_ID ?? sessionId,
	});
}

function sharedBeam(): BeamMemory {
	const configured = process.env.YEMU_MEMORY_SHARED_SURFACE_DB;
...
const TOOL_HANDLERS: Record<string, Handler> = {
	"yemu-memory_remember": handleRemember,
	"yemu-memory_recall": handleRecall,
	"yemu-memory_shared_remember": handleSharedRemember,
	"yemu-memory_shared_recall": handleSharedRecall,
	"yemu-memory_shared_forget": handleSharedForget,
	"yemu-memory_shared_stats": () => handleSharedStats(),
	"yemu-memory_sleep": handleSleep,
	"yemu-memory_stats": handleStats,
	"yemu-memory_get_stats": handleStats,
	"yemu-memory_invalidate": handleInvalidate,
	"yemu-memory_validate": handleValidate,
	"yemu-memory_get": handleGet,
	"yemu-memory_triple_add": handleTripleAdd,
	"yemu-memory_triple_query": handleTripleQuery,
	"yemu-memory_scratchpad_write": handleScratchpadWrite,
	"yemu-memory_scratchpad_read": handleScratchpadRead,
	"yemu-memory_scratchpad_clear": handleScratchpadClear,
	"yemu-memory_export": handleExport,
	"yemu-memory_update": handleUpdate,
	"yemu-memory_forget": handleForget,
	"yemu-memory_import": handleImport,
	"yemu-memory_diagnose": handleDiagnose,
	"yemu-memory_graph_query": handleGraphQuery,
	"yemu-memory_graph_link": handleGraphLink,
};
