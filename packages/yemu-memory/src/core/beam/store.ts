...
import { getYeMuMemoryRuntimeOptions, withYeMuMemoryRuntimeOptions } from "../runtime-options";
...
const SCRATCHPAD_MAX_ITEMS = Number.parseInt(process.env.YEMU_MEMORY_SP_MAX ?? "1000", 10);
...
function proactiveLinkingAllowed(beam: BeamMemoryState): boolean {
	const override = process.env.YEMU_MEMORY_PROACTIVE_LINKING;
 ...
 * `YeMuMemory.#withRuntimeOptions` has already exited by the time the task runs.
 */
function scheduleFactExtraction(beam: BeamMemoryState, memoryId: string, content: string): void {
	...
	const task = withYeMuMemoryRuntimeOptions(runtimeOptions, () => runFactExtraction(beam, memoryId, content));
 ...
 * `YEMU_MEMORY_NO_EMBEDDINGS` env, or an unresolved (empty) active model — so a
...
export function reconcileEmbeddingModel(beam: BeamMemoryState): void {
	...
	if (mismatch) {
		...
		logger.info("yemu-memory: embedding model changed, rebuilding", {
	...
	logger.info("yemu-memory: resuming interrupted embedding rebuild", { to: active, count: missing.length });
	rebuild(missing);
}
