...
import {
	isYemuAiModel,
	type ResolvedYeMuMemoryRuntimeOptions,
	resolveEmbeddingProvider,
	withYeMuMemoryRuntimeOptions,
	type YeMuMemoryEmbeddingRuntimeOptions,
	type YeMuMemoryLlmCompletion,
	type YeMuMemoryLlmRuntimeOptions,
} from "./runtime-options";

export interface YeMuMemoryOptions {
	...
	readonly embeddings?: false | YeMuMemoryEmbeddingRuntimeOptions;
	readonly llmEnabled?: boolean;
	readonly llmBaseUrl?: string;
	readonly llmApiKey?: ApiKey;
	readonly llmModel?: string | Model<Api>;
	readonly llm?: false | YeMuMemoryLlmRuntimeOptions | Model<Api> | YeMuMemoryLlmCompletion;
...
function hasOwn(options: YeMuMemoryOptions, key: keyof YeMuMemoryOptions): boolean {
	return Object.hasOwn(options, key);
}

function resolveRuntimeOptions(options: YeMuMemoryOptions): ResolvedYeMuMemoryRuntimeOptions | undefined {
	...
	let llm: ResolvedYeMuMemoryRuntimeOptions["llm"];
	if (options.llm === false) {
		llm = { enabled: false };
	} else if (typeof options.llm === "function") {
		llm = { enabled: true, complete: options.llm };
	} else if (isYemuAiModel(options.llm)) {
		llm = { enabled: true, model: options.llm };
	} else {
		const nestedLlm = options.llm !== undefined && !isYemuAiModel(options.llm) ? options.llm : undefined;
...
let defaultInstance: YeMuMemory | null = null;
...
function resolveDbPath(options: YeMuMemoryOptions, bank: string): string | undefined {
...
function defaultFor(bank: string | null | undefined = null): YeMuMemory {
	...
	if (defaultInstance === null || defaultInstance.bank !== targetBank) {
		...
		defaultInstance = new YeMuMemory({ bank: targetBank });
	}
	return defaultInstance;
}

export class YeMuMemory {
	readonly sessionId: string;
	...
	readonly db: Database;
	readonly runtimeOptions?: ResolvedYeMuMemoryRuntimeOptions;
	#ownsDb: boolean;
	#closed = false;

	constructor(options: YeMuMemoryOptions = {}) {
	...
	#withRuntimeOptions<T>(fn: () => T): T {
		return withYeMuMemoryRuntimeOptions(this.runtimeOptions, fn);
	}
}
...
export function getDefaultInstance(bank: string | null = null): YeMuMemory {
