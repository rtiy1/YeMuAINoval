...
function checkpointRoot(host: MemoryHost): string {
	...
	return path === undefined || path === ":memory:"
		? join(process.cwd(), ".yemu-memory-sync")
...
export class DeltaSync {
	...
	constructor(
		readonly yemuMemory: MemoryHost,
		checkpointDir?: string,
	) {
		this.db = databaseOf(yemuMemory);
		this.checkpointDir = checkpointDir ?? checkpointRoot(yemuMemory);
