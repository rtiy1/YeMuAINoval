...
import {
	...
	isYemuAiModel,
	type YeMuMemoryLlmCompleteOptions,
	type YeMuMemoryLlmCompletion,
} from "./runtime-options";

const ENV_MODEL_REPO = process.env.YEMU_MEMORY_LLM_REPO ?? "";
export interface RemoteLlmOptions {
	fetch?: FetchImpl;
}

const ENV_MODEL_FILE = process.env.YEMU_MEMORY_LLM_FILE ?? "";
...
function activeLlmOptions() {
	return getYeMuMemoryRuntimeOptions()?.llm;
}

function activeCustomCompletion(): YeMuMemoryLlmCompletion | undefined {
	return activeLlmOptions()?.complete;
}

function activeYemuAiModel(): Model<Api> | undefined {
	const model = activeLlmOptions()?.model;
	return isYemuAiModel(model) ? model : undefined;
}
...
function llmEnabled(): boolean {
	...
	if (activeCustomCompletion() !== undefined || activeYemuAiModel() !== undefined) {
		return true;
	}
	return envBool("YEMU_MEMORY_LLM_ENABLED", true);
}
...
function llmMaxTokens(): number {
	...
	return envInt("YEMU_MEMORY_LLM_MAX_TOKENS", 2048);
}

function llmContextTokens(): number {
	return envInt("YEMU_MEMORY_LLM_N_CTX", 2048);
}
...
function hostLlmEnabled(): boolean {
	...
	return envBool("YEMU_MEMORY_HOST_LLM_ENABLED", false);
}

function hostLlmContextTokens(): number {
	return envInt("YEMU_MEMORY_HOST_LLM_N_CTX", 32000);
}
...
function llmBaseUrl(): string {
	...
	return stripTrailingSlash(env("YEMU_MEMORY_LLM_BASE_URL"));
}
...
function llmModelName(): string {
	...
	return env("YEMU_MEMORY_LLM_MODEL") || "local";
}
...
function llmApiKey(): ApiKey {
	...
	return env("YEMU_MEMORY_LLM_API_KEY");
}

function sleepPrompt(): string {
	return env("YEMU_MEMORY_SLEEP_PROMPT").trim();
}
...
function formatSleepPrompt(memories: readonly string[], source = ""): string | null {
	const override = getYeMuMemoryRuntimeOptions()?.llm?.consolidationPrompt;
...
export function configuredLlmWillHandleCall(): boolean {
	return llmEnabled() && (activeCustomCompletion() !== undefined || activeYemuAiModel() !== undefined);
}
...
async function tryHostLlm(prompt: string, maxTokens: number, temperature: number): Promise<[boolean, string | null]> {
	...
	const raw = await callHostLlm(prompt, {
		...
		temperature,
		timeout: 15,
		provider: env("YEMU_MEMORY_HOST_LLM_PROVIDER").trim() || null,
		model: env("YEMU_MEMORY_HOST_LLM_MODEL").trim() || null,
	});
...
export async function callRemoteLlm(
	...
	try {
		...
		const response = await withAuth(llmApiKey(), async key => {
			...
			if (res.status === 401) {
				throw new ProviderHttpError("yemu-memory remote LLM request unauthorized (401)", 401, {
					headers: res.headers,
				});
			}
			return res;
...
async function summarizeChunk(
	...
	if (llmEnabled() && llmBaseUrl() !== "" && !envBool("YEMU_MEMORY_FORCE_LOCAL", false)) {
