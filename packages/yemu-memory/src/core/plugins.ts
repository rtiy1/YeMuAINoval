...
export const DEFAULT_PLUGIN_DIR = join(homedir(), ".hermes", "yemu-memory", "plugins");

export type PluginConfig = Record<string, unknown>;
export type MemoryDict = Record<string, unknown>;

export class YeMuMemoryPlugin {
	...
	constructor(config: PluginConfig = {}) {
		if (new.target === YeMuMemoryPlugin) throw new TypeError("YeMuMemoryPlugin is abstract");
		this.config = config;
		const ctor = this.constructor as typeof YeMuMemoryPlugin;
		this.name =
...
export class LoggingPlugin extends YeMuMemoryPlugin {
...
export class MetricsPlugin extends YeMuMemoryPlugin {
...
export class FilterPlugin extends YeMuMemoryPlugin {
...
export class CompressionPlugin extends YeMuMemoryPlugin {
...
export type PluginConstructor<T extends YeMuMemoryPlugin = YeMuMemoryPlugin> = new (config?: PluginConfig) => T;

export class PluginManager {
	private readonly registry = new Map<string, PluginConstructor>();
	private readonly instances = new Map<string, YeMuMemoryPlugin>();
	...
	registerPlugin(name: string, pluginClass: PluginConstructor): void {
		if (typeof pluginClass !== "function" || !(pluginClass.prototype instanceof YeMuMemoryPlugin)) {
			throw new TypeError("pluginClass must be a YeMuMemoryPlugin subclass");
		}
		if (this.registry.has(name)) throw new ValueError(`Plugin '${name}' is already registered`);
		this.registry.set(name, pluginClass);
	}
	loadPlugin(name: string, config: PluginConfig = {}): YeMuMemoryPlugin {
	...
	getPlugin(name: string): YeMuMemoryPlugin | null {
	...
	loadAll(configs: Record<string, PluginConfig> = {}): YeMuMemoryPlugin[] {
		const loaded: YeMuMemoryPlugin[] = [];
