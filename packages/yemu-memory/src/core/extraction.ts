...
import { getYeMuMemoryRuntimeOptions } from "./runtime-options";
...
export const EXTRACTION_PROMPT_TEMPLATE =
	env("YEMU_MEMORY_EXTRACTION_PROMPT") ||
	`You are an expert structured memory extractor for YeMuMemory v3.0+ MEMORIA tables.
...
export function buildExtractionPrompt(text: string, detectedLang = "en"): string {
	const template = getYeMuMemoryRuntimeOptions()?.llm?.extractionPrompt ?? EXTRACTION_PROMPT_TEMPLATE;
