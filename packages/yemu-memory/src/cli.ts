...
function resolveDbPath(context?: CliContext): string {
	return context?.dbPath ?? (context?.dataDir ? join(context.dataDir, "yemu-memory.db") : configuredDbPath());
}
...
async function withMemory<T>(context: CliContext | undefined, fn: (memory: BeamMemory) => T | Promise<T>): Promise<T> {
	...
	try {
		...
		// short-lived owners (e.g. `yemu-memory store …` / `yemu-memory sleep …`) don't
...
export const cmdExport: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: yemu-memory export <file.json>");
...
export const cmdImport: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: yemu-memory import <file.json>");
	...
	if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed))
		fail("Import file must contain a YeMuMemory export object", 1);
	return withMemory(context, memory => {
...
export const cmdRemember: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: yemu-memory store <content> [source] [importance]");
...
export const cmdRecall: CommandHandler = async (args, context) => {
	if (args.length === 0) usage("Usage: yemu-memory recall <query> [top_k]");
	const query = args[0] ?? "";
...
export const cmdUpdate: CommandHandler = (args, context) => {
	if (args.length < 2) usage("Usage: yemu-memory update <memory_id> <new_content> [importance]");
...
export const cmdDelete: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: yemu-memory delete <memory_id>");
	const memoryId = args[0] ?? "";
	return withMemory(context, memory => {
...
export const cmdScratchpad: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: yemu-memory scratchpad <read|write|clear> [content]");
	const subcmd = args[0];
	return withMemory(context, memory => {
		if (subcmd === "read") {
		...
		if (subcmd === "write") {
			if (args.length < 2) usage("Usage: yemu-memory scratchpad write <content>");
...
export const cmdBank: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: yemu-memory bank <list|create|delete> [name]");
	...
	try {
		...
		if (subcmd === "create") {
			if (args.length < 2) fail("Usage: yemu-memory bank create <name>");
			const name = args[1] ?? "";
		...
		if (subcmd === "delete") {
			if (args.length < 2) fail("Usage: yemu-memory bank delete <name>");
			const name = args[1] ?? "";
...
export function printHelp(context?: CliContext): void {
	out(context, "YeMuMemory - Local AI Memory System\n");
	out(context, "Usage: yemu-memory <command> [args]\n");
...
export async function runCli(args: readonly string[] = Bun.argv.slice(2), context?: CliContext): Promise<number> {
	...
	if (!handler) {
		...
		err(context, "Run 'yemu-memory --help' for usage.");
		return 2;
	}
	try {
