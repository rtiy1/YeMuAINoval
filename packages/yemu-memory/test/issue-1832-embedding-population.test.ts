/**
 * Regression for upstream-change-1832
 *
 * Before the fix:
...
import "./setup";
import { cmdRemember } from "@yemu/memory/cli";
import { BeamMemory } from "@yemu/memory/core/beam";
import { YeMuMemory } from "@yemu/memory/core/memory";
import { type ResolvedYeMuMemoryRuntimeOptions, withYeMuMemoryRuntimeOptions } from "@yemu/memory/core/runtime-options";
...
function withFakeMemory<T>(fn: (memory: YeMuMemory, calls: () => number) => Promise<T>): Promise<T> {
	const { provider, calls } = fakeProvider();
	const memory = new YeMuMemory({
...
/**
 * Re-enter the per-YeMuMemory runtime-options scope when reaching into `memory.beam`
 * directly (only `YeMuMemory.remember`/`recall`/etc. enter it automatically).
 */
function inScope<T>(memory: YeMuMemory, fn: () => T): T {
	return withYeMuMemoryRuntimeOptions(memory.runtimeOptions, fn);
}

function readEmbeddings(memory: YeMuMemory): EmbeddingRow[] {
	return memory.conn
...
describe("issue #1832 — embedding write/read coverage", () => {
	...
	it("flushes pending embeddings before close so short-lived `yemu-memory store` owners persist them", async () => {
		...
		const { provider, calls } = fakeProvider();
		const dbPath = `${tmpdir()}/yemu-memory-1833-${randomBytes(6).toString("hex")}.db`;
		const captured: string[] = [];
		const stdout = { write: (s: string) => captured.push(s) };
		const stderr = { write: (s: string) => captured.push(s) };
		const runtimeOptions: ResolvedYeMuMemoryRuntimeOptions = {
			embeddings: { provider },
		};

		await withYeMuMemoryRuntimeOptions(runtimeOptions, async () => {
	...
	it("scopes embedQuery() cache per provider so two YeMuMemory runtimes do not cross-contaminate", async () => {
		// Repro for #1833 review comment: `embedQuery()` caches by query text only, so a
		// second `YeMuMemory` in the same process with a different provider would read back
		// the first runtime's vector and score against it. Each YeMuMemory here exposes a
		...
		const alpha = new YeMuMemory({
			db: new Database(":memory:"),
			embeddings: { provider: alphaProvider },
		});
		const beta = new YeMuMemory({
			db: new Database(":memory:"),
