...
import { extractionRate, normalizeBatch, normalizeChat } from "@yemu/memory/core/chat-normalize";
import { getCostStats, initCostLog, logCost } from "@yemu/memory/core/cost-log";
import { estimateCost, estimateTokens } from "@yemu/memory/core/token-counter";
...
describe("cost log", () => {
	it("initializes the sqlite table and aggregates all and per-session stats", () => {
		const dbPath = join(mkdtempSync(join(tmpdir(), "yemu-memory-cost-")), "cost_log.db");
