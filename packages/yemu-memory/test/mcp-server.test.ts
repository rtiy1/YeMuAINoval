...
import { callToolJson, handleJsonRpc, runStdio } from "@yemu/memory/mcp-server";
import { getToolDefinitions, handleToolCall, TOOLS } from "@yemu/memory/mcp-tools";

let dataDir: string;

beforeEach(() => {
	dataDir = mkdtempSync(join(tmpdir(), "yemu-memory-mcp-server-"));
...
describe("MCP JSON handlers", () => {
	...
	it("wraps tool results in MCP text content", async () => {
		const response = await callToolJson("yemu-memory_stats", { bank: "server" });
	...
	it("dispatches remember, recall, stats, sleep, scratchpad, and bank operations", async () => {
		...
		const scratchWrite = await handleToolCall("yemu-memory_scratchpad_write", {
			content: "scratch note",
			bank: "work",
		});
		expect(scratchWrite.status).toBe("written");
		expect(scratchWrite.bank).toBe("work");
		const scratchRead = await handleToolCall("yemu-memory_scratchpad_read", { bank: "work" });
		expect(scratchRead.entries_count as number).toBeGreaterThanOrEqual(1);

		const stats = await handleToolCall("yemu-memory_stats", { bank: "work" });
		expect(stats.status).toBe("ok");
		expect(stats.bank).toBe("work");
		expect(stats.working).toBeDefined();

		const sleep = await handleToolCall("yemu-memory_sleep", { dry_run: true, bank: "work" });
		expect(sleep.status).toBe("ok");
		expect(sleep.dry_run).toBe(true);
		expect(sleep.bank).toBe("work");
	});

	it("uses YEMU_MEMORY_MCP_BANK when a call omits bank", async () => {
		process.env.YEMU_MEMORY_MCP_BANK = "env-bank";
		const remembered = await handleToolCall("yemu-memory_remember", { content: "env bank memory" });
		expect(remembered.bank).toBe("env-bank");
		const stats = await handleToolCall("yemu-memory_stats", {});
		expect(stats.bank).toBe("env-bank");
	});

	it("routes bank paths through BankManager validation and canonical layout", async () => {
		const defaultStats = await handleToolCall("yemu-memory_diagnose", {});
		expect(defaultStats.db_path).toBe(join(dataDir, "yemu-memory.db"));

		const workStats = await handleToolCall("yemu-memory_diagnose", { bank: "work" });
		expect(workStats.db_path).toBe(join(dataDir, "banks", "work", "yemu-memory.db"));
		await expect(handleToolCall("yemu-memory_diagnose", { bank: "../escape" })).rejects.toThrow();
	});

	it("links graph edges and queries related memories through a real BeamMemory", async () => {
		const first = await handleToolCall("yemu-memory_remember", {
			content: "Graph source memory about Ada and deterministic tests",
			bank: "graph",
		});
		const second = await handleToolCall("yemu-memory_remember", {
		...
		const link = await handleToolCall("yemu-memory_graph_link", {
			source_id: sourceId,
			target_id: targetId,
		...
		const query = await handleToolCall("yemu-memory_graph_query", {
