...
import { cmdRemember, cmdStats, memoryStats, runCli } from "@yemu/memory/cli";
import { BeamMemory } from "@yemu/memory/core/beam";
import { runDiagnostics } from "@yemu/memory/diagnose";

let root: string;

beforeEach(() => {
	root = mkdtempSync(join(tmpdir(), "yemu-memory-ts-cli-stats-parity-"));
...
describe("CLI stats parity", () => {
	it("prints real working, episodic, triple, bank, and database counts", async () => {
		const dbPath = join(root, "yemu-memory.db");
	...
	it("stats commands read the configured data directory, not the default home path", async () => {
		...
		expect(existsSync(join(customDataDir, "yemu-memory.db"))).toBe(true);
		...
		expect(statsIo.stdout).toContain(join(customDataDir, "yemu-memory.db"));
	});
});

describe("yemu-memory-stats diagnostic behavior parity", () => {
	...
	it("diagnostics initialize missing databases gracefully and report zero counts", () => {
		const dbPath = join(root, "empty", "yemu-memory.db");
