...
import {
	...
	update,
	YeMuMemory,
} from "@yemu/memory/core/memory";
import { openDatabase } from "@yemu/memory/db";

const roots: string[] = [];
let previousDataDir: string | undefined;

function tempRoot(): string {
	const root = mkdtempSync(join(tmpdir(), "yemu-memory-memory-facade-"));
	roots.push(root);
	return root;
}
...
function useTempDataDir(): string {
	const root = tempRoot();
	previousDataDir = process.env.YEMU_MEMORY_DATA_DIR;
	process.env.YEMU_MEMORY_DATA_DIR = root;
	return root;
}
...
afterEach(() => {
	...
	if (previousDataDir === undefined) {
		...
		process.env.YEMU_MEMORY_DATA_DIR = previousDataDir;
	}
...
describe("YeMuMemory facade", () => {
	...
	it("accepts an already-open Database handle for memory, annotations, and episodic graph writes", () => {
		const previousProactiveLinking = process.env.YEMU_MEMORY_PROACTIVE_LINKING;
		process.env.YEMU_MEMORY_PROACTIVE_LINKING = "1";
		const db = openDatabase(":memory:");
		const memory = new YeMuMemory({ db, sessionId: "external-db" });
		try {
			...
			memory.close();
			db.close();
			if (previousProactiveLinking === undefined) delete process.env.YEMU_MEMORY_PROACTIVE_LINKING;
			else process.env.YEMU_MEMORY_PROACTIVE_LINKING = previousProactiveLinking;
		}
	});

	it("stores duplicate-content batch items with distinct ids", () => {
		const memory = new YeMuMemory({ dbPath: join(tempRoot(), "yemu-memory.db"), sessionId: "batch" });
		try {
	...
	it("preserves legacy and Python-compatible aliases", async () => {
		const memory = new YeMuMemory({
			dbPath: join(tempRoot(), "yemu-memory.db"),
			session_id: "aliases",
		});
		try {
