...
import { cmdExport, cmdImport, cmdRemember, runCli } from "@yemu/memory/cli";

let root: string;

beforeEach(() => {
	root = mkdtempSync(join(tmpdir(), "yemu-memory-ts-cli-errors-parity-"));
...
describe("CLI usage and operation failure parity", () => {
	it("reports missing required arguments as usage errors without tracebacks", async () => {
		for (const [args, expected] of [
			[["store"], "Usage: yemu-memory store <content> [source] [importance]"],
			[["recall"], "Usage: yemu-memory recall <query> [top_k]"],
			[["update", "missing-id"], "Usage: yemu-memory update <memory_id> <new_content> [importance]"],
			[["delete"], "Usage: yemu-memory delete <memory_id>"],
			[["import"], "Usage: yemu-memory import <file.json>"],
			[["export"], "Usage: yemu-memory export <file.json>"],
			[["bank"], "Usage: yemu-memory bank <list|create|delete> [name]"],
		] as const) {
			const io = capture();
	...
	it("reports parse errors and unknown commands without tracebacks", async () => {
		...
		expect(unknown.stderr).toContain("Run 'yemu-memory --help' for usage.");
	...
	it("reports import file and JSON validation errors without tracebacks", async () => {
		...
		for (const payload of ["[]", '"not an export"']) {
			...
			expect(io.stdout).toBe("");
			expect(io.stderr).toContain("Import file must contain a YeMuMemory export object");
			expect(io.stderr).not.toContain("Traceback");
		}
	...
	it("bank validation errors are user-facing", async () => {
		for (const [args, expected, code] of [
			...
			[["bank", "create"], "Usage: yemu-memory bank create <name>", 2],
			[["bank", "delete"], "Usage: yemu-memory bank delete <name>", 2],
