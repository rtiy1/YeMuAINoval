...
import { extractFacts } from "@yemu/memory/core/extraction";
import { type ChatMessage, ExtractionClient } from "@yemu/memory/core/extraction/client";
import { getExtractionStats, resetExtractionStats } from "@yemu/memory/core/extraction/diagnostics";
import { resetHostLlmBackendForTests } from "@yemu/memory/core/llm-backends";
...
describe("extraction integration", () => {
	it("uses a fake OpenAI-compatible remote endpoint for extractFacts", async () => {
		...
		process.env.YEMU_MEMORY_LLM_BASE_URL = "http://fake-remote/v1";
		let payloadJson = "";
