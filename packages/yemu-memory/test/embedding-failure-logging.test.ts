...
import "./setup";
import { embed, resetEmbeddingProviderForTests, setLocalModelInitializerForTests } from "@yemu/memory/core/embeddings";
...
async function withLocalModelEnv<T>(fn: () => Promise<T>): Promise<T> {
	...
	process.env.YEMU_MEMORY_EMBEDDING_MODEL = "BAAI/bge-small-en-v1.5";
		...
		try {
			await withLocalModelEnv(async () => {
				...
				expect(debugSpy).toHaveBeenCalledWith(
					"yemu-memory: local embedding model failed to load",
	...
	it("escalates the same failure to warn when runtime debug is enabled", async () => {
		...
		try {
			await withLocalModelEnv(async () => {
				...
				expect(await withYeMuMemoryRuntimeOptions({ debug: true }, () => embed(["hello"]))).toBeNull();
