...
import { PolyphonicRecallEngine } from "@yemu/memory/core/polyphonic-recall";

const roots: string[] = [];
const toggleNames = [
	"YEMU_MEMORY_VOICE_VECTOR",
	"YEMU_MEMORY_VOICE_GRAPH",
	"YEMU_MEMORY_VOICE_FACT",
	"YEMU_MEMORY_VOICE_TEMPORAL",
] as const;
const savedEnv: Partial<Record<(typeof toggleNames)[number], string>> = {};

function tempDb(): string {
	const root = mkdtempSync(join(tmpdir(), "yemu-memory-ab-toggle-"));
	roots.push(root);
	return join(root, "yemu-memory.db");
}
...
describe("A/B polyphonic voice toggles", () => {
	it("treats falsy values as disabled with whitespace and case normalization", () => {
		...
		for (const value of falsyValues) {
			withEngine(engine => {
				process.env.YEMU_MEMORY_VOICE_VECTOR = value;
				process.env.YEMU_MEMORY_VOICE_GRAPH = value;
				process.env.YEMU_MEMORY_VOICE_FACT = value;
				process.env.YEMU_MEMORY_VOICE_TEMPORAL = value;
