...
import { normalizedRecallWeights } from "@yemu/memory/config";
import { BeamMemory } from "@yemu/memory/core/beam";

const beams: BeamMemory[] = [];
const ORIGINAL_ENV = {
	YEMU_MEMORY_VEC_WEIGHT: process.env.YEMU_MEMORY_VEC_WEIGHT,
	YEMU_MEMORY_FTS_WEIGHT: process.env.YEMU_MEMORY_FTS_WEIGHT,
	YEMU_MEMORY_IMPORTANCE_WEIGHT: process.env.YEMU_MEMORY_IMPORTANCE_WEIGHT,
};
...
describe("configurable recall scoring", () => {
	it("normalizes defaults, explicit weights, zeros, and negative inputs", () => {
		delete process.env.YEMU_MEMORY_VEC_WEIGHT;
		delete process.env.YEMU_MEMORY_FTS_WEIGHT;
		delete process.env.YEMU_MEMORY_IMPORTANCE_WEIGHT;
	...
	it("reads environment weights when no explicit config is supplied", () => {
		process.env.YEMU_MEMORY_VEC_WEIGHT = "0.7";
		process.env.YEMU_MEMORY_FTS_WEIGHT = "0.2";
		process.env.YEMU_MEMORY_IMPORTANCE_WEIGHT = "0.1";
	...
	it("lets environment weights affect BeamMemory defaults", async () => {
		process.env.YEMU_MEMORY_VEC_WEIGHT = "0.1";
		process.env.YEMU_MEMORY_FTS_WEIGHT = "0.1";
		process.env.YEMU_MEMORY_IMPORTANCE_WEIGHT = "0.8";
		const beam = makeBeam();
