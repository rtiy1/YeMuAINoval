...
import { CallableLlmBackend, resetHostLlmBackendForTests, setHostLlmBackend } from "@yemu/memory/core/llm-backends";
import {
	buildHostPrompt,
	callLocalLlm,
	callRemoteLlm,
	...
	localGgufAvailable,
	summarizeMemories,
} from "@yemu/memory/core/local-llm";
import { YeMuMemory } from "@yemu/memory/core/memory";
import { withYeMuMemoryRuntimeOptions } from "@yemu/memory/core/runtime-options";
...
describe("local LLM TypeScript port", () => {
	it("reports remote availability and calls OpenAI-compatible HTTP", async () => {
		process.env.YEMU_MEMORY_LLM_BASE_URL = "http://local-llm/v1";
		process.env.YEMU_MEMORY_LLM_API_KEY = "sk-test";
		process.env.YEMU_MEMORY_LLM_MODEL = "test-model";
		let auth = "";
		let model = "";
	...
	it("uses host backend before remote and skips remote on host miss", async () => {
		process.env.YEMU_MEMORY_LLM_ENABLED = "true";
		process.env.YEMU_MEMORY_HOST_LLM_ENABLED = "true";
		process.env.YEMU_MEMORY_LLM_BASE_URL = "http://remote/v1";
		let calls = 0;
	...
	it("renders host sleep prompt override without chat-template tokens", () => {
		process.env.YEMU_MEMORY_SLEEP_PROMPT = "Write in German. Source={source}. Memories:\n{memories}";
	...
	it("expands chunk budget when host backend will handle calls", () => {
		process.env.YEMU_MEMORY_LLM_ENABLED = "true";
		process.env.YEMU_MEMORY_HOST_LLM_ENABLED = "true";
		process.env.YEMU_MEMORY_HOST_LLM_N_CTX = "32000";
		process.env.YEMU_MEMORY_LLM_N_CTX = "2048";
	...
	it("uses a constructor-scoped completion function instead of remote URL settings", async () => {
		process.env.YEMU_MEMORY_LLM_ENABLED = "true";
		process.env.YEMU_MEMORY_LLM_BASE_URL = "http://remote.example/v1";
		let fetchCalls = 0;
		const fetchMock: FetchImpl = async () => {
			fetchCalls += 1;
		...
		try {
			const text = await withYeMuMemoryRuntimeOptions(memory.runtimeOptions, () =>
	...
	it("uses a constructor-scoped model-runtime Model instance", async () => {
		const model = createMockModel({
			handler: () => ({ content: ["model summary"] }),
		});
		const memory = new YeMuMemory({ llm: model });
		try {
			const text = await withYeMuMemoryRuntimeOptions(memory.runtimeOptions, () => complete("hello"));
	...
	it("lets llm:false override remote environment defaults", async () => {
		...
		const memory = new YeMuMemory({ llm: false });
		try {
			const text = await withYeMuMemoryRuntimeOptions(memory.runtimeOptions, () =>
				complete("hello", 0.3, { fetch: fetchMock }),
			);
