...
describe("provider all-tools parity", () => {
	...
	it("advertises required arguments for provider write/update/import tools", () => {
		expect(schemaFor("yemu-memory_remember").required).toContain("content");
		expect(schemaFor("yemu-memory_recall").required).toContain("query");
		expect(schemaFor("yemu-memory_scratchpad_write").required).toContain("content");
		expect(schemaFor("yemu-memory_update").required).toEqual(["memory_id", "content"]);
		expect(schemaFor("yemu-memory_forget").required).toContain("memory_id");
		expect(schemaFor("yemu-memory_export").required).toContain("output_path");
		expect(schemaFor("yemu-memory_import").required).toContain("input_path");
	});

	it("returns user-facing argument errors instead of mutating on missing arguments", async () => {
		for (const [name, args, expected] of [
			["yemu-memory_remember", {}, "content is required"],
			["yemu-memory_recall", {}, "query is required"],
			["yemu-memory_scratchpad_write", { content: "" }, "content is required"],
			["yemu-memory_update", { memory_id: "missing-id" }, "content or importance is required"],
			["yemu-memory_forget", {}, "memory_id is required"],
			["yemu-memory_export", {}, "output_path is required"],
			["yemu-memory_import", {}, "Either input_path (for file import) is required"],
		] as const) {
	...
	it("exports provider data to a file and imports it into a fresh isolated bank", async () => {
		...
		await handleToolCall("yemu-memory_scratchpad_write", {
			content: "portable provider scratch",
			bank: "source",
		});

		const exportPath = join(dataDir, "provider-export.json");
		const exported = await handleToolCall("yemu-memory_export", {
		...
		const imported = await handleToolCall("yemu-memory_import", { input_path: exportPath, bank: "dest" });
	...
	it("diagnose, validate, graph, and shared handlers return structured provider results", async () => {
		...
		const validate = await handleToolCall("yemu-memory_validate", {
			memory_id: memoryId,
			action: "attest",
			validator: "test",
			bank: "ops",
		...
		const diagnose = await handleToolCall("yemu-memory_diagnose", { bank: "ops" });
		expect(diagnose.status).toBe("ok");
		expect(diagnose.db_path).toContain("banks/ops/yemu-memory.db");
		const graphQuery = await handleToolCall("yemu-memory_graph_query", { seed_memory_id: memoryId, bank: "ops" });
		...
		expect(
			await handleToolCall("yemu-memory_graph_link", {
				source_id: memoryId,
				target_id: "other",
		...
		expect((await handleToolCall("yemu-memory_shared_forget", { memory_id: shared.memory_id })).status).toBe(
			"deleted",
		);
	});
});
