...
} from "@yemu/memory/config";
import { polyphonicRecallIsEnabled } from "@yemu/memory/core/polyphonic-recall";
import { isEnhancedRecallEnabled, isQueryCacheEnabled } from "@yemu/memory/core/query-cache";
...
describe("configureRecallFeatures", () => {
	...
	it("lets the env vars override the configured value in both directions", () => {
		configureRecallFeatures({ polyphonicRecall: true, enhancedRecall: true, proactiveLinking: true });
		expect(polyphonicRecallEnabled({ YEMU_MEMORY_POLYPHONIC_RECALL: "0" })).toBe(false);
		expect(enhancedRecallEnabled({ YEMU_MEMORY_ENHANCED_RECALL: "0" })).toBe(false);
		expect(proactiveLinkingEnabled({ YEMU_MEMORY_PROACTIVE_LINKING: "0" })).toBe(false);
		expect(isQueryCacheEnabled(true, { YEMU_MEMORY_ENHANCED_RECALL: "0" })).toBe(false);

		configureRecallFeatures({ polyphonicRecall: false, enhancedRecall: false, proactiveLinking: false });
		expect(polyphonicRecallEnabled({ YEMU_MEMORY_POLYPHONIC_RECALL: "1" })).toBe(true);
		expect(enhancedRecallEnabled({ YEMU_MEMORY_ENHANCED_RECALL: "1" })).toBe(true);
		expect(proactiveLinkingEnabled({ YEMU_MEMORY_PROACTIVE_LINKING: "1" })).toBe(true);
		expect(isQueryCacheEnabled(true, { YEMU_MEMORY_ENHANCED_RECALL: "1" })).toBe(true);
	});
