...
import { DeltaSync, EventType, MemoryEvent, MemoryStream, SyncCheckpoint } from "@yemu/memory/core/streaming";
...
describe("DeltaSync", () => {
	it("computes, applies, and persists checkpoints for allowed tables", () => {
		const root = mkdtempSync(join(tmpdir(), "yemu-memory-stream-"));
