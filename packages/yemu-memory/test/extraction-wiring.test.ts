...
import type { YeMuMemoryLlmCompletion } from "@yemu/memory/core/runtime-options";

const instances: YeMuMemory[] = [];
...
function makeMemory(llm: false | { complete: YeMuMemoryLlmCompletion }): YeMuMemory {
...
describe("remember(extract) wires the LLM fact extractor", () => {
		...
		const memory = makeMemory({
			complete: () => {
				...
				return JSON.stringify({
					...
					kg: [{ subject: "YeMuMemory", predicate: "uses", object: "SQLite" }],
				});
			},
		});
	...
	it("uses a runtime-configured remote LLM in background extraction", async () => {
		const previousEnabled = process.env.YEMU_MEMORY_LLM_ENABLED;
		const previousBaseUrl = process.env.YEMU_MEMORY_LLM_BASE_URL;
		let requestedUrl = "";
		let requestedBody = "";
		...
		try {
			...
			delete process.env.YEMU_MEMORY_LLM_BASE_URL;
			const memory = new YeMuMemory({
			...
			if (previousEnabled === undefined) delete process.env.YEMU_MEMORY_LLM_ENABLED;
			else process.env.YEMU_MEMORY_LLM_ENABLED = previousEnabled;
			if (previousBaseUrl === undefined) delete process.env.YEMU_MEMORY_LLM_BASE_URL;
			else process.env.YEMU_MEMORY_LLM_BASE_URL = previousBaseUrl;
		}
	});
