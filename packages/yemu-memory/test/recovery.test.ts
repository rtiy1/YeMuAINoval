...
import { createBackup, emergencyRestore, restoreBackup, verifyIntegrity } from "@yemu/memory/dr/recovery";

const tempDirs: string[] = [];
const SQLITE_HEADER = Buffer.from("SQLite format 3\0", "binary");

function makeTempDir(): string {
	const dir = mkdtempSync(join(tmpdir(), "yemu-memory-recovery-"));
	tempDirs.push(dir);
	return dir;
}
...
describe("SQLite recovery helpers", () => {
	it("creates a compressed backup with metadata", () => {
		...
		const dbPath = join(dir, "yemu-memory.db");
	...
	it("keeps the current WAL database untouched when a staged restore fails integrity", () => {
		...
		const badBackup = join(backupDir, "yemu-memory_backup_20260530_120000.db.gz");
	...
	it("leaves the original database intact when emergency restore exhausts corrupt backups", () => {
		...
		createSqliteDb(dbPath);
		mkdirSync(backupDir, { recursive: true });
		writeCorruptSqliteBackup(join(backupDir, "yemu-memory_backup_20260530_120000.db.gz"));
