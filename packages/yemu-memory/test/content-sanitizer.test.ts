...
import {
	...
	shannonEntropy,
	storeBlob,
} from "@yemu/memory/core/content-sanitizer";

const ORIGINAL_BLOB_DIR = process.env.YEMU_MEMORY_BLOB_DIR;

afterEach(() => {
	if (ORIGINAL_BLOB_DIR === undefined) {
		delete process.env.YEMU_MEMORY_BLOB_DIR;
	} else {
		process.env.YEMU_MEMORY_BLOB_DIR = ORIGINAL_BLOB_DIR;
	}
});

function useTempBlobDir(): string {
	const dir = mkdtempSync(join(tmpdir(), "yemu-memory-blobs-"));
	process.env.YEMU_MEMORY_BLOB_DIR = join(dir, "blobs");
	return process.env.YEMU_MEMORY_BLOB_DIR;
}
