...
import { getFastembedCacheDir } from "@yemu/utils";
import "./setup";
import {
	available,
	embed,
...
} from "@yemu/memory/core/embeddings";
...
const ENV_KEYS = [
	"NODE_ENV",
	"BUN_ENV",
	"YEMU_MEMORY_NO_EMBEDDINGS",
	"YEMU_MEMORY_EMBEDDING_MODEL",
...
describe("optional embeddings", () => {
	it("falls back cleanly when embeddings are disabled", async () => {
		await withEnv({ YEMU_MEMORY_NO_EMBEDDINGS: "1" }, async () => {
	...
	it("uses a fake provider and caches single-query embeddings", async () => {
		await withEnv({ YEMU_MEMORY_NO_EMBEDDINGS: undefined }, async () => {
			let calls = 0;
			setEmbeddingProviderForTests({
	...
	it("calls an OpenAI-compatible custom embeddings endpoint without requiring an API key", async () => {
		...
		const server = Bun.serve({
			...
			fetch: async request => {
				...
				expect(request.headers.get("user-agent")).toBe(`Oh-My-YeMu/${packageJson.version}`);
				expect(request.headers.get("http-referer")).toBe("https://github.com/rtiy1/YeMuAINoval/");
				expect(request.headers.get("x-openrouter-title")).toBe("Oh-My-YeMu");
		...
		try {
			await withEnv(
				{
					YEMU_MEMORY_NO_EMBEDDINGS: undefined,
					YEMU_MEMORY_EMBEDDING_MODEL: "openai/text-embedding-3-small",
					YEMU_MEMORY_EMBEDDING_API_URL: server.url.toString().replace(/\/+$/, ""),
					YEMU_MEMORY_EMBEDDING_API_KEY: undefined,
	...
	it("lets constructor-scoped noEmbeddings override enabled providers", async () => {
		...
		const memory = new YeMuMemory({ noEmbeddings: true });
		try {
			const result = await withYeMuMemoryRuntimeOptions(memory.runtimeOptions, () => embed(["hello"]));
	...
	it("uses a constructor-scoped embedding provider", async () => {
		...
		try {
			const result = await withYeMuMemoryRuntimeOptions(memory.runtimeOptions, () => embedQuery("cache me"));
		...
		await withEnv(
			{
				...
				BUN_ENV: undefined,
				YEMU_MEMORY_NO_EMBEDDINGS: undefined,
				YEMU_MEMORY_EMBEDDING_MODEL: "BAAI/bge-small-en-v1.5",
				YEMU_MEMORY_EMBEDDING_API_URL: undefined,
