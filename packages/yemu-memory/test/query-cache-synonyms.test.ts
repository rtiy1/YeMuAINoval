...
import { isEnhancedRecallEnabled, isQueryCacheEnabled, QueryCache } from "@yemu/memory/core/query-cache";
import { expandQuery, getSynonyms, normalizeQuery } from "@yemu/memory/core/synonyms";
...
describe("QueryCache", () => {
	...
	it("persists to sqlite when a db path is supplied", () => {
		const dir = mkdtempSync(join(tmpdir(), "yemu-memory-query-cache-"));
		try {
	...
	it("keeps enhanced recall and query cache disabled unless the Python env gate is set", () => {
		...
		expect(isQueryCacheEnabled(false, { YEMU_MEMORY_ENHANCED_RECALL: "1" })).toBe(false);
