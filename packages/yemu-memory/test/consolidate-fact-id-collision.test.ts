...
import { computeFactId, VeracityConsolidator } from "@yemu/memory/core/veracity-consolidation";
import { closeQuietly } from "@yemu/memory/db";
...
function withDb<T>(fn: (path: string, cons: VeracityConsolidator) => T): T {
	const dir = mkdtempSync(join(tmpdir(), "yemu-memory-veracity-"));
