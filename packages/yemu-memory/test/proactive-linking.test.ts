...
import "./setup";
import { configureRecallFeatures } from "@yemu/memory/config";
import { BeamMemory } from "@yemu/memory/core/beam";
import type { EpisodicGraph, RelatedMemory } from "@yemu/memory/core/episodic-graph";
import { YeMuMemory } from "@yemu/memory/core/memory";

const previousProactive = process.env.YEMU_MEMORY_PROACTIVE_LINKING;

afterEach(() => {
	if (previousProactive === undefined) delete process.env.YEMU_MEMORY_PROACTIVE_LINKING;
	else process.env.YEMU_MEMORY_PROACTIVE_LINKING = previousProactive;
...
describe("proactive memory linking", () => {
	...
	it("honors host configuration when the environment variable is unset", () => {
		delete process.env.YEMU_MEMORY_PROACTIVE_LINKING;
	...
	it("keeps host configuration scoped to each YeMuMemory instance", () => {
		delete process.env.YEMU_MEMORY_PROACTIVE_LINKING;
		const enabled = new YeMuMemory({
			sessionId: "proactive-yemu-memory-on",
			dbPath: ":memory:",
			proactiveLinking: true,
		});
		configureRecallFeatures({ proactiveLinking: false });
		const disabled = new YeMuMemory({
			sessionId: "proactive-yemu-memory-off",
			dbPath: ":memory:",
			proactiveLinking: false,
		});
		try {
	...
	it("lets the environment variable override instance configuration", () => {
		process.env.YEMU_MEMORY_PROACTIVE_LINKING = "0";
