...
function tempDb(): string {
	const root = mkdtempSync(join(tmpdir(), "yemu-memory-identity-parity-"));
	roots.push(root);
...
describe("identity memory parity", () => {
	...
	it("stores author and channel identity on remember and defaults channel to session", () => {
		const dbPath = tempDb();
		const identified = new YeMuMemory({
			dbPath,
			sessionId: "session-a",
			authorId: "abdias",
			authorType: "human",
		...
		const anonymous = new YeMuMemory({ dbPath, sessionId: "session-b" });
		try {
	...
	it("reports working stats through identity filters", () => {
		const dbPath = tempDb();
		const a = new YeMuMemory({ dbPath, sessionId: "a1", authorId: "abdias", channelId: "team" });
		const b = new YeMuMemory({ dbPath, sessionId: "b1", authorId: "sarah", channelId: "team" });
		try {
