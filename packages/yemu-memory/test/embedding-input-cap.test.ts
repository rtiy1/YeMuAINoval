...
import "./setup";
import { embed, resetEmbeddingProviderForTests, setEmbeddingProviderForTests } from "@yemu/memory/core/embeddings";
import { withYeMuMemoryRuntimeOptions } from "@yemu/memory/core/runtime-options";

/**
 * Regression coverage for issue #3126: `YeMuMemorySessionState.retainMessages`
 ...
 * input to `YEMU_MEMORY_EMBEDDING_MAX_INPUT_CHARS` (default 8192) before the
 * provider sees it.
 */
...
const ENV_KEY = "YEMU_MEMORY_EMBEDDING_MAX_INPUT_CHARS";
...
describe("embed() input cap (#3126)", () => {
	...
	it("honors YEMU_MEMORY_EMBEDDING_MAX_INPUT_CHARS env override", async () => {
		const provider = captureProvider();
		setEmbeddingProviderForTests(provider);
	...
	it("respects a constructor-scoped maxInputChars override", async () => {
		...
		await withEnvValue(undefined, () =>
			withYeMuMemoryRuntimeOptions({ embeddings: { maxInputChars: 256 } }, () => embed(["w".repeat(10_000)])),
		);
	...
	it("preserves both ends of a chronological transcript via the head/tail clip", async () => {
		...
		// `YeMuMemorySessionState.retainMessages` hands `embed()` the full
