...
import {
	...
	quantizeInt8,
} from "@yemu/memory/core/binary-vectors";
...
describe("binary vector helpers", () => {
	...
	it("normalizes YEMU_MEMORY_VEC_TYPE with Python-compatible fallback", () => {
		expect(getVecType({ YEMU_MEMORY_VEC_TYPE: "bit" })).toBe("bit");
		expect(getVecType({ YEMU_MEMORY_VEC_TYPE: "int8" })).toBe("int8");
		expect(getVecType({ YEMU_MEMORY_VEC_TYPE: "float32" })).toBe("float32");
		expect(getVecType({ YEMU_MEMORY_VEC_TYPE: "bogus" })).toBe("float32");
