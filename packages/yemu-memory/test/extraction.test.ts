...
import {
	...
	parseFacts,
} from "@yemu/memory/core/extraction";
import { getExtractionStats, resetExtractionStats } from "@yemu/memory/core/extraction/diagnostics";
...
describe("structured extraction", () => {
	...
	it("uses deterministic heuristic extraction when no LLM is configured", async () => {
		process.env.YEMU_MEMORY_LLM_ENABLED = "false";
	...
	it("routes enabled host LLM extraction before remote and keeps temperature zero", async () => {
		...
		process.env.YEMU_MEMORY_LLM_BASE_URL = "http://remote.invalid/v1";
	...
	it("prefers a configured completion with the extraction-prompt override at temperature zero", async () => {
		...
		let capturedPrompt = "";
		let capturedTemperature = -1;
		const resolved: ResolvedYeMuMemoryRuntimeOptions = {
			llm: {
				enabled: true,
		...
		const facts = await withYeMuMemoryRuntimeOptions(resolved, () =>
