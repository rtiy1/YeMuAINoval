...
import { cmdRecall, cmdRemember, cmdStats, runCli } from "@yemu/memory/cli";
import { BeamMemory } from "@yemu/memory/core/beam";

function tempRoot(): string {
	return mkdtempSync(join(tmpdir(), "yemu-memory-ts-cli-"));
}
...
describe("CLI command handlers", () => {
	...
	it("reports usage, parse, operation, and unknown-command errors without tracebacks", async () => {
		...
		try {
			...
			expect(usageIo.stderr).toContain("Usage: yemu-memory store <content> [source] [importance]");
			...
			expect(unknownIo.stderr).toContain("Run 'yemu-memory --help' for usage.");
