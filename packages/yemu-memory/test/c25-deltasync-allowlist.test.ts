...
import { ALLOWED_DELTA_TABLES, DeltaSync, SyncCheckpoint } from "@yemu/memory/core/streaming";

const roots: string[] = [];

function tempRoot(): string {
	const root = mkdtempSync(join(tmpdir(), "yemu-memory-c25-delta-"));
	roots.push(root);
	return root;
}

function seededMemory(): { memory: YeMuMemory; root: string } {
	const root = tempRoot();
	const memory = new YeMuMemory({ sessionId: "s1", dbPath: join(root, "yemu-memory.db") });
