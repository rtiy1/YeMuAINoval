...
import { agentLoop } from "@yemu/agent-core/agent-loop";
import {
	...
	YemuGenAIAttr,
} from "@yemu/agent-core/telemetry";
import type { AgentContext, AgentEvent, AgentLoopConfig, AgentMessage, AgentTool } from "@yemu/agent-core/types";
...
import type { EventStream } from "@yemu/model-runtime/utils/event-stream";
...
describe("agent-loop OTEL instrumentation", () => {
	...
	it("emits invoke_agent → chat hierarchy with OTEL and yemu.gen_ai extension attributes", async () => {
		...
		expect(invoke?.attributes[YemuGenAIAttr.AgentStepCount]).toBe(1);
		...
		expect(chat?.attributes[YemuGenAIAttr.AgentStepNumber]).toBe(0);
		...
		expect(chat?.attributes[YemuGenAIAttr.UsageTotalTokens]).toBe(58);
	...
	it("emits execute_tool spans parented to invoke_agent (not chat) per semconv", async () => {
		...
		// yemu.gen_ai.agent.step.count counts chat completions
		expect(invoke?.attributes[YemuGenAIAttr.AgentStepCount]).toBe(2);
	});
	...
	it("captures string systemPrompt without crashing when captureMessageContent is true", async () => {
		...
		const request = JSON.parse(chat?.attributes[YemuGenAIAttr.RequestMessages] as string) as Array<{
			content: unknown;
			role: string;
		}>;
	...
	it("captures bounded dashboard summary content when requested", async () => {
		...
		const responseText = JSON.parse(chat?.attributes[YemuGenAIAttr.ResponseText] as string);
	...
	it("invokes costEstimator and stamps yemu.gen_ai.cost.estimated_usd", async () => {
		const mock = createMockModel({
			...MOCK_IDENT,
			responses: [
				{
		...
		const chat = findSpan(exporter.getFinishedSpans(), "chat mock-model");
		expect(chat?.attributes[YemuGenAIAttr.CostEstimatedUsd]).toBeCloseTo(0.0105, 6);
		expect(chat?.attributes[YemuGenAIAttr.CostInputUsd]).toBeCloseTo(0.003, 6);
		expect(chat?.attributes[YemuGenAIAttr.CostOutputUsd]).toBeCloseTo(0.0075, 6);
	});
	...
	it("emits yemu.gen_ai.cost.unavailable_reason when the estimator declines", async () => {
		const mock = createMockModel({
			...MOCK_IDENT,
		...
		const chat = findSpan(exporter.getFinishedSpans(), "chat mock-model");
		expect(chat?.attributes[YemuGenAIAttr.CostUnavailableReason]).toBe("unsupported_tier");
		expect(chat?.attributes[YemuGenAIAttr.CostEstimatedUsd]).toBeUndefined();
	});
	...
	it("recordHandoff emits a one-shot handoff span with from/to agent identity", async () => {
		...
		expect(span?.attributes[YemuGenAIAttr.HandoffFromAgentName]).toBe("main");
		expect(span?.attributes[YemuGenAIAttr.HandoffToAgentName]).toBe("specialist");
	...
	it("records manual chat telemetry for non-loop model calls", async () => {
		...
		expect(span?.attributes[YemuGenAIAttr.AgentStepNumber]).toBe(7);
		expect(span?.attributes[YemuGenAIAttr.UsageTotalTokens]).toBe(17);
		expect(span?.attributes[YemuGenAIAttr.CostEstimatedUsd]).toBe(0.02);
		expect(JSON.parse(span?.attributes[YemuGenAIAttr.ResponseText] as string)).toEqual(["manual ok"]);
	});
...
describe("ChatUsageEvent.headers and yemu.gen_ai.gateway.* span attributes", () => {
	...
	it("auto-stamps yemu.gen_ai.gateway.* on the chat span when LiteLLM headers are present", async () => {
		const mock = createMockModel({
			...MOCK_IDENT,
			responses: [
				{
		...
		const chat = findSpan(exporter.getFinishedSpans(), "chat mock-model");
		expect(chat?.attributes[YemuGenAIAttr.GatewayName]).toBe("litellm");
		expect(chat?.attributes[YemuGenAIAttr.GatewayCallId]).toBe("ll-call-abc");
		expect(chat?.attributes[YemuGenAIAttr.GatewayRoutedTo]).toBe("anthropic/claude-sonnet-4-7");
		expect(chat?.attributes[YemuGenAIAttr.GatewayEndpoint]).toBe(mock.model.baseUrl);
	});
	...
	it("does not stamp gateway attributes when headers carry no known pattern", async () => {
		...
		const chat = findSpan(exporter.getFinishedSpans(), "chat mock-model");
		expect(chat?.attributes[YemuGenAIAttr.GatewayName]).toBeUndefined();
		expect(chat?.attributes[YemuGenAIAttr.GatewayCallId]).toBeUndefined();
		expect(chat?.attributes[YemuGenAIAttr.GatewayEndpoint]).toBeUndefined();
	});
	...
	it("stamps yemu.gen_ai.gateway.response_cache.status from cf-aig-cache-status without prompt-cache attrs", async () => {
		const mock = createMockModel({
			...MOCK_IDENT,
			responses: [
				{
		...
		const chat = findSpan(exporter.getFinishedSpans(), "chat mock-model");
		expect(chat?.attributes[YemuGenAIAttr.GatewayResponseCacheStatus]).toBe("hit");
	...
	it("classifies miss/bypass/unknown response-cache statuses on the chat span", async () => {
		...
		for (const { header, expected } of cases) {
			...
			await runAndDrain(agentLoop([createUserMessage("hi")], ctx, config, undefined, mock.stream));
			const chat = findSpan(exporter.getFinishedSpans(), "chat mock-model");
			expect(chat?.attributes[YemuGenAIAttr.GatewayResponseCacheStatus]).toBe(expected);
		}
	});
});
