...
} from "@yemu/agent-core/compaction/openai";
...
import { getOpenAICodexTransportDetails } from "@yemu/model-runtime/providers/openai-codex-responses";
import type {
	AssistantMessage,
...
describe("requestOpenAiRemoteCompaction timeout", () => {
	test("a never-responding endpoint rejects with TimeoutError instead of hanging", async () => {
		// Contract: the compact endpoint is a raw fetch outside the model-runtime stream
...
describe("requestRemoteCompaction wire formats", () => {
	...
	test("keeps the generic yemu summarizer format for other endpoints", async () => {
		let sentBody: unknown;
		const fetchMock: FetchImpl = async (_input, init) => {
			if (typeof init?.body !== "string") throw new Error("missing remote compaction request body");
			sentBody = JSON.parse(init.body) as unknown;
