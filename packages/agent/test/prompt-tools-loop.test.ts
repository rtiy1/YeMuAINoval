...
import type { AgentContext, AgentLoopConfig, AgentMessage, AgentTool } from "@yemu/agent-core/types";
import type { AssistantMessage, Context, Message, TextContent, ToolResultMessage } from "@yemu/model-runtime";
...
describe("agentLoop with owned in-band tool calls", () => {
	...
	it("uses YEMU_DIALECT=minimax when config.dialect is unset", async () => {
		const before = Bun.env.YEMU_DIALECT;
		Bun.env.YEMU_DIALECT = "minimax";
		try {
			const echoArgs: Array<{ msg: string }> = [];
			const toolSchema = type({ msg: "string" });
			const echoTool: AgentTool<typeof toolSchema, { msg: string }> = {
			...
			if (before === undefined) delete Bun.env.YEMU_DIALECT;
			else Bun.env.YEMU_DIALECT = before;
		}
	});
});
