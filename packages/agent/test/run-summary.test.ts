...
import { agentLoop, agentLoopDetailed } from "@yemu/agent-core/agent-loop";
import {
...
} from "@yemu/agent-core/run-collector";
import { EXECUTE_TOOL_STATUS_ATTR, GenAIAttr, YemuGenAIAggregateAttr } from "@yemu/agent-core/telemetry";
import type { AgentEvent, AgentLoopConfig, AgentMessage, AgentTool } from "@yemu/agent-core/types";
import type { AssistantMessage, Message } from "@yemu/model-runtime";
...
describe("AgentRunSummary aggregation", () => {
	...
	it("populates aggregate yemu.gen_ai.agent.* attributes on the invoke_agent span", async () => {
		const tracer = new RecordingTracer();
		const tool = buildTool({ name: "alpha", behavior: "ok" });
		const mock = createMockModel({
			responses: [
		...
		expect(invokeSpan?.attributes[YemuGenAIAggregateAttr.ChatsCount]).toBe(2);
		expect(invokeSpan?.attributes[YemuGenAIAggregateAttr.ToolsCount]).toBe(1);
		expect(invokeSpan?.attributes[YemuGenAIAggregateAttr.ToolsOkCount]).toBe(1);
		expect(invokeSpan?.attributes[YemuGenAIAggregateAttr.UsageInputTokensTotal]).toBe(6);
		expect(invokeSpan?.attributes[YemuGenAIAggregateAttr.UsageTotalTokensTotal]).toBe(13);
		expect(invokeSpan?.attributes[YemuGenAIAggregateAttr.ToolsInvoked]).toEqual(["alpha"]);
	});
});
