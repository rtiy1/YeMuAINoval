...
import * as snapcompact from "@yemu/compact";
import { preferredDialect } from "@yemu/model-catalog/identity";
...
import { createOpenAICodexCompactionRequestContext } from "@yemu/model-runtime/providers/openai-codex-responses";
import { convertTools } from "@yemu/model-runtime/providers/openai-responses";
import { buildResponsesInput, resolveOpenAICompatPolicy } from "@yemu/model-runtime/providers/openai-shared";
import { isRecord, logger, prompt, stringifyJson } from "@yemu/utils";
...
function extractFileOperations(
	...
	// Collect from previous compaction's details (if yemu-generated)
 ...
 * (yemu-native gateway) does not populate `errorStatus`, hence the legacy
...
export interface SummaryOptions {
	 ...
	 * `yemu.gen_ai.oneshot.kind` (`compaction_summary`, `compaction_short_summary`,
...
export interface HandoffOptions {
	 ...
	 * wrapped in an OTEL chat span tagged with `yemu.gen_ai.oneshot.kind = "handoff"`.
	 */
	telemetry?: AgentTelemetry;
	/**
	 * Active session thinking level. Threaded from `agent-session.ts` so
