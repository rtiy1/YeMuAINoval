...
} from "@yemu/model-runtime/dialect";
import * as AIError from "@yemu/model-runtime/error";
import { type CursorExecResolvedCarrier, kCursorExecResolved } from "@yemu/model-runtime/utils/block-symbols";
import {
	...
	signalListLabel,
} from "@yemu/model-runtime/utils/harmony-leak";
import { logger, sanitizeText, structuredCloneJSON } from "@yemu/utils";
...
export function normalizeTools(
	...
	injectIntent = injectIntent && Bun.env.YEMU_NO_INTENT !== "1";
	return tools?.map(t => {
...
async function prepareProviderCall(
	...
	const ownedDialect: Dialect | undefined = config.dialect ?? resolveOwnedDialectFromEnv(Bun.env.YEMU_DIALECT);
...
async function executeToolCalls(
	...
	const runTool = async (record: (typeof records)[number], index: number): Promise<void> => {
		...
		if (toolSpan && toolCall.intent) {
			toolSpan.setAttribute(YemuGenAIAttr.ToolCallIntent, toolCall.intent);
		}
