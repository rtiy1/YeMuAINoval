 ...
 * (https://opentelemetry.io/docs/specs/semconv/gen-ai/) plus `yemu.gen_ai.*`
...
export const DEFAULT_TRACER_NAME = "@yemu/agent-core";
...
export const enum YemuGenAIAttr {
	AgentStepNumber = "yemu.gen_ai.agent.step.number",
	AgentStepCount = "yemu.gen_ai.agent.step.count",
	RequestReasoningEffort = "yemu.gen_ai.request.reasoning.effort",
	RequestToolChoice = "yemu.gen_ai.request.tool.choice",
	RequestAvailableTools = "yemu.gen_ai.request.available_tools",
	RequestMessages = "yemu.gen_ai.request.messages",
	ResponseText = "yemu.gen_ai.response.text",
	ResponseToolCalls = "yemu.gen_ai.response.tool_calls",
	ResponseUpstreamProvider = "yemu.gen_ai.response.upstream_provider",
	UsageTotalTokens = "yemu.gen_ai.usage.total_tokens",
	UsageServerSideTools = "yemu.gen_ai.usage.server_tool_requests",
	CostEstimatedUsd = "yemu.gen_ai.cost.estimated_usd",
	CostInputUsd = "yemu.gen_ai.cost.input_usd",
	CostOutputUsd = "yemu.gen_ai.cost.output_usd",
	CostUnavailableReason = "yemu.gen_ai.cost.unavailable_reason",
	ToolStatus = "yemu.gen_ai.tool.status",
	ToolCallIntent = "yemu.gen_ai.tool.call.intent",
	HandoffFromAgentName = "yemu.gen_ai.handoff.from_agent.name",
	HandoffFromAgentId = "yemu.gen_ai.handoff.from_agent.id",
	HandoffToAgentName = "yemu.gen_ai.handoff.to_agent.name",
	HandoffToAgentId = "yemu.gen_ai.handoff.to_agent.id",
	...
	OneshotKind = "yemu.gen_ai.oneshot.kind",
	...
	GatewayName = "yemu.gen_ai.gateway.name",
	GatewayEndpoint = "yemu.gen_ai.gateway.endpoint",
	GatewayCallId = "yemu.gen_ai.gateway.call_id",
	GatewayRoutedTo = "yemu.gen_ai.gateway.routed_to",
	/** Cloudflare AI Gateway response-cache status (`cf-aig-cache-status`), never prompt-cache. */
	GatewayResponseCacheStatus = "yemu.gen_ai.gateway.response_cache.status",
}
 ...
 *   { usd: number }                — cost is known; emitted as yemu.gen_ai.cost.estimated_usd
 *   { unavailable: string }        — cost is intentionally unknown; emitted as
 *                                    yemu.gen_ai.cost.unavailable_reason
...
export interface ChatUsageEvent {
	 ...
	 * auto-stamped on the chat span as `yemu.gen_ai.gateway.*` attributes.
	 */
...
function emitTelemetryWarning(telemetry: AgentTelemetry | undefined, warning: AgentTelemetryWarning): void {
	...
	if (!hook) {
		if (warning.error === undefined) console.warn(`[yemu-agent] ${warning.message}`);
		else console.warn(`[yemu-agent] ${warning.message}`, warning.error);
		return;
	}
	try {
		hook(warning);
	} catch (err) {
		console.warn("[yemu-agent] onTelemetryWarning threw; swallowing:", err);
	}
}
...
export function applyInvokeAgentFinish(span: Span | undefined, stepCount: number): void {
	if (!span) return;
	span.setAttribute(YemuGenAIAttr.AgentStepCount, stepCount);
}
...
function buildChatRequestAttributes(stepNumber: number, request: ChatRequestSnapshot, provider: string): Attributes {
	const attrs: Attributes = {
		[YemuGenAIAttr.AgentStepNumber]: stepNumber,
	...
	if (request.reasoningEffort) attrs[YemuGenAIAttr.RequestReasoningEffort] = request.reasoningEffort;
	const toolChoice = serializeToolChoice(request.toolChoice);
	if (toolChoice) attrs[YemuGenAIAttr.RequestToolChoice] = toolChoice;
	if (request.tools && request.tools.length > 0) {
		attrs[YemuGenAIAttr.RequestAvailableTools] = request.tools.map(tool => tool.name);
	}
	return attrs;
}
...
function applyContentCaptureForRequest(telemetry: AgentTelemetry, span: Span, request: ChatRequestSnapshot): void {
	...
	if (requestMessages) span.setAttribute(YemuGenAIAttr.RequestMessages, requestMessages);
...
function applyContentCaptureForResponse(telemetry: AgentTelemetry, span: Span, message: AssistantMessage): void {
	...
	if (responseText) span.setAttribute(YemuGenAIAttr.ResponseText, responseText);
	const responseToolCalls = serializeResponseToolCallsForTelemetry(telemetry, message);
	if (responseToolCalls) span.setAttribute(YemuGenAIAttr.ResponseToolCalls, responseToolCalls);
...
function applyChatResponseAttributes(span: Span, message: AssistantMessage): void {
	...
	if (message.upstreamProvider) {
		span.setAttribute(YemuGenAIAttr.ResponseUpstreamProvider, message.upstreamProvider);
	}
...
function applyUsageAttributes(span: Span, usage: Usage | undefined): void {
	...
	span.setAttribute(YemuGenAIAttr.UsageTotalTokens, total);
	...
	if (usage.server) {
		...
		if (sums > 0) span.setAttribute(YemuGenAIAttr.UsageServerSideTools, sums);
	}
}
 ...
 * {@link YemuGenAIAttr.GatewayResponseCacheStatus}. Distinct from provider
...
function applyGatewayAttributes(
	...
	if (gateway) {
		span.setAttribute(YemuGenAIAttr.GatewayName, gateway.name);
		if (baseUrl) span.setAttribute(YemuGenAIAttr.GatewayEndpoint, baseUrl);
		if (gateway.callId) span.setAttribute(YemuGenAIAttr.GatewayCallId, gateway.callId);
		if (gateway.routedTo) span.setAttribute(YemuGenAIAttr.GatewayRoutedTo, gateway.routedTo);
	}
	const responseCacheStatus = classifyGatewayResponseCacheStatus(headers);
	if (responseCacheStatus) {
		span.setAttribute(YemuGenAIAttr.GatewayResponseCacheStatus, responseCacheStatus);
	}
}
...
function applyCostEstimateForUsage(
	...
	if ("unavailable" in result) {
		span.setAttribute(YemuGenAIAttr.CostUnavailableReason, result.unavailable);
	...
	span.setAttribute(YemuGenAIAttr.CostEstimatedUsd, result.usd);
	if (result.inputUsd != null) span.setAttribute(YemuGenAIAttr.CostInputUsd, result.inputUsd);
	if (result.outputUsd != null) span.setAttribute(YemuGenAIAttr.CostOutputUsd, result.outputUsd);
	const cost: AppliedCostEstimate = {
		costUsd: result.usd,
...
export async function recordManualChatTelemetry(
	...
	if (options.stepNumber != null) span.setAttribute(YemuGenAIAttr.AgentStepNumber, options.stepNumber);
...
export interface InstrumentedChatSpanOptions {
	...
	readonly stepNumber?: number;
	/**
	 * Tag stamped onto `yemu.gen_ai.oneshot.kind`. Values used by the agent:
	 ...
	 * `completeSimple` from `@yemu/model-runtime`. Use to retain a test injection
...
export async function instrumentedCompleteSimple<TApi extends Api>(
	...
	if (chatSpan) {
		if (oneshotKind) chatSpan.setAttribute(YemuGenAIAttr.OneshotKind, oneshotKind);
...
export const EXECUTE_TOOL_STATUS_ATTR = YemuGenAIAttr.ToolStatus;
...
/** Aggregate `yemu.gen_ai.agent.*` attributes stamped on the `invoke_agent` span. */
export const enum YemuGenAIAggregateAttr {
	ChatsCount = "yemu.gen_ai.agent.chats.count",
	ChatsTotalLatencyMs = "yemu.gen_ai.agent.chats.total_latency_ms",
	ChatsStopReasonPrefix = "yemu.gen_ai.agent.chats.stop_reason.",
	ToolsCount = "yemu.gen_ai.agent.tools.count",
	ToolsOkCount = "yemu.gen_ai.agent.tools.ok.count",
	ToolsErrorCount = "yemu.gen_ai.agent.tools.error.count",
	ToolsSkippedCount = "yemu.gen_ai.agent.tools.skipped.count",
	ToolsBlockedCount = "yemu.gen_ai.agent.tools.blocked.count",
	ToolsTimeoutCount = "yemu.gen_ai.agent.tools.timeout.count",
	ToolsAbortedCount = "yemu.gen_ai.agent.tools.aborted.count",
	ToolsTotalLatencyMs = "yemu.gen_ai.agent.tools.total_latency_ms",
	ToolsInvoked = "yemu.gen_ai.agent.tools.invoked",
	ToolsAvailable = "yemu.gen_ai.agent.tools.available",
	ToolsUnused = "yemu.gen_ai.agent.tools.unused",
	UsageInputTokensTotal = "yemu.gen_ai.agent.usage.input_tokens.total",
	UsageOutputTokensTotal = "yemu.gen_ai.agent.usage.output_tokens.total",
	UsageCacheReadInputTokensTotal = "yemu.gen_ai.agent.usage.cache_read.input_tokens.total",
	UsageCacheCreationInputTokensTotal = "yemu.gen_ai.agent.usage.cache_creation.input_tokens.total",
	UsageReasoningOutputTokensTotal = "yemu.gen_ai.agent.usage.reasoning.output_tokens.total",
	UsageTotalTokensTotal = "yemu.gen_ai.agent.usage.total_tokens.total",
	CostEstimatedUsdTotal = "yemu.gen_ai.agent.cost.estimated_usd.total",
	ErrorsCount = "yemu.gen_ai.agent.errors.count",
}

/** Stamp the aggregate `yemu.gen_ai.agent.*` attributes on the given span. */
function applyAggregateAttributes(span: Span, summary: AgentRunSummary, coverage: AgentRunCoverage): void {
	span.setAttribute(YemuGenAIAggregateAttr.ChatsCount, summary.chats.total);
	span.setAttribute(YemuGenAIAggregateAttr.ChatsTotalLatencyMs, summary.chats.totalLatencyMs);
	for (const [reason, count] of Object.entries(summary.chats.byStopReason)) {
		span.setAttribute(`${YemuGenAIAggregateAttr.ChatsStopReasonPrefix}${reason}.count`, count);
	}
	span.setAttribute(YemuGenAIAggregateAttr.ToolsCount, summary.tools.total);
	span.setAttribute(YemuGenAIAggregateAttr.ToolsOkCount, summary.tools.ok);
	span.setAttribute(YemuGenAIAggregateAttr.ToolsErrorCount, summary.tools.error);
	span.setAttribute(YemuGenAIAggregateAttr.ToolsSkippedCount, summary.tools.skipped);
	span.setAttribute(YemuGenAIAggregateAttr.ToolsBlockedCount, summary.tools.blocked);
	span.setAttribute(YemuGenAIAggregateAttr.ToolsTimeoutCount, summary.tools.timeout);
	span.setAttribute(YemuGenAIAggregateAttr.ToolsAbortedCount, summary.tools.aborted);
	span.setAttribute(YemuGenAIAggregateAttr.ToolsTotalLatencyMs, summary.tools.totalLatencyMs);
	if (coverage.toolsInvoked.length > 0) {
		span.setAttribute(YemuGenAIAggregateAttr.ToolsInvoked, [...coverage.toolsInvoked]);
	}
	if (coverage.toolsAvailable.length > 0) {
		span.setAttribute(YemuGenAIAggregateAttr.ToolsAvailable, [...coverage.toolsAvailable]);
	}
	if (coverage.toolsUnused.length > 0) {
		span.setAttribute(YemuGenAIAggregateAttr.ToolsUnused, [...coverage.toolsUnused]);
	}
	span.setAttribute(YemuGenAIAggregateAttr.UsageInputTokensTotal, summary.usage.inputTokens);
	span.setAttribute(YemuGenAIAggregateAttr.UsageOutputTokensTotal, summary.usage.outputTokens);
	span.setAttribute(YemuGenAIAggregateAttr.UsageCacheReadInputTokensTotal, summary.usage.cachedInputTokens);
	span.setAttribute(YemuGenAIAggregateAttr.UsageCacheCreationInputTokensTotal, summary.usage.cacheWriteTokens);
	span.setAttribute(YemuGenAIAggregateAttr.UsageReasoningOutputTokensTotal, summary.usage.reasoningOutputTokens);
	span.setAttribute(YemuGenAIAggregateAttr.UsageTotalTokensTotal, summary.usage.totalTokens);
	if (summary.cost.estimatedUsd > 0) {
		span.setAttribute(YemuGenAIAggregateAttr.CostEstimatedUsdTotal, summary.cost.estimatedUsd);
	}
	span.setAttribute(YemuGenAIAggregateAttr.ErrorsCount, summary.errors.total);
}
...
export function recordHandoff(
	...
	if (fromAgent?.name) attrs[YemuGenAIAttr.HandoffFromAgentName] = fromAgent.name;
	if (fromAgent?.id) attrs[YemuGenAIAttr.HandoffFromAgentId] = fromAgent.id;
	if (toAgent.name) attrs[YemuGenAIAttr.HandoffToAgentName] = toAgent.name;
	if (toAgent.id) attrs[YemuGenAIAttr.HandoffToAgentId] = toAgent.id;
