...
Typed Python bindings for the `yemu --mode rpc` protocol used by the coding agent.
...
- typed startup options for common `yemu --mode rpc` flags such as thinking level,
...
```python
from yemu_rpc import RpcClient
...
```python
from yemu_rpc import MessageUpdateEvent, RpcClient
...
from yemu_rpc import RpcClient, host_tool
...
```python
from yemu_rpc import RpcClient, host_uri
...
```python
from yemu_rpc import assistant_text, assistant_text_with_thinking
