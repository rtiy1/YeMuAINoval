...
from yemu_rpc import RpcClient, RpcCommandError, RpcConcurrencyError, RpcError, host_tool
from yemu_rpc.client import _RpcFrameDecoder
...
class RpcClientTests(unittest.TestCase):
    ...
    def test_xd_dispatched_custom_tool_events_carry_host_tool_name(self) -> None:
        ...
        With `tools.xdev` on, yemu invokes a custom tool through `write
        ...
        (yemu-agent#6696). `tool_execution_start` precedes the `host_tool_call`
...
class StopUnblocksPromptAndWaitTests(unittest.TestCase):
    ...
    def test_stop_during_prompt_unblocks_waiter(self) -> None:
        from yemu_rpc import RpcProcessExitError
...
class TerminatesProcessGroupTests(unittest.TestCase):
    ...
    the yemu process. Before the fix, stop() signalled only the leader pid, so
    such grandchildren reparented to the container init and kept running —
    once ballooning to tens of GB of RAM. yemu is now spawned in its own
    ...
    def test_stop_kills_grandchild_spawned_by_server(self) -> None:
        ...
        # Fake yemu server: spawn the long-lived grandchild, signal ready, then
