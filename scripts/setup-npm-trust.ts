...
const FALLBACK_REPO = "rtiy1/YeMuAINoval";
	...
	for (const pkg of packages) {
		...
		if (pkg.kind === "native") {
			for (const target of LEAF_TARGETS) {
				const leaf = `@yemu/native-${target.tag}`;
				if (!seen.has(leaf)) {
					seen.add(leaf);
					names.push(leaf);
				}
...
function placeholderManifest(name: string, target: NativeLeafTarget, repo: string): PlaceholderManifest {
	return {
		...
		description: `Placeholder for the ${target.tag} native addon of @yemu/native. The real binary is published during release.`,
		license: "MIT",
		os: [target.os],
		cpu: [target.cpu],
		repository: {
...
function placeholderReadme(name: string, target: NativeLeafTarget): string {
	return [
		`# ${name}`,
		"",
		`Placeholder package reserving the npm name for the \`${target.tag}\` native addon of \`@yemu/native\`.`,
		"",
...
async function publishNativeLeafPlaceholder(name: string, target: NativeLeafTarget, repo: string): Promise<boolean> {
	const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "yemu-native-placeholder-"));
	try {
		await Bun.write(
