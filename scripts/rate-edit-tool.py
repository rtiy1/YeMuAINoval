...
from yemu_rpc import (  # noqa: E402
    AgentEndEvent,
    AutoRetryEndEvent,
    AutoRetryStartEvent,
    ExtensionUiRequest,
...
def resolve_yemu_bin(raw: str | None) -> str:
    ...
    if not found:
        raise SystemExit("Could not find `yemu` on PATH. Set --yemu-bin or YEMU_BIN.")
    return found
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run OpenRouter fixture evaluations through yemu RPC mode."
    )
    parser.add_argument("--yemu-bin", default=os.environ.get("YEMU_BIN"))
...
async def run_all(args: argparse.Namespace) -> int:
    ...
    results_dir = (
        ...
        if args.results_dir
        else tmp_root / f"yemu-fixture-runs-{timestamp}"
    )
