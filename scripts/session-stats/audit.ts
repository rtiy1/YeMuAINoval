#!/usr/bin/env bun
/**
 * Token-usage audit over the local yemu session corpus (~/.yemu/agent/sessions/).
 *
 ...
 * asks a small model (default: anthropic/claude-sonnet-4-6 via @yemu/model-runtime)
 * to judge:
 ...
 *   bun scripts/session-stats/audit.ts --folder Projects-yemu --max-llm 6
 *   bun scripts/session-stats/audit.ts --json out.json
 *
 * Auth: resolves an API key for the classifier provider through yemu's auth
 * storage (~/.yemu/agent/agent.db: stored key, OAuth, or env var fallback).
 */
...
import { type GeneratedProvider, getBundledModel } from "@yemu/model-catalog/models";
import {
	type Api,
	AuthStorage,
	completeSimple,
...
import { getAgentDbPath, isEnoent } from "@yemu/utils";
import SYSTEM_PROMPT from "./audit-prompt.md" with { type: "text" };

const SESSIONS_ROOT = path.join(os.homedir(), ".yemu", "agent", "sessions");
const DEFAULT_MODEL = "anthropic/claude-sonnet-4-6";
const CACHE_PATH = path.join(os.homedir(), ".yemu", "stats-audit-cache.json");
...
function parseCli(argv: string[]): CliOptions {
	...
	if (values.help) {
		console.log(
			`session audit — token usage analysis over ~/.yemu/agent/sessions\n\n` +
				...
				`  --no-cache               skip the verdict cache (~/.yemu/stats-audit-cache.json)\n` +
...
async function openClassifier(modelSpec: string): Promise<Classifier> {
	...
	if (!apiKey) {
		throw new Error(`no credentials for provider "${provider}" (yemu login or env var required)`);
	}
	return { model, apiKey };
}
...
/** Narrow view of model-runtime's AssistantMessage used here (content + usage). */
