...
(`~/.yemu/agent/sessions/`). SQLite-backed; data is synced once into the same
`~/.yemu/stats.db` that `packages/stats` uses, then queried by short Python
scripts.
...
scripts/session-stats/
  sync.py          # walks ~/.yemu/agent/sessions/ and populates ss_* tables
...
bun run stats:audit -- --since 3d --folder Projects-yemu
...
The LLM phase (default `anthropic/claude-sonnet-4-6` via `@yemu/model-runtime`,
credentials resolved through yemu's auth storage — stored key, OAuth, or env
...
Verdicts are cached in `~/.yemu/stats-audit-cache.json` (keyed by session id +
