...
// yemu-memory is intentionally excluded — its embedding suites depend on a ~270MB
// fastembed model absent from CI runners, so they flake/time out under the parallel
// bucket; run `bun --cwd=packages/yemu-memory test` locally instead.
...
// cover. yemu-memory's embedding suites need a ~270MB fastembed model absent
// from CI runners, so they flake or time out under the parallel bucket.
const localOnlyWorkspacePackages = ["packages/yemu-memory"];

// Repo-level script tests retained by this embedded runtime.
const repoScriptTests = ["scripts/ci-concurrency.test.ts", "scripts/fix-dts-extensions.test.ts"];

const codingAgentNativePathPatterns = [
	/(^|\/)[^/]*(bash|native|browser|cmux|yemu-memory|hindsight|memory)[^/]*\.test\.ts$/i,
...
async function commandsForMode(mode: Mode): Promise<TestCommand[]> {
	switch (mode) {
		...
		// `all` set PLUS yemu-memory and yemu-web, which CI omits) and every repo
...
// The yemu-kata runner pods may inject cloud credentials (`AWS_*`) pod-wide via
...
// Override with YEMU_TEST_CHUNK_TIMEOUT (seconds).
function chunkTimeoutMs(): number {
	const raw = Number(Bun.env.YEMU_TEST_CHUNK_TIMEOUT?.trim());
...
// Defaults to the machine's available parallelism; `YEMU_TEST_CONCURRENCY`
...
function testConcurrency(total: number): number {
	const raw = Bun.env.YEMU_TEST_CONCURRENCY?.trim().toLowerCase();
	...
	throw new Error(`Invalid YEMU_TEST_CONCURRENCY=${JSON.stringify(raw)}; expected a positive integer, all, or max`);
}
...
export async function runTestCommandsInParallel(commands: TestCommand[], concurrency: number): Promise<void> {
	...
	console.log(
		`Running ${commands.length} test command(s), up to ${concurrency} in parallel ` +
			`(YEMU_TEST_CONCURRENCY=<n>|all to change).`,
	);
	...
	async function runAttempt(
		...
		return {
			exitCode,
			timedOut,
			output: `${stdout.text}${stderr.text}${timedOut ? `\n[watchdog] chunk exceeded ${Math.round(chunkTimeoutMs() / 1000)}s; killed with SIGKILL (YEMU_TEST_CHUNK_TIMEOUT to change)\n` : ""}`,
		};
	}
...
if (import.meta.main) {
	...
	const explicitConcurrency = Boolean(Bun.env.YEMU_TEST_CONCURRENCY?.trim());
