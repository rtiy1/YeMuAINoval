...
function analyze(fn: FunctionDeclaration, opts: Options): Candidate | null {
	...
	for (const yemu of params) {
		for (const ref of yemu.param.findReferencesAsNodes()) {
			if (Node.isIdentifier(ref) && ref.getStart() >= bodyStart && ref.getEnd() <= bodyEnd) yemu.refs.push(ref);
		}
		for (const ref of yemu.refs) {
			if (isWriteTarget(ref)) return skip(opts, name, `writes parameter \`${yemu.name}\``);
		}
	}
...
function buildReplacement(
	...
	for (let i = 0; i < candidate.params.length; i++) {
		const yemu = candidate.params[i];
		const uses = yemu.refs.length;
		...
		if (argNode === undefined) {
			substByParam.set(yemu.name, "undefined");
			continue;
		}
		...
		if (hoist) {
			const temp = freshName(`__inl_${yemu.name}`, reservedNames);
			reservedNames.add(temp);
			prefix.push(`const ${temp} = ${argNode.getText()};`);
			substByParam.set(yemu.name, temp);
		} else {
			substByParam.set(yemu.name, argNeedsParens(argNode) ? `(${argNode.getText()})` : argNode.getText());
		}
	}

	const paramEdits: Edit[] = [];
	for (const yemu of candidate.params) {
		const t = substByParam.get(yemu.name);
		if (t === undefined) continue;
		for (const r of yemu.refs) paramEdits.push({ start: r.getStart(), end: r.getEnd(), text: editTextFor(r, t) });
	}
