...
sys.path.insert(0, str(REPO_ROOT / "python/yemu-rpc/src"))

from yemu_rpc import (
    MessageEndEvent,
    MessageStartEvent,
    MessageUpdateEvent,
    RpcClient,
...
def resolve_repo_yemu_bin() -> str | None:
...
def resolve_yemu_bin(raw: str | None) -> str:
    if raw:
        return raw
    repo_bin = resolve_repo_yemu_bin()
    if repo_bin:
        return repo_bin
    found = shutil.which("yemu")
    if not found:
        raise SystemExit(
            "Could not find `yemu` on PATH and could not resolve the repo CLI. Set --yemu-bin or YEMU_BIN."
        )
    return found
...
async def run_all(
    ...
    yemu_bin = resolve_yemu_bin(args.yemu_bin)
...
def parse_args(description: str) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--yemu-bin",
        default=os.environ.get("YEMU_BIN"),
        help="Executable to launch. Defaults to the repo checkout CLI, then falls back to `yemu` on PATH.",
    )
    parser.add_argument(
