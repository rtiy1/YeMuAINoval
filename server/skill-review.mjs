...
const reviewMode = String(
	process.env.SKILL_REVIEW_MODE || (process.env.NODE_ENV === "production" ? "required" : "optional"),
)
	.trim()
	.toLowerCase();
...
const reviewStyle = String(process.env.SKILL_REVIEW_API_STYLE || "responses")
	.trim()
	.toLowerCase();
...
function extractReviewFiles(fileName, extension, buffer) {
	...
	const unpackedByName = new Map(
		Object.entries(unpacked).map(([name, bytes]) => [assertSafeArchivePath(name), bytes]),
	);
...
export function extractSkillPromptContract({ fileName, extension, buffer }) {
	...
	const primary =
		extension === ".zip" ? files.find(file => file.name.split("/").at(-1)?.toLowerCase() === "skill.md") : files[0];
	if (!primary?.content?.trim()) throw securityError("Skill 缺少可执行的 SKILL.md 指令");
	const references = files
		.filter(
			file =>
				file !== primary && file.content && file.name.replaceAll("\\", "/").toLowerCase().includes("/references/"),
		)
