const configuredProvider = String(
	process.env.EMAIL_PROVIDER ||
		(process.env.RESEND_API_KEY ? "resend" : process.env.NODE_ENV === "production" ? "disabled" : "console"),
)
	.trim()
	.toLowerCase();
...
function disabledDelivery() {
	if (!warnedDisabled) {
		warnedDisabled = true;
		console.warn(
			"Account email delivery is disabled. Configure EMAIL_PROVIDER=resend, RESEND_API_KEY, and EMAIL_FROM.",
		);
	}
