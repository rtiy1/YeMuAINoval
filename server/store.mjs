...
const dataFile = process.env.STORY_DATA_FILE
	? path.resolve(process.env.STORY_DATA_FILE)
...
async function getPostgresPool() {
	if (!postgresPoolPromise) {
		postgresPoolPromise = import("pg").then(
			({ Pool }) =>
				new Pool({
...
async function readRelationalState(client, auxiliary) {
	...
	const db = {
		...
		projects: projects.rows.map(row => ({
			...
			words: formatWords(Number(row.word_count || 0)),
	...
	for (const row of chapters.rows) {
		...
		db.chapters[row.project_id].push({
			id: numericChapterId(row.chapter_id),
			title: row.title,
			outline: row.outline,
			state: row.state,
			words: formatWords(Number(row.word_count || 0)),
	...
	for (const user of db.users) {
		await client.query(
			...
			[
				...
				user.email,
				user.passwordHash,
				user.settings ? JSON.stringify(user.settings) : null,
	...
	for (const project of db.projects) {
        ...
        style, tone, cover, is_active, created_at, updated_at
			...
			[
				...
				project.genre,
				project.status,
				integerValue(project.progress),
				integerValue(project.words),
				project.updated || "",
				integerValue(project.chapters),
				project.style || "",
				project.tone || "",
				project.cover || "cover-new",
...
function chapterRecords(db) {
	return db.projects.flatMap(project =>
		(db.chapters[project.id] || []).map((chapter, position) => ({
			...chapter,
...
async function syncRelationalState(client, previousDb, db) {
	...
	for (const [key, record] of previous.drafts) {
		if (!current.drafts.has(key))
			await client.query("DELETE FROM drafts WHERE project_id = $1 AND chapter_id = $2", [
				record.projectId,
				record.chapterId,
			]);
	}
	for (const [key, record] of previous.chapters) {
		if (!current.chapters.has(key))
			await client.query("DELETE FROM chapters WHERE project_id = $1 AND chapter_id = $2", [
				record.projectId,
				record.chapterId,
			]);
	}
	...
	if (!stateRow) {
       ...
       ON CONFLICT (state_key) DO NOTHING RETURNING state_key`,
		...
		const existing = await client.query(
			"SELECT data, schema_version FROM story_state WHERE state_key = $1 FOR UPDATE",
			["default"],
		);
...
function normalizeDb(db) {
	...
	for (const project of db.projects) {
		...
		if (typeof legacyDraft === "string") {
			...
			db.drafts[project.id] = firstChapter ? { [String(firstChapter.id)]: legacyDraft } : { __legacy: legacyDraft };
...
export async function loadDb() {
	if (postgresEnabled) {
		...
		try {
			...
			const result = await client.query("SELECT data, schema_version FROM story_state WHERE state_key = $1", [
				"default",
			]);
