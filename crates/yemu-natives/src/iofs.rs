...
//!
//! `yemu-walker` owns traversal and cache policy. This module keeps only the
...
fn walker_error_to_napi<E: std::fmt::Display>(err: yemu_walker::WalkError<E>) -> Error {
	match err {
		yemu_walker::WalkError::Interrupted(err) => Error::from_reason(err.to_string()),
		yemu_walker::WalkError::InvalidData { path, message } => Error::from_reason(format!(
...
pub(crate) const fn from_walker_file_type(file_type: yemu_walker::FileType) -> FileType {
	match file_type {
		yemu_walker::FileType::File => FileType::File,
		yemu_walker::FileType::Dir => FileType::Dir,
		yemu_walker::FileType::Symlink => FileType::Symlink,
	}
}

impl From<yemu_walker::CollectedEntry> for GlobMatch {
	fn from(entry: yemu_walker::CollectedEntry) -> Self {
		Self {
			path:      entry.path,
...
pub(crate) fn map_walker_error<E: std::fmt::Display>(err: yemu_walker::WalkError<E>) -> Error {
...
pub fn invalidate_fs_scan_cache(path: Option<String>) {
	match path {
		Some(path) => yemu_walker::invalidate_path_string(&path),
		None => yemu_walker::invalidate_all(),
	}
}
