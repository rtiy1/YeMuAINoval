//! Native utilities exported via N-API for the YeMu Agent toolchain.
//!
//! # Overview
...
//! # Example
//! ```ignore
//! use yemu_natives::text::visible_width;
//!
...
fn configure_rayon_pool() {
	...
	let result = match plan {
		RayonPoolPlan::WorkerThreads(threads) => rayon::ThreadPoolBuilder::new()
			...
			.build_global(),
		RayonPoolPlan::SkipGlobalPool => {
			yemu_uutils_ctx::set_rayon_global_pool_available(false);
			return;
		},
	};
	if result.is_ok() {
		yemu_uutils_ctx::set_rayon_global_pool_available(true);
	}
}
...
///
/// Bump policy: `__yemuNativesV{major}_{minor}_{patch}` — non-alphanumerics in
/// the version string are mapped to `_` to keep it a valid JS identifier.
/// MUST stay in sync with `VERSION_SENTINEL_EXPORT` in
/// `packages/natives/native/index.js` (which derives the name from
/// `package.json#version`).
#[napi(js_name = "__yemuNativesV17_2_0")]
pub const fn yemu_natives_version_sentinel() {}
...
/// [`yemu_install_tokio_runtime`], which the JS loader calls once `dlopen` has
/// returned.
///
...
/// Guards [`yemu_install_tokio_runtime`] so the runtime is built at most once
