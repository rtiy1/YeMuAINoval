...
impl From<yemu_ast::block::BlockRange> for BlockRange {
	fn from(value: yemu_ast::block::BlockRange) -> Self {
...
pub fn block_range_at(options: BlockRangeOptions) -> Result<Option<BlockRange>> {
	yemu_ast::block::block_range_at(yemu_ast::block::BlockRangeOptions {
		code: options.code,
		lang: options.lang,
		path: options.path,
		line: options.line,
...
pub fn enclosing_block_boundaries(options: EnclosingBoundaryOptions) -> Result<Option<Vec<u32>>> {
	yemu_ast::block::enclosing_block_boundaries(yemu_ast::block::EnclosingBoundaryOptions {
		code:   options.code,
		lang:   options.lang,
		path:   options.path,
		ranges: options
			.ranges
			.into_iter()
			.map(|range| yemu_ast::block::LineRange {
