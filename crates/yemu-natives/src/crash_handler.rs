...
//! native module loads, so any crash inside `yemu-native` writes an actionable
...
//!   `$XDG_STATE_HOME/yemu/logs/` on Linux / macOS when the user has migrated to
//!   XDG (i.e. that directory already exists and `YEMU_CODING_AGENT_DIR` isn't
//!   pointed somewhere custom), otherwise `<home>/<YEMU_CONFIG_DIR>/logs/`
//!   (defaulting to `~/.yemu/logs/`).
...
/// Default directory name for YeMu's per-user state (overridable via
/// `YEMU_CONFIG_DIR`, matching `packages/utils/src/dirs.ts`).
const DEFAULT_CONFIG_DIR: &str = ".yemu";

/// App name used as the XDG-root subdirectory (`$XDG_STATE_HOME/yemu/`),
/// matching `APP_NAME` in `packages/utils/src/dirs.ts`.
#[cfg(any(target_os = "linux", target_os = "macos"))]
const APP_NAME: &str = "yemu";
...
fn panic_disposition() -> PanicDisposition {
	if blocking_task_panic_scope_active() || yemu_uutils_ctx::is_active() {
		PanicDisposition::LoggedRecoverable
	} else {
		PanicDisposition::Fatal
	}
...
fn persist(report: &str, kind: CrashKind, echo_stderr: bool) {
	...
	if let Ok(mut f) = OpenOptions::new().create(true).append(true).open(&path) {
		...
		if echo_stderr {
			let _ =
				writeln!(std::io::stderr(), "yemu-native crash report written to {}", path.display());
		}
	}
}
...
fn logs_dir() -> Option<PathBuf> {
	let home = home_dir()?;
	let config_override = std::env::var_os("YEMU_CONFIG_DIR");
...
fn resolve_logs_dir(
	...
	// XDG takes precedence so users who migrated to `$XDG_STATE_HOME/yemu/logs/`
...
/// `$XDG_STATE_HOME` set, `$XDG_STATE_HOME/yemu` exists on disk, and
/// `YEMU_CODING_AGENT_DIR` is unset or pointing at the default agent dir.
#[cfg(any(target_os = "linux", target_os = "macos"))]
fn xdg_state_logs_from_env(home: &Path, config_dir_override: Option<&OsStr>) -> Option<PathBuf> {
	...
	let agent_override = std::env::var_os("YEMU_CODING_AGENT_DIR");
...
/// reads, no fs reads. `yemu_dir_exists` decides whether the candidate
/// `<xdg_state_home>/yemu` actually lives on disk.
#[cfg(any(target_os = "linux", target_os = "macos"))]
fn xdg_state_logs(
	xdg_state_home: Option<&OsStr>,
	agent_dir_override: Option<&OsStr>,
	default_agent_dir: &Path,
	yemu_dir_exists: impl FnOnce(&Path) -> bool,
) -> Option<PathBuf> {
	...
	let yemu_dir = Path::new(xdg).join(APP_NAME);
	if !yemu_dir_exists(&yemu_dir) {
		return None;
	}
	Some(yemu_dir.join("logs"))
}
#[cfg(any(target_os = "linux", target_os = "macos"))]
...
mod tests {
	...
	fn alloc_report_contains_size_alignment_and_backtrace() {
		...
		let report = format_alloc_report(layout);
		assert!(report.contains("yemu-native alloc crash"), "report missing header: {report}");
	...
	#[test]
	fn resolve_logs_dir_defaults_under_dot_yemu() {
		let dir = resolve_logs_dir(Path::new("/tmp/yemu-native-test-home"), None, None);
		assert_eq!(dir, PathBuf::from("/tmp/yemu-native-test-home/.yemu/logs"));
	}

	#[test]
	fn resolve_logs_dir_honors_relative_yemu_config_dir() {
		let dir = resolve_logs_dir(
			Path::new("/tmp/yemu-native-test-home"),
			Some(OsStr::new(".yemu-dev")),
			None,
		);
		assert_eq!(dir, PathBuf::from("/tmp/yemu-native-test-home/.yemu-dev/logs"));
	}

	#[test]
	fn resolve_logs_dir_reroots_absolute_yemu_config_dir_under_home() {
		// JS resolves the config root via `path.join(os.homedir(),
		// getConfigDirName())`, which never honors an absolute YEMU_CONFIG_DIR — it is
		// always re-rooted under `$HOME` (and `..` components are normalized away).
		let dir = resolve_logs_dir(
			Path::new("/tmp/yemu-native-test-home"),
			Some(OsStr::new("/var/tmp/yemu-native-state")),
			None,
		);
		assert_eq!(dir, PathBuf::from("/tmp/yemu-native-test-home/var/tmp/yemu-native-state/logs"));
	}
	...
	fn resolve_logs_dir_normalizes_parent_components_like_path_join() {
		let dir = resolve_logs_dir(
			Path::new("/tmp/yemu-native-test-home"),
			Some(OsStr::new("nested/../.yemu-dev")),
			None,
		);
		assert_eq!(dir, PathBuf::from("/tmp/yemu-native-test-home/.yemu-dev/logs"));
	}
	...
	fn xdg_state_logs_ignores_empty_agent_dir_override() {
		// An empty YEMU_CODING_AGENT_DIR is "unset", not a divergent override; it
		// must not disable XDG resolution.
		let dir = xdg_state_logs(
			Some(OsStr::new("/xdg/state")),
			Some(OsStr::new("")),
			Path::new("/tmp/yemu-native-test-home/.yemu/agent"),
			|_p| true,
		);
		assert_eq!(dir, Some(PathBuf::from("/xdg/state/yemu/logs")));
	}

	#[test]
	fn resolve_logs_dir_ignores_empty_yemu_config_dir() {
		let dir =
			resolve_logs_dir(Path::new("/tmp/yemu-native-test-home"), Some(OsStr::new("")), None);
		assert_eq!(dir, PathBuf::from("/tmp/yemu-native-test-home/.yemu/logs"));
	}
	...
	fn resolve_logs_dir_prefers_xdg_when_provided() {
		let dir = resolve_logs_dir(
			Path::new("/tmp/yemu-native-test-home"),
			None,
			Some(PathBuf::from("/xdg/state/yemu/logs")),
		);
		assert_eq!(dir, PathBuf::from("/xdg/state/yemu/logs"));
	}
	...
	#[cfg(any(target_os = "linux", target_os = "macos"))]
	#[test]
	fn xdg_state_logs_skipped_when_yemu_dir_missing() {
		let dir = xdg_state_logs(
			Some(OsStr::new("/xdg/state")),
			None,
			Path::new("/tmp/yemu-native-test-home/.yemu/agent"),
	...
	fn xdg_state_logs_skipped_when_xdg_state_home_unset_or_empty() {
		let default_agent = Path::new("/tmp/yemu-native-test-home/.yemu/agent");
	...
	fn xdg_state_logs_skipped_when_agent_dir_overridden() {
		// `YEMU_CODING_AGENT_DIR` pointing elsewhere mirrors the JS `isDefault === false`
	...
	fn xdg_state_logs_honored_when_agent_override_matches_default() {
		let default_agent = std::path::absolute(Path::new("./.yemu/agent")).unwrap();
		let dir = xdg_state_logs(
			Some(OsStr::new("/xdg/state")),
			Some(OsStr::new("./.yemu/agent")),
			&default_agent,
			|_p| true,
		);
		assert_eq!(dir, Some(PathBuf::from("/xdg/state/yemu/logs")));
	...
	#[cfg(any(target_os = "linux", target_os = "macos"))]
	#[test]
	fn default_agent_dir_uses_dot_yemu_by_default() {
		let dir = default_agent_dir(Path::new("/tmp/yemu-native-test-home"), None);
		assert_eq!(dir, PathBuf::from("/tmp/yemu-native-test-home/.yemu/agent"));
	}
	#[cfg(any(target_os = "linux", target_os = "macos"))]
	#[test]
	fn default_agent_dir_respects_yemu_config_dir() {
		let dir =
			default_agent_dir(Path::new("/tmp/yemu-native-test-home"), Some(OsStr::new(".yemu-dev")));
		assert_eq!(dir, PathBuf::from("/tmp/yemu-native-test-home/.yemu-dev/agent"));
	}

	#[test]
	fn build_crash_log_path_tags_kind_and_pid() {
		let dir = Path::new("/tmp/yemu-native-test-home/.yemu/logs");
		let panic_log = build_crash_log_path(dir, CrashKind::Panic, 4242, 1_700_000_000_000);
		assert_eq!(
			panic_log,
			PathBuf::from("/tmp/yemu-native-test-home/.yemu/logs/native-panic-4242-1700000000000.log")
		);
		let alloc_log = build_crash_log_path(dir, CrashKind::Alloc, 99, 1);
		assert_eq!(
			alloc_log,
			PathBuf::from("/tmp/yemu-native-test-home/.yemu/logs/native-alloc-99-1.log")
		);
	}
}
