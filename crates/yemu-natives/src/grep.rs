...
fn build_grep_walk_request(
	...
	order: yemu_walker::WalkOrder,
) -> Result<yemu_walker::WalkRequest> {
	let mut filter = yemu_walker::WalkFilter::files_only();
	if let Some(glob) = glob.map(str::trim).filter(|value| !value.is_empty()) {
		let pattern = glob_util::build_glob_pattern(glob, true);
		let compiled = yemu_walker::CompiledWalkGlob::new([pattern])
			.map_err(|err| Error::from_reason(format!("Invalid glob pattern: {err}")))?;
		filter = filter.glob(compiled);
	}

	Ok(yemu_walker::WalkRequest::new(search_path)
		.hidden(include_hidden)
		.gitignore(use_gitignore)
		.skip_git(true)
		.skip_node_modules(skip_node_modules)
		.follow_links(yemu_walker::FollowLinks::Never)
		.detail(yemu_walker::WalkDetail::Minimal)
		.size_hints(yemu_walker::SizeHintPolicy::WhenCheap)
		.order(order)
		.emit_root(false)
		.depth(1, usize::MAX)
		.directory_errors(yemu_walker::DirectoryErrorMode::SkipSkippable)
		.cache(false)
		.filter(filter))
}
...
fn collect_grep_candidates(
	...
	order: yemu_walker::WalkOrder,
	ct: &task::CancelToken,
) -> Result<Option<Vec<yemu_walker::FileCandidate>>> {
...
struct PassState {
	...
	deferred:          Mutex<Vec<yemu_walker::FileCandidate>>,
...
fn search_one_file<M: Matcher + Sync>(
	...
	matcher: &M,
	file: &yemu_walker::FileCandidate,
...
fn run_pass<M: Matcher + Sync>(
	candidates: &[yemu_walker::FileCandidate],
	matcher: &M,
	file_params: SearchParams,
	policy: ReadPolicy,
	...
	if parallel_allowed && yemu_walker::should_parallelize(candidates.len()) {
		yemu_walker::execute_candidates_init(
			candidates,
...
fn process_candidates<M: Matcher + Sync>(
	candidates: Vec<yemu_walker::FileCandidate>,
	matcher: &M,
	params: SearchParams,
	parallel_allowed: bool,
	stop_after_matches: Option<u64>,
	...
	request
		.for_each_file_candidate_parallel(
			|file| {
				...
				{
					return Ok(yemu_walker::ParallelWalkControl::Continue);
				}
				...
				Ok(yemu_walker::ParallelWalkControl::Continue)
			},
			|| ct.heartbeat(),
		)
...
fn flush_stream_window<M: Matcher + Sync>(
	window: &mut Vec<yemu_walker::FileCandidate>,
	...
	request
		.for_each_entry_with_heartbeat(
			...
			|entry| {
				...
				window.push(yemu_walker::FileCandidate {
			...
			|_| Ok(yemu_walker::WalkDecision::Include),
		)
		.map_err(iofs::map_walker_error)?;
...
fn run_streaming_grep<M: Matcher + Sync>(
	...
	match stop_after_matches {
		...
		Some(stop) if stop <= ORDERED_STREAMING_STOP_MAX_COUNT || yemu_walker::walk_workers() <= 1 => {
			run_sequential_grep(
				search_path,
				matcher,
				glob,
...
mod tests {
	...
	impl TempDirGuard {
		fn new() -> Self {
			...
			let path = std::env::temp_dir().join(format!("yemu-grep-test-{pid}-{nanos}-{seq}"));
