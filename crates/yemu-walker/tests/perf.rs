//! Ignored deterministic timing harness for yemu-walker.
//!
//! Run with:
//! cargo test --profile ci -p yemu-walker --test perf -- --ignored --nocapture
//! --test-threads=1
...
use yemu_walker::{WalkDetail, WalkOrder, WalkRequest};
...
#[test]
#[ignore = "run with: cargo test --profile ci -p yemu-walker --test perf -- --ignored --nocapture \
            --test-threads=1"]
...
fn build_synthetic_tree() -> PathBuf {
	let root = unique_temp_root("yemu-walker-perf");
...
fn synthetic_content(file_index: usize) -> String {
	...
	let filler = format!("line {file_index:05} deterministic yemu walker payload text\n");
