...
// `yemu_uutils_ctx`, `translate!` strings are literalized, `show_if_err!` is
// replaced by a context-stderr write plus `yemu_uutils_ctx::set_exit_code`, and
	...
	fn parse_ref(
		...
		if value.is_empty() {
			...
			err.insert(
				...
				// yemu-uutils: literalized `translate!("realpath-invalid-empty-operand")`
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match realpath_main(&matches) {
		// yemu-uutils: per-file failures accumulate their exit code via
		// `yemu_uutils_ctx::set_exit_code` (upstream's `show!` machinery).
		Ok(()) => yemu_uutils_ctx::exit_code(),
		Err(err) => {
			let code = err.code();
			let msg = err.to_string();
			if !msg.is_empty() {
				let _ = writeln!(yemu_uutils_ctx::stderr(), "realpath: {msg}");
			}
...
fn realpath_main(matches: &ArgMatches) -> UResult<()> {
	...
	for path in &paths {
		...
		if !quiet {
			// yemu-uutils: replacement for `show_if_err!` — report the error on
			...
			if let Err(err) = result.map_err_context(|| path.maybe_quote().to_string()) {
				let _ = writeln!(yemu_uutils_ctx::stderr(), "realpath: {err}");
...
fn canonicalize_relative(
	...
	// yemu-uutils: resolve the option path against the shell working directory;
	...
	// consulted.
	let abs = canonicalize(yemu_uutils_ctx::resolve(r), can_mode, resolve)?;
...
fn resolve_path(
	...
	// yemu-uutils: resolve the operand against the shell working directory; `p`
	...
	// consulted.
	let abs = canonicalize(yemu_uutils_ctx::resolve(p), can_mode, resolve)?;

	let abs = process_relative(abs, relative_base, relative_to);

	// yemu-uutils: replacement for `print_verbatim` + process stdout — writes
...
mod tests {
	...
	fn resolves_relative_operand_against_scope_cwd() {
		...
		// the call-site `yemu_uutils_ctx::resolve` patch makes this find the
