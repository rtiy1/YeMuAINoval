...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match tee(&opts) {
		Ok(()) => 0,
		Err(err) => {
			let _ = writeln!(yemu_uutils_ctx::stderr(), "tee: {err}");
			1
		},
	}
}
...
fn tee(options: &Options) -> Result<()> {
	// yemu-uutils: deliberately do not honor -i by installing a process-global
	...
	for name in &options.files {
		...
		match open(name, options.append) {
			...
			Err(err) => {
				let _ = writeln!(yemu_uutils_ctx::stderr(), "tee: {}: {err}", name.maybe_quote());
	...
	let copy_result = copy(yemu_uutils_ctx::stdin(), &mut output);
...
fn copy(mut input: impl Read, mut output: impl Write) -> Result<usize> {
	...
	loop {
		match input.read(&mut buffer) {
			...
			Err(err) => {
				let _ = writeln!(yemu_uutils_ctx::stderr(), "tee: error reading standard input: {err}");
				return Err(err);
			},
		}
	}
...
fn open(name: &OsString, append: bool) -> Result<NamedWriter> {
	let path = yemu_uutils_ctx::resolve(name);
		...
		self.writers.retain_mut(|writer| {
			...
			match result {
				...
				Err(err) => {
					...
					if report {
						let _ =
							writeln!(yemu_uutils_ctx::stderr(), "tee: {}: {err}", writer.name.maybe_quote());
						errors += 1;
					}
...
impl Write for Writer {
	fn write(&mut self, buf: &[u8]) -> Result<usize> {
		match self {
			...
			Self::Stdout => yemu_uutils_ctx::stdout().write(buf),
		}
	}
	...
	fn flush(&mut self) -> Result<()> {
		match self {
			...
			Self::Stdout => yemu_uutils_ctx::stdout().flush(),
		}
	}
}
