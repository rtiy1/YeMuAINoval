...
fn get_or_create_file(path: &str) -> Result<File, Box<dyn Error>> {
	let file = File::create(yemu_uutils_ctx::resolve(path))?;
	Ok(file)
}
...
fn parse_files0_args(config: &mut Config) -> Result<(), Box<dyn Error>> {
	...
	if mode == "-" {
		yemu_uutils_ctx::stdin().read_to_end(&mut buffer)?;
	} else {
		let mut file = File::open(yemu_uutils_ctx::resolve(mode))
	...
	if string_segments.iter().any(|s| s.is_empty()) {
		let _ = writeln!(yemu_uutils_ctx::stderr(), "find: invalid zero-length file name");
