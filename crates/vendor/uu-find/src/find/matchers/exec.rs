...
impl Matcher for SingleExecMatcher {
	fn matches(&self, file_info: &WalkEntry, _: &mut MatcherIO) -> bool {
		...
		if self.exec_in_parent_dir {
			...
			command.current_dir(yemu_uutils_ctx::cwd());
		}
		command.env_clear().envs(yemu_uutils_ctx::env_snapshot());
...
impl MultiExecMatcher {
	...
	fn run_command(&self, command: &mut argmax::Command, matcher_io: &mut MatcherIO) {
		...
		std_command.env_clear().envs(yemu_uutils_ctx::env_snapshot());
		match yemu_uutils_ctx::run_captured(&mut std_command) {
			Ok(status) => {
				if !status.success() {
