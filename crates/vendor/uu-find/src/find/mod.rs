...
use yemu_uutils_ctx::{stderr, stdout};
...
impl Default for Config {
	fn default() -> Self {
		Self {
			...
			today_start:       false,
			// Directory information and traversal are handled by yemu_walker,
...
fn walker_follow_links(follow: Follow) -> yemu_walker::FollowLinks {
	match follow {
		Follow::Never => yemu_walker::FollowLinks::Never,
		Follow::Roots => yemu_walker::FollowLinks::Roots,
		Follow::Always => yemu_walker::FollowLinks::Always,
	}
}

fn build_find_walk_request(config: &Config, root: &Path) -> yemu_walker::WalkRequest {
	yemu_walker::WalkRequest::new(root)
		...
		.visit_order(if config.depth_first {
			yemu_walker::VisitOrder::ContentsFirst
		} else {
			yemu_walker::VisitOrder::PreOrder
		})
...
fn process_dir_walk_request(
	...
	let status = request.for_each_entry_with_heartbeat(
		...
		|entry: yemu_walker::EntryMeta<'_>| {
			...
			if should_quit {
				...
				Ok(yemu_walker::WalkDecision::SkipDescend)
			} else {
	...
	match status {
		...
		Err(yemu_walker::WalkError::Interrupted(err)) => {
			ret_value = 1;
...
fn process_dir(
	...
	let resolved_root = yemu_uutils_ctx::resolve(dir);
...
/// yemu-uutils: BSD `find -E` compatibility (macOS muscle memory).
///
