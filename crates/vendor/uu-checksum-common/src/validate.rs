...
fn log_no_file_verified(filename: impl Display) {
	let _ =
		writeln!(yemu_uutils_ctx::stderr(), "{}: {filename}: no file was verified", command_name());
}
...
fn get_file_to_check(
	...
	if filename == "-" {
		Ok(Box::new(yemu_uutils_ctx::stdin())) // Use stdin if "-" is specified in the checksum file
	} else {
		let failed_open = || {
			write_file_report(
		...
		match File::open(yemu_uutils_ctx::resolve(filename)) {
			Ok(f) => {
				if f
					.metadata()
	...
	let file: Box<dyn Read> = if input_is_stdin {
		...
		match get_input_file(filename_input) {
			...
			Err(e) => {
				...
				let _ = writeln!(yemu_uutils_ctx::stderr(), "{}: {e}", command_name());
