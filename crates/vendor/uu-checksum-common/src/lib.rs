...
//
// yemu-uutils: vendored from uutils/coreutils 0.8.0 checksum_common and patched
...
pub(crate) fn report_error(error: &dyn std::fmt::Display) {
	let _ = writeln!(yemu_uutils_ctx::stderr(), "{}: {error}", command_name());
	yemu_uutils_ctx::set_exit_code(1);
}

pub(crate) fn report_warning(message: &str) {
	let _ = writeln!(yemu_uutils_ctx::stderr(), "{}: {message}", command_name());
}
...
fn finish_error(bin: &str, err: Box<dyn UError>) -> i32 {
	...
	if !message.is_empty() {
		let _ = writeln!(yemu_uutils_ctx::stderr(), "{bin}: {message}");
	}
