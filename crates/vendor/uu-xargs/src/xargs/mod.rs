...
impl CommandBuilder<'_> {
	...
	fn execute(self) -> Result<CommandResult, CommandExecutionError> {
		...
		if self.options.verbose {
			...
			let _ = writeln!(yemu_uutils_ctx::stderr(), "{line}");
		}
		...
		match &self.options.action {
			ExecAction::Command(_) => {
				...
				match yemu_uutils_ctx::run_captured(&mut command) {
					Ok(status) => {
						if status.success() {
...
fn do_xargs(args: &[&str]) -> Result<CommandResult, XargsError> {
	...
	let matches = match matches {
		...
		Err(e) => match e.kind() {
			ErrorKind::DisplayHelp | ErrorKind::DisplayVersion => {
				...
				// newline here.
				let _ = write!(yemu_uutils_ctx::stdout(), "{e}");
	...
	// process environment.
	let env: HashMap<OsString, OsString> = yemu_uutils_ctx::env_snapshot()
		.into_iter()
	...
	let args_file: Box<dyn Read> = if let Some(path) = &options.arg_file {
		Box::new(
			fs::File::open(yemu_uutils_ctx::resolve(path))
				.map_err(|e| format!("Failed to open {path}: {e}"))?,
		)
	} else {
		Box::new(yemu_uutils_ctx::stdin())
	};
