...
fn is_color_compatible_term() -> bool {
	let term = yemu_uutils_ctx::var("TERM").map(OsString::from);
	let colorterm = yemu_uutils_ctx::var("COLORTERM").map(OsString::from);
...
fn extract_quoting_style(
	...
	if let Some(style) = opt_quoting_style {
		...
		if let Some(style) = yemu_uutils_ctx::var("QUOTING_STYLE") {
...
fn parse_width(width_match: Option<&String>) -> Result<u16, LsError> {
	...
	let ret = match width_match {
		...
		None => match yemu_uutils_ctx::var("COLUMNS").map(OsString::from) {
...
impl Config {
	...
	pub fn from(options: &clap::ArgMatches) -> UResult<Self> {
		...
		let env_var_blocksize = yemu_uutils_ctx::var("BLOCKSIZE").map(OsString::from);
		let env_var_block_size = yemu_uutils_ctx::var("BLOCK_SIZE").map(OsString::from);
		let env_var_ls_block_size = yemu_uutils_ctx::var("LS_BLOCK_SIZE").map(OsString::from);
		let env_var_posixly_correct = yemu_uutils_ctx::var("POSIXLY_CORRECT").map(OsString::from);
		...
		// yemu-uutils: non-tty context, so SHOW_CONTROL_CHARS and the default both
		...
		let color = if needs_color {
			Some(match yemu_uutils_ctx::var("LS_COLORS") {
		...
		let tab_size = if needs_color {
			...
			options
				...
				.or_else(|| yemu_uutils_ctx::var("TABSIZE").and_then(|s| s.parse().ok()))
		}
...
fn parse_time_style(options: &clap::ArgMatches) -> Result<(String, Option<String>), LsError> {
	...
	if let Some(field) = options
		...
		.map(Cow::from)
		.or_else(|| yemu_uutils_ctx::var("TIME_STYLE").map(Cow::from))
	{
				...
				> options.indices_of(options::TIME_STYLE).unwrap().next_back()
		{
			...
			let field = if let Some(field) = field.strip_prefix("posix-") {
				...
				if yemu_uutils_ctx::var("LC_TIME").as_deref() == Some("POSIX")
					|| yemu_uutils_ctx::var("LC_ALL").as_deref() == Some("POSIX")
				{
