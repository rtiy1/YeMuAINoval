...
// are routed through `yemu_uutils_ctx`, `translate!` strings are literalized, and
...
// yemu-uutils: the `translate!` message templates are literalized with the
...
/// yemu-uutils: replacement for uucore's `show_error!` — writes the diagnostic
...
fn show_error(msg: impl std::fmt::Display) {
	...
	if !rendered.is_empty() {
		let _ = writeln!(yemu_uutils_ctx::stderr(), "ln: {rendered}");
	}
}
...
/// yemu-uutils: replacement for uucore's `prompt_yes!` — writes
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match ln_main(&matches) {
		...
		Err(err) => {
			let code = err.code();
			// yemu-uutils: `SomeLinksFailed` renders to an empty message
			// (upstream prints the per-file diagnostics as it goes); don't
			// emit a dangling "ln: " prefix for it.
			let msg = err.to_string();
			if !msg.is_empty() {
				let _ = writeln!(yemu_uutils_ctx::stderr(), "ln: {msg}");
			}
...
pub fn uu_app() -> Command {
	...
	Command::new("ln")
		...
		// yemu-uutils: free the `-h` short for the BSD `--no-dereference`
		...
		.arg(
			Arg::new(options::NO_DEREFERENCE)
				.short('n')
				// yemu-uutils: BSD/macOS ln spells this flag `-h` (`ln -sfh` is
...
fn exec(files: &[PathBuf], settings: &Settings) -> UResult<()> {
	...
	if !settings.no_target_dir {
		...
		// yemu-uutils: probe the destination via the resolved path.
		if files.len() > 2 || yemu_uutils_ctx::resolve(last_file).is_dir() {
	...
	if files.len() > 2 {
		// yemu-uutils: `uucore::execution_phrase()` reads the process argv,
...
fn link_files_in_dir(files: &[PathBuf], target_dir: &Path, settings: &Settings) -> UResult<()> {
	// yemu-uutils: resolved target directory for every syscall below; the
	// operand keeps its as-typed spelling for display and link-name building.
	let target_dir_fs = yemu_uutils_ctx::resolve(target_dir);
...
fn relative_path<'a>(src: &'a Path, dst: &Path) -> Cow<'a, Path> {
	// yemu-uutils: canonicalize from the resolved operands so `-r` computes the
	...
	if let Ok(src_abs) =
		canonicalize(yemu_uutils_ctx::resolve(src), MissingHandling::Missing, ResolveMode::Physical)
		&& let Ok(dst_abs) = canonicalize(
			yemu_uutils_ctx::resolve(dst.parent().unwrap()),
...
fn link(src: &Path, dst: &Path, settings: &Settings) -> UResult<()> {
	...
	// yemu-uutils: resolved counterparts of both operands for every filesystem
	...
	let src_fs = yemu_uutils_ctx::resolve(src);
	let dst_fs = yemu_uutils_ctx::resolve(dst);

	if dst_fs.is_symlink() || dst_fs.exists() {
		// yemu-uutils: probe numbered backups from the resolved destination so
	...
	let res: UResult<()> = if settings.symbolic {
		// yemu-uutils: the link is created at the resolved location, but its
		...
		// yemu-uutils: hard links dereference their target, so the resolved
		...
		match fs::hard_link(&p, &dst_fs) {
			...
			// yemu-uutils: same GNU-style rendering as the symlink arm (uucore
	...
	if settings.verbose {
		// yemu-uutils: verbose output goes to the context stdout.
		let mut out = yemu_uutils_ctx::stdout();
		write!(out, "{} -> {}", dst.quote(), source.quote())?;
		match backup_path {
			Some(path) => {
				// yemu-uutils: `path` derives from the resolved (absolute)
...
pub fn symlink<P1: AsRef<Path>, P2: AsRef<Path>>(src: P1, dst: P2) -> std::io::Result<()> {
	// yemu-uutils: the dir/file probe resolves the target against the shell
	// working directory (upstream consults the process cwd); the stored link
	// content is still the caller's as-typed `src`.
	if yemu_uutils_ctx::resolve(src.as_ref()).is_dir() {
		symlink_dir(src, dst)
	} else {
...
mod tests {
	...
	fn symlink_relative_operands_create_in_scope_cwd_with_literal_content() {
		...
		// the call-site `yemu_uutils_ctx::resolve` patch places the link in the
