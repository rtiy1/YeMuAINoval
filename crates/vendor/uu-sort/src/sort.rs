...
use yemu_uutils_ctx::{format_usage, rayon_global_pool_available};
...
// yemu-uutils: local stand-in for uucore's `show_error!`. Writes the
...
macro_rules! show_error {
    ($($args:tt)+) => {{
        let _ = writeln!(yemu_uutils_ctx::stderr(), "sort: {}", format_args!($($args)+));
    }};
}

// yemu-uutils: literalized (de-localized) strings formerly pulled from
...
// yemu-uutils: map the (formerly localized) field-spec parse message keys to
...
// yemu-uutils: slurp stdin fully on the calling thread (where the context is
...
impl Output {
	fn new(name: Option<impl AsRef<OsStr>>) -> UResult<Self> {
		let file = if let Some(name) = name {
			...
			let file = OpenOptions::new()
				.write(true)
				.create(true)
				.open(yemu_uutils_ctx::resolve(path))
	...
	fn into_write(self) -> BufWriter<Box<dyn Write>> {
		BufWriter::new(match self.file {
			...
			None => Box::new(yemu_uutils_ctx::stdout()),
		})
	}
...
/// all I/O through `yemu_uutils_ctx`, so it is safe inside the long-lived host
/// shell process.
pub fn run(args: Vec<OsString>) -> i32 {
	...
	match uu_sort(&matches, &legacy_warnings) {
		...
		Err(err) => {
			let code = err.code();
			// yemu-uutils: match uucore's entry point, which suppresses the
			// diagnostic when the error renders empty (e.g. `sort -C` on
			// disordered input with --check=silent).
			let rendered = err.to_string();
			if !rendered.is_empty() {
				let _ = writeln!(yemu_uutils_ctx::stderr(), "sort: {rendered}");
			}
...
fn uu_sort(matches: &ArgMatches, legacy_warnings: &[LegacyKeyWarning]) -> UResult<()> {
	...
	let mut tmp_dir = TmpDirWrapper::new(yemu_uutils_ctx::resolve(
	...
	for file in &files {
		// yemu-uutils: stdin is slurped on the calling thread when sorting; skip
...
fn sort_by<'a>(unsorted: &mut Vec<Line<'a>>, settings: &GlobalSettings, line_data: &LineData<'a>) {
	...
	// force sequential sort when yemu-native could not safely configure Rayon's
