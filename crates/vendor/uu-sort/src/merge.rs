...
fn replace_output_file_in_input_files(
	...
	if let Some(Ok(output_path)) = output.map(|path| yemu_uutils_ctx::resolve(path).canonicalize()) {
		for file in files {
			if let Ok(file_path) = yemu_uutils_ctx::resolve(file.as_os_str()).canonicalize()
