...
// `yemu_uutils_ctx::resolve` at the creation sites, so the printed path is the
// path actually created. TMPDIR/POSIXLY_CORRECT are read from the scope
// environment, all stdio goes through `yemu_uutils_ctx`, `translate!` strings are
...
impl Options {
	fn from(matches: &ArgMatches) -> Self {
		...
		let (tmpdir, template) = match matches.get_one::<OsString>(ARG_TEMPLATE) {
			...
			Some(template) => {
				// yemu-uutils: TMPDIR comes from the scope environment, not the
				// host process environment.
				let tmpdir = if let Some(tmpdir) = yemu_uutils_ctx::var(TMPDIR_ENV_VAR)
...
impl Params {
	fn from(options: Options) -> Result<Self, MkTempError> {
		...
		let (i, j) = match find_last_contiguous_block_of_xs(&template_str) {
			...
			// yemu-uutils: BSD `mktemp -t PREFIX` treats PREFIX as a name prefix,
...
pub fn run(argv: Vec<OsString>) -> i32 {
	let matches = match uu_app().try_get_matches_from(&argv) {
		Ok(matches) => matches,
		Err(err) => {
			// yemu-uutils: upstream maps a too-many-values clap error on the
			...
			if err.kind() == clap::error::ErrorKind::TooManyValues
				...
				let _ = writeln!(yemu_uutils_ctx::stderr(), "mktemp: too many templates");
				return 1;
			}
	...
	match mktemp_main(&argv, &matches) {
		...
		Err(err) => {
			let code = err.code();
			// yemu-uutils: --quiet failures surface as bare exit-code errors
			// that render to an empty message; don't emit a dangling
			// "mktemp: " prefix.
			let msg = err.to_string();
			if !msg.is_empty() {
				let _ = writeln!(yemu_uutils_ctx::stderr(), "mktemp: {msg}");
			}
...
fn mktemp_main(args: &[OsString], matches: &ArgMatches) -> UResult<()> {
	...
	// yemu-uutils: POSIXLY_CORRECT comes from the scope environment.
	if yemu_uutils_ctx::var("POSIXLY_CORRECT").is_some() {
	...
	// yemu-uutils: replacement for upstream's `println_verbatim` — writes the
...
fn dry_exec(tmpdir: &Path, prefix: &str, rand: usize, suffix: &str) -> PathBuf {
	// yemu-uutils: resolve the parent directory against the shell working
	// directory so the printed candidate matches where creation would occur.
	let tmpdir = yemu_uutils_ctx::resolve(tmpdir);
...
fn exec(dir: &Path, prefix: &str, rand: usize, suffix: &str, make_dir: bool) -> UResult<PathBuf> {
	...
	let dir = yemu_uutils_ctx::resolve(dir);
	...
	// yemu-uutils: unlike upstream (which re-joins the operand as typed), join
...
fn get_tmpdir_env_or_default() -> PathBuf {
	// yemu-uutils: read TMPDIR from the scope environment; when it is unset
	// there, fall back to the host default temp dir as upstream does.
	match yemu_uutils_ctx::var(TMPDIR_ENV_VAR) {
...
mod tests {
	...
	fn relative_tmpdir_resolves_against_scope_cwd() {
		...
		// the creation-site `yemu_uutils_ctx::resolve` patch makes this land in
	...
	fn bsd_t_prefix_creates_file_in_tmpdir() {
		...
		let (code, stdout, stderr) = run_in(root.clone(), tmpdir_env(&root), vec!["-t", "yemu"]);
		assert_eq!(code, 0);
		assert_eq!(stderr, "");
		let printed = PathBuf::from(stdout.trim_end_matches('\n'));
		assert!(printed.is_file(), "printed path {printed:?} must be a regular file");
		...
		assert!(name.starts_with("yemu."), "unexpected name {name}");
		assert_eq!(name.len(), "yemu.".len() + 10);
	}
