...
pub fn is_readable(path: &Path) -> bool {
	fs::metadata(yemu_uutils_ctx::resolve(path)).is_ok_and(|metadata| is_readable_metadata(&metadata))
}
...
pub fn safe_remove_file(
	...
	let dir_fd = DirFd::open(&yemu_uutils_ctx::resolve(parent), SymlinkBehavior::Follow).ok()?;
...
pub fn remove_dir_with_special_cases(path: &Path, options: &Options, error_occurred: bool) -> bool {
	match fs::remove_dir(yemu_uutils_ctx::resolve(path)) {
		...
		Err(_) if !error_occurred && yemu_uutils_ctx::resolve(path).read_dir().is_err() => {
...
pub fn safe_remove_dir_recursive(
	...
	let initial_mode = match fs::symlink_metadata(yemu_uutils_ctx::resolve(path)) {
	...
	let dir_fd = match DirFd::open(&yemu_uutils_ctx::resolve(path), SymlinkBehavior::Follow) {
		Ok(fd) => fd,
		Err(e) => {
			...
			if e.kind() == std::io::ErrorKind::PermissionDenied {
				...
				if fs::remove_dir(yemu_uutils_ctx::resolve(path)).is_ok() {
					verbose_removed_directory(path, options);
					return false;
				}
