...
// the shell working directory via `yemu_uutils_ctx::resolve` AT THE CALL SITE
// while the original operands are kept for display/error messages, stdio is
// routed through `yemu_uutils_ctx`, and the entry point no longer calls
...
// yemu-uutils: the upstream `set_to` field is gone with the set-date capability.
struct Settings {
	utc:         bool,
	format:      Format,
...
/// yemu-uutils: BSD `date` compatibility (macOS muscle memory).
///
		...
		match token {
			...
			"-r" => {
				if let Some(value) = toks.get(i + 1)
					...
					&& std::fs::symlink_metadata(yemu_uutils_ctx::resolve(Path::new(&argv[i + 1])))
...
pub fn run(argv: Vec<OsString>) -> i32 {
	// yemu-uutils: translate unambiguous BSD date forms before GNU clap parsing;
	// see `rewrite_bsd_invocation`.
	let argv = match rewrite_bsd_invocation(&argv) {
		None => argv,
		Some(Ok(rewritten)) => rewritten,
		Some(Err(msg)) => {
			let _ = writeln!(yemu_uutils_ctx::stderr(), "date: {msg}");
			return 1;
		},
	};
...
fn date_main(matches: &ArgMatches) -> UResult<()> {
	// yemu-uutils: the set-date capability is removed — a shell builtin must
	...
	// yemu-uutils: time-zone handling stays process-global — jiff reads the host
	...
	let dates: Box<dyn Iterator<Item = _>> = match &settings.date_source {
		...
		// yemu-uutils: `-f -` reads the context stdin, not the process stdin.
		...
		DateSource::File(path) => {
			// yemu-uutils: resolve the DATEFILE operand against the shell working
		...
		DateSource::FileMtime(path) => {
			// yemu-uutils: resolve the --reference FILE against the shell working
			// directory; `path` is kept for display.
			let metadata = std::fs::metadata(yemu_uutils_ctx::resolve(path))
			...
			// yemu-uutils: process-global TZ lookup, as upstream.
	...
	// yemu-uutils: buffered context stdout instead of the process stdout.
	let mut stdout = BufWriter::new(yemu_uutils_ctx::stdout());

	// Format all the dates
	let config = Config::new().custom(PosixCustom::new()).lenient(true);
	for date in dates {
		// yemu-uutils: a DATEFILE/stdin stream can be arbitrarily long; observe
		...
		match date {
			...
			Err((input, _err)) => {
				let _ = stdout.flush();
				// yemu-uutils: upstream `show!` — report the bad line to the
				// context stderr, record the failure exit code, and keep
				// processing the remaining lines.
				let _ = writeln!(yemu_uutils_ctx::stderr(), "date: invalid date '{input}'");
				yemu_uutils_ctx::set_exit_code(1);
			},
		}
	}
...
pub fn uu_app() -> Command {
	Command::new("date")
		...
		// yemu-uutils: the localized usage blob's FORMAT reference table moved to
		...
		.arg(
			Arg::new(OPT_SET)
				...
				.value_name("STRING")
				.allow_hyphen_values(true)
				// yemu-uutils: the set-date capability is removed; the option is
...
// yemu-uutils: literalized en-US FORMAT reference from the `date-usage` locale
...
/// yemu-uutils: upstream's `format_date_with_locale_aware_months` minus the
...
fn make_format_string(settings: &Settings) -> &str {
	match &settings.format {
		...
		// yemu-uutils: upstream derives the default format from the process
...
// yemu-uutils: `LazyLock` instead of upstream's `OnceLock` + `get_or_init`.
...
fn parse_date<S: AsRef<str> + Clone>(
	...
	if dbg_opts.debug {
		let _ = writeln!(yemu_uutils_ctx::stderr(), "date: input string: {input_str}");
	}

	// First, try to parse any timezone abbreviations
	if let Some(zoned) = try_parse_with_abbreviation(input_str, now) {
		if dbg_opts.debug {
			// yemu-uutils: context stderr instead of `stderr().lock()`.
...
// yemu-uutils: upstream's `convert_for_set` and the `set_system_datetime`
...
mod tests {
	...
	// --- yemu-uutils behavior contracts ---
