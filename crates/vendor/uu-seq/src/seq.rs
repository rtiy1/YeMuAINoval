...
// stdio is routed through `yemu_uutils_ctx` (the emission loops write to a
// `BufWriter` around the context stdout handle and poll
// `yemu_uutils_ctx::is_cancelled()` periodically, since seq can generate
...
/// yemu-uutils: how many emitted numbers to print between cancellation polls in
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match seq_main(&matches) {
		...
		Err(err) => {
			...
			if !msg.is_empty() {
				let _ = writeln!(yemu_uutils_ctx::stderr(), "seq: {msg}");
			}
...
fn seq_main(matches: &ArgMatches) -> UResult<()> {
	...
	match result {
		...
		Err(err) if err.kind() == std::io::ErrorKind::BrokenPipe => {
			...
			// yemu-uutils: the in-process builtin does not manipulate process
			...
			let _ = writeln!(yemu_uutils_ctx::stderr(), "seq: {err}");
			Ok(())
		},
...
fn fast_print_seq(
	...
	for i in 0..loop_cnt {
		// yemu-uutils: seq can generate effectively unbounded output; poll the
		// host cancel flag periodically so shell abort/timeout is observed.
		if i % CANCEL_POLL_INTERVAL == 0 && yemu_uutils_ctx::is_cancelled() {
			return Ok(());
		}
...
fn print_seq(
	...
	// yemu-uutils: buffer the context stdout handle instead of the (locked)
	// process stdout.
	...
	// yemu-uutils: iteration counter for periodic cancellation polling.
	let mut iterations: u64 = 0;
	while !done_printing(&value, &increment, &last) {
		// yemu-uutils: seq can generate effectively unbounded output; poll the
		// host cancel flag periodically so shell abort/timeout is observed.
		if iterations.is_multiple_of(CANCEL_POLL_INTERVAL) && yemu_uutils_ctx::is_cancelled() {
			return Ok(());
		}
		iterations += 1;
...
mod tests {
	...
	fn cancelled_scope_stops_emission() {
		// yemu-specific contract: a pre-cancelled scope aborts the (potentially
