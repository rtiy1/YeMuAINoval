...
// yemu-uutils: vendored from uutils/coreutils 0.8.0 and patched to run in-process
// as a shell builtin. Every filesystem syscall resolves its path operand
// against the shell working directory via `yemu_uutils_ctx::resolve` AT THE CALL
...
// `yemu_uutils_ctx`, `translate!` strings are literalized, and the entry point no
...
use yemu_uutils_ctx::format_usage;
...
/// yemu-uutils: replacement for uucore's `show!`/`show_if_err!`. Records the
...
fn show(err: &dyn UError) {
	yemu_uutils_ctx::set_exit_code(err.code());
	let _ = writeln!(yemu_uutils_ctx::stderr(), "mv: {err}");
}
...
fn run_matches(matches: &ArgMatches) -> UResult<()> {
	...
	if let Some(maybe_dir) = &target_dir {
		// yemu-uutils: resolve against the shell working directory before probing.
		if !yemu_uutils_ctx::resolve(Path::new(maybe_dir)).is_dir() {
...
/// In-process builtin entry point. The host installs a [`yemu_uutils_ctx`] scope
...
pub fn run(args: Vec<OsString>) -> i32 {
	let matches = match uu_app().try_get_matches_from(args) {
		...
		Err(err) => {
			...
			if err.use_stderr() {
				let _ = write!(yemu_uutils_ctx::stderr(), "{rendered}");
				return 1;
			}
			let _ = write!(yemu_uutils_ctx::stdout(), "{rendered}");
			return 0;
		},
	};

	// yemu-uutils: upstream renders this clap error then `process::exit`s; we
...
fn handle_two_paths(source: &Path, target: &Path, opts: &Options) -> UResult<()> {
	...
	// yemu-uutils: resolve operands against the shell working directory for the
	// filesystem probes; keep `source`/`target` for display and errors.
	let source_fs = yemu_uutils_ctx::resolve(source);
	let target_fs = yemu_uutils_ctx::resolve(target);
...
fn assert_not_same_file(
	...
	let source_abs = yemu_uutils_ctx::resolve(source);
	let target_abs = yemu_uutils_ctx::resolve(target);
...
fn move_files_into_dir(files: &[PathBuf], target_dir: &Path, options: &Options) -> UResult<()> {
	...
	// yemu-uutils: resolve against the shell working directory before probing.
	if !yemu_uutils_ctx::resolve(target_dir).is_dir() {
	...
	for sourcepath in files {
		// yemu-uutils: resolve for the existence probe; display the operand.
		if yemu_uutils_ctx::resolve(sourcepath)
			.symlink_metadata()
			.is_err()
		{
		...
		match rename(
			...
			Err(e) if e.to_string().is_empty() => yemu_uutils_ctx::set_exit_code(1),
			Err(e) => {
...
fn rename(
	...
	let from_fs = yemu_uutils_ctx::resolve(from);
	let to_fs = yemu_uutils_ctx::resolve(to);

	if to_fs.exists() {
		if opts.update == UpdateMode::None {
			if opts.debug {
				let _ = writeln!(yemu_uutils_ctx::stdout(), "skipped {}", to.quote());
			}
			return Ok(());
		}
		...
		// yemu-uutils: compute the backup path from the resolved target so
	...
	if let Some(context) = &opts.context {
		set_selinux_security_context(&yemu_uutils_ctx::resolve(to), Some(context))
			.map_err(|e| io::Error::other(e.to_string()))?;
	}

	if opts.verbose {
		let message = if let Some(path) = &backup_path {
			// yemu-uutils: `path` is derived from the resolved (absolute) target;
		...
		match display_manager {
			Some(pb) => pb.suspend(|| {
				let _ = writeln!(yemu_uutils_ctx::stdout(), "{message}");
			}),
			None => {
				let _ = writeln!(yemu_uutils_ctx::stdout(), "{message}");
			},
...
fn rename_fifo_fallback(from: &Path, to: &Path) -> io::Result<()> {
	...
	make_fifo(&to_fs).and_then(|_| fs::remove_file(yemu_uutils_ctx::resolve(from)))
}
...
fn rename_symlink_fallback(from: &Path, to: &Path) -> io::Result<()> {
	...
	let path_symlink_points_to = fs::read_link(yemu_uutils_ctx::resolve(from))?;
	unix::fs::symlink(path_symlink_points_to, yemu_uutils_ctx::resolve(to))?;
	#[cfg(not(any(target_os = "macos", target_os = "redox")))]
	{
		let _ = copy_xattrs_if_supported(from, to);
	}
	fs::remove_file(yemu_uutils_ctx::resolve(from))
}
...
fn rename_dir_fallback(
	...
	let total_size = dir_get_size(yemu_uutils_ctx::resolve(from)).ok();
	...
	#[cfg(all(unix, not(any(target_os = "macos", target_os = "redox"))))]
	let xattrs = fsxattr::retrieve_xattrs(yemu_uutils_ctx::resolve(from))
	...
	#[cfg(all(unix, not(any(target_os = "macos", target_os = "redox"))))]
	fsxattr::apply_xattrs(yemu_uutils_ctx::resolve(to), xattrs)?;

	result?;

	// Remove the source directory after successful copy
	fs::remove_dir_all(yemu_uutils_ctx::resolve(from))?;
...
fn copy_dir_contents(
	...
	fs::create_dir_all(yemu_uutils_ctx::resolve(to))?;
...
fn copy_dir_contents_recursive(
	...
	// yemu-uutils: resolve the directory for the read, but rebuild each child
	...
	let entries = fs::read_dir(yemu_uutils_ctx::resolve(from_dir))?;
	...
	for entry in entries {
		...
		if yemu_uutils_ctx::resolve(&from_path).is_symlink() {
		...
		} else if yemu_uutils_ctx::resolve(&from_path).is_dir() {
			// Recursively copy subdirectory (only real directories, not symlinks)
			fs::create_dir_all(yemu_uutils_ctx::resolve(&to_path))?;
			...
			#[cfg(not(unix))]
			{
				...
				fs::copy(yemu_uutils_ctx::resolve(&from_path), yemu_uutils_ctx::resolve(&to_path))?;
			}
		...
		if let Some(pb) = progress_bar
			&& let Ok(metadata) = yemu_uutils_ctx::resolve(&from_path).metadata()
		{
			pb.inc(metadata.len());
		}
	}
...
fn copy_file_with_hardlinks_helper(
	...
	{
		fs::hard_link(yemu_uutils_ctx::resolve(&existing_target), yemu_uutils_ctx::resolve(to))?;
		return Ok(());
	}

	if yemu_uutils_ctx::resolve(from).is_symlink() {
		// Copy a symlink file (no-follow).
		rename_symlink_fallback(from, to)?;
	} else if is_fifo(yemu_uutils_ctx::resolve(from).symlink_metadata()?.file_type()) {
		make_fifo(&yemu_uutils_ctx::resolve(to))?;
	} else {
		// Copy a regular file.
		fs::copy(yemu_uutils_ctx::resolve(from), yemu_uutils_ctx::resolve(to))?;
...
fn rename_file_fallback(
	...
	#[cfg(unix)]
	{
		if let (Some(tracker), Some(scanner)) = (hardlink_tracker, hardlink_scanner) {
			...
			if let Some(existing_target) = tracker.check_hardlink(from, to, scanner, &hardlink_options)
			{
				...
				fs::hard_link(yemu_uutils_ctx::resolve(&existing_target), &to_fs)?;
				fs::remove_file(yemu_uutils_ctx::resolve(from))?;
				return Ok(());
			}
		}
	...
	// Regular file copy
	fs::copy(yemu_uutils_ctx::resolve(from), &to_fs)
...
fn copy_xattrs_if_supported(from: &Path, to: &Path) -> io::Result<()> {
	match fsxattr::copy_xattrs(yemu_uutils_ctx::resolve(from), yemu_uutils_ctx::resolve(to)) {
		Ok(()) => Ok(()),
...
fn is_empty_dir(path: &Path) -> bool {
	fs::read_dir(yemu_uutils_ctx::resolve(path)).is_ok_and(|mut contents| contents.next().is_none())
}

/// Check if file is writable, returning the mode for potential reuse.
#[cfg(unix)]
fn is_writable(path: &Path) -> (bool, Option<u32>) {
	if let Ok(metadata) = yemu_uutils_ctx::resolve(path).metadata() {
...
/// yemu-uutils: replacement for uucore's `read_yes`, reading from the context
...
fn read_yes() -> bool {
	...
	let mut stdin = yemu_uutils_ctx::stdin();
	let mut buf = [0u8; 1];
	let mut first = None;
	loop {
...
/// yemu-uutils: the context stdin is a plain reader with no terminal concept, so
...
fn can_delete_file(path: &Path) -> bool {
	...
	let resolved = yemu_uutils_ctx::resolve(path);
