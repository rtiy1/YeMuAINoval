...
impl HardlinkTracker {
	...
	pub fn check_hardlink(
		...
		let metadata = match yemu_uutils_ctx::resolve(source).metadata() {
			Ok(meta) => meta,
			Err(e) => {
...
impl HardlinkGroupScanner {
	...
	pub fn scan_files(&mut self, files: &[PathBuf], options: &HardlinkOptions) {
		...
		for file in files {
				...
				&& options.verbose
			{
				...
				let _ =
					writeln!(yemu_uutils_ctx::stderr(), "warning: failed to scan {}: {e}", file.quote());
			}
	...
	fn scan_single_path(&mut self, path: &Path) -> io::Result<()> {
		...
		if yemu_uutils_ctx::resolve(path).is_dir() {
			// Recursively scan directory contents
			self.scan_directory_recursive(path)?;
		} else {
			let metadata = yemu_uutils_ctx::resolve(path).metadata()?;
			if metadata.nlink() > 1 {
				let key = (metadata.dev(), metadata.ino());
				self
					.hardlink_groups
	...
	fn scan_directory_recursive(&mut self, dir: &Path) -> io::Result<()> {
		...
		let entries = std::fs::read_dir(yemu_uutils_ctx::resolve(dir))?;
