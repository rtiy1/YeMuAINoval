...
// via `yemu_uutils_ctx::resolve` at the open/mmap call site (the original operand
...
// context stderr with `yemu_uutils_ctx::set_exit_code` (upstream `show!`), the
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match tac_main(&matches) {
		...
		Err(err) => {
			...
			if !msg.is_empty() {
				let _ = writeln!(yemu_uutils_ctx::stderr(), "tac: {msg}");
			}
...
/// yemu-uutils: replacement for upstream's `show!` — reports a recoverable
...
fn show(err: &TacError) {
	let _ = writeln!(yemu_uutils_ctx::stderr(), "tac: {err}");
...
fn buffer_tac_regex(
	...
	// yemu-uutils: write through the context stdout instead of the process stdout.
...
fn tac(filenames: &[OsString], before: bool, regex: bool, separator: &OsStr) -> UResult<()> {
	...
	for filename in filenames {
		...
		let data: &[u8] = if filename == "-" {
			...
			let path = yemu_uutils_ctx::resolve(filename);
		...
		// yemu-uutils: match ergonomics instead of upstream's `Some(ref pattern)`.
