...
/// yemu-uutils: BSD `tail -r` compatibility (macOS muscle memory).
///
...
fn rewrite_bsd_invocation(argv: &[OsString]) -> Option<Result<Vec<OsString>, String>> {
	...
	// yemu-uutils: `uu_tac` owns its clap command name and error prefix, so this
...
pub fn run(args: Vec<OsString>) -> i32 {
	// yemu-uutils: translate BSD-style `tail -r` before GNU clap parsing; see
	// `rewrite_bsd_invocation`.
	let args = match rewrite_bsd_invocation(&args) {
		None => args,
		Some(Ok(tac_args)) => return uu_tac::run(tac_args),
		Some(Err(msg)) => {
			let _ = writeln!(yemu_uutils_ctx::stderr(), "tail: {msg}");
			return 1;
		},
	};
	let settings = match parse_settings(args) {
		...
		Err(ArgsError::Other(err)) => {
			let code = err.code();
			let _ = writeln!(yemu_uutils_ctx::stderr(), "tail: {err}");
...
fn uu_tail(settings: &Settings) -> UResult<()> {
	...
	if settings.debug && settings.follow.is_some() {
		if observer.use_polling {
			let _ = writeln!(yemu_uutils_ctx::stderr(), "tail: using polling mode");
		} else {
			let _ = writeln!(yemu_uutils_ctx::stderr(), "tail: using notification mode");
		}
	}
...
fn tail_stdin(
	...
	// yemu-uutils: the context stdin is a plain reader with no backing file
	...
	// streaming (pipe) path.
	header_printer.print_input(input);
	let mut reader = BufReader::new(yemu_uutils_ctx::stdin());
	unbounded_tail(&mut reader, settings)?;
	Ok(())
}
...
fn unbounded_tail<T: Read>(reader: &mut BufReader<T>, settings: &Settings) -> UResult<()> {
	let mut writer = BufWriter::new(yemu_uutils_ctx::stdout().lock());
	match &settings.mode {
	...
	// yemu-uutils: upstream emulates Unix SIGPIPE on Windows by calling
	...
	// propagate to the caller, matching every other yemu-uutils builtin.
	writer.flush()?;
	Ok(())
}
	...
	R: Read + ?Sized,
{
	...
	let stdout = yemu_uutils_ctx::stdout();
...
mod tests {
	...
	fn run_in(cwd: PathBuf, args: Vec<&str>) -> (i32, String, String) {
		...
		let io = yemu_uutils_ctx::ScopeIo {
	...
	fn bounded_tail_broken_pipe_does_not_abort() {
		...
		let code = yemu_uutils_ctx::scope(io, || {
