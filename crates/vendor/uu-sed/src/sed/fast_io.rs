...
// Patched for yemu-uutils-ctx embedding: upstream's FastCopy (raw-fd metadata
...
impl<'a> LineReader<'a> {
	...
	pub fn open(path: &PathBuf) -> io::Result<Self> {
		if path.as_os_str() == "-" {
			// Patched for yemu-uutils-ctx embedding: read the context stdin.
			let boxed: Box<dyn Read> = Box::new(yemu_uutils_ctx::stdin());
			let reader = BufReader::new(boxed);
			return Ok(LineReader::ReadInput(ReadLineCursor::new(reader)));
		}

		// Patched for yemu-uutils-ctx embedding: input file operands resolve
...
// Patched for yemu-uutils-ctx embedding: output goes to plain `Write` handles
...
impl OutputBuffer {
	...
	#[cfg(unix)]
	pub fn new(w: Box<dyn OutputWrite + 'static>) -> Self {
		// Patched for yemu-uutils-ctx embedding: the writer is not fd-backed,
	...
	pub fn copy_file(&mut self, path: &PathBuf) -> io::Result<()> {
		...
		let Ok(file) = File::open(yemu_uutils_ctx::resolve(path)) else {
...
impl OutputBuffer {
	...
	// Patched for yemu-uutils-ctx embedding: the raw-fd write(2) and
