...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match exec(&buffer) {
		// yemu-uutils: a broken pipe means the consumer closed its end; a
		...
		ExecStop::Io(err) => {
			let _ = writeln!(yemu_uutils_ctx::stderr(), "yes: standard output: {}", strip_errno(&err));
			1
		},
		// yemu-uutils: the shell asked the scope to cancel (abort/timeout);
	...
	#[cfg(not(any(unix, target_os = "wasi")))]
	{
		for part in itertools::intersperse(i.map(|a| a.to_str()), Some(" ")) {
			let bytes = match part {
				...
				// yemu-uutils: literalized `translate!("yes-error-invalid-utf8")`.
...
/// yemu-uutils: why the output loop stopped. Upstream's `exec` only ever returns
...
/// yemu-uutils: replacement for upstream's `exec` — writes to the context stdout
...
fn exec(bytes: &[u8]) -> ExecStop {
	let mut stdout = yemu_uutils_ctx::stdout();
