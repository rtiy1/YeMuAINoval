...
// the no-argument dump iterates `yemu_uutils_ctx::env_snapshot()` and named
// lookups go through `yemu_uutils_ctx::var`, because the embedding shell's
...
fn printenv_main(matches: &ArgMatches) -> UResult<()> {
	...
	if variables.is_empty() {
		// yemu-uutils: replacement for `uucore::display::print_all_env_vars` —
		// dumps the scope environment map to the context stdout.
		let mut stdout = yemu_uutils_ctx::stdout();
		for (key, value) in yemu_uutils_ctx::env_snapshot() {
	...
	for env_var in variables {
		...
		// yemu-uutils: look the variable up in the scope environment (upstream
		// uses `std::env::var_os`) and write it to the context stdout.
		if let Some(var) = yemu_uutils_ctx::var(&env_var) {
			let mut stdout = yemu_uutils_ctx::stdout();
