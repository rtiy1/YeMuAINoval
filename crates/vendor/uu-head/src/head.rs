	...
	for file in &options.files {
		let res = if file == "-" {
			if (options.files.len() > 1 && !options.quiet) || options.verbose {
				if !first {
					let _ = writeln!(yemu_uutils_ctx::stdout());
				}
				let _ = writeln!(yemu_uutils_ctx::stdout(), "==> standard input <==");
			}
			// yemu-uutils: stdin is routed through the context as a plain reader,
			// so always take the streaming (non-seekable) path.
			let mut stdin = io::BufReader::with_capacity(BUF_SIZE, yemu_uutils_ctx::stdin());
			match options.mode {
			...
			let fs_path = yemu_uutils_ctx::resolve(Path::new(file));
			if fs_path.is_dir() {
				let _ = writeln!(
			...
			if (options.files.len() > 1 && !options.quiet) || options.verbose {
				...
				let _ = yemu_uutils_ctx::stdout().write_all(file.as_encoded_bytes());
				let _ = writeln!(yemu_uutils_ctx::stdout(), " <==");
			}
...
pub fn run(argv: Vec<OsString>) -> i32 {
	let args: Vec<OsString> = match arg_iterate(argv.into_iter()) {
		...
		Err(err) => {
			let _ = writeln!(yemu_uutils_ctx::stderr(), "head: {err}");
			return 1;
		},
	};
	...
	let options = match HeadOptions::get_from(&matches) {
		Ok(options) => options,
		Err(err) => {
			let _ = writeln!(yemu_uutils_ctx::stderr(), "head: {}", HeadError::MatchOption(err));
			return 1;
		},
	};
