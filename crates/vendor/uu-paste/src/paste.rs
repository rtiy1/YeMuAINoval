...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match paste(files, serial, delimiters, line_ending) {
		...
		Err(err) => {
			let code = err.code();
			let _ = writeln!(yemu_uutils_ctx::stderr(), "paste: {err}");
...
pub fn uu_app() -> Command {
	Command::new("paste")
		...
		.override_usage(yemu_uutils_ctx::format_usage("paste [OPTION]... [FILE]..."))
...
fn paste(
	...
	// yemu-uutils: all `-` operands share the scoped stdin and consume it in order.
	let stdin = Rc::new(RefCell::new(BufReader::new(yemu_uutils_ctx::stdin())));
	let mut sources = Vec::with_capacity(filenames.len());
	for filename in filenames {
		if filename == "-" {
			sources.push(InputSource::StandardInput(stdin.clone()));
		} else {
			// yemu-uutils: resolve filesystem access against shell cwd, while retaining
			// the user's spelling in diagnostics.
			let file = File::open(yemu_uutils_ctx::resolve(&filename)).map_err(|err| {
...
enum InputSource {
	File(BufReader<File>),
	StandardInput(Rc<RefCell<BufReader<yemu_uutils_ctx::CtxStdin>>>),
}
