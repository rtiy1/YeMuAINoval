...
// yemu-uutils: route standard streams through the invocation context, and
		...
		if filename == "-" {
			...
			let result = match mode {
				Mode::Bytes(ranges, opts) | Mode::Characters(ranges, opts) => {
					cut_bytes(yemu_uutils_ctx::stdin(), &mut out, ranges, opts)
				},
				Mode::Fields(ranges, opts) => {
					cut_fields(yemu_uutils_ctx::stdin(), &mut out, ranges, opts)
				},
			};
			if let Err(err) = result {
				let _ = writeln!(yemu_uutils_ctx::stderr(), "cut: {err}");
				yemu_uutils_ctx::set_exit_code(1);
			}
			stdin_read = true;
		} else {
			let result = File::open(yemu_uutils_ctx::resolve(Path::new(filename)))
...
// yemu-uutils: replace the terminating uucore entry macro and localization-aware
