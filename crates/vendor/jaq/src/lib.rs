...
//! in-process as a shell builtin via [`yemu_uutils_ctx`].
//!
...
//! - stdio goes through the [`yemu_uutils_ctx`] streams; every file path operand
//!   resolves through `yemu_uutils_ctx::resolve` against the shell's cwd.
...
pub fn run(argv: Vec<std::ffi::OsString>) -> i32 {
	...
	let cli = match Cli::parse(argv) {
		Ok(cli) => cli,
		Err(e) => {
			let _ = writeln!(yemu_uutils_ctx::stderr(), "Error: {e}");
			return 2;
		},
	};
	...
	if cli.version {
		...
		let _ = writeln!(yemu_uutils_ctx::stdout(), "{}", include_str!("help.txt"));
		return 0;
	}
	...
	match res {
		...
		Err(e) => {
			...
			let _ = write!(yemu_uutils_ctx::stderr(), "{e}");
			e.report()
		},
	}
}
...
fn real_main(cli: &Cli) -> Result<i32, Error> {
	if let Some(test_files) = &cli.run_tests {
		return Ok(match test_files.last() {
			Some(file) => {
				run_tests(io::BufReader::new(std::fs::File::open(yemu_uutils_ctx::resolve(file))?))
			},
			None => run_tests(io::BufReader::new(yemu_uutils_ctx::stdin())),
		});
	}
		...
		Some(filter) => {
			let (path, code) = match filter {
				cli::Filter::FromFile(path) => {
					(path.into(), std::fs::read_to_string(yemu_uutils_ctx::resolve(path))?)
				},
	...
	let last = if cli.files.is_empty() {
		let inputs = read::buffered(cli, io::BufReader::new(yemu_uutils_ctx::stdin()));
...
fn binds(cli: &Cli) -> Result<Vec<(String, Val)>, Error> {
	...
	let rawfile = cli.rawfile.iter().map(|(k, path)| {
		let s = std::fs::read_to_string(yemu_uutils_ctx::resolve(path))
	...
	let env = yemu_uutils_ctx::env_snapshot()
		.into_iter()
...
fn run_tests(read: impl BufRead) -> i32 {
	...
	for test in tests {
		...
		let _ = writeln!(yemu_uutils_ctx::stdout(), "Testing {}", test.filter);
		match run_test(test) {
			Err(e) => {
				let _ = writeln!(yemu_uutils_ctx::stderr(), "{e:?}");
			},
			Ok((expect, obtain)) if expect != obtain => {
				let _ = writeln!(yemu_uutils_ctx::stderr(), "expected {expect}, obtained {obtain}",);
			},
			Ok(_) => passed += 1,
		}
		total += 1;
	...
	let _ = writeln!(yemu_uutils_ctx::stdout(), "{passed} out of {total} tests passed");
