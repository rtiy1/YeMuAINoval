...
// `yemu_uutils_ctx::resolve` AT THE CALL SITE, while the original operands are
// kept for display/error messages and `%n` output (GNU prints operands as
// typed). All process-global stdio is routed through `yemu_uutils_ctx`,
...
/// yemu-uutils: stub for exotic targets (wasm, etc.) that are neither Unix nor
/// Windows — upstream stat cannot be built there.
#[cfg(not(any(unix, windows)))]
pub fn run(_argv: Vec<std::ffi::OsString>) -> i32 {
	use std::io::Write;
	let _ = writeln!(yemu_uutils_ctx::stderr(), "stat: unsupported on this platform");
	1
}

/// yemu-uutils: minimal counterpart of the real `uu_app` for exotic targets.
#[cfg(not(any(unix, windows)))]
pub fn uu_app() -> clap::Command {
	clap::Command::new("stat")
		.version(uucore::crate_version!())
		.about("Display file or file system status.")
		.override_usage(yemu_uutils_ctx::format_usage("stat [OPTION]... FILE..."))
}
...
mod imp {
	...
	// yemu-uutils: literalized from locales/en-US.ftl (`stat-after-help`).
...
for details about the options it supports.";
	...
	// yemu-uutils: `translate!` error strings literalized from locales/en-US.ftl.
	#[derive(Debug, Error)]
	enum StatError {
	...
	fn pad_and_print(result: &str, left: bool, width: usize, padding: Padding) {
		// yemu-uutils: write to the context stdout instead of `print!`.
		let mut out = yemu_uutils_ctx::stdout();
	...
	// yemu-uutils: raw-byte output is only reachable via `%m` (OsStr) on Unix.
	#[cfg(unix)]
	...
	fn print_it(output: &OutputType, flags: Flags, width: usize, precision: Precision) {
		...
		match output {
			...
			// yemu-uutils: context stdout instead of `print!`.
			OutputType::Unknown => {
				let _ = write!(yemu_uutils_ctx::stdout(), "?");
			},
		}
	}
	...
	fn print_os_str(s: &OsString, flags: Flags, width: usize, precision: Precision) {
		// yemu-uutils: on Unix, preserve non-UTF8 operands by printing raw bytes
		...
		#[cfg(unix)]
		{
			...
			if pad_and_print_bytes(yemu_uutils_ctx::stdout(), bytes, flags.left, width, precision)
				.is_err()
			{
	...
	fn get_quoted_file_name(
		display_name: &str,
		// yemu-uutils: takes the operand resolved against the shell working
		// directory for the `readlink` syscall; `display_name` stays as typed.
		resolved: &Path,
		file_type: FileType,
		from_user: bool,
	) -> Result<String, i32> {
		// yemu-uutils: QUOTING_STYLE comes from the scope environment (the
		// shell's exported variables), not the host process environment.
		let quoting_style = yemu_uutils_ctx::var("QUOTING_STYLE")
		...
		if file_type.is_symlink() {
			...
			match fs::read_link(resolved) {
				...
				Err(e) => {
					// yemu-uutils: `show_error!` replaced with a context-stderr write.
					let _ = writeln!(yemu_uutils_ctx::stderr(), "stat: {e}");
					Err(1)
				},
			}
		} else {
	...
	fn process_token_filesystem(t: &Token, meta: &StatFs, display_name: &str) {
		match *t {
			...
			// yemu-uutils: context stdout instead of `print!`.
			Token::Char(c) => {
				let _ = write!(yemu_uutils_ctx::stdout(), "{c}");
			},
	...
	fn write_raw_byte(byte: u8) {
		// yemu-uutils: context stdout instead of the process stdout, and no
		// `unwrap` — an in-process builtin must not panic on a broken pipe.
		let _ = yemu_uutils_ctx::stdout().write_all(&[byte]);
	}
	...
	impl Stater {
		...
		fn handle_escape_sequences(
			...
			*i += 1;
			if *i >= bound {
				// yemu-uutils: `show_warning!` replaced with a context-stderr
				// write; message literalized from locales/en-US.ftl.
				let _ = writeln!(yemu_uutils_ctx::stderr(), "stat: warning: backslash at end of format");
				return Token::Char('\\');
			}
			match chars[*i] {
					...
					if *i + 1 < bound {
						...
						if let Some((c, offset)) = format_str[byte_index..].scan_char(16) {
							*i += offset;
							...
							// yemu-uutils: `show_warning!` replaced with a
							// context-stderr write.
							let _ = writeln!(
								yemu_uutils_ctx::stderr(),
			...
			match *t {
				...
				Token::Directive { flag, width, precision, format } => {
					let output = match format {
						...
						// yemu-uutils: SELinux support is dropped; this is
		...
		fn do_stat(&self, file: &OsStr, stdin_is_fifo: bool) -> i32 {
			...
			let file = if display_name == "-" {
				if self.show_fs {
					// yemu-uutils: `show_error!` replaced with a context-stderr
					// write.
					let _ =
						writeln!(yemu_uutils_ctx::stderr(), "stat: {}", StatError::StdinFilesystemMode);
					return 1;
				}
			...
			// yemu-uutils: resolve the operand against the shell working
			// directory for every syscall below; `display_name` keeps the
			// operand as typed for `%n` and error messages.
			let resolved = yemu_uutils_ctx::resolve(&file);
			if self.show_fs {
				match statfs(resolved.as_os_str()) {
					...
					Err(error) => {
						// yemu-uutils: `show_error!` replaced with a
						// context-stderr write.
						let _ = writeln!(
							yemu_uutils_ctx::stderr(),
							"stat: {}",
				...
				match result {
					...
					Err(e) => {
						// yemu-uutils: `show_error!` replaced with a
						// context-stderr write.
						let _ = writeln!(yemu_uutils_ctx::stderr(), "stat: {}", StatError::CannotStat {
							file:  display_name.quote().to_string(),
							error: e.to_string(),
						});
						return 1;
		...
		fn default_format(show_fs: bool, terse: bool, show_dev_type: bool) -> String {
			...
			// yemu-uutils: `translate!` word lookups literalized from
			// locales/en-US.ftl.
	...
	/// yemu-uutils: BSD `stat -f FORMAT` compatibility (macOS muscle memory).
	///
	...
	pub fn run(argv: Vec<OsString>) -> i32 {
		...
		match stat_main(&matches) {
			...
			Err(err) => {
				let code = err.code();
				// yemu-uutils: `do_stat` already reports its errors to the
	...
	/// yemu-uutils: Windows-native metadata helpers for `stat`. Upstream's
	...
	/// yemu-uutils: Windows counterpart of `pretty_time`, formatting a
	...
	/// yemu-uutils: Windows counterpart of `process_token_filesystem`.
	#[cfg(windows)]
...
mod tests {
	...
	fn run_in(cwd: PathBuf, args: Vec<&str>) -> (i32, String, String) {
		...
		let code = yemu_uutils_ctx::scope(io, || run(argv));
	...
	fn resolves_relative_operand_against_scope_cwd() {
		...
		// call-site `yemu_uutils_ctx::resolve` patch makes this find the file.
