...
// yemu-uutils: the AsFd bound existed only for the splice fast path and the
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match cat_main(&matches) {
		...
		Err(err) => {
			let code = err.code();
			let _ = writeln!(yemu_uutils_ctx::stderr(), "cat: {err}");
...
fn cat_path(path: &OsString, options: &OutputOptions, state: &mut OutputState) -> CatResult<()> {
	match get_input_type(path)? {
		InputType::StdIn => {
			// yemu-uutils: stdin is a context reader with no backing fd, so the
			...
			let mut handle =
				InputHandle { reader: yemu_uutils_ctx::stdin(), is_interactive: false };
		...
		_ => {
			...
			let file = File::open(yemu_uutils_ctx::resolve(path))?;
	...
	I: IntoIterator<Item = &'a OsString>,
{
	...
	if state.skipped_carriage_return {
		let _ = write!(yemu_uutils_ctx::stdout(), "\r");
	}
...
fn get_input_type(path: &OsString) -> CatResult<InputType> {
	...
	let ft = match metadata(yemu_uutils_ctx::resolve(path)) {
		Ok(md) => md.file_type(),
		Err(e) => {
...
fn write_fast<R: FdReadable>(handle: &mut InputHandle<R>) -> CatResult<()> {
	// yemu-uutils: the Linux/Android splice() fast path needs real fds for both
...
fn handle_broken_pipe(_error: &io::Error) {
	// yemu-uutils: an in-process builtin must never call std::process::exit; a
