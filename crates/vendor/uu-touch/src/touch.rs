...
// `yemu_uutils_ctx`, `translate!` strings are literalized, `_POSIX2_VERSION` is
// read from the scope environment, `show!` accumulation goes through
// `yemu_uutils_ctx::set_exit_code`, and the entry point no longer calls
...
// yemu-uutils: upstream `src/error.rs`, inlined so the vendored crate is a
...
fn is_first_filename_timestamp(
	...
	timestamp.is_none()
		...
		&& date.is_none()
		&& files.len() >= 2
		// yemu-uutils: `_POSIX2_VERSION` comes from the scope environment (the
		// shell's exported variables), not the host process environment.
		// env check is last as the slowest op
		&& yemu_uutils_ctx::var("_POSIX2_VERSION").as_deref() == Some("199209")
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match touch_main(&matches) {
		...
		Err(err) => {
			...
			if !msg.is_empty() {
				let _ = writeln!(yemu_uutils_ctx::stderr(), "touch: {msg}");
			}
...
fn touch_main(matches: &ArgMatches) -> UResult<()> {
	let mut filenames: Vec<&OsString> = matches
		...
		.ok_or_else(|| {
			// yemu-uutils: literalized; `uucore::execution_phrase()` is "touch"
...
pub fn touch(files: &[InputFile], opts: &Options) -> Result<(), TouchError> {
	let (atime, mtime) = match &opts.source {
		Source::Reference(reference) => {
			// yemu-uutils: resolve the reference operand against the shell
			// working directory at the syscall site; the original operand is
			// kept for the error message.
			let (atime, mtime) = stat(&yemu_uutils_ctx::resolve(reference), !opts.no_deref)
...
fn touch_file(
	...
	// yemu-uutils: resolve the operand against the shell working directory for
	...
	if let Err(e) = metadata_result {
		...
		if opts.no_deref {
			...
			// yemu-uutils: upstream `show!` — print the error and accumulate the
			// exit code in the scope instead of process-global state.
			let _ = writeln!(yemu_uutils_ctx::stderr(), "touch: {e}");
			yemu_uutils_ctx::set_exit_code(e.code());
			return Ok(());
		}
		...
		if let Err(e) = File::create(&resolved) {
			...
			// yemu-uutils: upstream `show!` — see above.
			let _ = writeln!(yemu_uutils_ctx::stderr(), "touch: {e}");
			yemu_uutils_ctx::set_exit_code(e.code());
			return Ok(());
		}
...
fn parse_date(ref_zoned: Zoned, s: &str) -> Result<FileTime, TouchError> {
	...
	// ("%F", ISO_8601_FORMAT),
	// yemu-uutils: `TimeZone::system()` (and the `TZ` variable it consults)
...
fn parse_timestamp(s: &str) -> UResult<FileTime> {
	...
	// yemu-uutils: `TimeZone::system()` intentionally stays process-global.
...
fn pathbuf_from_stdout() -> Result<PathBuf, TouchError> {
	...
	#[cfg(windows)]
	{
		...
		// yemu-uutils: literalized error strings; the variant's Display supplies
...
mod tests {
	...
	fn relative_operand_creates_file_in_scope_cwd() {
		...
		// the call-site `yemu_uutils_ctx::resolve` patch makes the file land in
