...
// as a shell builtin. The YEMU_NUM_THREADS and YEMU_THREAD_LIMIT environment
// variables are read from the scope environment via `yemu_uutils_ctx::var` (the
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match nproc_main(&matches) {
		...
		Err(err) => {
			...
			if !msg.is_empty() {
				let _ = writeln!(yemu_uutils_ctx::stderr(), "nproc: {msg}");
			}
...
fn nproc_main(matches: &ArgMatches) -> UResult<()> {
		...
		Some(numstr) => match numstr.trim().parse::<usize>() {
			...
			Err(e) => {
				return Err(USimpleError::new(
					1,
					// yemu-uutils: literalized translate!("nproc-error-invalid-number")
	...
	let mut cores = if matches.get_flag(OPT_ALL) {
		...
		// YEMU_NUM_THREADS doesn't have an impact on --all
		// yemu-uutils: YEMU_NUM_THREADS comes from the scope environment.
		match yemu_uutils_ctx::var("YEMU_NUM_THREADS") {
			// Uses the OpenMP variable to force the number of threads
			// If the parsing fails, returns the number of CPU
			Some(threads) => {
				// In some cases, YEMU_NUM_THREADS can be "x,y,z"
				// In this case, only take the first one (like GNU)
				// If YEMU_NUM_THREADS=0, rejects the value
			...
			// the variable 'YEMU_NUM_THREADS' doesn't exist
	...
	// yemu-uutils: write to the context stdout instead of the process stdout.
	yemu_uutils_ctx::stdout()
...
mod tests {
	...
	#[test]
	fn scope_env_yemu_num_threads_forces_count() {
		let env = HashMap::from([("YEMU_NUM_THREADS".to_string(), "3".to_string())]);
		let (code, stdout, stderr) = run_in(env, vec![]);
		assert_eq!((code, stdout.as_str(), stderr.as_str()), (0, "3\n", ""));
	}

	#[test]
	fn yemu_thread_limit_caps_yemu_num_threads() {
		let env = HashMap::from([
			("YEMU_NUM_THREADS".to_string(), "64".to_string()),
			("YEMU_THREAD_LIMIT".to_string(), "2".to_string()),
		]);
		let (code, stdout, stderr) = run_in(env, vec![]);
	...
	#[test]
	fn all_prints_positive_integer_and_ignores_yemu_num_threads() {
		// --all reports hardware CPUs; YEMU_NUM_THREADS must not force it.
		let env = HashMap::from([("YEMU_NUM_THREADS".to_string(), "0".to_string())]);
	...
	fn process_environment_is_not_consulted() {
		...
		unsafe { std::env::set_var("YEMU_NUM_THREADS", "1234") };
		let (code, stdout, stderr) = run_in(HashMap::new(), vec![]);
		unsafe { std::env::remove_var("YEMU_NUM_THREADS") };
	...
	fn ignore_subtracts_and_floors_at_one() {
		let env = HashMap::from([("YEMU_NUM_THREADS".to_string(), "8".to_string())]);
		let (code, stdout, _) = run_in(env, vec!["--ignore=3"]);
		assert_eq!((code, stdout.as_str()), (0, "5\n"));

		let env = HashMap::from([("YEMU_NUM_THREADS".to_string(), "2".to_string())]);
