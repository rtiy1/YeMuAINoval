...
// `yemu_uutils_ctx`, `translate!` strings are literalized, per-file errors are
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match truncate_main(&matches) {
		...
		Err(err) => {
			let code = err.code();
			// yemu-uutils: don't emit a dangling "truncate: " prefix when the
			// error renders to an empty message.
			let msg = err.to_string();
			if !msg.is_empty() {
				let _ = writeln!(yemu_uutils_ctx::stderr(), "truncate: {msg}");
			}
...
fn do_file_truncate(filename: &OsString, create: bool, size: u64) -> UResult<()> {
	// yemu-uutils: resolve the operand against the shell working directory at
	// the open site; `filename` is kept for the error message.
	let resolved = yemu_uutils_ctx::resolve(filename);
...
fn truncate(
	...
	let reference_size = match reference {
		Some(reference_path) => {
			...
			let reference_metadata =
				metadata(yemu_uutils_ctx::resolve(&reference_path)).map_err(|error| {
					match error.kind() {
	...
	for filename in filenames {
		// yemu-uutils: upstream aborts on the first failing file; report the
		...
		if let Err(err) = file_truncate(no_create, reference_size, &mode, filename) {
			...
			yemu_uutils_ctx::set_exit_code(if err.code() == 0 { 1 } else { err.code() });
		}
	}
