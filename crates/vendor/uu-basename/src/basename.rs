...
// yemu-uutils: Patched for in-process embedding in the shell.
// All I/O is routed through thread-local stream buffers provided by
// `yemu-uutils-ctx`. Command-line arguments are parsed and errors are mapped
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match basename_main(&matches) {
		...
		Err(err) => {
			let code = err.code();
			let _ = writeln!(yemu_uutils_ctx::stderr(), "basename: {err}");
