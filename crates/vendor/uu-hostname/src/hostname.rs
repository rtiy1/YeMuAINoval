...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match hostname_main(&matches) {
		...
		Err(err) => {
			...
			if !msg.is_empty() {
				let _ = writeln!(yemu_uutils_ctx::stderr(), "hostname: {msg}");
			}
...
fn hostname_main(matches: &ArgMatches) -> UResult<()> {
	...
	match matches.get_one::<OsString>(OPT_HOST) {
		...
		// yemu-uutils: setting the process-global hostname from inside a shell
...
fn display_hostname(matches: &ArgMatches) -> UResult<()> {
	...
	// yemu-uutils: all output below goes to the context stdout instead of the
	// process stdout.
