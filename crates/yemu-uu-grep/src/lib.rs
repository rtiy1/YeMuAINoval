...
//! with directory recursion via `yemu-walker` and `--include` filtering via
//! `globset`. All I/O and path resolution is routed through `yemu-uutils-ctx` so
...
#[command(
	name = "grep",
	version = concat!("grep (yemu-uu-grep) ", env!("CARGO_PKG_VERSION")),
...
fn resolve_follow_links(cli: &Cli, matches: &ArgMatches) -> yemu_walker::FollowLinks {
	let mut selected = (0, yemu_walker::FollowLinks::Roots);
	choose_latest(&mut selected, last_index(matches, "recursive"), yemu_walker::FollowLinks::Roots);
	choose_latest(
		&mut selected,
		last_index(matches, "dereference_recursive"),
		yemu_walker::FollowLinks::Always,
	);
	if cli.directories == Some(DirectoryAction::Recurse) {
		choose_latest(
			&mut selected,
			last_index(matches, "directories"),
			yemu_walker::FollowLinks::Roots,
		);
	}
	choose_latest(
		&mut selected,
	...
	choose_latest(&mut selected, last_index(matches, "no_follow"), yemu_walker::FollowLinks::Never);
	choose_latest(&mut selected, last_index(matches, "follow_all"), yemu_walker::FollowLinks::Always);
	selected.1
}
...
fn grep_walk_request(root: &Path, follow_links: yemu_walker::FollowLinks) -> yemu_walker::WalkRequest {
	yemu_walker::WalkRequest::new(root)
		.hidden(true)
		.gitignore(false)
		.skip_git(false)
		.skip_node_modules(false)
		.follow_links(follow_links)
		.detail(yemu_walker::WalkDetail::Minimal)
		.order(yemu_walker::WalkOrder::Unordered)
		.emit_root(true)
		.depth(0, usize::MAX)
		.visit_order(yemu_walker::VisitOrder::PreOrder)
		.directory_errors(yemu_walker::DirectoryErrorMode::Visit)
		.same_file_system(false)
		.cache(false)
		.filter(yemu_walker::WalkFilter::all())
}
...
fn search_dir<M: Matcher, W: Write>(
	...
	opts: &Options,
	rules: &PathRules,
	follow_links: yemu_walker::FollowLinks,
	out: &mut W,
	had_error: &mut bool,
) -> bool {
	...
	let walk = request.for_each_entry_with_heartbeat(
		|| {
			if yemu_uutils_ctx::is_cancelled() {
				Err(io::Error::from(io::ErrorKind::Interrupted))
			} else {
				Ok::<(), io::Error>(())
			}
		},
		|entry: yemu_walker::EntryMeta<'_>| {
			if opts.quiet && any {
				return Ok(yemu_walker::WalkDecision::Stop);
			}
			if entry.file_type == yemu_walker::FileType::Dir {
				if entry.depth > 0 && !rules.allows_dir(Path::new(entry.relative_path)) {
					return Ok(yemu_walker::WalkDecision::SkipDescend);
				}
				return Ok(yemu_walker::WalkDecision::Include);
			}
			if entry.file_type != yemu_walker::FileType::File
				|| !rules.allows_file(Path::new(entry.relative_path))
			{
				return Ok(yemu_walker::WalkDecision::Skip);
			}
			...
			any |= matched;
			if opts.quiet && any {
				Ok(yemu_walker::WalkDecision::Stop)
			} else {
				Ok(yemu_walker::WalkDecision::Include)
			}
		},
		|error: yemu_walker::DirectoryError<'_>| {
	...
	*had_error |= had_error_state.get();
	match walk {
		Ok(yemu_walker::WalkStatus::Complete | yemu_walker::WalkStatus::Stopped) => any,
		Err(yemu_walker::WalkError::Interrupted(_)) if yemu_uutils_ctx::is_cancelled() => {
			// The shell wrapper owns the user-visible cancellation status.
			*had_error = true;
			any
		},
		Err(yemu_walker::WalkError::Interrupted(error)) => {
			*had_error = true;
			if !opts.no_messages {
				let _ = writeln!(yemu_uutils_ctx::stderr(), "grep: {error}");
			}
			any
		},
		Err(yemu_walker::WalkError::InvalidData { path, message }) => {
			*had_error = true;
			if !opts.no_messages {
				let display_path = display_path_for_operand(operand, resolved, &path);
				let _ = writeln!(
...
fn read_auxiliary_file(path: &OsStr) -> Result<Vec<u8>, String> {
	...
	let result = if path == OsStr::new("-") {
		yemu_uutils_ctx::stdin().read_to_end(&mut bytes)
	} else {
		File::open(yemu_uutils_ctx::resolve(path)).and_then(|mut file| file.read_to_end(&mut bytes))
	};
	result
		.map(|_| bytes)
...
fn execute_search<M: Matcher>(
	...
	let mut out = BufWriter::new(yemu_uutils_ctx::stdout());
	...
	for operand in files {
		...
		if processed_operand && yemu_uutils_ctx::is_cancelled() {
			had_error = true;
			break;
		}
		...
		if operand == OsStr::new("-") {
			...
			match process_reader(
				...
				Err(error) => {
					had_error = true;
					if !opts.no_messages {
						let _ = writeln!(yemu_uutils_ctx::stderr(), "grep: (standard input): {error}");
					}
				},
			}
			if yemu_uutils_ctx::is_cancelled() {
		...
		let resolved = yemu_uutils_ctx::resolve(operand);
		match std::fs::metadata(&resolved) {
			...
			Err(error) => {
				...
				if !opts.no_messages {
					let _ =
						writeln!(yemu_uutils_ctx::stderr(), "grep: {}: {error}", operand.to_string_lossy());
				}
			},
		}
		if yemu_uutils_ctx::is_cancelled() {
...
mod tests {
	...
	use yemu_uutils_ctx::{ScopeIo, scope};
	...
	fn version_flag_prints_and_exits_zero() {
		...
		assert!(
			stdout.contains("grep") && stdout.contains("yemu-uu-grep"),
	...
	fn recursive_search_observes_scope_cancellation() {
		...
		// yemu_walker, so directory walks ignored the uutils cancel flag and the
