...
async fn create_session_for_run(
	...
	// disabled (falling back to system binaries) via YEMU_DISABLE_UUTILS_BUILTINS;
	// the destructive pair additionally honors YEMU_DISABLE_UUTILS_DESTRUCTIVE.
	if !uutils_env_disabled(config, "YEMU_DISABLE_UUTILS_BUILTINS") {
		...
		if !uutils_env_disabled(config, "YEMU_DISABLE_UUTILS_DESTRUCTIVE") {
			if !uutils_env_disabled(config, "YEMU_DISABLE_RM_BUILTIN") {
				shell.register_builtin("rm", crate::coreutils::rm_builtin());
			}
			if !uutils_env_disabled(config, "YEMU_DISABLE_MV_BUILTIN") {
	...
	// YEMU_DISABLE_NOHUP_BUILTIN (session env or process env), in which case bare
	// `nohup` resolves to the real coreutils binary.
	let nohup_builtin_disabled = {
		let raw = config
			.session_env
			.as_ref()
			.and_then(|env| env.get("YEMU_DISABLE_NOHUP_BUILTIN").cloned())
			.or_else(|| std::env::var("YEMU_DISABLE_NOHUP_BUILTIN").ok());
...
async fn source_snapshot(
	...
	let source_info = SourceInfo::from("yemu-native:snapshot");
...
async fn run_shell_command_single(
		...
		&& let Some(config) = options.minimizer.as_ref()
	{
		...
		if !buffered.exceeded {
			...
			// in `yemu-native`. A supported filter that runs but leaves the output
...
async fn run_shell_command_once(
	...
	let source_info = SourceInfo::from("yemu-native:command");
	let result = session
		.shell
		.run_string(command, &source_info, &params)
		.await;
...
async fn run_shell_command_streams(
	...
	ensure_trailing_newline_for_heredoc(&mut command);
	let source_info = SourceInfo::from("yemu-shell:streams");
	let result = session
		.shell
		.run_string(command, &source_info, &params)
		.await;
...
/// backpressure: on a bounded channel (the yemu-native JS bridge) the send
...
impl builtins::Command for TimeoutCommand {
	...
	fn execute<SE: brush_core::ShellExtensions>(
		...
		async move {
			...
			let source_info = SourceInfo::from("yemu-native:timeout");
...
impl builtins::Command for NohupCommand {
	...
	fn execute<SE: brush_core::ShellExtensions>(
		...
		async move {
			...
			let mut params = context.params.clone();
			params.process_group_policy = ProcessGroupPolicy::NewProcessGroup;
			let source_info = SourceInfo::from("yemu-native:nohup");
			context
				.shell
				.run_string(command_line, &source_info, &params)
				.await
...
mod tests {
	...
	async fn kill_builtin_accepts_numeric_signal_for_multiple_processes() {
		...
		let source_info = SourceInfo::from("yemu-native:test");
	...
	async fn uutils_mkdir_resolves_cwd_and_displays_operand() {
		let tmp = std::env::temp_dir().join(format!("yemu-mkdir-{}", std::process::id()));
	...
	async fn uutils_mkdir_expands_tilde_for_every_brace_element() {
		let base = std::env::temp_dir().join(format!("yemu-mkdir-brace-{}", std::process::id()));
	...
	async fn uutils_mkdir_help_and_bad_flag_do_not_exit_process() {
		let tmp = std::env::temp_dir().join(format!("yemu-mkdir-help-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
		let tmp_str = tmp.to_str().expect("utf8 temp path");
	...
	async fn uutils_head_streams_stdin_and_reads_files() {
		let tmp = std::env::temp_dir().join(format!("yemu-head-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
	...
	async fn uutils_head_help_and_bad_flag_do_not_exit_process() {
		let tmp = std::env::temp_dir().join(format!("yemu-head-help-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
		let tmp_str = tmp.to_str().expect("utf8 temp path");
	...
	async fn uutils_filters_listing_find_grep() {
		let tmp = std::env::temp_dir().join(format!("yemu-utils-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		...
		params.set_fd(OpenFiles::STDOUT_FD, null_file().expect("null"));
		params.set_fd(OpenFiles::STDERR_FD, null_file().expect("null"));
		let si = SourceInfo::from("yemu-native:test");
	...
	async fn rg_builtin_uses_ripgrep_defaults() {
		let tmp = std::env::temp_dir().join(format!("yemu-rg-defaults-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
	...
	async fn fd_builtin_uses_fd_defaults() {
		let tmp = std::env::temp_dir().join(format!("yemu-fd-defaults-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(tmp.join("sub")).expect("sub dir");
	...
	async fn rg_builtin_defaults_to_cwd_unless_stdin_is_pipeline() {
		let tmp = std::env::temp_dir().join(format!("yemu-rg-stdin-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
	...
	async fn grep_quiet_and_line_regexp() {
		let tmp = std::env::temp_dir().join(format!("yemu-grepq-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
		std::fs::write(tmp.join("data.txt"), "foo\nbar\nbaz\n").expect("data");
		let tmp_str = tmp.to_str().expect("utf8");
	...
	async fn uutils_rm_and_mv_operate_on_shell_cwd() {
		let tmp = std::env::temp_dir().join(format!("yemu-rmmv-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
	...
	async fn uutils_rm_missing_file_prints_single_diagnostic() {
		let tmp = std::env::temp_dir().join(format!("yemu-rm-missing-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
		let tmp_str = tmp.to_str().expect("utf8");
	...
	async fn uutils_find_display_and_actions_split_paths() {
		let tmp = std::env::temp_dir().join(format!("yemu-find-split-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		...
		session
			.shell
			.run_string(
				"export YEMU_EXEC_ENV=zed; find . -name keep.log -exec sh -c 'echo \"$YEMU_EXEC_ENV $1\"' \
				 sh {} ';' > cap.txt",
				&si,
				&params,
			)
	...
	async fn uutils_sed_substitutes_streams_and_edits_in_place() {
		let tmp = std::env::temp_dir().join(format!("yemu-sed-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
	...
	async fn uutils_xargs_children_feed_pipeline_and_report_failure() {
		let tmp = std::env::temp_dir().join(format!("yemu-xargs-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
		let tmp_str = tmp.to_str().expect("utf8");
	...
	async fn uutils_jq_filters_json_and_propagates_exit_status() {
		let tmp = std::env::temp_dir().join(format!("yemu-jq-{}", std::process::id()));
		let _ = std::fs::remove_dir_all(&tmp);
		std::fs::create_dir_all(&tmp).expect("temp dir");
		std::fs::write(tmp.join("in.json"), "{\"name\":\"yemu\"}\n").expect("in.json");
		let tmp_str = tmp.to_str().expect("utf8");
	...
	async fn uutils_builtins_respect_disable_env() {
		...
		let mut all_off = create_session(&session_with(&[("YEMU_DISABLE_UUTILS_BUILTINS", "1")]))
			.await
			.expect("create_session");
		...
		let mut rm_off = create_session(&session_with(&[("YEMU_DISABLE_RM_BUILTIN", "1")]))
			.await
			.expect("create_session");
	...
	///
	/// Lives in `yemu-native` because the brush-core crate is excluded from the
	...
	fn unique_temp_dir(prefix: &str) -> std::path::PathBuf {
		...
		let nonce = std::time::SystemTime::now()
			...
			.expect("system time")
			.as_nanos();
		path.push(format!("yemu-shell-{prefix}-{}-{nonce}", std::process::id()));
...
"#,
	...
	/// "yemu-native:command: syntax error at end of input". The segmented chain
	...
	async fn embedded_external_command_runs_in_its_own_session() {
		...
		// Build the same kind of session yemu-native uses in production.
		let config = ShellConfig { session_env: None, snapshot_path: None, minimizer: None };
		let mut session = create_session(&config).await.expect("create_session");
	...
	/// leave the embedded host (YeMu) stopped on its next tty read. The earlier
