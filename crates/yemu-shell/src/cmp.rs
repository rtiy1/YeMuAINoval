...
enum InputReader {
	File(File),
	Stdin(yemu_uutils_ctx::CtxStdin),
	Bytes(Cursor<Vec<u8>>),
}
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match compare(&matches) {
		Ok(code) => code,
		Err(message) => {
			let _ = writeln!(yemu_uutils_ctx::stderr(), "cmp: {message}");
			2
		},
	}
}
...
fn command() -> Command {
	Command::new("cmp")
		.version(concat!("cmp (yemu-shell) ", env!("CARGO_PKG_VERSION")))
	...
	if name == OsStr::new("-") {
		return Ok(Input {
			reader:      BufReader::with_capacity(
				BUFFER_SIZE,
				InputReader::Stdin(yemu_uutils_ctx::stdin()),
			),
			regular_len: None,
		});
	}
