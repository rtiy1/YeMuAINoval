...
//!
//! Each builtin installs a [`yemu_uutils_ctx`] scope — the command's stdio file
...
/// Drives a patched uutils utility to completion under a [`yemu_uutils_ctx`]
...
fn run_caught(run: UutilRun, argv: Vec<OsString>) -> i32 {
	...
	if let Ok(code) = catch_unwind(|| run(argv)) {
		...
		let _ = writeln!(yemu_uutils_ctx::stderr(), "{name}: internal error");
		1
	}
}
...
uutil_builtin!(pub fn grep_builtin => yemu_uu_grep::run);
uutil_builtin!(pub fn rg_builtin => yemu_uu_grep::run_rg);
...
uutil_builtin!(pub fn diff_builtin => yemu_uu_diff::run);
...
mod tests {
	...
	fn scope_io(stderr: Box<dyn Write + Send>) -> yemu_uutils_ctx::ScopeIo {
		yemu_uutils_ctx::ScopeIo {
	...
	fn run_in_scope(run: UutilRun, argv: Vec<OsString>) -> (i32, String) {
		...
		let code = yemu_uutils_ctx::scope(scope_io(Box::new(ChanWriter(tx))), || run_caught(run, argv));
		let mut err = Vec::new();
