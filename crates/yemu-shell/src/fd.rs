//! In-process `fd` builtin backed by `yemu_walker`, `globset`, and `regex`.
...
use yemu_walker::CollectedEntry;
...
fn search(
	...
	for search_path in &search_paths {
		...
		let outcome = match request.collect_with_heartbeat(cancel_heartbeat(cancelled)) {
			Ok(outcome) => outcome,
			Err(yemu_walker::WalkError::Interrupted(_)) if cancelled.load(Ordering::Relaxed) => break,
...
fn fd_walk_request(
	...
	yemu_walker::WalkRequest::new(root)
		...
		.order(yemu_walker::WalkOrder::Path)
		.emit_root(true)
...
fn process_walker_entry<W: Write>(
	...
	path: &Path,
	depth: usize,
	file_type: yemu_walker::FileType,
	out: &mut W,
	matches: &mut usize,
) -> io::Result<yemu_walker::WalkDecision> {
	let is_directory = file_type == yemu_walker::FileType::Dir;
...
fn matches_walker_type_filter(
	...
	let kind_matches = if filter.has_kind() {
		(filter.regular && file_type == yemu_walker::FileType::File)
			|| (filter.directory && file_type == yemu_walker::FileType::Dir)
			|| (filter.symlink && file_type == yemu_walker::FileType::Symlink)
	} else {
		true
	};
	if !kind_matches {
...
/// closure, `yemu_walker`'s per-entry heartbeat never checks the flag and a
...
fn walker_error_to_io(err: yemu_walker::WalkError<io::Error>) -> io::Error {
	match err {
		yemu_walker::WalkError::Interrupted(err) => err,
		yemu_walker::WalkError::InvalidData { path, message } => {
			io::Error::other(format!("{}: {message}", path.display()))
		},
	}
}

fn walker_collect_error_to_io(err: yemu_walker::WalkError<String>) -> io::Error {
	match err {
		yemu_walker::WalkError::Interrupted(err) => io::Error::other(err),
		yemu_walker::WalkError::InvalidData { path, message } => {
			io::Error::other(format!("{}: {message}", path.display()))
		},
	}
...
fn process_collected_entry<W: Write>(
	...
	let is_directory = entry.file_type == yemu_walker::FileType::Dir;
	if depth == 0 && is_directory {
		return Ok(());
	}
...
mod tests {
	...
	fn walk_request(tree: &std::path::Path) -> yemu_walker::WalkRequest {
		yemu_walker::WalkRequest::new(tree)
			.hidden(true)
			.gitignore(false)
			.emit_root(true)
	}
	...
	fn cancel_heartbeat_aborts_collect_with_heartbeat() {
		...
		assert!(
			matches!(err, yemu_walker::WalkError::Interrupted(_)),
	...
	fn cancel_heartbeat_aborts_for_each_entry_with_heartbeat() {
		...
		let result = walk_request(&tree).for_each_entry_with_heartbeat(
			...
			|_entry| {
				...
				Ok::<_, std::io::Error>(yemu_walker::WalkDecision::Include)
			},
			|_error| Ok::<_, std::io::Error>(yemu_walker::WalkDecision::Include),
		);
