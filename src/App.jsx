...
import { api } from "./api";
import AssistantPanel from "./assistant-panel.jsx";
import { saveTextDocument, sendAgentNotification } from "./platform.mjs";
...
const localAgentCommands = new Set([
	"help",
	"settings",
	"status",
	"context",
...
function resolveWebCompactionThreshold(contextWindow, compaction) {
	...
	const reserve =
		explicitReserve == null ? Math.max(proportional, 16384) : Math.max(proportional, Number(explicitReserve) || 0);
	const effectiveReserve =
		(explicitReserve == null && reserve >= windowTokens - proportional) || reserve >= windowTokens
			? proportional
			: reserve;
...
const authQuotes = [
	{
		chapter: "第 8 章",
		title: "风从旧码头来",
		text: "“她终于明白，潮水从来不是为了带走什么。它只是一次次回来，提醒岸边的人，时间仍在往前。”",
	},
	{
		chapter: "第 1 章",
		title: "雨落之前",
		text: "“故事往往从一个微不足道的决定开始，而命运只负责把它推向更远的地方。”",
	},
	{ chapter: "第 17 章", title: "无人回信", text: "“他把最后一句话写在信里，等了很久，直到窗外的灯一盏一盏熄灭。”" },
	{
		chapter: "尾声",
		title: "仍在路上",
		text: "“写下去不是为了抵达结局，而是为了让此刻的心声，终于有一个可以回去的地方。”",
	},
];
...
const skillMeta = {
	...
	"story-long-scan": {
		label: "长篇题材趋势",
		command: "分析长篇网文的题材与市场趋势；不要声称获取了实时榜单",
		needsContent: false,
	},
	"story-short-scan": {
		label: "短篇题材趋势",
		command: "分析短篇网文的题材与市场趋势；不要声称获取了实时榜单",
		needsContent: false,
	},
	...
	"story-setup": {
		label: "写作准备建议",
		command: "给我一份开始写书前的准备建议，不执行系统部署",
		needsContent: false,
	},
};

// 需要正文的 skill
const contentSkills = new Set([
	"story-deslop",
	"story-review",
	"story-long-analyze",
	"story-short-analyze",
...
function agentRunTokenUsage(run) {
	...
	const output = String(
		result.edit_proposal?.revised_text || result.output || result.summary || result.message || run?.text || "",
	);
...
function parseNovelText(rawText) {
	...
	const headingPattern =
		/^(?:第.{1,16}[章回节卷部篇集](?:\s+|[:：、._-])?.*|(?:chapter|chap\.?)\s*\d+(?:\s+|[:：、._-])?.*|序章|楔子|前言|引子|尾声|后记|番外(?:\s*[一二三四五六七八九十百千万两\d]+)?.*)$/i;
	const chapters = [];
	const preface = [];
	let current = null;
	...
	for (const line of normalized.split("\n")) {
		...
		if (candidate && candidate.length <= 100 && headingPattern.test(candidate)) {
			if (!foundHeading && preface.join("\n").trim())
				chapters.push({ title: "前言", content: preface.join("\n").trim() });
			pushCurrent();
...
function parseSmartProposal(response) {
	...
	if (!parsed && typeof raw === "string") {
		...
		try {
			...
			try {
				parsed = objectText ? JSON.parse(objectText) : null;
			} catch {
				parsed = null;
			}
			if (!parsed) {
	...
	const tone = String(
		parsed.tone || parsed.mainline || parsed.main_line || parsed.storyline || parsed.synopsis || parsed.summary || "",
	).trim();
	const sourceChapters = parsed.chapters || parsed.outline || parsed.chapter_outline || parsed.chapterOutline || [];
	const chapters = Array.isArray(sourceChapters)
		? sourceChapters
				.map((chapter, index) => {
					...
					return {
						...
						content: String(
							chapter?.content || chapter?.outline || chapter?.summary || chapter?.plot || "",
						).trim(),
					};
				})
	...
	return {
		...
		tone: tone.slice(0, 2000),
		chapters: chapters
			.slice(0, 100)
			.map(chapter => ({ title: chapter.title.slice(0, 100), content: chapter.content.slice(0, 5000) })),
	};
}
...
function App() {
			...
			.then(([projectResponse, ideaResponse, foreshadowResponse, memoryResponse, dashboardResponse]) => {
				...
				setActiveProject(
					current =>
						...
						nextProjects.find(project => project.isActive) ||
						nextProjects[0],
				);
	...
	useEffect(() => {
		...
		api.getSkills()
			.then(response => {
				if (mounted) setSkillCatalog(response.skills || []);
			})
			.catch(() => undefined)
			.finally(() => {
	...
	useEffect(() => {
		...
		api.getChapters(activeProject.id)
			.then(async chapterResponse => {
				...
				const nextChapter =
					nextChapters.find(chapter => chapter.state === "current") ||
					nextChapters.reduce(
						(latest, chapter) =>
							String(chapter.updatedAt || "") > String(latest.updatedAt || "") ? chapter : latest,
						nextChapters[0],
					);
	...
	useEffect(() => {
		...
		api.getChapterDraft(activeProject.id, activeChapterId)
			...
			.finally(() => {
				if (mounted && activeDraftKeyRef.current === key) setDraftLoading(false);
			});
		return () => {
			mounted = false;
		};
	...
	useEffect(() => {
		...
		api.getChapterHistory(activeProject.id, activeChapterId)
			.then(response => {
				if (mounted) setHistorySnapshots(response.snapshots || []);
			})
			.catch(() => {
				if (mounted) setHistorySnapshots([]);
			})
			.finally(() => {
	...
	useEffect(() => {
		if (draftLoading || !activeProject?.id || activeChapterId == null || draft === savedDraftRef.current)
			return undefined;
	...
	const activeChapter = useMemo(
		() => chapters.find(chapter => String(chapter.id) === String(activeChapterId)) || null,
	...
	async function refreshGeneratedArtifacts(application) {
		...
		try {
			...
			if (projectResponse.project) {
				setProjects(current =>
					current.map(project => (project.id === projectResponse.project.id ? projectResponse.project : project)),
				);
	...
	async function persistDraft(projectId, chapterId, content, { silent = false } = {}) {
		...
		try {
			const request = saveQueueRef.current
				.catch(() => undefined)
				.then(() => api.saveChapterDraft(projectId, chapterId, content));
			...
			if (response.chapter)
				setChapters(current =>
					current.map(chapter =>
						String(chapter.id) === String(response.chapter.id) ? response.chapter : chapter,
					),
				);
	...
	async function createProject(event) {
		...
		try {
			...
			const response = await api.createProject({
				title,
				type: String(form.get("type") || "长篇"),
				genre: String(form.get("genre") || "现代言情"),
	...
	async function createChapter() {
		...
		try {
			...
			setToast("新章节已加入目录");
			void Promise.all([api.getProject(currentProject.id), api.getDashboard()])
				.then(([projectResponse, dashboardResponse]) => {
					if (projectResponse.project) {
						setProjects(current =>
							current.map(project =>
								project.id === projectResponse.project.id ? projectResponse.project : project,
							),
						);
						setActiveProject(projectResponse.project);
					}
	...
	async function splitChapter(position, title) {
		...
		try {
			...
			setChapters(current => [
				...current.map(chapter => (String(chapter.id) === String(currentChapter?.id) ? currentChapter : chapter)),
				nextChapter,
			]);
			if (nextResponse?.project) {
				setProjects(current =>
					current.map(project => (project.id === nextResponse.project.id ? nextResponse.project : project)),
				);
	...
	async function updateChapterState(chapter, state) {
		...
		try {
			...
			setChapters(current =>
				current.map(item => (String(item.id) === String(chapter.id) ? response.chapter : item)),
			);
	...
	async function createIdea(data) {
		try {
			const projectId = data && Object.hasOwn(data, "projectId") ? data.projectId : currentProject?.id;
	...
	async function editIdea(idea, data) {
		try {
			const response = await api.updateIdea(idea.id, {
				title: data.title,
				body: data.body,
				label: data.label,
	...
	async function confirmStoryMemories(candidates) {
		...
		try {
			const response = await api.confirmStoryMemories(
				currentProject.id,
				candidates.map(candidate => ({
					...candidate,
					characterName: candidate.characterName ?? candidate.character_name ?? "",
					replacesMemoryId: candidate.replacesMemoryId ?? candidate.replaces_memory_id ?? null,
					sourceChapterId: candidate.sourceChapterId ?? activeChapterId,
				})),
			);
			setStoryMemories(current => [
				...response.created,
				...response.updated,
				...current.filter(item => !response.updated.some(updated => updated.id === item.id)),
			]);
	...
	async function reviewChapter(title = "未命名章节") {
		...
		try {
			const response = await api.runStoryAgent({
				...
				payload: {
					title,
					genre: currentProject?.genre || "网络小说",
		...
		const request = (async () => {
			...
			try {
				...
				if (task.status === "completed" && task.result) {
					...
					setRunnerResult({
						status: task.status === "cancelled" ? "failed" : "failed",
						result: { message: task.error || task.statusMessage || "AI 任务未完成" },
	...
	async function deslopChapter() {
		...
		try {
			...
			setRunnerContext({
				...
				chapterTitle: chapters.find(chapter => String(chapter.id) === String(activeChapterId))?.title || "当前章节",
		...
		<div className={`app-shell web-surface ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
			<aside className={`sidebar ${showMobileMenu ? "is-open" : ""}`}>
				...
				<div className="recent-projects project-shelf">
					{projects.map(project => (
						<button
							key={project.id}
							className={`recent-project ${String(currentProject?.id) === String(project.id) ? "active" : ""}`}
							onClick={() => openProject(project)}
						>
							<span className={`mini-cover ${project.cover}`} aria-hidden="true">
				...
				<div className="sidebar-bottom">
					<button
						className={`nav-item theme-toggle ${theme}`}
						aria-label={theme === "dark" ? "切换为日间模式" : "切换为夜间模式"}
						title={sidebarCollapsed ? (theme === "dark" ? "日间模式" : "夜间模式") : undefined}
						onClick={toggleTheme}
					>
						<span className="theme-toggle-icon" aria-hidden="true">
							<Moon className="theme-moon" size={17} strokeWidth={1.8} />
							<Sun className="theme-sun" size={17} strokeWidth={1.8} />
						</span>
					...
					<div className="profile-chip" ref={profileMenuRef}>
						...
						<button
							type="button"
							className={`profile-more-button ${profileMenuOpen ? "active" : ""}`}
							aria-label="打开账户菜单"
			...
			<main className="main-shell">
				{activeSection !== "editor" && (
					<header className="topbar">
						...
						<div className="topbar-actions">
							...
							<button className="primary-button top-new-button" onClick={() => setShowNew(true)}>
								<Plus size={17} />
								新建作品
							</button>
						</div>
				...
				<div className="content-wrap">
					{activeSection === "editor" && currentProject && (
						<Editor
							...
							storyMemories={storyMemories.filter(memory => memory.projectId === currentProject.id)}
							onUpdateStoryMemory={updateStoryMemory}
							onDeleteStoryMemory={deleteStoryMemory}
							onConfirmStoryMemories={confirmStoryMemories}
							...
							onCreateHistory={createHistorySnapshot}
							lastAiRestore={lastAiRestore}
							onAiApplied={snapshot => setLastAiRestore(snapshot)}
							onAiRestored={() => setLastAiRestore(null)}
							onArtifactsApplied={refreshGeneratedArtifacts}
							onNotify={notify}
							onSave={saveDraft}
							...
							onDeslop={deslopChapter}
							deslopLoading={deslopLoading}
							onNewChapter={createChapter}
							onSplitChapter={splitChapter}
							onSelectChapter={selectChapter}
							onRenameChapter={renameChapter}
							onUpdateChapterState={updateChapterState}
							onDeleteChapter={deleteChapter}
							onOpenProject={openProject}
							onOpenSkill={openSkillRunner}
							onOpenSettings={() => setSettingsOpen(true)}
							applyRequest={editorApplyRequest}
							onApplyRequestHandled={() => setEditorApplyRequest(null)}
						/>
					)}
					{activeSection === "editor" && !currentProject && (
						<div className="page inner-page workspace-empty">
							<div className="empty-state">
								<div className="empty-state-icon">
									<BookOpen size={28} />
								</div>
								<h2>还没有作品</h2>
								<p>新建或导入作品后，工作台会直接显示正文和助手。</p>
								<div className="empty-state-actions">
									<button className="primary-button" onClick={() => setShowNew(true)}>
										<BookPlus size={17} />
										新建作品
									</button>
									<button className="secondary-button" onClick={() => setImportProjectOpen(true)}>
										<Download size={16} />
										导入文稿
									</button>
								</div>
					...
					{activeSection === "works" && (
						<Works
							...
							onEdit={p => setEditProjectTarget(p)}
							onDelete={deleteProject}
							onImport={() => setImportProjectOpen(true)}
						/>
					)}
					...
					{activeSection === "skill-market" && (
						<SkillMarket user={user} onNotify={notify} onSkillsChanged={refreshSkills} />
					)}
			...
			<div className="mobile-nav">
				{primaryNavItems.map(({ id, label, icon: Icon }) => (
					<button key={id} className={activeSection === id ? "active" : ""} onClick={() => selectSection(id)}>
						<Icon size={18} />
						<span>{label}</span>
					</button>
				))}
			...
			{importProjectOpen && (
				<ImportProjectModal
					...
					onClose={() => !importProjectLoading && setImportProjectOpen(false)}
			...
			{editProjectTarget && (
				<EditProjectModal
					...
					onSave={updates => {
						editProject(editProjectTarget, updates);
						setEditProjectTarget(null);
					}}
				/>
			)}
			{toast && (
				<div className={`toast ${toastKind(toast)}`} role="status" aria-live="polite">
					{toastKind(toast) === "loading" ? (
...
function WritingCalendar({ days = [] }) {
	...
	return (
		<div className="writing-calendar">
				...
				<div className="calendar-grid" role="img" aria-label="过去一年每日写作字数日历">
					{calendarCells.map((day, index) =>
						day ? (
							<span
								key={day.date}
								className={`calendar-cell level-${writingLevel(day.words)}`}
								title={`${day.date} · ${formatNumber(day.words)} 字`}
								aria-label={`${day.date}，写作 ${formatNumber(day.words)} 字`}
							/>
						) : (
...
function WritingStats({ stats, projects }) {
	const rankedProjects = [...projects]
		.sort(
			(left, right) =>
				Number(String(right.words || "0").replaceAll(",", "")) -
				Number(String(left.words || "0").replaceAll(",", "")),
		)
		.slice(0, 5);
	const metricCards = [
		...
		{
			label: "写作日均",
			value: formatNumber(stats?.averageWordsPerWritingDay),
			suffix: "字",
			icon: BarChart3,
			tone: "green",
		},
	...
	return (
		<div className="page inner-page analytics-page">
			<div className="page-heading analytics-heading">
				<div>
					<span className="section-overline">WRITING INSIGHTS</span>
					<h1>写作统计</h1>
					<p>每一次保存都算数。这里记录你的创作节奏，而不只是最终字数。</p>
				</div>
			...
			<section className="writing-calendar-card">
				<div className="calendar-summary">
					<div className="writing-days-total">
						<small>累计写作</small>
						<div>
							<strong>{formatNumber(stats?.totalWritingDays)}</strong>
							<span>天</span>
						</div>
						<p>
							{stats?.firstWritingDate
								? `从 ${stats.firstWritingDate.replaceAll("-", ".")} 开始记录`
								: "从今天开始留下第一格"}
						</p>
					</div>
					<div className="streak-summary">
						<div>
							<span className="streak-icon current">
								<Flame size={18} />
							</span>
							<p>
								<small>当前连续</small>
								<strong>{formatNumber(stats?.currentStreak)} 天</strong>
							</p>
						</div>
						<div>
							<span className="streak-icon best">
								<Trophy size={18} />
							</span>
							<p>
								<small>最长连续</small>
								<strong>{formatNumber(stats?.longestStreak)} 天</strong>
							</p>
						</div>
					</div>
				</div>
			...
			<section className="analytics-metrics" aria-label="写作指标">
				{metricCards.map(({ label, value, suffix, icon: Icon, tone }) => (
			...
			<div className="analytics-detail-grid">
				...
				<section className="project-ranking">
					<div className="section-heading compact">
						<div>
							<span className="section-overline">作品贡献</span>
							<h2>字数分布</h2>
						</div>
						<span className="tiny-meta">{projects.length} 部作品</span>
					</div>
					<div className="ranking-list">
						{rankedProjects.length ? (
							rankedProjects.map((project, index) => {
								...
								return (
									<div className="ranking-item" key={project.id}>
...
function ProfileCenter({ user, stats, projects, onNavigate, onOpenSettings, onOpenProject }) {
	...
	const milestones = [
		{
			label: "第一部作品",
			description: "建立属于自己的故事世界",
			earned: Number(stats?.projectCount || 0) >= 1,
			icon: BookPlus,
		},
		{
			label: "万字作者",
			description: "累计完成 10,000 字",
			earned: Number(stats?.totalWords || 0) >= 10000,
			icon: PenLine,
		},
		{
			label: "七日执笔",
			description: "累计写作达到 7 天",
			earned: Number(stats?.totalWritingDays || 0) >= 7,
			icon: Flame,
		},
		{
			label: "完成一本",
			description: "至少有一部作品已完结",
			earned: Number(stats?.completedProjects || 0) >= 1,
			icon: Trophy,
		},
	];

	return (
		<div className="page inner-page profile-page">
			<section className="profile-hero">
				<div className="profile-avatar-large">{user?.name?.slice(0, 1) || "作"}</div>
				<div className="profile-hero-copy">
					<span className="section-overline">AUTHOR PROFILE</span>
					<h1>{user?.name || "创作者"}</h1>
					<p>{user?.email}</p>
					<div>
						<span>
							<Clock3 size={13} />
							加入创作空间第 {formatNumber(joinedDays)} 天
						</span>
						<span>
							<PenLine size={13} />
							累计写作 {formatNumber(stats?.totalWritingDays)} 天
						</span>
					</div>
				</div>
				<button className="secondary-button profile-settings-button" onClick={onOpenSettings}>
					<Settings2 size={16} />
					模型与 API 设置
				</button>
			</section>
			...
			<section className="profile-overview-grid">
				...
				<article>
					<small>作品空间</small>
					<div>
						<strong>{formatNumber(stats?.projectCount)}</strong>
						<span>部</span>
					</div>
					<p>{formatNumber(stats?.chapterCount)} 个章节</p>
				</article>
				<article>
					<small>连续写作</small>
					<div>
					...
					<p>历史最佳 {formatNumber(stats?.longestStreak)} 天</p>
				</article>
				<article>
					<small>本月进度</small>
					<div>
						<strong>{formatNumber(stats?.monthWords)}</strong>
						<span>字</span>
					</div>
					<p>今天新增 {formatNumber(stats?.todayWords)} 字</p>
				</article>
			</section>
			...
			<div className="profile-content-grid">
				<section className="profile-panel">
					<div className="section-heading compact">
						<div>
							<span className="section-overline">创作成就</span>
							<h2>里程碑</h2>
						</div>
						<span className="tiny-meta">
							{milestones.filter(item => item.earned).length} / {milestones.length} 已获得
						</span>
					</div>
					<div className="milestone-grid">
						{milestones.map(({ label, description, earned, icon: Icon }) => (
							<div className={`milestone-card ${earned ? "earned" : ""}`} key={label}>
								<span>
									<Icon size={18} />
								</span>
								<div>
				...
				<section className="profile-panel account-panel">
					<div className="section-heading compact">
						<div>
							<span className="section-overline">账号信息</span>
							<h2>个人资料</h2>
						</div>
					</div>
					<dl>
						...
						<div>
							<dt>加入时间</dt>
							<dd>
								{joinedAt && !Number.isNaN(joinedAt.getTime()) ? joinedAt.toLocaleDateString("zh-CN") : "—"}
							</dd>
						</div>
					</dl>
					<button className="text-button" onClick={() => onNavigate("stats")}>
						<BarChart3 size={15} />
						查看完整写作统计
			...
			<section className="profile-panel profile-recent-projects">
				<div className="section-heading compact">
					<div>
						<span className="section-overline">最近创作</span>
						<h2>继续你的故事</h2>
					</div>
					<button className="text-button" onClick={() => onNavigate("works")}>
						管理全部作品 <ArrowUpRight size={15} />
					</button>
				</div>
					...
					{recentProjects.length ? (
						recentProjects.map(project => (
							<button key={project.id} onClick={() => onOpenProject(project)}>
								<span className={`row-cover ${project.cover}`}>{project.title.slice(0, 1)}</span>
								<span>
									<strong>{project.title}</strong>
									<small>
										{project.genre} · {project.chapters || 0} 章 · {project.words || 0} 字
									</small>
								</span>
	...
	const heading = isRegister
		...
		: isForgot
			? {
					overline: "找回创作空间",
					title: "重拾你的故事。",
					description: "输入注册邮箱，我们会发送一封密码重置邮件。",
				}
			: isReset
				? {
						overline: "设置新密码",
						title: "重新打开故事。",
						description: "新密码设置成功后，其他设备上的旧登录会话会全部失效。",
					}
		...
		<main className="auth-page">
			<section className="auth-brand-panel">
				...
				<div className="auth-progress-art" aria-hidden="true">
					{authQuotes.map((item, index) => (
						<span className={index === quoteIndex ? "active" : ""} key={item.title} />
					))}
				</div>
			...
			<section className="auth-form-panel">
				<div className="auth-form-wrap">
					<div className="auth-mode" aria-label="认证方式">
						<button type="button" className={isLogin ? "active" : ""} onClick={() => changeMode("login")}>
							登录
						</button>
						<button type="button" className={isRegister ? "active" : ""} onClick={() => changeMode("register")}>
							注册
						</button>
					</div>
					...
					<form className="auth-form" onSubmit={submit}>
						...
						{!isReset && (
							<label>
								...
								<div className="auth-input">
									<Mail size={16} />
									<input
										ref={isRegister ? emailInputRef : null}
										name="email"
										type="email"
										autoComplete="email"
						...
						<button className="auth-submit" disabled={submitting} type="submit">
							{submitting ? <LoaderCircle size={17} className="spin" /> : <ArrowUpRight size={17} />}
							{submitLabel}
						</button>
						{(isForgot || isReset) && (
					...
					<p className="auth-security">
						<LockKeyhole size={13} />
						{isForgot || isReset
							? "重置链接仅可使用一次，并会在短时间后过期。"
							: "密码经过加盐哈希处理，登录会话可随时退出。"}
					</p>
			...
			<section className="welcome-row">
				<div>
					...
					<p className="welcome-copy">
						{projects.length
							? "回到正在推进的故事，或者先和夜雨理清下一步。"
							: "和夜雨聊聊你的想法，或手动建立第一部作品。"}
					</p>
				</div>
				<div className="welcome-actions">
					{active ? (
						<>
							<button className="secondary-button" onClick={() => onNavigate("editor")}>
								<PenLine size={16} />
								进入工作台
							</button>
							<button className="primary-button" onClick={() => onOpen(active)}>
								<PenLine size={17} />
								继续写作
							</button>
						</>
				...
				<>
					<section className="focus-section overview-focus-section">
						<div className="section-heading">
							<div>
								<span className="section-overline">正在进行</span>
								<h2>当前作品</h2>
							</div>
							<button className="text-button" onClick={() => onNavigate("works")}>
						...
						<div className="focus-project">
							<div className={`large-cover ${active.cover}`}>
							...
							<div className="focus-copy">
								...
								<div className="focus-metrics">
									<span>
										<strong>{active.progress}%</strong> 完成度
									</span>
									<span>
										<strong>{active.words}</strong> 字
									</span>
									<span>更新于 {formatRelativeTime(active.updatedAt, active.updated)}</span>
								</div>
								<button className="dark-button" onClick={() => onOpen(active)}>
									继续写作 <ArrowUpRight size={16} />
								</button>
							</div>
							<div className="focus-side-note">
								<span className="note-label">下一章提示</span>
								<p>继续推进当前章节的情节。</p>
								<button
					...
					<section className="dashboard-summary dashboard-stat-strip" aria-label="创作数据概览">
						...
						<div className="summary-card">
							...
							<div>
								<small>今日新增</small>
								<strong>{formatNumber(stats?.todayWords)}</strong>
							</div>
						</div>
						<div className="summary-card">
							<span className="summary-icon yellow">
								<BookOpen size={16} />
							</span>
							<div>
								<small>章节总数</small>
								<strong>{formatNumber(stats?.chapterCount)}</strong>
							</div>
						</div>
						<div className="summary-card">
							...
							<div>
								<small>本周活跃</small>
								<strong>{formatNumber(stats?.activeDays)} 天</strong>
							</div>
						</div>
					</section>
					...
					<section className="dashboard-grid overview-pulse-grid">
						...
						<section className="overview-secondary-links" aria-label="工作区入口">
							<div className="section-heading compact">
								<div>
									<span className="section-overline">工作区</span>
									<h2>需要时再打开</h2>
								</div>
							</div>
					...
					<section className="recent-section">
						<div className="section-heading compact">
							<div>
								<span className="section-overline">作品空间</span>
								<h2>最近的作品</h2>
							</div>
							<button className="text-button" onClick={() => onNavigate("works")}>
								管理作品 <ArrowUpRight size={15} />
							</button>
						</div>
...
function WritingAssistantPage({
	...
	const starterPrompts = ["我想写一本小说", "我想写一个现代都市故事", "帮我构思一个悬疑短篇"];
	...
	useEffect(() => {
		...
		api.getSettings()
			.then(response => {
				if (mounted) setModel(response.settings?.model || "");
			})
			.catch(() => undefined);
		return () => {
			mounted = false;
	...
	function submitCustomQuestion(event, _questionId) {
		event?.preventDefault();
			...
			<div className="assistant-chat-shell">
				<header className="assistant-chat-header">
					<div className="assistant-chat-title">
						<span className="assistant-chat-mark">
							<Bot size={18} />
						</span>
						<div>
					...
					<div className="assistant-chat-actions">
						{session && (
							<button className="text-button" disabled={loading} onClick={onClear}>
								{session.phase === "writing" ? "开始另一本" : "新对话"}
							</button>
						)}
					</div>
				</header>
				...
				<div className="assistant-chat-stream" ref={conversationRef} aria-live="polite">
					{!hasStarted && !loading && (
						<div className="assistant-chat-empty">
							<div className="assistant-chat-empty-icon">
								<Bot size={28} />
							</div>
							<h2>从一个想法开始</h2>
							<p>
								描述你的目标。我会自动选择合适的 Skill；信息不足时，会像计划模式一样只确认真正影响下一步的内容。
							</p>
							<div className="assistant-chat-starters">
								{starterPrompts.map(item => (
									<button type="button" key={item} disabled={loading} onClick={() => send(item)}>
										{item}
									</button>
								))}
							</div>
					...
					{messages.map(message => (
						<div className={`assistant-chat-message ${message.role}`} key={message.id}>
							<div className="assistant-chat-avatar" aria-hidden="true">
							...
							{questions.map(question => (
								<section className="assistant-plan-question" key={question.id}>
										...
										<div>
											{[
												...
												...(question.options.some(option =>
													/^(其他|自定义|other)$/i.test(option.label || ""),
												)
													? []
													: [{ label: "其他", value: "__custom__", description: "自定义输入" }]),
											].map(option => (
												<button
													type="button"
													key={option.value}
													onClick={() =>
														option.value === "__custom__"
															? setCustomQuestionId(question.id)
															: send(option.value)
													}
												>
									...
									{customQuestionId === question.id && (
										<form
											className="assistant-plan-custom"
											onSubmit={event => submitCustomQuestion(event, question.id)}
										>
											<input
												autoFocus
												value={customAnswer}
												...
												aria-label="自定义答案"
											/>
											<button type="submit" disabled={!customAnswer.trim()}>
												继续
											</button>
										</form>
									)}
				...
				<div className="assistant-chat-composer">
					{session?.phase === "awaiting_confirmation" && session.proposal && (
						<button type="button" className="dark-button assistant-chat-proposal" onClick={onReviewProposal}>
							<BookOpenCheck size={16} />
							查看并确认建书方案
						</button>
					)}
					{needsModel && (
						<button type="button" className="secondary-button assistant-chat-settings" onClick={onOpenSettings}>
							<Settings2 size={15} />
							配置模型
						</button>
					)}
					{session?.phase === "writing" && session.projectId && (
						<div className="assistant-chat-complete">
							<p>
								<Check size={15} />
								作品已创建，可以进入编辑器继续写作。
							</p>
							<button type="button" className="primary-button" onClick={() => onOpenProject(session.projectId)}>
								<PenLine size={15} />
								打开作品
							</button>
						</div>
						...
						<>
							<form className="assistant-chat-form" onSubmit={submit}>
								<textarea
									...
									rows={1}
									maxLength={4000}
									disabled={loading || session?.phase === "awaiting_confirmation"}
									placeholder={hasStarted ? "继续补充，或直接提出新的创作任务…" : "描述你的创作目标…"}
									aria-label="输入创作想法"
								/>
								<button
									type="submit"
							...
							<div className="assistant-composer-tools">
								<button
									type="button"
									className={`composer-toggle ${webSearch ? "active" : ""}`}
									aria-pressed={webSearch}
									title={webSearch ? "已开启联网搜索：夜雨会先检索再回答" : "开启后夜雨会先联网检索再回答"}
									onClick={() => setWebSearch(value => !value)}
								>
									<Globe size={13} />
									<span>联网搜索</span>
								</button>
								<label title="自动选择或强制指定 Skill">
									<Wand2 size={13} />
									<select value={selectedSkill} onChange={event => setSelectedSkill(event.target.value)}>
										<option value="">自动选择 Skill</option>
										{availableSkills.map(item => (
											<option key={item.name} value={item.name}>
												{item.displayName || skillMeta[item.name]?.label || item.name}
											</option>
										))}
									</select>
								</label>
								<label title="切换后同步到全局设置">
									<Bot size={13} />
									<select
										value={model}
									...
									>
										<option value="">{modelLoading ? "读取模型中…" : "选择模型"}</option>
...
function WritingPulse({ stats }) {
	const daily = stats?.daily?.length
		? stats.daily
		: Array.from({ length: 7 }, (_, index) => ({ date: `empty-${index}`, words: 0 }));
	...
	const lastDate = /^\d{4}-\d{2}-\d{2}$/.test(daily.at(-1)?.date || "")
		? new Date(`${daily.at(-1).date}T00:00:00`)
		: null;
	return (
		<div className="pulse-panel">
			<div className="section-heading compact">
				<div>
					<span className="section-overline">本周写作</span>
					<h2>{stats?.weekWords ? "保持住这个节奏" : "从今天开始积累"}</h2>
				</div>
				<span className="tiny-meta">活跃 {stats?.activeDays || 0} 天</span>
			</div>
			<div className="pulse-number">
				<strong>{formatNumber(stats?.weekWords)}</strong>
				<span>字</span>
				{growth != null && (
					<em className={growth < 0 ? "negative" : ""}>
			...
			<div className="bar-chart" aria-label="近七天写作字数图表">
				{daily.map((item, index) => (
					<span
						key={item.date}
						style={{ height: `${Math.max(6, Math.round((Number(item.words || 0) / maxWords) * 100))}%` }}
						className={index === daily.length - 1 ? "today" : ""}
						title={`${item.date} · ${formatNumber(item.words)} 字`}
					/>
				))}
			</div>
			<div className="chart-labels">
				<span>{firstDate ? `${firstDate.getMonth() + 1}/${firstDate.getDate()}` : "7 天前"}</span>
				<span>今天</span>
				<span>{lastDate ? `${lastDate.getMonth() + 1}/${lastDate.getDate()}` : "今天"}</span>
			</div>
		</div>
	);
}
...
function ProjectCard({ project, onOpen }) {
	return (
		<button className="project-card" onClick={() => onOpen(project)}>
			<div className={`card-cover ${project.cover}`}>
...
function agentRunNotificationKey(run) {
	...
	const questionKey =
		`${view.completedInputItems.length}:` +
...
function AgentComposerMenu({ title, description, options, value, loading, onSelect }) {
	...
	return (
		<div className="agent-control-popover" role="listbox" aria-label={title} onKeyDown={moveFocus}>
			...
			<div className="agent-control-option-list">
				{options.map((option, index) => {
					...
					return (
						...
						>
							...
							<span className="agent-control-option-copy">
								<strong>{option.label}</strong>
				...
				{!loading && options.length <= 1 && (
					<div className="agent-control-empty">
						<Bot size={15} />
						<span>暂未读取到其他模型，请先在设置中测试连接。</span>
					</div>
				)}
			</div>
		</div>
...
function AgentThreadHistory({
	...
	const visibleThreads = useMemo(() => {
		...
		return threads.filter(
			thread =>
				!normalized ||
				`${thread.title} ${thread.latestMessage} ${chapterMap.get(String(thread.chapterId)) || ""}`
					.toLowerCase()
					.includes(normalized),
		);
			...
			<div className="agent-history-list">
				...
				{!loading &&
					visibleThreads.map(thread => {
						...
						const isRunning = thread.turns?.some(turn =>
							["queued", "running", "inProgress", "waiting_input"].includes(turn.task?.status || turn.status),
						);
						return (
							<article className={`agent-history-item ${isCurrent ? "current" : ""}`} key={thread.id}>
								<div className="agent-history-item-main">
									{editingId === thread.id ? (
										<form onSubmit={event => submitRename(event, thread)}>
											<input
												autoFocus
												value={editingTitle}
												maxLength={120}
										...
										<button
											type="button"
											disabled={switchingDisabled || isCurrent}
								...
								<div className="agent-history-actions">
									<button
										type="button"
										className={thread.isFavorited ? "active" : ""}
										onClick={() => onFavorite(thread, !thread.isFavorited)}
										aria-label={thread.isFavorited ? "取消收藏" : "收藏会话"}
										title={thread.isFavorited ? "取消收藏" : "收藏会话"}
									>
										<Star size={13} fill={thread.isFavorited ? "currentColor" : "none"} />
									</button>
									<button
										type="button"
										onClick={() => {
									...
									<button
										type="button"
										disabled={isCurrent || isRunning || isArchived}
				...
				{!loading && !visibleThreads.length && (
					<div className="agent-history-empty">
						<History size={20} />
						<span>{query ? "没有匹配的会话" : "还没有历史会话"}</span>
					</div>
				)}
			</div>
		</div>
...
function Editor({
	...
	const [agentCompaction, setAgentCompaction] = useState({
		enabled: true,
	...
	const visibleChapters = useMemo(
		() =>
			[...chapters]
				.filter(
					chapter =>
						!unfinishedOnly || chapter.state !== "done" || String(chapter.id) === String(displayChapter.id),
				)
				.sort((left, right) =>
					chapterOrder === "asc" ? Number(left.id) - Number(right.id) : Number(right.id) - Number(left.id),
				),
		[chapterOrder, chapters, displayChapter.id, unfinishedOnly],
	);
	const statusText =
		{
			saved: "已自动保存",
	...
	async function loadAgentModels() {
		...
		try {
			...
			if (response.modelCapabilities?.[agentModel])
				setAgentModelCapabilities(response.modelCapabilities[agentModel]);
		} catch (error) {
	...
	function removeAssistantAttachment(key) {
		...
		if (target) {
			setAssistantInput(current =>
				current.replace(new RegExp(`@${target.name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*`, "g"), ""),
			);
		}
	}
	...
	async function restoreAssistantThread(thread, controller) {
		...
		const running = [...restored]
			.reverse()
			.find(item => item.role === "agent" && ["queued", "running"].includes(item.status));
	...
	async function updateHistoricalThread(thread, updates) {
		try {
			...
			setAssistantHistory(current =>
				current
					.map(item => (item.id === thread.id ? response.thread : item))
					.sort(
						(left, right) =>
							Number(right.isFavorited) - Number(left.isFavorited) ||
	...
	async function archiveHistoricalThread(thread) {
		try {
			...
			setAssistantHistory(current =>
				current.map(item => (item.id === thread.id ? { ...item, status: "archived" } : item)),
			);
			onNotify("会话已归档");
		} catch (error) {
	...
	useEffect(() => {
		...
		if (!tracker.ready) {
			for (const run of agentRuns) tracker.seen.add(agentRunNotificationKey(run));
			tracker.ready = true;
			return;
		}
			...
			status === "needs_input"
				...
				: status === "failed"
					...
					: latestRun.editRequested
						? {
								title: `${ASSISTANT_NAME} 已生成修改`,
								body: `${displayChapter.title} · 修改已准备好，等待你审阅`,
							}
	...
	useEffect(() => {
		...
		async function apply() {
			...
			const start = Number.isFinite(requestedStart)
				? Math.min(Math.max(0, requestedStart), draft.length)
				: draft.length;
	...
	function openSplitDialog() {
		...
		const paragraphBreaks = [...draft.matchAll(/\n{2,}/g)]
			...
			.filter(position => position > 0 && position < draft.length);
		const nearestParagraph = paragraphBreaks.reduce(
			(nearest, position) =>
				Math.abs(position - fallbackPosition) < Math.abs(nearest - fallbackPosition) ? position : nearest,
			fallbackPosition,
		);
		...
		try {
			if (typeof Intl !== "undefined" && Intl.Segmenter) {
				...
				for (const part of segmenter.segment(draft)) {
					...
					if (part.isWordLike && word.length >= 2 && /[\u3400-\u9fffA-Za-z]/.test(word))
	...
	async function saveMemoryCandidates() {
		const selected = memoryCandidates
			.filter(candidate => candidate.selected)
			.map(({ id, selected, ...candidate }) => candidate);
		if (!selected.length) {
	...
	async function runLocalAssistantCommand(message) {
		...
		if (command.name === "help") {
			appendLocalCommandResult(
				message,
				editorAgentCommands.map(item => `${item.usage}　${item.description}`).join("\n"),
			);
			return true;
		}
		if (command.name === "context") {
			...
			const state =
				agentCompaction.enabled !== false && agentCompaction.strategy !== "off"
					? "Auto-compact on"
					: "Auto-compact off";
		...
		if (command.name === "models") {
			...
			appendLocalCommandResult(
				message,
				models.length
					? models.map(model => `${model === agentModel ? "●" : "○"} ${model}`).join("\n")
					: "当前连接没有返回可用模型。",
			);
			return true;
		}
		...
		if (command.name === "memory") {
			...
			appendLocalCommandResult(
				message,
				`已打开作品记忆 · ${activeMemories.length} 条\n这些记忆会由写作上下文分层读取；候选记忆仍需你确认后才会持久化。`,
			);
			return true;
		}
		...
		if (command.name === "chapters") {
			appendLocalCommandResult(
				...
				chapters
					.map(
						(chapter, index) =>
							`${String(index + 1).padStart(2, "0")}　${String(chapter.id) === String(displayChapter.id) ? "● " : ""}${chapter.title}　${chapter.words || 0} 字`,
					)
					.join("\n") || "当前作品还没有章节。",
			);
			return true;
		...
		if (command.name === "projects") {
			appendLocalCommandResult(
				...
				projects
					.map(
						(item, index) =>
							`${String(index + 1).padStart(2, "0")}　${String(item.id) === String(project.id) ? "● " : ""}${item.title}　${item.type} · ${item.genre}`,
					)
					.join("\n") || "当前还没有作品。",
			);
			return true;
			...
			appendLocalCommandResult(
				...
				storySkills.length
					? storySkills
							.map(
								skill =>
									`${skill.status === "ready" ? "●" : "○"} ${skill.displayName || skill.name}　${skill.description || skill.status}`,
							)
							.join("\n")
		...
		if (command.name === "tasks") {
			...
			appendLocalCommandResult(
				...
				tasks.length
					? tasks
							.slice(0, 20)
							.map(task => `${task.id}　${task.status}　${task.skill || "story"}\n${task.message || ""}`)
							.join("\n\n")
					: "当前作品还没有 Agent 任务。",
			);
			return true;
		...
		if (command.name === "retry") {
			const fallbackTask = [...assistantMessages]
				.reverse()
				.find(item => item.role === "agent" && ["failed", "cancelled"].includes(item.status) && item.taskId);
		...
		if (command.name === "draft") {
			appendLocalCommandResult(
				message,
				draft
					? `${draft.slice(0, 2400)}${draft.length > 2400 ? `\n\n…（共 ${draft.length} 字符）` : ""}`
					: "当前章节还没有正文。",
			);
			return true;
		}
		if (command.name === "history") {
			...
			appendLocalCommandResult(
				message,
				history.length
					? history.map((item, index) => `${index + 1}. ${item.text}`).join("\n")
					: "当前还没有对话历史。",
			);
			return true;
		}
		...
		try {
			...
			if (error?.name !== "AbortError") {
				setAssistantMessages(current =>
					current.map(item =>
						item.id === requestId
							? { ...item, status: "failed", text: error.message || "Agent 回答提交失败。" }
							: item,
					),
				);
			}
				...
				setAssistantMessages(current => {
					...
					if (hasServerMessage) {
						return current
							...
							.map(item =>
								item.id === response.input.id ? { ...item, steerStatus: response.input.status } : item,
							);
					}
	...
	async function submitAssistant(event, quickMessage = "") {
		...
		const command =
			slashCommand?.name === "plan"
				? {
						message:
							slashCommand.argument || "分析当前章节与作品工作区，给出分步骤执行方案，暂不写入或修改数据。",
		...
		const startedAt = performance.now();
		const attachedFiles = assistantAttachments.map(({ key, name, kind, content, reference }) => ({
			key,
			name,
			kind,
			content,
		...
		try {
			...
			const created = await api.createAgentTurn(
				thread.id,
				{
			...
			if (assistantAbortRef.current === controller) {
				assistantAbortRef.current = null;
				if (assistantTaskIdRef.current && assistantTaskIdRef.current === createdTaskId)
					assistantTaskIdRef.current = null;
	...
	function applyStreamedTask(task, requestId, startedAt, turn = null) {
		...
		setAssistantMessages(current =>
			current.map(item =>
				item.role === "agent" && (item.id === requestId || item.taskId === task.id)
					? {
							...item,
							taskId: task.id,
							...
							status: terminal
								...
								: task.status,
							response:
								response === null && ["queued", "running"].includes(task.status)
									? null
									: response || item.response,
							text: terminal
								? task.status === "completed"
									? agentResponseText(response)
									: task.error || task.statusMessage || "任务未完成。"
								: typeof task.partialOutput === "string" &&
										task.partialOutput.length >= String(item.text || "").length
									? task.partialOutput
									: item.text,
							...
							reasoningSummary: terminal
								? typeof task.reasoningSummary === "string"
									? task.reasoningSummary
									: item.reasoningSummary || ""
								: typeof task.reasoningSummary === "string" &&
										task.reasoningSummary.length >= String(item.reasoningSummary || "").length
									? task.reasoningSummary
									: item.reasoningSummary || "",
							reasoningHistory: Array.isArray(task.reasoningHistory)
								? task.reasoningHistory
								: item.reasoningHistory || [],
			...
			await api.streamAgentTurn(threadId, turnId, {
				...
				onEvent: (event, payload) => {
					if (["item/started", "item/completed"].includes(event) && payload?.turnId === turnId && payload.item) {
						setAssistantMessages(current =>
							current.map(item =>
								item.role === "agent" && (item.id === requestId || item.turnId === turnId)
									? {
											...item,
					...
					if (
						["item/agentMessage/delta", "item/plan/delta"].includes(event) &&
	...
	function stopAssistant() {
		...
		setAssistantMessages(current =>
			current.map(item =>
				item.role === "agent" && item.taskId === taskId
					? {
							...item,
							status: "cancelled",
							text: "已停止本次运行。",
	...
	async function clearAssistant() {
		...
		if (assistantThread?.id && activeTurnId)
			void api.interruptAgentTurn(assistantThread.id, activeTurnId).catch(() => undefined);
		else if (taskId) void api.cancelAiTask(taskId).catch(() => undefined);
		assistantAbortRef.current?.abort();
		assistantAbortRef.current = null;
		assistantTaskIdRef.current = null;
	...
	const characterIdeas = sortIdeas(
		matchedIdeas.filter(idea => idea.folder === "人物" || /人物|角色|主角|配角/.test(`${idea.label}`)),
	);
	const termIdeas = sortIdeas(
		matchedIdeas.filter(
			idea =>
				!outlineIdeas.some(outline => outline.id === idea.id) &&
				!characterIdeas.some(character => character.id === idea.id) &&
				(["世界观", "设定", "追踪"].includes(idea.folder) ||
					/词条|设定|世界|地点|规则|关系|追踪|时间线/.test(`${idea.label}${idea.title}`)),
		),
	);
	...
	const agentContextUsed = Math.max(
		0,
		latestAgentUsage
	...
	const agentSkillCommands = skills
		...
		.map(skill => ({
			...
			description:
				skill.source === "market"
					? `社区 Skill · ${skill.displayName || skill.name}`
					: skill.description || "运行这个 Story Skill",
	...
	const filteredAgentCommands = [...editorAgentCommands, ...agentSkillCommands]
		.filter(
			command =>
				!suggestionQuery ||
				command.name.includes(suggestionQuery) ||
				command.description.includes(suggestionQuery) ||
				command.aliases?.some(alias => alias.includes(suggestionQuery)),
		)
		.slice(0, 40);
	const filteredAgentFiles = agentFileOptions
		.filter(
			file =>
				!suggestionQuery || `${file.name} ${file.kind} ${file.description}`.toLowerCase().includes(suggestionQuery),
		)
		.slice(0, 40);
	const anchorIdeas = matchedIdeas
		.filter(
			idea =>
				idea.pinned || /剧情|冲突|场景|线索|锚点/.test(`${idea.label}${idea.title}${(idea.tags || []).join("")}`),
		)
		.slice(0, 4);
	const searchCount = searchQuery.trim()
		? (draft.toLowerCase().match(new RegExp(searchQuery.trim().replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "gi")) || [])
				.length
		: 0;
	const splitBefore = draft.slice(0, splitPosition).trim();
	const splitAfter = draft.slice(splitPosition).trim();
	const editorTabs = openChapterIds
		.map(id => chapters.find(chapter => String(chapter.id) === String(id)))
		.filter(Boolean);
	...
	return (
		<>
			<div
				className={`page editor-page ${readingMode ? "reading-mode" : ""} ${assistantOpen ? "" : "assistant-hidden"}`}
			>
				<div className="editor-document-tabs" aria-label="已打开章节">
					{editorTabs.map(chapter => {
						...
						return (
							<div className={`editor-document-tab ${current ? "active" : ""}`} key={chapter.id}>
								<button
									type="button"
									className="editor-document-tab-main"
									onClick={() => selectEditorChapter(chapter)}
									disabled={draftLoading && !current}
								>
									<FileText size={13} />
								...
								<button
									...
									title="关闭标签"
									onClick={event => closeEditorTab(event, chapter)}
								>
									<X size={12} />
								</button>
							</div>
				...
				<div className="editor-topline">
					<button
						type="button"
						className="mobile-chapter-button icon-button"
						aria-label="打开章节目录"
						title="打开章节目录"
					...
					{lastAiRestore && String(lastAiRestore.chapterId) === String(displayChapter.id) && (
						<button
							className="ai-restore-button"
							onClick={() => {
								commitDraftChange(lastAiRestore.content);
								onAiRestored?.();
								onNotify("已恢复应用 AI 修改前的正文");
							}}
						>
							<Undo2 size={14} />
							恢复 AI 修改前正文
					...
					<button
						...
						onClick={() => draftStatus === "error" && onSave()}
						title={draftStatus === "error" ? "点击重试保存" : statusText}
					>
						<span className={draftStatus === "saved" ? "saved-dot" : "unsaved-dot"} />
						{draftLoading ? "正在载入章节" : statusText}
					</button>
					<div className="editor-actions">
						<button className="icon-button" aria-label="导出本章" title="导出本章" onClick={exportChapter}>
							<Download size={17} />
						</button>
						<button
							...
							aria-label="导出全书"
							title="导出全书"
							disabled={exportingBook || draftLoading}
							onClick={exportProject}
						>
							{exportingBook ? <LoaderCircle size={17} className="spin" /> : <BookOpen size={17} />}
						</button>
						<button
							className="dark-button save-button"
							disabled={draftLoading || saveBusy}
							onClick={() => onSave()}
						>
							{saveBusy ? <LoaderCircle size={16} className="spin" /> : <Check size={16} />}
							{saveBusy ? "保存中" : "保存"}
						</button>
					</div>
				</div>
				...
				<div
					...
					ref={editorLayoutRef}
					style={{
						"--chapter-panel-width": `${workspaceWidths.chapter}px`,
						"--assistant-panel-width": `${workspaceWidths.assistant}px`,
					}}
				>
					{mobileRailOpen && (
						<button
					...
					<aside className={`chapter-rail ${mobileRailOpen ? "mobile-open" : ""}`}>
						<div className="rail-tabs" role="tablist" aria-label="作品资料">
							{[
								...
								<button
									key={label}
									role="tab"
									aria-selected={railTab === label}
									className={railTab === label ? "active" : ""}
									onClick={() => setRailTab(label)}
								>
									<Icon size={13} />
									<span>{label}</span>
								</button>
							...
							<div className="rail-header-actions">
								{railTab === "目录" && (
									<>
										...
										<button
											className={`icon-button small ${unfinishedOnly ? "active" : ""}`}
											aria-label="只看未完成章节"
											title={unfinishedOnly ? "显示全部章节" : "只看未完成章节"}
										...
										<button
											className="icon-button small"
											aria-label="切换章节排序"
											title={chapterOrder === "asc" ? "当前正序，点击倒序" : "当前倒序，点击正序"}
									...
									onClick={() =>
										railTab === "目录"
											...
											: railTab === "伏笔"
												...
												: railTab === "记忆"
													? onNotify("记忆可让夜雨在对话中整理，或在 CLI 里管理")
													: setIdeaPickerOpen(true)
									}
								>
									{railTab === "记忆" && memoryLoading ? (
										<LoaderCircle size={16} className="spin" />
									) : (
										<Plus size={16} />
									)}
								...
								<button
									type="button"
									className="icon-button small mobile-rail-close"
									aria-label="关闭章节目录"
									onClick={() => setMobileRailOpen(false)}
								>
									<X size={16} />
						...
						{railTab === "目录" && (
							<div className="chapter-list">
								{visibleChapters.map(chapter => {
									const current = String(chapter.id) === String(displayChapter.id);
									return (
										<div key={chapter.id} className={`chapter-item-wrap ${current ? "current" : ""}`}>
											<button
												className={`chapter-item ${current ? "current" : ""}`}
												onClick={() => selectEditorChapter(chapter)}
												disabled={draftLoading && !current}
											>
												...
												<span className="chapter-words">
													{chapter.words} 字 · {chapter.state === "done" ? "已完成" : "草稿"}
												</span>
											</button>
											<button
											...
											{menuOpenId === chapter.id && (
												<div className="chapter-menu" role="menu">
													<button onClick={() => startRename(chapter)}>重命名</button>
													<button onClick={() => void toggleChapterCompletion(chapter)}>
														{chapter.state === "done" ? "恢复为草稿" : "标记为完成"}
													</button>
													<button
														className="danger"
														onClick={() => {
						...
						{railTab === "大纲" && (
							<div className="rail-outline-list">
								{outlineEntries.length ? (
									outlineEntries.map((item, index) => (
										<button key={item.id} onClick={() => setMaterialViewing(item)}>
											<span>{String(index + 1).padStart(2, "0")}</span>
											<strong>{item.title}</strong>
											<small>{item.label || "大纲"}</small>
										</button>
									))
								) : (
									<div className="rail-empty-block">
										<BookMarked size={22} />
										<p>还没有已落库的大纲</p>
										<small>让 Agent 生成大纲并确认写入后会显示在这里。</small>
									</div>
								)}
							</div>
						)}
						{railTab === "人物" && (
							<div className="rail-entity-list">
								{characterIdeas.length ? (
									characterIdeas.map(idea => (
										<button key={idea.id} onClick={() => setMaterialViewing(idea)}>
											<span className="entity-dot coral" />
											<span>
												<strong>{idea.title}</strong>
												<small>{idea.body.slice(0, 42)}</small>
											</span>
										</button>
									))
								) : (
									<div className="rail-empty-block">
										<UsersRound size={22} />
										<p>还没有人物卡</p>
										<button onClick={() => setIdeaPickerOpen(true)}>从素材库添加</button>
									</div>
								)}
							</div>
						)}
						{railTab === "设定" && (
							<div className="rail-entity-list">
								{termIdeas.length ? (
									termIdeas.map(idea => (
										<button key={idea.id} onClick={() => setMaterialViewing(idea)}>
											<span className="entity-dot teal" />
											<span>
												<strong>{idea.title}</strong>
												<small>{idea.body.slice(0, 42)}</small>
											</span>
						...
						{railTab === "文件" && (
							<div className="rail-file-list">
								{storyFileIdeas.length ? (
									storyFileIdeas.map(idea => (
										<div className="rail-file-item" key={idea.id}>
											<button
												type="button"
												className="rail-file-open"
												onClick={() => setMaterialViewing(idea)}
											>
												<FileText size={14} />
												<span>
													<strong>{idea.title}</strong>
													<small>{storyFilePath(idea)}</small>
												</span>
												<ChevronRight size={12} />
											</button>
											<button
												type="button"
												className="rail-file-delete"
												aria-label={`删除文件 ${idea.title}`}
												title="删除文件"
												onClick={() => setFileDeleteTarget(idea)}
											>
												<Trash2 size={13} />
											</button>
										</div>
									...
									<div className="rail-empty-block">
										<FolderOpen size={22} />
										<p>还没有作品文件</p>
										<small>让 Agent 创建设定、大纲或资料文件后会显示在这里。</small>
									</div>
								)}
							</div>
						)}
						{railTab === "记忆" && (
							<div className="rail-memory-list">
								{storyMemories.filter(item => item.status !== "archived").length ? (
									storyMemories
										.filter(item => item.status !== "archived")
										.map(memory => (
											<button
												key={memory.id}
												className="rail-memory-item"
												onClick={() => setMemoryEditing(memory)}
											>
												<span className={`memory-type-dot ${memory.type}`} />
												<span>
													<strong>{memory.title}</strong>
													<small>
														{memory.characterName ? `${memory.characterName} · ` : ""}
														{memory.content.slice(0, 46)}
													</small>
												</span>
												<em>{memory.importance || 3}</em>
											</button>
										))
								) : (
									<div className="rail-empty-block">
										...
										<p>还没有确认的作品记忆</p>
										<button onClick={() => onNotify("记忆可让夜雨在对话中整理，或手动点击下方条目编辑")}>
											如何整理？
										</button>
									</div>
								)}
						...
						{railTab === "伏笔" && (
							<div className="rail-foreshadow-list">
								{projectForeshadows.length ? (
									projectForeshadows.map(item => (
										<button
											key={item.id}
											className="rail-foreshadow-item"
											onClick={() => openForeshadowEditor(item)}
										>
											<span className={`foreshadow-status-dot ${item.status}`} />
											<span className="rail-foreshadow-copy">
												<strong>{item.title}</strong>
												<small>
													{item.category || "未分类"} ·{" "}
											...
											<span className="foreshadow-importance" title={`重要性 ${item.importance || 3}`}>
												{item.importance || 3}
											</span>
										</button>
									))
									...
									<div className="rail-empty-block">
										<Pin size={22} />
										<p>还没有登记伏笔</p>
										<button onClick={() => openForeshadowEditor()}>新增第一个伏笔</button>
									</div>
								)}
							</div>
						)}
						{railTab === "目录" && (
							<button className="outline-link" onClick={() => setOutlineOpen(true)}>
								<List size={15} />
								打开完整大纲
							</button>
						)}
					...
					<div
						className="workspace-resizer chapter-resizer"
						role="separator"
						tabIndex={0}
						aria-label="调整章节栏宽度"
						...
						aria-valuemin="220"
						aria-valuemax="420"
						aria-valuenow={workspaceWidths.chapter}
						onKeyDown={event => nudgeWorkspace("chapter", event)}
						onPointerDown={event => startWorkspaceResize("chapter", event)}
					...
					<section className="writing-canvas">
						<div className="writing-toolbar">
							...
							<div className="toolbar-group">
								<div className="editor-style-anchor">
									<button
										className={`toolbar-button ${styleMenuOpen || editorFontSize !== 16 || editorLineHeight !== 2.05 ? "active" : ""}`}
										aria-label="正文样式"
									...
									{styleMenuOpen && (
										<div className="editor-style-menu" role="dialog" aria-label="正文样式设置">
											<div className="editor-style-menu-heading">
												<strong>正文样式</strong>
												<button
													type="button"
													aria-label="关闭正文样式"
											...
											<div className="editor-style-options">
												{[
													...
													<button
														type="button"
														key={option.value}
														className={editorFontSize === option.value ? "active" : ""}
														onClick={() => {
															setEditorFontSize(option.value);
															onNotify(`正文字号已切换为${option.label}`);
														}}
													>
														{option.label}
													</button>
											...
											<div className="editor-style-options">
												{[
													...
													<button
														type="button"
														key={option.value}
														className={editorLineHeight === option.value ? "active" : ""}
														onClick={() => {
															setEditorLineHeight(option.value);
															onNotify(`正文行距已切换为${option.label}`);
														}}
													>
														{option.label}
													</button>
											...
											{/\*\*[^*\n]+\*\*|_[^_\n]+_/.test(draft) && (
												<button
													type="button"
													className="editor-clean-markers"
													onClick={cleanInlineMarkdownMarkers}
												>
													<Wand2 size={13} />
													清理正文中的 ** / _ 标记
												</button>
								...
								<button
									className={`toolbar-button ${editorBold ? "active" : ""}`}
									aria-pressed={editorBold}
									aria-label="加粗显示"
									title="整篇加粗显示，不修改正文（Ctrl+B）"
									onClick={toggleEditorBold}
								>
									<strong>B</strong>
								</button>
								<button
									className={`toolbar-button ${editorItalic ? "active" : ""}`}
									aria-pressed={editorItalic}
									aria-label="斜体显示"
									title="整篇斜体显示，不修改正文（Ctrl+I）"
									onClick={toggleEditorItalic}
								>
									<Italic size={16} />
								</button>
							</div>
							<span className="toolbar-divider" />
							<div className="toolbar-group">
								<button className="toolbar-button" aria-label="复制全文" title="复制全文" onClick={copyChapter}>
									<Copy size={15} />
								</button>
								<button
									className="toolbar-button"
							...
							<div className="toolbar-group">
								<button
									...
									title="高频词分析"
									onClick={() =>
										frequentWords.length ? setFrequencyOpen(true) : onNotify("当前正文还没有可分析的重复词")
									}
								>
									<Highlighter size={15} />
								</button>
								...
								<button
									className={`toolbar-button ${speaking ? "active speaking" : ""}`}
									aria-label={speaking ? "停止朗读" : "朗读本章"}
									title={speaking ? "停止朗读" : "朗读本章"}
							...
							<div className="toolbar-group toolbar-group-end">
								<button
									className={`toolbar-button ${readingMode ? "active" : ""}`}
									aria-label={readingMode ? "退出阅读模式" : "阅读模式"}
									title={readingMode ? "退出阅读模式" : "阅读模式"}
									onClick={() => setReadingMode(mode => !mode)}
								>
									{readingMode ? <Minimize2 size={15} /> : <Maximize2 size={15} />}
								</button>
								<button
									className="toolbar-button danger-toolbar"
									aria-label="清空本章"
									title="清空本章"
						...
						{searchOpen && (
							<div className="editor-searchbar">
								...
								<input
									...
									aria-label="搜索本章正文"
								/>
								<span>{searchCount ? `${searchCount} 处` : searchQuery ? "未找到" : "输入关键词"}</span>
								<button
									className="icon-button small"
									aria-label="上一个搜索结果"
									title="上一个"
						...
						<div className="writing-body">
							...
							<header className="manuscript-heading">
								<span>
									{project?.title} · 第 {displayChapter.id} 章
								</span>
							...
							<div className="writing-footer">
								...
								<span className="footer-hint">Ctrl / ⌘ + S 保存 · Z 撤销</span>
							</div>
						</div>
					</section>

					<div
						className="workspace-resizer assistant-resizer"
						role="separator"
						tabIndex={0}
						aria-label="调整助手栏宽度"
						aria-orientation="vertical"
						aria-valuemin="320"
						aria-valuemax="520"
						aria-valuenow={workspaceWidths.assistant}
					...
					<aside className={`insight-rail agent-rail yemu-agent-rail ${assistantOpen ? "" : "collapsed"}`}>
						{assistantOpen ? (
							<AssistantPanel
								projectId={project?.id}
								projectTitle={project?.title || ""}
								chapterTitle={displayChapter.title}
								chapterFile={displayChapter.fileName || displayChapter.id}
							...
							<button
								...
								aria-label={`展开${ASSISTANT_NAME}`}
								title={`展开${ASSISTANT_NAME}`}
				...
				>
					<div className="modal" role="dialog" aria-modal="true">
						<div className="modal-heading">
							<div>
								<span className="section-overline">章节操作</span>
								<h2>清空本章正文？</h2>
							</div>
							<button className="icon-button" aria-label="关闭" onClick={() => setClearOpen(false)}>
								<X size={18} />
							</button>
						</div>
						<p className="confirm-text">正文会先进入撤销栈，确认后仍可点击撤销恢复。</p>
						<div className="modal-actions">
							<button type="button" className="secondary-button" onClick={() => setClearOpen(false)}>
								取消
							</button>
							<button
								type="button"
				...
				>
					<div className="modal" role="dialog" aria-modal="true">
						<div className="modal-heading">
							...
							<button className="icon-button" aria-label="关闭" onClick={() => setRenameTarget(null)}>
								<X size={18} />
							</button>
						</div>
						<form onSubmit={confirmRename}>
							...
							<div className="modal-actions">
								<button type="button" className="secondary-button" onClick={() => setRenameTarget(null)}>
									取消
								</button>
			...
			{deleteTarget && (
				...
				>
					<div className="modal" role="dialog" aria-modal="true">
						<div className="modal-heading">
							<div>
								<span className="section-overline">删除章节</span>
								<h2>确认删除</h2>
							</div>
							<button className="icon-button" aria-label="关闭" onClick={() => setDeleteTarget(null)}>
								<X size={18} />
							</button>
						</div>
						<p className="confirm-text">确定删除《{deleteTarget.title}》吗？此操作不可撤销。</p>
						<div className="modal-actions">
							<button type="button" className="secondary-button" onClick={() => setDeleteTarget(null)}>
								取消
							</button>
							<button type="button" className="dark-button danger-button" onClick={confirmDelete}>
								删除
							</button>
						</div>
					</div>
			...
			{fileDeleteTarget && (
				<div
					...
					role="presentation"
					onMouseDown={event =>
						event.target === event.currentTarget && !fileDeleteLoading && setFileDeleteTarget(null)
					}
				>
					<div className="modal" role="dialog" aria-modal="true" aria-labelledby="delete-story-file-title">
						<div className="modal-heading">
							<div>
								<span className="section-overline">删除作品文件</span>
								<h2 id="delete-story-file-title">确认删除</h2>
							</div>
							<button
								className="icon-button"
								aria-label="关闭"
						...
						<p className="confirm-text">
							即将删除《{fileDeleteTarget.title}》（{storyFilePath(fileDeleteTarget)}）。删除后它会同时从
							PostgreSQL 和 Agent 工作区中移除，此操作不可恢复。
						</p>
						<div className="modal-actions">
							<button
								type="button"
							...
							>
								{fileDeleteLoading ? <LoaderCircle size={16} className="spin" /> : <Trash2 size={16} />}
								{fileDeleteLoading ? "删除中" : "永久删除"}
							</button>
						</div>
					</div>
				</div>
			...
			{outlineOpen && (
				<div
					className="modal-backdrop"
					role="presentation"
					onMouseDown={event => event.target === event.currentTarget && setOutlineOpen(false)}
				>
					<div className="modal outline-modal" role="dialog" aria-modal="true">
						<div className="modal-heading">
							<div>
								<span className="section-overline">{project?.title}</span>
								<h2>章节大纲</h2>
							</div>
							<button className="icon-button" aria-label="关闭" onClick={() => setOutlineOpen(false)}>
								<X size={18} />
							</button>
						</div>
						{chapters.length ? (
							<ol className="outline-list">
								{chapters.map(chapter => (
									<li key={chapter.id}>
										<span className="outline-num">{String(chapter.id).padStart(2, "0")}</span>
										<button
										...
										<span className={`outline-state ${chapter.state === "done" ? "done" : ""}`}>
											{chapter.state === "done" ? "已完成" : "草稿"}
										</span>
										<span className="outline-words">{chapter.words} 字</span>
									</li>
								))}
							</ol>
						) : (
			...
			{historyOpen && (
				...
				>
					<div className="modal history-modal" role="dialog" aria-modal="true">
						<div className="modal-heading">
							<div>
								<span className="section-overline">编辑器</span>
								<h2>历史记录</h2>
							</div>
							<button className="icon-button" aria-label="关闭历史记录" onClick={() => setHistoryOpen(false)}>
								<X size={18} />
							</button>
						</div>
						<p className="modal-subtitle">选择一个快照恢复到正文，当前内容会保留在重做栈中。</p>
						<div className="history-list">
							{historyLoading ? (
								...
								historyRef.current.past
									...
									.reverse()
									.map((snapshot, index) => {
										const version = historyRef.current.past.length - index;
										const preview = snapshot.replace(/\s+/g, " ").trim();
										return (
											<button
												type="button"
												className="history-item"
												key={`${version}-${index}`}
												onClick={() => restoreHistory(snapshot)}
											>
												<span className="history-item-version">版本 {version}</span>
												<span className="history-item-copy">
													<strong>{formatNumber(snapshot.replace(/\s/g, "").length)} 字</strong>
													<small>{preview || "（空正文）"}</small>
												</span>
												<ChevronRight size={16} />
											</button>
										);
								...
								<div className="history-empty">
									<History size={22} />
									<p>还没有可恢复的编辑快照</p>
									<small>继续输入一会儿，历史记录会自动生成。</small>
								</div>
							)}
						</div>
						<div className="modal-actions">
							<button type="button" className="secondary-button" onClick={() => setHistoryOpen(false)}>
								关闭
							</button>
						</div>
					</div>
			...
			{frequencyOpen && (
				<div
					className="modal-backdrop"
					role="presentation"
					onMouseDown={event => event.target === event.currentTarget && setFrequencyOpen(false)}
				>
					<div className="modal frequency-modal" role="dialog" aria-modal="true">
						<div className="modal-heading">
							<div>
								<span className="section-overline">正文分析</span>
								<h2>高频词</h2>
							</div>
							<button className="icon-button" aria-label="关闭高频词" onClick={() => setFrequencyOpen(false)}>
								<X size={18} />
							</button>
						</div>
						<p className="modal-subtitle">
							统计当前章节中重复出现的词语，点击词语可回到正文定位第一次出现的位置。
						</p>
						<div className="frequency-cloud">
							{frequentWords.map(([word, count]) => (
								<button
									type="button"
									className="frequency-chip"
									key={word}
									onClick={() => locateFrequentWord(word)}
								>
									<strong>{word}</strong>
									<small>{count} 次</small>
								</button>
						...
						<div className="modal-actions">
							<button type="button" className="secondary-button" onClick={() => setFrequencyOpen(false)}>
								关闭
							</button>
						</div>
					</div>
			...
			{splitOpen && (
				<div
					className="modal-backdrop"
					role="presentation"
					onMouseDown={event => event.target === event.currentTarget && !splitLoading && setSplitOpen(false)}
				>
					<div className="modal split-modal" role="dialog" aria-modal="true">
						<div className="modal-heading">
							<div>
								<span className="section-overline">本地章节操作</span>
								<h2>按光标拆章</h2>
							</div>
							<button
								className="icon-button"
						...
						<p className="modal-subtitle">
							以当前光标位置为分界，在本地拆分正文并保存为新章节；此操作不调用 AI。
						</p>
						<form onSubmit={confirmSplit}>
							...
							<div className="split-preview">
								<div>
									<span>当前章节保留</span>
									<strong>{formatNumber(splitBefore.length)} 字</strong>
									<p>{splitBefore.slice(-90) || "拆分位置之前暂无正文"}</p>
								</div>
								<ChevronRight size={17} />
								<div>
									<span>新章节内容</span>
									<strong>{formatNumber(splitAfter.length)} 字</strong>
									<p>{splitAfter.slice(0, 90) || "拆分位置之后暂无正文"}</p>
								</div>
							</div>
							<div className="modal-actions">
								<button
								...
								<button type="submit" className="dark-button" disabled={splitLoading || !splitTitle.trim()}>
									{splitLoading ? <LoaderCircle size={16} className="spin" /> : <Split size={16} />}
									{splitLoading ? "拆分中" : "确认拆分"}
								</button>
							</div>
						</form>
					</div>
			...
			{memoryEditing && (
				<MemoryEditModal
					memory={memoryEditing}
					onClose={() => setMemoryEditing(null)}
					onSave={async updates => {
						const saved = await onUpdateStoryMemory?.(memoryEditing, updates);
						if (saved) setMemoryEditing(null);
					}}
					onDelete={async () => {
						const deleted = await onDeleteStoryMemory?.(memoryEditing);
						if (deleted) setMemoryEditing(null);
					}}
				/>
			)}
			{foreshadowOpen && (
				<ForeshadowModal
					project={project}
					chapters={chapters}
					target={foreshadowTarget}
					onClose={() => {
						setForeshadowOpen(false);
						setForeshadowTarget(null);
					}}
					onCreate={onCreateForeshadow}
					onUpdate={onUpdateForeshadow}
					onDelete={onDeleteForeshadow}
				/>
			)}
			{materialViewing && (
				<MaterialDetailModal
...
function MemoryReviewModal({ candidates, loading, onChange, onClose, onSave }) {
	return (
		<div
			className="modal-backdrop memory-backdrop"
			role="presentation"
		...
		>
			<div className="modal memory-review-modal" role="dialog" aria-modal="true">
				<div className="modal-heading">
					<div>
						<span className="section-overline">夜雨 · 作品记忆</span>
						<h2>确认本章长期事实</h2>
						<p className="modal-subtitle">以下只是候选项，勾选并修订后才会进入作品记忆。</p>
					</div>
					<button className="icon-button" disabled={loading} onClick={onClose}>
						<X size={18} />
					</button>
				</div>
				<div className="memory-candidate-list">
					{candidates.map(candidate => (
						<article className={`memory-candidate ${candidate.selected ? "selected" : ""}`} key={candidate.id}>
							<label className="memory-candidate-toggle">
								<input
									type="checkbox"
									checked={candidate.selected}
									onChange={event => onChange(candidate.id, "selected", event.target.checked)}
								/>
								<span>{memoryTypeLabels[candidate.type] || candidate.type}</span>
								<small>重要性 {candidate.importance || 3}</small>
							</label>
							<div className="form-row">
								<label>
									...
									<select
										...
										onChange={event => onChange(candidate.id, "type", event.target.value)}
									>
							...
							<label>
								...
								<textarea
									...
									onChange={event => onChange(candidate.id, "content", event.target.value)}
								/>
							</label>
							<div className="form-row">
								<label>
									角色
									<input
										value={candidate.character_name || ""}
										maxLength={80}
										onChange={event => onChange(candidate.id, "character_name", event.target.value)}
										placeholder="可选"
									/>
								</label>
								<label>
									重要性
									<select
										value={candidate.importance || 3}
										onChange={event => onChange(candidate.id, "importance", Number(event.target.value))}
									>
										{[1, 2, 3, 4, 5].map(value => (
											<option value={value} key={value}>
												{value}
				...
				<div className="modal-actions">
					<button className="secondary-button" disabled={loading} onClick={onClose}>
						取消
					</button>
					<button
						className="dark-button"
						disabled={loading || !candidates.some(item => item.selected)}
						onClick={onSave}
					>
						{loading ? <LoaderCircle size={16} className="spin" /> : <Check size={16} />}
						{loading ? "写入中" : `确认 ${candidates.filter(item => item.selected).length} 条`}
					</button>
				</div>
			</div>
		</div>
...
function MemoryEditModal({ memory, onClose, onSave, onDelete }) {
	...
	return (
		...
		>
			<div className="modal memory-edit-modal" role="dialog" aria-modal="true">
				<div className="modal-heading">
					<div>
						<span className="section-overline">作品记忆</span>
						<h2>编辑长期事实</h2>
					</div>
				...
				<div className="form-row">
					...
					<label>
						重要性
						<select value={form.importance} onChange={event => update("importance", Number(event.target.value))}>
							{[1, 2, 3, 4, 5].map(value => (
								<option value={value} key={value}>
									{value}
								</option>
				...
				<label>
					标题
					<input value={form.title} maxLength={160} onChange={event => update("title", event.target.value)} />
				</label>
				<label>
					内容
					<textarea
				...
				<div className="modal-actions">
					<button className="danger-text-button" onClick={onDelete}>
						<Trash2 size={15} />
						删除
					</button>
					<button
						className="secondary-button"
						onClick={() => onSave({ status: form.status === "archived" ? "active" : "archived" })}
					>
						{form.status === "archived" ? "恢复" : "归档"}
					</button>
					<button className="dark-button" onClick={() => onSave(form)}>
						<Check size={15} />
						保存
					</button>
				</div>
...
function ForeshadowModal({ project, chapters, target, onClose, onCreate, onUpdate, onDelete }) {
	...
	function update(field, value) {
		setForm(current => ({
			...current,
			[field]: value,
			...(field === "status" && value !== "resolved" ? { resolvedChapterId: "" } : {}),
		}));
	}
	...
	async function submit(event) {
		...
		const payload = {
			...
			plantChapterId: form.plantChapterId || null,
			targetChapterId: form.targetChapterId || null,
	...
	return (
		<div
			className="modal-backdrop foreshadow-backdrop"
			role="presentation"
			onMouseDown={event => event.target === event.currentTarget && !loading && onClose()}
		>
			<div
				className="modal foreshadow-modal"
				role="dialog"
				aria-modal="true"
				aria-labelledby="foreshadow-modal-title"
			>
				<div className="modal-heading">
					<div>
						<span className="section-overline">{project?.title || "作品"} · 叙事管理</span>
						<h2 id="foreshadow-modal-title">{editing ? "编辑伏笔" : "登记伏笔"}</h2>
					</div>
					<button
						className="icon-button"
						aria-label="关闭伏笔编辑"
				...
				<p className="modal-subtitle">
					记录线索如何埋下、准备在哪一章回收，{ASSISTANT_NAME}会自动把未回收伏笔加入章节上下文。
				</p>
				<form onSubmit={submit}>
					...
					<label>
						...
						<textarea
							...
							maxLength={2000}
							rows={4}
							placeholder="描述读者能看到的线索，以及它最终指向什么。"
						/>
					</label>
					<div className="form-row">
						<label>
							...
							<select value={form.status} onChange={event => update("status", event.target.value)}>
								{Object.entries(foreshadowStatusLabels).map(([value, label]) => (
						...
						<label>
							...
							<select value={form.importance} onChange={event => update("importance", event.target.value)}>
								{[1, 2, 3, 4, 5].map(value => (
									<option key={value} value={value}>
										{value} · {value >= 4 ? "关键线索" : value === 3 ? "普通线索" : "轻量线索"}
									</option>
								))}
							</select>
						</label>
					...
					<div className="form-row">
						<label>
							埋入章节
							<select
								value={form.plantChapterId}
								onChange={event => update("plantChapterId", event.target.value)}
							>
						...
						<label>
							计划回收章节
							<select
								value={form.targetChapterId}
								onChange={event => update("targetChapterId", event.target.value)}
							>
								<option value="">暂不指定</option>
								{chapters.map(chapter => (
									<option key={chapter.id} value={chapter.id}>
					...
					<label>
						...
						<select
							...
							onChange={event => update("resolvedChapterId", event.target.value)}
						>
					...
					{error && (
						<div className="skill-runner-validation" role="alert">
							{error}
						</div>
					)}
					<div className="modal-actions">
						...
						{editing && (
							<button type="button" className="danger-text-button" disabled={loading} onClick={remove}>
								<Trash2 size={15} />
								删除
							</button>
						)}
...
function Works({ projects, onOpen, onNew, onEdit, onDelete, onImport }) {
	...
	const statusCounts = {
		全部: projects.length,
		进行中: projects.filter(project => project.status !== "已完结").length,
		已完结: projects.filter(project => project.status === "已完结").length,
	};
	const filtered = projects.filter(project => {
		const typeMatch = typeFilter === "全部" || project.type === typeFilter;
		const statusMatch =
			statusFilter === "全部" ||
			(statusFilter === "已完结" ? project.status === "已完结" : project.status !== "已完结");
	...
	return (
		<>
			<div className="page inner-page">
				...
				<div className="works-toolbar works-toolbar-expanded">
				...
				{filtered.length ? (
					<div className="works-list">
						{filtered.map(project => (
							<div className="work-row-wrap" key={project.id}>
								<button className="work-row" onClick={() => onOpen(project)}>
									<div className={`row-cover ${project.cover}`}>
										<span>{project.title.slice(0, 1)}</span>
									</div>
									<div className="row-main">
										<div className="row-title">
											<h3>{project.title}</h3>
											<span className="muted-tag">{project.type}</span>
										</div>
										<p>
									...
									<div className="row-updated">
										<span>最近编辑</span>
										<strong>{formatRelativeTime(project.updatedAt, project.updated)}</strong>
									</div>
									<ChevronRight size={18} className="row-arrow" />
								</button>
								<button
									className="work-row-menu"
									aria-label="作品操作"
									title="作品操作"
									onClick={event => {
										event.stopPropagation();
								...
								{menuOpenId === project.id && (
									<div className="chapter-menu work-menu" role="menu">
										<button
											onClick={() => {
												onEdit(project);
												setMenuOpenId(null);
			...
			{deleteTarget && (
				<div
					className="modal-backdrop"
					role="presentation"
					onMouseDown={event => event.target === event.currentTarget && !deleteLoading && setDeleteTarget(null)}
				>
					<div className="modal" role="dialog" aria-modal="true" aria-labelledby="delete-project-title">
						<div className="modal-heading">
							<div>
								<span className="section-overline">删除作品</span>
								<h2 id="delete-project-title">确认永久删除</h2>
							</div>
							<button
								className="icon-button"
								aria-label="关闭"
						...
						<p className="confirm-text">
							即将删除《{deleteTarget.title}》及其全部章节、正文、编辑历史和关联素材。此操作不可恢复。
						</p>
						<div className="modal-actions">
							<button
								type="button"
							...
							>
								{deleteLoading ? <LoaderCircle size={16} className="spin" /> : <Trash2 size={16} />}
	...
	return (
		<>
			<div className="page inner-page">
				<div className="page-heading">
					<div>
						<span className="section-overline">创作知识库</span>
						<h1>素材与灵感</h1>
						<p>集中管理人物、设定、剧情锚点与随手灵感。</p>
					</div>
				...
				<div className="library-toolbar material-toolbar">
					<div className="library-search">
						...
						<input
							...
							value={query}
							onChange={event => setQuery(event.target.value)}
							placeholder="搜索标题、正文、目录或标签…"
							aria-label="搜索素材"
						/>
					</div>
					<select
						className="material-project-filter"
						value={projectFilter}
					...
					<button
						className={`material-pin-filter ${pinnedOnly ? "selected" : ""}`}
				...
				<div className="material-filter-row">
					<div className="filter-tabs">
						{labels.map(label => (
							<button
								key={label}
								className={labelFilter === label ? "selected" : ""}
								onClick={() => setLabelFilter(label)}
							>
								{label}
								<span>
									{label === "全部" ? ideas.length : ideas.filter(idea => idea.label === label).length}
								</span>
							</button>
						))}
					</div>
					<span className="tiny-meta">
						{filtered.length} / {ideas.length} 条素材
					</span>
				</div>

				{filtered.length ? (
					<div className="idea-grid material-grid">
						{filtered.map(idea => {
							const project = projects.find(item => item.id === idea.projectId);
							return (
								<button
									className={`idea-card material-card ${idea.color} ${idea.pinned ? "pinned" : ""}`}
									key={idea.id}
									onClick={() => setEditTarget(idea)}
								>
									<div className="idea-card-top">
										<span>
											{idea.label} · {idea.folder || "未分类"}
										</span>
										{idea.pinned ? <Pin size={15} /> : <MoreHorizontal size={16} />}
									</div>
									<h3>{idea.title}</h3>
									<p>{idea.body}</p>
									{(idea.tags || []).length > 0 && (
									...
									<div className="idea-card-foot">
										<span>
											{project?.title || "全局素材"} ·{" "}
											{formatRelativeTime(idea.updatedAt || idea.createdAt, "刚刚")}
										</span>
					...
					<div className="empty-state">
						...
						<h2>{ideas.length ? "没有匹配的素材" : "素材库还是空的"}</h2>
						<p>
							{ideas.length ? "调整关键词或筛选条件再试试。" : "先记录一个人物、一条设定或一句突然出现的台词。"}
						</p>
						{!ideas.length && (
							<button className="primary-button" onClick={() => setCreateOpen(true)}>
								<Plus size={17} />
			...
			{deleteTarget && (
				...
				>
					<div className="modal" role="dialog" aria-modal="true">
						<div className="modal-heading">
							<div>
								<span className="section-overline">删除素材</span>
								<h2>确认删除</h2>
							</div>
							<button className="icon-button" aria-label="关闭" onClick={() => setDeleteTarget(null)}>
								<X size={18} />
						...
						<p className="confirm-text">确定删除《{deleteTarget.title}》吗？</p>
						<div className="modal-actions">
							<button type="button" className="secondary-button" onClick={() => setDeleteTarget(null)}>
								取消
							</button>
...
function IdeaModal({ idea, projects, onClose, onSubmit, onDelete }) {
	...
	return (
		<div
			...
			onMouseDown={event => event.target === event.currentTarget && onClose()}
		>
			<div className="modal material-modal" role="dialog" aria-modal="true">
				<div className="modal-heading">
					<div>
						<span className="section-overline">{idea ? "编辑创作素材" : "收集创作素材"}</span>
						<h2>{idea ? "编辑素材卡" : "新素材卡"}</h2>
					</div>
				...
				<form onSubmit={submit}>
					...
					<label>
						...
						<textarea
							...
							placeholder="人物小传、世界观规则、剧情片段或灵感正文…"
							rows={7}
							maxLength={10000}
						/>
					</label>
					...
					<label>
						...
						<input
							...
							placeholder="逗号分隔，例如：女主，秘密，第一卷"
						/>
					</label>
					<label className="material-pin-toggle">
...
function MaterialDetailModal({ item, onClose, onInsert, onOpenChapter, onDelete }) {
	...
	return (
		<div
			className="modal-backdrop material-detail-backdrop"
			role="presentation"
			onMouseDown={event => event.target === event.currentTarget && onClose()}
		>
			<div
				className="modal material-detail-modal"
				role="dialog"
				aria-modal="true"
			...
			>
				<div className="modal-heading">
					<div>
						<span className="section-overline">
							{item.label || "资料"} · {item.folder || "未分类"}
						</span>
						<h2 id="material-detail-title">{item.title}</h2>
						{sourcePath && (
							<p className="material-detail-path">
								<FileText size={13} />
								{sourcePath}
							</p>
						)}
					</div>
					<button type="button" className="icon-button" aria-label="关闭资料查看器" onClick={onClose}>
						<X size={18} />
					</button>
				</div>
				...
				<div className="modal-actions">
					{sourcePath && onDelete && (
						<button type="button" className="secondary-button danger-button" onClick={() => onDelete(item)}>
							<Trash2 size={15} />
							删除文件
						</button>
					)}
					...
					{onOpenChapter && (
						<button type="button" className="secondary-button" onClick={onOpenChapter}>
							<BookOpen size={15} />
							打开章节
						</button>
					)}
					<button type="button" className="dark-button" onClick={() => onInsert(item)}>
						<PenLine size={15} />
						插入正文
					</button>
				</div>
...
function MaterialPicker({ ideas, projectId, onClose, onInsert }) {
	const [query, setQuery] = useState("");
	const normalizedQuery = query.trim().toLowerCase();
	const relevant = sortIdeas(
		ideas.filter(
			idea =>
				(!idea.projectId || idea.projectId === projectId) &&
				(!normalizedQuery ||
					[idea.title, idea.body, idea.label, idea.folder, ...(idea.tags || [])]
						.join(" ")
						.toLowerCase()
	...
	return (
		<div
			className="modal-backdrop material-picker-backdrop"
			role="presentation"
			onMouseDown={event => event.target === event.currentTarget && onClose()}
		>
			<div
			...
			>
				<div className="modal-heading">
					<div>
						<span className="section-overline">写作辅助</span>
						<h2 id="material-picker-title">插入素材</h2>
						<p className="material-picker-subtitle">显示当前作品与全局素材，点击后插入光标位置。</p>
					</div>
					<button className="icon-button" aria-label="关闭" onClick={onClose}>
						<X size={18} />
					</button>
				...
				<div className="material-picker-search">
					...
					<input
						...
						value={query}
						onChange={event => setQuery(event.target.value)}
						placeholder="搜索人物、设定、台词或标签…"
					/>
				</div>
				<div className="material-picker-list">
					{relevant.length ? (
						relevant.map(idea => (
							<button key={idea.id} className="material-picker-item" onClick={() => onInsert(idea)}>
								<span className={`material-picker-mark ${idea.color}`}>
									{idea.pinned ? <Pin size={14} /> : <Sparkles size={14} />}
								</span>
								<span className="material-picker-copy">
									<strong>{idea.title}</strong>
									<small>
										{idea.label} · {idea.folder || "未分类"}
									</small>
						...
						<div className="empty-state small">
							<Library size={20} />
							<p>{ideas.length ? "没有匹配的素材。" : "素材库为空，先去“素材与灵感”新增内容。"}</p>
						</div>
					)}
				</div>
			</div>
...
function SkillMarket({ user, onNotify, onSkillsChanged }) {
	...
	const filtered = items.filter(item => {
		const haystack = [item.name, item.description, item.category, item.author?.name, ...(item.tags || [])]
			.join(" ")
			.toLowerCase();
		return (
			(!normalizedQuery || haystack.includes(normalizedQuery)) &&
	...
	async function uninstallSkill(item) {
		...
		try {
			...
			setItems(current =>
				current.map(entry =>
					entry.id === item.id
						? { ...entry, installed: false, installCount: Math.max(0, Number(entry.installCount || 0) - 1) }
						: entry,
				),
			);
			await onSkillsChanged?.({ notifyResult: false });
	...
	return (
		<>
			<div className="page inner-page skill-market-page">
				<div className="page-heading">
					<div>
						<span className="section-overline">COMMUNITY SKILLS</span>
						<h1>技能市场</h1>
						<p>发现其他作者分享的创作工作流，也可以发布自己的 Skill。</p>
					</div>
				...
				<div className="skill-market-toolbar">
					...
					<select value={category} onChange={event => setCategory(event.target.value)} aria-label="按技能分类筛选">
					...
					<button className={mineOnly ? "selected" : ""} onClick={() => setMineOnly(value => !value)}>
						<UserRound size={14} />
						只看我的
					</button>
				</div>
				...
				{loading ? (
					<div className="skill-market-loading">
						<LoaderCircle size={22} className="spin" />
						<span>正在读取社区 Skill</span>
					</div>
				) : filtered.length ? (
					<div className="skill-market-grid">
						{filtered.map(item => (
							<article className={`market-skill-card ${item.isListed ? "" : "pending"}`} key={item.id}>
								...
								<footer>
									<span>
										<Download size={13} />
										{formatNumber(item.installCount)} 次导入
									</span>
									{item.isOwner && (
										<button type="button" className="market-delete" onClick={() => setDeleteTarget(item)}>
											下架
										</button>
									)}
									{!item.isListed && item.isOwner && (
										<button
											type="button"
											className="market-download"
											disabled={reviewingId === item.id}
											onClick={() => reviewAgain(item)}
										>
											{reviewingId === item.id ? (
											...
											{reviewingId === item.id ? "审查中" : "重新审查"}
										</button>
									)}
									{item.isListed && item.installed && (
										<button
											type="button"
											className="market-installed"
											disabled={installingId === item.id}
											onClick={() => uninstallSkill(item)}
										>
											...
											{installingId === item.id ? "处理中" : "已导入 · 移除"}
										</button>
									)}
									{item.isListed && !item.installed && (
										<button
					...
					<div className="empty-state skill-market-empty">
						<Package size={27} />
						<h2>{items.length ? "没有匹配的 Skill" : "技能市场还没有内容"}</h2>
						<p>{items.length ? "调整搜索词或筛选条件再试试。" : "成为第一个分享创作 Skill 的作者。"}</p>
						<button className="primary-button" onClick={() => setUploadOpen(true)}>
							<UploadCloud size={16} />
							上传第一个 Skill
						</button>
			...
			{uploadOpen && (
				<SkillUploadModal
					...
					onUploaded={item => {
						...
						onNotify(
							item.isListed
								? `安全审查通过，已上架 ${item.name}`
								: `${item.name} 已保存，等待专用模型审查后上架`,
						);
					}}
				/>
			)}
			{deleteTarget && (
				<div
					className="modal-backdrop"
					role="presentation"
					onMouseDown={event => event.target === event.currentTarget && !deleting && setDeleteTarget(null)}
				>
					<div className="modal" role="dialog" aria-modal="true">
						<div className="modal-heading">
							<div>
								<span className="section-overline">下架技能</span>
								<h2>下架《{deleteTarget.name}》？</h2>
							</div>
							<button
								className="icon-button"
								disabled={deleting}
						...
						<p className="confirm-text">下架后其他用户将无法继续下载，已下载到本地的文件不会被删除。</p>
						<div className="modal-actions">
							<button className="secondary-button" disabled={deleting} onClick={() => setDeleteTarget(null)}>
								取消
							</button>
							<button className="dark-button danger-button" disabled={deleting} onClick={deleteSkill}>
...
function SkillUploadModal({ categories, reviewConfig, user, onClose, onUploaded }) {
	...
	return (
		<div
			className="modal-backdrop skill-upload-backdrop"
			role="presentation"
			onMouseDown={event => event.target === event.currentTarget && !uploading && onClose()}
		>
			<div className="modal skill-upload-modal" role="dialog" aria-modal="true" aria-labelledby="skill-upload-title">
				<div className="modal-heading">
					<div>
						<span className="section-overline">发布到社区</span>
						<h2 id="skill-upload-title">上传 Skill</h2>
						<p className="skill-upload-author">发布者：{user?.name}</p>
					</div>
					<button className="icon-button" disabled={uploading} aria-label="关闭" onClick={onClose}>
						<X size={18} />
					</button>
				</div>
				<form onSubmit={submit}>
					...
					<label className={`skill-file-picker ${file ? "selected" : ""}`}>
						...
						<UploadCloud size={24} />
						<span>
							<strong>{file ? file.name : "选择 Skill 文件"}</strong>
							<small>
								{file ? `${formatFileSize(file.size)} · 点击可更换` : "支持 Markdown 或 ZIP，最大 2 MB"}
							</small>
						</span>
					</label>
					<div className="skill-upload-safety">
						...
						<span>
							{reviewConfig.configured
								...
								: "当前开发环境未配置专用审查模型，将只进行基础归档与秘密信息检查；生产环境会安全地阻止未审查发布。"}
						</span>
					</div>
					{error && <p className="skill-upload-error">{error}</p>}
					<div className="modal-actions">
						<button type="button" className="secondary-button" disabled={uploading} onClick={onClose}>
							取消
						</button>
						<button type="submit" className="dark-button" disabled={uploading}>
							{uploading ? <LoaderCircle size={15} className="spin" /> : <UploadCloud size={15} />}
							{uploading ? "上传中" : "发布 Skill"}
						</button>
					</div>
				</form>
			</div>
...
function Deconstruct({ onNotify, onRunSkill }) {
	...
	return (
		<div className="page inner-page">
			<div className="page-heading">
				<div>
					<span className="section-overline">结构工作室</span>
					<h1>拆文台</h1>
					<p>把读过的故事，变成下一本书的养分。</p>
				</div>
			...
			<div className="empty-state">
				...
				<h2>拆解一本参考书</h2>
				<p>粘贴一本你合法持有的小说正文，AI 会拆解黄金三章、人设架构、爽点设计与节奏控制。</p>
				<button className="primary-button" onClick={() => setImportOpen(true)}>
					<FolderOpen size={17} />
					导入参考书
				</button>
			...
			{importOpen && (
				<div
					className="modal-backdrop"
					role="presentation"
					onMouseDown={e => e.target === e.currentTarget && setImportOpen(false)}
				>
					<div className="modal deconstruct-modal" role="dialog" aria-modal="true">
						<div className="modal-heading">
							<div>
								<span className="section-overline">导入参考书</span>
								<h2>拆解一本小说</h2>
							</div>
							<button className="icon-button" aria-label="关闭" onClick={() => setImportOpen(false)}>
								<X size={18} />
							</button>
						</div>
						<form onSubmit={runAnalyze}>
							...
							<label>
								...
								<textarea
									...
									placeholder="粘贴小说正文（至少黄金三章）…"
									rows={8}
								/>
							</label>
							<div className="modal-note">
								<Info size={16} />
								<span>请确保你合法持有该作品的使用权。拆文仅用于学习与文学批评。</span>
							</div>
							<div className="modal-actions">
								<button type="button" className="secondary-button" onClick={() => setImportOpen(false)}>
									取消
								</button>
								<button type="submit" className="dark-button" disabled={loading}>
...
const skillCardMeta = {
	"story-long-scan": {
		label: "长篇题材趋势",
		desc: "基于模型知识分析长篇热门题材与市场方向，不获取实时榜单",
		tone: "coral",
		icon: "Target",
	},
	"story-short-scan": {
		label: "短篇题材趋势",
		desc: "基于模型知识分析短篇选题与市场方向，不获取实时榜单",
		tone: "coral",
		icon: "Target",
	},
	"story-deslop": {
		label: "自然化润色",
		desc: "检查并改写套话与常见 AI 句式痕迹",
		tone: "teal",
		icon: "WandSparkles",
	},
	"story-review": {
		label: "章节诊断",
		desc: "结构化审查章节并输出评分与建议报告，不直接修改正文",
		tone: "teal",
		icon: "BrainCircuit",
	},
};
	...
	return (
		<>
			<div className="page inner-page">
				<div className="page-heading">
					<div>
						<span className="section-overline">低频专项</span>
						<h1>高级工具</h1>
						<p>写作请走{ASSISTANT_NAME}与编辑器；这里只放题材趋势、自然化润色和章节诊断。</p>
					</div>
					<div className="page-heading-actions">
						...
						<button className="text-button" onClick={() => setGuideOpen(true)}>
							<CircleHelp size={15} />
							使用指南
						</button>
					</div>
				...
				<div className="toolkit-redirect-card">
					<div>
						<span className="section-overline">主路径</span>
						<h2>想开书或继续写？</h2>
						<p>建书方案用{ASSISTANT_NAME}，正文续写和润色在编辑器里更顺手。</p>
					</div>
					<div className="toolkit-redirect-actions">
						...
						<button className="secondary-button" onClick={() => onNavigate("works")}>
							<BookOpen size={16} />
							我的作品
						</button>
						<button className="secondary-button" onClick={() => onNavigate("deconstruct")}>
							<BookOpenCheck size={16} />
							拆文台
						</button>
					</div>
				</div>
				<div className="toolkit-status-bar">
					<span className={`skill-status ${readyCount ? "ready" : ""}`}>{readyCount} 可调用</span>
					<span className="skill-status needs_model">{needsModelCount} 需模型</span>
					<span className="tiny-meta">未配置模型时部分功能不可用，点击「模型设置」配置 LLM API</span>
				</div>
				{toolkitSkillGroups.map(group => {
					const groupSkills = group.skills.map(name => skillMap[name]).filter(Boolean);
					if (!groupSkills.length) return null;
					return (
						<section className="toolkit-group" key={group.label}>
							<div className="toolkit-group-label">{group.label}</div>
							<div className="skill-card-grid">
								{groupSkills.map(skill => {
									const card = skillCardMeta[skill.name] || {
										label: skill.name,
									...
									return (
										<button
											className={`skill-card ${card.tone} ${canCall ? "" : "disabled"}`}
											key={skill.name}
											disabled={!canCall}
											onClick={() => canCall && onRunSkill(skill.name)}
										>
											<div className="skill-card-top">
												<span className={`skill-card-icon ${card.tone}`}>
													<Icon size={22} />
												</span>
												<span className={`skill-status ${skill.status}`}>
													{skill.status === "ready"
														? "可调用"
														: skill.status === "needs_model"
															? "需模型"
											...
											<div className="skill-card-foot">
												...
												{skill.version && <span className="tiny-meta">v{skill.version}</span>}
				...
				<div className="tool-footer">
					<Sparkles size={17} />
					<span>叙事工坊 0.1 · 本地工作区</span>
					<button className="text-button" disabled={skillsLoading} onClick={() => onRefreshSkills()}>
						{skillsLoading ? <LoaderCircle size={14} className="spin" /> : null}
						{skillsLoading ? "刷新中" : "刷新状态"} {!skillsLoading && <ArrowUpRight size={14} />}
					</button>
				</div>
			</div>
...
function GuideModal({ onClose }) {
	return (
		<div className="modal-backdrop" role="presentation" onMouseDown={e => e.target === e.currentTarget && onClose()}>
			<div className="modal guide-modal" role="dialog" aria-modal="true">
				<div className="modal-heading">
					<div>
						<span className="section-overline">使用指南</span>
						<h2>叙事工坊怎么用</h2>
					</div>
					<button className="icon-button" aria-label="关闭" onClick={onClose}>
						<X size={18} />
				...
				<div className="guide-content">
					...
					<p>
						优先打开「{ASSISTANT_NAME}
						」，先描述想法，再确认篇幅、题材和设定，生成建书方案。也可以在「我的作品」手动新建。
					</p>
					<h3>章节写作</h3>
					<p>
						进入编辑器后，续写、自然化润色、章节诊断和素材插入都在当前章节上下文中完成，正文会自动保存。诊断类
						Skill 输出报告，不会直接修改正文。
					</p>
					<h3>素材库</h3>
					<p>集中管理人物、设定、剧情锚点与灵感卡，可关联到具体作品。</p>
					<h3>高级工具</h3>
					<p>
						题材趋势分析、自然化润色和章节诊断放在这里；趋势分析基于模型知识，不获取实时榜单。拆文台用于分析你合法持有并主动提供的参考正文。
					</p>
					<h3>设置</h3>
					<p>在左下角「设置」中配置 OpenAI 兼容的 API Base URL、Key 和模型名，所有 AI 调用会使用此配置。</p>
				</div>
				<div className="modal-actions">
					<button type="button" className="dark-button" onClick={onClose}>
						知道了
					</button>
				</div>
			</div>
			...
			<div className="review-dialog" role="dialog" aria-modal="true" aria-labelledby="review-report-title">
				<header className="review-report-header">
					<div>
						<span className="section-overline">
							{normalized.skill}
							{normalized.skillVersion ? ` · v${normalized.skillVersion}` : " · YeMu Agent Runtime"}
						</span>
						<h2 id="review-report-title">章节审稿报告</h2>
					</div>
					<button className="icon-button" aria-label="关闭审稿报告" title="关闭" onClick={onClose}>
						<X size={19} />
					</button>
				</header>
				<div className="review-verdict">
					<div className={`verdict-mark ${normalized.verdict.toLowerCase()}`}>
						<span>{normalized.score ?? "—"}</span>
						<small>评分</small>
					</div>
					<div>
						<span className="review-verdict-label">
							{verdictLabels[normalized.verdict] || normalized.verdict}
						</span>
						<p>{normalized.summary}</p>
					</div>
					<div className="severity-summary">
						{["S1", "S2", "S3", "S4"].map(severity => (
							<span key={severity} className={`severity-pill ${severity.toLowerCase()}`}>
				...
				<dl className="review-metadata">
					<div>
						<dt>请求模式</dt>
						<dd>{normalized.requestedMode}</dd>
					</div>
					<div>
						<dt>执行模式</dt>
						<dd>{normalized.effectiveMode}</dd>
					</div>
					<div>
						<dt>降级路径</dt>
						<dd>{normalized.fallback}</dd>
					</div>
					<div>
						<dt>平台规则</dt>
						<dd>{normalized.rubric}</dd>
					</div>
					<div>
						<dt>规则来源</dt>
						<dd>{normalized.rubricSource}</dd>
					</div>
				</dl>
				<section className="review-findings">
					<div className="review-section-heading">
						<h3>问题清单</h3>
						<span>{normalized.findings.length} 项</span>
					</div>
					{normalized.findings.length ? (
						normalized.findings.map((finding, index) => (
							<article className="review-finding" key={`${finding.location}-${index}`}>
						...
						<AgentMarkdown value={normalized.output} />
					) : (
						<div className="review-empty">
							<Check size={18} />
							<span>本轮审查未返回问题项</span>
						</div>
					)}
				</section>
			</div>
...
function SearchModal({ projects, chapters, ideas, onClose, onOpenProject, onSelectChapter, onNavigate }) {
	...
	const projectResults = q
		? projects.filter(p => (p.title + p.genre + p.type + p.status).toLowerCase().includes(q))
		: [];
				...
				<div className="search-results">
					...
					{projectResults.length > 0 && (
						<div className="search-group">
							<span className="search-group-label">作品</span>
					...
					{chapterResults.length > 0 && (
						<div className="search-group">
							<span className="search-group-label">当前作品章节</span>
							{chapterResults.map(c => (
								<button
									key={c.id}
									className="search-result-item"
									onClick={() => {
										void onSelectChapter?.(c);
										onNavigate("editor");
										onClose();
									}}
								>
					...
					{ideaResults.length > 0 && (
						<div className="search-group">
							<span className="search-group-label">灵感</span>
							{ideaResults.map(i => (
								<button
									key={i.id}
									className="search-result-item"
								...
								>
									...
									<span className="search-result-meta">{i.label}</span>
								</button>
							))}
						</div>
					)}
...
function ImportProjectModal({ loading, onClose, onImport }) {
	...
	return (
		<div
			className="modal-backdrop import-project-backdrop"
			role="presentation"
			onMouseDown={event => event.target === event.currentTarget && !loading && onClose()}
		>
			<div
				className="modal import-project-modal"
				role="dialog"
				aria-modal="true"
			...
			>
				<div className="modal-heading">
					<div>
						<span className="section-overline">本地 TXT</span>
						<h2 id="import-project-title">导入本地文稿</h2>
						<p className="modal-subtitle">
							浏览器会在本地读取 TXT，并按“第X章、序章、番外、Chapter X”等标题规则拆分章节；此流程不调用 AI 或
							story-import Skill。
						</p>
					</div>
					<button className="icon-button" aria-label="关闭导入" disabled={loading} onClick={onClose}>
						<X size={18} />
					</button>
				</div>
				<form onSubmit={submit}>
					<label className="import-file-picker">
						<input type="file" accept=".txt,text/plain" disabled={loading} onChange={chooseFile} />
						<span>
							<FolderOpen size={18} />
							<strong>{fileName || "选择 TXT 文件"}</strong>
							<small>也可以在下方直接粘贴全文</small>
						</span>
					</label>
					<div className="form-row">
						<label>
					...
					{content && (
						<div className="import-detection">
							<div>
								<BookOpen size={16} />
								<span>
									识别到 <strong>{chapters.length}</strong> 章
								</span>
							</div>
							<div>
								<FileText size={16} />
								<span>
									约 <strong>{formatNumber(totalWords)}</strong> 字
								</span>
							</div>
						</div>
					)}
					{chapters.length > 0 && (
						<div className="import-preview">
							{chapters.slice(0, 6).map((chapter, index) => (
								<span key={`${chapter.title}-${index}`}>
									<b>{String(index + 1).padStart(2, "0")}</b>
									{chapter.title}
									<small>{formatNumber(chapter.content.replace(/\s/g, "").length)} 字</small>
								</span>
							))}
							{chapters.length > 6 && <em>还有 {chapters.length - 6} 章…</em>}
						</div>
					)}
					{error && (
						<div className="skill-runner-validation" role="alert">
					...
					<div className="modal-actions">
						...
						<button type="submit" className="dark-button" disabled={loading || !content.trim()}>
...
function SmartCreateModal({ proposal, loading, onClose, onCreate }) {
	...
	function submit(event) {
		...
		const normalized = {
			...
			chapters: form.chapters
				.map(chapter => ({ title: chapter.title.trim(), content: chapter.content.trim() }))
				.filter(chapter => chapter.title && chapter.content),
		};
	...
	return (
		<div
			className="modal-backdrop smart-create-backdrop"
			role="presentation"
			onMouseDown={event => event.target === event.currentTarget && !loading && onClose()}
		>
			<div className="modal smart-create-modal" role="dialog" aria-modal="true" aria-labelledby="smart-create-title">
				<div className="modal-heading">
					<div>
						<span className="section-overline">AI 方案校验</span>
						<h2 id="smart-create-title">创建智能作品</h2>
						<p className="modal-subtitle">确认书名、主线和章节大纲后，将一次性生成可编辑的作品。</p>
					</div>
					<button className="icon-button" aria-label="关闭智能创建" disabled={loading} onClick={onClose}>
						<X size={18} />
					</button>
				</div>
				<form onSubmit={submit}>
					...
					<div className="form-row">
						<label>
							题材
							<input value={form.genre} maxLength={30} onChange={event => update("genre", event.target.value)} />
						</label>
						<label>
							流派 / 核心爽点
							<input
					...
					<div className="smart-outline-heading">
						<span>初始章节</span>
						<button
							type="button"
							className="secondary-button"
							disabled={loading || form.chapters.length >= 100}
							onClick={() =>
								setForm(current => ({
									...current,
									chapters: [
										...current.chapters,
										{ title: `第 ${current.chapters.length + 1} 章`, content: "" },
									],
								}))
							}
						>
					...
					<div className="smart-outline-list">
						{form.chapters.map((chapter, index) => (
							<div className="smart-outline-item" key={index}>
								<span className="smart-outline-index">{String(index + 1).padStart(2, "0")}</span>
								<div>
									<input
								...
								<button
									...
									title="删除章节"
									disabled={loading || form.chapters.length <= 1}
					...
					<div className="modal-actions">
						...
						<button type="submit" className="dark-button" disabled={loading}>
							{loading ? <LoaderCircle size={16} className="spin" /> : <BookPlus size={16} />}
							{loading ? "创建中" : "创建并开始写作"}
						</button>
					</div>
				</form>
	...
	return (
		...
		>
			<div className="modal" role="dialog" aria-modal="true" aria-labelledby="new-project-title">
				<div className="modal-heading">
					<div>
						<span className="section-overline">开始一个新故事</span>
						<h2 id="new-project-title">新建作品</h2>
					</div>
					<button className="icon-button" aria-label="关闭" title="关闭" onClick={onClose}>
						<X size={18} />
					</button>
				</div>
				<form onSubmit={onCreate}>
					<label>
						作品名
						<input name="title" autoFocus placeholder="例如：潮汐之上" />
					</label>
					<div className="form-row">
						<label>
							篇幅
							<select name="type" defaultValue="长篇">
								<option>长篇</option>
								<option>短篇</option>
								<option>参考书</option>
							</select>
						...
						<label>
							题材
							<select name="genre" defaultValue="现代言情">
								<ProjectGenreOptions />
							</select>
						</label>
					</div>
					<label>
						流派 / 核心爽点
						<input name="style" maxLength={80} placeholder="例如：重生复仇、甜宠拉扯" />
					</label>
					<div className="modal-note">
						<Sparkles size={16} />
						<span>创建后，你可以先写一句话故事核，其他设定随时补充。</span>
					</div>
					<div className="modal-actions">
						<button type="button" className="secondary-button" onClick={onClose}>
							取消
...
function ProjectGenreOptions({ value = "" }) {
	return (
		<>
			...
			{PROJECT_GENRE_GROUPS.map(group => (
	...
	return (
		<div className="modal-backdrop" role="presentation" onMouseDown={e => e.target === e.currentTarget && onClose()}>
			<div className="modal" role="dialog" aria-modal="true">
				<div className="modal-heading">
					<div>
						<span className="section-overline">编辑作品</span>
						<h2>作品设置</h2>
					</div>
					<button className="icon-button" aria-label="关闭" onClick={onClose}>
						<X size={18} />
				...
				<form onSubmit={submit}>
					<label>
						作品名
						<input value={form.title} onChange={e => update("title", e.target.value)} maxLength={80} />
					</label>
					<div className="form-row">
						<label>
							篇幅
						...
						<label>
							...
							<select value={form.genre} onChange={e => update("genre", e.target.value)}>
								<ProjectGenreOptions value={form.genre} />
							</select>
						</label>
					</div>
					<label>
...
function SkillRunnerModal({
	...
	const contextPayload = context
		...
		: {};
	const payload = needsContent
		? { ...contextPayload, content: useDraft ? draft : customContent }
		: context
			? { ...contextPayload, content: draft }
			: contextPayload;
	...
	return (
		<div
			className="modal-backdrop skill-runner-backdrop"
			role="presentation"
			onMouseDown={event => event.target === event.currentTarget && onClose()}
		>
			<div className="skill-runner-dialog" role="dialog" aria-modal="true" aria-labelledby="skill-runner-title">
				<header className="skill-runner-header">
					<div>
						<span className="section-overline">
							{meta.label} · {skillInfo?.executor || "skill"}
						</span>
				...
				<form className="skill-runner-form" onSubmit={handleRun}>
					...
					{needsContent && (
						<div className="skill-runner-content-toggle">
							<button
								type="button"
								className={useDraft ? "active" : ""}
								disabled={!hasDraft}
								onClick={() => setUseDraft(true)}
							>
								{hasDraft ? "使用当前正文" : "当前正文为空"}
							</button>
							<button type="button" className={!useDraft ? "active" : ""} onClick={() => setUseDraft(false)}>
								粘贴正文
							</button>
							{!useDraft && (
								<textarea
									...
									placeholder="粘贴需要处理的正文…"
									rows={4}
									className="skill-runner-content-input"
								/>
							)}
						</div>
					)}
					{context?.selectedText && (
						<div className="skill-runner-rewrite-modes" aria-label="局部重写长度">
							<span>重写方式</span>
							{[
								["similar", "相近长度"],
								["expand", "扩写细节"],
								...
								<button
									type="button"
									key={value}
									className={rewriteMode === value ? "active" : ""}
									onClick={() => setRewriteMode(value)}
								>
									{label}
								</button>
							))}
					...
					{context?.selectedText && (
						<div className="skill-runner-selection">
							<Highlighter size={14} />
							<span>已关联选中内容 · {formatNumber(context.selectedText.length)} 字</span>
						</div>
					)}
					{validationError && (
						<div className="skill-runner-validation" role="alert">
				...
				{result && (
					<SkillResultPanel
						...
						canApplyToDraft={canApplyToDraft}
						hasSelection={Boolean(context?.selectedText)}
						onApplyOutput={onApplyOutput}
						smartCreateMode={smartCreateMode}
						onUseSmartResult={onUseSmartResult}
					/>
				)}
			</div>
		</div>
...
function SkillResultPanel({
	...
	const statusLabel =
		{ completed: "完成", needs_model: "需模型", needs_input: "需输入", needs_adapter: "需适配", failed: "失败" }[
			result.status
		] || result.status;
	const isCompleted = result.status === "completed";
	const proposal = r.edit_proposal || null;
	const outputText =
		typeof (proposal?.revised_text ?? r.output) === "string"
			? (proposal?.revised_text ?? r.output)
			: r.output
	...
	function setAllHunks(accepted) {
		setHunks(current => current.map(hunk => (hunk.type === "equal" ? hunk : { ...hunk, accepted })));
	}
	...
	return (
		<section className="skill-result-panel">
			...
			{result.status === "needs_model" && (
				<div className="skill-result-notice">
					<Info size={15} />
					<p>{r.message || "执行该 Skill 需要配置 OPENAI_API_KEY。"}</p>
				</div>
			)}
			{result.status === "needs_input" && (
				<div className="skill-result-notice">
					<Info size={15} />
					<p>{r.message || "请补充所需输入。"}</p>
				</div>
			)}
			{result.status === "needs_adapter" && (
				<div className="skill-result-notice">
					<Info size={15} />
					<p>{r.message || "该 Skill 已注册，但当前应用还没有可执行它的适配器。"}</p>
				</div>
			)}
			{result.status === "failed" && (
				<div className="skill-result-notice skill-result-error">
					<Info size={15} />
					<p>{r.message || "执行失败。"}</p>
				</div>
			)}
			...
			{r.references_loaded && r.references_loaded.length > 0 && (
				<div className="skill-result-refs">
					...
					<div className="skill-result-refs-list">
						{r.references_loaded.map(ref => (
							<span key={ref} className="skill-result-ref-chip">
								{ref}
							</span>
						))}
					</div>
			...
			{checks.length > 0 && (
				<div className="skill-result-checks">
					...
					<div className="skill-result-checks-list">
						{checks.slice(0, 8).map((check, index) => (
							<div key={index} className="skill-result-check">
								{check.severity && (
									<span className={`severity-pill ${check.severity.toLowerCase()}`}>{check.severity}</span>
								)}
			...
			{outputText && (
				<div className="skill-result-output">
					<div className="skill-result-output-heading">
						<span className="skill-result-output-label">
							{proposal ? "可审阅修改建议" : isCompleted ? "输出" : "执行详情"}
						</span>
						{isCompleted && (
							<div className="skill-result-output-actions">
								<button type="button" onClick={copyOutput}>
									{copied ? <Check size={14} /> : <Copy size={14} />}
									{copied ? "已复制" : "复制结果"}
								</button>
								{proposal && changedHunks.length > 0 && (
									<>
										<button type="button" onClick={() => setAllHunks(true)}>
											全部接受
										</button>
										<button type="button" onClick={() => setAllHunks(false)}>
											全部拒绝
										</button>
									</>
								)}
								{smartCreateMode && onUseSmartResult && (
									<button
										type="button"
										className="smart-result-button"
										onClick={() => onUseSmartResult(result)}
									>
										<BookPlus size={14} />
										使用方案创建
									</button>
								)}
								{canApplyToDraft && onApplyOutput && (
									<>
										<button
											type="button"
											onClick={() =>
												onApplyOutput(reviewedText, "insert", {
										...
										>
											<PenLine size={14} />
											{hasSelection ? "应用已接受修改" : "插入正文"}
										</button>
										<button
											type="button"
											className="replace-output"
											onClick={() =>
												onApplyOutput(reviewedText, "replace", {
													taskId: result.taskId,
													summary: proposal?.summary || "",
												})
											}
					...
					{proposal && proposal.summary && (
						<p className="edit-proposal-summary">
							<Info size={14} />
							{proposal.summary}。以下内容尚未修改正文。
						</p>
					)}
					{proposal && changedHunks.length > 0 ? (
						<div className="edit-hunk-list">
							{changedHunks.map((hunk, index) => (
								<article className={`edit-hunk ${hunk.accepted ? "accepted" : "rejected"}`} key={hunk.id}>
									<header>
										<strong>修改 {index + 1}</strong>
										<span>{hunk.type === "insert" ? "新增" : hunk.type === "delete" ? "删除" : "替换"}</span>
										<div>
											<button
												className={hunk.accepted ? "active" : ""}
												onClick={() => toggleHunk(hunk.id, true)}
											>
												<Check size={13} />
												接受
											</button>
											<button
												className={!hunk.accepted ? "active" : ""}
												onClick={() => toggleHunk(hunk.id, false)}
											>
												<X size={13} />
												拒绝
											</button>
									...
									{hunk.original && (
										<div className="edit-hunk-copy original">
											<span>原文</span>
											<pre>{hunk.original}</pre>
										</div>
									)}
									{hunk.replacement && (
										<div className="edit-hunk-copy replacement">
											<span>建议</span>
											<pre>{hunk.replacement}</pre>
										</div>
									)}
									<p>
										<Info size={13} />
						...
						<div className="skill-result-comparison">
							<div>
								<span>原文选区</span>
			...
			{findings.length > 0 && (
				<div className="skill-result-findings">
					...
					{findings.slice(0, 10).map((finding, index) => (
						<div key={index} className="skill-result-finding">
							<span className={`severity-pill ${finding.severity.toLowerCase()}`}>{finding.severity}</span>
							<strong>{finding.issue}</strong>
...
function SettingsModal({ onClose, onNotify }) {
	...
	const [form, setForm] = useState({
		...
		model: "",
		reasoningEffort: "",
		thinkingBudgets: { ...TUI_THINKING_BUDGETS },
		temperature: 0.7,
		contextWindow: 100000,
		compaction: { ...TUI_COMPACTION_DEFAULTS },
	});
	...
	async function handleSave(event) {
		...
		try {
			...
			window.dispatchEvent(
				new CustomEvent("story:model-settings-updated", {
	...
	function updateField(field, value) {
		setForm(current => {
			...
			const reasoningEffort =
				configured && configured !== "off" && capabilities && !supported.has(configured)
					...
					<form className="settings-form" onSubmit={handleSave}>
						<div className="settings-layout">
							<nav className="settings-nav" aria-label="设置分类">
								<button
									type="button"
									className={activePane === "connection" ? "active" : ""}
									onClick={() => setActivePane("connection")}
								>
									<Globe size={16} />
									<span>
										<strong>连接与模型</strong>
										<small>服务商、地址和密钥</small>
									</span>
								</button>
								<button
									type="button"
									className={activePane === "generation" ? "active" : ""}
									onClick={() => setActivePane("generation")}
								>
									<BrainCircuit size={16} />
									<span>
										<strong>生成参数</strong>
										<small>思考强度与上下文</small>
									</span>
								</button>
								<button
									type="button"
									className={activePane === "context" ? "active" : ""}
									onClick={() => setActivePane("context")}
								>
									<Archive size={16} />
									<span>
										<strong>上下文维护</strong>
										<small>阈值、保留窗口与压缩</small>
									</span>
								</button>
								...
								{activePane === "connection" ? (
									<>
										<section className="settings-section">
											...
											<div className="settings-provider-picker">
												<button
													...
													onClick={() => updateField("provider", "openai")}
												>
													<Bot size={17} />
													<span>
														<strong>OpenAI 兼容</strong>
														<small>OpenAI、代理及兼容服务</small>
													</span>
													<Check size={15} />
												</button>
												<button
													type="button"
													className={form.provider === "anthropic" ? "active" : ""}
													onClick={() => updateField("provider", "anthropic")}
												>
													<Sparkles size={17} />
													<span>
														<strong>Anthropic</strong>
														<small>Claude 官方或兼容服务</small>
													</span>
													<Check size={15} />
												</button>
											</div>
										...
										<section className="settings-section settings-connection-fields">
											...
											<label className="settings-field">
												...
												<small>
													{settings?.apiKeyMask
														? `当前为 ${settings.apiKeyMask}，留空保持不变。`
														: "保存后会加密存储，不会回传明文。"}
												</small>
											</label>
											...
											<div className="settings-model-row">
												...
												<button
													type="button"
													className={`settings-fetch-models ${connectionStatus === "connected" ? "success" : ""}`}
									...
									<>
										<section className="settings-section">
													...
													{THINKING_BUDGET_LEVELS.map(level => (
														<label className="settings-field" key={level}>
															...
															<input
																...
																max="128000"
																step="1"
																value={form.thinkingBudgets[level]}
																onChange={event =>
																	updateThinkingBudget(level, Number(event.target.value))
																}
															/>
														</label>
													))}
												</div>
												<small className="settings-thinking-budget-note">
													精确预算仅对 Anthropic、Gemini 等 token-budget 模型生效；DeepSeek / OpenAI 等
													effort 模型由厂商按强度控制。
												</small>
											</div>
											...
											<label className="settings-field">
												<span>
													采样温度 <strong>{Number(form.temperature).toFixed(1)}</strong>
												</span>
												<input
													type="range"
													min="0"
										...
										<section className="settings-section">
											...
											<div className="settings-number-row">
												<label className="settings-field">
													...
													<input
														type="text"
														value={compactTokenCount(modelCapabilities?.maxTokens || 16384)}
														readOnly
													/>
													<small>
														自动读取模型目录；思考预算只占其中一部分，不再用 Web 的旧 16K 默认值覆盖。
													</small>
												</label>
												<label className="settings-field">
									...
									<>
										<section className="settings-section">
											<div className="settings-section-heading">
												<div>
													...
													<p>使用与 TUI 相同的触发顺序：固定 Tokens → 百分比 → Reserve。</p>
												</div>
												<strong className="settings-threshold-preview">
													{compactTokenCount(compactionThreshold)} threshold
												</strong>
											</div>
											<div className="settings-provider-picker">
												<button
													type="button"
													className={
														form.compaction.enabled && form.compaction.strategy !== "off" ? "active" : ""
													}
													onClick={() =>
														setForm(current => ({
															...current,
												...
												>
													...
													<span>
														...
														<small>Context-full · 保持当前会话</small>
													</span>
													<Check size={15} />
												</button>
												<button
													...
													onClick={() =>
														setForm(current => ({
															...current,
															compaction: { ...current.compaction, enabled: false, strategy: "off" },
														}))
													}
												>
													<X size={17} />
													<span>
														<strong>Off</strong>
														<small>仅保留手动 /compact</small>
													</span>
													<Check size={15} />
												</button>
											</div>
											<small className="settings-context-note">
												Web 工作区采用 TUI 的 Context-full 滚动摘要。TUI 的 Snapcompact
												位图归档依赖图像回读传输，当前 Web 模型通道不伪装支持；可用{" "}
												<code>/compact soft</code> 手动压缩。
											</small>
										</section>
										...
										<section className="settings-section">
											<div className="settings-section-heading">
												<div>
													<h3>触发阈值</h3>
													<p>设为 -1 表示 Default；固定 token 值优先于百分比。</p>
												</div>
											</div>
											<div className="settings-number-row">
												<label className="settings-field">
													...
													<input
														...
														max="1000000"
														step="1"
														value={form.compaction.thresholdTokens}
														onChange={event =>
															updateCompaction("thresholdTokens", Number(event.target.value))
														}
													/>
												...
												<label className="settings-field">
													...
													<input
														...
														max="99"
														step="1"
														value={form.compaction.thresholdPercent}
														onChange={event =>
															updateCompaction("thresholdPercent", Number(event.target.value))
														}
													/>
											...
											<div className="settings-number-row">
												...
												<label className="settings-field">
													...
													<input
														...
														onChange={event =>
															updateCompaction("keepRecentTokens", Number(event.target.value))
														}
													/>
						...
						<div className="settings-actions">
							<div className="settings-save-note">
								<LockKeyhole size={14} />
								<span>更改会应用到所有 Story Skills</span>
							</div>
							<button type="button" className="secondary-button" onClick={onClose}>
								取消
							</button>
