...
const SECTION_LABEL_RE =
	/^(核心设定|一句话梗概|主角设定|世界观骨架|后宫配置|核心冲突|副本世界规划|副本设计|卷纲|细纲|基本信息|力量体系|剧情单元|章节定位)(.*)$/;
...
export default function AgentMarkdown({ value, streaming = false }) {
	const source = normalizeModelOutput(String(value || "")).replace(/^\s*<\/?proposed_plan>\s*$/gm, "");
						...
						<thead>
							<tr>
								{headers.map((cell, cellIndex) => (
									<th key={`h:${cellIndex}`}>{inlineMarkdown(cell, `th:${cellIndex}`)}</th>
								))}
							</tr>
						</thead>
						<tbody>
							{rows.map((row, rowIndex) => (
								<tr key={`r:${rowIndex}`}>
									{headers.map((_, cellIndex) => (
										<td key={`c:${cellIndex}`}>
											{inlineMarkdown(row[cellIndex] || "", `td:${rowIndex}:${cellIndex}`)}
										</td>
									))}
								</tr>
							))}
				...
				<blockquote key={`quote:${index}`}>
					{quote.map((item, itemIndex) => (
						<React.Fragment key={itemIndex}>
							{itemIndex > 0 && <br />}
							{inlineMarkdown(item, `quote:${index}:${itemIndex}`)}
						</React.Fragment>
					))}
				</blockquote>,
			);
		...
		blocks.push(
			<p key={`p:${index}`}>
				{paragraph.map((item, itemIndex) => (
					<React.Fragment key={itemIndex}>
						{itemIndex > 0 && <br />}
						{inlineMarkdown(item, `p:${index}:${itemIndex}`)}
					</React.Fragment>
				))}
			</p>,
		);
	...
	return (
		<div className={`agent-markdown ${streaming ? "streaming" : ""}`}>
			{blocks}
